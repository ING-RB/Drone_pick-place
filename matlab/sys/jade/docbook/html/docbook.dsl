...
&version.dsl;   <!-- Version                                               -->
<!-- @CHANGE - handle imports -->
(element rptgen:importpost
 (let ((fileRef (attribute-string (normalize "url"))))
 (if (or
         (string=? (case-fold-down (file-extension fileRef)) "html")
         (string=? (case-fold-down (file-extension fileRef)) "htm")
         (string=? (case-fold-down (file-extension fileRef)) "shtml")
     )
  (make sequence
   (literal " -rG-ImPoRt-BeGiN- ")
   (literal fileRef)
   (literal " -Rg-iMpOrT-eNd- ")
  )
  (make element gi: "A"
	attributes: (list
             (list "HREF" (if (string=? (substring fileRef 0 1) ".")
		      fileRef
		      (string-append "file://" fileRef)
             ))
		     (list "TARGET" "_top"))
	(process-children))
 )
