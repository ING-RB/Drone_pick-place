...
(define (find-displayable-object objlist notlist extlist)
  (let loop ((nl objlist))
	...
	(let* ((objdata  (node-list-filter-by-gi
	       ...
	       <!-- @CHANGE - lower string to force case insensitive filter -->
	       (extension (case-fold-down (file-extension filename)))
	       <!-- /CHANGE -->
...
(define ($mediaobject$)
  (let* ((objects (node-list-filter-by-gi
	 ...
	 (caption (select-elements (children (current-node))
				   (normalize "caption")))
	 (imobjco (select-elements (children (current-node))
