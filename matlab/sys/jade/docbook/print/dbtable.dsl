...
(define ($process-cell$ entry preventry row overhang)
  (let* ((colnum    (cell-column-number entry overhang))
	 ...
	 (lastcolnum (if (> lastcellcolumn 0)
			 ...
			 0))
         ;; @CHANGE - make bold for headers and footers
	 (font-name (cond
	                ((have-ancestor? (normalize "thead") entry) %title-font-family%)
	                ((have-ancestor? (normalize "tfoot") entry) %title-font-family%)
	                (else %body-font-family%)))

	 (weight    (cond
	                ((have-ancestor? (normalize "thead") entry) 'bold)
	                ((have-ancestor? (normalize "tfoot") entry) 'bold)
	                (else 'medium)))
	...
	(if (equal? (gi entry) (normalize "entrytbl"))
	    ...
	    (
        ;; @CHANGE - produce empty space to retain styling
        if (node-list-empty? (children entry))
            (make paragraph
              (literal " "))
	    (make paragraph
