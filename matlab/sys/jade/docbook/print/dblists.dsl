...
(element (entry simplelist) 
  ...
  ;; @change GRA - 19-Nov-2008 - Removed paragraph generation because of introduced line break
  ;;  NOTE* This is not the 'right' fix see g505934
  (process-children))
  ;; @endchange
  ...
  (let ((type (inherited-attribute-string (normalize "type"))))
	...
	(make sequence
	  (if (equal? (child-number) 1)
	      (empty-sosofo)
  ;; @change GRA - 19-Nov-2008 - Added single space to align beginning of simple list elements
  ;;  NOTE* This is not the 'right' fix see g505934
		(make sequence
