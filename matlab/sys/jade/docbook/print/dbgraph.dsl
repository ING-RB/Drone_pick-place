...
(define acceptable-mediaobject-notations
  (list "GIF" "GIF87a" "GIF89a" "BMP" "WMF" "EMF" "TIF" "TIFF"))

(define acceptable-mediaobject-extensions
  (list "gif" "bmp" "wmf" "emf" "tif" "tiff"))
<!-- /CHANGE -->
...
<!-- @CHANGE - support for imageobjectco -->
...
(element imageobject
 ...
 <!-- @CHANGE - we also need to display inlinemediaobject -->
(element imagedata
  (if (or (have-ancestor? (normalize "mediaobject")) 
          (have-ancestor? (normalize "inlinemediaobject")))
      ($img$ (current-node) #t)
      ($img$ (current-node) #f)))

<!-- @CHANGE - remove return carriage at the start of a list
(element textobject
  (make display-group
    (process-children)))
-->
...
<!-- @CHANGE - make captions together with last object which is usually an 
               image
(element caption
  (process-children))
-->
