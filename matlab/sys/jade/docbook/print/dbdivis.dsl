...
;;@CHANGE - Added support to display legal notice on the title page
(define ($titlepage-footer$ nl )
  (make sequence 
		font-weight: 'bold
		(do-legal-notice nl)		))

(define (findchild nl childname)
	(select-elements 
		(children nl) 
		(normalize childname)))

(define (do-legal-notice bookinfonodelist)
    (if (do-single-titlepage? bookinfonodelist)
        (if-first-page 
            (make sequence 
                font-size: (* %footnote-size-factor% %bf-size%)
                line-spacing: 0pi
                (book-titlepage (findchild bookinfonodelist "pubdate") 'verso)
                (book-titlepage (findchild bookinfonodelist "copyright") 'verso)
                (book-titlepage (findchild bookinfonodelist "legalnotice") 'verso)
            )
            (empty-sosofo)
        )
        (empty-sosofo)
...
(define (do-single-titlepage? bookinfonodelist)
    (equal? (attribute-string (normalize "single-page") bookinfonodelist) (normalize "yes"))
)
;;  @CHANGE (end legal notice change)
  ...
  (let* ((bookinfo  (select-elements (children (current-node)) 
    ...
    (make sequence
	  ...
	  (make simple-page-sequence
	    ...
	    input-whitespace-treatment: 'collapse
	    use: default-text-style
;;@CHANGE - Added support for legal notice to be displayed on title page
        center-footer: ($titlepage-footer$ bookinfo)
        bottom-margin: 2pi
        footer-margin: 1pi
	    (book-titlepage nl 'recto)
        (if (do-single-titlepage? bookinfo)
            (if (node-list-empty? (findchild bookinfo "abstract"))
                (empty-sosofo)
                (make display-group
                    break-before: 'page
                    (book-titlepage (findchild bookinfo "abstract") 'verso))
            )
            (make display-group
                break-before: 'page
...
;; @CHANGE (end legal notice change)
	...
	(if (null? gilist)
		...
		(make simple-page-sequence
		  ...
		  ;; @CHANGE - Loop through remaining gilist to generate List of Figures
		  (loop (cdr gilist)))
