...
<!ENTITY mcode.dsl    SYSTEM "mcode.dsl"> <!-- @CHANGE -->
]>
...
(element rptgen:importpost
 (let* ((url (attribute-string (normalize "url")))
        (label (string-replace url "%20" " "))
       )
 (if (or
         (string=? (case-fold-down (file-extension url)) "rtf")
         (string=? (case-fold-down (file-extension url)) "doc")
     )
  (make sequence
