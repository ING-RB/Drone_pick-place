...
;; @CHANGE - add support for underline, strikethrough, and color
;;(element emphasis
  ...
  (if (attribute-string (normalize "role"))
      (let* ((nd (current-node))
        ...
        (cond
          ...
          ((equal? role (normalize "underline"))     ($score-seq$ 'after))
          ((equal? role (normalize "strikethrough")) ($score-seq$ 'through))
          ((equal? role (normalize "italic"))        ($italic-seq$))
          (else ($color-seq$ role))))
      ($italic-seq$)))
;; @ENDCHANGE
