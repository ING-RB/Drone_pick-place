...
test_msgs__msg__WStrings__init(test_msgs__msg__WStrings * msg)
{
  ...
  {
    bool success = rosidl_runtime_c__U16String__assign(&msg->wstring_value_default2, u"HellÃ¶ wÃ¶rld!");
    if (!success) {
      goto abort_init_1;
    }
  }
  ...
  {
    bool success = rosidl_runtime_c__U16String__assign(&msg->wstring_value_default3, u"ãã­ã¼ã¯ã¼ã«ã");
    if (!success) {
      goto abort_init_2;
    }
  }
