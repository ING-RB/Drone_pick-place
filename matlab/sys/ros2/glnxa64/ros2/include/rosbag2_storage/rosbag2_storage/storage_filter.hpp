...
struct StorageFilter
{
  ...
  // Code change for MATLAB: Filter by start time and end time using select API in frontend
  // If enableTimeFilter is true, then time filter will be considered between startTime and endTime
  bool enableTimeFilter = false;

  // messages to whitelist by timeStamp when reading a bag. Only messages within these
  // specified timeStamps will be returned.
