...
#define BOOST_INTRUSIVE_HAS_MEMBER_FUNCTION_CALLABLE_WITH_NS_BEG namespace mwboost {} namespace boost = mwboost; namespace mwboost { namespace container { namespace dtl {
...
struct allocator_traits
{
   ...
   typedef typename allocator_type::value_type value_type;
   ...
   #else
      //pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT(mwboost::container::dtl::, Allocator,
         pointer, value_type*)
            pointer;
      //const_pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(mwboost::container::dtl::, Allocator,
         const_pointer, typename mwboost::intrusive::pointer_traits<pointer>::template
      ...
      //void_pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(mwboost::container::dtl::, Allocator,
         void_pointer, typename mwboost::intrusive::pointer_traits<pointer>::template
            rebind_pointer<void>)
               void_pointer;
      //const_void_pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(mwboost::container::dtl::, Allocator,
         const_void_pointer, typename mwboost::intrusive::pointer_traits<pointer>::template
      ...
      //rebind_alloc & rebind_traits
         ...
         //C++11
         template <typename T> using rebind_alloc  = typename mwboost::intrusive::pointer_rebind<Allocator, T>::type;
         template <typename T> using rebind_traits = allocator_traits< rebind_alloc<T> >;
      #else    // #if !defined(BOOST_NO_CXX11_TEMPLATE_ALIASES)
         //Some workaround for C++03 or C++11 compilers with no template aliases
         template <typename T>
         struct rebind_alloc : mwboost::intrusive::pointer_rebind<Allocator,T>::type
         {
            typedef typename mwboost::intrusive::pointer_rebind<Allocator,T>::type Base;
            #if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
               template <typename... Args>
               rebind_alloc(BOOST_FWD_REF(Args)... args) : Base(mwboost::forward<Args>(args)...) {}
         ...
         struct rebind_traits
            : allocator_traits<typename mwboost::intrusive::pointer_rebind<Allocator, T>::type>
         {};
      ...
      template <class T>
      struct portable_rebind_alloc
      {  typedef typename mwboost::intrusive::pointer_rebind<Allocator, T>::type type;  };
   ...
   {
      const bool value = mwboost::container::dtl::
   ...
   select_on_container_copy_construction(const Allocator &a)
   {
      ...
      {
         static const bool value = ::mwboost::move_detail::and_
            < dtl::is_not_std_allocator<Allocator>
            , mwboost::container::dtl::has_member_function_callable_with_construct
                  < Allocator, T*, Args... >
            >::value;
         dtl::bool_<value> flag;
         allocator_traits::priv_construct(flag, a, p, ::mwboost::forward<Args>(args)...);
      }
   #endif
   ...
   template<class T>
   BOOST_CONTAINER_FORCEINLINE static void priv_construct(dtl::false_type, Allocator &, T *p, const ::mwboost::container::default_init_t&)
