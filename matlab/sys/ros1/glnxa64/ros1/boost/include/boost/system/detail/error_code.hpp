...
public:
    ...
    error_code( int val, const error_category & cat, source_location const * loc ) BOOST_NOEXCEPT:
        d1_(), lc_flags_( ( loc? reinterpret_cast<mwboost::uintptr_t>( loc ): 2 ) | +detail::failed_impl( val, cat ) )
    {
        d1_.val_ = val;
        d1_.cat_ = &cat;
    }
        ...
        d1_(), lc_flags_( 0 )
    {
        *this = ec;
        ...
        {
            lc_flags_ = ( loc? reinterpret_cast<mwboost::uintptr_t>( loc ): 2 ) | ( ec.lc_flags_ & 1 );
        }
    }
        ...
        d1_(), lc_flags_( 0 )
    {
        ...
        {
            *this = mwboost::system::error_code( ec.value(), pc2->original_category() );
        }
        else
    ...
    BOOST_SYSTEM_CONSTEXPR int value() const BOOST_NOEXCEPT
    {
        ...
        else
        {
            ...
            unsigned ch = static_cast<unsigned>( reinterpret_cast<mwboost::uintptr_t>( &ec.category() ) % 2097143 ); // 2^21-9, prime
    ...
    source_location const & location() const BOOST_NOEXCEPT
    {
        ...
        return lc_flags_ >= 4? *reinterpret_cast<source_location const*>( lc_flags_ &~ static_cast<mwboost::uintptr_t>( 1 ) ): loc;
    }
...
// This condition must be the same as the one in error_category_impl.hpp
            ...
            return std::error_code( 0, mwboost::system::system_category() );
...
inline std::size_t hash_value( error_code const & ec )
{
    ...
    mwboost::ulong_long_type id_ = cat.id_;

    if( id_ == 0 )
    {
        id_ = reinterpret_cast<mwboost::uintptr_t>( &cat );
    }

    mwboost::ulong_long_type hv = ( mwboost::ulong_long_type( 0xCBF29CE4 ) << 32 ) + 0x84222325;
    mwboost::ulong_long_type const prime = ( mwboost::ulong_long_type( 0x00000100 ) << 32 ) + 0x000001B3;
