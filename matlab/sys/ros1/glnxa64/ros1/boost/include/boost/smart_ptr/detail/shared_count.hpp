...
public:
    ...
    explicit sp_reference_wrapper( T & t): t_( mwboost::addressof( t ) )
    {
    }
...
{
    mwboost::uintptr_t v = reinterpret_cast<mwboost::uintptr_t>( p );

    // match mwboost::hash<T*>
...
public:
    ...
    template<class Y, class D>
    explicit shared_count( mwboost::movelib::unique_ptr<Y, D> & r ): pi_( 0 )
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        , id_(shared_count_id)
#endif
    {
        ...
        D2 d2( r.get_deleter() );
        pi_ = new sp_counted_impl_pd< typename mwboost::movelib::unique_ptr<Y, D>::pointer, D2 >( r.get(), d2 );
...
#endif
{
    ...
    {
        mwboost::throw_exception( mwboost::bad_weak_ptr() );
    }
}
