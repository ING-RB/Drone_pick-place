...
namespace chrono_detail
{
  ...
  steady_clock::time_point steady_clock::now( system::error_code & ec )
  {
    ...
    mwboost::winapi::LARGE_INTEGER_ pcount;
    if ( (nanosecs_per_tic <= 0.0L)
            || (!mwboost::winapi::QueryPerformanceCounter( &pcount )) )
    {
        mwboost::winapi::DWORD_ cause =
            ((nanosecs_per_tic <= 0.0L)
                    ? mwboost::winapi::ERROR_NOT_SUPPORTED_
                    : mwboost::winapi::GetLastError());
        if (::mwboost::chrono::is_throws(ec)) {
            mwboost::throw_exception(
                    system::system_error(
                            cause,
                            ::mwboost::system::system_category(),
                            "chrono::steady_clock" ));
        }
        else
        {
            ec.assign( cause, ::mwboost::system::system_category() );
