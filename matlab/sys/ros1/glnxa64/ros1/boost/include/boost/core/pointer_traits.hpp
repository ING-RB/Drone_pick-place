...
-> decltype(mwboost::pointer_traits<T>::to_address(v))
{
    return mwboost::pointer_traits<T>::to_address(v);
}
...
{
    return mwboost::detail::ptr_address(v.operator->(), 0);
}
...
{
    return mwboost::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return mwboost::to_address(v.operator->());
}
#endif
