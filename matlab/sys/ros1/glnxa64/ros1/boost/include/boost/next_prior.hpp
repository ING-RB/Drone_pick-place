...
//    const std::list<T>::iterator prev = mwboost::prior(p);
//    const std::list<T>::iterator next = mwboost::next(prev, 2);
...
struct prior_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        ...
        mwboost::iterators::reverse_iterator< T > rx(x);
        mwboost::iterators::advance(rx, n);
        return rx.base();
    }
};
