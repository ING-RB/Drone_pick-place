...
{
    static_assert( sizeof( stdcat_ ) >= sizeof( mwboost::system::detail::std_category ), "sizeof(stdcat_) is not enough for std_category" );

#if defined(BOOST_MSVC) && BOOST_MSVC < 1900
    // no alignof
#else

    static_assert( alignof( decltype(stdcat_align_) ) >= alignof( mwboost::system::detail::std_category ), "alignof(stdcat_) is not enough for std_category" );

#endif

#if !defined(BOOST_SYSTEM_DISABLE_THREADS)
    std::lock_guard<std::mutex> lk( mwboost::system::detail::stdcat_mx_holder<>::mx_ );
#endif

    if( sc_init_.load( std::memory_order_acquire ) == 0 )
    {
        ::new( static_cast<void*>( stdcat_ ) ) mwboost::system::detail::std_category( this, 0 );
...
// This condition must be the same as the one in error_condition.hpp
        ...
        static const mwboost::system::detail::std_category generic_instance( this, 0x1F4D3 );
...
// This condition must be the same as the one in error_code.hpp
        ...
        static const mwboost::system::detail::std_category system_instance( this, 0x1F4D7 );
    ...
    return *static_cast<mwboost::system::detail::std_category const*>( static_cast<void const*>( stdcat_ ) );
}
