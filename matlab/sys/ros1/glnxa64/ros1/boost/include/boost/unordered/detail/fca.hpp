  ...
  namespace unordered {
    namespace detail {
      ...
      template <class ValueType, class VoidPtr> struct node
      {
        ...
        typedef typename mwboost::pointer_traits<VoidPtr>::template rebind_to<
          node>::type node_pointer;

        node_pointer next;
        typename mwboost::aligned_storage<sizeof(value_type),
          mwboost::alignment_of<value_type>::value>::type buf;
      ...
      template <class Bucket> struct bucket_group
      {
        ...
        typedef
          typename mwboost::pointer_traits<bucket_pointer>::template rebind_to<
      ...
      public:
        ...
        typedef typename mwboost::pointer_traits<bucket_pointer>::difference_type
          difference_type;
      ...
      private:
        ...
        void increment() BOOST_NOEXCEPT
        {
          ...
          std::size_t n = std::size_t(mwboost::core::countr_zero(
          ...
          if (n < N) {
            ...
            std::ptrdiff_t x = mwboost::core::countr_zero(pbg->bitmask);
            p = pbg->buckets + x;
          }
        }
      };
      ...
      class grouped_bucket_array
          : mwboost::empty_value<typename mwboost::allocator_rebind<Allocator,
              node<typename mwboost::allocator_value_type<Allocator>::type,
                typename mwboost::allocator_void_pointer<Allocator>::type> >::
                type>
      {
        BOOST_MOVABLE_BUT_NOT_COPYABLE(grouped_bucket_array)

        typedef typename mwboost::allocator_value_type<Allocator>::type
          allocator_value_type;
        typedef
          typename mwboost::allocator_void_pointer<Allocator>::type void_pointer;
        typedef typename mwboost::allocator_difference_type<Allocator>::type
          difference_type;

      public:
        typedef typename mwboost::allocator_rebind<Allocator,
          node<allocator_value_type, void_pointer> >::type node_allocator_type;

        typedef node<allocator_value_type, void_pointer> node_type;
        typedef typename mwboost::allocator_pointer<node_allocator_type>::type
          node_pointer;
        typedef SizePolicy size_policy;

      private:
        typedef typename mwboost::allocator_rebind<Allocator, Bucket>::type
          bucket_allocator_type;
        typedef typename mwboost::allocator_pointer<bucket_allocator_type>::type
          bucket_pointer;
        typedef mwboost::pointer_traits<bucket_pointer> bucket_pointer_traits;

        typedef bucket_group<Bucket> group;
        typedef typename mwboost::allocator_rebind<Allocator, group>::type
          group_allocator_type;
        typedef typename mwboost::allocator_pointer<group_allocator_type>::type
          group_pointer;
        typedef typename mwboost::pointer_traits<group_pointer>
      ...
      public:
              ...
              size_(0), buckets(), groups()
        {
          ...
          buckets = mwboost::allocator_allocate(bucket_alloc, num_buckets);
          BOOST_TRY
          {
            groups = mwboost::allocator_allocate(group_alloc, num_groups);

            bucket_type* pb = mwboost::to_address(buckets);
            for (size_type i = 0; i < num_buckets; ++i) {
              new (pb + i) bucket_type();
            }

            group* pg = mwboost::to_address(groups);
          ...
          BOOST_CATCH(...)
          {
            mwboost::allocator_deallocate(bucket_alloc, buckets, num_buckets);
            BOOST_RETHROW
          }
          BOOST_CATCH_END
          ...
          BOOST_RV_REF(grouped_bucket_array) other) BOOST_NOEXCEPT
        {
          ...
          if (this == mwboost::addressof(other)) {
            return *this;
          }
        ...
        void swap(grouped_bucket_array& other)
        {
          ...
          bool b = mwboost::allocator_propagate_on_container_swap<
            allocator_type>::type::value;
          if (b) {
            mwboost::swap(get_node_allocator(), other.get_node_allocator());
          }
        }
        ...
        span<Bucket> raw()
        {
          ...
          return span<Bucket>(mwboost::to_address(buckets), size_);
        }
        ...
        void append_bucket_group(iterator itb) BOOST_NOEXCEPT
        {
          ...
          if (is_empty_bucket) {
            ...
            std::size_t n =
              static_cast<std::size_t>(mwboost::to_address(pb) - &buckets[0]);
