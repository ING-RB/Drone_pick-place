...
inline mwboost::winapi::UINT_ message_cp_win32()
{
#if defined(BOOST_SYSTEM_USE_UTF8)

    return mwboost::winapi::CP_UTF8_;

#else

    return mwboost::winapi::CP_ACP_;
...
inline char const * system_category_message_win32( int ev, char * buffer, std::size_t len ) BOOST_NOEXCEPT
{
    ...
    mwboost::winapi::UINT_ const code_page = message_cp_win32();

    int r = 0;

#if !defined(BOOST_NO_ANSI_APIS)

    if( code_page == mwboost::winapi::CP_ACP_ )
    {
        using namespace mwboost::winapi;

        DWORD_ retval = mwboost::winapi::FormatMessageA(
...

    {
        ...
        DWORD_ retval = mwboost::winapi::FormatMessageW(
        ...
        if( retval != 0 )
        {
            r = mwboost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, buffer, static_cast<int>( len ), NULL, NULL );
            mwboost::winapi::LocalFree( lpMsgBuf );
...
inline std::string system_category_message_win32( int ev )
{
    ...
    int r = mwboost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, 0, 0, NULL, NULL );
    ...
    r = mwboost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, &buffer[0], r, NULL, NULL );
