...
public:
    ...
    intrusive_ptr( intrusive_ptr<U> const & rhs, typename mwboost::detail::sp_enable_if_convertible<U,T>::type = mwboost::detail::sp_empty() )
...
// Move support
    ...
    intrusive_ptr(intrusive_ptr<U> && rhs, typename mwboost::detail::sp_enable_if_convertible<U,T>::type = mwboost::detail::sp_empty())
...
template<class T> inline bool operator==( intrusive_ptr<T> const & p, mwboost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( mwboost::detail::sp_nullptr_t, intrusive_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
...
template< class T > std::size_t hash_value( mwboost::intrusive_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return mwboost::hash< T* >()( p.get() );
}
...
template<class T> struct hash< ::mwboost::intrusive_ptr<T> >
{
    std::size_t operator()( ::mwboost::intrusive_ptr<T> const & p ) const BOOST_SP_NOEXCEPT
    {
