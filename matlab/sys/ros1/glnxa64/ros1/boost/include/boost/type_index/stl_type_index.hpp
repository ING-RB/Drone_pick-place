...
/// \brief Contains mwboost::typeindex::stl_type_index class.
///
/// mwboost::typeindex::stl_type_index class can be used as a drop-in replacement 
/// for std::type_index.
///
...
/// is defined mwboost::typeindex::ctti is usually used instead of mwboost::typeindex::stl_type_index.
...
inline std::string stl_type_index::pretty_name() const {
    static const char cvr_saver_name[] = "mwboost::typeindex::detail::cvr_saver<";
    ...
    const mwboost::core::scoped_demangled_name demangled_name(data_->name());

    const char* begin = demangled_name.get();
    if (!begin) {
        mwboost::throw_exception(std::runtime_error("Type name demangling failed"));
    }
...
#else
    return mwboost::hash_range(raw_name(), raw_name() + std::strlen(raw_name()));
#endif
}
...
inline stl_type_index stl_type_index::type_id() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME mwboost::remove_reference<T>::type no_ref_t;
    typedef BOOST_DEDUCED_TYPENAME mwboost::remove_cv<no_ref_t>::type no_cvr_prefinal_t;
    ...
    #  if (defined(__EDG_VERSION__) && __EDG_VERSION__ < 245) \
        ...
        typedef BOOST_DEDUCED_TYPENAME mwboost::conditional<
            mwboost::is_signed<no_cvr_prefinal_t>::value,
            mwboost::make_signed<no_cvr_prefinal_t>,
            mwboost::type_identity<no_cvr_prefinal_t>
...
inline stl_type_index stl_type_index::type_id_with_cvr() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME mwboost::conditional<
        mwboost::is_reference<T>::value ||  mwboost::is_const<T>::value || mwboost::is_volatile<T>::value,
        detail::cvr_saver<T>,
        T
    >::type type;
