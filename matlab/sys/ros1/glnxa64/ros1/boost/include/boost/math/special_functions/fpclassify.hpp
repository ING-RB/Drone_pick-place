...
//
// This must not be located in any namespace under mwboost::math
...
#else
inline bool is_nan_helper(__float128 f, const std::true_type&) { return mwboost::math::isnan(static_cast<double>(f)); }
inline bool is_nan_helper(__float128 f, const std::false_type&) { return mwboost::math::isnan(static_cast<double>(f)); }
#endif
#endif
}
...
inline int fpclassify_imp BOOST_NO_MACRO_EXPAND(T t, const generic_tag<true>&)
{
   ...
   if(::mwboost::math_detail::is_nan_helper(t, typename std::is_floating_point<T>::type()))
      return FP_NAN;
#elif defined(isnan)
   if(mwboost::math_detail::is_nan_helper(t, typename std::is_floating_point<T>::type()))
      return FP_NAN;
#elif defined(_MSC_VER) || defined(BOOST_BORLANDC)
   if(::_isnan(mwboost::math::tools::real_cast<double>(t)))
      return FP_NAN;
#endif
...
{
   return mwboost::math::detail::fpclassify_imp(t, generic_tag<true>());
}
#endif
...
{
   return mwboost::math::detail::isfinite_impl(t, generic_tag<true>());
}
#endif
...
{ //!< \brief return true if floating-point type t is finite.
   typedef typename detail::fp_traits<T>::type traits;
   typedef typename traits::method method;
   // typedef typename mwboost::is_floating_point<T>::type fp_tag;
   typedef typename tools::promote_args_permissive<T>::type value_type;
...
{ //!< \brief return true if floating-point type t is finite.
   typedef detail::fp_traits<long double>::type traits;
   typedef traits::method method;
   //typedef mwboost::is_floating_point<long double>::type fp_tag;
   typedef long double value_type;
   return detail::isfinite_impl(static_cast<value_type>(x), method());
}
#endif
...
{
   return mwboost::math::detail::isnormal_impl(t, generic_tag<true>());
}
#endif
...
{
   return mwboost::math::detail::isinf_impl(t, generic_tag<true>());
}
#endif
...
#ifdef isnan
template <> inline bool isnan BOOST_NO_MACRO_EXPAND<float>(float t){ return ::mwboost::math_detail::is_nan_helper(t, std::true_type()); }
template <> inline bool isnan BOOST_NO_MACRO_EXPAND<double>(double t){ return ::mwboost::math_detail::is_nan_helper(t, std::true_type()); }
template <> inline bool isnan BOOST_NO_MACRO_EXPAND<long double>(long double t){ return ::mwboost::math_detail::is_nan_helper(t, std::true_type()); }
