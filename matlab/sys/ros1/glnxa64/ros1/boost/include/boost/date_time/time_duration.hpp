...
namespace date_time {
  ...
  */
  ...
  class BOOST_SYMBOL_VISIBLE time_duration : private
      mwboost::less_than_comparable<T
    , mwboost::equality_comparable<T
    > >
   ...
   */
  template<class base_duration, mwboost::int64_t frac_of_second>
  ...
  private:
    ...
    BOOST_STATIC_CONSTANT(mwboost::int64_t, adjustment_ratio = (traits_type::ticks_per_second >= frac_of_second ? traits_type::ticks_per_second / frac_of_second : frac_of_second / traits_type::ticks_per_second));

  public:
    // The argument (ss) must be an integral type
    template <typename T>
    BOOST_CXX14_CONSTEXPR explicit subsecond_duration(T const& ss,
                                                      typename mwboost::enable_if<mwboost::is_integral<T>,
