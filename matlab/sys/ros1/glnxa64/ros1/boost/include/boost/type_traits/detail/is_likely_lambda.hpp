...
//
namespace mwboost {} namespace boost = mwboost; namespace mwboost{ namespace type_traits_detail{
...
{
  template<typename Q> static mwboost::true_type  test(decltype(&Q::operator())*);
  template<typename>   static mwboost::false_type test(...);
...
struct is_likely_stateless_lambda<
  T,
  typename mwboost::enable_if_<has_one_operator_call<T>::value>::type> :
     mwboost::is_convertible<T, typename equivalent_function_pointer<T>::type
>{};
...
namespace mwboost {} namespace boost = mwboost; namespace mwboost {
   namespace type_traits_detail {
      ...
      template<typename T>
      struct is_likely_stateless_lambda : public mwboost::integral_constant<bool, false> {};
}}
