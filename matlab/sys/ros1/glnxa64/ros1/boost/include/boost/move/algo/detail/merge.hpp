...
class adaptive_xbuf
{
   ...
   void move_assign(RandIt first, size_type n)
   {
      ...
      if(n <= m_size){
         mwboost::move(first, first+rand_diff_t(n), m_ptr);
         size_type sz = m_size;
         while(sz-- != n){
            m_ptr[sz].~T();
         }
         ...
         RandRawIt result = mwboost::move(first, first+rand_diff_t(m_size), m_ptr);
         mwboost::uninitialized_move(first+rand_diff_t(m_size), first+rand_diff_t(n), result);
         m_size = n;
      }
   }
   ...
   void push_back(RandIt first, size_type n)
   {
      ...
      mwboost::uninitialized_move(first, first+n, m_ptr+m_size);
      m_size += n;
   }
   ...
   void insert(iterator pos, RandIt it)
   {
      if(pos == (m_ptr + m_size)){
         ...
         //m_size updated
         mwboost::move_backward(pos, m_ptr+m_size-2, m_ptr+m_size-1);
         *pos = mwboost::move(*it);
      }
   }
   ...
   void initialize_until(size_type const sz, T &t)
   {
      ...
      if(m_size < sz){
         BOOST_TRY
         {
            ::new((void*)&m_ptr[m_size]) T(::mwboost::move(t));
            ++m_size;
            for(; m_size != sz; ++m_size){
               ::new((void*)&m_ptr[m_size]) T(::mwboost::move(m_ptr[m_size-1]));
            }
            t = ::mwboost::move(m_ptr[m_size-1]);
         }
         BOOST_CATCH(...)
         {
            while(m_size)
...
RandIt rotate_gcd(RandIt first, RandIt middle, RandIt last)
{
   ...
   if (middle == ret){
      mwboost::adl_move_swap_ranges(first, middle, middle);
   }
   else{
      ...
      , Buf &xbuf)
{
   if(first != middle && middle != last && comp(*middle, middle[-1])){
      ...
      if(len1 <= len2){
         first = mwboost::movelib::upper_bound(first, middle, *middle, comp);
         xbuf.move_assign(first, size_type(middle-first));
         op_merge_with_right_placed
            (xbuf.data(), xbuf.end(), first, middle, last, comp, op);
      }
      else{
         last = mwboost::movelib::lower_bound(middle, last, middle[-1], comp);
...
void merge_bufferless_ON2(RandIt first, RandIt middle, RandIt last, Compare comp)
{
   if((middle - first) < (last - middle)){
      while(first != middle){
         ...
         middle = mwboost::movelib::lower_bound(middle, last, *first, comp);
      ...
      while(middle != last){
         RandIt p = mwboost::movelib::upper_bound(first, middle, last[-1], comp);
   ...
   , Compare comp)
{
   ...
   while(1) {
      ...
      if (len1 > len2) {
         len11 = len1 / 2;
         first_cut +=  len11;
         second_cut = mwboost::movelib::lower_bound(middle, last, *first_cut, comp);
         len22 = size_type(second_cut - middle);
      }
      else {
         len22 = len2 / 2;
         second_cut += len22;
         first_cut = mwboost::movelib::upper_bound(first, middle, *second_cut, comp);
   ...
   while ( first != last && dest_first != original_r_first ) {
      if (r_first == r_last) {
         for(; dest_first != original_r_first; ++dest_first, ++first){
            ::new((iterator_to_raw_pointer)(dest_first)) value_type(::mwboost::move(*first));
            d.incr();
         }
         d.release();
         InputOutIterator end = ::mwboost::move(first, last, original_r_first);
         BOOST_ASSERT(end == r_last);
         mwboost::ignore_unused(end);
         return;
      }
      else if (comp(*r_first, *first)) {
         ::new((iterator_to_raw_pointer)(dest_first)) value_type(::mwboost::move(*r_first));
         d.incr();
         ++r_first;
      }
      else {
      ...
      typename iter_size<BidirectionalIterator1>::type buffer_size)
{
   if (len1 > len2 && len2 <= buffer_size)
   {
      ...
      {
         BidirectionalIterator2 buffer_end = mwboost::move(middle, last, buffer);
         mwboost::move_backward(first, middle, last);
         return mwboost::move(buffer, buffer_end, first);
      }
      else
         return first;
   }
   else if (len1 <= buffer_size)
   {
      ...
      {
         BidirectionalIterator2 buffer_end = mwboost::move(first, middle, buffer);
         BidirectionalIterator1 ret = mwboost::move(middle, last, first);
         mwboost::move(buffer, buffer_end, ret);
         return ret;
      }
      else
         return last;
                           ...
                           typename  iter_size<BidirectionalIterator>::type uninitialized_len)
{
   ...
   if(uninitialized_len)
   {
      ...
      ::mwboost::movelib::adaptive_xbuf<value_type, RandRawIt> xbuf(uninitialized, uninitialized_len);
