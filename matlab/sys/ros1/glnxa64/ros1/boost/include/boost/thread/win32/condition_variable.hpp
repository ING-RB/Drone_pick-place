...
namespace mwboost {} namespace boost = mwboost; namespace mwboost
{
    namespace detail
    {
        ...
        public:
            ...
            static bool no_waiters(mwboost::intrusive_ptr<basic_cv_list_entry> const& entry)
            {
        ...
        {
            mwboost::mutex internal_mutex;
            long total_count;
            unsigned active_generation_count;

            typedef basic_cv_list_entry list_entry;

            typedef mwboost::intrusive_ptr<list_entry> entry_ptr;
            ...
            {
                mwboost::lock_guard<mwboost::mutex> lk(internal_mutex);
                if(!wake_sem)
                {
            ...
            struct entry_manager
            {
                ...
                entry_manager(entry_ptr&& entry_, mwboost::mutex& mutex_):
                    entry(static_cast< entry_ptr&& >(entry_)), internal_mutex(mutex_)
                {}
#else
                entry_manager(entry_ptr const& entry_, mwboost::mutex& mutex_):
                ...
                {
                  if (entry) {
                    mwboost::lock_guard<mwboost::mutex> internal_lock(internal_mutex);
                    entry->remove_waiter();
                    entry.reset();
                  }
                }
    ...
    public:
...
#if defined BOOST_THREAD_USES_DATETIME
        bool timed_wait(unique_lock<mutex>& m,mwboost::system_time const& abs_time)
        {
        ...
        bool timed_wait(unique_lock<mutex>& m,mwboost::xtime const& abs_time)
        {
        ...
        template<typename predicate_type>
        bool timed_wait(unique_lock<mutex>& m,mwboost::system_time const& abs_time,predicate_type pred)
        {
        ...
        template<typename predicate_type>
        bool timed_wait(unique_lock<mutex>& m,mwboost::xtime const& abs_time,predicate_type pred)
        {
