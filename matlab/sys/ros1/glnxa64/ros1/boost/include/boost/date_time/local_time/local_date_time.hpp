...
namespace local_time {
   ...
   * required to be in the form of a mwboost::shared_ptr<time_zone_base>.
   */
  ...
  class BOOST_SYMBOL_VISIBLE local_date_time_base :  public date_time::base_time<utc_time_,
                                                            mwboost::posix_time::posix_time_system> {
  public:
    ...
    local_date_time_base(date_type d,
      ...
      zone_(tz)
    {
      if(tz != mwboost::shared_ptr<tz_type>() && tz->has_dst()){
        ...
        if(result == invalid_time_label){
          ...
          ss << "time given: " << d << ' ' << td;
          mwboost::throw_exception(time_label_invalid(ss.str()));
        }
          ...
          mwboost::throw_exception(dst_not_valid(ss.str()));
        }
      ...
      zone_(tz)
    {
      ...
      if(result == ambiguous) {
        if(calc_option == EXCEPTION_ON_ERROR){
          std::ostringstream ss;
          ss << "time given: " << d << ' ' << td;
          mwboost::throw_exception(ambiguous_result(ss.str()));
        }
        else{ // NADT on error
    ...
    explicit local_date_time_base(const mwboost::date_time::special_values sv,
                                  mwboost::shared_ptr<tz_type> tz = mwboost::shared_ptr<tz_type>()) :
      date_time::base_time<utc_time_type, time_system_type>(utc_time_type(sv)),
      zone_(tz)
    {}

    //! returns time zone associated with calling instance
    mwboost::shared_ptr<tz_type> zone() const
    {
      return zone_;
    }
    ...
    bool is_dst() const
    {
      if(zone_ != mwboost::shared_ptr<tz_type>() && zone_->has_dst() && !this->is_special()) {
    ...
    {
      if(zone_ != mwboost::shared_ptr<tz_type>()){
    ...
    local_date_time_base local_time_in(mwboost::shared_ptr<tz_type> new_tz,
    ...
    /*bool operator==(const date_time::base_time<mwboost::posix_time::ptime,mwboost::posix_time::posix_time_system>& rhs) const
    { // fails due to rhs.time_ being protected
      return date_time::base_time<mwboost::posix_time::ptime,mwboost::posix_time::posix_time_system>::operator==(rhs);
  ...
  private:
    mwboost::shared_ptr<tz_type> zone_;
    //bool is_dst_;

    /*! Adjust the passed in time to UTC?
     */
    utc_time_type construction_adjustment(utc_time_type t,
                                          mwboost::shared_ptr<tz_type> z,
                                          bool dst_flag)
    {
      if(z != mwboost::shared_ptr<tz_type>()) {
