...
namespace date_time {
 ...
 public:
   ...
   date_type get_date(year_type y) const BOOST_OVERRIDE
   {
     if((day_ == 29) && (month_ == 2) && !(calendar_type::is_leap_year(y))) {
       ...
       mwboost::throw_exception(std::invalid_argument(ss.str()));
     }
 ...
 private:
   ...
   *@code
     typedef mwboost::date_time::first_kday_after<date> firstkdayafter;
  ...
  /*! Date generator that takes a date and finds kday after
   *@code
     typedef mwboost::date_time::first_kday_before<date> firstkdaybefore;
