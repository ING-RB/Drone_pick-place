...
inline void atomic_increment( mwboost::uint_least32_t * pw )
{
    __atomic_fetch_add( pw, 1, __ATOMIC_RELAXED );
}

inline mwboost::uint_least32_t atomic_decrement( mwboost::uint_least32_t * pw )
{
    return __atomic_fetch_sub( pw, 1, __ATOMIC_ACQ_REL );
}

inline mwboost::uint_least32_t atomic_conditional_increment( mwboost::uint_least32_t * pw )
{
    // long r = *pw;
    // if( r != 0 ) ++*pw;
    // return r;

    mwboost::uint_least32_t r = __atomic_load_n( pw, __ATOMIC_RELAXED );
...
inline mwboost::uint_least32_t atomic_load( mwboost::uint_least32_t const * pw )
{
...
private:
    ...
    mwboost::uint_least32_t use_count_;        // #shared
    mwboost::uint_least32_t weak_count_;       // #weak + (#shared != 0)
