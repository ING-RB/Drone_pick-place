    ...
    class waiter_for_any_in_seq
    {
      ...
      struct all_futures_lock
      {
        ...
        all_futures_lock(std::vector<registered_waiter>& waiters) :
          count(waiters.size()), locks(new mwboost::unique_lock<mwboost::mutex>[count])
        {
          for (count_type_portable i = 0; i < count; ++i)
          {
            locks[i] = BOOST_THREAD_MAKE_RV_REF(mwboost::unique_lock<mwboost::mutex>(waiters[i].future_->mutex()));
          }
        }
