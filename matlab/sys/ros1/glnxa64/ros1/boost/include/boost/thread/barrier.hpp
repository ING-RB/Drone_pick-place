...
namespace mwboost {} namespace boost = mwboost; namespace mwboost
{
  namespace thread_detail
  {
    ...
    struct void_functor_barrier_reseter
    {
      ...
      : size_(size), fct_(mwboost::move(funct))
      {}
      template <typename F>
      ...
      //size_(BOOST_THREAD_RV(other).size_), fct_(mwboost::move(BOOST_THREAD_RV(other).fct_))
      {
      }
  ...
  class barrier
  {
    ...
    {
      if (count == 0) mwboost::throw_exception(
  ...
  public:
    ...
    bool wait()
    {
      mwboost::unique_lock < mwboost::mutex > lock(m_mutex);
