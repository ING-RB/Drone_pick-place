...
public:
    ...
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        mwboost::sp_scalar_constructor_hook( px );
#endif
    }
    ...
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        mwboost::sp_scalar_destructor_hook( px );
#endif
        mwboost::checked_delete( px );
    }
...
template<class T> inline bool operator==( scoped_ptr<T> const & p, mwboost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( mwboost::detail::sp_nullptr_t, scoped_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
