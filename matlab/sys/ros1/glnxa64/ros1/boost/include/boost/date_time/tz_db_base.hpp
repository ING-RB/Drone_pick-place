...
namespace mwboost {} namespace boost = mwboost; namespace mwboost {
  namespace date_time {
    ...
    public:
      ...
      typedef mwboost::date_time::dst_adjustment_offsets<time_duration_type> dst_adjustment_offsets;
      ...
      void load_from_file(const std::string& pathspec)
      {
        ...
        if(!ifs){
          mwboost::throw_exception(data_not_accessible(pathspec));
        }
      ...
      bool add_record(const string_type& region, 
                      mwboost::shared_ptr<time_zone_base_type> tz)
      {
      ...
      mwboost::shared_ptr<time_zone_base_type> 
      time_zone_from_region(const string_type& region) const 
      {
        ...
        if(record == m_zone_map.end()){
          return mwboost::shared_ptr<time_zone_base_type>(); //null pointer
        }
    ...
    private:
      typedef std::map<string_type, mwboost::shared_ptr<time_zone_base_type> > map_type;
      map_type m_zone_map;
      ...
      {
        typedef mwboost::char_separator<char_type, std::char_traits<char_type> > char_separator_type;
        typedef mwboost::tokenizer<char_separator_type,
        ...
        if ( std::distance ( tokens.begin(), tokens.end ()) != 3 ) {
          ...
          msg << "Expecting 3 fields, got " 
              ...
              << " fields in line: " << rule;
          mwboost::throw_exception(bad_field_count(msg.str()));
        }
      ...
      bool parse_string(string_type& s)
      {
        ...
        typedef mwboost::token_iterator_generator<mwboost::escaped_list_separator<char_type>, string_type::const_iterator, string_type >::type token_iter_type;

        token_iter_type i = mwboost::make_token_iterator<string_type>(s.begin(), s.end(),mwboost::escaped_list_separator<char_type>());
        ...
        mwboost::shared_ptr<rule_type> rules;
        ...
        if(has_dst){
          ...
          rules = 
            mwboost::shared_ptr<rule_type>(parse_rules(result[START_DATE_RULE],
                                                     result[END_DATE_RULE]));
        }
        string_type id(result[ID]);
        mwboost::shared_ptr<time_zone_base_type> zone(new time_zone_type(names, utc_offset, adjust, rules));
