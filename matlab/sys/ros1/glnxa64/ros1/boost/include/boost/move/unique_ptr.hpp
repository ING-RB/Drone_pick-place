...
struct deleter_types
{
   ...
   typedef ::mwboost::rv<D> &                                       deleter_arg_type2;
   #endif
};
...
class is_rvalue_convertible
{
   ...
   #else
   typedef typename bmupmu::if_c
      < ::mwboost::has_move_emulation_enabled<T>::value && !bmupmu::is_reference<T>::value
      , ::mwboost::rv<T>&
...
class unique_ptr
{
   ...
   BOOST_MOVE_FORCEINLINE unique_ptr(BOOST_MOVE_DOC0PTR(bmupd::nullptr_type), BOOST_MOVE_SEEDOC(deleter_arg_type2) d2) BOOST_NOEXCEPT
      : m_data(pointer(), ::mwboost::move(d2))
   {}
   ...
   BOOST_MOVE_FORCEINLINE unique_ptr(BOOST_RV_REF(unique_ptr) u) BOOST_NOEXCEPT
      : m_data(u.release(), ::mwboost::move_if_not_lvalue_reference<D>(u.get_deleter()))
   {}
   ...
   BOOST_MOVE_FORCEINLINE unique_ptr( BOOST_RV_REF_BEG_IF_CXX11 unique_ptr<U, E> BOOST_RV_REF_END_IF_CXX11 u
      ...
      : m_data(u.release(), ::mwboost::move_if_not_lvalue_reference<E>(u.get_deleter()))
   {
   ...
   unique_ptr& operator=(BOOST_RV_REF(unique_ptr) u) BOOST_NOEXCEPT
   {
      ...
      m_data.deleter() = ::mwboost::move_if_not_lvalue_reference<D>(u.get_deleter());
      return *this;
   }
   ...
   {
      this->reset(u.release());
      m_data.deleter() = ::mwboost::move_if_not_lvalue_reference<E>(u.get_deleter());
      return *this;
   }
   ...
   {
      ::mwboost::adl_move_swap(m_data.m_p, u.m_data.m_p);
      ::mwboost::adl_move_swap(m_data.deleter(), u.m_data.deleter());
   }
};
