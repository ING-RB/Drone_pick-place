...
#ifdef __SUNPRO_CC
   static const T m = mwboost::math::tools::max_value<T>();
   static const T val = log(m);
#else
   static const T val = log(mwboost::math::tools::max_value<T>());
#endif
   return val;
}
...
inline T log_min_value(const std::integral_constant<int, 0>& BOOST_MATH_APPEND_EXPLICIT_TEMPLATE_TYPE(T))
{
   BOOST_MATH_STD_USING
#ifdef __SUNPRO_CC
   static const T m = mwboost::math::tools::min_value<T>();
   static const T val = log(m);
#else
   static const T val = log(mwboost::math::tools::min_value<T>());
#endif
   return val;
}
