...
mwboost
    ...
    exception_detail
        {
        std::string diagnostic_information_impl( mwboost::exception const *, std::exception const *, bool, bool );
        }
    ...
    current_exception_diagnostic_information( bool verbose=true)
        {
        mwboost::exception const * be=current_exception_cast<mwboost::exception const>();
        ...
        if( be || se )
            ...
            return "Dynamic exception type: "+mwboost::core::demangle(p->name());
#endif
        else
...
mwboost
    ...
    exception_detail
        ...
        inline
        std::string
        diagnostic_information_impl( mwboost::exception const * be, std::exception const * se, bool with_what, bool verbose )
            {
            if( !be && !se )
                return "Unknown exception.";
#ifndef BOOST_NO_RTTI
            if( !be )
                be=dynamic_cast<mwboost::exception const *>(se);
            if( !se )
    ...
    diagnostic_information_what( exception const & e, bool verbose=true ) BOOST_NOEXCEPT_OR_NOTHROW
        ...
        try
            ...
            if( char const * di=exception_detail::get_diagnostic_information(e,0) )
                ...
                return "Failed to produce mwboost::diagnostic_information_what()";
#ifndef BOOST_NO_EXCEPTIONS
            }
        catch(
        ... )
