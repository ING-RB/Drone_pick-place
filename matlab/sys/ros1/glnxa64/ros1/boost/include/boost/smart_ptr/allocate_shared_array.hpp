...
struct sp_array_element {
    typedef typename mwboost::remove_cv<typename
        mwboost::remove_extent<T>::type>::type type;
};
...
struct sp_array_alignment {
    enum {
        value = sp_max_size<mwboost::alignment_of<T>::value,
            mwboost::alignment_of<U>::value>::value
    };
};
...
class sp_array_creator {
    ...
    typedef typename mwboost::type_with_alignment<sp_array_alignment<T,
...
private:
    typename mwboost::allocator_rebind<A, type>::type other_;
    std::size_t size_;
};
...
public:
    ...
    sp_array_base(const A& other, type* start, std::size_t size)
        : state_(other, size) {
        mwboost::alloc_construct_n(state_.allocator(),
            mwboost::first_scalar(start),
    ...
    sp_array_base(const A& other, type* start, std::size_t size, const U& list)
        ...
        mwboost::alloc_construct_n(state_.allocator(),
            mwboost::first_scalar(start), state_.size() * count,
            mwboost::first_scalar(&list), count);
    }
    ...
    void dispose() BOOST_SP_NOEXCEPT BOOST_OVERRIDE {
        mwboost::alloc_destroy_n(state_.allocator(),
            mwboost::first_scalar(sp_array_start<type>(this)),
            state_.size() * sp_array_count<type>::value);
    }
...
{
    return mwboost::allocate_shared<T>(mwboost::noinit_adapt(allocator), count);
}
...
{
    return mwboost::allocate_shared<T>(mwboost::noinit_adapt(allocator));
}
