...
private:
    ...
    typedef typename mwboost::remove_cv<T>::type T2;
    typedef typename mwboost::remove_cv<U>::type U2;

public:

    BOOST_STATIC_CONSTANT( bool, value =
        (mwboost::is_same<T2, U2>::value)
        || mwboost::is_void<T2>::value
        || mwboost::is_void<U2>::value
        || (mwboost::is_base_of<T2, U2>::value)
        || (mwboost::is_base_of<U2, T2>::value) );
};
...
public:
    ...
    typedef typename mwboost::conditional<

        mwboost::is_same<T2, U2>::value || mwboost::is_void<T2>::value || mwboost::is_base_of<T2, U2>::value,
        typename mwboost::copy_cv<T, U>::type,
        typename mwboost::copy_cv<U, T>::type
...
public:
    ...
    typedef typename mwboost::copy_cv<typename mwboost::copy_cv<typename composite_pointer_type<T2, U2>::type const, T>::type, U>::type type;
};

//Old compilers like MSVC-7.1 have problems using mwboost::conditional in
