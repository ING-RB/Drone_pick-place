...
namespace mwboost {} namespace boost = mwboost; namespace mwboost {  namespace movelib{

template <class RandomAccessIterator, class Compare>
class heap_sort_helper
{
   typedef typename mwboost::movelib::iter_size<RandomAccessIterator>::type  size_type;
   typedef typename mwboost::movelib::iterator_traits<RandomAccessIterator>::value_type value_type;
   ...
   static void adjust_heap(RandomAccessIterator first, size_type hole_index, size_type const len, value_type &value, Compare comp)
   {
      ...
      while (second_child < len) {
         if (comp(*(first + second_child), *(first + size_type(second_child - 1u))))
            second_child--;
         *(first + hole_index) = mwboost::move(*(first + second_child));
         hole_index = second_child;
         second_child = size_type(2u * (second_child + 1u));
      }
      if (second_child == len) {
         *(first + hole_index) = mwboost::move(*(first + size_type(second_child - 1u)));
      ...
      {  //push_heap-like ending
         ...
         while (hole_index > top_index && comp(*(first + parent), value)) {
            *(first + hole_index) = mwboost::move(*(first + parent));
            hole_index = parent;
            parent = size_type((hole_index - 1u) / 2u);
         }    
         *(first + hole_index) = mwboost::move(value);
      }
   }
   ...
   static void make_heap(RandomAccessIterator first, RandomAccessIterator last, Compare comp)
   {
      ...
      if (len > 1) {
         ...
         do {
            value_type v(mwboost::move(*(first + parent)));
   ...
   static void sort_heap(RandomAccessIterator first, RandomAccessIterator last, Compare comp)
   {
      ...
      while (len > 1) {
         ...
         --last;
         value_type v(mwboost::move(*last));
         *last = mwboost::move(*first);
   ...
   static void sort(RandomAccessIterator first, RandomAccessIterator last, Compare comp)
   {
      ...
      BOOST_ASSERT(mwboost::movelib::is_sorted(first, last, comp));
   }
};
