...
namespace mwboost {} namespace boost = mwboost; namespace mwboost {
    ...
    namespace detail { namespace variant {
        struct variant_hasher: public mwboost::static_visitor<std::size_t> {
            template <class T>
            std::size_t operator()(T const& val) const {
                mwboost::hash<T> hasher;
                return hasher(val);
            }
        };
    }}
    ...
    std::size_t hash_value(variant< BOOST_VARIANT_ENUM_PARAMS(T) > const& val) {
        std::size_t seed = mwboost::apply_visitor(detail::variant::variant_hasher(), val);
