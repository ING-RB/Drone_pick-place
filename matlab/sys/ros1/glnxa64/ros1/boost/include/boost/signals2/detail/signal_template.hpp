...
// signalN::slot_function
      ...
      private:
        ...
        mwboost::shared_ptr<connection> _connection;
      };
      ...
      public:
        ...
        BOOST_SIGNALS2_SIGNAL_IMPL_CLASS_NAME(BOOST_SIGNALS2_NUM_ARGS)(const combiner_type &combiner_arg,
          ...
          _shared_state(mwboost::make_shared<invocation_state>(connection_list_type(group_compare), combiner_arg)),
        ...
        void set_combiner(const combiner_type &combiner_arg)
        {
          ...
          if(_shared_state.unique())
            ...
            _shared_state = mwboost::make_shared<invocation_state>(*_shared_state, combiner_arg);
        }
      private:
...
#define BOOST_SIGNALS2_M_ARG_NAMES(arity) BOOST_PP_ENUM(arity, BOOST_SIGNALS2_M_ARG_NAME, ~)
          ...
          result_type m_invoke(const connection_body_type &connectionBody,
            typename mwboost::enable_if<mwboost::is_void<SlotResultType> >::type * = 0) const
          {
          ...
          template<typename SlotResultType>
          result_type m_invoke(const connection_body_type &connectionBody, 
            typename mwboost::disable_if<mwboost::is_void<SlotResultType> >::type * = 0) const
          {
...
#undef BOOST_SIGNALS2_M_ARG_NAME
        ...
        */
        void nolock_force_unique_connection_list(garbage_collecting_lock<mutex_type> &lock)
        {
          ...
          {
            _shared_state = mwboost::make_shared<invocation_state>(*_shared_state, _shared_state->connection_bodies());
          ...
          const slot_type &slot)
        {
          ...
          return mwboost::make_shared<connection_body<group_key_type, slot_type, Mutex> >(slot, _mutex);
        }
        ...
        const mwboost::shared_ptr<mutex_type> _mutex;
      };
...
#undef BOOST_SIGNALS2_MISC_STATEMENT
    ...
    namespace detail
    {
      ...
      private:
        mwboost::weak_ptr<detail::BOOST_SIGNALS2_SIGNAL_IMPL_CLASS_NAME(BOOST_SIGNALS2_NUM_ARGS)
...
// const mwboost::signals2::connection &,
...
#define BOOST_SIGNALS2_EXT_SIGNATURE(arity, Signature) \
  typename function_traits<Signature>::result_type ( \
  const mwboost::signals2::connection & BOOST_SIGNALS2_PP_COMMA_IF(BOOST_SIGNALS2_NUM_ARGS) \
