...
namespace mwboost {} namespace boost = mwboost; namespace mwboost { namespace parameter { namespace aux {
    ...
    template <typename U>
    ::mwboost::parameter::aux::yes_tag
        is_cv_reference_wrapper_check(
            ::mwboost::reference_wrapper<U> const volatile*
        );

    ::mwboost::parameter::aux::no_tag is_cv_reference_wrapper_check(...);
...
namespace mwboost {} namespace boost = mwboost; namespace mwboost { namespace parameter { namespace aux {
    ...
    template <typename T>
    using is_cv_reference_wrapper = ::mwboost::mp11::mp_bool<
        sizeof(
            ::mwboost::parameter::aux::is_cv_reference_wrapper_check(
                static_cast<
    ...
    // tagged_argument<>.  Can't use mwboost::unwrap_reference<> here
    // because it doesn't handle the case where T = reference_wrapper<U> cv.
    template <typename T>
    using unwrap_cv_reference = ::mwboost::mp11::mp_eval_if<
        ::mwboost::parameter::aux::is_cv_reference_wrapper<T>
      , ::mwboost::parameter::aux::unwrap_cv_reference_impl<T>
    ...
    >
    struct unwrap_cv_reference : ::mwboost::remove_reference<T>
    {
    };

    template <typename T>
    struct unwrap_cv_reference<T const,::mwboost::mpl::false_>
    {
        typedef T const type;
    };

    template <typename T>
    struct unwrap_cv_reference<T,::mwboost::mpl::true_> : T
    {
    };
    ...
    template <typename T>
    struct unwrap_cv_reference_impl : ::mwboost::remove_reference<T>::type
    {
    };
