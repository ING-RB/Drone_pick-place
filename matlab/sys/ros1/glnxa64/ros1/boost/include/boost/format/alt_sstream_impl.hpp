...
// --- Implementation  ------------------------------------------------------//
        ...
        str (const string_type& s) {
            ...
            if(sz != 0 && mode_ & (::std::ios_base::in | ::std::ios_base::out) ) {
                ...
                void *vd_ptr = mwboost::allocator_allocate(alloc_, sz, is_allocated_? eback() : 0);
                Ch *new_ptr = static_cast<Ch *>(vd_ptr);
#else
                Ch *new_ptr = mwboost::allocator_allocate(alloc_, sz, is_allocated_? eback() : 0);
#endif
...
#pragma warning(disable:4996)
            ...
            if(compat_traits_type::eq_int_type(compat_traits_type::eof(), meta))
                ...
                if(0 < add_size) {
                    new_size += add_size;
#ifdef _RWSTD_NO_CLASS_PARTIAL_SPEC
                    void *vdptr = mwboost::allocator_allocate(alloc_, new_size, is_allocated_? oldptr : 0);
                    newptr = static_cast<Ch *>(vdptr);
#else
                    newptr = mwboost::allocator_allocate(alloc_, new_size, is_allocated_? oldptr : 0);
#endif
                }
