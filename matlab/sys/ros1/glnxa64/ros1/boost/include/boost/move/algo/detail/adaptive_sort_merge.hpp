...
BOOST_STATIC_ASSERT((AdaptiveSortInsertionSortThreshold&(AdaptiveSortInsertionSortThreshold-1)) == 0);
   ...
   typedef ::mwboost::uintptr_t uintptr_t;
#else
...
{
   if(begin != with){
      ::mwboost::adl_move_swap_ranges(begin, end, with);
      if(key_next != key_range2)  //Avoid potential self-swapping
         ::mwboost::adl_move_swap(*key_next, *key_range2);
      if(key_next == key_mid){
         key_mid = key_range2;
      }
   ...
   (RandIt first1, RandIt last1, RandIt const last2, bool *const pis_range1_A, Compare comp)
{
   ...
   if(first1 != last1 && comp(*last1, last1[-1])){
      do{
         ...
         last1  = mwboost::movelib::lower_bound(last1, last2, *first1, comp);
   ...
   , Compare comp)
{
   ...
   ::mwboost::ignore_unused(key_count);
   //BOOST_ASSERT(n_block_a || n_block_b);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted_and_unique(key_first, key_first + key_count, key_comp));
   ...
   {  //Selection sort blocks
      ...
      for ( RandIt f = first+l_irreg1; n_block_left; --n_block_left) {
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(f, f+l_block, comp));
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first_min, first_min + l_block, comp));
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first+l_irreg1+n_bef_irreg2*l_block, first_irr2, comp));
   ...
   for(RandItKeys key_next = key_first; key_next != key_end; ++key_next){
      ...
      last1 += l_block;
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first, first1, comp));
   }

   merge_bufferless(is_range1_A ? first1 : last1, first_irr2, last_irr2, comp);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first, last_irr2, comp));
}
   ...
   if(max_collected){
      ...
      if(xbuf.capacity() >= max_collected){
         ...
         while(u != last && h < max_collected){
            typename XBuf::iterator const r = mwboost::movelib::lower_bound(ph0, xbuf.end(), *u, comp);
            //If key not found add it to [h, h+h0)
            if(r == xbuf.end() || comp(*u, *r) ){
               RandIt const new_h0 = mwboost::move(search_end, u, h0);
               search_end = u;
               ++search_end;
               ++h;
               xbuf.insert(r, u);
         ...
         mwboost::move_backward(first, h0, h0+h);
         mwboost::move(xbuf.data(), xbuf.end(), first);
      }
      else{
         while(u != last && h < max_collected){
            RandIt const r = mwboost::movelib::lower_bound(h0, search_end, *u, comp);
            //If key not found add it to [h, h+h0)
...
{
   mwboost::movelib::inplace_stable_sort(first, last, comp);
}
      ...
      , XBuf &xbuf)
{
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first, last, mwboost::movelib::unantistable(comp)));
}
                    ...
                    , XBuf & xbuf)
{
   ...
   BOOST_ASSERT(mwboost::movelib::is_sorted_and_unique(first, last, comp));
}
   ...
   , Compare comp, bool const is_stable, Op op)
{
   ...
   for(; n_block_left; --n_block_left){
      ...
      mwboost::ignore_unused(last_min);

      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first_reg, last_reg, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!next_key_idx || mwboost::movelib::is_sorted(first_min, last_min, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT((!next_key_idx || !comp(*first_reg, *first_min )));

      OutputIt orig_dest = dest;
      mwboost::ignore_unused(orig_dest);
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(orig_dest, dest, comp));

      if(first_reg == dest){
         dest = next_key_idx ? ::mwboost::adl_move_swap_ranges(first_min, last_min, first_reg)
                             : last_reg;
      }
      else{
...
//   BOOST_ASSERT(n_block_a || n_block_b);
   ...
   for (; n_block_left; --n_block_left) {
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first1, last1, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first2, last2, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!n_block_left || mwboost::movelib::is_sorted(first_min, last_min, comp));
      ...
      if(is_range1_A == is_range2_A){
         ...
         mwboost::ignore_unused(unmerged);
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first-l_block, unmerged, comp));
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first-l_block, buffer, comp));
   ...
   , RandItBuf const buf_first)
{
   ...
   for (; n_block_left; --n_block_left) {
      ...
      mwboost::ignore_unused(buffer_empty);
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(buffer_empty ? mwboost::movelib::is_sorted(first1, last1, comp) : mwboost::movelib::is_sorted(buffer, buffer_end, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first2, last2, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!n_block_left || mwboost::movelib::is_sorted(first_min, last_min, comp));
      ...
      if(is_range1_A == is_range2_A){
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first2, last2, comp));
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first_min, last_min, comp));
         first1 = first2;
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first, first1, comp));
      }
      else {
         ...
         first1 = unmerged;
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first, unmerged, comp));
      }
   ...
   RandIt res = op(forward_t(), buffer, buffer_end, first1);
   mwboost::ignore_unused(res);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(first, res, comp));
   BOOST_MOVE_ADAPTIVE_SORT_PRINT_L2("   merge_blocks_w_fwd: ", len);
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(dest, last_irr2, comp));
      ...
      , Op op)
{
   ...
   for(; l_merged < l_build_buf && l_left_space >= l_merged; l_merged = size_type(l_merged*2u)){
      ...
      while((elements_in_blocks - p0) > 2*l_merged) {
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(mwboost::movelib::is_sorted(pos-l_merged, pos+l_merged, comp));
