...
//
    ...
    weak_ptr( weak_ptr<Y> const & r, typename mwboost::detail::sp_enable_if_convertible<Y,T>::type = mwboost::detail::sp_empty() )
    ...
    weak_ptr( weak_ptr<Y> && r, typename mwboost::detail::sp_enable_if_convertible<Y,T>::type = mwboost::detail::sp_empty() )
...
#endif
    BOOST_SP_NOEXCEPT : px( r.lock().get() ), pn( static_cast< mwboost::detail::weak_count && >( r.pn ) )
    {
        mwboost::detail::sp_assert_convertible< Y, T >();
        r.px = 0;
    }
    ...
    weak_ptr( weak_ptr && r )
    BOOST_SP_NOEXCEPT : px( r.px ), pn( static_cast< mwboost::detail::weak_count && >( r.pn ) )
    {
        r.px = 0;
    }
    ...
    weak_ptr( shared_ptr<Y> const & r, typename mwboost::detail::sp_enable_if_convertible<Y,T>::type = mwboost::detail::sp_empty() )
    ...
    {
        return shared_ptr<T>( *this, mwboost::detail::sp_nothrow_tag() );
    }
...
private:
    ...
    mwboost::detail::weak_count pn; // reference counter
...
template< class T > std::size_t hash_value( mwboost::weak_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
...
template<class T> struct hash< ::mwboost::weak_ptr<T> >
{
    std::size_t operator()( ::mwboost::weak_ptr<T> const & p ) const BOOST_SP_NOEXCEPT
    {
        return p.owner_hash_value();
    }
};
...
template<class T> struct equal_to< ::mwboost::weak_ptr<T> >
{
    bool operator()( ::mwboost::weak_ptr<T> const & a, ::mwboost::weak_ptr<T> const & b ) const BOOST_SP_NOEXCEPT
    {
