...
namespace detail
{
    ...
    public:
        ...
        typedef typename mwboost::allocator_size_type<Allocator>::type size_type;
        typedef typename mwboost::allocator_difference_type<Allocator>::type difference_type;
        typedef T*                                       pointer;
        typedef typename mwboost::allocator_pointer<Allocator>::type allocator_pointer;
        typedef const T*                                 const_pointer;
        typedef T&                                       reference;
        typedef const T&                                 const_reference;
        typedef pointer                                  iterator;
        typedef const_pointer                            const_iterator;
        typedef mwboost::reverse_iterator<iterator>        reverse_iterator;
        typedef mwboost::reverse_iterator<const_iterator>  const_reverse_iterator;
        typedef typename mwboost::mpl::if_c< mwboost::has_trivial_assign<T>::value
    ...
    private:
        ...
        {
            copy_rai( begin, end, where, mwboost::has_trivial_assign<T>() );
        }

        static void copy_rai( const T* begin, const T* end,
                              pointer where, const mwboost::true_type& )
        {
        ...
        static void copy_rai( I begin, I end,
                              pointer where, const mwboost::integral_constant<bool, b>& )
        {
        ...
        {
            assign_impl( begin, end, where, mwboost::has_trivial_assign<T>() );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const mwboost::true_type& )
        {
            std::memcpy( where, begin, sizeof(T) * std::distance(begin,end) );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const mwboost::false_type& )
        {
            for( ; begin != end; ++begin, ++where )
                *where = *begin;
        }

        void unchecked_push_back_n( size_type n, const mwboost::true_type& )
        {
            std::uninitialized_fill( end(), end() + n, T() );
            size_ += n;
        }

        void unchecked_push_back_n( size_type n, const mwboost::false_type& )
        {
            for( size_type i = 0u; i < n; ++i )
                unchecked_push_back();
        }

        void auto_buffer_destroy( pointer where, const mwboost::false_type& )
        {
            (*where).~T();
        }

        void auto_buffer_destroy( pointer, const mwboost::true_type& )
        { }

        void auto_buffer_destroy( pointer where )
        {
            auto_buffer_destroy( where, mwboost::has_trivial_destructor<T>() );
        }
        ...
        void auto_buffer_destroy()
        {
            ...
            if( buffer_ ) // do we need this check? Yes, but only
                ...
                auto_buffer_destroy( mwboost::has_trivial_destructor<T>() );
        }

        void destroy_back_n( size_type n, const mwboost::false_type& )
        {
            BOOST_ASSERT( n > 0 );
        ...
        void destroy_back_n( size_type, const mwboost::true_type& )
        { }

        void destroy_back_n( size_type n )
        {
            destroy_back_n( n, mwboost::has_trivial_destructor<T>() );
        }

        void auto_buffer_destroy( const mwboost::false_type& x )
        {
            if( size_ )
        ...
        void auto_buffer_destroy( const mwboost::true_type& )
        {
            deallocate( buffer_, members_.capacity_ );
        }

        pointer move_to_new_buffer( size_type new_capacity, const mwboost::false_type& )
        {
        ...
        pointer move_to_new_buffer( size_type new_capacity, const mwboost::true_type& )
        {
            pointer new_buffer = allocate( new_capacity ); // strong
                                 ...
                                 const mwboost::true_type& )
        {
            ...
            mwboost::swap( l.size_, r.size_ );
            mwboost::swap( l.members_.capacity_, r.members_.capacity_ );
        }

        static void swap_helper( auto_buffer& l, auto_buffer& r,
                                 const mwboost::false_type& )
        {
            BOOST_ASSERT( l.is_on_stack() && r.is_on_stack() );
            ...
            size_type i = 0u;
            for(  ; i < min_size; ++i )
                mwboost::swap( (*smallest)[i], (*largest)[i] );
        ...
        void grow_back( size_type n, const mwboost::true_type& )
        {
            BOOST_ASSERT( size_ + n <= members_.capacity_ );
            size_ += n;
        }

        void grow_back( size_type n, const mwboost::false_type& )
        {
        ...
        {
            grow_back( n, mwboost::has_trivial_constructor<T>() );
        }

        void grow_back_one( const mwboost::true_type& )
        {
            BOOST_ASSERT( size_ + 1 <= members_.capacity_ );
            size_ += 1;
        }

        void grow_back_one( const mwboost::false_type& )
        {
            unchecked_push_back();
        }
        ...
        void grow_back_one()
        {
            grow_back_one( mwboost::has_trivial_constructor<T>() );
        }
    ...
    public:
        ...
        {
            BOOST_ASSERT( size_ + n <= members_.capacity_ );
            unchecked_push_back_n( n, mwboost::has_trivial_assign<T>() );
        }
        ...
        {
            BOOST_ASSERT( !empty() );
            auto_buffer_destroy( buffer_ + size_ - 1, mwboost::has_trivial_destructor<T>() );
            --size_;
        }
        ...
        void swap( auto_buffer& r )
        {
            ...
            if( both_on_heap )
            {
                mwboost::swap( get_allocator(), r.get_allocator() );
                mwboost::swap( members_.capacity_, r.members_.capacity_ );
                mwboost::swap( buffer_, r.buffer_ );
                mwboost::swap( size_, r.size_ );
                BOOST_ASSERT( is_valid() );
            ...
            if( exactly_one_on_stack )
            {
                ...
                mwboost::swap( get_allocator(), r.get_allocator() );  // assume nothrow
                mwboost::swap( members_.capacity_, r.members_.capacity_ );
                mwboost::swap( size_, r.size_ );
            ...
            BOOST_ASSERT( on_stack && r_on_stack );
            swap_helper( *this, r, mwboost::has_trivial_assign<T>() );
            BOOST_ASSERT( is_valid() );
            BOOST_ASSERT( r.is_valid() );
        }

    private:
        typedef mwboost::aligned_storage< N * sizeof(T),
                                        mwboost::alignment_of<T>::value >
                               storage;
