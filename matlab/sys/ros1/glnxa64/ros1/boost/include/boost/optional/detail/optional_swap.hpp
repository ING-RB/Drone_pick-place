...
# define BOOST_OPTIONAL_DETAIL_MOVE(EXPR_) mwboost::move(EXPR_)
#else
...
struct swap_selector<false>
{
    template <class T>
    static void optional_swap ( optional<T>& x, optional<T>& y ) 
    //BOOST_NOEXCEPT_IF(::mwboost::is_nothrow_move_constructible<T>::value && BOOST_NOEXCEPT_EXPR(mwboost::swap(*x, *y)))
    {
        if (x)
        {
            if (y)
...
template<class T>
struct optional_swap_should_use_default_constructor : mwboost::false_type {} ;
