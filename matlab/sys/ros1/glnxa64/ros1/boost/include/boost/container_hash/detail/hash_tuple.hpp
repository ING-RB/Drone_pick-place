...
inline typename mwboost::enable_if_<(I == std::tuple_size<T>::value),
        void>::type
...
{
    mwboost::hash_combine(seed, std::get<I>(v));
    mwboost::hash_detail::hash_combine_tuple<I + 1>(seed, v);
}
...
{
    std::size_t seed = 0;
    mwboost::hash_detail::hash_combine_tuple<0>(seed, v);
    return seed;
}
...
{
    return mwboost::hash_detail::hash_tuple(v);
}
