...
public:
    ...
    const shared_ptr<T>& postconstruct() const
    {
        if(!_postconstructed)
        {
            adl_postconstruct(_sp, const_cast<typename mwboost::remove_const<T>::type *>(_sp.get()));
...
private:
    friend class mwboost::signals2::deconstruct_access;
...
private:
    ...
    void destroy()
    {
        if( initialized_ )
        {
            ...
            using mwboost::signals2::detail::adl_predestruct;
            adl_predestruct(const_cast<typename mwboost::remove_const<T>::type *>(p));
            p->~T();
            initialized_ = false;
        }
    }
...
public:
    ...
    {
        mwboost::shared_ptr< T > pt( static_cast< T* >( 0 ), detail::deconstruct_deleter< T >() );

        detail::deconstruct_deleter< T > * pd = mwboost::get_deleter< detail::deconstruct_deleter< T > >( pt );
        ...
        mwboost::shared_ptr< T > retval( pt, static_cast< T* >( pv ) );
        mwboost::detail::sp_enable_shared_from_this(&retval, retval.get(), retval.get());
        return retval;
