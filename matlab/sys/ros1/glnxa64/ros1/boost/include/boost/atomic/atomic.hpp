...
public:
    ...
    BOOST_STATIC_ASSERT_MSG(sizeof(value_type) > 0u, "mwboost::atomic<T> requires T to be a complete type");
#if !defined(BOOST_ATOMIC_DETAIL_NO_CXX11_IS_TRIVIALLY_COPYABLE)
    BOOST_STATIC_ASSERT_MSG(atomics::detail::is_trivially_copyable< value_type >::value, "mwboost::atomic<T> requires T to be a trivially copyable type");
#endif
...
typedef atomic< mwboost::ulong_long_type > atomic_ullong;
typedef atomic< mwboost::long_long_type > atomic_llong;
#endif
...
typedef atomic< mwboost::intptr_t > atomic_intptr_t;
typedef atomic< mwboost::uintptr_t > atomic_uintptr_t;
#endif
