...
// mwboost::thread::future requires exception handling
// due to mwboost::exception::exception_ptr dependency
...
namespace mwboost {} namespace boost = mwboost; namespace mwboost
{
  ...
  shared_ptr<T const> static_shared_from_this(T const* that)
  {
    ...
    namespace detail
    {
        ...
        struct relocker
        {
            mwboost::unique_lock<mwboost::mutex>& lock_;

            relocker(mwboost::unique_lock<mwboost::mutex>& lk):
                lock_(lk)
            {
                lock_.unlock();
            }
        ...
        {
            typedef std::list<mwboost::condition_variable_any*> waiter_list;
            ...
            mwboost::exception_ptr exception;
            bool done;
            bool is_valid_;
            bool is_deferred_;
            bool is_constructed;
            launch policy_;
            mutable mwboost::mutex mutex;
            mwboost::condition_variable waiters;
            waiter_list external_waiters;
            mwboost::function<void()> callback;
            ...
            void set_executor_policy(executor_ptr_type aex, mwboost::lock_guard<mwboost::mutex>&)
            {
              set_executor();
              ex_ = aex;
            }
            void set_executor_policy(executor_ptr_type aex, mwboost::unique_lock<mwboost::mutex>&)
            {
              set_executor();
              ex_ = aex;
            }

            bool valid(mwboost::unique_lock<mwboost::mutex>&) { return is_valid_; }
            bool valid() {
              mwboost::unique_lock<mwboost::mutex> lk(this->mutex);
              return valid(lk);
            }
            void invalidate(mwboost::unique_lock<mwboost::mutex>&) { is_valid_ = false; }
            void invalidate() {
              mwboost::unique_lock<mwboost::mutex> lk(this->mutex);
              invalidate(lk);
            }
            void validate(mwboost::unique_lock<mwboost::mutex>&) { is_valid_ = true; }
            void validate() {
              mwboost::unique_lock<mwboost::mutex> lk(this->mutex);
              validate(lk);
            }
...
#endif
            notify_when_ready_handle notify_when_ready(mwboost::condition_variable_any& cv)
            {
                mwboost::unique_lock<mwboost::mutex> lock(this->mutex);
                do_callback(lock);
            ...
            {
                mwboost::lock_guard<mwboost::mutex> lock(this->mutex);
...
#define BOOST_THREAD_DO_CONTINUATION
            ...
            void do_continuation(mwboost::unique_lock<mwboost::mutex>& lock)
            {
...
#else
            void do_continuation(mwboost::unique_lock<mwboost::mutex>&)
            {
            }
#endif
...
#define BOOST_THREAD_DO_CONTINUATION \
            ...
            virtual void do_continuation(mwboost::unique_lock<mwboost::mutex>&) = 0;
#endif

#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            virtual void set_continuation_ptr(continuation_ptr_type continuation, mwboost::unique_lock<mwboost::mutex>& lock)
            {
...
#endif
            void mark_finished_internal(mwboost::unique_lock<mwboost::mutex>& lock)
            {
                done=true;
                waiters.notify_all();
            ...
            void do_callback(mwboost::unique_lock<mwboost::mutex>& lock)
            {
                if(callback && !done)
                {
                    mwboost::function<void()> local_callback=callback;
            ...
            void wait_internal(mwboost::unique_lock<mwboost::mutex> &lk, bool rethrow=true)
            {
              do_callback(lk);
              if (is_deferred_)
              {
              ...
              waiters.wait(lk, mwboost::bind(&shared_state_base::is_done, mwboost::ref(*this)));
              if(rethrow && exception)
              {
                  mwboost::rethrow_exception(exception);
              }
            }

            virtual void wait(mwboost::unique_lock<mwboost::mutex>& lock, bool rethrow=true)
            {
            ...
            bool timed_wait(Duration const& rel_time)
            {
                ...
                do_callback(lock);
                return waiters.timed_wait(lock, rel_time, mwboost::bind(&shared_state_base::is_done, mwboost::ref(*this)));
            }

            bool timed_wait_until(mwboost::system_time const& target_time)
            {
                mwboost::unique_lock<mwboost::mutex> lock(this->mutex);
                if (is_deferred_)
                    return false;
                ...
                do_callback(lock);
                return waiters.timed_wait(lock, target_time, mwboost::bind(&shared_state_base::is_done, mwboost::ref(*this)));
            }
#endif
#ifdef BOOST_THREAD_USES_CHRONO
            ...
            wait_until(const chrono::time_point<Clock, Duration>& abs_time)
            {
              ...
              do_callback(lock);
              if(!waiters.wait_until(lock, abs_time, mwboost::bind(&shared_state_base::is_done, mwboost::ref(*this))))
              {
...
#endif
            void mark_exceptional_finish_internal(mwboost::exception_ptr const& e, mwboost::unique_lock<mwboost::mutex>& lock)
            {
                exception=e;
                mark_finished_internal(lock);
            }
            ...
            {
                mwboost::unique_lock<mwboost::mutex> lock(this->mutex);
                mark_exceptional_finish_internal(mwboost::current_exception(), lock);
            }

            void set_exception_deferred(exception_ptr e)
            {
              unique_lock<mwboost::mutex> lk(this->mutex);
              if (has_value(lk))
              {
...
//              this->is_constructed = true;
            ...
            bool has_value(unique_lock<mwboost::mutex>& )  const
            {
                return done && ! exception;
            }
            ...
            launch launch_policy(mwboost::unique_lock<mwboost::mutex>&) const
            {
                return policy_;
            }

            future_state::state get_state(mwboost::unique_lock<mwboost::mutex>&) const
            {
                if(!done)
                {
            ...
            {
                mwboost::lock_guard<mwboost::mutex> guard(this->mutex);
                if(!done)
                {
                    return future_state::waiting;
                }
            ...
            {
                mwboost::lock_guard<mwboost::mutex> lock(this->mutex);
                callback=mwboost::bind(f,mwboost::ref(*u));
            }

            virtual void execute(mwboost::unique_lock<mwboost::mutex>&) {}
        ...
        {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
              typedef mwboost::optional<T> storage_type;
#else
              typedef mwboost::csbl::unique_ptr<T> storage_type;
#endif
            ...
            typedef typename conditional<mwboost::is_fundamental<T>::value,T,T const&>::type source_reference_type;
            typedef BOOST_THREAD_RV_REF(T) rvalue_source_type;
            typedef T move_dest_type;
#else
            typedef T& source_reference_type;
            typedef typename conditional<mwboost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >::value, BOOST_THREAD_RV_REF(T),T const&>::type rvalue_source_type;
            typedef typename conditional<mwboost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >::value, BOOST_THREAD_RV_REF(T),T>::type move_dest_type;
#endif
            ...
            void mark_finished_with_result_internal(source_reference_type result_, mwboost::unique_lock<mwboost::mutex>& lock)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result = result_;
#else
            ...
            void mark_finished_with_result_internal(rvalue_source_type result_, mwboost::unique_lock<mwboost::mutex>& lock)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result = mwboost::move(result_);
#elif ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                result.reset(new T(mwboost::move(result_)));
#else
            ...
            template <class ...Args>
            void mark_finished_with_result_internal(mwboost::unique_lock<mwboost::mutex>& lock, BOOST_THREAD_FWD_REF(Args)... args)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result.emplace(mwboost::forward<Args>(args)...);
#else
                result.reset(new T(mwboost::forward<Args>(args)...));
#endif
                this->mark_finished_internal(lock);
            }
#endif
            ...
            void mark_finished_with_result(rvalue_source_type result_)
            {
                ...
                mark_finished_with_result_internal(mwboost::move(result_), lock);
#else
                mark_finished_with_result_internal(static_cast<rvalue_source_type>(result_), lock);
#endif
            }

            storage_type& get_storage(mwboost::unique_lock<mwboost::mutex>& lk)
            {
                wait_internal(lk);
                return result;
            }
            virtual move_dest_type get(mwboost::unique_lock<mwboost::mutex>& lk)
            {
                return mwboost::move(*get_storage(lk));
            }
            move_dest_type get()
            {
                mwboost::unique_lock<mwboost::mutex> lk(this->mutex);
            ...
            virtual shared_future_get_result_type get_sh(mwboost::unique_lock<mwboost::mutex>& lk)
            {
...
//              this->is_constructed = true;
            ...
            {
                set_value_deferred(mwboost::move(result_));
//              unique_lock<mwboost::mutex> lk(this->mutex);
//              if (this->has_value(lk))
//                  throw_exception(promise_already_satisfied());
//
...
//              this->is_constructed = true;
            ...
            detail::shared_state_base
        {
            ...
            virtual T& get(mwboost::unique_lock<mwboost::mutex>& lock)
            {
                wait_internal(lock);
                return *result;
            }
            ...
            virtual T& get_sh(mwboost::unique_lock<mwboost::mutex>& lock)
            {
                wait_internal(lock);
                return *result;
            }
...
//              this->is_constructed = true;
            ...
            detail::shared_state_base
        {
            ...
            void mark_finished_with_result_internal(mwboost::unique_lock<mwboost::mutex>& lock)
            {
                mark_finished_internal(lock);
            }
            ...
            virtual void get(mwboost::unique_lock<mwboost::mutex>& lock)
            {
            ...
            virtual void get_sh(mwboost::unique_lock<mwboost::mutex>& lock)
            {
                this->wait_internal(lock);
            }
            void get_sh()
...
//              this->is_constructed = true;
        ...
        public:
          ...
          ~future_async_shared_state_base()
          {
            ...
            unique_lock<mwboost::mutex> lk(this->mutex);
            this->waiters.wait(lk, mwboost::bind(&shared_state_base::is_done, mwboost::ref(*this)));
#endif
          }

          virtual void wait(mwboost::unique_lock<mwboost::mutex>& lk, bool rethrow)
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
              {
                relocker rlk(lk);
        ...
        struct future_async_shared_state: future_async_shared_state_base<Rp>
        {
          ...
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
            this->thr_ = mwboost::thread(&future_async_shared_state::run, static_shared_from_this(this), mwboost::forward<Fp>(f));
#else
            mwboost::thread(&future_async_shared_state::run, static_shared_from_this(this), mwboost::forward<Fp>(f)).detach();
#endif
          }
        ...
        struct future_async_shared_state<void, Fp>: public future_async_shared_state_base<void>
        {
          ...
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
            this->thr_ = mwboost::thread(&future_async_shared_state::run, static_shared_from_this(this), mwboost::move(f));
#else
            mwboost::thread(&future_async_shared_state::run, static_shared_from_this(this), mwboost::move(f)).detach();
#endif
          }
        ...
        struct future_deferred_shared_state: shared_state<Rp>
        {
          ...
          : func_(mwboost::move(f))
          {
            this->set_deferred();
          }

          virtual void execute(mwboost::unique_lock<mwboost::mutex>& lck) {
            try
            {
              Fp local_fuct=mwboost::move(func_);
              relocker relock(lck);
              Rp res = local_fuct();
              relock.lock();
              this->mark_finished_with_result_internal(mwboost::move(res), lck);
            }
            catch (...)
            {
        ...
        private:
            ...
            {
                mwboost::shared_ptr<detail::shared_state_base> future_;
                detail::shared_state_base::notify_when_ready_handle handle;
                count_type index;

                registered_waiter(mwboost::shared_ptr<detail::shared_state_base> const& a_future,
            ...
            struct all_futures_lock
            {
                   ...
                   mwboost::scoped_array<mwboost::unique_lock<mwboost::mutex> > locks;

                all_futures_lock(std::vector<registered_waiter>& futures):
                    count(futures.size()),locks(new mwboost::unique_lock<mwboost::mutex>[count])
                {
                    for(count_type_portable i=0;i<count;++i)
                    {
                        locks[i]=BOOST_THREAD_MAKE_RV_REF(mwboost::unique_lock<mwboost::mutex>(futures[i].future_->mutex));
                    }
                }

                void lock()
                {
                    mwboost::lock(locks.get(),locks.get()+count);
                }
            ...
            mwboost::condition_variable_any cv;
...
//    typename mwboost::disable_if<is_future_type<Iterator>,Iterator>::type wait_for_any(Iterator begin,Iterator end)
//    {
//        if(begin==end)
//            return end;
//
...
//            waiter.add(*current);
//        }
//        return mwboost::next(begin,waiter.wait());
//    }

#ifdef BOOST_NO_CXX11_VARIADIC_TEMPLATES
    template<typename F1,typename F2>
    typename mwboost::enable_if<is_future_type<F1>,typename detail::future_waiter::count_type>::type wait_for_any(F1& f1,F2& f2)
    {
        detail::future_waiter waiter;
        waiter.add(f1);
        waiter.add(f2);
...
#else
    template<typename F1, typename... Fs>
    typename mwboost::enable_if<is_future_type<F1>, typename detail::future_waiter::count_type>::type
    ...
    namespace detail
    {
      ...
      public:
        ...
        typedef mwboost::shared_ptr<detail::shared_state<R> > future_ptr;
      ...
      public:
        ...
        state get_state(mwboost::unique_lock<mwboost::mutex>& lk) const
        {
            if(!future_)
            {
        ...
        bool is_ready(mwboost::unique_lock<mwboost::mutex>& lk) const
        {
        ...
        launch launch_policy(mwboost::unique_lock<mwboost::mutex>& lk) const
        {
        ...
        {
          if ( future_ ) {
            mwboost::unique_lock<mwboost::mutex> lk(this->future_->mutex);
        ...
        void wait() const
        {
            if(!future_)
            {
                mwboost::throw_exception(future_uninitialized());
            }
            future_->wait(false);
        }
        ...
        bool timed_wait_until(mwboost::system_time const& abs_time) const
        {
            if(!future_)
            {
                mwboost::throw_exception(future_uninitialized());
    ...
    namespace detail
    {
        ...
        BOOST_THREAD_FUTURE<Rp>
        make_future_async_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_sync_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_deferred_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_deferred_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_async_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_sync_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);


  #ifdef BOOST_THREAD_PROVIDES_EXECUTORS
        template<typename Ex, typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_executor_continuation_shared_state(Ex& ex, mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename Ex, typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_executor_continuation_shared_state(Ex& ex, mwboost::unique_lock<mwboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
        ...
        make_future_unwrap_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f);
#endif
    }
#if defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY)
      template< typename InputIterator>
      typename mwboost::disable_if<is_future_type<InputIterator>,
    ...
    private:
        ...
        detail::make_future_async_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_sync_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_deferred_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_deferred_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_async_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_sync_continuation_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

  #ifdef BOOST_THREAD_PROVIDES_EXECUTORS
        template<typename Ex, typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_executor_continuation_shared_state(Ex& ex, mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename Ex, typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_executor_continuation_shared_state(Ex& ex, mwboost::unique_lock<mwboost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
        ...
        template <class F, class Rp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_unwrap_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f);
#endif
#if defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY)
      template< typename InputIterator>
      friend typename mwboost::disable_if<is_future_type<InputIterator>,
        BOOST_THREAD_FUTURE<csbl::vector<typename InputIterator::value_type>  >
      >::type
      when_all(InputIterator first, InputIterator last);
    ...
    public:
        ...
        base_type(mwboost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))))
        {
        }
#if defined BOOST_THREAD_PROVIDES_FUTURE_UNWRAP
        ...
        {
            this->base_type::operator=(mwboost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }

        shared_future<R> share()
        {
          return shared_future<R>(::mwboost::move(*this));
        }
        ...
        move_dest_type get()
        {
            ...
            unique_lock<mwboost::mutex> lk(this->future_->mutex);
        ...
        typename mwboost::disable_if< is_void<R2>, move_dest_type>::type
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename mwboost::result_of<F(BOOST_THREAD_FUTURE)>::type>
        ...
        template <typename R2>
        inline typename mwboost::disable_if< is_void<R2>, BOOST_THREAD_FUTURE<R> >::type
    ...
    public:
        ...
        {
            base_type::operator=(mwboost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }
        ...
        template <typename R2>
        typename mwboost::disable_if< is_void<R2>, typename detail::shared_state<R>::shared_future_get_result_type>::type
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename mwboost::result_of<F(shared_future)>::type>
...
#include <boost/thread/detail/atomic_redef_macros.hpp>
    ...
    public:
        ...
        promise(mwboost::allocator_arg_t, Allocator a)
        {
        ...
        ~promise()
        {
            if(future_)
            {
                mwboost::unique_lock<mwboost::mutex> lock(future_->mutex);

                if(!future_->done && !future_->is_constructed)
                {
                    future_->mark_exceptional_finish_internal(mwboost::copy_exception(broken_promise()), lock);
                }
            }
        }
        ...
        void set_executor(executor_ptr_type aex)
        {
          ...
          if (future_.get()==0)
          {
              mwboost::throw_exception(promise_moved());
          }
          mwboost::lock_guard<mwboost::mutex> lk(future_->mutex);
        ...
        BOOST_THREAD_FUTURE<R> get_future()
        {
            ...
            if (future_obtained)
            {
                mwboost::throw_exception(future_already_retrieved());
            }
...
#if defined  BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class TR>
        typename mwboost::enable_if_c<is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type
            set_value(TR const &  r)
        {
            ...
            if(future_->done)
            {
                mwboost::throw_exception(promise_already_satisfied());
            }
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            future_->mark_finished_with_result_internal(mwboost::move(r), lock);
#else
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            future_->set_value_deferred(mwboost::move(r));
#else
        ...
        void emplace(BOOST_THREAD_FWD_REF(Args) ...args)
        {
            ...
            future_->mark_finished_with_result_internal(lock, mwboost::forward<Args>(args)...);
        }

#endif

        void set_exception(mwboost::exception_ptr p)
        {
            lazy_init();
            mwboost::unique_lock<mwboost::mutex> lock(future_->mutex);
            if(future_->done)
        ...
        {
          set_exception(mwboost::copy_exception(ex));
        }
        void set_exception_deferred(mwboost::exception_ptr p)
        {
            lazy_init();
            if (future_.get()==0)
            {
        ...
        {
          set_exception_deferred(mwboost::copy_exception(ex));
        }

        // setting the result with deferred notification
#if defined  BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class TR>
        typename mwboost::enable_if_c<is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type set_value_at_thread_exit(TR const& r)
        {
          if (future_.get()==0)
          {
              mwboost::throw_exception(promise_moved());
        ...
        void set_value_at_thread_exit(BOOST_THREAD_RV_REF(R) r)
        {
          ...
          future_->set_value_at_thread_exit(mwboost::move(r));
        }
        void set_exception_at_thread_exit(exception_ptr e)
        {
          if (future_.get()==0)
        ...
        {
          set_exception_at_thread_exit(mwboost::copy_exception(ex));
        }
...
#include <boost/thread/detail/atomic_redef_macros.hpp>
    ...
    class promise<void>
    {
        typedef mwboost::shared_ptr<detail::shared_state<void> > future_ptr;
...
namespace mwboost {} namespace boost = mwboost; namespace mwboost { namespace container {
    ...
    struct uses_allocator< ::mwboost::promise<R> , Alloc> : true_type
    {
    };
}}
#if ! defined  BOOST_NO_CXX11_ALLOCATOR
...
namespace mwboost {} namespace boost = mwboost; namespace mwboost
{
    ...
    namespace detail
    {
      ...
      struct task_base_shared_state:
            ...
            detail::shared_state<R>
        {
            ...
            {
                {
                    mwboost::lock_guard<mwboost::mutex> lk(this->mutex);
                    if(started)
                    {
                        mwboost::throw_exception(task_already_started());
                    }
                    started=true;
                }
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
                do_run(mwboost::move(args)...);
#else
                do_run();
#endif
            }
            ...
            void owner_destroyed()
            {
                ...
                {
                    started=true;
                    this->mark_exceptional_finish_internal(mwboost::copy_exception(mwboost::broken_promise()), lk);
                }
            }
        };
        ...
        public:
            ...
            void do_apply(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->set_value_at_thread_exit(f(mwboost::move(args)...));
                }
#else
            void do_apply()
            {
            ...
            void do_run(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->mark_finished_with_result(f(mwboost::move(args)...));
                }
#else
            void do_run()
            {
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                  R res((f()));
                  this->mark_finished_with_result(mwboost::move(res));
#else
           ...
           task_base_shared_state<R>
            ...
            public:
                ...
                void do_apply()
                {
                    ...
                    {
                        R r((f()));
                        this->set_value_at_thread_exit(mwboost::move(r));
                    }
#endif
                    catch(...)
                    {
    ...
    {
      typedef mwboost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task_ptr;
      mwboost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task;
  #else
    template<typename R>
    class packaged_task<R()>
    {
      typedef mwboost::shared_ptr<detail::task_base_shared_state<R()> > task_ptr;
      mwboost::shared_ptr<detail::task_base_shared_state<R()> > task;
  #endif
#else
    template<typename R>
    class packaged_task
    ...
    public:
        ...
        explicit packaged_task(R(*f)(), BOOST_THREAD_FWD_REF(ArgTypes)... args)
        {
            ...
            task= task_ptr(new task_shared_state_type(f, mwboost::move(args)...));
            future_obtained=false;
        }
  #else
        ...
        explicit packaged_task(BOOST_THREAD_FWD_REF(F) f
            , typename mwboost::disable_if<is_same<typename decay<F>::type, packaged_task>, dummy* >::type=0
            )
        {
          typedef typename decay<F>::type FR;
            ...
            typedef detail::task_shared_state<FR,R> task_shared_state_type;
#endif
            task = task_ptr(new task_shared_state_type(mwboost::forward<F>(f)));
            future_obtained = false;
...
#if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
            typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(mwboost::move(f)));
#else
            typedef detail::task_shared_state<F,R()> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(mwboost::move(f)));
#endif
...
#if defined(BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNCTION_PTR)
        template <class Allocator>
        packaged_task(mwboost::allocator_arg_t, Allocator a, R(*f)())
        {
          typedef R(*FR)();
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
        ...
        packaged_task(mwboost::allocator_arg_t, Allocator a, BOOST_THREAD_FWD_REF(F) f)
        {
          typedef typename decay<F>::type FR;
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(mwboost::forward<F>(f)), D(a2, 1) );
          future_obtained = false;
        }
#else // ! defined BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class F, class Allocator>
        packaged_task(mwboost::allocator_arg_t, Allocator a, const F& f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
        ...
        template <class F, class Allocator>
        packaged_task(mwboost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(mwboost::move(f)), D(a2, 1) );
          future_obtained = false;
        }
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            packaged_task temp(mwboost::move(other));
#else
        ...
        {
          if (!valid())
              mwboost::throw_exception(task_moved());
          mwboost::lock_guard<mwboost::mutex> lk(task->mutex);
          task->set_executor_policy(aex, lk);
        }
#endif
        void reset() {
            if (!valid())
              mwboost::throw_exception(future_error(system::make_error_code(future_errc::no_state)));
        ...
        void make_ready_at_thread_exit() {
          ...
          if (task->has_value()) mwboost::throw_exception(promise_already_satisfied());
          task->apply();
        }
#endif
        template<typename F>
...
namespace mwboost {} namespace boost = mwboost; namespace mwboost { namespace container {
    template <class R, class Alloc>
    struct uses_allocator< ::mwboost::packaged_task<R> , Alloc> : true_type
    {};
}}
#if ! defined  BOOST_NO_CXX11_ALLOCATOR
namespace std {
...
namespace detail
{
  ...
  make_future_deferred_shared_state(BOOST_THREAD_FWD_REF(Fp) f) {
    shared_ptr<future_deferred_shared_state<Rp, Fp> >
        h(new future_deferred_shared_state<Rp, Fp>(mwboost::forward<Fp>(f)));
  ...
  async(launch policy, R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF(mwboost::detail::make_future_async_shared_state<Rp>(
              BF(
                  f
                  , thread_detail::decay_copy(mwboost::forward<ArgTypes>(args))...
              )
          ));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF(mwboost::detail::make_future_deferred_shared_state<Rp>(
              BF(
                  f
                  , thread_detail::decay_copy(mwboost::forward<ArgTypes>(args))...
              )
  ...
  async(launch policy, R(*f)()) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      ...
      ret.set_async();
      mwboost::thread( mwboost::move(pt) ).detach();
      return ::mwboost::move(ret);
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      std::terminate();
      //BOOST_THREAD_FUTURE<R> ret;
  ...
  template <class F, class ...ArgTypes>
  BOOST_THREAD_FUTURE<typename mwboost::result_of<typename decay<F>::type(
  ...
  async(launch policy, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF(mwboost::detail::make_future_async_shared_state<Rp>(
              BF(
                  thread_detail::decay_copy(mwboost::forward<F>(f))
                , thread_detail::decay_copy(mwboost::forward<ArgTypes>(args))...
              )
          ));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
  ...
  template <class F>
  BOOST_THREAD_FUTURE<typename mwboost::result_of<typename decay<F>::type()>::type>
  async(launch policy, BOOST_THREAD_FWD_REF(F) f) {
    typedef typename mwboost::result_of<typename decay<F>::type()>::type R;
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
    typedef packaged_task<R()> packaged_task_type;
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      packaged_task_type pt( mwboost::forward<F>(f) );
      ...
      //BOOST_THREAD_FUTURE<R> ret;
      //return ::mwboost::move(ret);
      //          return mwboost::detail::make_future_deferred_shared_state<Rp>(
      //              BF(
      //                  thread_detail::decay_copy(mwboost::forward<F>(f))
      //              )
      //          );
...
namespace detail {
    ...
    public:
      ...
      : that(x.that), f_(mwboost::move(x.f_))
      {
        x.that.reset();
      }
...
namespace detail {
    ...
    public:
      ...
      void init(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f)
      {
        ...
        shared_state_nullary_task<Rp,Cont> t(this->shared_from_this(), mwboost::forward<Fp>(f));
        ex.submit(mwboost::move(t));
      }
    ...
    make_future_executor_shared_state(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f) {
      ...
      h->init(ex, mwboost::forward<Fp>(f));
      return BOOST_THREAD_FUTURE<Rp>(h);
    }
...
//#if ! defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
  ...
  async(Executor& ex, R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(mwboost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            f
            , thread_detail::decay_copy(mwboost::forward<ArgTypes>(args))...
        )
  ...
  async(Executor& ex, R(*f)(BOOST_THREAD_FWD_REF(A1)), BOOST_THREAD_FWD_REF(A1) a1) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(mwboost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            f
            , thread_detail::decay_copy(mwboost::forward<A1>(a1))
        )
    ));
  }
#endif // defined BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNCTION_PTR
  ...
  async(Executor& ex, BOOST_THREAD_FWD_REF(F) f)  {
    ...
    return mwboost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            thread_detail::decay_copy(mwboost::forward<F>(f))
        )
    );
  ...
  async(Executor& ex, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1, BOOST_THREAD_FWD_REF(A2) a2) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(mwboost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            thread_detail::decay_copy(mwboost::forward<F>(f))
          , thread_detail::decay_copy(mwboost::forward<A1>(a1))
          , thread_detail::decay_copy(mwboost::forward<A2>(a2))
        )
    ));
  }
  ...
  async(R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), f, mwboost::forward<ArgTypes>(args)...));
  }
  #else
  template <class R>
  BOOST_THREAD_FUTURE<R>
  ...
  async(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args) {
      return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), mwboost::forward<F>(f), mwboost::forward<ArgTypes>(args)...));
  }
#else
  template <class F>
  BOOST_THREAD_FUTURE<typename mwboost::result_of<F()>::type>
  async(BOOST_THREAD_FWD_REF(F) f) {
      return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), mwboost::forward<F>(f)));
  }
#endif
  ...
  BOOST_THREAD_FUTURE<typename decay<T>::type> make_future(BOOST_THREAD_FWD_REF(T) value) {
    ...
    p.set_value(mwboost::forward<future_value_type>(value));
  ...
  BOOST_THREAD_FUTURE<typename detail::deduced_type<T>::type> make_ready_future(BOOST_THREAD_FWD_REF(T) value) {
    ...
    promise<future_value_type> p;
    p.set_value(mwboost::forward<T>(value));
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  ...
  BOOST_THREAD_FUTURE<T> make_exceptional_future(E ex) {
    promise<T> p;
    p.set_exception(mwboost::copy_exception(ex));
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  ...
  BOOST_THREAD_FUTURE<T> make_exceptional_future() {
    promise<T> p;
    p.set_exception(mwboost::current_exception());
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  template <typename T>
...
namespace detail
{
  ...
  public:
    ...
    : parent(mwboost::move(f)),
      continuation(mwboost::move(c))
    {
    }

    void init(mwboost::unique_lock<mwboost::mutex> &lock)
    {
    ...
    void call() {
      try {
        this->mark_finished_with_result(this->continuation(mwboost::move(this->parent)));
      } catch(...) {
        this->mark_exceptional_finish();
      }
    ...
    static void run(shared_ptr<mwboost::detail::shared_state_base> that_)
    {
  ...
  public:
    ...
    {
      try {
        this->continuation(mwboost::move(this->parent));
        this->mark_finished_with_result();
      } catch(...) {
        this->mark_exceptional_finish();
      }
  ...
  public:
    ...
    : base_type(mwboost::move(f), mwboost::forward<Fp>(c))
    {    }

    void launch_continuation() {
#if defined BOOST_THREAD_FUTURE_BLOCKING
      mwboost::lock_guard<mwboost::mutex> lk(this->mutex);
      this->thr_ = mwboost::thread(&future_async_continuation_shared_state::run, static_shared_from_this(this));
#else
      mwboost::thread(&base_type::run, static_shared_from_this(this)).detach();
#endif
    }
  };
...
namespace detail {
  ...
  public:
    ...
    template <class Ex>
    void init(mwboost::unique_lock<mwboost::mutex> &lk, Ex& ex)
    {
    ...
    void launch_continuation() {
      ...
      this->get_executor()->submit(mwboost::move(fct));
    }
...
#if defined BOOST_THREAD_FUTURE_BLOCKING
      mwboost::lock_guard<mwboost::mutex> lk(this->mutex);
      this->thr_ = mwboost::thread(&base_type::run, static_shared_from_this(this));
#else
      mwboost::thread(&base_type::run, static_shared_from_this(this)).detach();
#endif
    }
  ...
  public:
    ...
    virtual void execute(mwboost::unique_lock<mwboost::mutex>& lk) {
      this->parent.wait();
      this->call(lk);
    }
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_deferred_continuation_shared_state(
      mwboost::unique_lock<mwboost::mutex> &lock,
      BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<future_deferred_continuation_shared_state<F, Rp, Cont> >
        h(new future_deferred_continuation_shared_state<F, Rp, Cont>(mwboost::move(f), mwboost::forward<Fp>(c)));
    h->init(lock);
    return BOOST_THREAD_FUTURE<Rp>(h);
  }
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_async_continuation_shared_state(
      mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<future_async_continuation_shared_state<F,Rp, Cont> >
        h(new future_async_continuation_shared_state<F,Rp, Cont>(mwboost::move(f), mwboost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_future_sync_continuation_shared_state(
    ...
    shared_ptr<future_sync_continuation_shared_state<F,Rp, Cont> >
        h(new future_sync_continuation_shared_state<F,Rp, Cont>(mwboost::move(f), mwboost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<future_executor_continuation_shared_state<F,Rp, Cont> >
        h(new future_executor_continuation_shared_state<F,Rp, Cont>(mwboost::move(f), mwboost::forward<Fp>(c)));
    h->init(lock, ex);
  ...
  make_shared_future_deferred_continuation_shared_state(
    ...
    shared_ptr<shared_future_deferred_continuation_shared_state<F, Rp, Cont> >
        h(new shared_future_deferred_continuation_shared_state<F, Rp, Cont>(f, mwboost::forward<Fp>(c)));
    h->init(lock);
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_shared_future_async_continuation_shared_state(
      mwboost::unique_lock<mwboost::mutex> &lock, F f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<shared_future_async_continuation_shared_state<F,Rp, Cont> >
        h(new shared_future_async_continuation_shared_state<F,Rp, Cont>(f, mwboost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_shared_future_sync_continuation_shared_state(
    ...
    shared_ptr<shared_future_sync_continuation_shared_state<F,Rp, Cont> >
        h(new shared_future_sync_continuation_shared_state<F,Rp, Cont>(f, mwboost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_shared_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<shared_future_executor_continuation_shared_state<F, Rp, Cont> >
        h(new shared_future_executor_continuation_shared_state<F, Rp, Cont>(f, mwboost::forward<Fp>(c)));
    h->init(lock, ex);
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename mwboost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type>
  BOOST_THREAD_FUTURE<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func) {
    typedef typename mwboost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    ...
    mwboost::unique_lock<mwboost::mutex> lock(sentinel->mutex);

    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF((mwboost::detail::make_future_async_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, mwboost::move(*this), mwboost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF((mwboost::detail::make_future_deferred_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, mwboost::move(*this), mwboost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::sync)) {
      return BOOST_THREAD_MAKE_RV_REF((mwboost::detail::make_future_sync_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, mwboost::move(*this), mwboost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
      ...
      return BOOST_THREAD_MAKE_RV_REF((mwboost::detail::make_future_executor_continuation_shared_state<Ex, BOOST_THREAD_FUTURE<R>, future_type>(ex,
                    lock, mwboost::move(*this), mwboost::forward<F>(func)
                )));
#endif
  ...
  BOOST_THREAD_FUTURE<R>::then(BOOST_THREAD_FWD_REF(F) func)  {
    ...
    return this->then(this->launch_policy(), mwboost::forward<F>(func));
#else
    typedef typename mwboost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_.get()!=0, future_uninitialized());
  ...
  template <typename R2>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename mwboost::result_of<F(BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >)>::type>
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename mwboost::result_of<F(shared_future<R>)>::type>
  shared_future<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func)  const
  {
    typedef typename mwboost::result_of<F(shared_future<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_.get()!=0, future_uninitialized());

    mwboost::unique_lock<mwboost::mutex> lock(this->future_->mutex);
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF((mwboost::detail::make_shared_future_async_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, mwboost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF((mwboost::detail::make_shared_future_deferred_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, mwboost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::sync)) {
      return BOOST_THREAD_MAKE_RV_REF((mwboost::detail::make_shared_future_sync_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, mwboost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
    } else if (underlying_cast<int>(policy) & int(launch::executor)) {
      typedef executor Ex;
      Ex& ex = *(this->future_->get_executor());
      return BOOST_THREAD_MAKE_RV_REF((mwboost::detail::make_shared_future_executor_continuation_shared_state<Ex, shared_future<R>, future_type>(ex,
                    lock, *this, mwboost::forward<F>(func)
                )));
#endif
    } else if (underlying_cast<int>(policy) & int(launch::inherit)) {
...
namespace detail
{
  ...
  struct mfallbacker_to
  {
    ...
    : value_(mwboost::move(v))
    {}

    T operator()(BOOST_THREAD_FUTURE<T> fut) {
      return fut.get_or(mwboost::move(value_));
    }
  };
  template <typename T>
  ...
  BOOST_THREAD_FUTURE<R>::fallback_to(BOOST_THREAD_RV_REF(R2) v) {
    return then(detail::mfallbacker_to<R>(mwboost::move(v)));
  }
...
namespace detail
{
    ...
    void launch_continuation()
    {
      ...
      if (! unwrapped.valid() )
      {
        if (wrapped.has_exception()) {
          ...
          {
            lk.unlock();
            mwboost::unique_lock<mwboost::mutex> lk2(unwrapped.future_->mutex);
            unwrapped.future_->set_continuation_ptr(this->shared_from_this(), lk2);
          } else {
            this->mark_exceptional_finish_internal(mwboost::copy_exception(future_uninitialized()), lk);
          }
        }
      } else {
  ...
  make_future_unwrap_shared_state(mwboost::unique_lock<mwboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f) {
    shared_ptr<future_unwrap_shared_state<F, Rp> >
        h(new future_unwrap_shared_state<F, Rp>(mwboost::move(f)));
  ...
  BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >::unwrap()
  {
    ...
    return mwboost::detail::make_future_unwrap_shared_state<BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >, R2>(lock, mwboost::move(*this));
  }
#endif
...
namespace detail
{
  ...
  struct future_when_all_vector_shared_state: future_async_shared_state_base<csbl::vector<F> >
  {
    ...
    static void run(shared_ptr<mwboost::detail::shared_state_base> that_) {
      ...
      try {
        mwboost::wait_for_all(that->vec_.begin(), that->vec_.end());
        that->mark_finished_with_result(mwboost::move(that->vec_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
    }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = mwboost::thread(&future_when_all_vector_shared_state::run, this->shared_from_this());
#else
      mwboost::thread(&future_when_all_vector_shared_state::run, this->shared_from_this()).detach();
#endif
    }
  ...
  public:
    ...
    future_when_all_vector_shared_state(values_tag, BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures) {
      vec_.push_back(mwboost::forward<T0>(f));
      typename alias_t<char[]>::type{
          ( //first part of magic unpacker
          vec_.push_back(mwboost::forward<T>(futures)),'0'
          )..., '0'
  ...
  struct future_when_any_vector_shared_state: future_async_shared_state_base<csbl::vector<F> >
  {
    ...
    static void run(shared_ptr<mwboost::detail::shared_state_base> that_)
    {
      ...
      try {
        mwboost::wait_for_any(that->vec_.begin(), that->vec_.end());
        that->mark_finished_with_result(mwboost::move(that->vec_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = mwboost::thread(&future_when_any_vector_shared_state::run, this->shared_from_this());
#else
      mwboost::thread(&future_when_any_vector_shared_state::run, this->shared_from_this()).detach();
#endif
    }
  ...
  public:
    ...
    future_when_any_vector_shared_state(values_tag,
      ...
      typename alias_t<char[]>::type{
          ( //first part of magic unpacker
          vec_.push_back(mwboost::forward<T>(futures))
          ,'0'
          )...,
          '0'
      }; //second part of magic unpacker
  ...
  struct wait_for_all_fctr {
    template <class ...T>
    void operator()(T&&... v) {
      mwboost::wait_for_all(mwboost::forward<T>(v)...);
    }
  };
  ...
  struct wait_for_any_fctr {
    template <class ...T>
    void operator()(T&&... v) {
      mwboost::wait_for_any(mwboost::forward<T>(v)...);
    }
  };
  ...
  struct future_when_all_tuple_shared_state: future_async_shared_state_base<Tuple>
  {
    ...
    static void run(shared_ptr<mwboost::detail::shared_state_base> that_) {
      ...
      try {
        // TODO make use of apply(that->tup_, mwboost::detail::wait_for_all_fctor());
        that->wait_for_all(Index());

        that->mark_finished_with_result(mwboost::move(that->tup_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
    }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = mwboost::thread(&future_when_all_tuple_shared_state::run, this->shared_from_this());
#else
      mwboost::thread(&future_when_all_tuple_shared_state::run, this->shared_from_this()).detach();
#endif
  ...
  public:
    ...
    future_when_all_tuple_shared_state(values_tag, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(Fs) ... futures) :
      tup_(mwboost::csbl::make_tuple(mwboost::forward<F>(f), mwboost::forward<Fs>(futures)...))
    {
    }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = mwboost::thread(&future_when_any_tuple_shared_state::run, this->shared_from_this());
#else
      mwboost::thread(&future_when_any_tuple_shared_state::run, this->shared_from_this()).detach();
#endif
    }
  ...
  when_all(BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures) {
    ...
    shared_ptr<factory_type>
        h(new factory_type(detail::values_tag_value, mwboost::forward<T0>(f), mwboost::forward<T>(futures)...));
    h->init();
    return BOOST_THREAD_FUTURE<container_type>(h);
  }
#endif
