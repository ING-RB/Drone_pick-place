...
namespace date_time {
  ...
  str_from_delimited_time_duration(const std::basic_string<char_type>& s)
  {
    ...
    int hour =0; 
    bool is_neg = (s.at(0) == '-');
    mwboost::int64_t fs=0;
    int pos = 0;
      
    typedef typename std::basic_string<char_type>::traits_type traits_type;
    typedef mwboost::char_separator<char_type, traits_type> char_separator_type;
    typedef mwboost::tokenizer<char_separator_type,
                             typename std::basic_string<char_type>::const_iterator,
                             std::basic_string<char_type> > tokenizer;
    typedef typename mwboost::tokenizer<char_separator_type,
                             typename std::basic_string<char_type>::const_iterator,
    ...
    for(tokenizer_iterator beg=tok.begin(); beg!=tok.end();++beg){
      switch(pos) {
      case 0: {
        hour = mwboost::lexical_cast<int>(*beg);
        break;
      }
      case 1: {
        min = mwboost::lexical_cast<unsigned short>(*beg);
        break;
      }
      case 2: {
        sec = mwboost::lexical_cast<unsigned short>(*beg);
        break;
      }
      case 3: {
        ...
        if(digits >= precision) {
          // drop excess digits
          fs = mwboost::lexical_cast<mwboost::int64_t>(beg->substr(0, precision));
        }
        else {
          fs = mwboost::lexical_cast<mwboost::int64_t>(*beg);
        }
#endif
        if(digits < precision){
  ...
  parse_undelimited_time_duration(const std::string& s)
  {
    ...
    bool wrap_off = false;
    bool ret_part = true;
    mwboost::offset_separator osf(offsets, offsets+4, wrap_off, ret_part); 
    typedef mwboost::tokenizer<mwboost::offset_separator,
    ...
    for(tokenizer_iterator ti=tok.begin(); ti!=tok.end();++ti){
      switch(pos) {
        case 0: 
          {
            hours = mwboost::lexical_cast<int>(*ti); 
            break;
          }
        case 1: 
          {
            min = mwboost::lexical_cast<short>(*ti); 
            break;
          }
        case 2: 
          {
            sec = mwboost::lexical_cast<short>(*ti); 
            break;
          }
        case 3:
          {
            ...
            if(digits >= precision) {
              // drop excess digits
              fs = mwboost::lexical_cast<mwboost::int64_t>(char_digits.substr(0, precision));
            }
            else if(digits == 0) {
              fs = 0; // lexical_cast doesn't like empty strings
            }
            else {
              fs = mwboost::lexical_cast<mwboost::int64_t>(char_digits);
            }
#endif
            if(digits < precision){
              // trailing zeros get dropped from the string, 
