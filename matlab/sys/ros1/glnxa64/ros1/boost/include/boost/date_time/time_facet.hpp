...
namespace date_time {
  ...
  class BOOST_SYMBOL_VISIBLE time_facet :
    public mwboost::date_time::date_facet<typename time_type::date_type , CharT, OutItrT> {
    typedef time_formats< CharT > formats_type;
   public:
    typedef typename time_type::date_type date_type;
    typedef typename time_type::time_duration_type time_duration_type;
    typedef mwboost::date_time::period<time_type,time_duration_type> period_type;
    typedef mwboost::date_time::date_facet<typename time_type::date_type, CharT, OutItrT> base_type;
                ...
                const time_type& time_arg) const
    {
      ...
      mwboost::algorithm::replace_all(local_format,
        mwboost::as_literal(formats_type::full_24_hour_time_format),
        mwboost::as_literal(formats_type::full_24_hour_time_expanded_format));
      mwboost::algorithm::replace_all(local_format,
        mwboost::as_literal(formats_type::short_24_hour_time_format),
        mwboost::as_literal(formats_type::short_24_hour_time_expanded_format));
      ...
      if (local_format.find(posix_zone_string_format) != string_type::npos) {
        if(time_arg.zone_abbrev().empty()) {
          ...
          mwboost::algorithm::erase_all(local_format, posix_zone_string_format);
        }
        else{
          mwboost::algorithm::replace_all(local_format,
      ...
      if (local_format.find(zone_name_format) != string_type::npos) {
        if(time_arg.zone_name().empty()) {
          ...
          ss << ' ' << zone_name_format;
          mwboost::algorithm::erase_all(local_format, ss.str());
        }
        else{
          mwboost::algorithm::replace_all(local_format,
                                        zone_name_format,
      ...
      if (local_format.find(zone_iso_extended_format) != string_type::npos) {
        if(time_arg.zone_name(true).empty()) {
          ...
          mwboost::algorithm::erase_all(local_format, zone_iso_extended_format);
        }
        else{
          mwboost::algorithm::replace_all(local_format,
      ...
      if (local_format.find(zone_iso_format) != string_type::npos) {
        if(time_arg.zone_abbrev(true).empty()) {
          ...
          mwboost::algorithm::erase_all(local_format, zone_iso_format);
        }
        else{
          mwboost::algorithm::replace_all(local_format,
                                        zone_iso_format,
      ...
      if (local_format.find(fractional_seconds_or_none_format) != string_type::npos) {
        ...
        if (!frac_str.empty()) {
          ...
          mwboost::algorithm::erase_all(local_format,
                                      fractional_seconds_or_none_format);
        }
      }
                ...
                const time_duration_type& time_dur_arg) const
    {
      ...
      if (time_dur_arg.is_negative()) {
        ...
        mwboost::algorithm::replace_all(format,
        ...
        mwboost::algorithm::erase_all(format, duration_sign_negative_only);
        // remove all the %+ in the string with '+'
        mwboost::algorithm::replace_all(format,
                                      duration_sign_always,
                                      positive_sign);
      ...
      if (format.find(unrestricted_hours_format) != string_type::npos) {
        ...
        mwboost::algorithm::replace_all(format, unrestricted_hours_format, hours_str);
      }
      ...
      if (format.find(hours_format) != string_type::npos) {
        ...
        mwboost::algorithm::replace_all(format, hours_format, hours_str);
      }
      ...
      if (format.find(fractional_seconds_or_none_format) != string_type::npos) {
        ...
        if (frac_str.size()) {
          ...
          mwboost::algorithm::erase_all(format,
                                      fractional_seconds_or_none_format);
        }
      }
  ...
  protected:
    ...
    integral_as_string(IntT val, int width = 2)
    {
      ...
      char_type buff[34];
      ss << _i64toa(static_cast<mwboost::int64_t>(val), buff, 10);
#else
      ss << val;
#endif
      return ss.str();
  ...
  class BOOST_SYMBOL_VISIBLE time_input_facet :
    public mwboost::date_time::date_input_facet<typename time_type::date_type , CharT, InItrT> {
    public:
      typedef typename time_type::date_type date_type;
      typedef typename time_type::time_duration_type time_duration_type;
      typedef typename time_duration_type::fractional_seconds_type fracional_seconds_type;
      typedef mwboost::date_time::period<time_type,time_duration_type> period_type;
      typedef mwboost::date_time::date_input_facet<typename time_type::date_type, CharT, InItrT> base_type;
    ...
    protected:
      ...
      InItrT check_special_value(InItrT& sitr,InItrT& stream_end, temporal_type& tt, char_type c='\0') const
      {
        ...
        if(mr.current_match == match_results::PARSE_ERROR) {
          ...
          mwboost::throw_exception(std::ios_base::failure("Parse failed. No match found for '" + tmp + "'"));
