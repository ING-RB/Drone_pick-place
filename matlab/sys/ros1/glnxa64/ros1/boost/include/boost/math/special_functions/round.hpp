...
inline tools::promote_args_t<T> round(const T& v, const Policy& pol, const std::false_type&)
{
   ...
   if(!(mwboost::math::isfinite)(v))
   {
      return policies::raise_rounding_error("mwboost::math::round<%1%>(%1%)", nullptr, static_cast<result_type>(v), static_cast<result_type>(v), pol);
   }
   //
...
inline int iround(const T& v, const Policy& pol)
{
   ...
   T r = mwboost::math::round(v, pol);
   if(r > static_cast<result_type>((std::numeric_limits<int>::max)()) || r < static_cast<result_type>((std::numeric_limits<int>::min)()))
   {
      return static_cast<int>(policies::raise_rounding_error("mwboost::math::iround<%1%>(%1%)", nullptr, v, 0, pol));
   }
...
inline long lround(const T& v, const Policy& pol)
{
   ...
   {
      return static_cast<long int>(policies::raise_rounding_error("mwboost::math::lround<%1%>(%1%)", nullptr, v, 0L, pol));
   }
...
inline long long llround(const T& v, const Policy& pol)
{
   ...
   {
      return static_cast<long long>(policies::raise_rounding_error("mwboost::math::llround<%1%>(%1%)", nullptr, v, static_cast<long long>(0), pol));
   }
