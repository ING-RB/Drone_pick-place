...
template< class X, class Y, class T > inline void sp_enable_shared_from_this( mwboost::shared_ptr<X> const * ppx, Y const * py, mwboost::enable_shared_from_this< T > const * pe )
{
    if( pe != 0 )
    {
...
template< class X, class Y > inline void sp_enable_shared_from_this( mwboost::shared_ptr<X> * ppx, Y const * py, mwboost::enable_shared_from_raw const * pe );
...
template< class T, class Y > inline void sp_pointer_construct( mwboost::shared_ptr< T > * ppx, Y * p, mwboost::detail::shared_count & pn )
{
    mwboost::detail::shared_count( p ).swap( pn );
    mwboost::detail::sp_enable_shared_from_this( ppx, p, p );
}
...
template< class T, class Y > inline void sp_pointer_construct( mwboost::shared_ptr< T[] > * /*ppx*/, Y * p, mwboost::detail::shared_count & pn )
{
    ...
    mwboost::detail::shared_count( p, mwboost::checked_array_deleter< T >() ).swap( pn );
}

template< class T, std::size_t N, class Y > inline void sp_pointer_construct( mwboost::shared_ptr< T[N] > * /*ppx*/, Y * p, mwboost::detail::shared_count & pn )
{
...
template< class T, class Y > inline void sp_deleter_construct( mwboost::shared_ptr< T > * ppx, Y * p )
{
    mwboost::detail::sp_enable_shared_from_this( ppx, p, p );
}
...
template< class T, std::size_t N, class Y > inline void sp_deleter_construct( mwboost::shared_ptr< T[N] > * /*ppx*/, Y * /*p*/ )
{
    sp_assert_convertible< Y[N], T[N] >();
}
...
public:
    ...
    typedef typename mwboost::detail::sp_element< T >::type element_type;
    ...
    BOOST_CONSTEXPR shared_ptr( mwboost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT : px( 0 ), pn()
    {
    }

#endif

    BOOST_CONSTEXPR shared_ptr( mwboost::detail::sp_internal_constructor_tag, element_type * px_, mwboost::detail::shared_count const & pn_ ) BOOST_SP_NOEXCEPT : px( px_ ), pn( pn_ )
    {
    }

#if !defined( BOOST_NO_CXX11_RVALUE_REFERENCES )

    BOOST_CONSTEXPR shared_ptr( mwboost::detail::sp_internal_constructor_tag, element_type * px_, mwboost::detail::shared_count && pn_ ) BOOST_SP_NOEXCEPT : px( px_ ), pn( std::move( pn_ ) )
    {
    }
    ...
    {
        mwboost::detail::sp_pointer_construct( this, p, pn );
    }
    ...
    {
        mwboost::detail::sp_deleter_construct( this, p );
    }
    ...
    template<class D> shared_ptr( mwboost::detail::sp_nullptr_t p, D d ): px( p ), pn( p, static_cast< D&& >( d ) )
    {
    }

#else

    template<class D> shared_ptr( mwboost::detail::sp_nullptr_t p, D d ): px( p ), pn( p, d )
    {
    }
    ...
    template<class D, class A> shared_ptr( mwboost::detail::sp_nullptr_t p, D d, A a ): px( p ), pn( p, static_cast< D&& >( d ), a )
    {
    }

#else

    template<class D, class A> shared_ptr( mwboost::detail::sp_nullptr_t p, D d, A a ): px( p ), pn( p, d, a )
    {
    }
...
// ... except in C++0x, move disables the implicit copy
    ...
    template<class Y>
    shared_ptr( weak_ptr<Y> const & r, mwboost::detail::sp_nothrow_tag )
    BOOST_SP_NOEXCEPT : px( 0 ), pn( r.pn, mwboost::detail::sp_nothrow_tag() )
    {
        if( !pn.empty() )
        {
            px = r.px;
    ...
    shared_ptr( shared_ptr<Y> const & r, typename mwboost::detail::sp_enable_if_convertible<Y,T>::type = mwboost::detail::sp_empty() )
    ...
    explicit shared_ptr( std::auto_ptr<Y> & r ): px(r.get()), pn()
    {
        ...
        Y * tmp = r.get();
        pn = mwboost::detail::shared_count( r );

        mwboost::detail::sp_deleter_construct( this, tmp );
    }
    ...
    template<class Ap>
    explicit shared_ptr( Ap r, typename mwboost::detail::sp_enable_if_auto_ptr<Ap, int>::type = 0 ): px( r.get() ), pn()
    {
    ...
    template< class Y, class D >
    shared_ptr( mwboost::movelib::unique_ptr< Y, D > r ): px( r.get() ), pn()
    {
        mwboost::detail::sp_assert_convertible< Y, T >();

        typename mwboost::movelib::unique_ptr< Y, D >::pointer tmp = r.get();
    ...
    template<class Ap>
    typename mwboost::detail::sp_enable_if_auto_ptr< Ap, shared_ptr & >::type operator=( Ap r )
    {
        this_type( r ).swap( *this );
        return *this;
    }
    ...
    template<class Y, class D>
    shared_ptr & operator=( mwboost::movelib::unique_ptr<Y, D> r )
    {
        // this_type( static_cast< unique_ptr<Y, D> && >( r ) ).swap( *this );

        mwboost::detail::sp_assert_convertible< Y, T >();

        typename mwboost::movelib::unique_ptr< Y, D >::pointer p = r.get();
        ...
        {
            tmp.px = p;
            tmp.pn = mwboost::detail::shared_count( r );

            mwboost::detail::sp_deleter_construct( &tmp, p );
        }
...
// Move support
    ...
    shared_ptr( shared_ptr && r ) BOOST_SP_NOEXCEPT : px( r.px ), pn( static_cast< mwboost::detail::shared_count && >( r.pn ) )
    {
        r.px = 0;
    }
    ...
    shared_ptr( shared_ptr<Y> && r, typename mwboost::detail::sp_enable_if_convertible<Y,T>::type = mwboost::detail::sp_empty() )
...
#endif
    BOOST_SP_NOEXCEPT : px( r.px ), pn( static_cast< mwboost::detail::shared_count && >( r.pn ) )
    {
        mwboost::detail::sp_assert_convertible< Y, T >();
        r.px = 0;
    }
    ...
    shared_ptr & operator=( mwboost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
    {
        this_type().swap(*this);
        return *this;
    }
    ...
    typename mwboost::detail::sp_dereference< T >::type operator* () const BOOST_SP_NOEXCEPT_WITH_ASSERT
    {
        BOOST_ASSERT( px != 0 );
        return *px;
    }
    
    typename mwboost::detail::sp_member_access< T >::type operator-> () const BOOST_SP_NOEXCEPT_WITH_ASSERT
    {
        BOOST_ASSERT( px != 0 );
        return px;
    }
    
    typename mwboost::detail::sp_array_access< T >::type operator[] ( std::ptrdiff_t i ) const BOOST_SP_NOEXCEPT_WITH_ASSERT
    {
        BOOST_ASSERT( px != 0 );
        BOOST_ASSERT( i >= 0 && ( i < mwboost::detail::sp_extent< T >::value || mwboost::detail::sp_extent< T >::value == 0 ) );

        return static_cast< typename mwboost::detail::sp_array_access< T >::type >( px[ i ] );
    }
...
#include <boost/smart_ptr/detail/operator_bool.hpp>
    ...
    void * _internal_get_local_deleter( mwboost::detail::sp_typeinfo_ const & ti ) const BOOST_SP_NOEXCEPT
    {
    ...
    mwboost::detail::shared_count _internal_count() const BOOST_SP_NOEXCEPT
    {
        return pn;
    }
...
private:
    ...
    mwboost::detail::shared_count pn;    // reference counter
...
template<class T> inline bool operator==( shared_ptr<T> const & p, mwboost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( mwboost::detail::sp_nullptr_t, shared_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
...
// get_pointer() enables mwboost::mem_fn to recognize shared_ptr
...
public:
    ...
    {
        return mwboost::detail::basic_get_deleter<D>( deleter_ );
    }
...
{
    D * d = mwboost::detail::basic_get_deleter<D>( p );

    if( d == 0 )
    {
        d = mwboost::detail::basic_get_local_deleter( d, p );
    }

    if( d == 0 )
    {
        mwboost::detail::esft2_deleter_wrapper *del_wrapper = mwboost::detail::basic_get_deleter<mwboost::detail::esft2_deleter_wrapper>(p);
// The following get_deleter method call is fully qualified because
// older versions of gcc (2.95, 3.2.3) fail to compile it when written del_wrapper->get_deleter<D>()
        if(del_wrapper) d = del_wrapper->::mwboost::detail::esft2_deleter_wrapper::get_deleter<D>();
    }
...
{
    mwboost::detail::spinlock_pool<2>::scoped_lock lock( p );
    return *p;
}
...
{
    mwboost::detail::spinlock & sp = mwboost::detail::spinlock_pool<2>::spinlock_for( p );
...
template< class T > std::size_t hash_value( mwboost::shared_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return mwboost::hash< typename mwboost::shared_ptr<T>::element_type* >()( p.get() );
}
...
template<class T> struct hash< ::mwboost::shared_ptr<T> >
{
    std::size_t operator()( ::mwboost::shared_ptr<T> const & p ) const BOOST_SP_NOEXCEPT
    {
        return std::hash< typename ::mwboost::shared_ptr<T>::element_type* >()( p.get() );
    }
};
