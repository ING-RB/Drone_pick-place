...
public:
    ...
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        mwboost::sp_array_constructor_hook( px );
#endif
    }
    ...
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        mwboost::sp_array_destructor_hook( px );
#endif
        mwboost::checked_array_delete( px );
    }
...
template<class T> inline bool operator==( scoped_array<T> const & p, mwboost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( mwboost::detail::sp_nullptr_t, scoped_array<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
