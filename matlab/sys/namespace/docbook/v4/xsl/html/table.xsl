        ...
        <xsl:when test="$colspec.colnum=$countcol">
          ...
          <col>
            <xsl:choose>
              ...
              <!-- @CHANGE: Pass through to HTML only if last character is * in colspecs --> 
              <xsl:when test="$colspec/@colwidth and
                             $colspec/parent::*/colspec/@colwidth[substring(normalize-space(.),string-length(normalize-space(.)),1) = '*']">
                <xsl:attribute name="width">
                  <xsl:choose>
                    <xsl:when test="normalize-space($colspec/@colwidth) = '*'">
                      <xsl:value-of select="'1*'"/>
              ...
              <!-- @CHANGE: Remove check for "*" since that condition is never met -->
