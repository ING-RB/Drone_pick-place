...
<!-- @CHANGE: Ensure valid class name when using number to define color -->
          <xsl:variable name="colorNumber" select="substring-after(@role, '#')"/>
          <xsl:choose>
            <xsl:when test="string-length($colorNumber) &gt; 0">
              <xsl:attribute name="class">
...
<!-- @CHANGE: If not a style, assume it is a color -->
    <xsl:if test="@role
                  and @role != 'strong'
                  and @role != 'underline'
                  and @role != 'bold'
...
<!-- @CHANGE: Add underline -->
          <xsl:when test="@role = 'underline'">
             <u><xsl:apply-templates/></u>
          </xsl:when>
<!-- @ENDCHANGE -->

<!-- @CHANGE: Explicit italic -->
