...
<!-- @CHANGE use rptgen custom routine instead
...
<!-- @CHANGE Alignment handled by parent media object element g392454
          <xsl:if test="@align">
            <xsl:attribute name="align">
              <xsl:choose>
                <xsl:when test="@align = 'center'">middle</xsl:when>
...
<!-- @BEGIN CHANGE.  Use rptgen map.  See callouts.xsl
...
<!-- @CHANGE -->
	  <xsl:with-param name="usemap">
             <xsl:if test="./parent::imageobject/preceding-sibling::areaspec">
                #<xsl:value-of select="generate-id(./parent::imageobject/preceding-sibling::areaspec)"/>
             </xsl:if>
          </xsl:with-param>
<!-- @ENDCHANGE -->
        <xsl:with-param name="alt">
