...
<!-- @CHANGE  Add outline only for non-empty labels (G513779) -->
      <xsl:variable name="bookmarkLabelString">
        <xsl:value-of select="normalize-space(translate($bookmark-label, $a-dia, $a-asc))" />
      </xsl:variable>
      <xsl:if test="string-length(bookmarkLabelString)>0">
        <fo:bookmark internal-destination="{$id}">
          <xsl:attribute name="starting-state">
            <xsl:value-of select="$bookmarks.state"/>
          </xsl:attribute>
    ...
    <x:xmpmeta xmlns:x="adobe:ns:meta/">
	  ...
	  <xsl:if test="$authors">
        ...
        <rdf:Description rdf:about="" xmlns:xmp="http://ns.adobe.com/xap/1.0/">
          ...
          <!-- @CHANGE Get version information from VERSION file -->
          <!-- Creator Tool -->
          <xmp:CreatorTool>
