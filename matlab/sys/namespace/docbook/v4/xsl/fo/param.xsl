...
<!-- @CHANGE: Removed starting /, caused problems with stylesheet editor generate.toc component -->
...
<!--@CHANGE: Insert page number in page -->
...
<xsl:param name="minimized.margin.size">0.02in</xsl:param>
...
<!-- @CHANGE: Rptgen prefers not to show comments -->
...
<!-- @CHANGE add support for header.content and footer.content -->
<xsl:template name="footer.content">
  ...
  <fo:block xmlns:fo="http://www.w3.org/1999/XSL/Format"><xsl:choose>
      <xsl:when test="$double.sided != 0 and ($sequence = 'even' or $sequence = 'blank') and $position = 'left'"><fo:page-number/></xsl:when>
      <xsl:when test="$double.sided != 0 and ($sequence = 'odd' or $sequence = 'first') and $position = 'right'"><fo:page-number/></xsl:when>
      <xsl:when test="$double.sided = 0 and $position='center'"><fo:page-number/></xsl:when>
      <xsl:otherwise/>
  </xsl:choose></fo:block>
</xsl:template>
...
<xsl:template name="header.content">
  ...
  <fo:block xmlns:fo="http://www.w3.org/1999/XSL/Format"><xsl:choose>
      ...
      <xsl:when test="$position = 'left'"><!-- enter left cell content --></xsl:when>
      <xsl:when test="$position = 'right'"><!-- enter right cell content --></xsl:when>
