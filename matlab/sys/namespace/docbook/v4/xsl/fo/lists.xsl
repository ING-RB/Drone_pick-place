...
<xsl:template match="orderedlist">
  ...
  <xsl:variable name="label-width">
    <xsl:choose>
      <xsl:when test="$pi-label-width = ''">
<!-- @CHANGE - Modified label-width to be dynamically calculated based on the length of the longest label needed.  0.6 em is allowed for every character.  
                E.g., If there are 120 children, the largest label needed will be '120.'.  '120.' is 3 digits plus 1 period = 4 characters => 2.4em (see g564490) -GRA 10-July-2009    
                N.b., the original value was :
	<xsl:value-of select="$orderedlist.label.width"/>
-->
	<xsl:value-of select="concat((string-length(count(*))+1)*0.6,'em')"/>
<!-- @ENDCHANGE -->    
