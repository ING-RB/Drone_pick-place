...
<!-- @CHANGE: Anchors may exist as a child of fo:flow.  In docbook, this happens
  when an anchor is a child of chapter.  Change to use fo:wrapper instead of fo:inline -->
...
<!-- @CHANGE - add linkid to dynamically choose between fileref and url.  Since Adobe does not like URLs of the form file:///a/b/c.pdf, we use plain paths whenever possible -->            
    <!-- Use @fileRef when we are linking to PDF.  
        Acrobat does not like referring PDF as URI.
        Use @url for other format -->
  ...
  <xsl:variable name ="ulink.url">
    <xsl:call-template name="fo-external-image">
      <xsl:with-param name="filename" select="$linkid"/>
    </xsl:call-template>
  </xsl:variable>
