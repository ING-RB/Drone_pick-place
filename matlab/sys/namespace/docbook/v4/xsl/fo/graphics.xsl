  ...
  <fo:external-graphic>
    <xsl:attribute name="src">
      <xsl:call-template name="fo-external-image">
        ...
        <xsl:choose>
            <xsl:when test="ancestor-or-self::*[@role='landscape'] or ancestor-or-self::*[@role='portrait']">
    ...
    <!--@CHANGE Delay writing out width attribute -->      
    ...
    <!--@CHANGE Write out width attribute only if it is not auto.  An auto value breaks FOP's scale-to-fit -->      
    ...
    <!--@CHANGE Delay writing out height attribute -->      
    ...
    <!--@CHANGE Write out height attribute only if it is not auto.  An auto value breaks FOP's scale-to-fit -->      
    ...
    <!-- @CHANGE, specify block and inline progression maximum to prevent overflow -->
    <xsl:attribute name="block-progression-dimension.maximum">
      <xsl:variable name="region-body-height-pts">
        <xsl:call-template name="get-region-body-height-in-points"/>
      </xsl:variable>         
      <xsl:value-of select="concat($region-body-height-pts,'pt')"/>
    </xsl:attribute>

    <xsl:attribute name="inline-progression-dimension.maximum">
...
<xsl:template match="mediaobject[@role]|mediaobjectco[@role]">

    <xsl:variable name="master-reference-name">
        <xsl:choose>
            <xsl:when test="@role = 'landscape'">
                <xsl:value-of select="'maximized-landscape'"/>
            </xsl:when>
            <xsl:when test="@role = 'portrait'">
                <xsl:value-of select="'maximized-portrait'"/>
            </xsl:when>
        </xsl:choose>
    </xsl:variable>
    <rg-page-break master-reference="{$master-reference-name}">
        <fo:block text-align="center" display-align="center">

            <xsl:call-template name="basic-mediaobject-match"/>
...
<xsl:template match="mediaobject|mediaobjectco" name="basic-mediaobject-match">

    <xsl:call-template name="process.media.object"/>

</xsl:template>

<xsl:template name="process.media.object">
...
<!-- @CHANGE template to find region.body.height -->
<!-- ==================================================================== -->
<xsl:template name="get-region-body-height-in-points">
  <xsl:variable name="page-height-pts">
    <xsl:call-template name="length-in-points">
      <xsl:with-param name="length" select="$page.height"/>
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="page-margin-top-pts">
    <xsl:call-template name="length-in-points">
      <xsl:with-param name="length" select="$page.margin.top"/>
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="page-margin-bottom-pts">
    <xsl:call-template name="length-in-points">
      <xsl:with-param name="length" select="$page.margin.bottom"/>
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="body-margin-top-pts">
    <xsl:call-template name="length-in-points">
      <xsl:with-param name="length" select="$body.margin.top"/>
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="body-margin-bottom-pts">
    <xsl:call-template name="length-in-points">
      <xsl:with-param name="length" select="$body.margin.bottom"/>
    </xsl:call-template>
  </xsl:variable>
 
  <!-- hard coded to section.title.level1.properties.fontsize 
    Subtract a little more space for the title -->  
  <xsl:variable name="title-font-size-pts"
    select="$body.font.master * 2.0736" />
  
  <xsl:value-of select="$page-height-pts - $page-margin-top-pts - $page-margin-bottom-pts - $body-margin-top-pts - $body-margin-bottom-pts - $title-font-size-pts"/>
</xsl:template>    
<!-- @ENDCHANGE -->
