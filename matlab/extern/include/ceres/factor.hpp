...
namespace mw_ceres {
    ...
    static std::unordered_map<std::string, int> NodeTypeInt{
        ...
        {"VEL3", 4}, {"IMU_BIAS", 5},
        {"EIGEN_QUATERNION", 6}, {"POSE_SE3_SCALE", 11}, 
        {"TRANSFORM_SE3", 12}, {"CAMERA_INTRINSICS", 13}};
    ...
    static std::unordered_map<VariableType, std::string> VariableTypeString{
        ...
        {VariableType::Transform_SE3, "TRANSFORM_SE3"},
        {VariableType::Camera_Intrinsics, "CAMERA_INTRINSICS"}
    };
    ...
    protected:
        ...
        std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> jacobian;
        double** jacobianPtr = nullptr; // jacobian pointer for ceres API
    ...
    public:
        ...
        /// This method is optional, update it only when the factor 
        ///  requires a custom loss function to handle outliers.
        virtual ceres::LossFunctionWrapper* createFactorLossFcn() const {
            return nullptr; 
        }

        virtual ~Factor() {
            delete[] jacobianPtr; // Deallocate the array of pointers
        }
        ...
        void setJacobianAndResidual(std::vector<double*> pBlocks) {
            auto cost = createFactorCostFcn();
            int residualSize = cost->num_residuals();
            // Initialize jacobian and residual with the correct size
            auto pBlockSizes = cost->parameter_block_sizes();
            size_t nBlock = pBlockSizes.size();
            jacobianPtr = new double* [nBlock];
            jacobian.resize(nBlock);
            residual.resize(residualSize);
            for (size_t i = 0; i < nBlock; i++) {
                // nRow = residualSize, nCol = pBlockSizes[i]
                jacobian[i].resize(residualSize, pBlockSizes[i]);
                jacobianPtr[i] = jacobian[i].data();
            }
            // Compute jacobian and residual
            cost->Evaluate(pBlocks.data(), residual.data(), jacobianPtr);
            delete cost;
        }
        std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> getJacobian() {
            return jacobian;
        }
    ...
    public:
        ...
        size_t getInformationLength() const {
