...
%   Copyright 2018 The MathWorks, Inc.
    ...
    error(message('MATLAB:virtualfileio:hadooploader:indeterminateHadoopClasspath'));
...
function classpath = iDiscoverFromHadoopCommand(hadoopInstallFolder)
    [status, classpathMsg] = matlab.io.internal.vfs.hadoop.callHadoop(hadoopInstallFolder, 'classpath');
    if status ~= 0
        classpath = [];
        return;
    end
...
function iThrowIfNonExistant(jars)
    for ii = 1:numel(jars)
        if ~exist(jars{ii}, 'file')
            error(message('MATLAB:virtualfileio:hadooploader:jarFileNotFound',jars{ii}));
        end
    end
end
