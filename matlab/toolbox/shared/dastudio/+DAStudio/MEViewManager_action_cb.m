...
troot  = DAStudio.ToolRoot;
action = findobj(troot, '-isa', 'DAStudio.Action', 'id', id);
...
switch (fcn)      
    ...
    case 'manageView'        
        ...
        h.showDialog('manage', 'me_view_manager_dialog_ui');
    case 'resetToFactory'
        message = DAStudio.message('modelexplorer:DAS:ResetToFactorySettings');        
        dp = DAStudio.DialogProvider;
        dp.questdlg(message, DAStudio.message('modelexplorer:DAS:ResetToFactorySettingsTitle'), ...
            {DAStudio.message('modelexplorer:DAS:Yes'), DAStudio.message('modelexplorer:DAS:No')}, ...
            DAStudio.message('modelexplorer:DAS:No'), {@resetViewCallback, h, ''});
    ...
    case 'resetAllToFactory'
        message = DAStudio.message('modelexplorer:DAS:ResetAllToFactorySettings');        
        dp = DAStudio.DialogProvider;
        dp.questdlg(message, DAStudio.message('modelexplorer:DAS:ResetToFactorySettingsTitle'), ...
            {DAStudio.message('modelexplorer:DAS:Yes'), DAStudio.message('modelexplorer:DAS:No')}, ...
            DAStudio.message('modelexplorer:DAS:No'), {@resetViewCallback, h, 'all'});
    ...
    case 'suggestionsWhatIsThis'
        helpview('simulink', 'ModelExplorer_Suggestions_WhatsThis');
    otherwise
...
if strcmp(proceed, DAStudio.message('modelexplorer:DAS:Yes'))
    if isempty(all)
        h.resetView;
    else
        % Fill Proxy
        delete(findobj(h.VMProxy, '-isa', 'DAStudio.MEView'));
        ...
        domain = findobj(h.VMProxy.Domains, 'Name', h.ActiveDomainName);
