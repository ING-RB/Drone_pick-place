function checkFiniteDblColVec(h, prop, value)
%CHECKFINITEDBLCOLVEC Check if value is a finite double column vector
...
if  (n~=1) || ~isa(value, 'double') || any(isinf(value)) ...
    ...
    if ischar(h)
        msg = sprintf(['The %s input argument of %s must be a finite double '...
            'column vector.'], prop, h);
    else
        msg = sprintf(['The ''%s'' property of ''%s'' must be a finite '...
            'double column vector.'], prop, class(h));
    end
    throwAsCaller(MException('MATLAB:datatypes:NotFiniteDblColVec', msg));
end
%---------------------------------------------------------------------------
% [EOF]