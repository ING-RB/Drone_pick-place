function checkFiniteRealDblMat(h, prop, value, reqSize)
%CHECKFINITEREALDBLMAT Check if value is a finite real double matrix
...
if  (m~=reqSize(1)) || (n~=reqSize(2)) || ~isa(value, 'double') ...
        || any(any(isinf(value))) || any(any(isnan(value))) || ~isreal(value)
    if ischar(h)
    ...
    throwAsCaller(MException('MATLAB:datatypes:NotFiniteRealDblMat', msg));
end
%---------------------------------------------------------------------------
% [EOF]