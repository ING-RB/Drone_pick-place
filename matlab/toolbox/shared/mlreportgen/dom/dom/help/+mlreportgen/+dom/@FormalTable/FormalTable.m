...
%
%     Note: Word automatically combines consecutive tables with matching
%     style names into a single table. To render consecutive
%     tables as separate tables in DOCX reports, do one of the following
%     options:
%       - Set the style names of the conecutive tables to different values
%       - Insert some content in between the tables. For example, insert an
%         empty mlreportgen.dom.Paragraph element between the tables.
%
...
%      Id                      - Id of this table
%      IsSortable              - Whether to create sortable table colums (HTML only)
%      HAlign                  - Horizontal alignment of this table
%      Header                  - Header section of this table
%      KeepWithinPage          - Keep content generated by table within the same page
...
properties
     ...
     %IsSortable Whether to create sortable table columns
     %    Valid values:
     %
     %    true  - table columns are sortable
     %    false - (default) table columns are not sortable
     %
     %    Note: This property applies only to the HTML output type
     %    reports. You can sort a table by clicking the column headers.
     %    The first time you click, the sorting direction is ascending and
     %    when you click again, the sorting direction will be descending.
     %    In sorting, the entire row is moved including all its content and
     %    formatting. In addition, if the last row doesn't specify its own
     %    bottom border then after sorting it gets the bottom border of the
     %    table.
     IsSortable;
