...
%    posEnum = compareDocumentPosition(thisNode,otherNode) compares the
%    position in the document  of thisNode with that of another node.
%    Returns a double value that encodes the position of the other node
%    relative to this node. Use the following MATLAB expression to decode
%    the position result:
%
%    bitor(POS,POSITION_ENUM) == POS
%
%    where POS is the value returned by this method and POSITION_ENUM is
%    the value returned by one of the following node methods:
%
...
%    DOCUMENT_POSITION_CONTAINED_BY The node contains the other node,
%                                   which also follows this node.
%    DOCUMENT_POSITION_CONTAINS     The other node contains this nod.
%                                   The other node precedes this node.
