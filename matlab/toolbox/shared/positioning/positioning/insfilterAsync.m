classdef insfilterAsync < fusion.internal.AsyncMARGGPSFuserBase & ...
    ...
    methods
        ...
        %   object with the same properties.
            ...
            s = saveObject(obj);
            cpObj = insfilterAsync('ReferenceFrame', obj.ReferenceFrame);
            loadObject(cpObj, s);
        end
    end
    ...
    % Facade for methods in fusion.internal.tuner.insfilterAsync
    methods
    ...
    methods (Static, Hidden)
        function [tunerparams, staticparams]  = getParamsForAutotune
            [tunerparams, staticparams]  = fusion.internal.tuner.insfilterAsync.getParamsForAutotune;
        end
        function [cost, stateEst] = tunerfuse(params, sensorData, groundTruth, cfg)
            [cost, stateEst] = fusion.internal.tuner.insfilterAsync.tunerfuse(params, sensorData, groundTruth, cfg);
        end
        function measNoise = getMeasNoiseExemplar
            measNoise = fusion.internal.tuner.insfilterAsync.getMeasNoiseExemplar;
        end
        function tf = hasMeasNoise
            tf = fusion.internal.tuner.insfilterAsync.hasMeasNoise;
        end
    end
    methods (Access = protected)
        function sensorData = processSensorData(~, sensorData)
            sensorData = fusion.internal.tuner.insfilterAsync.processSensorData(sensorData);
        end
        function groundTruth = processGroundTruth(~, groundTruth)
            groundTruth = fusion.internal.tuner.insfilterAsync.processGroundTruth(groundTruth);
        end
        function varargout = makeTunerOutput(obj, info, measNoise)
            [varargout{1:nargout}] = fusion.internal.tuner.insfilterAsync.makeTunerOutput(obj, info, measNoise);
        end
        function crossValidateInputs(~, sensorData, groundTruth) 
            fusion.internal.tuner.insfilterAsync.crossValidateInputs(sensorData, groundTruth) 
        end
    end
end % classdef
