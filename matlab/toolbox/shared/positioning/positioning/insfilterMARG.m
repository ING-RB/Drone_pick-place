classdef insfilterMARG < fusion.internal.MARGGPSFuserBase & ...
    ...
    methods
        ...
        %   object with the same properties.
            ...
            s = saveObject(obj);
            cpObj = insfilterMARG('ReferenceFrame', obj.ReferenceFrame);
            loadObject(cpObj, s);
        end
    end
    ...
    % Facade for methods in fusion.internal.tuner.insfilterMARG
    methods
    ...
    methods (Static, Hidden)
        function [tunerparams, staticparams]  = getParamsForAutotune
            [tunerparams, staticparams]  = fusion.internal.tuner.insfilterMARG.getParamsForAutotune;
        end
        function [cost, stateEst] = tunerfuse(params, sensorData, groundTruth, cfg)
            [cost, stateEst] = fusion.internal.tuner.insfilterMARG.tunerfuse(params, sensorData, groundTruth, cfg);
        end
        function measNoise = getMeasNoiseExemplar
            measNoise = fusion.internal.tuner.insfilterMARG.getMeasNoiseExemplar;
        end
        function tf = hasMeasNoise
            tf = fusion.internal.tuner.insfilterMARG.hasMeasNoise;
        end
    end
    methods (Access = protected)
        function sensorData = processSensorData(~, sensorData)
            sensorData = fusion.internal.tuner.insfilterMARG.processSensorData(sensorData);
        end
        function groundTruth = processGroundTruth(~, groundTruth)
            groundTruth = fusion.internal.tuner.insfilterMARG.processGroundTruth(groundTruth);
        end
        function varargout = makeTunerOutput(obj, info, measNoise)
            [varargout{1:nargout}] = fusion.internal.tuner.insfilterMARG.makeTunerOutput(obj, info, measNoise);
        end
        function crossValidateInputs(~, sensorData, groundTruth) 
            fusion.internal.tuner.insfilterMARG.crossValidateInputs(sensorData, groundTruth) 
        end
    end
end % classdef
