classdef insfilterErrorState < fusion.internal.ErrorStateIMUGPSFuserBase & ...
...
%   residualmvo    - Residuals and residual covariance from monocular
%                    visual odometry
...
%#codegen
    ...
    properties (Constant, Hidden)
        ...
        AccelerometerNoiseUnits = ['(m/s', squared, ')', squared];
        AccelerometerBiasNoiseUnits = ['(m/s', squared, ')', squared];
        GyroscopeNoiseUnits = ['(rad/s)', squared];
        GyroscopeBiasNoiseUnits = ['(rad/s)', squared];
    end

    methods
        function obj = insfilterErrorState(varargin)
        ...
        %   insfilterErrorState object with the same properties.
        
            s = saveObject(obj);
            cpObj = insfilterErrorState('ReferenceFrame', ...
                obj.ReferenceFrame);
            loadObject(cpObj, s);
        end
    end
    ...
    methods (Access = protected)
        ...
        function groups = getPropertyGroups(obj)
            if ~isscalar(obj)
                ...
                basicList.State = obj.State;
                basicList.StateCovariance = obj.StateCovariance;
                
                procNoiseList.GyroscopeNoise = obj.GyroscopeNoise;
                procNoiseList.AccelerometerNoise = obj.AccelerometerNoise;
                procNoiseList.GyroscopeBiasNoise = obj.GyroscopeBiasNoise;
                procNoiseList.AccelerometerBiasNoise = obj.AccelerometerBiasNoise;
                
                basicGroup = matlab.mixin.util.PropertyGroup(basicList);
                procNoiseGroup = matlab.mixin.util.PropertyGroup( ...
                    procNoiseList, 'Process Noise Variances');
                
                groups = [basicGroup procNoiseGroup];
            end
        end
    end
    ...
    methods (Hidden, Static)
        function name = matlabCodegenRedirect(~)
            name = 'fusion.internal.coder.insfilterErrorStateCG';
        end
    end


    % Methods related to autotuner (tune() function).
    % Facade for methods in fusion.internal.tuner.insfilterErrorState
    methods
        ...
        %   properties of the insfilterErrorState, FILT, to reduce the
        ...
        %   'Accelerometer','Gyroscope', "MVOOrientation', 'MVOPosition',
        %   'GPSPosition', and 'GPSVelocity'.  Each variable is an array of
        %   1-by-3 matrices of the corresponding sensor readings. Gaps in
        %   sensor data is denoted with missing.  The groundTruth input is a
        %   table which may contain any of the variables:
        ...
        %     GyroscopeBias	        1-by-3 gyroscope bias, body 
        %                                   frame
        %     AccelerometerBias	        1-by-3 accelerometer bias, body 
        %                                   frame
        %     VisualOdometryScale       1-by-1 visual odometry scale factor 
        %   
        ...
        %   insfilterErrorState, FILT, according to CFG which is produced by the
        ...
        %
        %   % Automatically tune the insfilterErrorState to improve the pose 
        %   % estimate. Skip tuning some properties.
        %   fuseTuned = insfilterErrorState('State', initialState, ...
        %       'StateCovariance', initialStateCovariance);
        ...
        %           fusemvo(fuseTuned, MVOPosition(ii,:), tunedmn.MVOPositionNoise, ...
    ...
    methods (Static, Hidden)
        function [tunerparams, staticparams]  = getParamsForAutotune
            [tunerparams, staticparams]  = fusion.internal.tuner.insfilterErrorState.getParamsForAutotune;
        end
        function [cost, stateEst] = tunerfuse(params, sensorData, groundTruth, cfg)
            [cost, stateEst] = fusion.internal.tuner.insfilterErrorState.tunerfuse(params, sensorData, groundTruth, cfg);
        end
        function measNoise = getMeasNoiseExemplar
            measNoise = fusion.internal.tuner.insfilterErrorState.getMeasNoiseExemplar;
        end
        function tf = hasMeasNoise
            tf = fusion.internal.tuner.insfilterErrorState.hasMeasNoise;
        end
    end
    methods (Access = protected)
        function sensorData = processSensorData(~, sensorData)
            sensorData = fusion.internal.tuner.insfilterErrorState.processSensorData(sensorData);
        end
        function groundTruth = processGroundTruth(~, groundTruth)
            groundTruth = fusion.internal.tuner.insfilterErrorState.processGroundTruth(groundTruth);
        end
        function varargout = makeTunerOutput(obj, info, measNoise)
            [varargout{1:nargout}] = fusion.internal.tuner.insfilterErrorState.makeTunerOutput(obj, info, measNoise);
        end
        function crossValidateInputs(~, sensorData, groundTruth) 
            fusion.internal.tuner.insfilterErrorState.crossValidateInputs(sensorData, groundTruth) 
        end
    end
end
