classdef (Hidden) insfilterNonholonomic
%   This class is for internal use only. It may be removed in the future. 

%INSFILTERNonholonomic Tuner specific functions for the insfilterNonholonomic
    ...
    methods (Static)
        ...
        function [cost, states] = tunerfuse(params, sensorData, groundTruth, cfg)
            if ~isempty(coder.target) || ~cfg.UseMex
                states = fusion.internal.tuner.processINSFilterNonholonomic(params, sensorData);
            else
                states = fusion.internal.tuner.mex.processINSFilterNonholonomic_mex(params, sensorData);
            end
            cost  = rmsStateErr(states, groundTruth, stateinfo(insfilterNonholonomic)); 
        end
        ...
        function groundTruth = processGroundTruth(groundTruth)
           ...
           assert(~isempty(groundTruth), message('shared_positioning:tuner:InputMustBeNonempty'));
            ...
            sinfo = stateinfo(insfilterNonholonomic);
        ...
        function varargout = makeTunerOutput(obj, info, measNoise)
            params = info(end);
            reset(obj);
            fusion.internal.tuner.insfilterNonholonomic.configure(obj, params, true);
        ...
        function configure(x, y, preserveDstType)
            % Configure object or struct x with the insfilterNonholonomic parameters
            ...
            x.GyroscopeBiasDecayFactor     = cast(y.GyroscopeBiasDecayFactor , 'like', ex.GyroscopeBiasDecayFactor);
            x.AccelerometerBiasNoise       = cast(y.AccelerometerBiasNoise , 'like', ex.AccelerometerBiasNoise);
            x.AccelerometerBiasDecayFactor = cast(y.AccelerometerBiasDecayFactor , 'like', ex.AccelerometerBiasDecayFactor);
            x.ZeroVelocityConstraintNoise  = cast(y.ZeroVelocityConstraintNoise , 'like', ex.ZeroVelocityConstraintNoise);
