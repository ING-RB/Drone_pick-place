...
%IMUFILTER Tuner specific functions for the imufilter
    ...
    methods (Static)
        ...
        function [cost, q] = tunerfuse(params, sensorData, groundTruth, cfg)
            if ~isempty(coder.target) || ~cfg.UseMex
                q = fusion.internal.tuner.processIMUFilter(params, sensorData);
            else
                q = fusion.internal.tuner.mex.processIMUFilter_mex(params, sensorData);
            end
        ...
        function varargout = makeTunerOutput(obj, info)
            ...
            % originally.
            fusion.internal.tuner.imufilter.configure(obj, params, true);
            
            varargout = {}; % no outputs for imufilter/tune
        end

        function configure(x, y, preserveDstType)
            % Configure object or struct x with the imufilter parameters
