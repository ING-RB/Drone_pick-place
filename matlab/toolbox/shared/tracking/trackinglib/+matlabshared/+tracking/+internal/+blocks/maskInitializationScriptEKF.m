...
dlgParams.FcnInfo.Predict.JacobianFcn = {StateTransitionJacobianFcn};
dlgParams.FcnInfo.Predict.HasJacobian = HasStateTransitionJacobianFcn;
...
dlgParams.FcnInfo.Correct.JacobianFcn = cell(NumberOfMeasurements,1);
dlgParams.FcnInfo.Correct.HasJacobian = false(NumberOfMeasurements,1);
...
for kk=1:NumberOfMeasurements
    ...
    dlgParams.FcnInfo.Correct.JacobianFcn{kk} = eval( sprintf('MeasurementJacobianFcn%d',kk) );
    % Sample time
    dlgParams.MeasurementFcnSampleTime{kk} = eval( sprintf('MeasurementFcn%dSampleTime',kk) );
    % The rest
    dlgParams.FcnInfo.Correct.HasJacobian(kk) = eval( sprintf('HasMeasurementJacobianFcn%d',kk) );
...
% Main MaskInit code
[p,signalCheck] = matlabshared.tracking.internal.blocks.maskInitializationFcnEKF(gcbh,dlgParams);
