classdef CubatureKalmanFilter < handle
    %
    
    % Leave the above row empty to suppress the message: "Help for X is
    % inherited from superclass matlabshared.tracking.internal.CubatureKalmanFilter"
    
    %CubatureKalmanFilter   Cubature Kalman filter for object tracking
    %   This cubature Kalman filter is designed for tracking. You can use
    ...
    %   tracks. The cubature Kalman filter is used for tracking objects
    %   that move according to a nonlinear motion model, or that are
    %   measured by a nonlinear measurement model.
    %
    %   The cubature Kalman filter algorithm implements a discrete time,
    ...
    %
    %   The cubature Kalman filter algorithm involves two steps.
    ...
    %
    %
    %   Constructing an CubatureKalmanFilter object:
    %
    %   CKF = CubatureKalmanFilter returns an cubature Kalman filter
    %   object with default state transition function and measurement
    %   function and assumes an additive noise model.
    %
    %   CKF = CubatureKalmanFilter(StateTransitionFcn, MeasurementFcn,
    ...
    %   for the state.
    %
    %   CKF = CubatureKalmanFilter(..., Name, Value) configures the
    %   cubature Kalman filter object properties, specified as one or more
    %   name-value pair arguments. Unspecified properties have default
    %   values.
    %
    %   Using the predict method:
    %
    %   [x_pred, P_pred] = predict(CKF) returns the prediction of state and
    ...
    %
    %   [x_pred, P_pred] = predict(CKF, varargin) additionally, lets you
    %   specify any parameters that will be passed to and used by the
    %   @StateTransitionFcn.
    %
    %   Using the correct method:
    %
    %   [x_corr, P_corr] = correct(CKF, z) returns the correction of the
    ...
    %
    %   [x_corr, P_corr] = correct(CKF, z, varargin) additionally, lets you
    %   specify any parameters that will be passed to and used by the
    %   @MeasurementFcn
    %
    %   Notes:
    ...
    %
    %      If the object tracked by CKF is detected
    %         predict(CKF);
    %         trackedLocation = correct(CKF, objectLocation);
    %      Else
    %         trackedLocation = predict(CKF);
    %      End
    %
    %   CubatureKalmanFilter methods:
    %
    ...
    %
    %   CubatureKalmanFilter properties:
    %
    ...
    %   References:
    %	[1] Ienkaran Arasaratnam and Simon Haykin, "Cubature Kalman Filters",
    %       IEEE Transactions on Automatic Control, pages 1254-1269, Volume:54,
    %       Issue:6, June 2009.
    ...
    properties(Dependent, Hidden)
        Alpha	% To use with UKF transformation
        Beta	% To use with UKF transformation
        Kappa	% To use with UKF transformation
    end
    ...
    methods
        ...
        % Constructor
        %----------------------------------------------------------------------
        function obj = CubatureKalmanFilter(varargin)
            %
            ...
            coder.internal.errorIf(firstNVIndex==2 || firstNVIndex>4, ...
                'shared_tracking:CubatureKalmanFilter:invalidInputsToConstructor', firstNVIndex-1);
            ...
            validateattributes(hasWrapping, {'numeric', 'logical'},...
                {'scalar','binary'},...
                'CubatureKalmanFilter', 'HasMeasurementWrapping');
            ...
            % Set the alpha, beta and kappa values with the correct type
            obj.pAlpha = cast(obj.constAlpha, obj.pDataType);
            obj.pBeta = cast(obj.constBeta, obj.pDataType);
            obj.pKappa = cast(obj.constKappa, obj.pDataType);
        end
        ...
        function [x_pred, P_pred] = predict(obj, varargin)
            ...
            %   The internal state and covariance of the cubature Kalman
            ...
            if obj.pIsFirstCallPredict
                ...
                coder.internal.errorIf(narginActual ~= narginExpected && ...
                    ...
                    'shared_tracking:CubatureKalmanFilter:invalidNumInputsToPredict',...
                    'StateTransitionFcn');
                % 3)
                if ~obj.pIsValidStateTransitionFcn
                    coder.internal.errorIf(...
                        obj.pPredictor.validateStateTransitionFcn(obj.StateTransitionFcn,obj.pState,obj.pW,varargin{:}),...
                        'shared_tracking:CubatureKalmanFilter:StateNonMatchingSizeOrClass',...
        ...
        function [x_corr, P_corr] = correct(obj, z, varargin)
            ...
            %   covariance of the cubature Kalman filter are overwritten
            ...
            coder.internal.errorIf(numel(obj) > 1, ...
                'shared_tracking:CubatureKalmanFilter:NonScalarFilter', ...
                'cubature Kalman filter', 'correct');
            ...
            if coder.internal.is_defined(obj.pN)
                matlabshared.tracking.internal.validateInputSizeAndType...
                    ('z', 'CubatureKalmanFilter', z, obj.pN);
            else
                matlabshared.tracking.internal.validateInputSizeAndType...
                    ('z', 'CubatureKalmanFilter', z);
            end
        ...
        function newCKF = clone(CKF)
            ...
            % properties
            newCKF = obj(...
                'HasAdditiveProcessNoise', CKF.HasAdditiveProcessNoise,...
                'HasAdditiveMeasurementNoise', CKF.HasAdditiveMeasurementNoise,...
                'DataType', CKF.pDataType, 'HasMeasurementWrapping', CKF.HasMeasurementWrapping);
            ...
            for kk = coder.unroll(1:numel(ppProperties))
                ...
                if coder.internal.is_defined(CKF.(ppProperties{kk}))
                    newCKF.(ppProperties{kk}) = CKF.(ppProperties{kk});
                end
            end
        end
        ...
        function setMeasurementSizes(obj, measurementSize, measurementNoiseSize)
            ...
            % measurement and measurement noise to N and V, respectively.
            % Use this function to set the expected sizes when working with
            % variable-sized measurements and before passing measurements
            % with new sizes to other object functions that use
            % measurements for example correct, distance, etc.
            validateattributes(measurementSize, {'numeric'}, {'real', ...
                'positive', 'integer', 'scalar'}, 'CubatureKalmanFilter');
    ...
    methods
        ...
        function set.State(obj, value)
            ...
            if coder.internal.is_defined(obj.pM)
                coder.internal.assert(isscalar(value) || numel(value)==obj.pM, ...
                    'shared_tracking:CubatureKalmanFilter:invalidStateDims', obj.pM);
            else
        ...
        function value = get.State(obj)
            ...
            coder.internal.assert(isempty(coder.target) || coder.internal.is_defined(obj.pState),...
                'shared_tracking:CubatureKalmanFilter:getUndefinedState');
        ...
        function set.StateCovariance(obj, value)
            ...
            validateattributes(value, ...
                ...
                'CubatureKalmanFilter', 'StateCovariance');
        ...
        function value = get.StateCovariance(obj)
            ...
            if isempty(coder.target)
                ...
                coder.internal.assert(coder.internal.is_defined(obj.pM),...
                    'shared_tracking:CubatureKalmanFilter:getUndefinedStateCovariance');
                stateCovarianceScalarExpandIfNecessary(obj);
                value = obj.pStateCovariance;
            end
        end
        ...
        function set.ProcessNoise(obj, value)
            validateattributes(value, ...
                {obj.pDataType}, ...
                {'real', 'finite', 'nonsparse', '2d', 'nonempty','square'},...
                'CubatureKalmanFilter', 'ProcessNoise');
        ...
        function value = get.ProcessNoise(obj)
            ...
            if isempty(coder.target)
                ...
                coder.internal.assert(coder.internal.is_defined(obj.pW),...
                    'shared_tracking:CubatureKalmanFilter:getUndefinedProcessNoise');
                processNoiseScalarExpandIfNecessary(obj);
                value = obj.pProcessNoise;
            end
        end
        ...
        function set.MeasurementNoise(obj, value)
            validateattributes(value, ...
                {obj.pDataType}, ...
                {'real', 'finite', 'nonsparse', '2d', 'nonempty', 'square'},...
                'CubatureKalmanFilter', 'MeasurementNoise');
        ...
        function value = get.MeasurementNoise(obj)
            ...
            if isempty(coder.target)
                ...
                coder.internal.assert(coder.internal.is_defined(obj.pV),...
                    'shared_tracking:CubatureKalmanFilter:getUndefinedMeasurementNoise');
                ensureMeasurementNoiseIsDefined(obj);
                value = obj.pMeasurementNoise;
            end
        end
        ...
        function set.StateTransitionFcn(obj, value)
            validateattributes(value, {'function_handle'},...
                {'nonempty'}, 'CubatureKalmanFilter', 'StateTransitionFcn');
            obj.pIsValidStateTransitionFcn = false;
        ...
        function set.MeasurementFcn(obj, value)
            validateattributes(value, {'function_handle'}, ...
                {'nonempty'}, 'CubatureKalmanFilter', 'measurementFcn');
            obj.pIsValidMeasurementFcn = false;
        ...
        function set.HasAdditiveProcessNoise(obj, value)
            validateattributes(value, {'numeric', 'logical'},...
                {'scalar','binary'},...
                'CubatureKalmanFilter', 'HasAdditiveProcessNoise');
            if value
        ...
        function set.HasAdditiveMeasurementNoise(obj, value)
            validateattributes(value, {'numeric', 'logical'},...
                {'scalar','binary'},...
                'CubatureKalmanFilter', 'HasAdditiveMeasurementNoise');
            if value
        ...
        function set.Alpha(obj,~)
            coder.internal.errorIf(true,'shared_tracking:CubatureKalmanFilter:ReadOnlyProperty','Alpha',class(obj));
        end
        %----------------------------------------------------------------------
        ...
        function set.Beta(obj,~)
            coder.internal.errorIf(true,'shared_tracking:CubatureKalmanFilter:ReadOnlyProperty','Beta',class(obj));
        end
        %----------------------------------------------------------------------
        ...
        function set.Kappa(obj,~)
            coder.internal.errorIf(true,'shared_tracking:CubatureKalmanFilter:ReadOnlyProperty','Kappa',class(obj));
        end
        %----------------------------------------------------------------------
    ...
    methods (Access = protected)
        ...
        %--------------------------------------------------------------------
        function sobj = saveobj(CKF)
            sobj = struct(...
                'HasAdditiveProcessNoise',     CKF.HasAdditiveProcessNoise, ...
                'StateTransitionFcn',          CKF.StateTransitionFcn, ...
                'HasAdditiveMeasurementNoise', CKF.HasAdditiveMeasurementNoise, ...
                'MeasurementFcn',              CKF.MeasurementFcn, ...
                'State',                       CKF.State, ...
                'StateCovariance',             CKF.StateCovariance, ...
                'ProcessNoise',                CKF.ProcessNoise, ...
                'MeasurementNoise',            CKF.MeasurementNoise, ...
                'pM',                          CKF.pM, ...
                'pN',                          CKF.pN, ...
                'pW',                          CKF.pW, ...
                'pV',                          CKF.pV, ...
                'pState',                      CKF.pState, ...
                'pSqrtStateCovariance',        CKF.pSqrtStateCovariance, ...
                'pSqrtStateCovarianceScalar',  CKF.pSqrtStateCovarianceScalar, ...
                'pSqrtProcessNoise',           CKF.pSqrtProcessNoise, ...
                'pSqrtProcessNoiseScalar',     CKF.pSqrtProcessNoiseScalar, ...
                'pSqrtMeasurementNoise',       CKF.pSqrtMeasurementNoise, ...
                'pSqrtMeasurementNoiseScalar', CKF.pSqrtMeasurementNoiseScalar, ...
                'pHasPrediction',              CKF.pHasPrediction, ...
                'pIsValidStateTransitionFcn',  CKF.pIsValidStateTransitionFcn, ...
                'pIsValidMeasurementFcn',      CKF.pIsValidMeasurementFcn, ...
                'pIsStateColumnVector',        CKF.pIsStateColumnVector,...
                'pDataType',                   CKF.pDataType,...
                'pIsFirstCallPredict',         CKF.pIsFirstCallPredict,...
                'pIsFirstCallCorrect',         CKF.pIsFirstCallCorrect, ...
                'pAlpha',					   CKF.pAlpha, ...
                'pBeta',					   CKF.pBeta, ...
                'pKappa',					   CKF.pKappa, ...
                'pCrossCov',                   CKF.pCrossCov,...
                'HasMeasurementWrapping',      CKF.HasMeasurementWrapping);
            % No need to save:
        ...
        function loadobjHelper(CKF,sobj)
            ...
            if ~isempty(sobj.StateTransitionFcn)
                CKF.StateTransitionFcn = sobj.StateTransitionFcn;
            end
            if ~isempty(sobj.MeasurementFcn)
                CKF.MeasurementFcn = sobj.MeasurementFcn;
            end
            
            % Tunable protected/private properties
            CKF.pHasPrediction             = sobj.pHasPrediction;
            CKF.pIsValidStateTransitionFcn = sobj.pIsValidStateTransitionFcn;
            CKF.pIsValidMeasurementFcn     = sobj.pIsValidMeasurementFcn;
            CKF.pIsFirstCallPredict        = sobj.pIsFirstCallPredict;
            CKF.pIsFirstCallCorrect        = sobj.pIsFirstCallCorrect;
            CKF.pAlpha                     = sobj.pAlpha;
            CKF.pBeta                      = sobj.pBeta;
            CKF.pKappa                     = sobj.pKappa;
            ...
            if ~isempty(sobj.pIsStateColumnVector)
                CKF.pIsStateColumnVector = sobj.pIsStateColumnVector;
            end
            if ~isempty(sobj.pState)
                CKF.pState = sobj.pState;
            end
            
            % Determine if object originated in the Square-root version
            if isfield(sobj,'pSqrtStateCovariance')
                loadSqrtObj(CKF,sobj);
            else
                loadNonSqrtObj(CKF,sobj);
            end
            
            % Load the pCrossCov property if exists
            if isfield(sobj,'pCrossCov') && ~isempty(sobj.pCrossCov)
                CKF.pCrossCov = sobj.pCrossCov;
            end
        ...
        function loadSqrtObj(CKF,sobj)
            ...
            if ~isempty(sobj.pSqrtStateCovariance)                 
                ...
                CKF.pSqrtStateCovariance = sobj.pSqrtStateCovariance;
            end

            if ~isempty(sobj.pSqrtProcessNoise)                 
                % isempty check is a must-have for pIsSetStateCovariance
                % being loaded correctly                
                CKF.pSqrtProcessNoise = sobj.pSqrtProcessNoise;
            end

            if ~isempty(sobj.pSqrtMeasurementNoise)                 
                % isempty check is a must-have for pIsSetStateCovariance
                % being loaded correctly                
                CKF.pSqrtMeasurementNoise = sobj.pSqrtMeasurementNoise;
            end
            ...
            % Scalar properties
            CKF.pSqrtStateCovarianceScalar = sobj.pSqrtStateCovarianceScalar;          
            CKF.pSqrtProcessNoiseScalar = sobj.pSqrtProcessNoiseScalar;          
            CKF.pSqrtMeasurementNoiseScalar = sobj.pSqrtMeasurementNoiseScalar;            
        end
        
        function loadNonSqrtObj(CKF,sobj)
            ...
            if ~isempty(sobj.pStateCovariance)
                % isempty check is a must-have for pIsSetStateCovariance
                % being loaded correctly
                CKF.pSqrtStateCovariance = matlabshared.tracking.internal.cholPSD(sobj.pStateCovariance);
            end
            
            if ~isempty(sobj.pProcessNoise)
                % isempty check is a must-have for pIsSetStateCovariance
                % being loaded correctly
                CKF.pSqrtProcessNoise = matlabshared.tracking.internal.cholPSD(sobj.pProcessNoise);
            end
            
            if ~isempty(sobj.pMeasurementNoise)
                % isempty check is a must-have for pIsSetStateCovariance
                % being loaded correctly
                CKF.pSqrtMeasurementNoise = matlabshared.tracking.internal.cholPSD(sobj.pMeasurementNoise);
            end
            ...
            if (sobj.pStateCovarianceScalar >= 0)
                CKF.pSqrtStateCovarianceScalar = matlabshared.tracking.internal.cholPSD(sobj.pStateCovarianceScalar);
            else
                CKF.pSqrtStateCovarianceScalar = sobj.pStateCovarianceScalar;
            end           
            
            if (sobj.pProcessNoiseScalar >= 0)
                CKF.pSqrtProcessNoiseScalar = matlabshared.tracking.internal.cholPSD(sobj.pProcessNoiseScalar);
            else
                CKF.pSqrtProcessNoiseScalar = sobj.pProcessNoiseScalar;
            end            
            
            if (sobj.pMeasurementNoiseScalar >= 0)
                CKF.pSqrtMeasurementNoiseScalar = matlabshared.tracking.internal.cholPSD(sobj.pMeasurementNoiseScalar);
            else
                CKF.pSqrtMeasurementNoiseScalar = sobj.pMeasurementNoiseScalar;
            end            
        end
        ...
        function ensureStateAndStateCovarianceIsDefined(obj)
            ...
            coder.internal.assert(coder.internal.is_defined(obj.pState) && ...
                ...
                'shared_tracking:CubatureKalmanFilter:UnknownNumberOfStates');
        ...
        function ensureProcessNoiseIsDefined(obj)
            ...
            coder.internal.assert(coder.internal.is_defined(obj.pW),...
                'shared_tracking:CubatureKalmanFilter:UnknownNumberOfProcessNoiseInputs');
        ...
        function validateMeasurementFcn(obj,z,fname,varargin)
            ...
            if ~obj.pIsValidMeasurementFcn
                coder.internal.errorIf(isempty(obj.MeasurementFcn),...
                    'shared_tracking:CubatureKalmanFilter:undefinedMeasurementFcn');
            end
            % 2)
            ...
            coder.internal.errorIf(narginActual >= 0 && ... %negative if varies
                ...
                'shared_tracking:CubatureKalmanFilter:invalidNumInputsToCorrect',...
                fname,'MeasurementFcn');
            % 3)
            if ~obj.pIsValidMeasurementFcn
                coder.internal.errorIf(...
                    obj.pCorrector.validateMeasurementFcn(obj,z,varargin{:}),...
                    'shared_tracking:CubatureKalmanFilter:MeasurementNonMatchingSizeOrClass',...
                    'MeasurementFcn');
    ...
    methods ( Access = {?matlabshared.tracking.internal.CubatureKalmanFilter, ?matlabshared.tracking.internal.UKFCorrector})
        function ensureMeasurementNoiseIsDefined(obj)
            ...
            coder.internal.assert(coder.internal.is_defined(obj.pV),...
                'shared_tracking:CubatureKalmanFilter:UnknownNumberOfMeasurementNoiseInputs');
    ...
    methods (Static)
        ...
        % correct object type has to be created. Note: the saveobj method
        % is inherited as is from the superclass.
        %--------------------------------------------------------------------
        function retCKF = loadobj(sobj)
            ...
            retCKF = matlabshared.tracking.internal.CubatureKalmanFilter...
