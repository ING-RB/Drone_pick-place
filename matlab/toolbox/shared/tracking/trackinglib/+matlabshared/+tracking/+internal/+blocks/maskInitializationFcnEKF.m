function [p,signalCheck] = maskInitializationFcnEKF(blkH,dlgParams)
% maskInitializationFcnEKF Mask initialization function for the EKF block
%
...
dlgParams.FcnInfo = ...
    ...
    'EKF', blkH, dlgParams.FcnInfo);

% Set the IO ports provided to the user based on the dialog settings
matlabshared.tracking.internal.blocks.configureBlockEKFUKF(blkH, 'EKF', dlgParams);
...
if strcmp(get_param(bdroot,'SimulationStatus'),'stopped')
    ...
    p = matlabshared.tracking.internal.blocks.processDialogDataEKF(dlgParams,p);
end
end