function d = EKFDistanceNonAdditive(EKF, z_matrix, measurementParams)
% EKFDistanceAdditive Computes distances between measurements and an
% extended Kalman filter object, for models with non-additive measurement
% noise.
...
vZeros = zeros(size(EKF.MeasurementNoise,1),1,classToUse);
if isempty(EKF.MeasurementJacobianFcn)
    dHdx = matlabshared.tracking.internal.numericJacobianNonAdditive(EKF.MeasurementFcn, EKF.State, vZeros, measurementParams, 1); 
    dHdv = matlabshared.tracking.internal.numericJacobianNonAdditive(EKF.MeasurementFcn, EKF.State, vZeros, measurementParams, 2); 
else
    [dHdx,dHdv] = EKF.MeasurementJacobianFcn(EKF.State, vZeros, measurementParams{:});
end

if ~isa(dHdx, classToUse)
    dHdx = cast(dHdx, classToUse);
    dHdv = cast(dHdv, classToUse);
end

if EKF.HasMeasurementWrapping
    [zEstimated, wrapping] = EKF.MeasurementFcn(EKF.State, vZeros, measurementParams{:});
    zEstimated = cast(zEstimated(:), classToUse);
    residualCovariance = dHdx * EKF.StateCovariance * dHdx' + dHdv * EKF.MeasurementNoise * dHdv.';
    ...
    zEstimated = EKF.MeasurementFcn(EKF.State, vZeros, measurementParams{:});
    zEstimated = cast(zEstimated(:), classToUse);
    residualCovariance = dHdx * EKF.StateCovariance * dHdx' + dHdv * EKF.MeasurementNoise * dHdv.';
end
