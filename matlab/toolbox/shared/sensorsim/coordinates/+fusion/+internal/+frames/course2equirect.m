function projCourse = course2equirect(radCourse, lla)
%COURSE2EQUIRECT converts course to its equirectangular geodetic projection
%
%   radCourse - Nx1 vector where course is expressed in radians clockwise
%               from "North"
%
...
Vnorth = cos(radCourse);
Veast = sin(radCourse);

% transform to equirectangular projection derivatives
dlat = Vnorth ./ (k * (N .*  clat  + dNbydlat .* slat) .* clat ...
                    - (N .* -slat  + dNbydlat .* clat) .* slat);
dlon = Veast ./ rho;

% report back projected angle
projCourse = atan2(dlon,dlat);