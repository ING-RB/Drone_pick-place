...
if mod(numel(varargin),2)==1
    coder.internal.prefer_const(varargin{end});
    iter = varargin{end};
else
    iter = coder.const(10);
end
...
if nargin<4
    ...
    [r,s] = cloth(iter, x, y, complex(varargin{1:2}));
end
...
function [r,s] = cloth(iter, x, y, varargin)
...
if isempty(coder.target)
    ...
    % Calculate final size of interpolated curve
    nUp = (n-1) * 2^iter + 1;
    Zout = zeros(1,nUp,'like',z);

    % Populate initial matrix
    Zout(1:n) = z;
    nCur = n;
    for i=1:iter
        % Add point between each pair of existing points and smooth curve
        Zout(1:(nCur*2-1)) = interpolate(Zout(1:nCur),z,args);
        nCur = nCur*2-1;
    end
end
...
function Zout = interpolate(zIn,z,args)
Zout = insert(zIn,args{:});
Zout = optimize(Zout,z,args{:});
Zout = optimize(Zout,z,args{:});
Zout = optimize(Zout,z,args{:});