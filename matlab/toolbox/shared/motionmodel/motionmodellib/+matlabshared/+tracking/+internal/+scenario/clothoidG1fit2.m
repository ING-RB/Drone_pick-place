...
iteeny = find(abs(d)<1e-9 & abs(a)<1e-9);
...
if ~isempty(iteeny)
    [z(iteeny),dz(iteeny)] = fresnelgSmallab1(a(iteeny),d(iteeny),c(iteeny));
end
...
function [z,dz] = fresnelgSmallab1(a,d,c)
% Evaluate:
%              /1
%              |
%        z  =  |   exp(i * (a*s.^2 + (d-a)*s + c)) ds
...
%              /0
%
% and the partial derivative dz/da by series expansion for small a,b.
...
%              /1
%              |
% z = exp(i*c) | exp(i*s * (a*s + b)) ds
%              |
%              /0
%
%               /1
% dz            |
% -- = exp(i*c) | i*s^2 * exp(i*s * (a*s + b))
% da            |
%               /0
%
%               Inf          n
%               __          __                            /1
%              \           \                              |
% z = exp(i*c)  >  i^n / n! > choose(n,k) * a^k * b^(n-k) | s^(n+k) ds
%              /__         /__                            |
%              n=0         k=0                            /0        
%
%               Inf          n
%               __          __
%              \           \                              
% z = exp(i*c)  >  i^n / n! > choose(n,k) * a^k * b^(n-k) / (n+k)
%              /__         /__
%              n=0         k=0
%
%                Inf          n
%                __          __
% dz             \           \                              
% -- = i*exp(i*c) >  i^n / n! > choose(n,k) * a^k * b^(n-k) / (n+k+2)
% da             /__         /__
%                n=0         k=0

n = 5;
m = length(a);
tbl1 = zeros(m,n,'like',1i);
tbl2 = zeros(m,n,'like',1i);
z = ones(m,1,'like',1i);
dz = z./3;

tbl1(:,1) = b;
tbl1(:,2) = a;
tbl1(:,1:2) = tbl1(:,1:2)*1i/1;
z = z + tbl1(:,1)/2 + tbl1(:,2)/3;
dz = dz + tbl1(:,1)/4 + tbl1(:,2)/5;

for k=2:n-1
    tbl2(:,1:k)   = tbl1(:,1:k).*a;
    tbl1(:,1:k)   = tbl1(:,1:k).*b;
    tbl1(:,2:k+1) = tbl1(:,2:k+1) + tbl2(:,1:k);
    tbl1(:,1:k+1) = tbl1(:,1:k+1)*1i/k;
    for j=1:k+1
        z = z + tbl1(:,j)/(j+k);
        dz = dz + tbl1(:,j)/(j+k+2);
    end
