...
% Copyright 2010-2024 The MathWorks, Inc.
  ...
  methods
    ...
    % title.
      textValue = moReporter.getTextValue();
      if ischar(textValue)
        joBodyText = moReporter.uddReport.createElement('phrase', textValue);
      else
        joBodyText = textValue; % is a node and formatted already, do not wrap around a phrase
      end
    end
    ...
    % reporter.
      ...
      if ~isempty(joTitle)
        ...
        joSpace = moReporter.uddReport.createTextNode(' ');
        joEmphasis.appendChild(joSpace);
        uddVarReport.appendChild(joEmphasis);
      end
      joBodyText = moReporter.makeReportBodyText();
     if(strcmpi(joBodyText.getNodeName, "#text"))
        joBodyText = moReporter.uddReport.createElement('phrase', joBodyText); 
        textStyle = createProcessingInstruction(moReporter.uddReport, 'db2dom', ...
        'style-name="rgDDVarText"');
        appendChild(joBodyText, textStyle);
        firstNode = joBodyText.getFirstChild();
        joBodyText.insertBefore(textStyle, firstNode);
      end
    ...
    % this reporter.
      ...
      pi = createProcessingInstruction(moReporter.uddReport, 'db2dom', ...
        'style-name="rgDDVarParagraph"');
      appendChild(joPara, pi);
      joPara.appendChild(joValue);     
      joTitle = moReporter.makeTitleText();
      if ~isempty(joTitle)
        joTitle = moReporter.uddReport.createElement('title', joTitle);
        pi = createProcessingInstruction(moReporter.uddReport, 'db2dom', ...
        'style-name="rgDDVarParagraphTitle"');
        appendChild(joTitle, pi);
    ...
    % data type.
      ...
      try
        ...
        tableStyle = createProcessingInstruction(moReporter.uddReport, 'db2dom', ...
        'style-name="rgDDVarTable"');
        appendChild(joTable, tableStyle);
        firstNode = joTable.getFirstChild();
        joTable.insertBefore(tableStyle, firstNode);
    ...
    function makeBackLink(moReporter, moLinkedReporter, postfix)
      ...
      joEmphasis.setAttribute('xml:space', 'preserve');
      moLinkedReporter.joParentLink = joEmphasis;
    end
