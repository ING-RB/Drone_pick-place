...
% Copyright 1997-2011 The MathWorks, Inc.
  ...
  methods
    ...
    function isFiltered = isFilteredProperty(this, variableObject, property)
        ...
        if (~isempty(this.moOpts.PropertyFilterCode))
            
            try
                variableName = this.ReportTitle; %#ok<NASGU> - these variables may be 
                propertyName = property.Name;    %#ok<NASGU> - used by the eval code below

                % NOTE: The code below will likely use the local variables "variableName", "variableObject",
                % "propertyName", so it is important to (a) keep them available and (b) keep their names the same
                eval(this.moOpts.PropertyFilterCode);
