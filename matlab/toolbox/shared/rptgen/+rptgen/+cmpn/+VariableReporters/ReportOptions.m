...
% Copyright 1997-2011 The MathWorks, Inc.
  ...
  properties
    ...
    % Dynamically determines which properties of structured variables
  ...
  methods
    ...
    function moOptions = ReportOptions(moOpts)
    % moOptions = ReportOptions(moOpts) constructs a report options
    ...
    % must match the option property names of this object.
      if nargin == 1 
        if ~isempty(moOpts)
          ...
          moOptions.PropertyFilterCode = moOpts.PropertyFilterCode;
        end
      end
    end
