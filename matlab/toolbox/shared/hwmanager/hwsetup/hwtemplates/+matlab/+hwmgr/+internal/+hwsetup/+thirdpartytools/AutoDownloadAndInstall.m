classdef AutoDownloadAndInstall < matlab.hwmgr.internal.hwsetup.TemplateBase
    % AUTODOWNLOADANDINSTALL - Template to guide users through the process of
    % automatically downloading and installing third-party tools.
    %
    % AutoDownloadAndInstall Properties:
    ...
    %
    % AutoDownloadAndInstall Methods(Inherited):
    ...
    properties (Access = private)
        % InstructionSets - Object representing the instruction set
        InstructionSets
    end

    methods
        function obj = AutoDownloadAndInstall(varargin)
            ...
            % get instructionset specific to this screen
    ...
    methods(Access = private)
        function createWidgets(obj)
            ...
            obj.ContentGrid.RowHeight = {'fit', 'fit', 'fit', 24, '1x'};
            ...
            installStatusTable.Column = [1, 3];
            installStatusTable.Row = row + 4;
            installStatusTable.Visible = 'off';
            installStatusTable.Border = 'off';
        ...
        function updateTpDownloadAndInstallTable(obj)
            ...
            installStatus = obj.Workflow.getInstallStatus(obj.InstructionSetNames);
            for indx = 1:length(installStatus)
                % Update status to 'Pass' for installed tools
                if installStatus{indx}
                    obj.TpDownloadAndInstallTable.Status{indx} = matlab.hwmgr.internal.hwsetup.StatusIcon.Pass;
                end
            end
            obj.NextButton.Enable = 'off';
            obj.InstallButton.Visible = 'on';

            if all([installStatus{:}])
                obj.NextButton.Enable = 'on';
                obj.InstallButton.Visible = 'off';
                obj.InstallDescriptionLabel.Text = getString(message('hwsetup:template:TpToolsAlreadyInstalled'));
            end
        end
    ...
    methods(Access = protected)
        function initializeWidgets(obj)
            ...
            obj.Title.Text = getString(message('hwsetup:template:AutoDownloadAndInstallTitle'));
            ...
            obj.updateTpDownloadAndInstallTable();
        end
        ...
        function downloadAndInstall(obj, ~, ~)
            %DOWNLOADANDINSTALL starts the download and install of
            %thirdparty tools

            try
                for i = 1:numel(obj.InstructionSets)
                    tpName = obj.TpDownloadAndInstallTable.Name{i};
                    if obj.InstructionSets{i}.isDownloadRequired
                        obj.setBusy(i);
                        obj.logMessage(['Downloading ' tpName]);
                        [status, msg] = obj.InstructionSets{i}.download();
                        if ~status                            
        ...
        function [status, msg ] = register(obj, instructionSetobj)
            %REGISTER - Registers a third-party tool by creating a 
            % registry file with installation details.
        ...
        function [status, msg] = createRegistryFile(obj, ...
                instructionSetobj, registryFileLocation, registryFileName)
            ...
            try
                ...
                % Write the installation location to the registry file.
                fprintf(fid, 'installLocation = %s', instructionSetobj.getInstallFolder);
                fclose(fid);

                % Create a file to indicate that the installation was done with HWSetup.
        ...
        function updateInstallStatusForSuccess(obj)
            %UPDATEINSTALLSTATUSFORSUCESS method to update the install
        ...
        function setSuccess(obj, idx)
            %SETSUCESS Set the pass icon in status and clear error
            % messages if any
