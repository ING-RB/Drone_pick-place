...
class CachedDistanceMap
{
  public:
	...
	{
	  distances_[i][j] = sqrt((double)i*i + j*j);
	}
      }
    }
    ~CachedDistanceMap()
	     ...
	     unsigned char* marked)
{
  ...
  unsigned int di = static_cast<unsigned int>(abs((double)i - src_i));
  unsigned int dj = static_cast<unsigned int>(abs((double)j - src_j));
