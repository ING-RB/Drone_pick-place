...
#ifdef _WIN32
#define DRAND_VAL linCongRand()
#else
#define DRAND_VAL drand48()
#endif
...
LIBRARY_API long long pf_global_seed = -1;
    ...
    pf_init_model_fn_t random_pose_fn, void *random_pose_data)
{
    ...
    // Preserve the original behavior if a seed is not specified
    if (pf_global_seed < 0)
    {
        pf_global_seed = time(NULL);
    }

#ifdef _WIN32
    setLinCongSeed((unsigned int)pf_global_seed);
#else
    srand48(pf_global_seed);
#endif
...
{	
	// Parts of this code is inspired by pf_update_resample (without resampling)
	pf_sample_set_t *set_a, *set_b;
	pf_sample_t *sample_a, *sample_b;
	int count;
	...
	// There are two sets. This logic gives the other set that is not the current set.
	set_b = pf->sets + (pf->current_set + 1) % 2;

	// Create the kd tree to compute statistics		
	pf_kdtree_clear(set_b->kdtree);
	...
	while (set_b->sample_count < set_a->sample_count)
	{			
		sample_a = set_a->samples + count++;
		sample_b = set_b->samples + set_b->sample_count++;
		sample_b->pose = sample_a->pose;
