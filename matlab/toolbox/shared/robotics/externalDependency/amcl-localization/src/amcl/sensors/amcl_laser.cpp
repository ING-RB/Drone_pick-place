...
#ifdef LIBRARY_API
// Force Visual Studio compiler to not optimize AMCLLaserData destructor into a scalar delete.
void ForceVectors()
{
  AMCLLaserData tempData;
  tempData.ranges = new double[2][2];
};
						     ...
						     temp_obs(NULL)
{
  ...
  this->max_beams = static_cast<int>(max_beams);
  this->map = map;
...
void 
AMCLLaser::SetModelBeam(double z_hit_in,
                        double z_short_in,
                        double z_max_in,
                        double z_rand_in,
                        double sigma_hit_in,
                        double lambda_short_in,
                        double chi_outlier_in)
{
  this->model_type = LASER_MODEL_BEAM;
  this->z_hit = z_hit_in;
  this->z_short = z_short_in;
  this->z_max = z_max_in;
  this->z_rand = z_rand_in;
  this->sigma_hit = sigma_hit_in;
  this->lambda_short = lambda_short_in;
  this->chi_outlier = chi_outlier_in;
}

void 
AMCLLaser::SetModelLikelihoodField(double z_hit_in,
                                   double z_rand_in,
                                   double sigma_hit_in,
                                   double max_occ_dist_in)
{
  ...
  map_update_cspace(this->map, max_occ_dist_in);
}

void 
AMCLLaser::SetModelLikelihoodFieldProb(double z_hit_in,
				       double z_rand_in,
				       double sigma_hit_in,
				       double max_occ_dist_in,
				       bool do_beamskip_in,
				       double beam_skip_distance_in,
				       double beam_skip_threshold_in, 
				       double beam_skip_error_threshold_in)
{
  this->model_type = LASER_MODEL_LIKELIHOOD_FIELD_PROB;
  this->z_hit = z_hit_in;
  this->z_rand = z_rand_in;
  this->sigma_hit = sigma_hit_in;
  this->do_beamskip = do_beamskip_in;
  this->beam_skip_distance = beam_skip_distance_in;
  this->beam_skip_threshold = beam_skip_threshold_in;
  this->beam_skip_error_threshold = beam_skip_error_threshold_in;
  map_update_cspace(this->map, max_occ_dist_in);
}
...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  for (j = 0; j < set->sample_count; j++)
  {
    ...
    for (i = 0; i < data->range_count; i += step)
    {
      ...
      int mi, mj;
      mi = static_cast<int>(MAP_GXWX(self->map, hit.v[0]));
      mj = static_cast<int>(MAP_GYWY(self->map, hit.v[1]));
...
double AMCLLaser::LikelihoodFieldModelProb(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  step = static_cast<int>(ceil((data->range_count) / static_cast<double>(self->max_beams))); 
...
void AMCLLaser::reallocTempData(int new_max_samples, int new_max_obs){
  ...
  max_obs = new_max_obs; 
  max_samples = std::max(max_samples, new_max_samples);
