...
nvPairs = { 'ReadVariableNames', true;
    ...
    'HeaderLines', 0;
    'FileEncoding','';
    'Encoding','';  % Use auto-charset detection
    'DateLocale','';
    'EndOfLine',CRLF;
    'CommentStyle',{};
    'TextType','char';
...
[readVarNames,readRowNames,delimiter,format,...
    ...
    commentstyle,texttype,datetimetype,durationtype,...
    preserveVariableNames,numheaderlines,varibleNamingRule,...
    supplied,otherArgs]...
...
if supplied.VariableNamingRule 
    varibleNamingRule = validatestring(varibleNamingRule,["modify","preserve"]);
    preserveVariableNames = (varibleNamingRule=="preserve");
...
BOM = matlab.io.text.internal.checkBOMFromFilename(file);
if ~isempty(BOM.Encoding)
    if (~supplied.FileEncoding && ~supplied.Encoding) || (fileEncoding == "system")
        % If no encoding was specified or 'system' was specified,
        % and the file contains a BOM, use the implied encoding.
        fileEncoding = BOM.Encoding;
    elseif ~isequal(fileEncoding, BOM.Encoding)
        % Alternatively, if an encoding was specified and it
        % conflicts with the supplied encoding, issue a warning
        matlab.io.internal.utility.warnOnBOMmismatch(BOM,fileEncoding);
    end
end
...
inferFormat = ~supplied.Format || strcmp(format,'auto');
...
try
    if ~supplied.Delimiter || ~supplied.HeaderLines || inferFormat
        ...
        if strcmp(eol, char([13 10]))
            detectIOargs.LineEnding = {char(13), newline, char([13 10])};
        else
        ...
        if supplied.DateLocale
            ...
            detectIOargs.DatetimeLocale = matlab.internal.datetime.getDefaults('locale');
        end

        file = matlab.io.internal.validators.validateFileName(file);
        % Choose the first match from the list of valid file names.
        file = file{1};

        emptyColType = 'double';
        if supplied.ReadVariableNames
            detectIOargs.ReadVariableNames = readVarNames;
        end
        opts = matlab.io.text.internal.getTextOpts(file,emptyColType,detectIOargs);
    ...
    if inferFormat
        ...
        format = guessFormat(fid, vnline, delimiter, whitespace, eol, treatAsEmpty, datetimetype, durationtype, detectIOargs.DatetimeLocale, opts, otherArgs);
    ...
    if inferFormat % read data using the detected format and update as it fails.
        ...
        if ~feof(fid)
            ...
            fseek(fid, startDataPosn, 'bof');
            format = updateFormatGuess(fid, format, delimiter, whitespace, eol, treatAsEmpty, numNonDataLines, datetimetype, durationtype, detectIOargs.DatetimeLocale, opts, otherArgs);
            
            if ~supplied.ReadVariableNames
                % May have detected no variable names, but then the format changed to all
                % strings. In that case, read variable names
                if matlab.io.text.internal.formatIsAllString(format)
                    readVarNames = true;
                    ...
                    readVarNames = matlab.io.text.internal.detectVariableNames(format,vnline,delimiter,whitespace,eol,otherArgs);
                    if ~readVarNames
                        fseek(fid,endHeaderPos,'bof');
...
try
    if readVarNames
        ...
        varNames = matlab.io.text.internal.determineVarNames(vnline,format,delimiter,whitespace,eol,true,otherArgs);
        ...
        if numel(varNames) ~= length(rawData)
            ...
            if ~isempty(id) && collectOutput % collect output is true.
                ...
                varNames = matlab.io.text.internal.determineVarNames([vnline eol],format,delimiter,whitespace,eol,true,otherArgs);
...
t = table.setReadtableMetaData(t, rawData, readRowNames, ...
...
%-------------------------------------------------------------------------------
function format = guessFormat(fid,vnline,delimiter,whitespace,eol,treatAsEmpty,datetimetype,durationtype,dtLocale,opts,otherArgs)
...
if isempty(dataLine{1}) || isempty(dataLine{1}{1})
    ...
    format = matlab.io.text.internal.determineFormatString(dataLine{1}{1}, ...
        delimiter, whitespace, eol, treatAsEmpty, opts.VariableTypes, datetimetype, durationtype, dtLocale, false, otherArgs);
end
end


%-------------------------------------------------------------------------------
function format = updateFormatGuess(fid,format,delimiter,whitespace,eol,treatAsEmpty,numNonDataLines,datetimetype,durationtype,dtLocale,opts,otherArgs)
...
while ~feof(fid)
    ...
    for jj = 1:numel(dataLines)
        ...
        if pos ~= length(dataLines{jj})
            format = matlab.io.text.internal.determineFormatString(dataLines{jj}, ...
                delimiter, whitespace, eol, treatAsEmpty, opts.VariableTypes, datetimetype, durationtype, dtLocale, false, otherArgs, format);
            skipValuesFormat(3:3:end) = format(2:2:end);
        end
...
if strcmp(enc,'system')
    fid = fopen(file,'rt','n',matlab.internal.i18n.locale.default.Encoding);% text mode: CRLF -> LF on windows (no-op on linux)
else
    fid = fopen(file,'rt','n',enc); % text mode: CRLF -> LF on windows (no-op on linux) 
end
if fid > -1
    fid = matlab.io.internal.utility.updateFIDforBOM(fid, enc);
end
end
%-------------------------------------------------------------------------------
...
for colID = find(tooShort)
    ...
    elseif iscell(data{colID})
        ...
        data{colID} = matlab.internal.datatypes.lengthenVar(data{colID},len);
    end
end
end
