...
classdef SupportPackageRegistryPluginBase < handle & matlab.mixin.Heterogeneous
    ...
    %   be found (and thereby execute any postInstallCmd, preUninstallCmd
    ...
    methods(Static)
        ...
        function spPkg = readSpPkgRegistry(sppkgxml)
            ...
            try
                ...
                domNode = parseFile(Parser,sppkgxml);
                ...
                % available
                if currpkg.item(0).hasAttribute('extrainfocheckboxcmd')
                ...
                % this to "hardware"
                supportCategory = '';
                if currpkg.item(0).hasAttribute('supportcategory')
                ...
                displayName       = char(currpkg.item(0).getAttribute('displayname'));
                 if isempty(displayName)
                    displayName = spPkg.Name;
                end
                spPkg.FwUpdateDisplayName = char(currpkg.item(0).getAttribute('fwupdatedisplayname'));
                % Format the display name as it appears in the dropdown
                % list in the targetupdater UI.
                spPkg.FwUpdateDisplayName = matlabshared.supportpkg.internal.getFwUpdateDisplayName(spPkg, displayName);
        ...
        function [spPkgBaseCodeCell,spPkgClassLocCell]  = getBaseCodesFromPluginClasses(pluginMetaClasses)
            ...
            % Cache of the data will be utilized to improve performance
            ...
            % initialize the cacheData, if empty
            if isempty(cacheData)
                cacheData = containers.Map('KeyType', 'char', 'ValueType', 'any');
            end
            ...
            for i = 1:numel(pluginMetaClasses)
                pluginMetaClassName = pluginMetaClasses(i).Name;
                % Check if basecode for the pluginMetaClass is already
                % cached
                if (cacheData.isKey(pluginMetaClassName))
                    cacheValue = cacheData(pluginMetaClassName);
                    spPkgBaseCodeCell{i} = cacheValue.BaseCode;
                    spPkgClassLocCell{i} = cacheValue.ClassLocation;
                else
                    spPkgBaseCodeCell{i} = pluginMetaClasses(i).PropertyList(...
                        strcmp({pluginMetaClasses(i).PropertyList.Name},'BaseCode')).DefaultValue;
                    spPkgClassLocCell{i} = which(pluginMetaClassName);
                    % add the basecode and class location to the cache
                    cacheData(pluginMetaClassName) = struct('BaseCode', spPkgBaseCodeCell{i}, ...
                        'ClassLocation', spPkgClassLocCell{i});
                end
            end
        end
