...
%VERTCAT Vertical concatenation of se3, so3, se2, or so2 arrays
%   C = VERTCAT(A,B) is called for the syntax '[A;B]' when A or B is an
%   object. The returns this vertical concatenation
...
if coder.const(~anyCell(varargin{:}))
    ...
    % Do a vertical concatenation of the indices. Since we need index tuples
    ...
    catTuples = vertcat(T1.MInd, T2.MInd*1i);
    ...
    for i = 3:n
        ...
        T2 = varargin{i};
        catTuples = vertcat(catMInd, T2.MInd*1i);
    ...
    % Input has cell arrays. Base vertcat handles it.
    v = cell(1,nargin);
    for ii=1:nargin
