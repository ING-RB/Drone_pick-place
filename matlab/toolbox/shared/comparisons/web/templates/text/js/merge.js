...
function merge (idOfDiffToMerge) {
    ...
    if (diffNode != null) {
      ...
      function () {
          ...
          diffNode.classList.add('mergedDiff');
...
function undo () {
    ...
    if (Math.abs(state.numLinesToIncrement) > 0) {
        ...
        var rightLineNum =
      parseInt(diffs[diffs.length - 1].getAttribute('data-right-line'));
        _changeRightFileLineNumbersAll(rightLineNum + 1,
            -state.numLinesToIncrement);
    }
    ...
    setPreformattedNodeText(document.getElementById('rightText'),
        state.rightText);
...
function _fireMergeActionFinished () {
    ...
    rootWindow.sendMessage('mergefinished', {
        Undos: UNDO_STACK.length,
        Redos: REDO_STACK.length,
        NumDifferences: NUM_DIFFS,
        SelectedDiffMerged: selectedDiffMerged
    });
}
...
function _insertLine (diffLine, newText, newLineNum) {
    ...
    var lineNumNode;
    if (hasHyperlink) {
        lineNumNode = _createOpenRightHyperlinkNode(newLineNum);
    } else {
        lineNumNode = _createNoLinkLineNumNode(newLineNum);
    }
...
function _createIdenticalTextNode (text) {
    var fullLineText =
    _widthAdjustText(text) + _SPACE + '.' + _SPACE + _widthAdjustText(text);
    return document.createTextNode(fullLineText);
}

function _widthAdjustText (text) {
    text = _replaceTabsWithSpaces(text);
...
function _replaceTabsWithSpaces (text) {
    ...
    return text.replace(
        /\t/g, function (matchedString, indexOfTabInOriginalString) {
            var indexOfCharacterAfterTab =
        indexOfTabInOriginalString + offsetFromPreviousReplaceOperations + 1;
            var numCharactersUntilNextTabLocation =
        indexOfCharacterAfterTab % NUM_SPACES_PER_TAB;
            // + 1 as a replacement for the tab character
            var numSpacesToAdd =
        NUM_SPACES_PER_TAB - numCharactersUntilNextTabLocation + 1;
            if (numSpacesToAdd !== 0) {
...
function _incrementRightFileLineNumbers (fromNode, lineNum) {
    ...
    var nodesToIncrement =
    nodeArray.slice(nodeArray.indexOf(fromNode), nodeArray.length);
...
function _changeRightVisibleLineNumber (node, num) {
    ...
    var newLineNumNode;
    if (hasHyperlink) {
        newLineNumNode = _createOpenRightHyperlinkNode(num);
    } else {
        newLineNumNode = _createNoLinkLineNumNode(num);
    }
...
function getElementsByClassName (nodeToSearchFrom, className) {
    var elements = [];
    nodeToSearchFrom.querySelectorAll('.' + className).forEach(function (elem) {
        elements.push(elem);
    });
    return elements;
}
...
function getFileNodeTextById (id) {
    ...
    return text.substring(getTextWorkaroundPrefix.length,
        text.length - getTextWorkaroundSuffix.length);
}
...
fireSelectionChanged () {
    ...
    if (CURRENT_SECTION) {
        ...
        var classes = CURRENT_SECTION.className;
        merged = classes && (classes.search('merged') >= 0);
    }
    rootWindow.sendMessage('selectionchanged',
        { CurrentSelection: sectionId, Merged: merged });
}
