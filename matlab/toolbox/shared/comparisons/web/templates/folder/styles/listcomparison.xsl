...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="FileListEditScript">
        ...
        <script language="javascript">
            ...
            function setQueueLength() {
                var pending = document.querySelectorAll("span[status='pending']");
        ...
        <div id="queueinfoholder">
            <p id="queueinfo"><xsl:value-of select="msg:format0('QueueLength')"/>
                ...
                <span class="action">
                    <xsl:attribute name="onclick">MATLAB.feval('comparisons_private', ['skip', '<xsl:value-of select="@id"/>'], 0);</xsl:attribute>
                    <xsl:value-of select="msg:format0('Skip')"/>
                </span>&nbsp;&nbsp;&nbsp;&nbsp;
                <!-- "Cancel All" hyperlink -->
                <span class="action">
                    <xsl:attribute name="onclick">MATLAB.feval('comparisons_private', ['cancel','<xsl:value-of select="@id"/>'], 0);</xsl:attribute>
    ...
    <xsl:template name="openhyperlink">
        ...
        <span class="action">
            <xsl:attribute name="onclick">MATLAB.feval('comparisons_private', ['view', '<xsl:value-of select="/FileListEditScript/@id"/>', '<xsl:value-of select="$side"/>', '<xsl:value-of select="."/>'], 0);</xsl:attribute>
    ...
    <xsl:template name="comparehyperlink">
        ...
        (<span class="action">
            ...
            <xsl:attribute name="onclick">MATLAB.feval('comparisons_private', ['compare', '<xsl:call-template name="reportid"/>', '<xsl:value-of select="."/>'], 0)</xsl:attribute>
            <xsl:value-of select="$string"/>
         </span>)
    </xsl:template>
        ...
        <td>
            ...
            <span>
                <xsl:attribute name="status">
                    <xsl:choose>
                        <xsl:when test="@contentsMatch='queued'">pending</xsl:when>
                        <xsl:when test="@contentsMatch='comparing'">pending</xsl:when>
