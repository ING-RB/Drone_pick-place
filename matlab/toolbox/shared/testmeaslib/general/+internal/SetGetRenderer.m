...
%% Constructor
    ...
    methods (Hidden = true)
        ...
        %of handles to MATLAB objects.
            ...
            obj.dispRender(false,@internal.SetGetRenderer.renderValueForGet);
        end
        ...
        %of handles to MATLAB objects.
            ...
            obj.dispRender(true,@internal.SetGetRenderer.renderValueForSet);
        end
    ...
    methods (Access=protected)
        function dispRender(obj,showOnlySettableProperties,fcnValueRenderer)
        ...
        %indicates to the correct renderer for a given situation.
            ...
            propertiesToShow = metaObj.PropertyList;

            % Select the publicly accessible gettable properties (note this
            % means that we cannot show write only properties
            propertiesToShow = propertiesToShow(strcmp('public',{propertiesToShow.GetAccess}));

            if showOnlySettableProperties
                % G642515: Select the publicly accessible settable properties
                propertiesToShow = propertiesToShow(strcmp('public',{propertiesToShow.SetAccess}));
            end

            % Filter out hidden properties
            propertiesToShow = propertiesToShow(~[propertiesToShow.Hidden]);

            % If there's no properties to show, return a message to that
            % effect.
            if isempty(propertiesToShow)
                if showOnlySettableProperties
                    msgID = 'testmeaslib:SetGetRenderer:noSettableProperties';
                else
                    msgID = 'testmeaslib:SetGetRenderer:noGettableProperties';
                end
                messageToDisplay = message(msgID,class(obj));
                disp(messageToDisplay.getString())
                return
            end

            propertyNames = {propertiesToShow.Name};
