function info = socketcanlist()
% socektcanlist Return information on available SocketCAN device channels.
%
%   INFO = SOCKETCANLIST() returns information about available SocketCAN devices as a table.
%
%   Example:
%       info = socketcanlist()
...
arch = computer('arch');
devicePath = fullfile(toolboxdir(fullfile('shared','testmeaslib','hwutils','vnt')), 'private', arch, 'socketcanlistplugin');
...
% Catch asyncio Channel creation errors if the device drivers are not available.
% Return empty info table if asyncio Channel creation errors out.
try
...
for ii = 1:numel(asyncioChannel.VendorDetails)
    % Include canInterfaces which are brought online from the detected list.
