...
const std::vector<size_t> MemoryConfig::DEFAULT_SIZE_LEVELS = {
    8 * MEGA_BYTE, 16 * MEGA_BYTE, 64 * MEGA_BYTE, 256 * MEGA_BYTE, 1024 * MEGA_BYTE};
...
void MemoryConfig::update(size_t lastPoolSize,
                          MallocMode lastMode,
                          bool startFromCurrentSizeLevel) {
    size_t& sizeLevelIndex =
        (lastMode == DISCRETE ? fDiscreteSizeLevelIndex : fUnifiedSizeLevelIndex);
    ...
    std::vector<size_t>::iterator it = std::upper_bound(
        fSizeLevels.begin() + static_cast<std::vector<size_t>::difference_type>(offset),
        fSizeLevels.end(), lastPoolSize);
    sizeLevelIndex =
        (it == fSizeLevels.end() ? fSizeLevels.size() - 1
                                 : static_cast<size_t>(std::distance(fSizeLevels.begin(), it)));
}

size_t MemoryConfig::roundUpPoolSizeWithLevel(size_t size, MallocMode mode) const {
    const size_t sizeLevelIndex =
        mode == DISCRETE ? fDiscreteSizeLevelIndex : fUnifiedSizeLevelIndex;
    // Find the index of the first pool size that's greater than size
    std::vector<size_t>::const_iterator it = std::upper_bound(
        fSizeLevels.begin() + static_cast<std::vector<size_t>::difference_type>(sizeLevelIndex),
        fSizeLevels.end(), size);
...
ActionLogger::ActionLogger(const std::string& fileName)
...
void ActionLogger::logAction(Action action) {
    ...
    const std::string& mode =
        action.type == ALLOCATE ? getMallocModeString(action.mode) : NA_STRING;
    logFile << type << DELIMITER << action.var << DELIMITER << action.size << DELIMITER << mode
            << '\n';
}
...
void ActionLogger::writeFileHeader() {
    if (!fHeaderWritten) {
        ...
        // Write header line
        logFile << "Action Type" << DELIMITER << "Variable ID" << DELIMITER << "Size" << DELIMITER
                << "Malloc Mode" << '\n';
        fHeaderWritten = true;
    }
}
...
cudaError_t CudaAllocator::rawMalloc(void** devPtr, size_t size, MallocMode mode) {
    ...
    if (gEnableGpuRuntimeLog) {
        ...
        std::stringstream ss;
        ss << "rawMalloc  CudaAllocator:      " << " devPtr: " << std::setw(13) << *devPtr
           << " size: " << getSizeString(size) << " mode: " << std::setw(10) << mallocModeStr;
...
std::ostream& operator<<(std::ostream& ss, const MemoryBlock& block) {
    ss << std::setw(13) << &block << " prev: " << std::setw(13) << (void*)(block.prev())
       << " next: " << std::setw(13) << (void*)(block.next()) << " data: " << block.data()
       << " size: " << getSizeString(block.size()) << " allocated: " << block.isAllocated()
...
///////////////////////////////// Memory Pool /////////////////////////////////
MemoryPool::MemoryPool(void* data,
                       size_t size,
                       Allocator& allocator,
...
bool MemoryPool::hasAllocatedBlocks() const {
    ...
    bool allFree = fFreeBlocks.size() == 1 && block->data() == fData && block->size() == fSize;
    return !allFree;
}
...
#ifndef MW_SINGLETON_MEMORY_MANAGER
        // When the singleton memory manager is used, the destructor runs when MATLAB exits.
        // We suppress warnings in this case.
        internalMessage("Found allocated GPU memory resources. Freeing all associated GPU memory.",
                        false);
#endif
    }
    deletePools();
...
size_t MemoryManager::getTotalPoolSize() const {
    size_t bytes = 0;
    for (PoolList::const_iterator iter = fDiscretePools.begin(); iter != fDiscretePools.end();
         ++iter) {
        MemoryPool* pool = *iter;
        bytes += pool->size();
    }
    for (PoolList::const_iterator iter = fUnifiedPools.begin(); iter != fUnifiedPools.end();
         ++iter) {
        MemoryPool* pool = *iter;
        bytes += pool->size();
    }
...
cudaError_t MemoryManager::allocatePool(size_t size, MallocMode mode, MemoryPool*& pool) {
    // Allocate raw memory
    void* data = nullptr;
...
MemoryPool* MemoryManager::getFirstSuitableMemoryPool(size_t size, MallocMode mode) const {
    ...
    for (PoolList::const_iterator iter = pools.begin(); iter != pools.end(); ++iter) {
        MemoryPool* pool = *iter;
...
void MemoryManager::freeEmptyPools() {
    ...
    fConfig.update(maxDiscretePoolSize, MallocMode::DISCRETE,
                   /*startFromCurrentSizeLevel = */ false);
...
class Container {
  public:
    Container() {}
    ~Container();
    void setInstance(const gcmemory::MemoryConfig& config, bool loggingEnabled);
...
void Container::setInstance(const gcmemory::MemoryConfig& config, bool loggingEnabled) {
...
DLL_EXPORT cudaError_t gcmemory::mwCudaMallocImpl(void** devPtr, size_t size) {
    gcmemory::MemoryManager& manager = gcmemory::getMemoryManager();
    return manager.allocate(devPtr, size, gcmemory::DISCRETE);
}

DLL_EXPORT cudaError_t gcmemory::mwCudaMallocManagedImpl(void** devPtr, size_t size) {
    gcmemory::MemoryManager& manager = gcmemory::getMemoryManager();
    return manager.allocate(devPtr, size, gcmemory::UNIFIED);
}

DLL_EXPORT cudaError_t gcmemory::mwCudaFreeImpl(void* devPtr) {
    gcmemory::MemoryManager& manager = gcmemory::getMemoryManager();
