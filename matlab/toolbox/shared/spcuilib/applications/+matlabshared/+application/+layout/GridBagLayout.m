classdef GridBagLayout < matlabshared.application.layout.AbstractLayout
    %GridBagLayout - Constructs a GridBagLayout layout manager.
    %   matlabshared.application.layout.GridBagLayout(H) Constructs a
    %   GridBagLayout object to manage the Handle Graphics object H.  H can
    %   be a figure, uicontainer or uipanel.
    %
    ...
    %
    %   % Example
    %       hl = matlabshared.application.layout.GridBagLayout(figure);
    ...
    methods
        ...
        function this = GridBagLayout(hPanel, varargin)
            ...
            this@matlabshared.application.layout.AbstractLayout(hPanel);
        ...
        function add(this, h, row, col, varargin)
            ...
            add@matlabshared.application.layout.AbstractLayout(this, h, row, col);
            ...
            if ~isappdata(h, this.CONSTRAINTSTAG)
                setappdata(h, this.CONSTRAINTSTAG, matlabshared.application.layout.GridBagConstraints);
            end
        ...
        function insert(this, type, indx)
            ...
            switch lower(type)
                case 'row'
                    ...
                    vw = [vw(1:indx-1); 0; vw(indx:end)];
                    this.VerticalWeights = vw;
                case 'column'
                    if indx > cols + 1
                    ...
                    hw = [hw(1:indx-1) 0 hw(indx:end)];
                    this.HorizontalWeights = hw;
            end
        ...
        function removeAll(this)
            %removeAll Clear the entire grid
            this.Grid = [];
        end
        
        function clean(this)
            %clean   Remove rows/columns with only NaN values.
            %
            %   See also add, insert, remove.
            
            g = this.Grid;
            
            % Clean up any rows that are all nan.
            g(all(isnan(g), 2), :) = [];
            
            % Clean up any columns that are all nan.
            g(:, all(isnan(g), 1)) = [];
            
            this.Grid = g;
        end
        
        function b = contains(this, h)
            %contains  Returns true if the layout is managing the widget
            
            b = any(any(this.Grid == double(h)));
        end
        ...
        function setConstraints(this, row, col, varargin)
            ...
            if strcmpi(varargin{1}, 'default')
                ...
                if isempty(oldConstraints)
                    c = matlabshared.application.layout.GridBagConstraints(varargin{:});
            ...
            updateForNewConstraints(this);
        end
    end
    
    methods (Access = protected)

        function updateForNewConstraints(this)
            update(this);
        end
        ...
        function layout(this)
            ...
            panelpos     = getPanelPosition(this);
            ctag         = this.CONSTRAINTSTAG;
            [rows, cols] = size(grid);
            ...
            [~, ~, minwidth, minheight] = getMinimumSize(this);
    ...
    methods
        ...
        function [totalWidth, totalHeight, width, height] = getMinimumSize(this)
            grid   = this.Grid;
            height = zeros(size(grid));
            width  = height;
            ctag   = this.CONSTRAINTSTAG;
            [rows, cols] = size(grid);
            ...
            for indx = 1:rows
                for jndx = 1:cols
                    if ishghandle(grid(indx,jndx))
                        ...
                        height(indx:indx+n-1,jndx) = minh;
                        width(indx,jndx:jndx+m-1)  = minw;
                    end
                end
            end
            ...
            totalWidth = sum(width) + this.HorizontalGap * (size(grid, 2) + 1);
            totalHeight = sum(height) + this.VerticalGap * (size(grid, 1) + 1);
        end
        ...
        function set.VerticalGap(this, vGap)
            ...
            if ~isscalar(vGap) || ~isnumeric(vGap) || isnan(vGap) || isinf(vGap)
                error(message('Spcuilib:application:ScalarNumericTypeError', 'VerticalGap'))
            end
        ...
        function set.HorizontalGap(this, hGap)
            ...
            if ~isscalar(hGap) || ~isnumeric(hGap) || isnan(hGap) || isinf(hGap)
                error(message('Spcuilib:application:ScalarNumericTypeError', 'HorizontalGap'))
            end
        ...
        function set.VerticalWeights(this, vWeights)
            if ~any(isnumeric(vWeights)) || any(isnan(vWeights)) || any(isinf(vWeights))
                error(message('Spcuilib:application:ScalarNumericTypeError', 'VerticalWeights'))
            end
        ...
        function set.HorizontalWeights(this, hWeights)
            if ~any(isnumeric(hWeights)) || any(isnan(hWeights)) || any(isinf(hWeights))
                error(message('Spcuilib:application:ScalarNumericTypeError', 'HorizontalWeights'))
            end
    ...
    methods (Hidden)
        function setLayoutHeight(this, height)
            if nargin < 2
                [~, height] = getMinimumSize(this);
            end
            this.setConstraints(this.Panel, ...
                'Fill', 'Both', ...
...
weights = weights(:);
weights = [weights; zeros(nw-length(weights), 1)];
weights = weights(1:nw);

if dimension == 1
    weights = reshape(weights, nw, 1);
else
    weights = reshape(weights, 1, nw);
end
