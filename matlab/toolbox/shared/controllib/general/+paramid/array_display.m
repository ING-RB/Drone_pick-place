...
%Check whether we can include html in display
showHTML = desktop('-inuse') && feature('hotlinks');
...
k = 0;
% Methods
if showHTML && ~isempty( methods(cls) )
   str = sprintf( '<a href="matlab:methods(''%s'')">methods</a>', cls );
   k = k + 1;
end
% Superclasses
if showHTML && ~isempty( superclasses(cls) )
   if ~isempty(str)
      str = [str ','];
   end
...
% Events
if showHTML && ~isempty( events(cls) )
   if ~isempty(str)
      str = [str ','];
   end
