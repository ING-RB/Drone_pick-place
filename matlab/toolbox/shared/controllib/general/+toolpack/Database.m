classdef Database < handle
  % Named database for custom data storage.
  ...
  properties (Dependent, GetAccess = public, SetAccess = private)
    % Database name (read-only string, default = '')
    Name
  end
  
  properties (Dependent, Access = public)
    % Database data (any MATLAB data type, default = []).
    Data
  end
  ...
  methods (Access = public)
    function this = Database(name)
      % Creates a named database.
      %
      % Example: obj = toolpack.Database('my_wkspc')
      if nargin < 1
        % Default argument
        name = '';
      end
