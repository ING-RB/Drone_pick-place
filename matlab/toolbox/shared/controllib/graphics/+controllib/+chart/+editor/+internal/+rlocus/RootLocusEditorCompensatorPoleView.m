classdef RootLocusEditorCompensatorPoleView < controllib.chart.internal.view.characteristic.BaseCharacteristicView & ...
    ...
    %% Constructor
    methods
        function this = RootLocusEditorCompensatorPoleView(responseView,data)
    ...
    methods (Access = protected)
        function build_(this)
            this.PoleMarkers = createGraphicsObjects(this,"scatter",1,1,this.Response.NResponses,Tag='RootLocusTunablePoleScatter');
        ...
        function updateData(this,~,~,ka)
            ...
            pzData = this.Response.ResponseData.RootLocusCompensatorPoles;
        ...
        function removePole(this,source)
            arguments
                this (1,1) controllib.chart.editor.internal.rlocus.RootLocusEditorCompensatorPoleView
            ...
            if strcmp(fig.SelectionType,"normal")
                ...
                [~,idx] = min(dist);
                pr = find(response.Compensator.P{1,1} == real(response.Compensator.P{1,1}));
                pidx = pr(idx);
                pole = response.Compensator.P{1,1}(pidx);
                response.Compensator.P{1,1}(pidx) = [];
                if this.Response.IsDiscrete
                    response.Compensator.K(1,1) = response.Compensator.K(1,1)/(1-pole);
                else
                    response.Compensator.K(1,1) = -response.Compensator.K(1,1)/pole;
                end
        ...
        function movePole(this,source,action,optionalInputs)
            ...
            persistent WML WBMClear location ptr idx pidx;
            ax = ancestor(source,'axes');
            fig = ancestor(ax,'figure');
            if strcmp(fig.SelectionType,"normal")
                response = getResponse(this.Response);
                poleData = this.Response.ResponseData.RootLocusCompensatorPoles;
                switch action
                    case 'init'
                    ...
                    case 'move'
                        ...
                        location = 1/timeConversionFcn(1/newPoint(1));
                        response.Compensator.P{1,1}(pidx) = location;
                        if this.Response.IsDiscrete
                            response.Compensator.K(1,1) = (1-location)/(1-oldLocation)*response.Compensator.K(1,1);
                        else
                            response.Compensator.K(1,1) = location/oldLocation*response.Compensator.K(1,1);
                        end
