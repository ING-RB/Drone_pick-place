classdef NicholsEditorCompensatorComplexConjugateZeroView < controllib.chart.internal.view.characteristic.FrequencyCharacteristicView & ...
    ...
    %% Constructor
    methods
        function this = NicholsEditorCompensatorComplexConjugateZeroView(responseView,data)
    ...
    methods (Access = protected)
        function build_(this)
            this.ZeroMarkers = createGraphicsObjects(this,"scatter",this.Response.NRows,...
                this.Response.NColumns,this.Response.NResponses,Tag='NicholsTunableCCZeroScatter');
            weakThis = matlab.lang.WeakReference(this);
            set(this.ZeroMarkers,ButtonDownFcn=@(es,ed) moveZero(weakThis.Handle,es,'init'));
        ...
        function updateData(this,ko,ki,ka)
            pzData = this.Response.ResponseData.NicholsCompensatorComplexConjugateZeros;
        ...
        function updateStyle_(this,~,ko,ki,ka)
            if this.IsInitialized
                set(this.ZeroMarkers(ko,ki,ka),Marker='o',LineWidth=1.5*get(groot,"DefaultLineLineWidth"),MarkerFaceColor='none');
        ...
        function removeZero(this,source)
            arguments
                this (1,1) controllib.chart.editor.internal.nichols.NicholsEditorCompensatorComplexConjugateZeroView
            ...
            if strcmp(fig.SelectionType,"normal")
                ...
                zeroData = this.Response.ResponseData.NicholsCompensatorComplexConjugateZeros;
        ...
        function moveZero(this,source,action,optionalInputs)
            ...
            persistent WML WBMClear location ptr ko ki idx zidx zcidx;
            ax = ancestor(source,'axes');
            fig = ancestor(ax,'figure');
            if strcmp(fig.SelectionType,"normal")
                response = getResponse(this.Response);
                ...
                switch action
                    case 'init'
                        ...
                        computeZeroLocus(this,w,zeta,ko,ki,zidx);
        ...
        function computeZeroLocus(this,w,zeta,ko,ki,zidx)
            ...
            Hshift = (2*zeta*w^2*1j)./(w^2+2*zeta*w*freqs*1j-freqs.^2);
            ...
            ph = phaseConversionFcn(angle(Hshift));
            m = abs(Hshift);            
            phaseLociDamp = this.ZeroMarkers(ko,ki,this.Response.NominalIndex).XData(zidx)+ph;
            mags = magConversionFcnInv(this.ZeroMarkers(ko,ki,this.Response.NominalIndex).YData(zidx));
            magLociDamp = magConversionFcn(mags.*m);

            this.ZeroLocus(ko,ki,this.Response.NominalIndex).XData = [phaseLociFreq NaN phaseLociDamp];
            this.ZeroLocus(ko,ki,this.Response.NominalIndex).YData = [magLociFreq NaN magLociDamp];
            this.ZeroLocus(ko,ki,this.Response.NominalIndex).UserData.Frequency = [freqs NaN w*ones(size(freqs))];
            this.ZeroLocus(ko,ki,this.Response.NominalIndex).UserData.Damping = [zeta*ones(size(freqs)) NaN damps];
