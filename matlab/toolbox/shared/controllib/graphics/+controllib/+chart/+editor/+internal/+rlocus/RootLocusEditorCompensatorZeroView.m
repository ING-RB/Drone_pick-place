classdef RootLocusEditorCompensatorZeroView < controllib.chart.internal.view.characteristic.BaseCharacteristicView & ...
    ...
    %% Constructor
    methods
        function this = RootLocusEditorCompensatorZeroView(responseView,data)
    ...
    methods (Access = protected)
        function build_(this)
            this.ZeroMarkers = createGraphicsObjects(this,"scatter",1,1,this.Response.NResponses,Tag='RootLocusTunableZeroScatter');
        ...
        function updateData(this,~,~,ka)
            ...
            pzData = this.Response.ResponseData.RootLocusCompensatorZeros;
        ...
        function removeZero(this,source)
            arguments
                this (1,1) controllib.chart.editor.internal.rlocus.RootLocusEditorCompensatorZeroView
            ...
            if strcmp(fig.SelectionType,"normal")
                ...
                [~,idx] = min(dist);
                zr = find(response.Compensator.Z{1,1} == real(response.Compensator.Z{1,1}));
                zidx = zr(idx);
                zero = response.Compensator.Z{1,1}(zidx);
                response.Compensator.Z{1,1}(zidx) = [];
                if this.Response.IsDiscrete
                    response.Compensator.K(1,1) = response.Compensator.K(1,1)*(1-zero);
                else
                    response.Compensator.K(1,1) = -response.Compensator.K(1,1)*zero;
                end
        ...
        function moveZero(this,source,action,optionalInputs)
            ...
            persistent WML WBMClear location ptr idx zidx;
            ax = ancestor(source,'axes');
            fig = ancestor(ax,'figure');
            if strcmp(fig.SelectionType,"normal")
                response = getResponse(this.Response);
                zeroData = this.Response.ResponseData.RootLocusCompensatorZeros;
                switch action
                    case 'init'
                    ...
                    case 'move'
                        ...
                        response.Compensator.Z{1,1}(zidx) = location;
                        if this.Response.IsDiscrete
                            response.Compensator.K(1,1) = (1-oldLocation)/(1-location)*response.Compensator.K(1,1);
                        else
                            response.Compensator.K(1,1) = oldLocation/location*response.Compensator.K(1,1);
                        end
