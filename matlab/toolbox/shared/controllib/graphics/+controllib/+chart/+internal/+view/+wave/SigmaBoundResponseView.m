classdef SigmaBoundResponseView < controllib.chart.internal.view.wave.BaseResponseView & ...
    ...
    %% Constructor
    methods
        function this = SigmaBoundResponseView(response,optionalInputs)            
            arguments
                response (1,1) controllib.chart.response.internal.SigmaBoundResponse
            ...
            this.NPatches = size(response.ResponseData.SingularValue{1},1);
            build(this);
        end
    end
    ...
    methods
        ...
        function set.FrequencyScale(this,FrequencyScale)
            arguments
                this (1,1) controllib.chart.internal.view.wave.SigmaBoundResponseView
            ...
            for k = 1:this.NPatches
                for ka = 1:this.Response.NResponses
                    ...
                    switch this.Response.BoundType
                        case {"SBound" "TBound"}
                            br = find(isnan(w));
                            wBound = cell(length(br),1);
                            svBound = cell(length(br),1);
                            N = -inf;
                            for ii = 1:length(br)
                                if ii == 1
                                    wBound{ii} = w(1:br(ii)-1);
                                    svBound{ii} = sv(1:br(ii)-1);
                                else
                                    wBound{ii} = w(br(ii-1)+1:br(ii)-1);
                                    svBound{ii} = sv(br(ii-1)+1:br(ii)-1);
                                end
                                ixp = find(wBound{ii}>0);
                                ixn = find(wBound{ii}<0);
                                if this.Response.ResponseData.IsReal(ka)
                                    wBound{ii} = [-flipud(wBound{ii}(ixp,:));wBound{ii}(ixp,:)];
                                    svBound{ii} = [fliplr(svBound{ii}(:,ixp)) svBound{ii}(:,ixp)];
                                else
                                    wBound{ii} = [wBound{ii}(ixn,:);wBound{ii}(ixp,:)];
                                    svBound{ii} = [svBound{ii}(:,ixn) svBound{ii}(:,ixp)];
                                end
                                switch this.FrequencyScale
                                    case "log"
                                        if ~this.Response.ResponseData.IsReal(ka)
                                            wBound{ii} = abs(wBound{ii});
                                            ...
                                            svBound{ii} = svBound{ii}(wBound{ii}>0);
                                            wBound{ii} = wBound{ii}(wBound{ii}>0);
                                ...
                                N = max(N,length(wBound{ii}));
                            end
                            w = zeros(N,length(br));
                            sv = zeros(N,length(br));
                            for ii = 1:length(br)
                                n = length(wBound{ii});
                                w(:,ii) = [wBound{ii};NaN(N-n,1)];
                                sv(:,ii) = [svBound{ii}';NaN(N-n,1)];
                            end
                        otherwise
                            ixp = find(w>0);
                            ixn = find(w<0);
