classdef SigmaResponseView < controllib.chart.internal.view.wave.BaseResponseView & ...
    ...
    %% Constructor
    methods
        function this = SigmaResponseView(response,optionalInputs)            
            arguments
                response (1,1) controllib.chart.response.SigmaResponse
            ...
            this.NLines = size(response.ResponseData.SingularValue{1},1);
            build(this);
        end
    end
    ...
    methods
        function updateArrows(this,optionalArguments)
            ...
            if ~isvalid(this) || ~isvalid(this.Response) || ~this.Response.IsResponseValid
                return;
            end
    ...
    methods
        ...
        function set.FrequencyScale(this,FrequencyScale)
            arguments
                this (1,1) controllib.chart.internal.view.wave.SigmaResponseView
    ...
    methods (Access = protected)
        function createCharacteristics(this,data)
            ...
            % PeakResponse
            if isprop(data,"SigmaPeakResponse") && ~isempty(data.SigmaPeakResponse)
                c = controllib.chart.internal.view.characteristic.SigmaPeakResponseView(this,data.SigmaPeakResponse);
            end 
            this.Characteristics = c;                                  
        end

        function createResponseObjects(this)
            this.ResponseLines = createGraphicsObjects(this,"line",this.NLines,1,this.Response.NResponses,...
                Tag='SigmaResponseLine');
            this.PositiveArrows = createGraphicsObjects(this,"patch",this.NLines,1,this.Response.NResponses,...
                Tag='SigmaPositiveArrow',HitTest='off',PickableParts='none');
            this.NegativeArrows = createGraphicsObjects(this,"patch",this.NLines,1,this.Response.NResponses,...
                Tag='SigmaNegativeArrow',HitTest='off',PickableParts='none');
        end

        function createSupportingObjects(this)
            this.NyquistLines = createGraphicsObjects(this,"constantLine",1,1,2,...
                Tag='SigmaNyquistLine',HitTest='off',PickableParts='none');
        ...
        function updateResponseData(this,optionalInputs)
            ...
            for k = 1:this.NLines
                for ka = 1:this.Response.NResponses
                    ...
                    if this.Response.UseMaximumSingularValue
                        data = magnitudeConversionFcn(this.Response.ResponseData.SingularValue{ka}(k,:));
                        sv = max(data,[],'all')*ones(size(data));
                    else
                        sv = magnitudeConversionFcn(this.Response.ResponseData.SingularValue{ka}(k,:));
                    end
                    if this.Response.ResponseData.IsReal(ka)
                        w = [-flipud(w);w]; %#ok<AGROW>
                        sv = [fliplr(sv),sv]; %#ok<AGROW>
                    end
                    ...
                    this.ResponseLines(k,1,ka).YData = sv;
                end
            end
        ...
        function cbFrequencyUnitChanged(this,conversionFcn)
            ...
            for k = 1:this.NLines
                for ka = 1:this.Response.NResponses
                    dtRow = this.ResponseLines(k,1,ka).DataTipTemplate.DataTipRows(2);
                    dtRow.Label = getString(message('Controllib:plots:strFrequency')) + " (" + this.FrequencyUnitLabel + ")";
                    newValue = conversionFcn(dtRow.Value);
                    dtRow.Value = newValue;
                    this.ResponseLines(k,1,ka).DataTipTemplate.DataTipRows(2) = dtRow;
                end
            end
        end
    end
