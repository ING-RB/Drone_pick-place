classdef TimePhasorOutputPeakResponseView < controllib.chart.internal.view.characteristic.TimeOutputCharacteristicView
    ...
    %% Constructor
    methods
        function this = TimePhasorOutputPeakResponseView(responseView,data)
    ...
    methods (Access = protected)
        ...
        function updateData(this,ko,~,ka)
            ...
            xl = this.PeakResponseLines(ko,1,ka);
            ...
            % Update X line
            xl.XData = [0 realPeakAmplitude];
            xl.YData = [0 imaginaryPeakAmplitude];
        end
            ...
            if this.IsInitialized
                for ko = 1:this.Response.NRows
                    for ka = 1:this.Response.NResponses
                        ...
                        this.PeakResponseLines(ko,1,ka).XData = conversionFcn(this.PeakResponseLines(ko,1,ka).XData);
                        ...
                        this.PeakResponseMarkers(ko,1,ka).DataTipTemplate.DataTipRows(row).Value = this.PeakResponseLines(ko,1,ka).XData(1);
                    end
                end
            end
        end
        ...
        function l = getSupportingObjects_(this,ko,~,ka)
            l = this.PeakResponseLines(ko,1,ka);
        end
    end
end