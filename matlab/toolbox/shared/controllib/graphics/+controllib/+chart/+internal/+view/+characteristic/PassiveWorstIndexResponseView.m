classdef PassiveWorstIndexResponseView < controllib.chart.internal.view.characteristic.SigmaPeakResponseView
    ...
    %% Constructor
    methods
        function this = PassiveWorstIndexResponseView(responseView,data)
    ...
    methods (Access = protected)
        function build_(this)
            ...
            set(this.PeakResponseMarkers,Tag='PassiveWorstIndexResponseScatter')
            set(this.PeakResponseXLines,Tag='PassiveWorstIndexResponseXLine')
            set(this.PeakResponseYLines,Tag='PassiveWorstIndexResponseYLine')
        end

        function updateData(this,~,~,ka)
            data = this.Response.ResponseData;
            peakResponseData = data.PassiveWorstIndexResponse;
        ...
        function updateDataTips_(this,~,~,ka,nameDataTipRow,customDataTipRows)
            data = this.Response.ResponseData.PassiveWorstIndexResponse;

            % Peak Gain Row
            magnitudeConversionFcn = getMagnitudeUnitConversionFcn(this,this.Response.IndexUnit,this.MagnitudeUnit);
            switch data.ResponseData.PassiveType
                case {'input','output','io'}
                    str = getString(message('Controllib:plots:strMinIndex'));
                otherwise
                    str = getString(message('Controllib:plots:strMaxIndex'));
            end
            peakGainRow = dataTipTextRow(str + " (" + this.MagnitudeUnitLabel + ")",...
                magnitudeConversionFcn(data.Value{ka}),'%0.3g');
