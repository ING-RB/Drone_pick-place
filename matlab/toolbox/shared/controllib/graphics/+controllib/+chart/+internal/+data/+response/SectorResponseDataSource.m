classdef SectorResponseDataSource < controllib.chart.internal.data.response.FrequencyResponseDataSource
    % controllib.chart.internal.data.response.SectorResponseDataSource
    %   - manage source and data objects for given sector response
    %   - inherited from controllib.chart.internal.data.response.FrequencyResponseDataSource
    %
    % h = SectorResponseDataSource(model)
    %   model           DynamicSystem
    %
    % h = SectorResponseDataSource(_____,Name-Value)
    ...
    %   Q                           sector geometry data
    %   SectorWorstIndexResponse    worst index characteristic, controllib.chart.internal.data.characteristics.SectorWorstIndexResponseData
    %
    ...
    properties (SetAccess = protected)
        ...
        RelativeIndex
        % "Q": double array or DynamicSystem
        % Sector data.
        Q
    end
    
    properties (Dependent, SetAccess=private)
        % "SectorWorstIndexResponse": controllib.chart.internal.data.characteristics.SectorWorstIndexResponseData scalar
        % Worst index response characteristic.
        SectorWorstIndexResponse
    end

    %% Constructor
    methods
        function this = SectorResponseDataSource(model,Q,frequencyResponseOptionalInputs)
            arguments
                model
                Q
    ...
    methods
        function [commonFrequencyFocus,frequencyUnit] = getCommonFrequencyFocus(this,frequencyScale,arrayVisible)
            arguments
                this (:,1) controllib.chart.internal.data.response.SectorResponseDataSource
    ...
    %% Get/Set methods
    methods
        function SectorWorstIndexResponse = get.SectorWorstIndexResponse(this)
            arguments
                this (1,1) controllib.chart.internal.data.response.SectorResponseDataSource
            end
            SectorWorstIndexResponse = getCharacteristics(this,"SectorWorstIndexResponse");
        end
    end

    %% Protected methods
    methods (Access = protected)
        function updateData(this,sectorResponseOptionalInputs,frequencyResponseOptionalInputs)
            arguments
                this (1,1) controllib.chart.internal.data.response.SectorResponseDataSource
                sectorResponseOptionalInputs.Q = this.Q
            ...
            try
                ...
                ParamList = {sectorResponseOptionalInputs.Q,frequencyResponseOptionalInputs.Frequency};
                [sysList,~,M0,W1,W2,w] = DynamicSystem.checkSectorInputs(sysList,ParamList);
            ...
            this.Q = sectorResponseOptionalInputs.Q;
        ...
        function characteristics = createCharacteristics_(this)
            ...
            characteristics = controllib.chart.internal.data.characteristics.SectorWorstIndexData(this);
        end
        ...
        function indexFocus = computeIndexFocus(this,frequencyFocus,indexScale)
            ...
            for ka = 1:this.NResponses
                ...
                if idx1 >= idx2
                    ...
                    if indexScale=="log"
                        index_k(index_k<0) = NaN; %exclude negative data
                    end
