classdef TimeMagnitudePhaseOutputAxesView < controllib.chart.internal.view.axes.TimeOutputAxesView
    % TimeView
    ...
    %% Constructor
    methods
        function this = TimeMagnitudePhaseOutputAxesView(chart,varargin)
            arguments
    ...
    methods        
        ...
        function unparentResponseViews(this)
            ...
            for k = 1:length(this.ResponseViews)
                ...
                set(this.ResponseViews(k).PhaseMarkerText,Parent=[]);
            end
        end
    end
    ...
    methods (Access = protected)
        function responseView = createResponseView(this,response)
            ...
            responseView = controllib.chart.internal.view.wave.TimeMagnitudePhaseOutputResponseView(response,...
        ...
        function postParentResponseView(this,responseView)
            arguments
                this (1,1) controllib.chart.internal.view.axes.TimeMagnitudePhaseOutputAxesView
                responseView (1,1) controllib.chart.internal.view.wave.TimeMagnitudePhaseOutputResponseView
            end
            updateFocus(this);
            ...
            for k = 1:length(this.ResponseViews)
                ...
                for ko = 1:rv.Response.NRows
                    for ka = 1:rv.Response.NResponses
                        phaseResponseLine = rv.PhaseResponseLines(ko,1,ka);
                        magnitudeResponseLine = getResponseObjects(rv,ko,1,ka);
                        parentAxes = magnitudeResponseLine{1}.Parent;
                        phaseResponseLine.Parent = parentAxes;
                        rv.PhaseMarkerPatch(ko,1,ka).Parent = parentAxes;
                    end
                end
            end
        ...
        function [timeFocus, amplitudeFocus] = computeFocus(this,responses)
            ...
            if ~isempty(responses)
                ...
                [timeFocus_, amplitudeFocus_, timeUnit] = getCommonFocusForMultipleData(data,...
                    {responses.ArrayVisible},ShowMagnitude=true,ShowReal=false,...
                    ShowImaginary=false);
