classdef PassiveWorstIndexData < controllib.chart.internal.data.characteristics.SigmaPeakResponseData
    % controllib.chart.internal.data.PassiveWorstIndexData
    %   - class for computing peak response of passive plot
    %   - inherited from controllib.chart.internal.data.characteristics.SigmaPeakResponseData
    %
    % h = PassiveWorstIndexData(data)
            ...
            for ka = 1:data.NResponses
                ...
                if ~isempty(relInd)
                    for ct = 1:size(relInd,1)
                        ...
                        if strcmpi(data.PassiveType,'relative')
                            indMax = find(ind_ct==max(ind_ct),1,'last');
                        else
                            indMax = find(ind_ct==min(ind_ct),1,'last');
                        end
