...
%  attributes.  Valid properties include Color, SemanticColor, LineStyle,
%  LineWidth, and Marker.
...
if nargin==2
   ...
   Props = {'Color','SemanticColor','LineStyle','LineWidth','Marker'};
   Values = cell(1,5);
   ...
   % Finalize settings
   [Color,LineStyle,Marker] = StyleFinish(Values{[1 3 5]});
   ...
   if ~isempty(Values{2})
       this.SemanticColors = string(Values{2});
       this.EnableTheming = true;
   end
   if ~isempty(Values{3}) || isempty(this.LineStyles)
       this.LineStyles = {LineStyle};
   end
   % Linewidth has a default factory value.
   if ~isempty(Values{4}) 
       this.LineWidth = Values{4};
   end
   
   if ~isempty(Values{5}) || isempty(this.Markers)
       this.Markers = {Marker};
   end
...
if isequal(rgbcolor,[0.8 0.8 1])
   % Special color for unscaled loop in tuning goals
   name = 'faint blue';
else
   % Preprocessing
   if min(rgbcolor)>0.9
