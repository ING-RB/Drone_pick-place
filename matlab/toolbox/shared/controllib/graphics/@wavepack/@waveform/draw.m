...
if ~isempty(this.DataFcn)
    ...
    for ct=1:length(this.Data)
        this.Data(ct).Exception = false;
        this.Data(ct).ExceptionReason = [];
    end
...
for ct=1:length(this.Data)
    this.Data(ct).Exception = this.Data(ct).Exception || ~(this.checksize(this.Data(ct)));
end
...
if NormalRefresh
   ...
   idxException = find(cell2mat(get(this.Data,{'Exception'})));
   if ~isempty(idxException) && ...
         strcmp(RespPlot.DataExceptionWarning,'on')
     tempData = this.Data(idxException);
     idxReason = find(~cellfun(@isempty, get(tempData,{'ExceptionReason'})));
     if isempty(idxReason)
         ctrlMsgUtils.warning('Controllib:plots:PlotMissingData')
     else
         warning(tempData(idxReason(1)).ExceptionReason)
     end
