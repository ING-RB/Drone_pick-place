...
if numel(Group)>0
    Group = handle(Group);
    L = event.proplistener(Group, ...
        findprop(Group(1),'DisplayName'),'PostSet',@(es,ed) LocalSetName(es,ed,this));
...
if ~isempty(Group)
    Group = handle(Group);
    L = event.proplistener(Group, ...
        findprop(Group(1),'Selected'),'PostSet',@(es,ed) localUpdateSelected(es,ed,this));
