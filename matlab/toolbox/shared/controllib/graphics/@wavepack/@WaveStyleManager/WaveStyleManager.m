...
if ~isequal(ColorOrder,get(0,'factoryAxesColorOrder'))
    this.EnableTheming = false;
end
NumberOfColors = size(ColorOrder,1);
TheColorList = cell(NumberOfColors,2);

for ct = 1:NumberOfColors
    TheColorList{ct,1} = ColorOrder(ct,:);
    TheColorList{ct,2} =  LocalGetColorName(ColorOrder(ct,:));   
end
...
function name = LocalGetColorName(rgbcolor)

ColorTable64 = wavepack.colordefs('Color64');
ColorTable64 = [cellfun(@(v) v, ColorTable64(:,1),'UniformOutput',false),ColorTable64(:,2)];
...
% function ColorTable = localGetColorTable
