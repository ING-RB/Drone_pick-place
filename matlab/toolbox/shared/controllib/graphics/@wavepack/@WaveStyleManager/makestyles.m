...
case 'none'
   ...
   if this.EnableTheming
       set(style,'SemanticColors',this.SemanticColorList(1));
   end
case 'response'
   for ct=1:nstyles
      style(ct).Colors = ColorList(ct);
      if this.EnableTheming
          style(ct).SemanticColors = this.SemanticColorList(ct);
      end
   end
case 'responsearray'
...
% Set EnableTheming on style based on StyleManager
for ct = 1:nstyles
    style(ct).EnableTheming = this.EnableTheming;
end
