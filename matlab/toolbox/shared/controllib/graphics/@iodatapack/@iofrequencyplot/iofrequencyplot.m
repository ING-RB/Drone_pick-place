function h = iofrequencyplot(varargin)
%TIMEPLOT  Constructor for @iotimeplot class (time series plot).
%
%  H = IOFREQUENCYPLOT(AX,[Ny Nu]) creates a @iofrequencyplot object with
%  Ny+Nu axes (channels) in the area occupied by the axes with handle AX.
%
%  H = IOFREQUENCYPLOT([Ny Nu]) uses GCA as default axes.
%
%  H = IOFREQUENCYPLOT([Ny Nu],'Property1','Value1',...) initializes the plot
%  with the specified attributes.

%  Copyright 2013 The MathWorks, Inc.

% Create class instance
h = iodatapack.iofrequencyplot;
