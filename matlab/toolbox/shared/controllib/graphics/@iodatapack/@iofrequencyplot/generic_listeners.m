...
%GENERIC_LISTENERS  Installs generic listeners for @iodataplot class.
...
%--------------------------------------------------------------------------
function LocalAxesGrouping(this, ~)
% Purpose: Re-parent the line handles to conform with current AxesGrouping
% mode.
...
nax = numel(Axes);
nobj = numel(this.BackgroundLines)/nax;
...
idx = strcmp(this.IOGrouping, RespGrouping);
...
switch eventsrc.Name
   case 'InputVisible'
      ...
      this.AxesGrid.RowVisible(s(1)*sz(3)+(1:numel(Rvis))) = Rvis;
   case 'OutputVisible'
      if strcmp(this.IOGrouping,'all')
         refresh(this)
      end
      Rvis = this.io2rcvis('r',eventdata.NewValue);
      this.AxesGrid.RowVisible(1:numel(Rvis)) = Rvis;
end
