...
switch XY
    case 'X'
        if isempty(this.XLimitsContainer)
            ...
            uniqueColumnLabels = unique(this.ColumnLabel(cellfun(@(x) ~isempty(x),this.ColumnLabel)));
            limitsContainer = controllib.widget.internal.cstprefs.LimitsContainer(...
                'NumberOfGroups',1 + nSelect*length(uniqueColumnLabels),...
    ...
    case 'Y'
        if isempty(this.YLimitsContainer)
            ...
            nLimitRows = subGridSize;
            uniqueRowLabels = unique(this.RowLabel(cellfun(@(x) ~isempty(x),this.RowLabel)));
            limitsContainer = controllib.widget.internal.cstprefs.LimitsContainer(...
                'NumberOfGroups',1 + nSelect*length(uniqueRowLabels),...
                'NumberOfLimits',nLimitRows);
...
switch es.Name
    case 'AutoScale'
        ...
        if limitsContainer.NGroups == 1
            % If there is only one group, xLimMode is not a cell array
            this.XLimMode = xLimMode;
        else
            switch limitsContainer.SelectedGroupIdx
                case 1
                    % If common group is selected, set xLimMode for all
                    % groups
                    this.XLimMode(:) = {xLimMode};
                otherwise
...
switch es.Name
    case 'AutoScale'
        ...
        if nGroups == 1
            % If there is only one group, yLimMode is not a cell array
            ...
            case 1
                ...
                for k = 1:nLimits
                    ...
                    if ~any(isnan(limits{k}))
                        for idx = k:nLimits:max([1,(nGroups-1)*nLimits])
