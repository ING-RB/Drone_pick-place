...
switch menuType
    ...
    case 'characteristics'
        ...
        mRoot = uimenu('Parent', AxGrid.UIContextMenu,...
            'Label', getString(message('Controllib:plots:strCharacteristics')), ...
            'Tag', menuType);
        
    case 'fullview'
        % Full View (zoom out)
        mRoot = uimenu('Parent', AxGrid.UIContextMenu,...
            'Label', getString(message('Controllib:plots:strFullView')), ...
            'Tag', menuType,...
    ...
    case 'normalize'
        % Normalized Y range
        mRoot = uimenu('Parent', AxGrid.UIContextMenu,...
            'Label', getString(message('Controllib:plots:strNormalize')), ...
            'Tag', menuType,...
    ...
    case 'properties'
        ...
        mRoot = uimenu('Parent', AxGrid.UIContextMenu, ...
            'Label',getString(message('Controllib:plots:strPropertiesLabel')),...
...
function localToggleNormalize(~, ~,AxGrid)
...
function localSyncNormalize(~,~,AxGrid,mRoot)
% Updates check status of Full View
set(mRoot,'Checked',AxGrid.YNormalization)


function LocalToggleFullView(~,~,AxGrid)
% Zooms out
AxGrid.XlimMode = 'auto';
AxGrid.YlimMode = 'auto';


function localSyncFullView(~,~,AxGrid,mRoot)
% Updates check status of Full View
...
function LocalEditProp(~,~,this) 
% Open property editor 
PropEdit = PropEditor(this);
setTarget(PropEdit,this);
show(PropEdit);


function localSyncWaveForms(~,eventData,Plot,WFProp,mRoot)
% PreSet listener to the property holding the set of waveforms shown on the plot.
% Ensures that the number of submenus matches the number of waveforms and installs
...
function localToggleWaveFormVisibility(~, ~,wf)
% Toggles visibility of particular response
...
function localUpdateWaveFormMenuLabels(~,~,mRoot,WaveForms)
% Callback which sets the response menu checked status equal to the visibility
