...
% Note: Use third dimension of axes array to check if there is a subgrid
% For example bodeplots with iogrouping=all dont update both magnitude and
% phase limits together
if isempty(varargin)  || strcmpi(h.YLimSharing,'all') || ...
        (any(strcmp(h.AxesGrouping,{'all' 'row'})) && isequal(size(h.getaxes,3),1))
...
if Ylim(1)<=0 && any(strcmp(h.YScale(row_index),'log'))
    ctrlMsgUtils.error('Controllib:plots:Limits1')
end
