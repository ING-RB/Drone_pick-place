...
p.FactoryValue = struct('HeightRatio',[],...
   'HorizontalGap',16,'LeftMargin',0,'TopMargin',0,'VerticalGap',16);
...
p = schema.prop(c,'PrintLayoutManager','on/off');    % Print Layout management
p.FactoryValue = 'off';

% Private
% REVISIT: make private when private prop can be accessed in local functions of methods
p = schema.prop(c,'BackgroundAxes','MATLAB array');         % Background axes
%set(p,'AccessFlags.PublicGet','off','AccessFlags.PublicSet','off')

p(1) = schema.prop(c,'XLimitsContainer','MATLAB array');
p(2) = schema.prop(c,'YLimitsContainer','MATLAB array');
