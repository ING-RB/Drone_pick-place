...
% Currently only used by plotconstr objects, 11/20/2008
...
case 'init'
   ...
   X0 = CP(1,1);   Y0 = CP(1,2);
   for ct=1:Nselect,
      % RE: To avoid distortions due to patch thickness, mouse-selected constraint 
...
case 'track'   
   ...
   if Nselect>1 || isempty(Status),
      ...
      Status = getString(message('Controllib:gui:strMoveSelectedObjectsToLocation'));
   end
...
case 'finish'
   ...
   % Dispatch finish to each constraint
   for ct=1:Nselect
      move(this.SelectedObjects(ct),'finish');
   end
   
   % Set status
   Status = getString(message('Controllib:gui:strMovedSelectedObjects'));
   if Nselect==1
      try
