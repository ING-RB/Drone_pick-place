classdef HPFilterMode < ctrluis.toolstrip.dataprocessing.absFilterMode
    %
    ...
    methods
        function obj = HPFilterMode(varargin)
            %HPFILTERMODE
            % Inputs: Figure handle, SaveAsEnabled flag
            obj = obj@ctrluis.toolstrip.dataprocessing.absFilterMode(varargin{:});
            
            %Set Object properties
            obj.Name   = 'HighPassFilter';
            obj.MSGIds = struct(...
                'Close',       'Controllib:dataprocessing:lblHPFilter_Close', ...
                'CloseNoSave', 'Controllib:dataprocessing:lblHPFilter_Close_No_Save');
        end
    end
    
    %Mode API Implementation
    properties(Constant = true)
        DISPLAYNAME = getString(message('Controllib:dataprocessing:lblHPFilter'));
        DESCRIPTION = getString(message('Controllib:dataprocessing:lblHPFilter_Description'));
        ICON        = 'highpassFilter';
        ICON_16     = 'HighPassFilter_16';
        %ICON_24     = 'HighPassFilter_24';
        %ICON_40     = 'HighPassFilter_60x40';
    end
    ...
    methods(Access = protected)
        function createSection(this)
            ...
            CutoffLblTxt = getString(message('Controllib:dataprocessing:lblHPFilter_Cutoff'));
            SecName = getString(message('Controllib:dataprocessing:lblHPFilter'));
            
            % Create label and edit field for cutt-off frequency.
            lblCutoff      = matlab.ui.internal.toolstrip.Label(CutoffLblTxt);
        ...
        function createPatch(this)
            ...
            for ct=numel(sig):-1:1
                ...
                Fs(ct)  = pi/data.Ts{ct};
                pEdge   = [this.Cutoff*Fs(ct) data.Frequency{ct}(end)];
        ...
        function updatePatch(this,delta)
            ...
            for ct=1:numel(this.FilterPatch.hPatch)
                if any(strcmp(this.FilterPatch.Sig(ct),this.SignalSelector.SelectedSignals))
                    ...
                    ylim    = get(hAx,'ylim');
                    lEdge = this.Cutoff*this.FilterPatch.Fs(ct)*delta;
        ...
        function updateFilter(this)
            ...
            [a,b,c,d] = ctrluis.toolstrip.dataprocessing.butter(this.FilterOptions(1).Order,this.Cutoff,'high');
            this.Filter = struct(...
                'a', a, ...
                'b', b, ...
                'c', c, ...
