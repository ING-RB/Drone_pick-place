...
if ndata>nviews
   ...
   for ct=ndata:-1:nviews+1
      MagPoints(ct,1) = controllibutils.utCustomCopyLineObj(cv.MagPoints(1),Parent);
      MagLines(ct,1) = controllibutils.utCustomCopyLineObj(cv.MagLines(1),Parent);
      MagCrossLines(ct,1) = controllibutils.utCustomCopyLineObj(cv.MagCrossLines(1),Parent);
      MagTips(ct,1) = wrfc.createDefaultHandle;
   end
   
   % Phase Characteristics
   Parent = get(cv.PhasePoints(1),'Parent');
   for ct=ndata:-1:nviews+1
      PhasePoints(ct,1) = controllibutils.utCustomCopyLineObj(cv.PhasePoints(1),Parent);
      PhaseLines(ct,1) = controllibutils.utCustomCopyLineObj(cv.PhaseLines(1),Parent);
      PhaseCrossLines(ct,1) = controllibutils.utCustomCopyLineObj(cv.PhaseCrossLines(1),Parent);
      PhaseTips(ct,1) = wrfc.createDefaultHandle;
   end
