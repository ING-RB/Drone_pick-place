...
if strcmp(Event,'postlim') 
   ...
   for ct=1:numel(cv.Points)
      ...
      if ~isempty(rData.Time) && ~isempty(rData.Amplitude)
          YDot = utInterp1(rData.Time*tunitconv(rData.TimeUnits,cv.AxesGrid.XUnits),...
              rData.Amplitude(:,ct),XDot);
      else
          YDot = NaN;
      end
      if isfinite(cd.FinalValue(ct))
         ...
         if YNorm && ~isempty(rData.Time) && ~isempty(rData.Amplitude)
            YDot = normalize(rData,YDot,Xlim*tunitconv(cv.AxesGrid.XUnits,rData.TimeUnits),ct); % take normalization into account
         end
