...
if localHasCommonFrequencyVector(this);   
    ...
    MagBoundData = cat(4,RespData(1,:).Magnitude);
    PhaseBoundData = cat(4,RespData(1,:).Phase);
    this.Bounds.UpperMagnitudeBound = max(MagBoundData,[],4);
    this.Bounds.LowerMagnitudeBound = min(MagBoundData,[],4);
    this.Bounds.UpperPhaseBound = max(PhaseBoundData,[],4);
    this.Bounds.LowerPhaseBound = min(PhaseBoundData,[],4);
...
UpperMagBound = max(MagBoundData,[],4);
LowerMagBound = min(MagBoundData,[],4);

UpperPhaseBound = max(PhaseBoundData,[],4);
LowerPhaseBound = min(PhaseBoundData,[],4);
