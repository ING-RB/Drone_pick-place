...
if strcmp(Event,'postlim')
   ...
   for ct=1:s1*s2
      ...
      YDot = rData.Amplitude(rData.Time==cd.Time(ct),ct);
      ...
      if YNorm && isfinite(YDot) && ~isempty(rData.Time) && ~isempty(rData.Amplitude)
          YDot = normalize(rData,YDot,Xlim*tunitconv(cv.AxesGrid.XUnits,rData.TimeUnits),ct);
      end
      ...
      if OutScope || isnan(XDot)
          ...
          set(double(cv.HLines(ct)),...
             'XData',[rData.Time(1)*tunitconv(rData.TimeUnits,cv.AxesGrid.XUnits),XDot],'YData',[YDot,YDot],'Zdata',[-10 -10])     
