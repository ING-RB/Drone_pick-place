...
repHndls = cell(nresp,1);
DataSrcs=get(responses,{'DataSrc'});
for k=1:nresp
   if ~isempty(DataSrcs{k})
      s = size(DataSrcs{k}.Model);
      if length(s)>=3
         repHndls{k}=reshape(strcmpi(get(responses(k).view,'Visible'),'on'),s(3:end));
      else
         repHndls{k}=strcmpi(get(responses(k).view,'Visible'),'on');
      end
   end
end
dataInfo.obj=h;
...
if isempty(figNumber)
  ...
  figNumber=figure( ...
     'WindowStyle','normal',...
     'Name',getString(message('Controllib:gui:strModelSelectorForLTIArrays')), ...
     'NumberTitle','off', ...
     'Color',StdColor, ...
     'IntegerHandle','off',...
     'MenuBar','none', ...
  ...
  helpHndl=uicontrol(figNumber, ...
     ...
     'BackgroundColor',StdColor, ...
     'String',getString(message('Controllib:gui:lblArrays')));
  
  %---Initialize UserData
  arrayIndex = find(cellfun(@numel,repHndls)>1);
  dimInfo = cellfun(@size,repHndls(arrayIndex),'UniformOutput',false);
  ArraySrcs = DataSrcs(arrayIndex);
  k = numel(arrayIndex);  % number of model arrays
  str = cell(k,1);
  for i=1:k
     % Model name
     if isempty(ArraySrcs{i}) || isempty(ArraySrcs{i}.Name)
        str{i} = responses(arrayIndex(i)).name;
     else
        str{i} = ArraySrcs{i}.Name;
     end
  end
  %---Initialize selarray
  selarray=cell(1,k);
  ...
  if k==0 %all the sysyems have dim <= 2
      return
  end
  responseH=size(repHndls{arrayIndex(1)});
  ...
  helpHndl=uicontrol(figNumber, ...
     ...
     'BackgroundColor',StdColor, ...
     'String',getString(message('Controllib:gui:strSelectionCriteria')), ...
     'Tag', 'criterionlabel');
  helpHndl=uicontrol(figNumber, ...
     'Style','listbox', ...
     'Units',StdUnit, ...
     ...
     'BackgroundColor',[1 1 1], ...
     'String',{getString(message('Controllib:gui:strIndexIntoDimensions')), ...
  ...
  txtHndl=uicontrol(figNumber, ...
     ...
     'Tag','status', ...
     'String',getString(message('Controllib:gui:strShowSelectedPlots')));
  %------------------------------------
  %=========apply======================
  helpHndl=uicontrol(figNumber, ...
     'Style','pushbutton', ...
     ...
     'String',getString(message('Controllib:general:strOK')),...
  ...
  helpHndl=uicontrol(figNumber, ...
     ...
     'BackgroundColor',StdColor, ...
     'String',getString(message('Controllib:general:strCancel')), ...
  ...
  helpHndl=uicontrol(figNumber, ...
     ...
     'BackgroundColor',StdColor, ...
     'String',getString(message('Controllib:general:strHelp')), ...
  ...
  closeHndl=uicontrol(figNumber, ...
     ...
     'BackgroundColor',StdColor, ...
     'String',getString(message('Controllib:general:strApply')), ...
     'Callback',{@localGetSelection});
  
  selarray{1}=localAddrulemake(figNumber,responseH, 0, []);
  ...
  localsetallEnable(figNumber, responseH, 2);
...
arrayIndex = find(cellfun(@numel,repHndls)>1);
dimInfo = cellfun(@size,repHndls(arrayIndex),'UniformOutput',false);
DataSrcs = DataSrcs(arrayIndex);
k = numel(arrayIndex);  % number of model arrays
str = cell(k,1);
for i=1:k
   % Model name
   if isempty(DataSrcs{i}) || isempty(DataSrcs{i}.Name)
      str{i} = RespObj.responses(arrayIndex(i)).name;
   else
      str{i} = DataSrcs{i}.Name;
   end
end
NewNames = str;
...
[~,refreshIndex,refreshFromIndex]=intersect(NewNames,OldNames);
...
[~,indold]=intersect(arrayIndex,index);
...
if isempty(ArrayStr)
   ...
   [~,arrayIndex]=intersect(SystemNames,ArrayStr);
...
dispStr={...
    getString(message('Controllib:gui:lblPeakResponse')), ...
    getString(message('Controllib:gui:lblSettlingTimeSec')), ...
    getString(message('Controllib:gui:lblRiseTimeSec')), ...
    getString(message('Controllib:gui:lblSteadyState'))};
...
if flag == 0  %called in init, set up frames
   ...
   mainFrmHndl=uicontrol(figNumber, ...
      ...
      'String', getString(message('Controllib:gui:strSelectionCriterionSetup')), ...
      'Tag', 'criterionlabel', ...
      'BackgroundColor',StdColor);
   helpHndl=uicontrol(figNumber, ...
      'Style','popupmenu', ...
      ...
      'BackgroundColor',[1 1 1], ...
      'String',{ getString(message('Controllib:gui:strShowAll')), ...
...
case 'resppack.timeplot'
    switch plotType
        case 'step'
            ...
            timeName={'Peak','Time','Time','Value'}; %name of $ variable for each characteristic - used in a warning string
            characteristicName={'resppack.StepPeakRespData','resppack.SettleTimeData','resppack.StepRiseTimeData','resppack.TimeFinalValueData'}; %class name of characteristic data object
        ...
        case 'impulse'
            boundName={'Peak Response','Transient Time'}; %string used by warnings and errors to identify characteristics
            timeName={'Peak','Time'}; %name of $ variable for each characteristic - used in a warning string
            %characteristicName={'resppack.ImpulsePeakRespData','resppack.SettleTimeData'}; %class name of characteristic data object
            characteristicName={'wavepack.TimePeakAmpData','resppack.TransientTimeData'}; %class name of characteristic data object
            characteristicViewName={'wavepack.TimePeakAmpView','resppack.TransientTimeView'}; %class name of characteristic view object
...
case 'resppack.simplot'
    ...
    timeName={'Peak'}; %name of $ variable for each characteristic - used in a warning string
...
case 'resppack.bodeplot'
    if prod(h.axesgrid.size) == 2 %all SISO systems
        ...
        timeName={'Peak','Gain margin', 'Phase margin'}; %name of $ variable for each characteristic - used in a warning string
...
case 'resppack.nyquistplot'   
   if prod(h.axesgrid.size) == 1 %all SISO systems 
        ...
        timeName={'Gain margin', 'Phase margin'}; %name of $ variable for each characteristic - used in a warning string
        ...
        timeName={''}; %name of $ variable for each characteristic - used in a warning string
...
for i=1:length(checkTag)
  ...
  if get(checkHndl(i), 'Value') == 1
    ...
    if ~isempty(existingChars) && ~iscell(existingChars)
    ...
    %if the desired characteristic does not exist then create it
    thisSysChars{1}='';
    ...
    if isempty(thisSysChars) || ~any(I) %none of this char exists for one system
        %no need to handle simplot addchar method since this can't be a simplot
        c_old = get(h.responses,'characteristics');
        if iscell(c_old)
            c_old  = reshape([c_old{:}],[numel([c_old{:}]) 1]);
        end
        if strcmpi(class(h),'resppack.simplot') %simplot requires extra arg due to input chars
        ...
        if iscell(c_new)
            c_new  = reshape([c_new{:}],[numel([c_new{:}]) 1]);
        end
    ...
    if ~isempty(thisChars)
        ...
        %thisvalue=cellvalue(arrayvalue);
          ...
          if isempty(hasDollar)
             warndlg(getString(message('Controllib:gui:errEvaluatingConstraint',...
                   boundName{str2num(tempStr(end))}(1:end-5),...
                   charStr)), ...
                getString(message('Controllib:gui:strModelSelectorWarning')));
             errflag = 1;
             thisout = true(dimInfo);
          else
             [editStr,errmsg]=LocalArrayIndexingParser(editStr);
             if ~isempty(errmsg)
                newlines = sprintf('\n\n');
                warndlg([getString(message('Controllib:gui:errEvaluatingConstraint2', ...
                      boundName{str2num(tempStr(end))}(1:end-5))), ...
                      newlines errmsg], ... 
                   getString(message('Controllib:gui:strModelSelectorWarning')));
                 errflag = 1;
                ...
                try
                   ...
                   newlines = sprintf('\n\n');
                   errmsg=strrep(ME.message, varnm, '$');
                   warndlg([getString(message('Controllib:gui:errEvaluatingConstraint2', ...
                         boundName{str2num(tempStr(end))}(1:end-5))), ...
                         newlines errmsg], ... 
                      getString(message('Controllib:gui:strModelSelectorWarning')));
                ...
                %
                while numel(thisout) > prod(dimInfo)
                   thisout=shiftdim(all(thisout,1),1);
                end
                
                % reshape to recover from $(:) type operations
                %
                if numel(thisout) == prod(dimInfo)
                   thisout = reshape(thisout,dimInfo);
                else         
                   warndlg(getString(message('Controllib:gui:errEvaluatingConstraint3',...
                            boundName{str2num(tempStr(end))}(1:end-5))),...
                            getString(message('Controllib:gui:strModelSelectorWarning')));
                   errflag = 1;
                   thisout = true(dimInfo);
          ...
          warnStr = sprintf(' %s',getString(message('Controllib:gui:warnFRDStabilityMargins')));
    end %if noCharExists
...
if ~errflag
   ...
   case 'on', 
      set(statusHndl, 'String', sprintf('%s%s',...
          getString(message('Controllib:gui:msgSelectedResultedInNModels',numvis)), ....
          warnStr));
   case 'off',
      set(statusHndl,'String', ...
         getString(message('Controllib:gui:msgViewSelectedNModels',numvis)));
...
CharStr{1} = getString(message('Controllib:gui:lblSteadyState'));
CharStr{2} = getString(message('Controllib:gui:lblRiseTimeSec'));
CharStr{3} = getString(message('Controllib:gui:lblSettlingTimeSec'));
CharStr{4} = getString(message('Controllib:gui:lblPeakResponse'));

switch plotType
case 'impulse'
      CharStr{3} = getString(message('Controllib:gui:lblTransientTimeSec'));
   	NumActive = [3 4];
case 'step'
    NumActive = [1 2 3 4];
case 'sigma'
    if strcmpi(h.axesgrid.Yunits,'dB')
        magStr = sprintf(' (%s)',getString(message('Controllib:gui:strDB')));
    end
    NumActive = 4;
    CharStr{4} = sprintf('%s%s',...
        getString(message('Controllib:gui:lblPeakResponse')),...
        magStr);
case 'initial'
	NumActive = 4;
case 'bode'
    ...
    if strcmpi(h.axesgrid.Yunits(2),'rad')
        phaseStr = sprintf(' (%s)',getString(message('Controllib:gui:strRad')));
    elseif strcmpi(h.axesgrid.Yunits(2),'deg')
        phaseStr = sprintf(' (%s)',getString(message('Controllib:gui:strDeg')));
    end
    %magStr = ' (dB)';
    CharStr{4} = sprintf('%s%s',...
        getString(message('Controllib:gui:lblPeakResponse')),...
    ...
    if prod(RespObj.axesgrid.size) == 2 %all SISO systems
		NumActive = [2 3 4];		
        CharStr{3} = sprintf('%s%s',...
            getString(message('Controllib:gui:strGainMargin')),...
            magStr);
        CharStr{2} = sprintf('%s%s',...
            getString(message('Controllib:gui:strPhaseMargin')),...
            phaseStr);
    end  
case 'nichols'
   magStr = sprintf(' (%s)',getString(message('Controllib:gui:strDB'))); %Nichols Y is always DB
    phaseStr = sprintf(' (%s)',getString(message('Controllib:gui:strDeg')));
...
case 'nyquist'
    phaseStr = sprintf(' (%s)',getString(message('Controllib:gui:strDeg'))); %Nyq is always abs and deg
    magStr = sprintf(' (%s)',getString(message('Controllib:gui:strDB')));
    NumActive = [];
    CharStr{4} = '';
    if prod(RespObj.axesgrid.size) == 1 %all SISO systems
...
if ~isempty(ix)
  ...
  if length(jx) ~= length(kx)
    errmsg = getString(message('Controllib:gui:errMismatchedParentheses'));
    return
  end
  for n=length(ix):-1:1			% work backwards..
    ...
    switch length(cl)
    case 0
      errmsg = getString(message('Controllib:gui:errUseMultipleIndexingForMIMO')); 
    case 1
...
if flag == 1
   ...
   thisenable = 'on';
   if flag == 2
      set(statusHndl, 'String', getString(message('Controllib:gui:msgShowSelectedPlots')));
   else
      set(statusHndl, 'String', getString(message('Controllib:gui:msgShowUnselectedPlots')));
   end
end
...
function localArray(~, ~,h,varargin)
...
for i=dimLength:-1:3    % i should be > 3 for shift to be active
...
function localConstrain(~, ~)

figNumber=gcbf;
thisedit=gcbo;
str=get(thisedit, 'String');
tag=get(thisedit, 'Tag');
tagNum=str2num(tag(10:end)); %#ok<*ST2NM>
...
try
  ...
  warndlg(getString(message('Controllib:gui:warnValidMatlabExpression')), ...
     getString(message('Controllib:gui:strModelSelectorWarning')));
...
if maxvalue>length(get(list, 'String')) || minvalue<1
  selvalue = 1:length(get(list,'String'));
  warndlg(getString(message('Controllib:gui:warnValuesMustBeInRange',...
        num2str(length(get(list, 'String'))))), ...
     getString(message('Controllib:gui:strModelSelectorWarning')));
end
set(list, 'Value', selvalue);

function localCriterion(~, ~)
...
if value==2
  set(statueHndl, 'String', getString(message('Controllib:gui:msgEnterMatlabExpressionInstruct')));
else
  set(statueHndl, 'String', getString(message('Controllib:gui:msgShowSelectedPlots')));
end
...
function localGetSelection(~, ~, varargin)
...
statusHndl=findobj(figNumber, 'Tag', 'status');
set(statusHndl, 'String', ...
    getString(message('Controllib:gui:msgEnterMatlabExpressionInstruct')));



function LocalDeleteResp(eventSrc, ~, fig)

infoData = get(fig,'userdata');
if ~infoData.obj.isBeingDestroyed
    arrayCombo = findobj(fig,'tag','Array');
    arrayComboValue = get(arrayCombo,'Value'); %current combo box value
    deletedResps = find(eventSrc == infoData.obj.responses); %deleted response positions
    [~,I] = intersect(infoData.arrayIndex, deletedResps); %I are the positions of deleted resps in the combo
...
if isempty(ver('control')) || ~license('test','Control_Toolbox')
    identguihelp('response_arrayselector');
else
    ctrlguihelp('response_arrayselector');
end
