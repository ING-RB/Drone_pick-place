...
if ni<3
    ctrlMsgUtils.error('Controllib:general:ThreeOrMoreInputsRequired', ...
        'resppack.nicholsplot/plot','resppack.nicholsplot/plot');
end
...
% Frequency
if ~isreal(w)
    ctrlMsgUtils.error('Controllib:plots:Plot1','resppack.nicholsplot/plot')
end
w = w(:);
nf = length(w);
...
if nf2~=nf | nf3~=nf
    ctrlMsgUtils.error('Controllib:plots:Plot2','plot(NICHOLSPLOT,W,M,...)','W','M')
elseif ny2~=ny | nu2~=nu
    ctrlMsgUtils.error('Controllib:plots:Plot2','plot(NICHOLSPLOT,W,M,P,...)','M','P')
elseif ~isreal(mag)
    ctrlMsgUtils.error('Controllib:plots:Plot3','plot(NICHOLSPLOT,W,M,P,...)','M')
elseif ~isreal(phase)
    ctrlMsgUtils.error('Controllib:plots:Plot3','plot(NICHOLSPLOT,W,M,P,...)','P')
end
