...
if strcmp(Event,'postlim')
   ...
   for ct=1:s1*s2
      ...
      XDot = cd.Time(ct)*tunitconv(cd.TimeUnits,cv.AxesGrid.XUnits);
      YDot = cd.YSettle(ct);
      OutScope = (Xauto(ceil(ct/s1)) && (XDot>Xlim(2)));
      if OutScope
         XDot = 0.9999*Xlim(2);
         YDot = utInterp1(rData.Time*tunitconv(rData.TimeUnits,cv.AxesGrid.XUnits),rData.Amplitude(:,ct),XDot);
         Color = get(ax,'Color');
      else
         Color = get(cv.Points(ct),'Color');
      end
      
      if cd.YSettle(ct) < cd.FinalValue(ct)
         YUpper = cd.YSettle(ct);
         YLower = 2*cd.FinalValue(ct) - cd.YSettle(ct);
      else
         YUpper = 2*cd.FinalValue(ct) - cd.YSettle(ct);
         YLower = cd.YSettle(ct);
      end
      % Take normalization into account
      if YNorm && isfinite(YDot)
         Factor = tunitconv(cv.AxesGrid.XUnits,rData.TimeUnits);
         YDot = normalize(rData,YDot,Xlim*Factor,ct);
         YUpper = normalize(rData,YUpper,Xlim*Factor,ct);
         YLower = normalize(rData,YLower,Xlim*Factor,ct);
      end
