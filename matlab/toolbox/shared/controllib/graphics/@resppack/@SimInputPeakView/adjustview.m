...
if strcmp(Event,'postlim')
   ...
   XDot = cd.Time(1)*tunitconv(cd.TimeUnits,cv.AxesGrid.XUnits);
   ...
   if InScope
      ...
      if ~isnan(YDot) && ~isempty(rData.Amplitude)
          ...
          YDot = utInterp1(rData.Time,rData.Amplitude,XDot);
      end
   ...
   for ct=1:length(cv.Points)
      ...
      if YNorm && ~isnan(YDot)
         Y = normalize(rData,YDot,Xlim*tunitconv(cv.AxesGrid.XUnits,cd.TimeUnits),ct);
      else
         Y = YDot;
      end
