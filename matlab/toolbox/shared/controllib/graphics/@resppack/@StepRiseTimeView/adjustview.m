...
if strcmp(Event,'postlim') 
   ...
   for ct=1:s1*s2
      ...
      Xlim = get(ax,'Xlim');
      % Dot position
      TLow = cd.TLow(ct)*tunitconv(cd.TimeUnits,cv.AxesGrid.XUnits);
      THigh = cd.THigh(ct)*tunitconv(cd.TimeUnits,cv.AxesGrid.XUnits);
      ...
      if isfinite(THigh)
         ...
         if Xauto(ceil(ct/s1)) && ~isempty(rData.Time)
