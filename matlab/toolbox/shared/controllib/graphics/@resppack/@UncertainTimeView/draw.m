...
if strcmpi(this.UncertainType,'Bounds')
    ...
    if strcmp(this.AxesGrid.YNormalization,'on') || (length(Data.Data)<2)
        ...
        if isequal(Data.Ts,0)
            ...
            XData = [Bounds.Time;Bounds.Time(end:-1:1)]*tunitconv(Data.TimeUnits,this.AxesGrid.XUnits);
            ...
            switch this.Style
                case 'stairs'
                    for ct = 1:Ny*Nu
                        ...
                        set(double(this.UncertainPatch(ct)), ...
                            'XData', T*tunitconv(Data.TimeUnits,this.AxesGrid.XUnits), ...
                            'YData', Y);
                    end
                case 'stem'
        ...
        if isequal(Data.Ts,0)
            for ct = 1:Ny*Nu
                ...
                for ct1 = 1:length(RespData)
                    YData = [YData; RespData(ct1).Amplitude(:,ct);NaN];
                    XData = [XData; RespData(ct1).Time(:);NaN];
                end
                set(this.UncertainLines(ct),'XData',XData,'YData',YData,'ZData',-2 * ones(size(XData)))
            end
        end
