...
%     T = numerictype('half')    creates a half    numerictype object.
%
...
firstArg_Is_Char = false;
firstArg_Is_NumericOrLogical = false;
firstArg_Is_EmbedNumericType = false;
firstArg_Is_SLNumericType = false;

if isnumeric(varargin{1}) || islogical(varargin{1})   

    firstArg_Is_NumericOrLogical = true;
    varargin{1} = double(varargin{1});
    
elseif ischar(varargin{1})

    firstArg_Is_Char = true;
    first_char_position = 1;

elseif isnumerictype(varargin{1})

    firstArg_Is_EmbedNumericType = true;

elseif isa(varargin{1},'Simulink.NumericType') 

    firstArg_Is_SLNumericType = true;
...
elseif isstring(varargin{1})
    ...
    error(message('fixed:numerictype:invalidInputArg', 1));
end

for idx = 2 : nargin
    if isnumeric(varargin{idx})
        % UDD datastructures does not support half
        % cast all inputs to double before constructing
        % this is will also cast singles to doubles.
        % The arguments are scalars so the cost of converting single to
        % doubles is no more significant than the extra work of
        % explicitly testing if input is half
        % cast to double
        varargin{idx} = double(varargin{idx});
    elseif isstring(varargin{idx})
        varargin{idx} = convertStringsToChars(varargin{idx});
        if ~first_char_position
            first_char_position = idx;
        end
    elseif ~first_char_position && ischar(varargin{idx})
        first_char_position = idx;
    end
end

if first_char_position > 1 && ~firstArg_Is_EmbedNumericType
...
elseif firstArg_Is_NumericOrLogical
    ...
    for ii = 3 : valueIndex
        if ~isnumeric(varargin{ii})
            error(message('fixed:numerictype:invalidInputArg', ii));
    ...
    if firstArg_Is_SLNumericType
        ...
        this.Bias           = S.Bias;
        n1=1;
    elseif firstArg_Is_EmbedNumericType
        this = varargin{1};
        n1=1;
    else
        this = embedded.numerictype;
...
[WordLength, slope,fraction, exponent, bias] = getScalingWordLengthFromStr(dataTypeNameStr,pos);
...
if ( slope == 1 && fraction == 1 && bias == 0 )
    if isscaleddouble
        ...
        DataType = numerictype(signed, WordLength, -exponent);
    end
else
    [exponent,slopeAdjustmentFactor] = fixed.internal.type.normalizeSlopeTerms(exponent,slope,fraction);
    
    if isscaleddouble
        DataType = numerictype('Signed', signed, 'WordLength',WordLength, ...
            'FixedExponent',exponent, 'DataType','scaleddouble', ...
            'Scaling','slopebias', 'SlopeAdjustmentFactor',slopeAdjustmentFactor, ...
            'Bias', bias);
    else
        DataType = numerictype(signed, WordLength, ...
            slopeAdjustmentFactor, exponent, bias);
    end
    
end
    
function [WordLength, slope,fraction, exponent, bias] = getScalingWordLengthFromStr(dataTypeNameStr,pos)
    ...
    if (pos < end_pos)
        dataTypeNameStr = strrep(dataTypeNameStr,'p','.');
        dataTypeNameStr = strrep( dataTypeNameStr,'n','-');
        ...
        while (pos < end_pos)
            ...
            switch dataTypeNameStr(pos)
                case 'S'
                    slope = ...
                        eval(dataTypeNameStr(pos+1: next_pos));
                case 'E'
                    exponent = ...
                        eval(dataTypeNameStr(pos+1: next_pos));
                case 'B'
...
function DataType = getFromBuiltInTypesStr(dataTypeNameStr)

switch lower(dataTypeNameStr)
    
    case 'double'
        DataType = fixed.internal.type.ntDouble();
        
    case {'single','float'}
        DataType = fixed.internal.type.ntSingle();
        
    case 'half'
        DataType = fixed.internal.type.ntHalf();
        
    case {'boolean','bool','logical'}
        DataType = fixed.internal.type.ntBoolean();
        
    case 'int64'
        DataType = fixed.internal.type.ntInt64();
        
    case 'int32'
        DataType = fixed.internal.type.ntInt32();
        
    case 'int16'
        DataType = fixed.internal.type.ntInt16();
        
    case 'int8'
        DataType = fixed.internal.type.ntInt8();
        
    case 'uint64'    
        DataType = fixed.internal.type.ntUint64();
        
    case 'uint32'
        DataType = fixed.internal.type.ntUint32();
        
    case 'uint16'
        DataType = fixed.internal.type.ntUint16();
        
    case 'uint8'
        DataType = fixed.internal.type.ntUint8();
        
    otherwise
        
        if (fixed.internal.type.isNameOfTraditionalFixedPointType(dataTypeNameStr))
...
function DataType = resolveNonNumericInput(firstInputArg)

dataTypeNameStr = firstInputArg;

DataType = getFromBuiltInTypesStr(dataTypeNameStr);
