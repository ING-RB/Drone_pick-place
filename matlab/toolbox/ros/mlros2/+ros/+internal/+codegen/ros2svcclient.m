...
%      "Liveliness"  - Level of reporting that they require from Service Servers 
...
%#codegen
    ...
    methods
        ...
        % Parse the inputs to the constructor
            ...
            validateattributes(qosDeadline,{'double'},{'scalar', 'nonnegative', 'nonnan'},'ros2svcclient','Deadline');

            qosLifespan = coder.internal.getParameterValue(pStruct.Lifespan,0,varargin{:});
            if qosLifespan==Inf
                qosLifespan=0;
            end
            validateattributes(qosLifespan,{'double'},{'scalar', 'nonnegative', 'nonnan'},'ros2svcclient','Lifespan');

            qosLiveliness = coder.internal.getParameterValue(pStruct.Liveliness,'automatic',varargin{:});
            validateStringParameter(qosLiveliness,{'automatic','default','manual'},'ros2svcclient','Liveliness');

            qosLeaseDuration = coder.internal.getParameterValue(pStruct.LeaseDuration,0,varargin{:});
            if qosLeaseDuration==Inf
                qosLeaseDuration=0;
            end
            validateattributes(qosLeaseDuration,{'double'},{'scalar', 'nonnegative', 'nonnan'},'ros2svcclient','LeaseDuration');
 
            qosAvoidROSNamespaceConventions = coder.internal.getParameterValue(pStruct.AvoidROSNamespaceConventions,false,varargin{:});
            validateattributes(qosAvoidROSNamespaceConventions,{'logical'},{'nonempty'},'ros2svcclient','AvoidROSNamespaceConventions');
