...
%
%   ACTLIST = ros2("action","list") lists action names that are currently
%   registered on the ROS 2 network through either servers or clients.
%   Simplified form:
%   ros2 action list
%
%   ACTIONTYPES = ros2("action","type",ACTNAME) lists action types that are
%   currently registered on the ROS2 network for the provided ACTNAME.
%   Simplified form:
%   ros2 action type ACTNAME
%
...
%   contents of the ros2bagreader FOLDERPATH as a structure. The information
...
function [cmd, subcmd, cmdoption, domainid] = getparameters(varargin)
    expectedCommands = {'node','topic','service','action','msg','bag','DomainID'};
    stringOfCommands = 'node, topic, service, action, msg, bag';
    expectedNodeCommands = {'list'};
    expectedTopicCommands = {'list','endpoints'};
    expectedServiceCommands = {'list', 'type', 'endpoints'};
    expectedActionCommands = {'list', 'type', 'endpoints'};
    ...
    while argsParsed < nargin
        ...
        switch opt
          ...
          case 'topic'
            ...
            if (nargin > argsParsed + 2) 
                if isequal(varargin{argsParsed+3}, '-t') % i.e. we have more args and next is -t
                    subcmd = 'listtypes';
                    argsParsed = argsParsed+3;
                elseif any(strcmpi(varargin{argsParsed+2}, {'endpoints'}))
          ...
          case 'service'
            ...
            elseif isequal(subcmd, 'type')
                ...
                argsParsed = argsParsed+3;
            elseif isequal(subcmd, 'endpoints')
                argsParsed = argsParsed+2;
            else
                argsParsed = argsParsed+2;
            end
          case 'action'
            ...
            cmd = opt;
            subcmd = validatestring(varargin{argsParsed+2},expectedActionCommands, argsParsed+2);
            if (nargin > argsParsed + 2) && isequal(varargin{argsParsed+3}, '-t') % i.e. we have more args and next is -t
                subcmd = 'listtypes';
                argsParsed = argsParsed+3;
            elseif isequal(subcmd, 'type')
                narginchk(argsParsed+3,nargin)
                cmdoption = varargin{argsParsed+3};
                validateattributes(cmdoption,{'char','string'},{'nonempty'},'ros2','acttype',argsParsed+3)
                argsParsed = argsParsed+3;
            elseif isequal(subcmd, 'endpoints')
                argsParsed = argsParsed+2;
            else
