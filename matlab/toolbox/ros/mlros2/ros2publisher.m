classdef ros2publisher < ros.ros2.internal.QOSUser & ...
    ...
    properties (Transient, Access = ?ros.internal.mixin.InternalAccess)
        ...
        %CurrentRMW - Currrent RMW implementation needed for determining
        %whether we should enable qos event callbacks or not
        CurrentRMW
    end

    properties (Hidden,SetAccess = private)
        %EnableBuffering - When this is true, messages will be buffered to
        %publish the messages at faster rate. By default it is true.
        EnableBuffering = true

        %EnableValidation - When true, message validation is done before
        %sending it to buffer.
    ...
    properties (Transient, Access = ?matlab.unittest.TestCase)
        %InternalMsgUtils - MCOS C++ object which contains utils for
        %messages.
        InternalMsgUtils
    end
    methods (Access = public)
        ...
        %   (help ros2publisher) for more details.
            ...
            obj.NewMessageCallbackHandler = ...
                ros.internal.CallbackHandler(matlab.internal.WeakHandle(obj), ...
                                             @processNewMessage, @deadlinePubCallback, @livelinessLostCallback, @IncompatibleQoSPubCallback);
            
            obj.DeadlineMissedPubQoSFcn = paramParser.Results.DeadlineCallback;
            obj.LivelinessLostQoSFcn = paramParser.Results.LivelinessCallback;
            obj.IncompatibleQoSPubFcn = paramParser.Results.IncompatibleQoSCallback;
            ...
            if obj.EnableValidation && obj.EnableBuffering
                try
                    % Setup env variables
                    [pathEnv, amentPrefixEnv, cleanPath, cleanAmentPath] = ros.internal.ros2.setupRos2Env(); %#ok<ASGLU>
                    obj.InternalMsgUtils = ros2utils.msgutils.internal.Ros2msgUtilsWrapper;
                    if ~ifMessageTypeRegistered(obj.InternalMsgUtils, obj.MessageType)
                        ...
                        dllPaths{end + 1} = obj.MessageInfo.path;

                        registerMessageType(obj.InternalMsgUtils, ...
    		                obj.MessageType, cppFactoryClass, cppElementType, dllPaths)
                    end

                    % This is needed until g2946584 is fixed
                    obj.InternalMsgUtils.validate(obj.MessageType,ros2message(obj.MessageType));
                catch
                    %Incase of any issues, disable buffering to use old
                    %implementation.
                    enableBuffering(obj,false)
                    disp(getString(message('ros:internal:transport:BufferingDisabled')))
                end
            end
            ...
            % Set up ordered inputs
                ...
                paramParser = addQOSEventsToParser(obj, paramParser, 'ros2publisher');
            end
        end
        ...
        %   removes it from the network
            ...
            obj.DeadlineMissedPubQoSFcn = [];
            obj.LivelinessLostQoSFcn = [];
            obj.IncompatibleQoSPubFcn = [];
        ...
        % Do basic error checking (further checking done internally)
            ...
            try
                if obj.EnableValidation && obj.EnableBuffering
                    obj.InternalMsgUtils.validate(obj.MessageType,msg);
                end
                % disableMessageQueue(obj.InternalNode, obj.ServerPublisherHandle)
    ...
    methods(Hidden)
        function enableBuffering(obj,val)
            validateattributes(val, {'logical'}, {'scalar'}, 'enableBuffering', 'val')
            obj.EnableBuffering = val;
            if val
                enableMessageQueue(obj.InternalNode,obj.ServerPublisherHandle);
            else
                disableMessageQueue(obj.InternalNode,obj.ServerPublisherHandle);
            end
        end
    ...
    methods (Access = private)
        ...
        %createPublisher Create publisher on ROS 2 network
            ...
            deadlineCallbackFcn = obj.NewMessageCallbackHandler.DeadlineCallbackName;
            livelinessCallbackFcn = obj.NewMessageCallbackHandler.LivelinessCallbackName;
            incompatibleQoSCallbackFcn = obj.NewMessageCallbackHandler.IncompatibleQoSCallbackName;
            ...
            try
                returnCall = addPublisher(obj.InternalNode, ...
                                          ...
                                          deadlineCallbackFcn, ...
                                          livelinessCallbackFcn, ...
                                          incompatibleQoSCallbackFcn, ...
                                          obj.CurrentRMW, ...
                                          additionalSettings, ...
                                          dllPaths);
                ...
                if obj.EnableBuffering
                    % Enable the message queue to buffer the messages
                    enableMessageQueue(obj.InternalNode,obj.ServerPublisherHandle);
                end

                % Initialize qos event callback to process requests.
                initQoSEventCallback(obj.InternalNode, ...
                                      returnCall.handle, ...
                                      obj.NewMessageCallbackHandler, ...
