...
%   Copyright 2019-2024 The MathWorks, Inc.
    ...
    methods
        ...
        %   (help ros2subscriber) for more details.
            ...
            obj.NewMessageCallbackHandler = ...
                    ros.internal.CallbackHandler(matlab.internal.WeakHandle(obj), ...
                                                 @processNewMessage, @deadlineSubCallback, @livelinessChangedCallback, @IncompatibleQoSSubCallback, @messageLostCallback);
                
            obj.NewMessageFcn = cb;
            obj.DeadlineMissedSubQoSFcn = paramParser.Results.DeadlineCallback;
            obj.LivelinessChangedQoSFcn = paramParser.Results.LivelinessCallback;
            obj.IncompatibleQoSSubFcn = paramParser.Results.IncompatibleQoSCallback;
            obj.MessageLostFcn = paramParser.Results.MessageLostCallback;
            ...
            % node and topic always needed, type and callback optional
                ...
                paramParser = addQOSEventsToParser(obj, paramParser, 'ros2subscriber');
        ...
        %   removes it from the network
            ...
            obj.NewMessageFcn = [];
            obj.DeadlineMissedSubQoSFcn = [];
            obj.LivelinessChangedQoSFcn = [];
            obj.IncompatibleQoSSubFcn = [];
            obj.MessageLostFcn = [];
        ...
        % Do not modify handler if callback is disabled for subscriber
            if obj.EnableCallback %#ok<MCSUP>
                if isempty(cb)
    ...
    methods(Hidden)
        % function setMsgPreprocessing sets the preprocessing conditions
        % for message data
        function setMsgPreprocessing(obj, messageOption, path)

            if obj.MessageType == "sensor_msgs/CompressedImage" && messageOption == "image"
                % for compressed image, we don't require any processing,
                % hence returning from here
                return;
            end

            readMessageOptions = ["message", "image", "compressedimage", "pointcloud", "laserscan"];
            idxReadMessageOption = int32(find(readMessageOptions == messageOption));
            updateSubDetails(obj.InternalNode, ...
                               obj.ServerSubscriberHandle, ...
    ...
    methods (Access = private)
        ...
        % If enabled, set up to use callback handler method as callback
            ...
            messageLostCallbackFcn = obj.NewMessageCallbackHandler.MessageLostCallbackName;
