classdef LnxNodeExecutor < ros.codertarget.internal.NodeInterface & ...
    ...
    methods
        ...
        function runNode(obj,modelName,workspaceFolder,rosMasterURI,nodeHost,varargin)
            ...
            % Assumes that workspaceFolder name has already
            % been expanded
            ...
            if obj.IsRemote
                ...
                if isequal(obj.ROSVersion,'ros')
                    ...
                    % Run command
                    cmd = [' ' quotePath(nodeExecutable) ' ' cmdArgs ' > ' quotePath(logFile) ' 2>&1 &'];
                    ...
                    rmwImplementation = ros.internal.utilities.getCurrentRMWImplementation;
                    orgPathEnvValue = getenv('PATH');
                    if ismac % For maci64 and maca64
                        archSpecificEnv = 'DY_LDLIBRARY_PATH';
                    else % For glnxa64
                        archSpecificEnv = 'LD_LIBRARY_PATH';
                    end
                    orgArchSpecificPathEnvValue = getenv(archSpecificEnv);

                    customMsgRegistry = ros.internal.CustomMessageRegistry.getInstance('ros2');
                    customMsgDirList = getBinDirList(customMsgRegistry);
                    setenv('CUSTOM_LD_LIBRARY_PATH',strjoin(customMsgDirList, pathsep));
                    resetCustomLdLibraryPath = onCleanup(@()unsetenv('CUSTOM_LD_LIBRARY_PATH'));

                    [clearRMW, resetDDSEnv,resetPath,cleanPlatformPath] = ...
                        ros.ros2.internal.setRMWImplPathEnv(rmwImplementation, ...
                                                            orgPathEnvValue, ...
                                                            archSpecificEnv, ...
                                                            orgArchSpecificPathEnvValue); %#ok<ASGLU>

                    resetEnv = ros.ros2.internal.setUserMiddlewareEnvironment(rmwImplementation); %#ok<NASGU>
   
                    [~,packageName] = fileparts(fileparts(nodeExecutable));
                    cmd = ['run ' packageName ' ' nodeName ' > ' quotePath(logFile) ' 2>&1 &'];
        ...
        function stopNode(obj,modelName)
            ...
            % Stop the RouDi Application, if no node is using it.
            rmwImplementation = ros.internal.utilities.getCurrentRMWImplementation;
            if ~obj.IsRemote && strcmp(rmwImplementation, 'rmw_iceoryx_cpp')
        ...
        function folder = expandFolder(obj,folder)
            % From '~muser/foo/bar' to '~muser/"foo/bar"'
            tmp = regexp(folder,'(?<tilde>^~\w*/)(?<dir>.*)','names');
            % ~/new dir(1)/ros_ws -> ~/"new dir(1)/ros_ws". Double quotes
            % to allow veriable expansion
            if ~isempty(tmp)
                folder = [tmp.tilde doubleQuotePath(tmp.dir)];
                folder = strtrim(system(obj.SystemExecutor, ['echo ' folder]));
            end
        end
    end
    ...
    methods (Access = ?ros.slros.internal.InternalAccess)
        ...
        %   input arguments have been validated. 
            ...
            % does not exist.
            try
                logOut = string(system(obj.SystemExecutor,['cat ' quotePath(logFile) ' ']));
            catch
                logOut = '';
            end
        ...
        %   folders.
            ...
            try
                ...
                % writable
                system(obj.SystemExecutor, ['[ -w ' quotePath(fileName) ' ]']);
    ...
    methods (Access = protected)               
        function ret = getRunCommand(obj, workspaceFolder, ...
            ...
            if isequal(obj.ROSVersion,'ros')
                ...
                sudoBashCmd = obj.SystemExecutor.commandWithSudo(quotePath(nodeExecutable));
                ...
                ret = [ret 'cd ' quotePath([workspaceFolder '/devel']) ' ; '...
                    'source ' quotePath([workspaceFolder '/devel/setup.bash']) '; ' ... % Source the Catkin workspace
                    sudoBashCmd ' ' cmdArgs ' ' ...   % Run node executable
                    '&> ' quotePath(logFile) ' &'...  % Pipe all output into a log file
                    ];
            else
                ...
                rmwImplementation = netProf.getCurrentRMWImplementation;
                ...
                ret = ['export DISPLAY=:0.0; ' ...
                    ...
                    'cd ' quotePath([workspaceFolder '/install/' modelname ]) '; ' ...
                    'source ' quotePath([workspaceFolder '/install/setup.bash']) '; ' ...  % Source the ROS 2 project folder
                    quotePath(nodeExecutable) ' ' args ' ' ...               % Run node executable
                    '&> ' quotePath(logFile) ' &'...                         % Pipe all output into a log file
                    ];
            end
        end
        ...
        function nodeExecutable = getNodeExecutable(obj,nodeName,exeFolder)
            ...
            % executables
            cmd = ['find ' quotePath(exeFolder)  ' -executable -type f -name ' nodeName];
            try
        ...
        function killNodeForcefully(obj,nodeName)
            ...
            if status ~= 0
                if contains(result,'Operation not permitted')
                    ...
                    error(message('ros:slros:rosdevice:StopROSNodeNoPrivileges',nodeName,''));
                elseif contains(result,'no process found')
                    ...
                    error(message('ros:slros:rosdevice:StopROSNodeError',nodeName,result));
                end
            end
        end
    end
...
function pathStr = doubleQuotePath(pathStr)
pathStr = ['"' pathStr '"'];
end
