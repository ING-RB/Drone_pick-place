...
%   Copyright 2016-2024 The MathWorks, Inc.
    ...
    properties (Constant)
        ...
        DefaultCatkinWorkspace  = '/home/user/catkin_ws'
        DefaultROS2Workspace    = '/home/user/ros2_ws'
        DefaultSSHPort          =  22;
        DefaultROSInstall       = '/opt/ros/noetic'
        DefaultROS2Install      = '/opt/ros/jazzy'
        DefaultROS2InstallHumble      = '/opt/ros/humble'
    end
    ...
    methods (Static)
        ...
        function ret = setGetPassword(password)
        %setPassword Set and get the persistent password if the user selects to not save it

            persistent persistantPassword;
            if ~ischar(persistantPassword)
                persistantPassword = '';
            end

            if nargin
                persistantPassword = password;
            end
            ret = persistantPassword;
        end
        ...
        %getPassword Get password for login
            ...
            deviceParams = ros.codertarget.internal.DeviceParameters;
            if deviceParams.getParam('savepassword')
                % Get password from preferences if user has selected "Remember my password"
                password = deviceParams.getParam('password');
            else
                % Get the persistent password if the user has selected to
                % not save password
                password = deviceParams.setGetPassword;
            end
        ...
        %setPassword Set password for login
            ...
            if deviceParams.getParam('savepassword')
                % Save password to preferences if user has selected "Remember my password"
                deviceParams.setParam('password', password);
            else
                % Set the persistent password if the user has selected to
                % not save password
                deviceParams.setParam('password', '');
                deviceParams.setGetPassword(password);
            end
        end
