...
%   Copyright 2020-2021 The MathWorks, Inc.
    ...
    methods (Access = public)
        ...
        % Parse the input
            ...
            try
                ...
                if useRandPort && ~ros.internal.NetworkIntrospection.isValidMaster(obj.MasterURI)
                    % The core is being launched from test and it could not
                    % start properly. Throw error so that in the exception
                    % handling it will try to recreate once again with
                    % different port
                    error(message('ros:mlros:core:FailedToStart', obj.Port));
                end
            catch ex
                if useRandPort
                    % The core is launched from test. There could be some
                    % issue with the selected port. Try once with another
                    % port.
                    newPort = ros.internal.findOpenCorePort;
                    disp("ROS master failed on port " + num2str(obj.Port) + "; retrying on " + num2str(newPort) + ".");
                    obj.Port = newPort;
                    obj.launchCore(isMasterCheckDone);
                else
                    throw(ex)
    ...
    methods (Access = private)
        ...
        %   another core) is already running on the same port.
            ...
            % Logging messages if MW_ROS_CORE_SHUTDOWN_LOG_MSGS is enabled
            if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
                if ~ispc
                    [status, result] = system(sprintf('ps ax | grep -E ''ros|python'''));
                    disp("Status before launching ros core :"+status)
                    disp("result before launching ros core: "+result)
                else
            ...
            if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
                if ~ispc
                    [status, result] = system(sprintf('ps ax | grep -E ''ros|python'''));
                    disp("Status after launching ros core :"+status)
                    disp("result after launching ros core: "+result)
                else
                    [status, result] = system(sprintf('tasklist | findstr /I "ros python WindowsTerminal"'));
                    disp("Status after launching ros core :"+status)
                    disp("result after launching ros core: "+result)
        ...
        %   delete the object.
            ...
            if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
                if ~ispc
                    [status, result] = system(sprintf('ps ax | grep -E ''ros|python'''));
                    disp("Status before deleting core :"+status)
                    disp("Result before deleting core :"+result)
                else
                    [status, result] = system(sprintf('tasklist | findstr /I "ros python WindowsTerminal"'));
                    disp("Status before deleting core :"+status)
                    disp("Result before deleting core :"+result)
            ...
            if obj.IsCoreStarted
                ...
                if ispc
                    ...
                    [killStatus, result]= ros.internal.killROSProcess(obj.RosMasterCmd,obj.RosCoreCommandName);

                    if isequal(killStatus,false)
                        if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
                            disp("Result for failed Process Killed : " + result)
                        end
                        warning(message('ros:mlros:core:ShutdownError'));
                    ...
                    try
                        [status, result] = ros.internal.killProcessByPID(obj.RosCorePID);
                    catch error
                        if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
                            disp("error occured while killing process by PID")
                            disp(error.message)
                        end
                    end
                ...
                try
                    ...
                    if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
                        disp("waitForProcessToDie failed")
                        disp(error.message)
                    end
                end
