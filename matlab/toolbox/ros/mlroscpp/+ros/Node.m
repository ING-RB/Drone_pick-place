...
%   Copyright 2020-2023 The MathWorks, Inc.
    ...
    properties (Constant, Access = ?ros.internal.mixin.ROSInternalAccess)
        ...
        DefaultLogPath = fullfile(tempdir, [ 'ros1_log_' char(datetime('now','Format','ddMMyyyyHHmmssSSS'))]);
        ...
        %   down.
            if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
                if ~ispc
                    ...
                    disp("Status before deleting node :"+status)
                    disp("Result before deleting node :"+result)
                else
            ...
            if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
                if ~ispc
                    [status, result] = system(sprintf('ps ax | grep -E ''ros|python'''));
                    disp("Status after deleting node :"+status)
                    disp("Result after deleting node :"+result)
                else
                    [status, result] = system(sprintf('tasklist | findstr /I "ros python WindowsTerminal"'));
                    disp("Status after deleting node :"+status)
                    disp("Result after deleting node :"+result)
