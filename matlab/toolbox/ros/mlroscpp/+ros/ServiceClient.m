...
%                     Default: "object"
%
%    "IsPersistent" - Determines if the services connection is persistent. 
%                     With a persistent connection, a client always stays  
%                     connected to a service. The connection persists until  
%                     the service client gets deleted or the service server 
%                     becomes unavailable. 
%                     In case of non-persistent connection, a client normally 
%                     does a lookup and reconnects to a service each time.
%                     This connection is slower but potentially allows a    
%                     client to connect to a different node each time it    
%                     does a service call, assuming that the lookup returns
%                     a different node. 
%                     Persistent connections significantly improve performance 
%                     for repeated requests, however, they also make the  
%                     client more fragile to service failures when the 
%                     connection is lost.
%                     Options are:
%                        true  - Client makes a persistent connection
%                        false - Client makes a nonpersistent connection
%                     Default: true
%
...
%   Copyright 2014-2022 The MathWorks, Inc.
    ...
    properties (SetAccess = private)
        ...
        %IsPersistent - Indicates if the service client creates persistent or not
        IsPersistent
    end
    ...
    properties (Constant, Access = private)
        ...
        %DefaultIsPersistent - Default persistent value
        DefaultIsPersistent = true
    end
    ...
    methods (Access = public)
        ...
        % If no node specified, use the global node.
            ...
            % Persistent settings
            obj.IsPersistent = paramParser.Results.IsPersistent;
            persistentSettings = ...
                struct('isPersistent', obj.IsPersistent);
            ...
            createSvcClient(obj,serviceName,persistentSettings);
        end
    ...
    methods (Access = ?matlab.unittest.TestCase)
        ...
        % node and service name are always needed, type is optional
            ...
            addParameter(paramParser, 'IsPersistent', obj.DefaultIsPersistent, ...
                         @(x) validateattributes(x, {'logical'}, ...
                                                 {'scalar','nonempty'}, ...
                                                 'ServiceClient', 'IsPersistent'));
