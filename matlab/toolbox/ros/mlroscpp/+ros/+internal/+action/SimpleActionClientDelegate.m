classdef SimpleActionClientDelegate < ...
    ...
    methods
        ...
        %createActionClient Create simple action client for an action name and type
            ...
            onCancelReceivedCallbackFcn='';
            onCancelAllReceivedCallbackFcn='';
            onCancelBeforeRecivedCallbackFcn='';
        ...
        %sendGoal Send a goal message to the action server
            ...
            %ROS1 and ROS2 use same signatures for backend API.
            %ROS2 supports multiple goals for action client and requires
            %goalIndex to be sent from the front end. In order to keep the same
            %signature between ROS1 and ROS2, a dummy goalIndex is passed
            %to ROS1 backend as an argument.
            %Dummy GoalIndex.
            goalIndex = 0 ;
            sendGoalToActServer(internalNode, actClientHandle,goalIndex, goalMsg);
        end

        function cancelGoal(~, internalNode, actClientHandle)
        %cancelGoal Cancel an active or pending goal (sent by this client)

            cancelGoal(internalNode, actClientHandle,0); %Dummy Goal Index
        end
        ...
        %   with the state.
            ...
            if ~isempty(internalNode) && ...
                ...
                % get the state.
                res = getActState(internalNode, actClientHandle,0);
        ...
        %getResult gets the result of last goal sent to action server.
            ...
            if ~isempty(internalNode) && ...
                ...
                % get the result.
                result = getActResult(internalNode, actClientHandle,0);
            end
        end
    end
end
