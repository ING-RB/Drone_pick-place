classdef ActionIntrospection < ...
    ...
    methods
        ...
        function infoString = infoStructToString(~, infoStruct)
            ...
            % Print action type
            infoString = infoString.append([message('ros:mlros:action:InfoType').getString ': ' infoStruct.ActionType newline]);
            infoString = infoString.append(newline);
            
            % Print message types
            infoString = infoString.append([message('ros:mlros:action:InfoGoalType').getString ': ' infoStruct.GoalMessageType newline]);
            infoString = infoString.append([message('ros:mlros:action:InfoFeedbackType').getString ': ' infoStruct.FeedbackMessageType newline]);
            infoString = infoString.append([message('ros:mlros:action:InfoResultType').getString ': ' infoStruct.ResultMessageType newline]);
            infoString = infoString.append(newline);
            
            % Print information about action server(s)
            infoString = infoString.append([message('ros:mlros:action:InfoServer').getString ':']);
            if isempty(infoStruct.ActionServer)
                infoString = infoString.append([' ' message('ros:mlros:action:InfoNone').getString]);
            end
            infoString = infoString.append(newline);
            
            for i = 1:numel(infoStruct.ActionServer)
                infoString = infoString.append(['* ' infoStruct.ActionServer(i).NodeName ...
                    ' (' infoStruct.ActionServer(i).URI ')' newline]);
            end
            infoString = infoString.append(newline);
            
            % Print information about action clients
            infoString = infoString.append([message('ros:mlros:action:InfoClients').getString ':']);
            ...
            for i = 1:numel(infoStruct.ActionClients)
                infoString = infoString.append(['* ' infoStruct.ActionClients(i).NodeName ...
                    ' (' infoStruct.ActionClients(i).URI ')' newline]);
            end
        end
    end
