        ...
        function n = nodeInstance(op, name, masterURI, nodeHost)
            ...
            function initNode(name, masterURI, nodeHost)
                if isempty(name)
                    ...
                    if ~isempty(getenv('TEST_HARNESS')) && ~isempty(getenv('TestLogLevel'))
                        % increase the LogLevel in test environment 
                        nodeCreateFcn = @(n,uri,nhost)ros.internal.Global.createNodeWithDefaultName(...
                            n, uri, 'NodeHost', nhost, 'LogLevel', str2double(getenv('TestLogLevel')));
                    else
            ...
            function clearNode
                ...
                if ~isempty(node)
                    if isValidNode(node)
                        ...
                        disp(getString(message('ros:mlros:node:ShuttingDownInvalidGlobal')));
                    end
                    delete(node);
                    node = [];
                end
