    ...
    methods (Access = public, Static)
        ...
        %      types - Cell array of strings of topic types
            ...
            try
                ...
                error(message('ros:mlros:topic:CannotRetrieveTopic'));
            end
        end
        ...
        %if the MasterURI is different, a new node is created
            ...
            if active && isValidNode(node) && (isempty(masterURI) || isequal(node.MasterURI, masterURI))
                % return the global node
                return;
            elseif isempty(masterURI) 
                if ~active
                    error(message('ros:mlros:node:GlobalNodeNotRunning'));
                elseif ~isValidNode(node)
                    error(message('ros:mlros:node:GlobalNodeNotInGoodState'));
                end
            end

            if isempty(Node) || ~isValidNode(Node) || ~isequal(Node.MasterURI, masterURI)
                %Make the nodename unique by using current time-stamp.
                nodeName = ['/matlab_introspec_' char(datetime('now','Format','ddMMyyyyHHmmssSSS'))];
