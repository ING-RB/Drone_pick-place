...
% Copyright 2019-2024 The MathWorks, Inc.
    ...
    methods (Hidden)
        ...
        function ret = setROSControlPkgInfo(~, mdlName,pkgInfo,srcFiles,incFiles)
            ret = ros.codertarget.internal.ROSControlUtil.setROSControlPkgInfo(mdlName,pkgInfo,srcFiles,incFiles);
        end

        function copyControllerPluginFiles(~,anchorDir, pkgName, bDir)
            ros.codertarget.internal.ROSControlUtil.copyControllerPluginFiles(...
                    anchorDir,...
                    pkgName,...
                    bDir);
        end
        ...
        function actionDep = getActionDependencies(~, includeList)
            actionDep = {};
            if ismember('mlroscpp_actclient.h',includeList) || ismember('mlroscpp_actserver.h',includeList)
                actionDep = {'actionlib'};
            end
        end
        ...
        function tfDep = getTransformationDependencies(~, includeList)
            tfDep = {};
            if ismember('mlroscpp_transform.h',includeList) || ismember('slros_generic_transform.h',includeList)
                tfDep = {'tf2_ros'};
            end
        end
    end
