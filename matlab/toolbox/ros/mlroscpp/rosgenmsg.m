...
%      folderPath = fullfile(pwd,"rosCustomMessages");
...
%      rosgenmsg(folderPath,CreateShareableFile=true)
%
...
% Input processing
    ...
    customRegistry = ros.internal.CustomMessageRegistry.getInstance('ros', true);
    ...
    msgFilesDirs = [{folderPath} customROSMessageShareDir {rosmsgSrcPathRoot,extraROSMessagePath}];
    ...
    libConsoleBridgeMap = containers.Map({'win64','maci64', 'maca64','glnxa64'}, ...
                                         { ...
    ...
    libClassLoaderMap = containers.Map({'win64','maci64', 'maca64','glnxa64'}, ...
                                       { ...
    ...
    libRosconsoleMap = containers.Map({'win64','maci64', 'maca64','glnxa64'}, ...
                                      { ...
    ...
    libRoscppMap = containers.Map({'win64','maci64', 'maca64','glnxa64'}, ...
                                  { ...
    ...
    libRoscppSerializationMap = containers.Map({'win64','maci64', 'maca64','glnxa64'}, ...
                                               { ...
    ...
    libRosTime = containers.Map({'win64','maci64', 'maca64','glnxa64'},...
                                {...
    ...
    % rosbag changes
    librosbag_storage = containers.Map({'win64','maci64', 'maca64','glnxa64'},...
                                {...
    ...
    libroslz4 = containers.Map({'win64','maci64', 'maca64','glnxa64'},...
                                {...
    ...
    libtopic_tools = containers.Map({'win64','maci64', 'maca64','glnxa64'},...
                                {...
    ...
    libmatlab_rosbag = containers.Map({'win64','maci64', 'maca64','glnxa64'},...
                                {...
    ...
    libactionlib = containers.Map({'win64','maci64', 'maca64','glnxa64'},...
                                {...
    ...
    if ispc
        boostThreadLib = dir(fullfile(libBoostDirs,'mwboost_thread*.lib'));
        libboostthread = containers.Map('win64',fullfile(libBoostDirs,boostThreadLib.name));
    else
        libboostthread = containers.Map({'maci64', 'maca64','glnxa64'},...
            {...
    ...
    for iPkg = 1:numel(pkgDirs)
        ...
        if iPkg == 1
            builder = ros.internal.CatkinBuilder(genDir, pkgInfos{iPkg}, UseNinja=useNinja, SuppressOutput=suppressOutput);
        else
    ...
    for dirIndex = 1:length(customInstallDirs)
        ...
        cmake_prefix_path = [cmake_prefix_path ';' customInstallDirs{dirIndex}]; %#ok<AGROW>
    end
    ...
    %Store the Release information, MessageTypes, ServiceTypes and ActionTypes
    %into a MAT file. This information can be used when customers want to
    %share these generated custom messages to other machines.
    ...
    if isequal(createShareableFile,true)
        ...
        outZipFile = fullfile(folderPath,'matlab_msg_gen_ros1.zip');
        if isequal(suppressOutput, false)
        ...
        zip(outZipFile,[pkgsToZip,{fullfile(folderPath,'matlab_msg_gen_ros1',computer('arch'),'src','VersionInfo.mat')},{fullfile(folderPath,'matlab_msg_gen_ros1',computer('arch'),'install')}]);
        clear dotprinter;
    end

    %Give the final instructions to the user after rosgenmsg.
