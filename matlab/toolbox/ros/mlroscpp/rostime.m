...
% The operation should be parsed and valid
    if isSystemTime
        ...
        % ROS and ROS 2 treats all time as UTC.
        % POSIXTIME(T) - If T is unzoned, then
        ...
        secs = floor(systemEpochTime);
        nsecs = round((systemEpochTime - secs) * 1e9);
        % Normalize the second and nanosecond values
        [sec,nsec] = ros.internal.Parsing.normalizeSecsNsecs(secs,nsecs);

        if strcmpi(dataFormat, 'struct')
            currentTime.Sec = uint32(sec);
            currentTime.Nsec = uint32(nsec);
        else
            currentTime = ros.msg.Time(sec, nsec);
        end
    else
        % Let ROS decide what time to return (system or simulation time)
        if strcmp(dataFormat,'struct')
            [currentTime, isSimTime] = getCurrentROSTimeStruct;
        else
...
function [currentTime, isSimTime] = getCurrentROSTimeStruct
%getCurrentROSTime Retrieve the current ROS time

% Return the current time
    timeObj = ros.internal.Time;
    currentTime = timeObj.CurrentTimeStruct;
    isSimTime = timeObj.IsSimulationTime;
end

function sysTime = getCurrentSystemTimeStruct
%getCurrentSystemTime Retrieve Unix system time as ROS time structure
% Construct time object
    timeObj = ros.internal.Time;
    sysTime = timeObj.CurrentSystemTimeStruct;
end
