...
if ispc
    ...
    execCommand = [cmd, ' ', extrasetup, ' ', catkinPrefixPath, ' ', pyenvDir, ' ', cmakeDir, ' ', cmdlinewindows];
else
...
if nargin > 2
    pathEnv = ['"' fullfile(matlabroot,'bin',computer('arch'),'cmake','bin') ':' getenv('PATH') '"'];
    ...
    execCommandArgsMap('maca64')  = ['bash -c ', '''{ PATH=', pathEnv '; ' execCommand , '; } ' logFileCmd, ''''];
