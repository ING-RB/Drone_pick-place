...
% check if the process is running or not
    ...
    if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
        disp("* Inside killROSProcess function *");
    end
    ...
    if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
        disp("* Exiting killROSProcess function *");
    end
end
...
% Create a map of name of applications per platform
    appNameMap = containers.Map({'win64','maci64','maca64','glnxa64'}, ...
                                {[appName, '.exe'], appName,appName, appName});
...
% killproc kill the application
    ...
    % Create a map of name of applications per platform
    appNameMap = containers.Map({'win64','maci64','maca64','glnxa64'}, ...
                                {cmdWindow, cmdWindow, cmdWindow, cmdWindow});
    % Create a map of system commands that will query for running application
    isAppRunningCmdMap = containers.Map({'win64','maci64','maca64','glnxa64'}, ...
    ...
    if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
        disp("Status after Process Killed: "+status)
        disp("Result after Process Killed: "+result)
    end
    ...
    if getenv('MW_ROS_CORE_SHUTDOWN_LOG_MSGS')
        [st, res] = system(sprintf('tasklist | findstr /I "ros python WindowsTerminal"'));
