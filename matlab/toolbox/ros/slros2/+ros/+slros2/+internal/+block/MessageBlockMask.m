classdef MessageBlockMask < ros.slros.internal.block.CommonMessageMask
    ...
    methods
        ...
        function messageClassChange(obj, block)
            ...
            % don't change it
            msgList = obj.getMessageList();
            srvList = obj.getServiceList();
            actList = obj.getActionList();

            if obj.MsgMaskObj.isEntityTypeCompatible(block,msgList,srvList,actList)
        ...
        function selectButtonPressed(obj, block)
            ...
            %   the service type selection dialog or the action type selection dialog.

            if obj.MsgMaskObj.isActionClass(block)
                % Configure a action type selection dialog
                msgDlg = ros.slros2.internal.dlg.ActionTypeSelector();
            elseif obj.MsgMaskObj.isServiceClass(block)
    ...
    methods(Hidden,Static)
        ...
        function actList = getActionList()
            actList = ros.ros2.internal.Introspection.getAllActionTypesStatic;
        end

        function srvList = getServiceList()
            srvList = ros.ros2.internal.Introspection.getAllServiceTypesStatic;
        end
        ...
        function allMsgs = getMessageList()
            ...
            srvList = ros.ros2.internal.Introspection.getAllServiceTypesStatic;
            actList = ros.ros2.internal.Introspection.getAllActionTypesStatic;
            tagList = {'Request','Response','Feedback','Goal','Result'};
            
            filteredMsg = zeros(numel(msgList),1,'logical');
            for i=1:numel(msgList)
                tagIdx = find(cellfun(@(x)endsWith(msgList{i},x),tagList), 1);
                if ~isempty(tagIdx)
                    typeName = extractBefore(msgList{i},tagList{tagIdx});
                    isService = any(strcmp(srvList,typeName));
                    isAction = any(strcmp(actList,typeName));
                    if isService || isAction
                        filteredMsg(i) = true;
                    end
                end
            end

            allMsgs = msgList(~filteredMsg);
        end
    end
end
