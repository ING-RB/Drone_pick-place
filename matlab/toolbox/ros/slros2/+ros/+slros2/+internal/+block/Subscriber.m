classdef Subscriber < ros.slros2.internal.block.ROS2PubSubBase & ...
    ...
    methods (Access = protected)
                ...
                if ~isCodegen
                    ...
                    if newMessageCount ~= obj.LastMessageCount
                        ...
                        if isequal(latestMsg, obj.LastReceivedMsg)
                            ...
                            msg = obj.ConversionFcn(latestMsg, obj.EmptySeedBusStruct, obj.TruncateAction, obj.ModelName, obj.Cast64BitIntegersToDouble);
        ...
        function releaseImpl(obj)
            ...
            elseif coder.target('Rtw')
                % Reset subscriber pointer
                coder.ceval([obj.BlockId, '.resetSubscriberPtr(); //']);
            end
        end
    end
