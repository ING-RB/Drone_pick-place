classdef ReadImage <  ros.slros2.internal.block.ReadXBase ...
...
%                       'bayer_rggb8', 'bayer_rggb16',       ...     %bayer_rggbX
%                       'bayer_bggr8', 'bayer_bggr16',       ...     %bayer_bggrX
%                       'bayer_gbrg8', 'bayer_gbrg16',       ...     %bayer_gbrgX
%                       'bayer_grbg8', 'bayer_grbg16']               %bayer_grbgX 
%
...
%#codegen
    ...
    properties (Dependent, Access = private, Nontunable)
        ...
        %InputNumChannels Number of channels in input image signal
        InputNumChannels

        %OutputNumChannels Number of channels in output image signal
        OutputNumChannels
    ...
    properties (Constant, Access = {...
        ...
        InvalidTypeID = 'ros:slros2:image:InvalidType'
    end
    ...
    properties (Constant, Access = {?matlab.unittest.TestCase, ?ros.slros2.internal.block.ReadImage})
    ...
    properties(Constant, Hidden)
        ImageEncodingSet = matlab.system.StringSet( ...
            ros.msg.sensor_msgs.internal.ImageEncoding.AllEncodings);
    end
    ...
    methods
        ...
        function val = get.InputNumChannels(obj)
            val = obj.ImageEncodingInternal.NumChannels;
        end

        function val = get.OutputNumChannels(obj)
            if obj.ImageEncodingInternal.IsBayer
                val = 3;
            else
                val = obj.ImageEncodingInternal.NumChannels;
            end
    ...
    methods (Access = protected)
        ...
        % Do not attempt conversion if the Data field is truncated
            ...
            if isfield(busstruct,'format') % sensor_msgs/CompressedImage
                ...
                signalFormat = char(busstruct.format(1:busstruct.format_SL_Info.CurrentLength)');
                
                sameEncoding = strncmpi(signalFormat, [obj.ImageEncoding, ';'], numel(obj.ImageEncoding) + 1);
                sameFormat = any(strcmp({'jpeg', 'jpg', 'png'}, signalFormat));
                
                if ~sameEncoding && ~sameFormat
                ...
                % Do not attempt conversion if incoming encoding differs
                % from stored encoding
                signalEncoding = char(busstruct.encoding(1:busstruct.encoding_SL_Info.CurrentLength)');
                
                if ~strcmpi(signalEncoding, obj.ImageEncoding)
                    errorCode = obj.ErrorCodeEncodingMismatch;
                    return;
                end
            ...
            % Size, data type
            if coder.target('Rtw')
                [sz, dt] = obj.getOutputImageSpecification;

                obj.Image = zeros(sz, dt);
            end

            [height, width, channels] = size(rawImage);
            obj.Image(1:height, 1:width, 1:channels) = rawImage;
        ...
        %outputs Return image and alpha values based on current state
            if obj.HasAlpha
                fullImage = obj.Image(:, :, 1:obj.OutputNumChannels - 1);
            else
                fullImage = obj.Image(:, :, 1:obj.OutputNumChannels);
            end

            if nargout==2
                if obj.HasAlpha
                    fullAlpha = obj.Image(:, :, obj.OutputNumChannels);
                else
            ...
            if obj.VariableSizeOutputs
                image = obj.createVarsizeOutputs(fullImage);
            else
                image = fullImage;
            end
        end
        ...
        %createVarsizeOutputs Trim outputs to size of incoming data
            if obj.HasAlpha
                coder.varsize('image', [obj.MaximumImageSize, obj.OutputNumChannels - 1], [1, 1, 0]);
                image = fullImage(1:obj.ImageSize(1), 1:obj.ImageSize(2), :);
            else
                coder.varsize('image', [obj.MaximumImageSize, obj.OutputNumChannels], [1, 1, 0]);
                image = fullImage(1:obj.ImageSize(1), 1:obj.ImageSize(2), :);
            end
        ...
        %decodeImage Convert raw uint8 data to array
            ...
            if obj.ShowAlphaOutput
                [tempImage, alpha] = ros.msg.sensor_msgs.internal.ImageReader.readImage(...
                    data, width, height, obj.ImageEncodingInternal);

                if obj.HasAlpha
                    % Concatenate the image with its alpha values
                    image = cat(3, tempImage, alpha);
                else
                    image = tempImage;
                end
            else
                image = ros.msg.sensor_msgs.internal.ImageReader.readImage(...
                    data, width, height, obj.ImageEncodingInternal);
            end
        ...
        %decompressImage Convert compressed image to array
            ...
            if coder.target('MATLAB')
                try
                    ...
                    msg = ros2message('sensor_msgs/CompressedImage');
                    msg.data = data;
                    msg.format = format;

                    imageRaw = rosReadImage(msg);
                    ...
                    image = zeros([obj.MaximumImageSize, obj.OutputNumChannels],obj.DataType);
            ...
            elseif coder.target('Rtw')
                ...
                errorCode = obj.ErrorCodeSuccess;
                image = zeros([obj.MaximumImageSize, obj.OutputNumChannels],obj.DataType);
                if isfield(busstruct,'format')
                    coder.cinclude('slros_read_image.h');
                    fullImage = zeros([obj.MaximumImageSize, obj.OutputNumChannels],obj.DataType);
    
                    % Add code for compressed images on ROS
                    decompressImageMethod = coder.const(sprintf('decompressImage<%d>', ...
                                                                obj.InputNumChannels));
        ...
        function validateInputsImpl(obj,~)
            validateInputsImpl@ros.slros2.internal.block.ReadXBase(obj);
        ...
        %getOutputSizeImpl Return Size for each output port
            ...
            if obj.HasAlpha
                varargout{1} = [obj.MaximumImageSize, obj.OutputNumChannels-1]; % Image
                if obj.ShowAlphaOutput && obj.HasAlpha
                    varargout{2} = obj.MaximumImageSize;
                end
            else
                varargout{1} = [obj.MaximumImageSize, obj.OutputNumChannels]; % Image
            end
        end
        ...
        function [sz, dt, cp] = getImageSpecification(obj)
            sz = [obj.MaximumImageSize,obj.OutputNumChannels];
            dt = obj.DataType;
            cp = false;
        end

        function [sz, dt, cp] = getOutputImageSpecification(obj)
            sz = [obj.MaximumImageSize, obj.OutputNumChannels];
            dt = obj.DataType;
            cp = false;
        end
    ...
    methods(Access = protected, Static)
        ...
        % Define property section(s) for System block dialog
            imageGroup = ros.slros2.internal.block.mixin.ConfigureUsingROS2.getConfigurableSettingsGroup( ...
                slrosBlockmaskMessage('ImagePropertiesPrompt'), ...
                {'MaximumImageSize', 'ImageEncoding'}, ...
                message('ros:slros2:topicselector:ImageDialogTitle').string, ...
        ...
        function topicSelector = generateTopicTable(node, varargin)
            topicSelector = ros.slros2.internal.ImageTopicTable(node, varargin{:});
        end
    end
end
