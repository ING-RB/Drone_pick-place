...
%ApplyTransformBlockMask - Block mask callbacks for ROS 2 Apply Transform
%block
...
%   Copyright 2023-2024 The MathWorks, Inc.
    ...
    properties (Constant)
        ...
        MaskType = 'ros.slros2.internal.block.ApplyTransform'

        %SysObjBlockName - System object block name
        SysObjBlockName = 'ApplyTransform'

        %EntitySignalBlk - Signal Specification block name for entity port
        EntitySignalBlk = 'SignalSpecification1'
    end
    ...
    methods
        ...
        %entityMsgTypeSelect Update bus type for blocks under subsystem
            ...
            [entityBusName,~] = ros.slros2.internal.bus.Util.rosMsgTypeToDataTypeStr(entityMsgType);
            entity_sig_blk = [block '/' obj.EntitySignalBlk];
        ...
        %updateIconAndBuses updates icon and Simulink buses
            ...
            ros.internal.setBlockIcon(blkH, 'rosicons.ros2lib_applytransform');
            obj.messageLoadFcn(block);
            % Update system object output bus name
            sysobj_block = [block '/' obj.SysObjBlockName];
            entityMsgType = get_param(block, 'EntityMsgType');
            [~,entityMsgName] = ros.slros2.internal.bus.Util.rosMsgTypeToDataTypeStr(entityMsgType);
    ...
    methods(Static)
        function messageLoadFcn(block)
            if ~ros.slros.internal.block.CommonMask.isLibraryBlock(block)
                entityMsgType = get_param(block,'EntityMsgType');
                ros.slros2.internal.bus.Util.createBusIfNeeded(entityMsgType,bdroot(block));
                ros.slros2.internal.bus.Util.createBusIfNeeded('geometry_msgs/TransformStamped',bdroot(block));
            end
        end

        function dispatch(methodName, varargin)
        %dispatch Dispatch to static methods in this class
            obj = ros.slros2.internal.block.ApplyTransformBlockMask;
