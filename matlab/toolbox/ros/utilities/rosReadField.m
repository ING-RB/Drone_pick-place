...
%                                  Default: false
% 
%      "DataType" - Optional property used by code generation that 
%                   specifies the type of the data in the messages that 
%                   will be read by the function. During execution in 
%                   MATLAB, this property is ignored, and a warning will be 
%                   thrown if this property is supplied but does not match 
%                   the type of the message's data. In this case, the
%                   actual type of the message's data is used to make the 
%                   output.
%                   Default: 'single'
%
...
% Ensure the generated code is not inlined
    ...
    if isfield(msg, 'Data')
        % ROS message struct
        % Sanity check to make sure message is not empty
        coder.internal.assert(~isempty(msg.Data),'ros:mlroscpp:codegen:EmptyInputMsg','rosReadField');
        specialMsgUtil = ros.internal.SpecialMsgUtil;
    else
        % ROS 2 message struct
        % Sanity check to make sure message is not empty
        coder.internal.assert(~isequal(msg.data,0),'ros:mlroscpp:codegen:EmptyInputMsg','rosReadField');
