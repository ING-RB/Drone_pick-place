...
if isfield(data,'ROS') && isfield(data.ROS,'RemoteBuild') ...
    ...
    if isfield(data.Runtime, 'BuildAction')
        ...
        if isequal(action, message('ros:slros2:codegen:ui_buildopts_buildaction_buildandrun').getString)
            ...
            if ispc
                archSpecificEnv = 'PATH';
                customPathEnv = 'CUSTOM_PATH';
            elseif ismac % For maci64 and maca64
                archSpecificEnv = 'DY_LDLIBRARY_PATH';
                customPathEnv = 'CUSTOM_LD_LIBRARY_PATH';
            else % For glnxa64
            ...
            setenv(customPathEnv,strjoin(customMsgDirList, pathsep));
            resetCustomPathEnv = onCleanup(@()unsetenv(customPathEnv));
