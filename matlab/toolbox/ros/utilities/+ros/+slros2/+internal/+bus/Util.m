...
%   Copyright 2019-2024 The MathWorks, Inc.
    ...
    methods (Static)
        ...
        function [topLevelMsgTypes, topLevelSvcTypes, topLevelActTypes, ...
                  pubSubMsgBlockList, paramBlockList, ...
                  getTfBlockList, applyTfBlockList, ...
                  imageBlockList, timeBlockList, ...
                  svcCallBlockList, svcServerBlockList, ...
                  sendActionGoalBlockList, monitorActionGoalBlockList, ...
                  cancelActionGoalBlockList, logfileBlockList, writeBlockList, ...
                  ros2pacerBlock] = getROS2BlocksInModel(model)
        ...
        %message types
            ...
            ros2pacerBlock = ros.slros.internal.bus.Util.listBlocks(model, ...
            ...
            applyTfBlockList = ...
                ros.slros.internal.bus.Util.listBlocks(model, ...
                                                       ros.slros2.internal.block.ApplyTransformBlockMask.MaskType);
            % Find Logfile Blocks
            logfileBlockList = ...
                ros.slros.internal.bus.Util.listBlocks(model, ...
                                                       ['(' ...
            ...
            % Find Service Server Blocks
            svcServerBlockList = ...
                ros.slros.internal.bus.Util.listBlocks(model, ...
                                                       ros.slros2.internal.block.ReceiveRequestBlockMask.getMaskType);
            if ~isempty(svcServerBlockList)
                % We need two message types for each service.
                topLevelSvcTypes = get_param(svcServerBlockList, 'serviceType');
                topLevelMsgTypes = [topLevelMsgTypes; ...
                                    strcat(topLevelSvcTypes,'Request');
                                    strcat(topLevelSvcTypes,'Response')];
            end

            % Find Send Action Goal Blocks
            sendActionGoalBlockList = ...
                ros.slros.internal.bus.Util.listBlocks(model, ...
                                                       ros.slros2.internal.block.SendActionGoalBlockMask.getMaskType);
            if ~isempty(sendActionGoalBlockList)
                % We need two message types for each service.
                topLevelActTypes = get_param(sendActionGoalBlockList, 'actionType');
                topLevelMsgTypes = [topLevelMsgTypes; ...
                                    strcat(topLevelActTypes,'Goal');
                                    'unique_identifier_msgs/UUID'];
            end

            % Find Monitor Action Goal Blocks
            monitorActionGoalBlockList = ...
                ros.slros.internal.bus.Util.listBlocks(model, ...
                                                       ros.slros2.internal.block.MonitorActionGoalBlockMask.getMaskType);

            if ~isempty(monitorActionGoalBlockList)
                topLevelActTypes = get_param(monitorActionGoalBlockList, 'actionType');
                topLevelMsgTypes = [topLevelMsgTypes; ...
                    strcat(topLevelActTypes,'Feedback');
                    strcat(topLevelActTypes,'Result')];
            end

            % Find Cancel Action Goal Blocks
            cancelActionGoalBlockList = ...
                ros.slros.internal.bus.Util.listBlocks(model, ...
                                                       ros.slros2.internal.block.CancelActionGoalBlockMask.getMaskType); 

            if ~isempty(cancelActionGoalBlockList)
                topLevelMsgTypes = [topLevelMsgTypes;
        ...
        function [emptyRosMsg, ros2MsgInfo] = newMessageFromSimulinkMsgType(rosMsgType,command)
        ...
        %message type
            ...
            if isempty(ros2MsgCache__) || isequal(command, 'clear')
                ros2MsgCache__ = containers.Map;
                if isequal(command, 'clear')
                    % Immediately return after clearing cache
                    return;
                end
            end
        ...
        function [actGoalBusType, actGoalBusName, ...
                  actResultBusType, actResultBusName, ...
                  actFeedbackBusType, actFeedbackBusName] = rosActionTypeToDataTypeStr(rosActionType)
        %rosActionTypeToDataTypeStr - Get the data type strings corresponding to action type
        %   Note that there are three buses associated with each action
        %   type: one bus for the goal request, one bus for goal feedback and one bus for the goal result.

            rosGoalMessageType = strcat(rosActionType, 'Goal');
            [actGoalBusType,actGoalBusName] = ros.slros2.internal.bus.Util.rosMsgTypeToDataTypeStr(rosGoalMessageType);
            rosResultMessageType = strcat(rosActionType, 'Result');
            [actResultBusType,actResultBusName] = ros.slros2.internal.bus.Util.rosMsgTypeToDataTypeStr(rosResultMessageType);
            rosFeedbackMessageType = strcat(rosActionType, 'Feedback');
            [actFeedbackBusType, actFeedbackBusName] = ros.slros2.internal.bus.Util.rosMsgTypeToDataTypeStr(rosFeedbackMessageType);
        end
