classdef (Abstract) ROSMonitorActionGoalBase < matlab.System
...
%   Copyright 2023 The MathWorks, Inc.
    ...
    properties (Nontunable)
        ...
        %SLFeedbackOutputBusName - Simulink Bus Name for output (goal feedback message)
        SLFeedbackOutputBusName = ''

        %SLResultOutputBusName - Simulink Bus Name for output (goal result message)
        SLResultOutputBusName = ''
    ...
    methods
        function obj = ROSMonitorActionGoalBase(varargin)
            coder.allowpcode('plain');
            % Support name-value pair arguments when constructing object
            setProperties(obj,nargin,varargin{:})
        end

        function set.SLResultOutputBusName(obj, val)
            validateattributes(val, {'char'}, {}, '', 'SLResultOutputBusName');
            obj.SLResultOutputBusName = val;
        end

        function set.SLFeedbackOutputBusName(obj, val)
            validateattributes(val, {'char'}, {}, '', 'SLFeedbackOutputBusName');
            obj.SLFeedbackOutputBusName = val;
        end
