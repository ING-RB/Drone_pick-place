...
%   Copyright 2014-2023 The MathWorks, Inc.
    ...
    methods
        ...
        % This function can timeout if Master is not reachable. It is the
        % caller's responsibility to check reachability with
        % verifyReachable() or isReachable()

        % To access the TransformationTree, we have to create a new ROS
        % node. It will be deleted when this function exists.
            [~,nodeName] = fileparts(tempname);
            node = obj.createNode(nodeName);
            pTfTree = ros.TransformationTree(node,"DataFormat","struct");
            ...
            frames = pTfTree.AvailableFrames;
            for i=1:2
                if isempty(frames)
                    pause(1);
                    frames = pTfTree.AvailableFrames;
