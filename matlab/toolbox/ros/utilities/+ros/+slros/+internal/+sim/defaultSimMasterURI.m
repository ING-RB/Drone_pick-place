...
function ret = pingHost(localIfIp, remoteHostIp)
    % pc: -w: timeout (milliseconds), -n: count, -l: buffer size, -S: source address
    % glnxa64: -W: timeout (seconds), -c: count, -I: source address
    % macosx: -W: timeout (seconds), -c: count, -S: source address
    d = dictionary("win64",['ping -w 1000 -n 1 -S ' localIfIp ' ' remoteHostIp],...
        "glnxa64",['ping -W 1 -c 1 -I ' localIfIp ' ' remoteHostIp],...
        "maci64",['ping -W 1 -c 1 -S ' localIfIp ' ' remoteHostIp],...
        "maca64",['ping -W 1 -c 1 -S ' localIfIp ' ' remoteHostIp]);
    [st,msg] = system(d(computer('arch')));
    if (st == 0) && ~isempty(regexpi(msg,'\sTTL=','once'))
        ret = true;
    else
        ret = false;
    end
