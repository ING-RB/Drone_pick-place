...
%   Copyright 2014-2023 The MathWorks, Inc.
    ...
    properties(Constant)
        ...
        VarInt64Support = 'SL_Bus_ROSEnable64bitIntegers'
        ...
        % CacheVersion Update this version number to invalidate any
        % generated message cache files in +bus_conv_fcns folder in
        % previous release
        CacheVersion = 1;
    end
    ...
    methods (Static)
            ...
            paramBlockList = ...
                ros.slros.internal.bus.Util.listBlocks(model, ...
                                                       ['(' ...
                                                        ...
                                                        ')']);
            % Find Get Transform and Apply Transform Blocks
            getTfBlockList = ...
                ros.slros.internal.bus.Util.listBlocks(model, ...
                                                       ros.slros.internal.block.GetTransformBlockMask.MaskType);
            applyTfBlockList = ...
                ros.slros.internal.bus.Util.listBlocks(model, ...
                                                       ros.slros.internal.block.ApplyTransformBlockMask.MaskType);
            if ~isempty(getTfBlockList) || ~isempty(applyTfBlockList)
                topLevelMsgTypes = [topLevelMsgTypes; 'geometry_msgs/TransformStamped'];
            end
        ...
        function refCachedMap = getDataFromCacheFile(refCachedMapStoragePath)
            % GETDATAFROMCACHEFILE Get message meta-data saved in cache
            % file
            cacheVer = ros.slros.internal.bus.Util.CacheVersion;
            refCachedMap = containers.Map();
            refCachedMap('Version') = cacheVer;
            if isfile(refCachedMapStoragePath)
                try
                    origData = load(refCachedMapStoragePath);
                    % If the version stored in the cache file does not match the
                    % required version, delete cache file and return empty value.
                    if isKey(origData.refCachedMap,'Version') && ...
                            (cacheVer == origData.refCachedMap('Version'))
                        refCachedMap = origData.refCachedMap;
                    else
                        delete(refCachedMapStoragePath);
                    end
                catch
                    % Delete a corrupted MAT-file
                    delete(refCachedMapStoragePath);
                end
            end
        end

        function throwInt64Warning(propertyName, msgType, origDtype, newDtype,model)
            ...
            diag = MSLDiagnostic(...
                ...
                propertyName, msgType, origDtype, newDtype,model));
            reportAsWarning(diag);
        end

        function ret = isInt64Enabled(modelName)
            arguments
                modelName {mustBeTextScalar}
            end
            ws = get_param(modelName,'ModelWorkspace');
            varName = ros.slros.internal.bus.Util.VarInt64Support;
            ret = ws.hasVariable(varName) && ws.getVariable(varName);
        end

        function setInt64SLBusMode(modelName,val)
            arguments
                modelName {mustBeTextScalar}
                val {mustBeA(val,'logical')}
            end
            ws = get_param(modelName,'ModelWorkspace');
            assignin(ws,ros.slros.internal.bus.Util.VarInt64Support,val);
            % Delete all buses
            if isequal(get_param(modelName,'HardwareBoard'),message('ros:slros2:codegen:ui_hwboard').getString)
                ros.slros2.internal.bus.Util.clearSLBusesInGlobalScope();
            else
                ros.slroscpp.internal.bus.Util.clearSLBusesInGlobalScope(modelName);
            end
        end
        ...
        function loadROS2LibraryDictionary()
            ...
            libFile = fullfile(matlabroot,'toolbox','ros','slros2','ros2lib.slx');
            persistent ros2DictionaryLoaded__
            if isempty(ros2DictionaryLoaded__)
                ros2DictionaryLoaded__ = false;
            end
            if ~ros2DictionaryLoaded__ && isfile(libFile)
                Simulink.LibraryDictionary.refresh(libFile);
        ...
        function loadROSLibraryDictionary()
            ...
            % Dictionary data linkage issue, call the dictionary refresh
            % commands below on Simulink load time.
            libFile = fullfile(matlabroot,'toolbox','ros','slroscpp','robotlib.slx');
            persistent rosDictionaryLoaded__
            if isempty(rosDictionaryLoaded__)
                rosDictionaryLoaded__ = false;
            end
            if ~rosDictionaryLoaded__ && isfile(libFile)
                Simulink.LibraryDictionary.refresh(libFile);
        		Simulink.LibraryDictionary.getInstanceOfDependencyInfo();
