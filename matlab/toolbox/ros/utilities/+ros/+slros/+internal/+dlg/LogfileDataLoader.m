classdef LogfileDataLoader < handle
    ...
    methods
        ...
        function obj = LogfileDataLoader(modelName,varargin)
            ...
            if nargin > 1
                obj.FilterSpec = validatestring(varargin{1},{'*.db3;*.mcap;metadata.yaml','*.bag'});
            end
            if nargin > 2
                validateattributes(varargin{2},{'ros.BagSelection','ros2bagreader'},{},'retreiveDialog');
    ...
    methods (Hidden)
        ...
        function [shouldClose, errorMsg] = dlgApplyClose(obj)
            ...
            try
                ...
                % Process logfile
                if endsWith(string(obj.Filename),'.db3') || endsWith(string(obj.Filename),'.db3.zstd') || ...
                        endsWith(string(obj.Filename),'.mcap') || endsWith(string(obj.Filename),'.mcap.zstd')
                    % pass bag file path if .db3 file is selected
                    dataobj = ros2bagreader(obj.Filename);
                elseif endsWith(string(obj.Filename),'metadata.yaml')
                    % pass bag folder path to read one or more db3 files in a folder
                    % if metadata.yaml file is selected
                    dataobj = ros2bagreader(fileparts(obj.Filename));
                else
