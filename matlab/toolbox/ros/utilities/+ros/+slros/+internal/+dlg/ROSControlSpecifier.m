classdef ROSControlSpecifier < matlab.apps.AppBase
    ...
    properties(Constant,Hidden)
        ...
        ParamTableVarName = 'ROSControlDynamicReconfTable_';
    ...
    properties (Access = private)
        ...
        ROSVersion      % Version of ROS
        ROSControlUtil  % Utilities for ROS control
    end
    ...
    methods (Access = private)
            ...
            if ~isequal(event.PreviousData,event.NewData) && ...
                ...
                if ~iscvar(event.NewData)
                    throwError(app,message('ros:slros:roscontrol:ResourceNameInvalidTitle').getString,...
                        message('ros:slros:roscontrol:ResourceNameInvalidMsg',event.NewData, app.ROSVersion).getString);
                    event.Source.Data(event.Indices(1),event.Indices(2)) = event.PreviousData;
                end
            end
        end

        function validateClassName(app,event)
            pattern = '^\w+$';

            if isempty(regexp(event.Value, pattern,'once'))
                % Input from user is not a CamelCase string, revert to
                % previous value
                app.ClassNameEditField.Value = event.PreviousValue;
                throwError(app, message('ros:slros:roscontrol:ClassNameInvalidTitle').getString, ...
                    message('ros:slros:roscontrol:ClassNameInvalidMsg',event.Value, app.ROSVersion).getString);
            end
        end
        ...
        function setDynamicReconfigureFlag(~, blockFullName, param, value)
            set_param(blockFullName,...
                'UserData', struct(param,value),...
        ...
        function paramTableData = initializeParamTable(app)
            % INITIALIZEPARAMTABLE Generate the data table that will be
            % used in the 'Parameters' UI

            % Pre-allocate a small empty array for parameter data
            paramTableData =  repmat({"","","",false},64,1);
            tunablePrms = app.Resources.DynamicReconfigurableBlocks;
            tableIdx = 1;
            for k=1:numel(tunablePrms)
                blkParamName = tunablePrms(k).FindSysParam;
                blkParamVal = tunablePrms(k).FindSysVal;
                params =  tunablePrms(k).Parameters;
                % Do not search for blocks under masked subsystems
                blksOfThisType = find_system(app.ModelName,'LookUnderMasks',...
                    'on',blkParamName,blkParamVal);
                for j=1:numel(blksOfThisType)
                    for l=1:numel(params)
                        paramTableData(tableIdx,:) = {...
                            get_param(blksOfThisType{j},"Name"),...
                            get_param(blksOfThisType{j},"Parent"),...
                            params{l},...
                            getDynamicReconfigureFlag(app,blksOfThisType{j}, params{l})};
                        tableIdx = tableIdx + 1;
                    end
                end
            end
            % Trim empty arrays
            emptyIdx = arrayfun(@(xn)isempty(char(xn)),[paramTableData{:,1,1,1}]);
            paramTableData(emptyIdx,:,:,:) = [];
        end
        ...
        function [inportInfoTable,outportInfoTable] = initializePortInfoTable(app)
            ...
            mdlName = app.ModelName;
            rosVer = strrep(lower(app.ROSVersion),' ','');
            if isempty(app.RootIOPorts)
                delete(app.FigContainer);
                error(message('ros:slros:roscontrol:NoRootIOPorts',mdlName,rosVer));
            end
    ...
    methods (Access = private)
        ...
        function startupFcn(app, modelName)
            ...
            app.Description.Text = getString(message('ros:slros:roscontrol:Description',char(app.ModelName),app.ROSVersion));
            ...
            app.ParamTable.Data = initializeParamTable(app);
        ...
        function okButtonPushed(app, ~)
            if ~bdIsLoaded(app.ModelName)
                warning(message('ros:slros:roscontrol:ModelAlreadyClosed',app.ModelName, app.ROSVersion));
                delete(app.FigContainer)
                return;
            end
            ...
            assignin(w,app.ParamTableVarName,app.ParamTable.Data);
            ...
            for k=1:height(app.ParamTable.Data)
                prm = app.ParamTable.Data(k,:,:,:);
                setDynamicReconfigureFlag(app,[prm{2},'/',prm{1}],prm{3},prm{4});
            end
            ...
            % To enable dynamic reconfiguration parameter tuning, the
            % parameter behavior must be "Tunable"
            set_param(app.ModelName,"DefaultParameterBehavior","Tunable");

            % Apply the setting in the configuration set
            cset = getActiveConfigSet(app.ModelName);
            if ~isequal(codertarget.data.getParameterValue(cset,'ROS.GenerateROSControl'),...
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.FigContainer.Name = getString(message('ros:slros:roscontrol:FigTitle',app.ROSVersion));
            ...
            app.ClassNameEditField.Placeholder = getString(message('ros:slros:roscontrol:ClassNamePlaceHolder', app.ROSVersion));
            app.ClassNameEditField.ValueChangedFcn = createCallbackFcn(app, @validateClassName, true);
            ...
            app.ParamTab = uitab(app.PortTabGroup,"Title","Parameters");
            ...
            paramTabGridLayout = uigridlayout(app.ParamTab,[1 1],'RowHeight',{'fit'},'ColumnWidth',{'fit'});
            ...
            % Create Param Table
            app.ParamTable = uitable(paramTabGridLayout);
            app.ParamTable.Layout.Row = [1 2];
            app.ParamTable.Layout.Column = [1 2];

            if strcmp(app.ROSVersion, 'ROS')
                % ROS
                dynamicParamConfigName = getString(message('ros:slros:roscontrol:ParamEnableDynReconf'));
            else
                % ROS 2
                dynamicParamConfigName = getString(message('ros:slros2:ros2control:ParamAsROS2Param'));
            end
            app.ParamTable.ColumnName = {getString(message('ros:slros:roscontrol:ParamSrcBlkCol'));...
                getString(message('ros:slros:roscontrol:ParamSrcBlkPathCol'));...
                getString(message('ros:slros:roscontrol:ParamNameCol'));...
                dynamicParamConfigName};
            app.ParamTable.RowName = {};
            app.ParamTable.ColumnEditable = [false false false true];
            app.ParamTable.Multiselect = 'off';
            app.ParamTable.Tooltip = getString(message('ros:slros:roscontrol:ParamTabTooltip'));
            app.ParamTable.Tag = 'ParameterMapTable';   
            ...
            app.GenCodeCheckbox.Text = getString(message('ros:slros:roscontrol:GenCodeChkBoxLabel', strrep(lower(app.ROSVersion),' ','')));
            app.GenCodeCheckbox.Tooltip =  getString(message('ros:slros:roscontrol:GenCodeChkBoxTooltip', strrep(lower(app.ROSVersion),' ',''), app.ROSVersion));
    ...
    methods (Access = public)
        ...
        function app = ROSControlSpecifier(varargin)
            ...
            if isempty(runningApp)
                ...
                % Set ROS version from model ConfigSet
                modelName = varargin{1};
                app.ROSVersion = ros.codertarget.internal.ROSControlUtilBase.getROSVersion(modelName);
                if strcmp(app.ROSVersion, 'ROS')
                    % ROS hardware board
                    app.ROSControlUtil = ros.codertarget.internal.ROSControlUtil;
                else
                    % ROS 2 hardware board
                    app.ROSControlUtil = ros.codertarget.internal.ROS2ControlUtil;
                end

                % Instantiate the Input/Output resource names
                app.Resources = app.ROSControlUtil.getROSControlInterfaces();
                ...
                try
                    ...
                    % In an event of error during compilation of the model
                    % in the startup function (for instance, getting the
                    % port dimensions and data-types) delete the figure
                    % container in the app if app handle is valid
                    if isvalid(app)
                        delete(app.FigContainer);
                    end
                    rethrow(ME);
