...
%   Copyright 2014-2023 The MathWorks, Inc.
    ...
    properties(SetAccess=private)
        ...
        %GetTfBlocks - List of ROS Get Transform blocks
        GetTfBlocks

        %ApplyTfBlocks - List of ROS Apply Transform blocks
        ApplyTfBlocks
        ...
        %TransformGetterList - Info about Get Transform blocks in model
        %   This is a list of cgen.ROSGetTfBlockInfo objects.
        TransformGetterList = ros.slros.internal.cgen.ROSGetTfBlockInfo.empty
    ...
    methods
        function info = augmentBlockInfo(obj, info)
            ...
            info.TransformGetters = getContainersMap(obj.TransformGetterList);
            info.ServiceCallers = getContainersMap(obj.ServiceCallerList);
            if ~isempty(obj.ImageBlocks)
                if ismember('sensor_msgs/CompressedImage', obj.TopLevelMsgTypes)
                    info.ImageDepends = {'cv_bridge', 'sensor_msgs'};
                else
                    info.ImageDepends = {'sensor_msgs'};
                end
            end
        ...
        function obj= ROSModelInfo(model)
            ...
            [obj.TopLevelMsgTypes, obj.TopLevelServiceTypes, obj.LibraryBlocks, ...
             obj.ParameterBlocks, obj.GetTfBlocks, obj.ApplyTfBlocks, ...
             obj.ImageBlocks, obj.TimeBlocks, obj.ServiceCallBlocks] = ...
        ...
        % Get the dependencies for message types
            ...
            if ~isempty(obj.GetTfBlocks)
                info.nodeDependencies{end+1} = 'tf2_ros';
            end
    ...
    methods(Access = private)
        ...
        function obj = getInfoFromROSBlocks(obj)
            ...
            % Get more information about get transform blocks
            obj.TransformGetterList = obj.getInfoFromGetTfBlocks;
        ...
        function tfGetList = getInfoFromGetTfBlocks(obj)
        %getInfoFromGetTfBlocks Iterate over blocks and extract information
        %from Get Transform blocks

            tfGetList = [];
            for i = 1:numel(obj.GetTfBlocks)
                block = obj.GetTfBlocks{i};
                sysObjBlock = [block '/SourceBlock'];

                % Build info structure for Get Transform block
                s = ros.slros.internal.cgen.ROSGetTfBlockInfo;
                s.SlBusName = get_param(sysObjBlock,'SLBusName');
                s.Label = get_param(sysObjBlock,'BlockId');
                s.Comment = sprintf('For Block %s', block);
                tfGetList = [tfGetList s]; %#ok<AGROW>
            end
        end
