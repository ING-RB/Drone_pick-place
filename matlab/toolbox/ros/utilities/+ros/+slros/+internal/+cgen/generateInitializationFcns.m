...
%   Copyright 2014-2023 The MathWorks, Inc.
    ...
    % Include image header if ROS Read Image blocks with CompressedImage
    if ~isempty(modelinfo.ImageBlocks) && ismember('sensor_msgs/CompressedImage', modelinfo.TopLevelMsgTypes)
        initinfo.Includes{end+1} = cgenConsts.PredefinedCode.ImageHeaderFile;
    end

    if ~isempty(modelinfo.ImageBlocks) && ismember('sensor_msgs/Image', modelinfo.TopLevelMsgTypes)
        initinfo.Includes{end+1} = cgenConsts.PredefinedCode.ConvertToImageHeaderFile;
    end
    ...
    % Include transformation header if ROS Get Transform blocks exist in
    % model
    if ~isempty(modelinfo.GetTfBlocks)
        initinfo.Includes{end+1} = cgenConsts.PredefinedCode.TransformHeaderFile;
    end
    ...
    if ~isempty(modelinfo.ImageBlocks)
        ...
        genfiles.HeaderFiles{end+1} = fullfile( ...
            cgenConsts.PredefinedCode.Location, ...
            cgenConsts.PredefinedCode.ConvertToImageHeaderFile);
    end
    ...
    % Package transform header if needed
    if ~isempty(modelinfo.GetTfBlocks)
        genfiles.HeaderFiles{end+1} = fullfile( ...
            cgenConsts.PredefinedCode.Location, ...
            cgenConsts.PredefinedCode.TransformHeaderFile);
    end
...
function varInfo = defineVariables(modelinfo, cgeninfo)
    ...
    for i=1:numel(modelinfo.TransformGetterList)
        tfGet = modelinfo.TransformGetterList(i);
        varInfo(end+1) = struct(...
            'Comment', tfGet.Comment, ...
            'Declaration', sprintf('SimulinkTransform<%s> %s', ...
                                   tfGet.SlBusName, tfGet.Label), ...
            'Value', '');
    end
