...
if ~isempty(model) && ros.codertarget.internal.isMATLABConfig(model)
    ...
    declareHeaderFile =  'ros_structmsg_declare.h';
...
fileIncludesSection = StringWriter;
srcFile = StringWriter;

if ~isempty(model) && ros.codertarget.internal.isMATLABConfig(model)
    % Struct declaration only required for MATLAB Code generation
    hdrFileIncludesSection.addcr(['#include "' declareHeaderFile '"']);
    hdrFileIncludesSection.addcr(['#include ' rosHeader]);
else
    fileIncludesSection.addcr(['#include ' rosHeader]);
end

% Generate header include section.
% For ROS 1, place this section into header file.
% For ROS 2, place this section into source file. (g3466156)
...
for i = 1:numel(svcTypeList)
    ...
    fileIncludesSection.addcr(['#include <' headerName '>']);
end
...
for i=1:numel(keylist)
    ...
    if ~isempty(headerName)
        % When handling ROS 2 Action messages, "Goal", "Feedback", and
        % "Result" message may or may not share the same include file.
        % Feedback and Result message types may not be same as Goal always.
        % Here only unique header files are included.
        if ~ismember(headerName, headerFiles)
            headerFiles = [headerFiles;headerName];
            fileIncludesSection.addcr(['#include <' headerName '>']);
        end
    end
...
if ~isempty(modelTypesHdr)
    if iscell(modelTypesHdr)
        for k=1:numel(modelTypesHdr)
            fileIncludesSection.addcr(['#include "' modelTypesHdr{k} '"']);
        end
    else
        fileIncludesSection.addcr(['#include "' modelTypesHdr '"']);
    end
end

fileIncludesSection.addcr(['#include "' convUtilsHeaderFile '"']);
fileIncludesSection.addcr;

if ~strcmp(rosHeader,'<ros/ros.h>') && ~isempty(model) && ~ros.codertarget.internal.isMATLABConfig(model)
    % ROS 2 Simulink workflow
    % Only source file is required
    srcFile.add(fileIncludesSection);
    srcFile.addcr;
    srcFile.add(hdrFileFcnDeclarations);
    srcFile.addcr;
    srcFile.add(srcFileFcnDefinitions);
    srcFile.addcr;
    ros.slros.internal.cgen.insertHeaderGuards(srcFile, convCodeHeaderFile);
else
    % Write to conversion header file
    headerFile = StringWriter;
    headerFile.add(fileIncludesSection);
    headerFile.addcr;
    ...
    % Write to conversion src file
    srcFile.addcr(['#include "' convCodeHeaderFile '"']);
    srcFile.addcr;
    srcFile.add(srcFileFcnDefinitions);
    srcFile.addcr;
end
...
srcFile.write(sourceFileFullName);

if ~isempty(model) && ros.codertarget.internal.isMATLABConfig(model)
    % Create MATLAB message struct declaration header file
    structDecalFile = StringWriter;
    for i=1:numel(keylist)
        bus2cppFcnInfo = bus2cppMasterMap(keylist{i});
        structDecalFile.addcr(['struct ' bus2cppFcnInfo.SLBusName ';']);
        structDecalFile.addcr;
    end
    ros.slros.internal.cgen.insertHeaderGuards(structDecalFile, declareHeaderFile);
    declareHeaderFileFullName = fullfile(destDir, declareHeaderFile);
    structDecalFile.write(declareHeaderFileFullName);

    genfiles.HeaderFiles = { convCodeHeaderFile, declareHeaderFile };
