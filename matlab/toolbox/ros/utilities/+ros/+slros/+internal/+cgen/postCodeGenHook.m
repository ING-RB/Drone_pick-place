    ...
    if ros.codertarget.internal.isMATLABConfig(hCS)
        ...
        if any(contains(buildInfo.getSourceFiles(true,true), 'rosReadImage.cpp'))
            ...
            copyfile(fullfile(scriptLoc, cgenConsts.PredefinedCode.ConvertToImageHeaderFile), ...
                fullfile(buildDir, cgenConsts.PredefinedCode.ConvertToImageHeaderFile),'f');
        end
    else
        ...
        if isTopLevelModel
            ...
            scriptLoc = ros.slros.internal.cgen.Constants.PredefinedCode.Location;
            % Copy build_ros_model.sh to the same directory as the archive file
            ...
            if (Util.isROSControlEnabled(model))
                ros.codertarget.internal.ROSControlUtil.generateROSControlFiles(rosProjectInfo, buildDir, buildInfo);
            else
...
function loc_generateNodeInterface(modelInfo, buildInfo, bDir)
    ...
    if isequal(get_param(modelInfo.ModelName,'CodeInterfacePackaging'),'C++ class')
        ...
        if modelInfo.HasExplicitPartitions
            modelInfo.PartitionStepDefinition = fileread('rt_main.cpp');
        end
    else
