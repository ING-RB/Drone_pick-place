    ...
    methods
        ...
        %readXYZ Returns the (x,y,z) coordinates of all points
            ...
            if isempty(coder.target)
                yOff = double(pc.getFieldOffset(yIdx));
                zOff = double(pc.getFieldOffset(zIdx));

                fieldNames = {'x', 'y', 'z'};
                offsets = [xOff yOff zOff];
                dataTypes = [obj.TypeConversion.matlabToROSType(xMlType) ...
                            obj.TypeConversion.matlabToROSType(yMlType) ...
                            obj.TypeConversion.matlabToROSType(zMlType)];
                xyzRequested = true;
                rgbRequested = false;
                ...
                validPointIndicesCasted = cast(validPointIndices(:)-1,'like',pc.PointStep);

                pcReaderCppObj = roscpp.util.internal.RosConvFncsManager;
                xyz = pcReaderCppObj.readPointCloud(data, pc.Width, pc.Height, ...
                    pc.PointStep, fieldNames, offsets, dataTypes, xyzRequested, rgbRequested, validPointIndicesCasted, numel(validPointIndices));
                if any(~pointIdxIsValid(:))
                    ...
                    fieldPoints(pointIdxIsValid, :) = xyz;
                    xyz = fieldPoints;
                end
            else
        ...
        function rgb = readRGB(obj, pc, rgbIdx, pointIndices)
            ...
            if isempty(coder.target)
                fieldNames = {'rgb'};
                offsets = double(pc.getFieldOffset(rgbIdx));
                dataTypes = double(pc.getFieldDatatype(rgbIdx));
                xyzRequested = false;
                rgbRequested = true;
                numPointsActual = min(pc.Height*pc.Width, fix(numel(pc.Data)/double(pc.PointStep)));
                pointIdxIsValid = (0 < pointIndices) & (pointIndices <= numPointsActual);
                ...
                pcReaderCppObj = roscpp.util.internal.RosConvFncsManager;
                rgbRaw = pcReaderCppObj.readPointCloud(pc.Data, pc.Width, pc.Height, ...
                    pc.PointStep, fieldNames, offsets, dataTypes, xyzRequested, rgbRequested, validPointIndicesCasted, numel(validPointIndices));
                ...
                rgb = NaN(numPoints,3);             
                rgb(pointIdxIsValid, :) = rgbRaw(pointIdxIsValid,:);
            else
