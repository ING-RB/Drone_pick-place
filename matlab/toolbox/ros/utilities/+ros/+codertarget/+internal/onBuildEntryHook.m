...
% Check that target language is C++ and error out if it is not
    ...
    if (ros.codertarget.internal.Util.isROSControlEnabled(modelName)) && ...
        ...
        rosVersion = ros.codertarget.internal.ROSControlUtilBase.getROSVersion(modelName);
        diag = MSLException(mdlHandle, message('ros:slros:roscontrol:NeedsNonReusableFcn',modelName,rosVersion));
        throw(diag);
    end
