classdef DeploymentHooks < ros.slros.internal.InternalAccess
    ...
    methods (Static)
        function loadCommand(varargin)
            ...
            if ros.codertarget.internal.isMATLABConfig(varargin{1})
                ...
                supportedBoards = ros.codertarget.internal.getSupportedHardwareBoards('matlab');
            else
                modelName = varargin{1};
                data = codertarget.data.getData(getActiveConfigSet(modelName));
                if  ros.codertarget.internal.Util.isRefModelSubDirectoryEnabled(modelName)
                    % Referenced models already been packaged into the same
                    % tgz file as the top-level model, hence, ignore
                    % referenced models.
                    modelRefNames = [];
                else
            ...
            msgPkgListFile = strcat(modelName, 'CustomMsgs.txt');
            ...
            % Create file listing all custom message packages
            cgenInfoSL = ros.codertarget.internal.ROSSimulinkCgenInfo.getInstance;
            customMsgPkgs = cgenInfoSL.getCustomMsgPkgName();
            hasCustomMsg = ~isempty(customMsgPkgs) && cgenInfoSL.getBuildMissingMsgs;
            cgenInfoSL.reset();

            if hasCustomMsg
                customMsgList = StringWriter;
                for customMsgPkg = customMsgPkgs
                    customMsgList.addcr(strcat(customMsgPkg{1}, '.tgz'));
                end
                customMsgList.write(msgPkgListFile);
            end
            ...
            % Transfer all custom message package archives
            if hasCustomMsg
                if isMATLABCodegen
                    folderRelative = '';
                else
                ...
                for i = 1:numel(customMsgPkgs)
                    putFile(rosTarget, fullfile(folderRelative, [customMsgPkgs{i} '.tgz']), validRemoteWorkspace);
                end
            end
            ...
            % Transfer text file listing all custom message packages
            if hasCustomMsg
                putFile(rosTarget, msgPkgListFile, validRemoteWorkspace);
            end
            ...
            if ros.codertarget.internal.Util.isRefModelSubDirectoryEnabled(modelName)
                pid = system(rosTarget, ...
                    ...
                    [validRemoteWorkspace '/' modelName] ' ' validRemoteWorkspace ' 1 &>/dev/null & echo $!']);
            else
                pid = system(rosTarget, ...
                    ['source ' validRemoteWorkspace setupScriptShell '; '...
                    backgroundScript ' ' buildScript ' ' ...
            ...
            while ~done
                ...
                logSize = depObj.displayLogMessage(logSize, currLogSize, modelName, buildLog, rosTarget);
    ...
    %% Helper functions for accessing target information
    methods (Static)
        function logSize = displayLogMessage(logSize, currLogSize, modelName, buildLog, rosTarget)
            if ~isnan(currLogSize) && (currLogSize > logSize)
                try
                    % Avoid message overflow when passing from remote
                    % device to host machine (g3293443)
                    % Each machine can have different configuration for
                    % ARG_MAX, hence, using minimum value 4096 as the
                    % filter.
                    % https://docs.opensvc.com/latest/_static/argmax.html
                    if (currLogSize-logSize) > 4096
                        % To avoid overflow, copy file to workspace and
                        % display directly if more than 4096 new
                        % characters are found.
                        tempLogFile = [modelName '_build.log'];
                        getFile(rosTarget, buildLog, tempLogFile);
                        logChars = fileread(tempLogFile);
                        disp(logChars(logSize+1:end));
                        % Update logSize for next batch
                        logFileInfo = dir(tempLogFile);
                        logSize = logFileInfo.bytes;
                        delete(tempLogFile);
                    else
        ...
        function enableCustomMessageDeployment()
            cgenInfoSL = ros.codertarget.internal.ROSSimulinkCgenInfo.getInstance;
            cgenInfoSL.setBuildMissingMsgs(true);
        end
    end
    ...
    methods (Static, Access = private)
        function runLocalROSNode(nodeName, buildAction, varargin)
            if contains(buildAction, 'run')
                ...
                runCmdsMap = containers.Map({'win64','glnxa64','maci64', 'maca64'},...
