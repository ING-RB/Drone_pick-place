...
%   Copyright 2014-2023 The MathWorks, Inc.
    ...
    methods (Static)
        ...
        %relativeToAbsolute Convert relative to absolute path
            ...
            % If already specifying relativePath as AbsolutePath. Do
            % nothing. For example "/usr/local"
            if ~matlab.io.internal.filesystem.isAbsolutePathNoIO(relativePath)
                % Case 1: handle '.' and '..' input that Java handled
                % but the internal absolutePath utility does not (g2550523)
                % Case 2: relative path from current working directory.
                % For example, "test"
                relativePath = fullfile(pwd, relativePath);
            end

            % matlab.io.internal.filesystem.resolvePath will return empty
            % for non-exist folders and files.
            if ~exist(relativePath,'dir') && ~exist(relativePath,'file')
                [filePath, name, ext] = fileparts(relativePath);
                absFilePathInfo = matlab.io.internal.filesystem.resolvePath(filePath);
                absFilePath = char(absFilePathInfo.ResolvedPath);
                absPath = char([absFilePath, filesep, name, ext]);
            else
                absPathInfo = matlab.io.internal.filesystem.resolvePath(relativePath);
                absPath = char(absPathInfo.ResolvedPath);
            end
