classdef CustomMessageRegistry < handle
    ...
    properties (Constant, Hidden)
        ...
        PREFACTIONNAME = 'CustomActionMap'
    ...
    methods (Access=private, Hidden)
        ...
        function [validPref, validPrefSrv, validPrefAction] = checkAndGetPref(h)
            ...
            if isequal(h.ROSVERSION,'ros2')
                actionPrefPresent = ispref(h.PREFGROUP, h.PREFACTIONNAME);
                if actionPrefPresent
                    validPrefAction = isa(prefStruct.CustomActionMap, 'containers.Map');
                    if validPrefAction
                        h.ActionMap = prefStruct.CustomActionMap;
                        assert(isequal(h.VERSION, prefStruct.CustomMessageMapVersion));
                    end
                end
            else
        ...
        function saveActionPref(h)
            if isequal(h.ROSVERSION,'ros2')
                setpref(h.PREFGROUP, h.PREFACTIONNAME, h.ActionMap);
            else
