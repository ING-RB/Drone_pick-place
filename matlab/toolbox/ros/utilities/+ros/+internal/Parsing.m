...
%#codegen
    ...
    methods (Access = public, Static)
        ...
        function [startIdx, endIdx] = findNameValueRangeIndex(args, validNVNames)
        %findNameValueRangeIndex Returns index of start and end name-value occurrence
        %
        %   [startIdx, endIdx] = ros.internal.Parsing.findNameValueRangeIndex(args, validNVNames)
        %   returns the start and end index of the occurrence of set of
        %   valid name-value names VALIDNVNAMES in the argument list ARGS.
        %   If the name cannot be found, this function will return [].
        %   Partial and case-insensitive matching is supported.
        %
        %   Examples:
        %       % Here STARTIDX will be 1, ENDIDX will be 2
        %       idx = ros.internal.Parsing.findNameValueIndex(...
        %          {'CancelFcn', function_handle.empty, 'NodeName', 'test'},...
        %          {'CancelFcn'})
        %
        %       % STARTIDX and ENDIDX will be [], because name cannot be found
        %       idx = ros.internal.Parsing.findNameValueIndex(...
        %          {'192.168.1.1', 13111},...
        %          {'NodeName'})
            ...
            for i = 1:length(args)
                ...
                % If this is still executed, the validation was successful.
                % Return the start index if it is empty.
                if isempty(startIdx)
                    startIdx = i;
                    endIdx = i+1;
                else
    ...
    methods (Static)
        ...
        function validTime = validateROS2Time(time, funcName, varName)
            %validateROS2Time Validate any user input that represents a ROS 2 time
            %
            %   VALIDTIME = validateROS2Time(TIME, FUNCNAME, VARNAME)
            %   validates that a user input TIME represents a ROS 2 time.
            %   TIME is valid if it is a scalar numeric value, a scalar Time
            %   object, or a scalar time struct.
            %   The output VALIDTIME will always be a valid ros.msg.Time
            %   object.
            ...
            % Allow numeric or ros2time struct input
            validateattributes(time, {'numeric', 'ros2time', 'struct'}, ...
                {'nonempty', 'scalar'}, funcName, varName);

            % Restrict the possible values of numeric inputs.
            % Note that only non-negative times are allowed in ROS.
            if isnumeric(time)
                % Convert numeric to a valid ROS 2 time object
                [sec,nsec] = numericToUnsignedSecsNsecs(time,funcName,varName);
                validTime = ros2time(sec,nsec);
            elseif isstruct(time)
                % Convert struct to a valid ROS time object
                if ~isfield(time,'sec') || ~isfield(time,'nanosec')
                    error(message('ros:mlroscpp:time:InvalidStructureInput'));
                end
                validTime = ros2time(double(time.sec),double(time.nanosec));
            else
                % Return handle to input Time object
                validTime = time;
            end
