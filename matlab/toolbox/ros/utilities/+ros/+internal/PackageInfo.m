...
% Copyright 2019-2024 The MathWorks, Inc.
    properties
        ...
        TopLevelProjectName % Top-level project name that contains sub-directory projects
        SubDirectories % Extra sub-directory projects to be added
        ExportedLibraryName % Name of the exported library for Library projects
        ModelName         % Name of current model or function
    end
    ...
    methods
        ...
        function h = set.TopLevelProjectName(h, value)
            validateattributes(value,{'char','string'},{'nonempty'});
            if ~isvarname(value)
                error(message('ros:utilities:util:NotValidPackageName',value));
            end
            h.TopLevelProjectName = convertStringsToChars(value);
        end

        function h = set.SubDirectories(h, value)
            value = convertStringsToChars(value);
            validateattributes(value,{'cell'},{'nonempty'});
            cellfun(@(x)validateattributes(x,{'char','string'},{'nonempty'}),value);
            h.SubDirectories = value;
        end
        ...
        function h = set.ExportedLibraryName(h, value)
            validateattributes(value,{'char','string'},{'nonempty'});
            h.ExportedLibraryName = convertStringsToChars(value);
        end

        function h = set.ModelName(h, value)
            validateattributes(value,{'char','string'},{'nonempty'});
            h.ModelName = convertStringsToChars(value);
        end
    end
    ...
    methods (Hidden, Static, Access = {?ros.internal.ROSProjectBuilder})
        ...
        %parameters
            ...
            parser.addParameter('subDirectories','',@(x)validateattributes(x,{'cell'},{'nonempty'}));
            parser.addParameter('topLevelProjectName','',@(x)validateattributes(x,{'char','string'}));
            parser.addParameter('exportedLibraryName','',@(x)validateattributes(x,{'char','string'},{'nonempty','scalar'}));  
            parser.addParameter('modelName','',@(x)validateattributes(x,{'char','string'},{'nonempty','scalar'}));
        end
    end
    ...
    methods
        ...
        %parser with parsed inputs available as the first argument.
            ...
            if ~isempty(parser.Results.subDirectories)
                h.SubDirectories = parser.Results.importedLibs;
            end
            if ~isempty(parser.Results.topLevelProjectName)
                h.TopLevelProjectName = parser.Results.topLevelProjectName;
            end
            if ~isempty(parser.Results.exportedLibraryName)
                h.ExportedLibraryName = parser.Results.exportedLibraryName;
            end

            if ~isempty(parser.Results.modelName)
                h.ModelName = parser.Results.modelName;
            end
        end
