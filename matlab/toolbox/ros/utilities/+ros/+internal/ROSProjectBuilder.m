...
% Copyright 2019-2024 The MathWorks, Inc.
    ...
    properties (Abstract, Constant, Hidden)
        ...
        CUSTOMACTION_FOLDER_SUFFIX
    end

    properties (Constant, Hidden)
        PKGINFONAME = 'packageInfo.mat';
        MSVCVERSION = [20170000, 20190000, 20220000];
        MSVCNAME = 'Microsoft';
    end
    ...
    properties (Hidden, SetAccess=protected)
        ...
        GenCodeOnly    = false; %Generate code only flag (to avoid remote resource query)
        SkipLocalEnvCheck = false; %Skip local env check flag (to avoid local compiler validation)
        GccLocation    = ''; %Path to gcc compiler that is compatible with MATLAB libstdc++
        GppLocation    = ''; %Path to g++ compiler that is compatible with MATLAB libstdc++
    end

    properties (SetAccess=private,GetAccess=public)
        SuppressOutput = false; %Build without printing in command line.
    end

    properties (SetAccess = protected)
        Packages         %PackageInfos for given packages
        RootDir          %Directory under which src/<package> is created
        UseNinja       = true;  %To turn off Ninja if necessary.
    end
    ...
    methods (Abstract, Static, Hidden)
        ...
        formatCommand();
        getCMakePath();
        getBuildCommand(rootFolder, useNinja);
    end
    ...
    methods (Hidden)
        ...
        %them to base-class builder
            ...
            h.Parser.addParameter('SkipLocalEnvCheck',false,@(x)islogical(x));
            h.Parser.addParameter('SuppressOutput',false,@(x)islogical(x));
            h.Parser.addParameter('UseNinja',true,@(x)islogical(x));
        ...
        %build system
            ...
            if ~isempty(ind)
                ...
                result = sprintf("%s",replace(fullfile(theStatusFile.folder,theStatusFile.name),'\','/'));
            end
        end
        ...
        function newDir = parseSubDirectory(~,originalDir)
        %local utility to parse sub directory for copying files and adding
        %path to CMakeLists.txt
            if startsWith(originalDir, replace(matlabroot,'\','/'))
                newDir = replace(originalDir,replace(matlabroot,'\','/'),'matlab');
            else
                % Erase windows driver letter
                newDir = erase(originalDir,':');
            end
        end

        function newIncludeDirs = copyIncludeFiles(h, srcFiles, destDir, includeDirs)
        %local utility to copy header files to a destDir
        %Based on includeDirs, this function will decide to copy entire
        %folders or files itself.
        %If the copy does not succeed, throws an error

            newIncludeDirs = includeDirs;
            [~,pkgname] = fileparts(destDir);

            for srcf = srcFiles
                % Default case when only one file need to be copied
                currentSrcFile = srcf{1};
                currentDest = destDir;
                destBeenParsed = false;
                for incIdx = 1:numel(includeDirs)
                    if startsWith(srcf{1}, includeDirs{incIdx})
                        %Include directory specified, create and copy
                        %to nested folder under source folder.
                        %Otherwise, copy file directly to source
                        %folder.
                        if ~destBeenParsed
                            % Only parse once for each source file
                            srcFolder  = fileparts(currentSrcFile);
                            currentSubDir = h.parseSubDirectory(srcFolder);
                            currentDest = [destDir '/' currentSubDir];
                            destBeenParsed = true;
                        end

                        % Update include directory to point to
                        % sub-directory in source folder
                        if strcmp(newIncludeDirs{incIdx},includeDirs{incIdx})
                            incSubDir = h.parseSubDirectory(includeDirs{incIdx});
                            newIncludeDirs{incIdx} = ['include/' pkgname '/' incSubDir];
                        end
                    end
                end
                % Copy files or folders to /src folder
                currentSrcFile = ros.codertarget.internal.ProjectTool.convertToUnixPath(currentSrcFile);
                currentDest = ros.codertarget.internal.ProjectTool.convertToUnixPath(currentDest);

                % Create new subfolder if it does not exist
                if ~exist(currentDest,'dir')
                    mkdir(currentDest);
                end
                [status, msg, msgid] = ...
                    copyfile(currentSrcFile, currentDest,'f');
                if ~status
                    error(msgid, msg);
                end
            end
        ...
        %Update the structure so it is a relative path
            ...
            for i = 1:numel(pkgInfoStruct.ActionFiles)
                ...
                pkgInfoStruct.ActionFiles{i} = [obj.CUSTOMACTION_FOLDER_SUFFIX, fname,fext];
            end
        end
        ...
        %Create a given package
            ...
            %copy include files
            newIncludeDirs = h.copyIncludeFiles(pkgInfo.IncludeFiles, packageName, pkgInfo.IncludeDirectories);
            h.copyFiles(pkgInfo.LibIncludeFiles, packageName);

            %update include directories for CMakeLists.txt
            if ~isempty(newIncludeDirs)
                pkgInfo.IncludeDirectories = newIncludeDirs;
            end
    ...
    methods (Access=protected)
        ...
        %Parse Inputs
            ...
            h.SkipLocalEnvCheck = h.Parser.Results.SkipLocalEnvCheck;
            h.UseNinja = h.Parser.Results.UseNinja;
            h.SuppressOutput = h.Parser.Results.SuppressOutput;

            if h.SkipLocalEnvCheck
            ...
            elseif ~ismac
                % On Linux, by now mex is configured with g++, check and pick the g++
    ...
    methods
        ...
        function [result, installDir] = buildPackage(h, ~, varargin)
        ...
        %as a trick we could use h.buildPackage({},'--packages-upto pkgName')
            cmd = h.getBuildCommand(h.RootDir, h.UseNinja);

            curDir = cd (h.RootDir);
            clnUp = onCleanup(@()cd(curDir));

            if isequal(h.SuppressOutput, true)
                [status, result] = h.runBuildSystemCommand(cmd, varargin{:});
                %get all status files
                statusFiles = dir(fullfile(h.RootDir, 'log','build*','*','stdout_stderr.log'));
                buildDirs = dir(fullfile(h.RootDir, 'log','build*'));
                [~,index] = sort([buildDirs(:).datenum],'descend');
                if ~isempty(index)
                    % when build related errors occurs outside colcon like
                    % visual studio errors, 'stdout_stderr.log' is not
                    % available. This creates stdout_stderr.log in the latest
                    % build directory and write the errors to it and
                    % display the log link.
                    latestBuildDir = buildDirs(index(1)); %the latest build directory
                    statusLatestBuildLog = dir(fullfile(latestBuildDir.folder,latestBuildDir.name,'*','stdout_stderr.log'));
                    if isempty(statusLatestBuildLog)
                        buildErrorLog = sprintf("%s",replace(fullfile(latestBuildDir.folder,latestBuildDir.name, "stdout_stderr.log"),'\','/'));
                        fileID = fopen(buildErrorLog,'w');
                        fprintf(fileID,'%s\n',result);
                        fclose(fileID);
                        if status ~= 0
                            error(buildErrorLog); %#ok<SPERR>
                        end
                    end
                end
                [~,ind] = sort([statusFiles(:).datenum],'descend');
                ...
                if nargout < 1
                    fprintf("%s",message('ros:utilities:util:BuildLog',result).getString());
                end
            end
