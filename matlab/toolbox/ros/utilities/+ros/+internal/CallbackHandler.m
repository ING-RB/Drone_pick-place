...
%   Copyright 2021-2024 The MathWorks, Inc.
    ...
    properties (Constant, Access = {?ros.internal.mixin.ROSInternalAccess, ?ros.internal.mixin.InternalAccess})
        ...
        DeadlineCallbackName = 'processDeadlineEventMessage'

        LivelinessCallbackName = 'processLivelinessEventMessage'

        MessageLostCallbackName = 'processMessageLostEventMessage'

        IncompatibleQoSCallbackName = 'processIncompatibleQoSEventMessage'
    end
    ...
    properties (Transient, Access = {?ros.internal.mixin.ROSInternalAccess, ?ros.internal.mixin.InternalAccess})
        ...
        DeadlineCallbackFcn = function_handle.empty

        LivelinessCallbackFcn = function_handle.empty

        MessageLostCallbackFcn = function_handle.empty

        IncompatibleQoSCallbackFcn = function_handle.empty
    ...
    methods
        function obj = CallbackHandler(primaryObject, varargin)
            obj.PrimaryObject = primaryObject;
            if nargin > 1
                obj.CallbackFcn = varargin{1};
            end
            if nargin > 2
                obj.DeadlineCallbackFcn = varargin{2};
                obj.LivelinessCallbackFcn = varargin{3};
                obj.IncompatibleQoSCallbackFcn = varargin{4};
            end
            if nargin > 5
                obj.MessageLostCallbackFcn = varargin{5};
            end
        end
        ...
        function processDeadlineEventMessage(obj, varargin)

            if ~isempty(obj.DeadlineCallbackFcn)
                feval(obj.DeadlineCallbackFcn, get(obj.PrimaryObject), varargin{:});
            end
        end

        function processLivelinessEventMessage(obj, varargin)

            if ~isempty(obj.LivelinessCallbackFcn)
                feval(obj.LivelinessCallbackFcn, get(obj.PrimaryObject), varargin{:});
            end
        end

        function processMessageLostEventMessage(obj, varargin)

            if ~isempty(obj.MessageLostCallbackFcn)
                feval(obj.MessageLostCallbackFcn, get(obj.PrimaryObject), varargin{:});
            end
        end

        function processIncompatibleQoSEventMessage(obj, varargin)
            if ~isempty(obj.IncompatibleQoSCallbackFcn)
                feval(obj.IncompatibleQoSCallbackFcn, get(obj.PrimaryObject), varargin{:});
            end
        end
    end
end
