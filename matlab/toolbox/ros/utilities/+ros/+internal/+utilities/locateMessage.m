...
for i = 1:numel(DirsToLook)
    dirsToLook = fullfile(convertStringsToChars(...
          DirsToLook{i}));
    packagedirs{end+1} = fullfile(dirsToLook, packageName, args); %#ok<AGROW>
    if isfolder(packagedirs{end})
        dirs = fullfile(packagedirs{end},msgFile);
        if isfile(dirs)
            location = dirs;
            break;
        end
