...
customMsgInstallDirs = fullfile(fileparts(registry.getBinDirList));
mFilePaths = fullfile(customMsgInstallDirs,'m');
if ~iscell(mFilePaths)
    mFilePaths = {mFilePaths};
end
clnup = []; %#ok<NASGU>
...
if isequal(rosver,'ros')
    ...
    [data,info] = getCachedMap(msgMapROS2,msg,msgInfo,rosver,mFilePaths);
end
end

function [data,info] = getCachedMap(msgMap,msg,msgInfo,rosver,mFilePaths)
...
if isKey(msgMap, msg)
    ...
    if isequal(rosver,'ros2') && msgInfo.custom
        curpath = path;
        clnup = onCleanup(@(x)path(curpath));
        for i=1:numel(mFilePaths)
            addpath(mFilePaths{i});
        end
    end
