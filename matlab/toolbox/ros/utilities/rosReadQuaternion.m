...
% Ensure the generated code is not inlined
    ...
    if isfield(msg, 'X')
        ...
        coder.internal.assert(~isempty(msg.X),'ros:mlroscpp:codegen:EmptyInputMsg','rosReadQuaternion');
        specialMsgUtil = ros.internal.SpecialMsgUtil;
    else
        % ROS 2 message struct
        % Sanity check to make sure message is not empty
        coder.internal.assert(~isequal(msg.x,0),'ros:mlroscpp:codegen:EmptyInputMsg','rosReadQuaternion');
