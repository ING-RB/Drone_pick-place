...
function setUserPath(newPath)
  if exist(newPath, 'dir')
    % Ensure that p is an absolute path
    if isAbsolute(newPath)
      newPath = builtin("_canonicalizepath", newPath);
      oldUserPath = system_dependent('getuserworkfolder');
      addpath(newPath);
      % Only perform the rest of the update if newPath was added
      % Else, do nothing (addpath will warn in that case)
      if (isOnPath(newPath))
        rmpathWithoutWarning(oldUserPath);
        s = matlab.settings.internal.settings;
...
function status = isAbsolute(file)
  if ispc
    file = strrep(file,'\','/'); % Normalize file seperators
...
function y = isOnPath(loc)
    % Assumes loc is already canonicalized
    if ispc
        ps = ';';
    else
        ps = ':';
    end
    y = ismember(loc, split(path,ps));
end
