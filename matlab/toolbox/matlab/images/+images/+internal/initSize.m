...
%   Copyright 1993-2018 The MathWorks, Inc.
  ...
  image_width  = images.internal.getImWidth(im_handle);
  image_height = images.internal.getImHeight(im_handle); 
  ...
  ax_pos = matlab.ui.internal.PositionUtils.getDevicePixelPosition(ax_handle);
  fig_pos = matlab.ui.internal.PositionUtils.getDevicePixelPosition(fig_handle);
  ...
  while (is_width_bigger_than_screen || is_height_bigger_than_screen)
      screen_per_image_pixel = images.internal.findZoomMag('out',screen_per_image_pixel);
  ...
  % Are existing extents sufficient?
  wontFit = new_fig_width>orig_fig_width || new_fig_height>orig_fig_height;
 
  % Calculate new figure position
  if wontFit
      % Move the figure only if the width/height change
  ...
  matlab.ui.internal.PositionUtils.setDevicePixelPosition(fig_handle, fig_pos);
  matlab.ui.internal.PositionUtils.setDevicePixelPosition(ax_handle, ax_pos);
  ...
  function calculateDimensions
     on_screen_image_width = ceil(image_width * screen_per_image_pixel);
     on_screen_image_height = ceil(image_height * screen_per_image_pixel);
