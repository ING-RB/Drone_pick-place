...
common_args = struct('Filename','',...
    ...
    'Interpolation',get(groot,'DefaultImageInterpolation'));
...
if ~isempty(string_indices) && num_args > 1
    ...
    if is_first_string_first_arg && even_nargs_after_first_string
        ...
        if ~any(matches) %i.e. none
            error('images:imageDisplayParsePVPairs:unrecognizedParameter',...
                '%s',getString(message('MATLAB:images:imageDisplayParsePVPairs:unrecognizedParameter',...
                varargin{param1_index}, mfilename)));
        end
    end
end
...
if num_args<0
error('images:validate:tooFewInputs', '%s',getString(message('MATLAB:images:validate:tooFewInputs',mfilename)));
elseif num_args>2
error('images:validate:tooManyInputs', '%s',getString(message('MATLAB:images:validate:tooManyInputs',mfilename)));
end
...
switch num_args
    ...
    case 2
        ...
        elseif (size(varargin{2},2) == 3)
            ...
            error('images:imageDisplayParsePVPairs:invalidInputs',...
                '%s',getString(message('MATLAB:images:imageDisplayParsePVPairs:invalidInputs')))
...
if rem(length(in),2)~=0
  error('images:imageDisplayParsePVPairs:oddNumberArgs', '%s',getString(message('MATLAB:images:imageDisplayParsePVPairs:oddNumberArgs', upper( function_name ))));
end
    ...
    switch prop_string
        case 'DisplayRange'
            if isempty(in{k+1})
                ...
                common_args.(prop_string) = images.internal.checkDisplayRange(in{k+1},mfilename);
            end
        ...
        otherwise
	error('images:imageDisplayParsePVPairs:unrecognizedParameter', '%s',getString(message('MATLAB:images:imageDisplayParsePVPairs:unrecognizedParameter', prop_string, function_name)));
