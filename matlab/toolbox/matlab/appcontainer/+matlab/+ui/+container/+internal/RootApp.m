classdef RootApp < matlab.ui.container.internal.AppContainer ...
    ...
    methods (Static)
        ...
        function instance = getInstance()
            instance = matlab.ui.container.internal.RootApp.getSingleton();
            matlab.ui.container.internal.RootApp.doPollOnState(instance, 30);

            if instance.State == matlab.ui.container.internal.appcontainer.AppState.TERMINATED
                % if state changes to TERMINATED, it may indicate a UIContainer is actively starting up. Try again to connect.
                instance = matlab.ui.container.internal.RootApp.getSingleton();
                matlab.ui.container.internal.RootApp.doPollOnState(instance, 60);
                % TODO: consider warning when RootApp times out without connecting
            end

            if feature('webui') && ~desktop('-inuse')
                % Undock the document groups (editor, figure, and VE) so that these
                % documents can appear while the JSD is hidden. Also, Disable the
                % DockControls in the nodesktop mode as we don't want someone to
                % accidentally dock something into the hidden JSD.
                matlab.ui.container.internal.RootApp.undockDocumentGroupsAndDisableDockControls(instance);
            end
        end
    end
    
    methods (Access = private, Static)
        function instance = getSingleton()
            if feature('webui') && ~desktop('-inuse')
                desktop('-hidden');
            end
        ...
        function doPollOnState(instance, timeout)
            hTimer = timer('TimerFcn', @(h,e)stop(h), 'StartDelay', timeout);
            start(hTimer);
            while isvalid(instance) && hTimer.TasksExecuted == 0 && instance.State == matlab.ui.container.internal.appcontainer.AppState.INITIALIZING
        ...
        function undockDocumentGroupsAndDisableDockControls(instance)
            % TODO: find out a better way of doing rather than adding
            % the following logic for each document type.
            
            editorGroup = instance.getEditorGroup();
            editorGroup.Docked = false;
            editorGroup.DisableDockControls = true;

            figureGroup = instance.getFigureGroup();
            figureGroup.Docked = false;
            figureGroup.DisableDockControls = true;

            variableEditorGroup = instance.getVariableGroup();
            variableEditorGroup.Docked = false;
            variableEditorGroup.DisableDockControls = true;
        end
    ...
    methods
        ...
        function selectCommandWindow(this)
            this.selectTool('commandWindow');
        end
        ...
        function group = getFigureGroup(this)
            group = this.getDocumentGroup("defaultfigure");
        end
        ...
        function bringDocumentToFront(this, document)
            ...
            data.groupId = document.DocumentGroupTag;
    ...
    methods (Static = true, Hidden = true)
        function setStatusMessage(statusStr)
            % g3172852: introducing static private method to support Simulink startup use-case and bypass RootApp handshake with client
            % If you want to use this API, please contact the MATLAB Desktop team. 
            import matlab.ui.container.internal.root.AbstractStatusBarMixin

            narginchk(1, 1);
            if isempty(statusStr) || (isequal(numel(statusStr), 1) && strcmp(statusStr, ""))
                data.type = 'clearStatus';
            else 
                data.type = 'setStatus';
                data.value = statusStr;
            end
