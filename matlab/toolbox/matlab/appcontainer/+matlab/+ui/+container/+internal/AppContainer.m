classdef AppContainer < matlab.ui.container.internal.appcontainer.PeeredProperties
    ...
    properties (Dependent) % These properties are stored in the container peer model
        ...
        % Warning: If WindowBounds are set before the AppContainer is made visible,
        % they will not account for the dimensions of the window trim. Thus the app window
        % will appear slightly larger than the set value when it is finally made visible. (g3232980)
    ...
    properties (Access = private)
        ...
        PrivateMinSize;
        DebugPortInternal = 0;
        % Numeric id of the most recently received close request from browser layer
        LatestCloseRequestId;
    end
    ...
    properties (GetAccess = private, Constant)
        MCOSPropertyNames = {
            ...
            'WindowBoundsModified', 'WindowURL', 'OfferDocumentMaximizeButton', 'EnableTheming', ...
            'uuid'};
        JSPropertyNames = {
            'tag', 'title', 'cleanStart', 'persistDocuments', 'persistLayout', ...
...
%                             this.BrowserHandle = [];
                ...
                % g3415424 close dialog window in MATLAB Online
                if isRemoteClient && ~isempty(this.Window) && isvalid(this.Window)
                    this.Window.hide;
                end
            end
        ...
        function set.uuid(this, value)
            this.setProperty('uuid', value);
        end
        ...
        function value = get.WindowBounds(this)
            ...
            % If attached to a JavaScript-authored UIContainer via a custom page
            if this.PrivateState ~= AppState.RUNNING && ~isempty(this.Window) && ~isempty(this.AppPage)
                ...
                % g3268615: client side follows bottom-left convention as
                % of 24b, AppContainer clients use top-left
                value = this.getProperty('WindowBounds');
                if isstruct(value)
                    value = [value.x value.y value.w, value.h];
                end
                value = this.convertOrigin(value);
            end
        end
        ...
        function value = get.Product(this)
            value = this.getProperty('Product');
        end
        ...
        function value = get.Scope(this)
            value = this.getProperty('Scope');
        end 
        ...
        function value = get.UndockedPage(this)
            value = this.getProperty('UndockedPage');
        end
        ...
        function addPanel(this, panel)
            ...
            if isa(panel,'matlab.ui.internal.FigurePanel')
                this.handleThemeSupport(panel.Figure);
            end
        ...
        function addDocument(this, document)
            ...
            if isa(document,'matlab.ui.internal.FigureDocument') 
                this.handleThemeSupport(document.Figure);
            end
...
% 
%         end
        function selectTool(this, id)
            data.id = id;
            data.type = 'selectTool';
        ...
        function documents = getDocuments(this, varargin)
            ...
            if nargin > 2
                error("Arguments exceeds the number of valid inputs. Arguments must not exceed 1");
            elseif nargin == 2
        ...
        function closeDocument(this, documentGroupTag, tagOrTitle, varargin)
            ...
            if isempty(tagOrTitle)
                error("Document tag or title must not be empty");
            end
            ...
            if force || document.canClose()
                if isvalid(document) % if canclose method didn't kill the doc
                    ...
                    if isvalid(document)
                        % g3414501: check isvalid again in case canClose ran asynchronously
        ...
        function [params, features] = extractFeatures(params)
            ...
            for idx = 1:length(params)
                ...
                if ~isempty(features)
                    ...
                    features = intersect(features, string(matlab.ui.container.internal.AppContainer.SupportedFeatures));
                    return;
                end
            end  
        end
    ...
    methods (Access = protected)       
        function handlePeerNode(this)
            ...
            if ~this.Attached && this.AppPage == ""
                ...
                % g3032029: Configure initial set of documents that are added
                % to not bring the App window to front
                for i = 1:length(documents)
                    documents{i}.AddWithoutWindowFocus = true;
    ...
    methods (Access = ?matlab.unittest.TestCase)
        ...
        function handleThemeSupport(this,fig)
            
             if this.EnableTheming
                 % Configure figures within apps that have opted into 
                 % theming to always use the same theme as the desktop.
                 matlab.graphics.internal.themes.figureUseDesktopTheme(fig);
             else
                 % If app has not opted into theming, it should always 
                 % appear light. If automatic figure theming is enabled,
                 % lock down the theme to be light.
                 autoFigThemesEnabled = false;
                 try 
                     autoFigThemesEnabled = feature('AutomaticFigureThemes');
                 catch
                    % The AutomaticFigureThemes feature is not yet
                    % available in all clusters.
                 end
                 if autoFigThemesEnabled
                     fig.Theme = matlab.graphics.internal.themes.lightTheme;
                 end
             end
        end
    end
    ...
    methods (Access = private)
        ...
        function initializeView(this, attach)
            ...
            if (this.AppPage == "" && ~attach)
                this.MessageChannel = "/" + this.uuid + "/uicontainer";
            else
            ...
            this.ModelChannel = ['/app/' convertStringsToChars(this.uuid)];
        ...
        function window = buildWindow(this, windowBounds)
            ...
            window.MATLABWindowExitedCallback = @MATLABWindowExitedCallback;

            window.MATLABClosingCallback = @MATLABClosingCallback;

            window.WindowStateCallback = @windowStateChangeHandler;

            window.WindowResized = @windowResizedCallback;

            window.FocusGained = @windowFocusGainedCallback;

            window.FocusLost = @windowFocusLostCallback;
            ...
            function windowFocusGainedCallback (win, ~)
                notify(this, 'FocusGained');
            end

            function windowFocusLostCallback (win, ~)
                notify(this, 'FocusLost');
            end

            function MATLABWindowExitedCallback(win, ~)
                win.close();
                this.runCloseTasks(false);
            end
            ...
            function canWindowClose = ensureCanCloseIsExecuted(win)
                ...
                if ~this.CanCloseExecuted
                    ...
                    % called from windowClosingCallback or
            ...
            function MATLABClosingCallback(win, ~)
               result = ensureCanCloseIsExecuted(win);
               % With MATLABClosingCallback we have to call exit
               % explicity to allow MATLAB to exit.
               if result
                  exit;
               end
            end
        ...
        function result = canClose(this)
            if isempty(this.CanCloseFcn)
                result = true;
            elseif ~this.CanCloseExecuting && ~this.isDocumentCanCloseFcnInProgress()
        ...
        function handlePeerEvent(this, data)
            if isvalid(this)
                ...
                if shouldReply 
                    this.LatestCloseRequestId = data.(dataField).id; % allow this to update independent of canClose fcn execution
                    if ~this.CanCloseExecuting && ~this.isDocumentCanCloseFcnInProgress() % not already processing a close request
                    % Reply to close query
                    event.type = 'closeReply';
                    event.approve = this.canClose();
                        event.id = this.LatestCloseRequestId; % return the latest value, as the most recent id is the only one honored by browser (g3184355)
        ...
        function document = getDocumentByTitle(this, documentGroupTag, title)
            ...
            for i=1:length(documents)
                if isvalid(documents{i}) && documents{i}.DocumentGroupTag == documentGroupTag && documents{i}.Title == title
        ...
        function addDocumentInMotion(this, uuid, hDoc, pendingMotion)
            ...
            % synchronicity logic only in effect for visble documents in RootApp
            % exclude invisible and phantom documents from synchronicity logic for now (g2837589)
            if this.State ~= AppState.RUNNING || ~isvalid(hDoc) || ~hDoc.Visible || hDoc.Phantom || ...
                (~isa(this, 'matlab.ui.container.internal.RootApp') && ~this.Visible)
                return;
            end
        ...
        function refreshDocumentMap(this, doNotClear)
            if (nargin < 2 || ~doNotClear) && this.ViewInitialized && (this.PrivateState == matlab.ui.container.internal.appcontainer.AppState.RUNNING)
                ...
                for i = 1:length(keysToRemove)
                    doc = this.DocumentMap(keysToRemove{i});
                    if isvalid(doc) && ismethod(doc, 'removePeerPropertyListener')
                        doc.removePeerPropertyListener();
                    end
        ...
        function result = isDocumentCanCloseFcnInProgress(this)
            result = false;
            documents = this.getDocuments();
            for index = 1:numel(documents)
                if documents{index}.CanCloseExecuting == true
                    result = true;
                    break;
                end
            end
