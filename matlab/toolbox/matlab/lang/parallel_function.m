function varargout = parallel_function(range, F, consume, supply, ...
                                       ...
                                       M, divide)
%PARALLEL_FUNCTION Basis of parfor
%   PARALLEL_FUNCTION is the basis of parfor, but is not officially supported,
%   either with this name or API.
%
%   See also parfor, spmd, parfeval, parpool, parcluster.
...
%   The range specifies a semi-open interval (BASE-1,LIMIT].  If BASE >= LIMIT, nothing
%   happens because the interval is empty.  Otherwise, let N denote LIMIT-(BASE-1),
...
%   discussed below. We will use the convention that N_0 = BASE-1 and
%   N_k = LIMIT.  Independently of k and the intermediate N_j, (N_0,N_k] is the
%   union of the subintervals (N_0,N_1],...,(N_{k-1},N_k].
%
%   PARALLEL_FUNCTION's essence:
%
%     * In parallel, evaluate F(N_{j-1},N_j) in "worker" processes, j = 1,...,k.
%
%   The process on which PARALLEL_FUNCTION is called is the "client".
%
...
%
%   For each call to PARALLEL_FUNCTION, the every time F is called in a worker,
%   Rin is IDENTITY.  Workers return their value of Rout to the client after each
%   evaluation of F, and do not retain any partially-reduced state in between
%   invocations of F.  The client combines all the partially-reduced states
%   computed on the workers.  The client combines these value by
...
%
%     * [O, r] = F(N_{j1-1}, N_{j+1}, I1, IDENTITY); % done in one worker
%
...
%   run to run of the identical calls to PARALLEL_FUNCTION.
%
%   The server transmits the value of O and R to the client, as in the above
%   cases.
%
...
%   inf, or some other valid means of specifying where to run the loop.
%   PARALLEL_FUNCTION will try to reserve min(N,M) workers.  In the
%   subsequent discussion W will denote the number of workers actually reserved,
%   so W <= min(N,M) <= N.
%
%   Valid values for M are:
%   - 0 - which means "run the loop in the client as quickly as possible"
%   - strictly positive numeric scalars - which means "run the loop on a parallel
%     pool of M (or fewer) workers (if one is already available or can be auto-created)"
%   - 'debug' - which means "run the loop in the client in a 'debug' mode"
%   - an instance of parallel.Pool, which explicitly runs the loop on the specified
%     pool
%   - an instance of parallel.Cluster, which explicitly runs the loop on the
%     specified cluster object, with no parallel pool required
%   - an instance of parallel.parfor.Options, which fully specifies where
%     to run the body of the loop.
%
%   In all cases, this argument, in combination with N, determines a set of
...
%
%   As before, the N_i are chosen to divide (BASE-1,LIMIT] as evenly as possible
%   into k subintervals.
%
...
%         equivalently, the length of NN is what we have denoted by k.
%
...
if nargin >= 5 && ~isempty(reduce)
    ...
    identity =  0; % ... benefit of ... distributed_execution
end
...
if nargin >= 7 && ~isempty(concat)
    ...
    concatenator = internal.parallel.parfor.online_concatenator(concat, empty);
else
    empty = []; % solely for the benefit of empty loops.
    concatenator = []; % solely for the benefit of distributed_execution
end

% M is already validated by parallel_M_check, simply set the value if none specified.
if nargin < 9
    M = Inf;
end
...
% Now that we know that the range is not empty, choose where to run based on M.

% First, resolve any raw Cluster or Pool objects to the corresponding Options object.
if isa(M, 'parallel.Cluster') || isa(M, 'parallel.Pool')
    if ~isempty(M)
        M = parforOptions(M);
    else
        % Empty pool, fall back to serial (M = 0)
        M = 0;
    end
elseif isnumeric(M) && M > 0 && PCTInstalled
    % Resolve positive M into parforOptions.
    [isPoolRunning, pool] = pctTryCreatePoolIfNecessary();
    if isPoolRunning
        M = parforOptions(pool, 'MaxNumWorkers', M);
    else
        % No pool, and auto-create didn't create one - fall
        % back to the "0" case.
        M = 0;
    end
elseif isa(M, 'parallel.parfor.Options') || isequal(M, 'debug')
    % No action required
else
    % No PCT
    M = 0;
...
% There should be three remaining cases: numeric, 'debug', or Options object.
engineBuilder = []; % Implies use fall-back executor.
if isequal(M, 'debug')
    % Use the debug engine. Fabricate an effective number of workers.
    localW = max(1, floor(log2(N)));
    engineBuilder = @(varargin) localparfor(localW, varargin{:});
elseif isa(M, 'parallel.parfor.Options')
    % Build the engine from the options
    engineBuilder = @(initData, F) createEngine(M, initData, F, N);
else
    % Must be left with the M == 0 case.
    assert(isequal(M, 0), ...
           'Expected to find M==0.');
end

% Get the engine
if isempty(engineBuilder)
    engine = [];
else
...
% Validate divide after call to run locally as this isn't used in the local case
W = engine.getNumWorkers();
if nargin >= 10 && ~isempty(divide)
    ...
    % Use the options structure to divide the range
    NN = engine.divideRangeIntoIntervals(base, limit, W);
end
...
try
    R = distributed_execution(...
        engine, base, limit, W, k, NN, NN0, consume, supply, ...
        reduce, identity, concatenator, ...
...
varargout = results(nargout);
    ...
    if bitand(consume_supply_reduce_concat, reduce_bit) ~= 0 % always result R in this case
        if bitand(consume_supply_reduce_concat, concat_bit) ~= 0
            assert(nresults <= 2, 'Only up to 2 outputs supported.');
            if nresults == 2
                C = {R, S};
            else
                C = {R};
            ...
            assert(nresults <= 1, 'Only 1 output supported for this case.');
            C = {R};
        end
    elseif bitand(consume_supply_reduce_concat, concat_bit) ~= 0 % always result S in this case
        ...
        assert(nresults == 0, 'No outputs supported for this case.');
        C = {};
    end
    end
...
function NN = divide_harmonic(base, limit, W)
N = limit - base;
...
% It is important that we make these logical as we are going to cast the
% values to double and assume that the values are zero or one (a bit of a hack that
...
[initialDispatchSize, normalDispatchSize] = P.getDispatchSizes(k, W);
assert(initialDispatchSize > 0 && initialDispatchSize <= k, ...
    'Incorrect initial dispatch size: %d', initialDispatchSize);

thisDispatchSize = min(k, initialDispatchSize);
...
try
    ...
    % Notify that all intervals have been added.
    P.allIntervalsAdded();
    ...
    while numToConsume > 0
        ...
        chunkSize = min(maxSize, numToConsume);
        ...
        for i = 1:numel(tags)
            ...
            if thisTag <= j
                if doConsume || doConcat
                    consume_reduce(thisTag, out{i, 1});
                end
                if doReduce
                    R = reduce(R, out{i, 2});
                end
...
channel = make_general_channel(F, identity, doConsume, doReduce, doConcat);

end

function channel = make_general_channel(F, identity, doConsume, doReduce, doConcat)
    function OR = channel_general(C)
        % Exit condition that sends back the reduce variable if requested
        if isempty(C)
            % 'final' intervals are no longer used.
            OR = [];
            return;
        end
        ...
        if doConsume
            ...
            if doReduce
                if doConcat
                    [R, O] = F(C{:}, identity); % ship "S" to client, so call it O
                else
                    R = F(C{:}, identity);
                end
            else
        ...
        % Always return O and R - workers do not store partially-reduced values, and
        % always start again from the identity.
        OR = {O, R};
    end
...
%----------------------------------------------------------------------------
% Convenience wrappers areound PCT installation/licence checks.
...
%----------------------------------------------------------------------------
function P = localparfor(W, initData, ~)
P = struct('addInterval', @nAddInterval, ...
           'getNumWorkers', @() W, ...
           'allIntervalsAdded', @() [], ...
           'getCompleteIntervals', @nGetCompleteIntervals, ...
           'divideRangeIntoIntervals', @divide_harmonic, ...
           'getDispatchSizes', @(k, W) deal(k, 2*W), ...
           'complete', @(varargin)[]); % 'complete' does nothing for local execution.
...
pendingQueue = cell(0, 2);
    ...
    function [tags, results] = nGetCompleteIntervals(numIntervals)
        ...
        for i = 1:numIntervals
            ...
            try
                out = feval(channels{currentChannelIndex}, channelArg{:});
            catch err
...
% It is not appropriate for this warning to be issued during
% serialization/deserialization:
warnId = 'MATLAB:mir_warning_maybe_uninitialized_temporary';
state = warning('off', warnId);
restore = onCleanup(@() warning(state));
x = parallel.internal.pool.mxSerializeDeserialize(x);
end
