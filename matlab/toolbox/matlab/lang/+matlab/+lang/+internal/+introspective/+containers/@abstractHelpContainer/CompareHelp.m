function detectedAnyChange = CompareHelp(this, prevFilePath)
    ...
    if ~ischar(prevFilePath) || ~isfile(prevFilePath)
...
function detectedAnyChange = compareForSimpleElement(this, prevClassInfo, elementKeyword)
    ...
    elementInfoFcnHandle = @(metaData)prevClassInfo.getSimpleElementInfo(metaData, elementKeyword, ~this.onlyLocalHelp);
...
function prevClassInfo = getPreviousClassInfo(prevFilePath)
    ...
    switch lastDir(1) 
    case '+'
        nameResolver  = matlab.lang.internal.introspective.resolveName(prevFilePath);
        prevClassInfo = nameResolver.classInfo;
    case '@'
        nameResolver  = matlab.lang.internal.introspective.resolveName(parentPath);        
        prevClassInfo = nameResolver.classInfo;
    otherwise
        prevClassInfo = matlab.lang.internal.introspective.classInformation.simpleMCOSConstructor(fileName, ...
                        prevFilePath, true);
    end
end
...
function detectedHelpChange = checkMemberHelpUpdates(memberIterator, getInfo)
    ...
    while memberIterator.hasNext
        ...
        if isempty(classMemberInfo)
            % classMemberInfo will be empty for new methods/properties
            detectedHelpChange = true;
        else
            if hasHelpChanged(classMemberInfo.getHelp, memberHelpContainer.getHelp)
                detectedHelpChange = true;
            else
