classdef HelpContainerFactory
    ...
    methods (Static)
        function this = create(filePath, varargin)
            ...
            %	hC = matlab.lang.internal.introspective.containers.HelpContainerFactory.create(filePath);
            %
            ...
            p.addParameter('noSuperClassHelp', false, @islogical);
            ...
            if ~isempty(metaInfo) % filePath is a classdef file
                this = matlab.lang.internal.introspective.containers.ClassHelpContainer(filePath, ...
                    metaInfo, p.Results.onlyLocalHelp, p.Results.noSuperClassHelp);
            else
                this = matlab.lang.internal.introspective.containers.FunctionHelpContainer(filePath);
            end
        end
    end
end
...
function metaInfo = getMetaInfo(filePath)
    ...
    metaInfo = [];
    if matlab.lang.internal.introspective.isClassMFile(noExtFile)
        % True for both old and new MATLAB Class Object System
        qualifiedName = matlab.lang.internal.introspective.containers.getQualifiedFileName(filePath);

        % metaInfo is empty for old MATLAB Class Object System classes.
        metaInfo = matlab.lang.internal.introspective.getMetaClass(qualifiedName);
    end
end
