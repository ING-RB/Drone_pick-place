function usage = getUsage(functionName)
    ...
    %
    %   Examples:
    %      usage = matlab.lang.internal.introspective.getUsage("magic");
    %
    ...
    %           rhs: "n"
    %
    %      usage = matlab.lang.internal.introspective.getUsage("LinearModel.Plot");
    %
    ...
    sigs = matlab.lang.internal.introspective.getSignatures(functionName);
...
function usage = sigs2struct(sigs, functionName)
    ...
    for sig = sigs
        ...
        if s.contains('(')
            ...
            argumentList = splitArgumentList(argumentList);
            eqs = argumentList == "=";
            names = [eqs(2:end), false];
            argumentList(names) = '"' + argumentList(names) + '"';
            argumentList(eqs) = [];
            cur.rhs = argumentList;
        else
            if isempty(cur.lhs)
                s = split(strip(s), ' ');
                if ~isscalar(s)
        ...
        if ~isempty(regexpi(strip(functionName), append('^', functionNamePattern, '$'), 'once'))
