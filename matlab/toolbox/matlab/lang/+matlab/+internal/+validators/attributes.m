...
% attribute not found value
    ...
    case 'binary'
        fcn = @(x)(islogical(x) || (isnumeric(x) && isreal(x) && all( x(:)==0 | x(:)==1 )));
    ...
    case 'real'
        % if it is not numeric, it is real
...
function valueAttributor( fcn, A, attrName, fname, id, msgId, argname, argpos)
    ...
    if e.identifier ~= "MATLAB:UndefinedFunction" && e.identifier ~= "MATLAB:math:mustBeNumericCharOrLogical"
        rethrow(e);
    end
...
function monotonicAttributor( fcn, A, attrName, fname, id, msgId, argname, argpos)
 ...
 if ~isnumeric( A )
    ...
    if any(isnan(A(:)), 'all') || ~isreal(A) || ~all( fcn(diff(A),0), 'all' )
    argDes = matlab.internal.validators.getArgumentDescriptor( msgId, argname, argpos );
...
% check that v is a usable expected value
    if ~isreal(v) || issparse(v) || ~isnumeric(v) || ~isscalar(v) || isinf(v) || v < 0 || ~(v == floor(v))
...
% check that v is a usable expected size, don't do empties correctly
    if ~isreal(v) || issparse(v) || ~isnumeric(v) || isscalar(v) || any(isinf(v(:))) || any(v(:) < 0 ) || ~(all(v == floor(v) | isnan(v)))
...
% and throw the error
    ...
    if isa(A,classes{i})
        isAInClasses = true;
        break;
    elseif ~isa(classes{i}, "classID") && strcmp(classes{i}, 'numeric')
        classes{i} = 'double, single, uint8, uint16, uint32, uint64, int8, int16, int32, int64';
    end
    end

    if ~isAInClasses
    if all(cellfun(@(x) isa(x, 'classID'), classes))
            classesStr = cellfun(@(x)x.Name, classes);
        classesStr = sprintf('%s, ', classesStr(:));
    else
...
function [ A, classes, attrs, fname, msgId, argname, argpos ]  = checkInputs( inputs )
    ...
    classes = {classes};
    elseif isa(classes, 'classID')
    classes = num2cell(classes);
    elseif ~iscellstr( classes )
    if feature('packages') == 1
        error('MATLAB:validateattributes:badClassList', ...
              message('MATLAB:validateattributes:badClassListWithPackages', 'classID').getString())
    else
