function varargout = setxor(A, B, flag1, flag2)
...
if nargin < 3
    [varargout{1:nlhs}] = cellsetxorR2012a(A, B);
else
    ...
    if flaginds(2) || flaginds(3) % 'stable'/'sorted' specified
        ...
        [varargout{1:nlhs}] = cellsetxorR2012a(A, B, logical(flaginds(1:3)));
    elseif flaginds(5) % trailing 'R2012a' specified
        [varargout{1:nlhs}] = cellsetxorR2012a(A, B, logical(flaginds(1:3)));
    elseif flaginds(4) % trailing 'legacy' specified
        [varargout{1:nlhs}] = cellsetxorlegacy(A, B, logical(flaginds(1)));
...
ambiguous = ((size(a,1)==0 && size(a,2)==0) || isscalar(a)) && ...
    ((size(b,1)==0 && size(b,2)==0) || isscalar(b));
