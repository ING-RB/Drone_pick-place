...
if nrhs == 1
    ...
    if flaginds(6) && flaginds(6)~=nrhs-1
        error(message('MATLAB:UNIQUE:LegacyTrailing'))
    end
    if flaginds(7) && flaginds(7)~=nrhs-1
    ...
    if flaginds(4) || flaginds(5) % 'stable'/'sorted' specified
        ...
        [varargout{1:nlhs}] = matlab.internal.math.uniqueCellstrHelper(A,logical(flaginds(1:5)));
    elseif flaginds(6) % trailing 'legacy' specified
        [varargout{1:nlhs}] = celluniquelegacy(A,logical(flaginds(1:3)));
...
isrow = ismatrix(a) && size(a,1)==1 && size(a,2) ~= 1;
