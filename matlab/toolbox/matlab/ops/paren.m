...
%     for the field name. The field name is called 'dynamic' because the
%     name is established at runtime. The syntax for referencing a 
%     structure S with dynamic field name f is
%
%        S.(f)
%
%     The following example results in aVal getting the value stored in 
%     S.field_two, that is, 2:
%
%        S.field_one = 1; 
%        S.field_two = 2; 
%        fieldname = 'field_two'; 
%        aVal = S.(fieldname); 
%
%     The next example shows a function that accepts two inputs: the first
%     (account_id) a dynamic field name, and the second (year) an index
%     into that field:
% 
...
%
%        acctinfo = get_acct_summary('jmahoney114', 2021);
%
%     The dot-parentheses syntax can also be used to dynamically reference
%     object properties and methods. If obj is an instance of a class with
%     the property 'property1', then you can reference the property using a
%     variable with the dot-parentheses syntax:
%
%        label = "property1";
%        out = obj.(label);
%
%     If the class has a method 'method1' that takes a single argument, you
%     can call the method using the dot-parentheses syntax:
%
%        methodName = "method1";
%        arg = 5;
%        out = obj.(methodName)(arg);
%
%     The dot-parentheses syntax can also be used by class authors who
%     overload indexing using mixins in the matlab.mixin.indexing package.
%     Class authors use this syntax to delegate handling of the indexing to
%     another object. This use is known as forwarding, and occurs when the
%     dynamic name is replaced by an object of type
%     matlab.indexing.IndexingOperation. See the help for
%     matlab.indexing.IndexingOperation for more details.
%
