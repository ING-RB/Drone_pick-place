...
if nargin < 3 || matlab.internal.math.partialMatch(opt, 'fix', 2)
    c = idivide_fix(a,b);
elseif matlab.internal.math.partialMatch(opt, 'floor', 2)
    c = idivide_floor(a,b);
elseif matlab.internal.math.partialMatch(opt, 'ceil')
    c = idivide_ceil(a,b);
elseif matlab.internal.math.partialMatch(opt, 'round')
    c = a ./ b;
else
...
% Validate input.
if ~matlab.internal.math.isSizeCompatible(a,b)
...
if isfloat(a)
    c = cast(fix(a./double(b)), "like", b);
elseif isfloat(b)
    c = cast(fix(double(a)./b), "like", a);
else
    c = (a - rem(a,b)) ./ b;
end
...
if isfloat(a)
    c = cast(floor(a./double(b)), "like", b);
elseif isfloat(b)
    c = cast(floor(double(a)./b), "like", a);
else
    c = (a - rem(a,b)) ./ b;
...
if isfloat(a)
    c = cast(ceil(a./double(b)), "like", b);
elseif isfloat(b)
    c = cast(ceil(double(a)./b), "like", a);
else
    c = (a - rem(a,b)) ./ b;
    idx = (b ~= 0) & (sign(a) == sign(b)) & ((b .* c) ~= a);
    c(idx) = c(idx) + 1;
