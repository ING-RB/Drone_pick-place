...
persistent defaultWorkspaceID lastUpdate
...
try
    ...
    elseif isa(var, 'dictionary')
        % dictionary provides its own display, so we should use it.  Create it for an arbitrarily wide
        % display (1000 characters), since the WSB table will truncate as the column is narrowed.
        c = compactRepresentationForSingleLine(var, matlab.display.DisplayConfiguration, 1000);
        retval = com.mathworks.widgets.spreadsheet.data.ValueSummaryFactory.getInstance(c.Representation, size(var));
        
        % Overload usage of setMissing so that the 1x1 size isn't shown.
        retval.setMissing(true);
    end
...
if builtin('isnumeric', in)
    if isscalar(in)
        if issparse(in) && nnz(in) == 0
            % Make sure empty sparse shows as 0 (g2133764)
            outVal = createComplexScalar(0);
        else
    ...
    elseif isempty(in)
        ...
        try
            ...
            % If the short-circuit for getting the value summary for empty
            % values fails, let it run through the normal method to get the
            % value summary data.
...
if isinteger(in)
    ...
    elseif isa(in, 'half')
        outVal = ComplexScalarFactory.valueOf(double(real(toReport)), double(imag(toReport)));
    else
...
if isinteger(in)
    ...
    elseif isa(in, 'half')
        outVal = ComplexArrayFactory.valueOf(double(real(toReport)), double(imag(toReport)));
    else
