...
for i = 1:s(1)
    for j = 1:s(2)
        try
            if any(in{i,j} == ["i", "j", "s", "out"])
                % Avoid eval'ing any data which is the text of the local
                % variables.  The user is expecting 's' pasted into the Variable
                % Editor, not the size(in).  
                out{i, j} = in{i, j};
            else
                eval(['out{i, j} = ' in{i, j} ';']);
            end
...
if endRow == 0 || endColumn == 0 || localIsEmpty(value)
    if isstruct(value) || isjava(value) || istabular(value) || isstring(value) || ...
            isdatetime(value) || isduration(value) || iscategorical(value)
...
if nargin>=4
    ...
    if isa(varargin{2},'dataset') || istabular(varargin{2})
...
if localIsEmpty(toReport)
        ...
        if isreal(toReport)
            ...
            if isscalar(toReport)
                ...
                elseif isinteger(toReport)
                    ...
                    ca = ComplexScalarFactory.valueOf(real(double(toReport)), imag(double(toReport)));
                end
            else
                if isfloat(toReport)
                ...
                elseif isinteger(toReport)
                    ...
                    ca = ComplexArrayFactory.valueOf(real(double(toReport)), imag(double(toReport)));
                end
            end
        end
    ...
    elseif isa(toReport, 'optim.problemdef.OptimizationExpression') && ...
            isscalar(toReport) && ~isNonLinearExpression(toReport)
        % Treat scalar linear OptimiztionExpression objects like strings, and
        % create StringArray objects for their display.
        val = expand2str(toReport);
        isMissing = false;
        ca = com.mathworks.mlwidgets.array.data.StringArray.valueOf(...
            val, isMissing);
    elseif isa(toReport, 'optim.problemdef.OptimizationConstraint') && ...
            isscalar(toReport) && ~isNonLinearConstraint(toReport)
        % Treat scalar linear OptimiztionExpression objects like strings, and
        % create StringArray objects for their display.
        val = expand2str(toReport);
        isMissing = false;
...
function newString = localReplaceNullCharactersStr(s)
    newString = replace(s, char(0), ' ');

    % Use strlength to handle scalar string input to this function
    strLen = strlength(newString);
    longStrings = (strLen) > getTextTruncationLength;
    if any(longStrings)
        str = arrayfun(@(x) workspacefunc(...
            'getshortvalueobjectj', x, false, getTextTruncationLength), newString(longStrings), ...
            'UniformOutput', false);
        newString(longStrings) = str;
    end
...
% Calls to ValueDataSection should take Java-isms (i.e. zero-based row and
% column specifiers)
if isempty(toReport) && ~isstruct(toReport) && ~istabular(toReport) && ~isstring(toReport) ...
        && ~isdatetime(toReport) && ~isduration(toReport) && ~iscategorical(toReport)
    sz = size(toReport);
...
elseif isa(in,'int64')  && isproperty
    in = int64(in);
    clazz = 'int64';   
elseif isa(in, 'optim.problemdef.OptimizationExpression') && ...
        isNonLinearExpression(in)
    clazz = 'optim.problemdef.OptimizationExpression%NonLinear';
elseif isa(in, 'optim.problemdef.OptimizationConstraint') && ...
        isNonLinearConstraint(in)
    clazz = 'optim.problemdef.OptimizationConstraint%NonLinear';
elseif isa(in, 'InputOutputModel')
    % Append the super class for Java usage using a character that can't appear in class names
    clazz = [class(in) '%InputOutputModel'];
else
    clazz = class(in);
end
...
isNewStyleObject = false;
isSystemObject = false;
if isa(in,'dataset') || istabular(in)
...
if isobject(in) && ~isstring(in)
    ...
    if length(st) > 2
        try
            % two calls deep in the stack is arrayviewfunc
            callingClass = extractBefore(st(3).name, ".");
            props = getCurrentContextPropsAndPerms(in, callingClass);
        catch
            % There may be an error in getting the current context/permissions.
            % In this case, revert to prior behavior of showing these objects in
            % the object-view since we can't determine if they have no
            % properties.
            props = 1;
        end
    else
    ...
    out.setNumProperties(length(props));
end
...
if istall(in)
    % Calling isreal() on tall variables errors, and currently tall arrays
    % cannot contain sparse data, so set attributes = 1.
    attributes = 1;
elseif ~isreal(in)
    if issparse(in)
        attributes = 3;
...
isBuiltin = exist(class(object), 'builtin') == 5;
for i = 1:length(names)
    ...
    if isBuiltin
        % Builtins will not experience the performance regression because their
        % definition is not in a MATLAB file.  Fill in the fields from the
        % class.
        description{i} = thisMetaProp.Description;
        ddescription{i} = thisMetaProp.DetailedDescription;
    else
        % Don't access thisMetaProp.Description and DetailedDescription
        % It is causing a performance regression g2655428.  Fill them in
        % as empty, as if they weren't set at all.
        description{i} = '';
        ddescription{i} = '';
    end
end
...
if isempty(classnameBeingDebugged)
    ...
    if ~currentContextHasProtectedAccess
        debugClassMeta = meta.class.fromName(classnameBeingDebugged);
        if ~isempty(debugClassMeta)
            currentContextHasProtectedAccess = debugClassMeta < mc;
        end
    end
end
...
if metaProperty.Dependent || isempty(classnameBeingDebugged) || isempty(meta.class.fromName(classnameBeingDebugged))
    available = false;
    return
end
...
if isempty(classnameBeingDebugged) || isempty(meta.class.fromName(classnameBeingDebugged))
    available = false;
    return
end
...
if isa(propertyMetaInfo.GetAccess,'meta.class') || (iscell(propertyMetaInfo.GetAccess) && ...
        all(cellfun(@(x) isa(x, 'meta.class'), propertyMetaInfo.GetAccess)))
...
try
    ...
    % If there are any errors (for example, if classes were removed from the
    % path), assume private access.
    access = Access.PRIVATE;
end
...
elseif ischar(val) && isunix && strlength(strjoin(string(val))) > 4000
    % This is very targeted fix to addresss an issue where the java text area
    % doesn't show text after a certain length.  This call to workspacefunc will
    % truncate the text, and show a <truncated> message at the end.  Use strjoin
    % of the string value to handle multi-line char arrays.  It will be a no-op
    % for string arrays or char vectors.
    obj = workspacefunc('getshortvalueobjectj', val, true, 4000);
...
elseif numel(val) > maxElements || length(size(val)) > maxElements
    ...
    try
        ...
        % catch any exceptions in the display call.  This can happen if you have
        % errors in a class file for which you have a variabe of open in the
        % Variable Editor
        out = evalc('disp(char(cannotReferenceMessage))');
    end
end
...
if isa(in,'dataset')
    varNames = datasetProperties.VarNames;
else
    varNames = datasetProperties.VariableNames;
end
...
for varIndex = 1:size(in, 2)
    ...
    if (isnumeric(varData) || islogical(varData)) && (~isobject(varData) || isa(varData, 'half'))
    ...
    elseif localIsString(varData)
        ...
        varData(missing) = "<missing>";       
        varData = localReplaceNullCharactersStr(varData);
        ca = com.mathworks.mlwidgets.array.data.StringArray.valueOf(cellstr(varData), missing);
...
for col = 1:size(in, 2)
    ...
    val = in(:, col);
    val = localReplaceNullCharactersStr(val);
 ...
 elseif startRow==endRow && startRow==1 && startIndex==endIndex && endIndex==1 && isequal([1,1] , size(value))
    ...
    try
        ...
        if startRow == 1 && endRow == 1
            % If there is an error referencing value by index, just use it as is
            toReport = value;
        else
            rethrow(ex);
        end
...
if isa(dataArray,'dataset')
    startColumnIndexes = cumsum([1 datasetfun(@(x) getVariableSize(x,2)*ismatrix(x)*~ischar(x)*~isa(x,'dataset')*~istabular(x)...
        +ischar(x)+isa(x,'dataset')+istabular(x), ...
          tdataArray)]);
else
    startColumnIndexes = cumsum([1 varfun(@(x) getVariableSize(x,2)*ismatrix(x)*~ischar(x)*~isa(x,'dataset')*~istabular(x)...
        +ischar(x)+isa(x,'dataset')+istabular(x), ...
...
function b = isNonLinearExpression(var)
    b = getExprType(var) == optim.internal.problemdef.ImplType.Nonlinear;

function b = isNonLinearConstraint(var)
    exprType1 = getExprType(getExpr1(var));
    exprType2 = getExprType(getExpr2(var));
    b = (exprType1 > optim.internal.problemdef.ImplType.Linear) || ...
        (exprType2 > optim.internal.problemdef.ImplType.Linear);
