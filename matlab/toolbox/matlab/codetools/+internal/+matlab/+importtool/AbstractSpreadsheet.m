...
classdef AbstractSpreadsheet < handle & JavaVisible
    ...
    methods (Static, Access = public)
        function obj = createSpreadsheet
            ...
            if ~internal.matlab.importtool.AbstractSpreadsheet.readtableImport
                ...
                obj = internal.matlab.importtool.ReadtableSpreadsheet;
            end
        end
    ...
    methods (Static, Access = protected)
        ...
        function varName = generateVariableName(S)
            % Use a replacement style of hex, otherwise non-English sheet names
            % may resolve to the same MATLAB identifier.
            varName = matlab.lang.makeUniqueStrings(...
                matlab.lang.makeValidName(S, "ReplacementStyle", "hex"), {}, namelengthmax);
        end
    end
    ...
    methods (Static)
        ...
        function [numericReplaceArr, numericReplaceIdx, datetimeReplaceArr, ...
            ...
            % (g716691)
            function b = isValid(value)
                if ~isnumeric(value)
                    % Text that is "nan" is not unimportable -- it is valid
                    b = strcmpi(value, "nan"); 
                else
                    b = isnan(value);
                end
            ...
            numericReplaceIdx = isnan(numericData(:)) & ...
                cellfun(@(x) isempty(x) || ~isValid(x), raw(:));
            ...
            for col=1:length(outputVars)
                if columnTargetTypes{col}.isCellArrayColumn || columnTargetTypes{col}.isTextArray
                    if isstring(raw{1, col})
                        ...
                        if internal.matlab.importtool.server.ImportUtils.isStringTextType
        ...
        function [colTypes, outputType, rules] = getCodeGenInputsFromJava(...
                columnTargetTypes, allocationFcn, rules)
            % Get the codegen inputs from their java representations
            colTypes = internal.matlab.importtool.AbstractSpreadsheet.getVariableTypesFromJava(...
                columnTargetTypes, allocationFcn);
            outputType = internal.matlab.importtool.server.OutputTypeFactory.getOutputTypeFromJava(allocationFcn);
            
            if iscell(rules)
                rules = cellfun(@internal.matlab.importtool.server.rules.ImportRuleFactory.getImportRuleFromJava, ...
                    rules, 'UniformOutput', false);
                rules = [rules{:}];
            end
        end
        
        function varTypes = getVariableTypesFromJava(columnTargetTypes, allocationFcn)
            % Retrieve a set of column data types from the Java representations
            % for them.
            varTypes = strings(0);
            for j = 1:length(columnTargetTypes{1})
                c = columnTargetTypes{1}{j};
                for i = 1:length(c)
                    colType = c{i};
                    if isCategoricalArray(colType)
                        t = "categorical";

                    elseif isCellArrayColumn(colType)
                        if internal.matlab.importtool.server.ImportUtils.isStringTextType || ...
                                allocationFcn == "internal.matlab.importtool.AbstractSpreadsheet.stringAllocationFcn"
                            t = "string";
                        else
                            t = "char";
                        end

                    elseif isDatetimeArray(colType)
                        t = "datetime";
                        
                    elseif isTimeArray(colType)
                        t = "datetime";

                    elseif isNumericArray(colType)
                        t = "double";

                    elseif isTextArray(colType)
                        if internal.matlab.importtool.server.ImportUtils.isStringTextType || ...
                                allocationFcn == "internal.matlab.importtool.AbstractSpreadsheet.stringAllocationFcn"
                            t = "string";
                        else
