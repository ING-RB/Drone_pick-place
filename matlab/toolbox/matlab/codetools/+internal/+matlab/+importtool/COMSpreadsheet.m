...
classdef COMSpreadsheet < internal.matlab.importtool.AbstractSpreadsheet
    ...
    methods
        ...
        function obj = COMSpreadsheet(application)
            ...
            if ~internal.matlab.importtool.COMSpreadsheet.updateDefaultIgnoreRemoteRequests
                ...
                try
                    ...
                    % Ignore errors.  It's possible the IgnoreRemoteRequest
                    % property isn't available on some configurations.
                end

                try
                    application2.Quit
                catch
                    % Ignore errors coming from closing the COM client
                end
        ...
        function columnNames = getDefaultColumnNames(this,sheetname,row,avoidShadow)
            ...
            data = data(:)';
            columnNames = internal.matlab.importtool.server.ImportUtils.getDefaultColumnNames(evalin('caller','who'),data,-1,avoidShadow,true);
