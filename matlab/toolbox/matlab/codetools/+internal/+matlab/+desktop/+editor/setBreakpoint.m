...
%   'MATLAB:lineBeyondFileEnd' or 'MATLAB:lineBeyondPFileEnd' is encountered, 
%   the error will be silently ignored. If any other error occurs, a dialog 
%   with that message will be shown.
% 
...
%handleDbStopException handles exceptions that may be thrown when calling dbstop.
    ...
    switch exception.identifier
        ...
        case 'MATLAB:lineBeyondPFileEnd'
            % silently ignore this error.
        case 'MATLAB:lang:lineInOutOfSyncPFile'
            message = exception.message;
            status = DbStopStatus.PCODE_NOT_IN_SYNC;
        case 'MATLAB:class:NotOnPath'
            message = exception.message;
            status = DbStopStatus.CLASS_NOT_ON_PATH;
        case 'MATLAB:lang:BreakpointInAnonymousFunctionInScript'
            message = exception.message;
            status = DbStopStatus.ANONYMOUS_NOT_IN_A_FUNCTION;
        otherwise
            message = exception.message;
            status = DbStopStatus.OTHER_EXCEPTION;
    end
    
    % Remove any html tags for consistency since some forms of evaluation do not show them anyway
