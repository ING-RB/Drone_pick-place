...
%                         from the given dbstatus entry.
    ...
    for i=1:numBreakpoints
        ...
        if zeroBasedLineNumber < 0
            % Most likely a p-file breakpoint with no corresponding m-file
            % Ignore it since we can't display this in the editor anyway
            continue;
        end
        ...
        if MatlabBreakpoint.isLineBreakpoint(anonymousFunctionIndex)
            ...
            breakpoint = MatlabBreakpoint.createAnonymousConditional(zeroBasedLineNumber, anonymousFunctionIndex, expression, file);
        end
