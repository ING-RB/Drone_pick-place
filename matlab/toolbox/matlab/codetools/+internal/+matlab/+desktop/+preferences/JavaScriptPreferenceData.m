...
for i = 0:prefData.size()-1 
    prefMap(i+1).keys = jsonencode(cell(prefData.get(i).keySet().toArray()));
    prefMap(i+1).values = jsonencode(cell(prefData.get(i).values().toArray()));
end
preferenceData =  prefMap;
searchMap = struct('keys','','values','');
sData = com.mathworks.mde.desk.SearchPreferences.getSearchData();
searchMap.keys = jsonencode(cell(sData.keySet.toArray()));
searchMap.values = jsonencode(cell(sData.values.toArray()));
