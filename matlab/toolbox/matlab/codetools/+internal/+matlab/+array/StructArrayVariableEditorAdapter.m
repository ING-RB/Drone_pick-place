...
classdef StructArrayVariableEditorAdapter
    ...
    methods (Static=true)
        ...
        function scalarNumeric = variableEditorIsScalarNumericSelection(...
            ...
            % Limit the row/column intervals to the size of the struct
            % array.  Use the grid size since we're working with rows and
            % columns (which are fields)
            sz = internal.matlab.array.StructArrayVariableEditorAdapter.variableEditorGridSize(a);
            rowInterval(rowInterval > sz(1)) = [];
            colInterval(colInterval > sz(2)) = [];
        ...
        function [names, indices, classes, cellstrflag, charwidths] = variableEditorColumnNames(a)
            ...
            for col=1:length(names)
                if strcmp(classes{col}, 'char')
                    cellstrflag(col) = iscellstr(varCell(:, col)); %#ok<ISCLSTR>
                end
            end
        ...
        function variableEditorPaste(a, workspaceVariableName, ...
            ...
            baseVariableName = matlab.internal.datatoolsservices.getBaseVariableName( ...
        ...
        function [code,msg] = variableEditorSetDataCode(a,workspaceVariableName,row,col,rhs)
            ...
            if col <= numFields && ...
                ...
                if isstring(rhs)
                    ...
                    tabChar = char(9);
                    rhs = ['"' char(replace(replace(rhs, newline, '" + newline + "'), ...
                        tabChar, '" + char(9) + "')) '"'];
                else
                    idxs = strfind(rhs, '"');
        ...
        function [code,msg] = variableEditorSortCode(this, varName, ...
                structFields, direction, missingPlacement)
            arguments
                this
                varName
                structFields
            ...
            if all(allNumbers | allLogical | allStrings) && ~any(emptyVals)
                ...
                if direction
                    code = ['[~,' indexName '] = sortrows([' varToSort '].''' missingPlacementSyntax '); ' varName ' = ' varName '(' indexName '); clear ' indexName ];
                else
                    code = ['[~,' indexName '] = sortrows([' varToSort '].'', "descend"' missingPlacementSyntax '); ' varName ' = ' varName '(' indexName '); clear ' indexName ];
                end
        ...
        function [code, openvarCode, msg] = variableEditorCreateNewStructCode(...
                this, varName, rowIntervals, columnIntervals, whosOutput)
            % Generates code to create a new scalar struct or struct
            % vector, with the specified rows/fields from the original
            % struct vector. If whosOutput is provided, use that rather
            % than evaling in caller.
            fieldNames = fieldnames(this);
            if (nargin < 5)
                whoOutput = evalin('caller','whos');
            else
                whoOutput = whosOutput;
            end
        ...
        function [code, openvarCode, msg] = variableEditorCreateNewNumericArrayCode(this, varName, rowIntervals, columnIntervals, whosOutput)
            % If whosOutput is provided, use that rather than evaling in caller.
            if (nargin < 5)
                whoOutput = evalin('caller','whos');
            else
                whoOutput = whosOutput;
        ...
        function [code, openvarCode, msg] = variableEditorCreateNewCellArrayCode(this, varName, rowIntervals, columnIntervals, whosOutput)
            % If whosOutput is provided, use that rather than evaling in caller.
            if (nargin < 5)
                whoOutput = evalin('caller','whos');
            else
        ...
        function [code, openvarCode, msg] = variableEditorCreateSeparateVariablesCode(this, varName, rowIntervals, columnIntervals, whosOutput)
            [numColIntervals, ~] = size(columnIntervals);
            first = 1;
            maxOpenVars = 5;
            numOfOpenVars = 1;
        ...
        function [code, openvarCode, msg] = variableEditorCreateNewDatasetArrayCode(this, varName, rowIntervals, columnIntervals, whosOutput)
            fieldNames = fieldnames(this);
        ...
        function [code, openvarCode, msg] = variableEditorCreateNewTableCode(this, varName, rowIntervals, columnIntervals, whosOutput)
            fieldNames = fieldnames(this);
            % If whosOutput is provided, use that rather than evaling in caller.
            if (nargin < 5)
                whoOutput = evalin('caller','whos');
...
function assignVariableInWorkspace(theWorkspace, workspaceVariableName, currentValue) %#ok<INUSD>
    baseVariableName = matlab.internal.datatoolsservices.getBaseVariableName(workspaceVariableName);
