...
%UIIMPORT Opens a dialog to import data
%
%   UIIMPORT Opens a dialog to interactively load data from a file
%   or the clipboard. MATLAB displays a preview of the data in the file.
%
%   UIIMPORT(FILENAME) opens the file specified in FILENAME using either
%   the Import Tool or the Import Wizard depending on the file type. For
%   spreadsheet and text files uiimport opens the file using the Import
%   Tool. For all other file types (image, audio, MAT file, etc..), the
%   uiimport function initiates the Import Wizard.
%
...
%   presented first.
%
%   S = UIIMPORT(...) opens the file using the Import Wizard, with
%   resulting variables stored as fields in the struct S.
%
%   See also LOAD, CLIPBOARD, READTABLE, IMPORTDATA.
...
if internal.matlab.importtool.peer.shouldShowJSImportTool || ...
        ~(matlab.internal.feature('webui') == 0 && Capability.isSupported(Capability.LocalClient))
    if isSynchronous
        varargout{1} = uiimportNoJVM(varargin{:});
    else
        uiimportNoJVM(varargin{:});
    end
    return;
end
...
optionArg = (nargin > 0) && any(varargin{1} == ["-file", "-pastespecial"]);
iWizardOpen = ~isempty(internal.matlab.importtool.ImportableFileIdentifier.getSetImportWizardInstance);
drawnow;
if iWizardOpen && ...
        ((nargin == 0) || optionArg || ... % assume Import Wizard usage when no arguments are specified or -file/-pastespecial specified
        (~isempty(varargin{1}) && ... % file is specified and it uses the Import Wizard
         (~optionArg && internal.matlab.importtool.ImportableFileIdentifier.usesImportWizard(localGetAbsolutePath(varargin{1})))))
    if ~isSynchronous
        ...
        wizInstance = internal.matlab.importtool.ImportableFileIdentifier.getSetImportWizardInstance;
        awtinvoke(wizInstance, 'setVisible', true);
        wizInstance.toFront;
        if nargin ~= 0
...
% Use the new Import Tool to open pasted data
if useClipboard && ~isSynchronous
    com.mathworks.mlwidgets.importtool.TextImportClient.open()
    return
end
...
if useFileDialog
    ...
    [fileExtDesc, fileExtList] = internal.matlab.importtool.ImportableFileExtension.getImportToolFileChooserDropDownInfo;
    ...
    if com.mathworks.mlwidgets.workspace.ImportFileChooser.getState() == javax.swing.JFileChooser.APPROVE_OPTION
        fileSelected = com.mathworks.mlwidgets.workspace.ImportFileChooser.getSelectedFile;
    end
    if (isempty(fileSelected))
        ...
        if ctorFile.length == 0
            showEmptyFileError();
            return;
        end
    end
    type = finfo(fileAbsolutePath);
    if useSpreadsheetImportTool(isSynchronous,type)
    ...
    ctorFile = java.io.File(fileAbsolutePath);
    if ctorFile.length>1e6 && useTextImportTool(isSynchronous, fileAbsolutePath, type) && ...
            ~internal.matlab.importtool.peer.shouldShowJSImportTool
        waitFrame = showLargeFileWindow;
    end
elseif ~useClipboard   
    path = convertPathToChar(varargin{1});
    fileAbsolutePath = localGetAbsolutePath(path);   
    ctorFile = java.io.File(fileAbsolutePath);
    
    % Detect and report empty input files - display an error dialog
    if ctorFile.exists && ctorFile.length == 0
        showEmptyFileError();
        return;
    end
...
if ~useClipboard && type == "fig"
    % Load the fig file directly using openfig
    eval("openfig('" + fileAbsolutePath + "');")
    return;
end

% ctorHeaderLines>=0 is the test used in com.mathworks.mde.dataimport.ImportWizardContents
% to detect text files. If this condition is met and the Text Import Tool
% preference is set, then use the Text Import Tool
if ~useClipboard && useTextImportTool(isSynchronous, fileAbsolutePath, type)
    com.mathworks.mlwidgets.importtool.TextImportClient.open(fileAbsolutePath, waitFrame)
    
    % TODO: Add synchronous deprecation warning
%    hideLargeFileWindow(waitFrame);
    return
end
...
if ( ~isempty(ad.datastruct) && ad.isSpreadsheet )
    ...
    % Use a ReplacementStyle of hex here, to match the results of
    % importdata.  The importdata function still uses genvarname,
    % and we need to match the sheet names that importdata returns.
    % The hex replacement style mimics what genvarname does with
    % invalid characters.
    internalSheetNames = iGenVarName(sheetNameCells{1}, 'ReplacementStyle', 'hex');
...
if ~isSynchronous
    internal.matlab.importtool.ImportableFileIdentifier.getSetImportWizardInstance(wiz);
end
javaMethodEDT('setName', wiz, 'ImportWizard');
scaledWidth = javaMethodEDT('scaleSize', 'com.mathworks.util.ResolutionUtils', 702);
scaledHeight= javaMethodEDT('scaleSize', 'com.mathworks.util.ResolutionUtils', 435);
javaMethodEDT('setSize', wiz, scaledWidth, scaledHeight);
...
% the caller's workspace.
    ...
    function filePreview(~, b)
        ...
        [description, headerLines, textDelimiter] = ...
            gatherFilePreviewData(fileAbsolutePath);
    ...
    function [description, headerLines, textDelimiter] = gatherFilePreviewData(fileAbsolutePath)
        type = '';
        loadcmd = '';
        ...
        ad.loadcmd = loadcmd;
        ad.isSpreadsheet = any(strcmpi(type, internal.matlab.importtool.ImportableFileExtension.getSpreadsheetFileExtensions(false)));
    ...
    function [variables, sizes, byteses, classes, colHeaders, rowHeaders] = ...
        ...
        if ~isempty(variables)
            ...
            for i = 1:length(variables)
                if (checkIsField(ad.datastruct.(variables{i}), sheetName))
        ...
        if isempty(realValue)
            if numel(size(value)) == 2 && isnumeric(value)
                ...
                if isempty(realValue)
                    ...
                    realValue = strrep(strrep(realValue, '<strong>', ''), '</strong>', '');
    ...
    function finish(unused, b)
        ...
        if isSynchronous
            ...
            internal.matlab.importtool.ImportableFileIdentifier.getSetImportWizardInstance([]);
            if ~isempty(newData)
...
out = strrep(out, char([13 10]), newline);
out = strrep(out, char(13), newline);
end
...
function state = useSpreadsheetImportTool(isSynchronous, type)
    state = internal.matlab.importtool.ImportableFileIdentifier.useSpreadsheetImportTool(...
        isSynchronous, type);
end

function state = useTextImportTool(isSynchronous, fileAbsolutePath, finfoType)
    import matlab.internal.capability.Capability;

    state = internal.matlab.importtool.ImportableFileIdentifier.useTextImportTool(...
        isSynchronous, fileAbsolutePath, finfoType);
end
...
medium = getMedium(in);
switch (medium)
    case com.mathworks.mde.dataimport.MultimediaEvent.MEDIUM_IMAGE
        imageviewHelper(file);
    case com.mathworks.mde.dataimport.MultimediaEvent.MEDIUM_MOVIE
        movieviewHelper(file);
    case com.mathworks.mde.dataimport.MultimediaEvent.MEDIUM_SOUND
        soundviewHelper(file);
end
end
...
function varName = iGenVarName(S, varargin)
    % Calls makeUniqueStrings on the results of makeValidName for the
    % input cell array S.  varargin is passed onto makeValidName.
    varName = matlab.lang.makeUniqueStrings(...
        matlab.lang.makeValidName(S, varargin{:}), {}, namelengthmax);
end
...
function fileAbsolutePath = localGetAbsolutePath(fileAbsolutePath)
    ...
    % We have been given a file.  Absolute, or in the current
    ...
    fid = fopen(fileAbsolutePath, 'r');
    if fid > 0
        fileAbsolutePath = fopen(fid);  % Get the full pathname if not in pwd.
        fclose(fid);
    end
end

function waitFrame = showLargeFileWindow()
    % This is a Java frame
...
function f = checkIsField(st, field)
    f = false;
    try
        f = isfield(st, field);
    catch
...
function path= convertPathToChar(path)
    if isstring(path) && isscalar(path)
        path = char(path);
    end     
end

function showEmptyFileError()
    javaMethodEDT('showMessageDialog', ...
        'com.mathworks.mwswing.MJOptionPane', [], ...
        getString(message('MATLAB:codetools:uiimport:CannotImportFromEmptyFile')), ...
        getString(message('MATLAB:codetools:uiimport:ImportWizard')), 0);
end
