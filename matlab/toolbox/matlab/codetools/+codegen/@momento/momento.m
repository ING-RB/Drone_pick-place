...
if isa(h, 'matlab.graphics.Graphics') ... % HG2
        || isa(h, 'codegen.CodeGenAble') % CodeGenAble MCOS object
...
elseif isa(hObj, 'codegen.CodeGenAble')
    % Filter out default values for MCOS CodeGenAble objects
    IgnorePropTests = @(h, prop) prop.HasDefault && isequal(get(h, prop.Name), prop.DefaultValue) || IgnorePropTests(h, prop);
...
if isa(hObj, 'handle.handle')
    ...
    IgnorePropTests = @(h, prop) (prop.Hidden ...
        || ~any(strcmp(prop.SetAccess,'public')) ...
        || ~any(strcmp(prop.GetAccess,'public')) ...
        || IgnorePropTests(h, prop));
end
...
if ~ishghandle(hObj,'hggroup')
    ...
    catch anError % We don't need to use the error object.  Move on.
    end
end
...
if any(strcmp(prop_name, {'ContextMenu', 'CurrentObject', 'CurrentAxes', ...
