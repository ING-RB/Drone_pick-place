...
%
%  Use SAVE and/or SAVEFIG for full object serialization.
%
%  Limitations
...
for n = 1:length(mcode_str)
    ...
    str = [str,mcode_str{n},newline]; %#ok<AGROW>
end

if options.MLint
    messageArray =  checkcode('-text',str,'.m');
    % Convert the messages to text:
    msgStr = '';
    for i=1:length(messageArray)
        currMessage = messageArray(i);
        msgText = sprintf('%d %s',currMessage.line,currMessage.message);
        if i~=1
            % Generating code in a loop.  There is no way to effectively
            % preallocate memory for this.
            msgStr = [msgStr newline msgText]; %#ok<AGROW>
        else
            msgStr = msgText;
        end
    end
...
elseif strcmp(option.Output,'-editor')
    ...
    if ~matlab.desktop.editor.isEditorAvailable
        local_display_mcode(mcode_str,struct('Output','-cmdwindow'));
    end
    ...
    if nargout('matlab.desktop.editor.newDocument') > 0
        ...
        % In MOL matlab.desktop.editor.newDocument does not return
        % an output
        matlab.desktop.editor.newDocument(mcode_str);
    end
...
bool = any(handle(hInput) == handle(hIgnore));
...
retval = true;
if isa(h, 'matlab.graphics.Graphics')
    if is_codegen_for_uiFig(h)
      retval = false;
    end
    return;
end
if ishandle(h)
    if isa(h, 'codegen.CodeGenAble')
        % CodeGenAble MCOS object
        cls = metaclass(h);
        if ~isempty(cls.ContainingPackage)
            package_name = cls.ContainingPackage.Name;
        else
            package_name = '';
        end
    else
...
function retval = is_codegen_for_uiFig(h)
% Not allowing code gen for uifigures
f  =ancestor(h,'figure');
retval = isa(f,'matlab.ui.Figure') && matlab.ui.internal.FigureServices.isUIFigure(f);
