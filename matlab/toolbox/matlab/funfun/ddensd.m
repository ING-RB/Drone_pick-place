function sol = ddensd(ddefun,dely,delyp,history,tspan,options,varargin)
...
import matlab.ode.internal.dde.*;
solver_name = 'ddensd';
...
% Sizes and indices
D = ydel(a,ya,varargin{:});
Dp = ypdel(a,ya,varargin{:});
nydel = numel(D);
nypdel = numel(Dp);
...
output_ty = isstruct(options) && ...
    isfield(options,"OutputTY") && options.OutputTY;
...
if ~isempty(events)
        Nevents_fun = @(t,y,Z,varargin)Nevents(t,y,Z, ...
            yIdx,ypIdx,ypdIdx,ydel,events,IVP,IVP_T0,IVP_YP0,ypdel,DELTA,MINCHANGE,varargin{:});
    options = ddeset(options,'Events',Nevents_fun);
end

% Adjust output type if it changed
options.OutputTY = output_ty;

ws = warning('off','MATLAB:deval:NonuniqueSolution');
restoreWarningNonuniqueSolution = onCleanup(@() warning(ws));

try
    Ndde_fun = @(t,y,Z,varargin)Ndde(t,y,Z, ...
        ydel,yIdx,ypIdx,ypdIdx,ddefun,IVP,IVP_T0,IVP_YP0,ypdel,DELTA,MINCHANGE,varargin{:});
    Ndelays_fun = @(t,y,varargin)Ndelays(t,y, ...
        ydel,ypdel,IVP,IVP_T0,DELTA,MINCHANGE,varargin{:});
    sol = ddesd(Ndde_fun,Ndelays_fun,Nhistory,tspan,options,varargin{:});
catch e
...
Zpd = Z(:,ypdIdx);
Z = Z(:,yIdx);
ZP = matlab.ode.internal.dde.Nypdel(t,y,Zp,Zpd,ydel,ypIdx,ypdIdx,IVP,IVP_T0,...
    IVP_YP0,ypdel,DELTA,MINCHANGE,varargin{:});  % approx delayed derivative
[value,isterminal,direction] = events(t,y,Z,ZP,varargin{:});
end % Nevents
