...
[X,Y] = interpGuess(solinit,ismbvp,nregions,c,nparam,neqn);
if ismbvp
    MBVP = updateMBVP(X,nregions,nstages);
end
...
while ~done
    ...
    for iter = 1 : maxNewtIter
        ...
        if ~refinedMesh
            [F,nODEeval] = odeFcn(X,Y,nregions,MBVP,ismbvp,xyVectorized,ode,nODEeval);
        end
        refinedMesh = false;
        [RHS,BCval,nBCeval] = colloc_RHS(X,Y,F,nregions,ismbvp,MBVP,bc,nstages,neqn,A,nBCeval);

        if needGlobalJacobian
            % setup and factor the global Jacobian
            [collocJac,Joptions,dBCoptions,nBCeval,nODEeval] = colloc_Jac(X,Y,F,BCval, ...
                bcjac,bc,jac,ode,neqn,nregions,ismbvp,MBVP,nstages,bigI,bigA, ...
                c,Joptions,dBCoptions,nBCeval,nODEeval);
            dPHIdy = decomposition(collocJac,'CheckCondition',false);
            rc = rcond(dPHIdy);
            if rc == 0  || isnan(rc)
        ...
        for probe = 1 : maxProbes
            ...
            [Fnew,nODEeval] = odeFcn(X,Ynew,nregions, ...
                MBVP,ismbvp,xyVectorized,ode,nODEeval);
            [RHSnew,~,nBCeval] = colloc_RHS(X,Ynew,Fnew,nregions, ...
                ismbvp,MBVP,bc,nstages,neqn,A,nBCeval);
        ...
        if distYnew < 0.1*rtol
            maxInterpResidual = colloc_maxresidual(X,Ynew,Fnew,ismbvp, ...
                MBVP,nregions,nstages,threshold,B,sq5);
    ...
    ymid = interpolateYmid(X,Y,F,ismbvp,MBVP,neqn,nregions,nstages,sq5);

    if solutionVerificationStage
        % Use three error samples per mesh interval.
        nsamples = 3;
        [res,nODEeval] = residualEstimate(X,Y,ymid,F,nsamples,nregions, ...
            MBVP,nstages,threshold,ismbvp,xyVectorized,ode,nODEeval);
    else
        ...
        if max(res) < rtol
            ...
            nsamples = 2;
            [res2,nODEeval] = residualEstimate(X,Y,ymid,F,nsamples,nregions, ...
                MBVP,nstages,threshold,ismbvp,xyVectorized,ode,nODEeval);
            res = max(res,res2);
        end
    end
    ...
    if solutionVerificationStage && ( max(res) <= rtol)
        ...
        [XX,YY,FF,meshHistory,nODEeval] = newSolutionProfile(X,Y,ymid,F, ...
            res,errorReductionGuard,ismbvp,MBVP,nstages,rtol,c,neqn, ...
            nregions,xyVectorized,ode,meshHistory,nODEeval);
        if numel(XX) > Nmax
...
sol = outputSol(X,Y,F,ymid,solver_name,ismbvp,MBVP,nstages,neqn,nparam,nregions);

if minRC < eps("like",minRC)
    warning(message('MATLAB:bvp5c:IllCondJac',sprintf('%g',minRC)));
end
...
% Local functions
%---------------------------------------------------------------------------
function [X,Y] = interpGuess(sol,ismbvp,nregions,c,nparam,neqn)
...
if ismbvp
    ...
    for region = 1 : nregions
        ...
        xreg = sol.x(xidx);
        yreg = sol.y(:,xidx);
        [Xreg,Yreg] = interpGuess_region(sol,xreg,yreg,{region},c,nparam,neqn);
        X = [X, Xreg]; %#ok<AGROW>
        Y = [Y, Yreg]; %#ok<AGROW>
    end
else
    [X,Y] = interpGuess_region(sol,sol.x,sol.y,{},c,nparam,neqn);
end
end  % interpGuess

%---------------------------------------------------------------------------
function [X,Y] = interpGuess_region(sol,sol_xreg,sol_yreg,region,c,nparam,neqn)
% In a region, evaluate/interpolate the initial guess at collocation points.
h = diff(sol_xreg);
x0  = sol_xreg(1:end-1);  % left subinterval boundaries
xc1 = x0 + c(1)*h;
xc2 = x0 + c(2)*h;
X = [x0; xc1; xc2];
X = [X(:); sol_xreg(end)];
...
%---------------------------------------------------------------------------
function [F,nODEeval] = odeFcn(X,Y,nregions,MBVP,ismbvp,xyVectorized,ode,nODEeval)
...
if ismbvp
    ...
    for region = 1 : nregions
        ...
        xreg = X(xidx);
        yreg = Y(:,xidx);
        [F(:,xidx), nODEeval] = odeFcn_region(xreg,yreg,{region}, ...
            ismbvp,xyVectorized,ode,nODEeval);
    end
else
    [F,nODEeval] = odeFcn_region(X,Y,{},ismbvp,xyVectorized,ode,nODEeval);
end
end  % odeFcn

%---------------------------------------------------------------------------
function [F,nODEeval] = odeFcn_region(X,Y,region,ismbvp,xyVectorized,ode,nODEeval)
...
%---------------------------------------------------------------------------
function [Jn,Jnc1,Jnc2,Jnp1,Joptions,nODEeval] = odeJac_region(x,y,f, ...
    Jpropagated,region,jac,ode,c,Joptions,nODEeval)
...
%---------------------------------------------------------------------------
function res = bcaux(Ya,Yb,neqn,bc)
...
%---------------------------------------------------------------------------
function [dBCdya,dBCdyb,dBCoptions,nBCeval] = bcJac(ya,yb,bcVal, ...
    bcjac,neqn,bc,dBCoptions,nBCeval)
...
if isempty(bcjac)
    ...
    [dBCdya,dBCoptions.fac_dya,~,nF] = ...
        odenumjac(@bcaux,{ya(:),yb(:),neqn,bc},bcVal,dBCoptions);
    nBCeval = nBCeval + nF;
...
%---------------------------------------------------------------------------
function [RHS,RHSbc,nBCeval]= colloc_RHS(X,Y,F,nregions,ismbvp,MBVP, ...
    bc,nstages,neqn,A,nBCeval)
...
[xreg,yreg,freg] = getRegionData(1,X,Y,F,ismbvp,MBVP);
RHSode = colloc_RHS_region(xreg,yreg,freg,nstages,neqn,A);

for region = 2 : nregions  % MBVP
    [xreg,yreg,freg] = getRegionData(region,X,Y,F,ismbvp,MBVP);
    RHSreg = colloc_RHS_region(xreg,yreg,freg,nstages,neqn,A);
    RHSode = [RHSode; RHSreg]; %#ok<AGROW>
end
...
%---------------------------------------------------------------------------
function RHSode = colloc_RHS_region(X,Y,F,nstages,neqn,A)
...
%---------------------------------------------------------------------------
function [Jac,Joptions,dBCoptions,nBCeval,nODEeval] = colloc_Jac(X,Y,F,bcVal, ...
    bcjac,bc,jac,ode,neqn,nregions,ismbvp,MBVP,nstages,bigI,bigA,c, ...
    Joptions,dBCoptions,nBCeval,nODEeval)
...
if ismbvp
    ya = Y(:,MBVP.LBCidx);
    yb = Y(:,MBVP.RBCidx);
    [JACbcL,JACbcR,dBCoptions,nBCeval] = bcJac(ya,yb,bcVal,bcjac,neqn,bc, ...
        dBCoptions,nBCeval);
...
if ismbvp
    ...
    for region = 1 : nregions
        ...
        [xreg,yreg,freg] = getRegionData(region,X,Y,F,ismbvp,MBVP);
        [JACregI,JACregJ,JACregV,Joptions,nODEeval] = colloc_JacODE_region(xreg,yreg,freg, ...
            {region},nstages,neqn,bigI,bigA,jac,ode,c,Joptions,nODEeval);
    ...
    [JACodeI,JACodeJ,JACodeV,Joptions,nODEeval] = colloc_JacODE_region(X,Y,F, ...
        {},nstages,neqn,bigI,bigA,jac,ode,c,Joptions,nODEeval);
...
%---------------------------------------------------------------------------
function [JacI,JacJ,JacV,Joptions,nODEeval] = colloc_JacODE_region(X,Y,F,region,nstages, ...
    neqn,bigI,bigA,jac,ode,c,Joptions,nODEeval)
...
for i = 1 : numel(h)
    ...
    % Evaluate Jacobians
    [Jn,Jnc1,Jnc2,Jnp1,Joptions,nODEeval] = odeJac_region(x,y,f,Jnp1, ...
        region,jac,ode,c,Joptions,nODEeval);
...
%---------------------------------------------------------------------------
function maxInterpResidual = colloc_maxresidual(X,Y,F,ismbvp,MBVP, ...
    nregions,nstages,threshold,B,sq5)
%COLLOC_MAXRESIDUAL  Compute max residual in the collocation equations.

[xreg,yreg,freg] = getRegionData(1,X,Y,F,ismbvp,MBVP);
maxInterpResidual = colloc_maxresidual_region(xreg,yreg,freg,nstages, ...
    threshold,B,sq5);

for region = 2:nregions  % MBVP
    [xreg,yreg,freg] = getRegionData(region,X,Y,F,ismbvp,MBVP);
    regionInterpResidual = colloc_maxresidual_region(xreg,yreg,freg, ...
        nstages,threshold,B,sq5);
    maxInterpResidual = max(maxInterpResidual,regionInterpResidual);
end
end  % colloc_maxresidual

%---------------------------------------------------------------------------
function maxInterpResidual = colloc_maxresidual_region(X,Y,F,nstages, ...
    threshold,B,sq5)
...
%---------------------------------------------------------------------------
function ymid = interpolateYmid(X,Y,F,ismbvp,MBVP,neqn,nregions,nstages,sq5)
%INTERPOLATE_YMID  Interpolate Y at the midpoints of mesh subintervals.

[xreg,yreg,freg] = getRegionData(1,X,Y,F,ismbvp,MBVP);
ymid = interpolateYmid_region(xreg,yreg,freg,nstages,sq5);

for region = 2 : nregions  % MBVP
    [xreg,yreg,freg] = getRegionData(region,X,Y,F,ismbvp,MBVP);
    ymidreg = interpolateYmid_region(xreg,yreg,freg,nstages,sq5);
    ymid = [ymid,zeros(neqn,1),ymidreg]; %#ok<AGROW> % pad with zeros
end
end  % interpolateYmid

%---------------------------------------------------------------------------
function ymid = interpolateYmid_region(X,Y,F,nstages,sq5)
...
ymid = y(:,1:end-1) + ...
    ...
    (40-15*sq5)/192*K3 - 1/192*K1(:,2:end)) .* h;
end  % interpolateYmid_region

%---------------------------------------------------------------------------
function [res,nODEeval] = residualEstimate(X,Y,Ymid,F,nsamples,nregions, ...
    MBVP,nstages,threshold,ismbvp,xyVectorized,ode,nODEeval)
%RESIDUAL_ESTIMATE  Estimate the residual in each mesh subinterval.

[xreg,yreg,freg,ymidreg] = getRegionData(1,X,Y,F,ismbvp,MBVP,Ymid);
[res,nODEeval] = residualEstimate_region(xreg,yreg,ymidreg,freg,nsamples,{1}, ...
    nstages,threshold,ismbvp,xyVectorized,ode,nODEeval);

for region = 2:nregions  % MBVP
    [xreg,yreg,freg,ymidreg] = getRegionData(region,X,Y,F,ismbvp,MBVP,Ymid);
    [res_reg,nODEeval] = residualEstimate_region(xreg,yreg,ymidreg,freg,nsamples,{region}, ...
        nstages,threshold,ismbvp,xyVectorized,ode,nODEeval);
    res = [res,0,res_reg];  %#ok<AGROW> % pad with 0
end
end  % residualEstimate

%---------------------------------------------------------------------------
function [res,nODEeval] = residualEstimate_region(X,Y,ymid,F,nsamples,region, ...
    nstages,threshold,ismbvp,xyVectorized,ode,nODEeval)
...
for i = 1 : numel(h)
    ...
    [yres,ypres] = ntrp4h(xres,x(i),y(:,i),x(i+1),y(:,i+1),...
        ymid(:,i),yp(:,i),yp(:,i+1));
    [F, nODEeval] = odeFcn_region(xres,yres,region,ismbvp,xyVectorized,ode,nODEeval);
    residual = ypres - F;
...
%---------------------------------------------------------------------------
function [XX,YY,FF,meshHistory,nODEeval] = newSolutionProfile(X,Y,Ymid,F,errEst, ...
    errorReductionGuard,ismbvp,MBVP,nstages,rtol,c,neqn,nregions,xyVectorized, ...
    ode,meshHistory,nODEeval)
...
[xreg,yreg,freg,ymidreg,errEstReg] = getRegionData(1,X,Y,F, ...
    ismbvp,MBVP,Ymid,errEst);
[XX,YY,FF,nODEeval] = newSolutionProfile_region(xreg,yreg,ymidreg,freg, ...
    errEstReg,{1},canRemovePoints,nstages,rtol,c,neqn,ismbvp, ...
    xyVectorized,ode,nODEeval);

for region = 2 : nregions  % MBVP
    [xreg,yreg,freg,ymidreg,errEstReg] = getRegionData(region,X,Y,F, ...
        ismbvp,MBVP,Ymid,errEst);
    [xxreg,yyreg,ffreg,nODEeval] = newSolutionProfile_region(xreg, ...
        yreg,ymidreg,freg,errEstReg,{region},canRemovePoints, ...
        nstages,rtol,c,neqn,ismbvp,xyVectorized,ode,nODEeval);
    XX = [XX,xxreg]; %#ok<AGROW>
    YY = [YY,yyreg]; %#ok<AGROW>
    FF = [FF,ffreg]; %#ok<AGROW>
end
end  % newSolutionProfile

%---------------------------------------------------------------------------
function [XX,YY,FF,nODEeval] = newSolutionProfile_region(X,Y,ymid,F,errEst,region,...
    canRemovePoints,nstages,rtol,c,neqn,ismbvp,xyVectorized,ode,nODEeval)
    ...
    if errEst(i) <= rtol
        ...
        if canRemovePoints && ...
            ...
            if predEst < 0.5 * rtol  % replace 3 intervals with 2
                ...
                [ff, nODEeval] = odeFcn_region(xx,yy,region,ismbvp,xyVectorized,ode,nODEeval);
...
%---------------------------------------------------------------------------
function sol = outputSol(X,Y,F,ymid,solver_name,ismbvp,MBVP,nstages,neqn,nparam,nregions)
...
[xreg,yreg,freg,ymidreg] = getRegionData(1,x,y,F,ismbvp,MBVP,ymid);
[xout,yout,ypout,ymidout] = outputData(xreg,yreg,freg,ymidreg,nstages,neqn);

for region = 2:nregions   % MBVP
    [xreg,yreg,freg,ymidreg] = getRegionData(region,x,y,F,ismbvp,MBVP,ymid);
    [xoutreg,youtreg,ypoutreg,ymidoutreg] = outputData(xreg,yreg,freg,ymidreg,nstages,neqn);
    xout = [xout, xoutreg];    %#ok<AGROW>
    yout = [yout, youtreg];    %#ok<AGROW>
    ypout = [ypout, ypoutreg]; %#ok<AGROW>
    % add zeros for internal interface intervals
    ymidout = [ymidout, zeros(neqn,1), ymidoutreg]; %#ok<AGROW>
end
...
%---------------------------------------------------------------------------
function [xout,yout,ypout,ymidout] = outputData(x,y,f,ymid,nstages,neqn)
...
%---------------------------------------------------------------------------
function MBVP = updateMBVP(X,nregions,nstages)
...
%---------------------------------------------------------------------------
function [xreg,yreg,freg,ymidreg,errest] = getRegionData(region,X,Y,F,ismbvp,MBVP,Ymid,ErrEst)
