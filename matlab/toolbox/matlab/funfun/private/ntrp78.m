function [yinterp,ypinterp] = ntrp78(tinterp,t,y,~,~,h,f,idxNonNegative)
%NTRP78  Interpolation helper function for ODE78.
%   YINTERP = NTRP78(TINTERP,T,Y,TNEW,YNEW,H,F,IDX) uses data computed in
%   ODE78 to approximate the solution at time TINTERP. TINTERP may be a
...
%   ignored.
%
%   [YINTERP,YPINTERP] = NTRP78(TINTERP,T,Y,TNEW,YNEW,H,F,IDX) returns also
...
%
%   The input f is the matrix of stage values with stages 2, 3, 4, 5, and
%   13 omitted.
%
...
for k = 1:nt
    yinterp(:,k) = y + h*yinterpWSS((tinterp(k) - t)/h,f, ...
...
function sp = yinterpWSS(theta,f,BI2,BI3,BI4,BI5,BI6,BI7)
...
function sp = ypinterpWSS(theta,f,BI2,BI3,BI4,BI5,BI6,BI7)
