...
[Mtype, M] =  odemass(odeIsFuncHandle,ode,t0,y0,options,datatype,varargin);
if Mtype > 0
    if Mtype > 1
        ...
        elseif Msingular == "yes"
            error(message('MATLAB:ode23s:MassSingularYes'));
        end
    end
end

[datatype,~] = matlab.ode.internal.updateDatatype(datatype,M,solver_name,"mass matrix");
...
[datatype,~] = matlab.ode.internal.updateDatatype(datatype,dfdy,solver_name,"Jacobian");
...
if isempty(htry)
    ...
    if normcontrol
        if Mtype > 0
            rh = 1.25*sqrt(0.5 * (norm(odesolve(Factors, piv, DfDt)) / wt)) / rtol^pow;
        else
            rh = 1.25 * sqrt(0.5 * (norm(DfDt) / wt)) / rtol^pow;
        end
    else
        if Mtype > 0
            rh = 1.25*sqrt(0.5*norm(odesolve(Factors, piv, DfDt) ./ wt,inf)) / rtol^pow;
        else
...
datatype = superiorfloat(zeros(datatype),dfdy);
...
if nargout > 0
    if output_sol
        ...
        k1data = zeros(neq,chunk,"like",prototype);
        k2data = zeros(neq,chunk,"like",prototype);
    else
...
while ~done
    ...
    while true                            % Evaluate the formula.
        ...
        f1 = ode(t + 0.5*h, y + 0.5*h*k1, odeArgs{:});
        Mk1 = M * k1;
        k2 = odesolve(Factors, piv, f1 - Mk1) + k1;
        ...
        ynew = y + h*k2;
        f2 = ode(tnew, ynew, odeArgs{:});
        k3aux = (f2 - e32*(M*k2 - f1) - 2*(Mk1 - f0) + (h*d)*dfdt);
        k3 = odesolve(Factors, piv, k3aux);
        ndecomps = ndecomps + 1;
        nfevals = nfevals + 2;
        nsolves = nsolves + 3;
    ...
    if output_sol
        ...
        if nout > length(tout)
            ...
            k1data = [k1data, zeros(neq,chunk,"like",prototype)]; %#ok<AGROW>
            k2data = [k2data, zeros(neq,chunk,"like",prototype)]; %#ok<AGROW>
        end
