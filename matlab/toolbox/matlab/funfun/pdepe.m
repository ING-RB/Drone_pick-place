...
%   the underlying ODE solver are available in PDEPE. PDEPE will overwrite
%   settings for Mass, MStateDependence, MvPattern, Jacobian, JPattern, 
%   Jconstant, and Vectorized. See ODESET for details.
%
...
if support
    % D must only be single if single sparse is on due to spdiags
    D = ones(npde,nx,"like",U);
else
    D = ones(npde,nx);
end
% For elliptic problems, BC constraints are always applied in pdeodes as
% algebraic constraints, so set D = 0 on boundaries too. 
D(c == 0, :) = 0;
if ~singular
    D(qL == 0, 1) = 0;
end
...
S = kron(spdiags(ones(nx,3,"like",D),-1:1,nx,nx), ones(npde,npde,"like",D));
...
if any(c==0) || any(qR==0) || (~singular && any(qL==0))
    massSingular = "yes";
else
    massSingular = "no";
end
% Overwrite possible values in options struct when their values are assumed
% by pdepe. In particular, mass and Jacobian options must be set here.
opts = odeset(options,Mass=M,MassSingular=massSingular,MvPattern=[],...
                      Jacobian=[],JPattern=S,JConstant="off",Vectorized="off",...
                      Events=eventfcn);

% Call DAE solver
tfinal = t(end);
fun_pdeodes = @(tnow,y)pdeodes(tnow,y,npde,nx,singular,m,xmesh,...
    xi,pde,bc,zxmp1,xzmp1,xim,varargin{:});
try
    if hasEvents
        [t,y,te,ye,ie] = ode15s(fun_pdeodes,t,y0(:),opts);
    else
        [t,y] = ode15s(fun_pdeodes,t,y0(:),opts);
    end
catch ME
    if strcmp(ME.identifier,'MATLAB:daeic12:IndexGTOne')
        error(message('MATLAB:pdepe:SpatialDiscretizationFailed'))
    elseif strcmp(ME.identifier,'MATLAB:ode15s:MassMatrixAllZero')
        error(message('MATLAB:pdepe:NoParabolic'))
    else
        rethrow(ME);
    end
end
...
if t(end) ~= tfinal
    nt = length(t);
    if ~hasEvents || isempty(te) || (te(end) ~= t(end))  % did not stop on a terminal event
        warning(message('MATLAB:pdepe:TimeIntegrationFailed',sprintf('%e',t(end))));
    end
end

sol = zeros(nt,nx,npde,"like",U);
for i = 1:npde
    sol(:,:,i) = y(:,i:npde:end);
end
varargout{1} = sol;
if hasEvents % [sol,t,sole,te,ie] = pdepe(...)
    varargout{2} = t(:);
    sole = zeros(length(te),nx,npde,"like",U);
    for i = 1:npde
...
function dudt = pdeodes(tnow,y,npde,nx,singular,m,xmesh,...
    xi,pde,bc,zxmp1,xzmp1,xim,varargin)
%PDEODES  Assemble the difference equations and evaluate the time derivative
%   for the ODE system.

u = reshape(y,npde,nx);
up = zeros(npde,nx,"like",u);
...
% Below, denom reset from 0 to 1 corresponds to algebraic constraints for
% 0 = p + qf. These must match with setting components of the mass matrix
% to 0. 
