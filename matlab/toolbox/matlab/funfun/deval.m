...
if ~isfield(sol,'extdata') && any(solver == ["ode113","ode15s","ode23","ode45"])
...
tint = xint(:).';  
tdir = sign(t(end) - t(1));
had2sort = ~issorted(tint,"monotonic");
if had2sort
...
evaluated = 0;
inds = matlab.internal.math.discretize(tdir*full(tint),tdir*t,false);
while evaluated < Nxint
  ...
  bottom = inds(evaluated+1);
  if tint(evaluated+1) == t(end)
    % adjust for end of interval
    bottom = bottom + 1;
  end
  ...
  if ~isempty(index1)
      ...
      elseif (bottom > 2) && (t(bottom) == t(bottom-1)) % Interface point
          ...
          % warn if requested solution or derivative is discontinuous
          % across interface
          if any( yLeft(idx) ~= yRight(idx) ) || ...
              ( Spxint_requested && any( ypLeft(idx) ~= ypRight(idx)) )
