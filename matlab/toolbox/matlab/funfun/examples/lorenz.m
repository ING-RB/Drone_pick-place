...
switch action
    ...
    case 'start'
        ...
        head = line('SeriesIndex',2,'Marker','.','MarkerSize',25,'LineStyle','none','XData',y(1),'YData',y(2),'ZData',y(3)) ;
        body = animatedline('SeriesIndex',2);
        tail = animatedline('SeriesIndex',1);
        ...
        while (get(axHndl,'Userdata') == play) && (h >= hmin)
            ...
            % Compute the slopes
            s1 = lorenzeq(t, y);
            s2 = lorenzeq(t+h, y+h*s1);
            s3 = lorenzeq(t+h/2, y+h*(s1+s2)/4);
...
% ===============================================================================
function ydot = lorenzeq(~,y)
% LORENZEQ Equation of the Lorenz chaotic attractor.
%   ydot = lorenzeq(t,y,SIGMA,RHO,BETA).
%   The differential equation is written in almost linear form.
%
%   With the values of SIGMA, RHO, BETA below, the orbit ranges chaotically
%   back and forth around two different points, or attractors.  It is bounded,
%   but not periodic and not convergent.
