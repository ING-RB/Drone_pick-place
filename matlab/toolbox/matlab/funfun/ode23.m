...
if Mtype > 0  % non-trivial mass matrix
    ...
    nfevals = nfevals + 1;
    [~,prototype] = matlab.ode.internal.updateDatatype(dataType,M,solver_name,"mass matrix");
end
...
if nargout > 0
    if output_sol
        ...
        f3d  = zeros(neq,4,chunk,"like",prototype);
    else
...
while ~done
    ...
    while true
        hB = h * B;
        f(:,2) = ode(t+h*A(1),y+f*hB(:,1),odeArgs{:});
        f(:,3) = ode(t+h*A(2),y+f*hB(:,2),odeArgs{:});
        tnew = t + h*A(3);
        if done
    ...
    if output_sol
        ...
        if nout > length(tout)
            ...
            f3d  = cat(3,f3d,zeros(neq,4,chunk,"like",prototype));
        end
