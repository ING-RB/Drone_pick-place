...
while ~done
    ...
    for iter=1:maxNewtIter
        if needGlobalJacobian
            ...
            dPHIdy = decomposition(dPHIdy,'CheckCondition',false);
            rc = rcond(dPHIdy);
            if rc == 0  || isnan(rc)
...
if minRC < eps("like",minRC)
    warning(message('MATLAB:bvp4c:IllCondJac',sprintf('%g',minRC)));
end
...
N = size(y,2);
y_N1 = matlab.internal.math.viewColumns(y, N-1); % shared copy y(:,1:N-1)
y_2N = y(:,2:N);
yp_N1 = matlab.internal.math.viewColumns(yp, N-1); % shared copy yp(:,1:N-1)
yp_2N = yp(:,2:N);

rowscal = 1 ./ diffx; % diffx is expected to be row vector
slope = (y_2N - y_N1) .* rowscal; % scaling each column
c = (3*slope - 2*yp_N1 - yp_2N) .* rowscal;
d = (yp_N1 + yp_2N - 2*slope) .* (rowscal.^2);

rowscal2 = hnode .* diffx;
Sx = ((d.*rowscal2 + c).*rowscal2 + yp_N1).*rowscal2 + y_N1;
Spx = (3.*d.*rowscal2 + 2.*c).*rowscal2 + yp_N1;
...
if ismbvp
    ...
    NewtRes(:) = RHS(nBCs+1:end); % Copy into NewRes and NewRes keeps its shape.
end
...
for region = 1:nregions
    ...
    yreg = y(:,xidx);
    ypreg = yp(:,xidx);
    hreg = diff(xreg);   % row vector

    iidx = xidx(1:end-1);   % mesh interval index
    Nint = length(iidx);

    % the mid-points
    temp =  (NewtRes(:,iidx) .* (1.5./hreg)) ./ ...
        max(abs(Fmid(:,iidx)),threshold);
    ...
    temp = (ypLob - fLob) ./ max(abs(fLob),threshold);
...
if isempty(Fjac)  % use numerical approx
    if npar > 0   % unknown parameters
        ...
        dPoptions.thresh = repmat(threshval,npar,1);
        dPoptions.fac = [];
    end
end
