...
[neq, tspan, ntspan, next, t0, tfinal, tdir, y0, f0, odeArgs, ...
    options, threshold, rtol, normcontrol, normy, userhmin, userhmax, htry, htspan, datatype] = ...
...
[Mtype, Mt, Mfun, Margs, dMoptions] = odemass(odeIsFuncHandle,ode, ...
    t0,y0,options,datatype,varargin);
...
t = t0;
y = y0;
[datatype,prototype] = matlab.ode.internal.updateDatatype(datatype,Mt,solver_name,"mass matrix");
...
if Mtype > 0
    ...
    if DAE
        ...
        if isempty(yp0)
            yp0_OK = false;
            yp0 = zeros(neq,1,"like",prototype);
        else
            yp0 = yp0(:);
            if length(yp0) ~= neq
...
if Jconstant
    if isempty(Jac) % use odenumjac
        ...
        nfevals = nfevals + nF;
        npds = npds + 1;
    elseif ~isnumeric(Jac)  % not been set via 'options'
...
if DAE && ~yp0_OK
    ...
    elseif DAE && yp0_OK
        ...
        [Factors, piv] = odefactorize(Mt);
        yp = odesolve(Factors, piv, f0);
        ndecomps = ndecomps + 1;
        nsolves = nsolves + 1;
    end
...
[~,prototype] = matlab.ode.internal.updateDatatype(datatype,dfdy,solver_name,"Jacobian");
...
if isempty(htry)
    ...
    if ~DAE
        ...
        nfevals = nfevals + 1;
        DfDt = ((f1 - f0) ./ tdel) + dfdy*yp;  % dfdt = (f1 - f0) ./ tdel;
        if normcontrol
            if Mtype > 0
                temp = odesolve(Factors, piv, DfDt);
                rh = 1.25 * sqrt(0.5 * (norm(temp) / wt) / rtol);
            else
            ...
            if Mtype > 0
                temp = odesolve(Factors, piv, DfDt);
                rh = 1.25*sqrt(0.5*norm(temp ./ wt,inf) / rtol);
            else
...
dif = zeros(neq,maxk+2,"like",prototype);
dif(:,1) = h * yp;
...
if Mtype == 4
    ...
    [dMpsidy,dMoptions.fac] = odenumjac(@odemxv, ...
        {Mfun,t,y,psi,Margs{:}}, Mt*psi, dMoptions); %#ok<CCAT>
    Miter = Miter + dMpsidy;
end
...
if DAE
    ...
    Miter = RowScale .* Miter;
end

[Factors, piv] = odefactorize(Miter);
ndecomps = ndecomps + 1;
havrate = false;
...
if nargout > 0
    if output_sol
        ...
        kvec = zeros(1,chunk,"like",prototype);
        dif3d = zeros(neq,maxk+2,chunk,"like",prototype);
    else
...
while ~done
    ...
    while true                            % Evaluate the formula.
        ...
        while ~gotynew
            ...
            difkp1 = zeros(neq,1,"like",prototype);
            if normcontrol
                normynew = norm(ynew);
            ...
            for iter = 1:maxit
                ...
                rhs = hinvGak*ode(tnew,ynew,odeArgs{:}) -  Mtnew*(psi+difkp1);
                if DAE                          % Account for row scaling.
                    rhs = RowScale .* rhs;
                end

                del = odesolve(Factors, piv, rhs); % solve 
            ...
            if tooslow
                    ...
                    if ~Jcurrent
                        if Janalytic
                            ...
                            [dfdy,Joptions.fac,nF] = odenumjac(ode, ...
                                {t,y,odeArgs{:}}, f0, Joptions); %#ok<CCAT>
                ...
                elseif absh <= hmin
                    warning(message('MATLAB:ode15s:IntegrationTolNotMet', ...
    ...
    if output_sol
        ...
        if nout > length(tout)
            ...
            kvec = [kvec, zeros(1,chunk,"like",prototype)]; %#ok<AGROW>
            dif3d = cat(3,dif3d, zeros(neq,maxk+2,chunk,"like",prototype));
        end
    ...
    Jcurrent = Jconstant;
    if Mtype < 2
        % MtYpe = 0 or 1: Constant mass matrix I or M.
        Mcurrent = true;
    elseif Mtype == 2
