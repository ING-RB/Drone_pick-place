...
if isempty(htry)
    ...
    if ~DAE
        ...
        if normcontrol
            if Mtype > 0
                rh = 1.43 * sqrt(0.5 * (norm(odesolve(Factors, piv, DfDt)) / wt) / rtol);
            else
                rh = 1.43 * sqrt(0.5 * (norm(DfDt) / wt) / rtol);
            end
        else
            if Mtype > 0
                rh = 1.43*sqrt(0.5*norm( odesolve(Factors, piv, DfDt) ./ wt,inf) / rtol);
            else
...
if nargout > 0
    if output_sol
        ...
        zdata = zeros(neq,chunk,"like",prototype);
        znewdata = zeros(neq,chunk,"like",prototype);
    else
...
while ~done
    ...
    while true                            % Evaluate the formula.
        ...
        [ynew,znew,iter,itfail,rate] = ...
            itsolve(Mtnew,tnew,ynew,znew,gamma,h,Factors,piv,ode,rtol,...
            wt,rate,Mtype,Mfun,Margs,DAE,RowScale,odeArgs);
    ...
    if output_sol
        ...
        if nout > length(tout)
            ...
            zdata = [zdata, zeros(neq,chunk,"like",prototype)]; %#ok<AGROW>
            znewdata = [znewdata, zeros(neq,chunk,"like",prototype)]; %#ok<AGROW>
        end
...
function [y,z,iter,itfail,rate] = ...
    itsolve(M,t,y,z,gamma,h,Factors,piv,ode,rtol,wt,rate,...
    Mtype,Mfun,Margs,DAE,RowScale,odeArgs)
...
% Factors,piv is the decomposition of the matrix M-gamma*h*dfdy, where dfdy is
