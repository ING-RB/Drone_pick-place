...
[haveEventFcn,eventFcn,eventArgs,valt,teout,yeout,ieout] = ...
    odeevents(FcnHandlesUsed,ode,t0,y0,options,{yp0(:), odeArgs{:}}); %#ok<CCAT>
if haveEventFcn
...
[Jac,dfdy,dfdyp,Jconstant,dfdy_options,dfdyp_options,nfcn] = ...
    ode15ipdinit(ode,t0,y0,yp0,f0,options,odeArgs);

[datatype,~] = matlab.ode.internal.updateDatatype(datatype,dfdy,solver_name,"Jacobian");
[~,prototype] = matlab.ode.internal.updateDatatype(datatype,dfdyp,solver_name,"Jacobian");
...
mesh = zeros(1,maxk+2,"like",prototype);
mesh(1) = t0;
meshsol = zeros(neq,maxk+2,"like",prototype);
meshsol(:,1) = y0;
...
while ~done
    ...
    while true                            % Evaluate the formula.
        ...
        while ~gotynew
            ...
            tooslow = false;
            for iter = 1:maxit
                rhs = - ode(tnew,ynew,ypnew,odeArgs{:});
            ...
            if tooslow
                ...
                if ~PDscurrent   % always current if Jconstant
                    if isempty(dfdy_options) && isempty(dfdyp_options)
                        ...
                        f = ode(t,y,yp,odeArgs{:});
                        nfevals = nfevals + 1;
                    end
                    [dfdy,dfdyp,dfdy_options,dfdyp_options,NF] = ...
                        ode15ipdupdate(Jac,ode,t,y,yp,f,dfdy,dfdyp,dfdy_options,dfdyp_options,odeArgs);
...
function [vtry,isterminal,direction] = events_aux(ttry,ytry,eventfun,tnew,...
    ynew,ypnew,mesh_k,meshsol_k,varargin)
