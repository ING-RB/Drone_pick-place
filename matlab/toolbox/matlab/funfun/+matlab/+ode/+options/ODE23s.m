%matlab.ode.options.ODE23s  Solver options for ode23s solver
%   obj = matlab.ode.options.ODE23s creates the solver options object
%   with default property values.
%
%   obj = matlab.ode.options.ODE23s(...,Name=Value) sets the property NAME
%   to VALUE.
%
...
classdef (Sealed = true) ODE23s < matlab.ode.internal.LegacyStiffSolverOptions
    methods
        function obj = ODE23s(nv)
            arguments
                nv.?matlab.ode.options.ODE23s
            end
            for k=fields(nv)'
                obj.(k{:}) = nv.(k{:});
            end
            obj.ID = matlab.ode.SolverID.ode23s;
        end
    end
end
