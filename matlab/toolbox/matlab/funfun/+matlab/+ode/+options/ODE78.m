%matlab.ode.options.ODE78  Solver options for ode78 solver
%   obj = matlab.ode.options.ODE78 creates the solver options object
%   with default property values.
%
%   obj = matlab.ode.options.ODE78(...,Name=Value) sets the property NAME
%   to VALUE.
%
...
classdef (Sealed = true) ODE78 < matlab.ode.internal.LegacyNonstiffSolverOptions
    methods
        function obj = ODE78(nv)
            arguments
                nv.?matlab.ode.options.ODE78
            end
            for k=fields(nv)'
                obj.(k{:}) = nv.(k{:});
            end
            obj.ID = matlab.ode.SolverID.ode78;
            obj.DefaultRefine = 8;
        end
    end
end
