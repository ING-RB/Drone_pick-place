%matlab.ode.options.ODE89  Solver options for ode89 solver
%   obj = matlab.ode.options.ODE89 creates the solver options object with
%   default property values.
%
%   obj = matlab.ode.options.ODE89(...,Name=Value) sets the property NAME
%   to VALUE.
%
...
classdef (Sealed = true) ODE89 < matlab.ode.internal.LegacyNonstiffSolverOptions
    methods
        function obj = ODE89(nv)
            arguments
                nv.?matlab.ode.options.ODE89
            end
            for k=fields(nv)'
                obj.(k{:}) = nv.(k{:});
            end
            obj.ID = matlab.ode.SolverID.ode89;
            obj.DefaultRefine = 8;
        end
    end
end
