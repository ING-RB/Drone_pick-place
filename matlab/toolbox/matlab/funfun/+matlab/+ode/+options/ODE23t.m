%matlab.ode.options.ODE23t  Solver options for ode23t solver
%   obj = matlab.ode.options.ODE23t creates the solver options object
%   with default property values.
%
%   obj = matlab.ode.options.ODE23t(...,Name=Value) sets the property NAME
%   to VALUE.
%
...
classdef (Sealed = true) ODE23t < matlab.ode.internal.LegacyStiffSolverOptions
    methods
        function obj = ODE23t(nv)
            arguments
                nv.?matlab.ode.options.ODE23t
            end
            for k=fields(nv)'
                obj.(k{:}) = nv.(k{:});
            end
            obj.ID = matlab.ode.SolverID.ode23t;
        end
    end
end
