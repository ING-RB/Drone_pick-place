%matlab.ode.options.ODE45  Solver options for ode45 solver
%   obj = matlab.ode.options.ODE45 creates the solver options object with
%   default property values.
%
%   obj = matlab.ode.options.ODE45(...,Name=Value) sets the property NAME
%   to VALUE.
%
...
classdef (Sealed = true) ODE45 < matlab.ode.internal.LegacyNonstiffSolverOptions
    methods
        function obj = ODE45(nv)
            arguments
                nv.?matlab.ode.options.ODE45
            end
            for k=fields(nv)'
                obj.(k{:}) = nv.(k{:});
            end
            obj.ID = matlab.ode.SolverID.ode45;
            obj.DefaultRefine = 4;
        end
    end
end
