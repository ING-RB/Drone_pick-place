...
t = t0;
y = y0;
[datatype,~] = matlab.ode.internal.updateDatatype(datatype,Mt,solver_name,"mass matrix");
...
if isempty(htry)
    ...
    if normcontrol
        if Mtype > 0
            rh = 1.43*sqrt(0.5 * (norm(odesolve(Factors, piv, DfDt)) / wt)) / rtol^pow;
        else
            rh = 1.43 * sqrt(0.5 * (norm(DfDt) / wt)) / rtol^pow;
        end
    else
        if Mtype > 0
            rh = 1.43*sqrt(0.5*norm(odesolve(Factors, piv, DfDt)./ wt,inf)) / rtol^pow;
        else
...
if nargout > 0
    if output_sol
        ...
        t2data = zeros(1,chunk,"like",prototype);
        y2data = zeros(neq,chunk,"like",prototype);
    else
...
while ~done
    ...
    while true                            % Evaluate the formula.
        ...
        [y2,z2,iter,itfail1,rate] = ...
            itsolve(Mt2,t2,y2,z2,d,h,Factors,piv,ode,...
            rtol,wt,rate,Mtype,Mfun,Margs,odeArgs);
        ...
        if ~itfail1
            ...
            [ynew,znew,iter,itfail2,rate] = ...
                itsolve(Mtnew,tnew,ynew,znew,d,h,Factors,piv,ode,rtol, ...
                wt,rate,Mtype,Mfun,Margs,odeArgs);
            nfevals = nfevals + iter;
            nsolves = nsolves + iter;
        end
        ...
        if itfail1 || itfail2                % Unable to evaluate a stage.
            ...
            est2 = odesolve(Factors, piv, est1);
            nsolves = nsolves + 1;
    ...
    if output_sol
        ...
        if nout > length(tout)
            ...
            t2data = [t2data, zeros(1,chunk,"like",prototype)]; %#ok<AGROW>
            y2data = [y2data, zeros(neq,chunk,"like",prototype)]; %#ok<AGROW>
        end
...
function [y,z,iter,itfail,rate] = ...
    itsolve(M,t,y,z,d,h,Factors,piv,ode,rtol,wt,rate,Mtype,Mfun,Margs,odeArgs)
...
% Factors,piv is the lu decomposition of the matrix M-d*h*dfdy, where dfdy is an
...
for iter = 1:maxiter
    ...
    rhs = h * ode(t,y,odeArgs{:}) - M * z;

    del = odesolve(Factors, piv, rhs);
