...
%SUBSASGNPARENS Subscripted assignment to a duration.
...
if ~isscalar(s)
    ...
    case '.'
        ...
        if name == "Format"
            ...
            error(message('MATLAB:duration:SubArrayPropertyAssignment',name));
        end
        ...
        % per-element property (of which duration, unlike datetime, has none).
        % These calls to subsrefParens and subsasgnDot give the proper error
    ...
    assert(false,'dur(...).Name = val should be caught as an error in all cases.');
end
...
% tabular, for assignments like t.d(i) = val when d is a duration) can call
