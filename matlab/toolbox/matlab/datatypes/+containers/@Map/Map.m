%containers.Map constructs a Map.
%   myMap = containers.Map() creates an empty Map. The properties of myMap
...
%   data that you want stored in the Map and a key is a unique reference to
%   that data. Either the KEYS or the VALUES array may be a cell array, in
%   which case the actual keys and/or values are extracted from the cell
%   array. Valid keys are either numeric real scalars, character vectors in
%   a cell array, or a string array. Values can be of any type. The Map's
%   KeyType and ValueType properties are set based on the key and value
%   provided. NOTE: string arrays are accepted as keys, but converted
%   internally into character vectors. i.e. The KeyType remains 'char' when
%   a Map is created with a string array.
% 
%   myMap = containers.Map(KEYS, VALUES, 'UniformValues', B) where B is a
%   logical scalar. If B is false, this creates a Map with the ValueType
%   set to 'any'. If it is true, all values must be of the same type.
%
%   myMap = containers.Map('KeyType', kType, 'ValueType', vType) constructs
%   a Map with no data that uses a key type of kType, and a value type of
%   vType. Valid values for kType are: 'char','double', 'single', 'int32',
%   'uint32', 'int64', 'uint64'. Valid values for vType are: 'char',
%   'double', 'single', 'int8', 'uint8', 'int16', 'uint16', 'int32',
%   'uint32', 'int64', 'uint64', 'logical', or 'any'. The order of the key
%   type and value type arguments is not important, but both must be
%   provided.
%
%   Examples:
%
...
%   one of: 'char', 'double', 'single', 'int32', 'uint32', 'int64',
%   'uint64'. If a different numeric type is used as a key, it will be
