function h = categoricalHistogram(~, args, parent)
...
%   CATEGORIES.  CATEGORIES is a categorical array, string array, cell
%   array of character vectors, or pattern scalar. Categories must be
%   unique.
%
...
%             'percentage'   The height of each bar is (100 * number of
%                            elements in category) / (total number of
%                            elements), and the sum of the bar heights
...
%                            <= NUMEL(X).
%                    'cdf'   Cumulative distribution function estimate. The
%                            height of each bar is the cumulative relative
%                            number of observations in each category and all
...
[opts,passthrough] = parseinput(args);
...
if isscalar(ancestorAxes)
    if (isfield(opts,'Orientation') && (opts.Orientation == "horizontal")) || ...
            (~isfield(opts,'Orientation') && (get(parent, 'defaultCategoricalhistogramOrientation') == "horizontal"))
        matlab.graphics.internal.configureAxes(ancestorAxes,1,xaxis);
    else
        matlab.graphics.internal.configureAxes(ancestorAxes,xaxis,1);
    end
end

hObj = matlab.graphics.chart.primitive.categorical.Histogram('Parent', parent, ...
...
function [opts,passthrough] = parseinput(input)
...
if ~isempty(input)
    ...
    elseif matlab.internal.datatypes.isScalarText(x)
        % First input is a Name-Value pair
        x = categorical([]);
    else
    ...
    if ~isempty(input)
        ...
        if isa(in,"pattern")
            ...
            opts.Categories = reshape(x.categoryNames(matches(x.categoryNames,in)),1,[]); % a row
            input(1) = [];
            inputoffset = inputoffset + 1;
        elseif ~matlab.internal.datatypes.isScalarText(in)
            validateattributes(in,{'string','cell','categorical','pattern'}, ...
            ...
            if ~isa(in,'categorical')
                ...
                in.codes(in.codes==0) = []; % categorical.undefCode
                opts.Categories = cellstr(reshape(in,1,[])); % a row
            end
            input(1) = [];
            inputoffset = inputoffset + 1;
        end
        ...
        for i=1:2:inputlen
            ...
            switch name
                case 'Categories'
                    if isa(value,"pattern")
                        if ~isscalar(value)
                            error(message('MATLAB:categorical:InvalidNamesOrPattern','CATEGORIES'));
                        end
                        opts.Categories = reshape(x.categoryNames(matches(x.categoryNames,value)),1,[]); % a row
                    else
                        validateattributes(value,{'string','cell','categorical','pattern'},{'vector'}, ...
                        ...
                        if ~isa(value,'categorical')
                            ...
                            % The histogram bins will be based on categories' values, not its categories
                            % Filter out undefined categories before extracting category names
                            value.codes(value.codes==0) = []; % categorical.undefCode
                ...
                case 'Normalization'
                    opts.Normalization = validatestring(value, {'count', 'countdensity', 'cumcount',...
                        'probability', 'pdf', 'percentage', 'cdf'}, funcname, 'Normalization', i+1+inputoffset);
                case 'BinCounts'
                ...
                case 'ShowOthers'
                    try
                        opts.ShowOthers = matlab.lang.OnOffSwitchState(value);
                    catch % mimic the validatestring err msg
                        error(message('MATLAB:categorical:histogram:InvalidShowOthers',i+1+inputoffset));
                    end
                    if ~isscalar(opts.ShowOthers)
                        error(message('MATLAB:categorical:histogram:InvalidShowOthers',i+1+inputoffset));
                    end
                otherwise
        ...
        if isfield(opts,'BinCounts')
            ...
            if isfield(opts, 'ShowOthers') && opts.ShowOthers
