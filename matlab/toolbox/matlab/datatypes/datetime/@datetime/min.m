...
if nargin == 1 % min(a)
    isUnary = true;
    aData = a.data;
elseif nargin == 2 % min(a,b), including min(a,[]) treated as binary
    isUnary = false;
    if nargout > 1
    ...
    for ii = 1:(nargin-2) % min(...,'ComparisonMethod',method) is not supported
...
if isUnary
    ...
    if isreal(aData)
        if nargout <= 1
            c.data = min(aData,[],dim,missingFlag);
        else
            if linearFlag
                [c.data,i] = min(aData,[],dim,missingFlag,'linear');
            elseif isscalar(dim)
                [c.data,i] = min(aData,[],dim,missingFlag);
            else % Matching numeric: 2nd output supported for vector DIM only with linear indices
                error(message('MATLAB:min:secondOutputNotSupported'));
            end
        end
    else
        if isscalar(dim)
            if nargout <= 1
                c.data = minMaxUnary(aData,dim,omitMissing,false,linearFlag);
            else
                [c.data,i] = minMaxUnary(aData,dim,omitMissing,false,linearFlag);
            end
        else
            ...
            if nargout <= 1
                cData = minMaxUnary(aData,1,omitMissing,false,linearFlag);
                c.data = reshape(cData,szOut);
            elseif linearFlag
                linInds = permuteWorkingDims(linInds,dim);
                [cData,i] = minMaxUnary(aData,1,omitMissing,false,linearFlag);
                c.data = reshape(cData,szOut);
                i = linInds(i);
                i = reshape(i,szOut);
            else % Matching numeric: 2nd output supported for vector DIM only with linear indices
    ...
    if isreal(aData) && isreal(bData)
        c.data = min(aData,bData,missingFlag);
    else
        c.data = minMaxBinary(aData,bData,omitMissing,false);
    end
end
