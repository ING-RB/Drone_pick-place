...
%GENERATEBINEDGESFROMCALENDARDURATION Generate bin edges from calendarDuration.
%   EDGES = GENERATEBINEDGESFROMCALENDARDURATION(DUR,XMIN,XMAX,HARDLIMITS,MAXNBINS)
%   returns a vector of datetime edges of uniform width DUR (where DUR is a
%   scalar calendarDuration) generated using the datetime limits XMIN and
%   XMAX. If HARDLIMITS is true, then XMIN and XMAX are forced to be the
%   first and last edges. Otherwise, the first and last edges are
%   calculated and may be different than XMIN and XMAX. If the number of
%   bins would exceed MAXNBINS, then DUR is ignored and EDGES is equivalent
%   to generateBinEdgesFromNumBins(MAXNBINS,XMIN,XMAX,HARDLIMITS).
...
if nbinsupperbounds <= maxnbins
    if hardlimits
        ...
        edges = [leftedge leftedge+(1:max(nbinsupperbounds,1))*dur];
        index = max(find(edges >= xmax, 1),2);
