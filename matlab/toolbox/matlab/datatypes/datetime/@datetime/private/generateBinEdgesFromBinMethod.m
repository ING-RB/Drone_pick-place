...
%   returns a vector of datetime edges of uniform width BINMETHOD (where
%   BINMETHOD is a char array indicating a unit of time, e.g. 'hour')
%   generated using the datetime limits XMIN and XMAX. If HARDLIMITS is
...
%
%   [EDGES,TWOEDGESFORMAT,FMT] = GENERATEBINEDGESFROMBINMETHOD(BINMETHOD,XMIN,XMAX,HARDLIMITS,MAXNBINS)
%   returns TWOEDGESFORMAT and FMT, which are specific to the
%   implementation of datetime/discretize. FMT is a datetime format to use
%   when displaying EDGES. If TWOEDGESFORMAT is true, then a two-edges
%   format (i.e. open/closed interval notation such as
%   '(23-Sep-2016,24-Sep-2016]') should be used when generating category
%   names from EDGES. Otherwise, FMT should be used for generating category
%   names from EDGES.
