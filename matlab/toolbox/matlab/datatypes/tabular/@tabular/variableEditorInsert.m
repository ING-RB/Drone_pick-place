...
if orientation == "columns"
    ...
    this(row:size(varData,1)+row-1, newVarIndices) = varData;
    for k=1:length(newVarIndices)
        ...
        ind = 1;
        while matches(newVarName,this.varDim.labels)
    ...
    this = this(:, [1:insertIndex-1 numVars+1:end insertIndex:numVars]);
    ...
    if row>numRows+1
       ...
       % = varData
       out = this;
       out(row:row+size(varData,1)-1, insertIndex:insertIndex+size(data,2)-1) = varData;
       return
    end
    
    % Add the pasted table after the last row
    try
        this(numRows+1:numRows+size(varData,1), insertIndex:insertIndex+size(data,2)-1) = varData;
    catch
        ...
        this(numRows+1:numRows+size(tmpVarData,1), insertIndex:insertIndex+size(tmpVarData,2)-1) = tmpVarData;
    end
    ...
    if insertIndex<=numRows
        this = this([1:row-1 numRows+1:numRows+size(varData,1) row:numRows], 1:numVars);
    end
end
