...
if isa(var,'categorical')
    ...
    try
        ...
        throw(addCause(MException(message('MATLAB:table:VarUniqueMethodFailed',varName)), ME));
    end
    if size(gidx,1) ~= size(var,1)
        error(message('MATLAB:table:VarUniqueMethodFailedNumRows',varName));
    end
        
    if isnumeric(var) || islogical(var) || isstring(var) || isdatetime(var) || isduration(var) || iscalendarduration(var)
    ...
    elseif isstring(var) || isdatetime(var) || isduration(var) || iscalendarduration(var)
        gnames = cellstr(glevels);
    elseif iscell(var)
        gnames = glevels;
    end
...
gl = full(glevels);
gnames = compose('%d',gl); % a little less than 19 significant digits
ufmt = (fix(gl) == gl) & (gl > intmax('int64'));
gnames(ufmt) = compose('%u',gl(ufmt)); % a little more than 19 significant digits
gfmt = (fix(gl) ~= gl)| (gl < intmin('int64')) | (gl > intmax('uint64'));
if any(gfmt)
    gnames(gfmt) = compose('%g',gl(gfmt)); % six significant digits
    ...
    if length(unique(gnames)) < length(gnames)
        ...
        for i = 1:length(tryFmt)
            gnames(gfmt) = compose(tryFmt{i},gl(gfmt));
