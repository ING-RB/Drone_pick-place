...
if isdatetime(this.rowDim.labels) || isduration(this.rowDim.labels)
    ...
    % colIntervals also includes the row times, so decrement it
    colIntervals = colIntervals - 1;
end
...
for k=1:size(colIntervals,1)
    ...
    for index=index1:index2
        ...
        varIndexArray = [varIndexArray num2str(index) ]; %#ok<AGROW>
        if index<index2 || k<size(colIntervals,1)
            varNameArray = [varNameArray ',']; %#ok<AGROW>
            varIndexArray = [varIndexArray ',']; %#ok<AGROW>
        end
        if ~isvarname(varNames{index})
            allVarNames = false;
        end
    end
...
if numVariables>1
    ...
    varIndexArray = ['[' varIndexArray ']'];
end

if allVarNames
    out = [varName,' = removevars(',varName,', ' varNameArray,');'];
else
    out = [varName,' = removevars(',varName,', ' varIndexArray,');'];
end
