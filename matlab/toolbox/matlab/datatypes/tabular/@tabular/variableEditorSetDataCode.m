...
import matlab.internal.tabular.private.subsrefRecurser
...
if ~isempty(j)
    ...
    elseif isa(vardata, 'string')
        ...
        if ismissing(rhs)
            ...
            % Special handling for any newline or tab characters in the
            % assignment
            tabChar = char(9);
            if contains(rhs, newline) || contains(rhs, tabChar)
                rhs = char(strrep(strrep(rhs, newline, '" + newline + "'), ...
                    tabChar, '" + char(9) + "'));
            else
                rhs = char(rhs);
            end
        end
    elseif isa(vardata, 'datetime')
        if isdatetime(rhs) && isnat(rhs)
            rhs = 'NaT';
            emptyRHS = true;
        end
    end
    ...
    dotSubsStr = generateDotSubscripting(a,colname, varname);
    if isGroupColumn
        relcol = col-varIndices(j)+1;
        if iscell(vardata) % e.g. x.BloodPressure{10,2} = rhs;
            % Generate code only if the new value is different
            subsrefStruct = substruct('.', colname,'{}',{row,relcol});
            if emptyRHS && ~singleCell
                % can't assign multiple values to some table variable types
                % with a single value, so build a command with multiple
                % empties:  t.column(1,:) = {{},{}};
                str = [dotSubsStr, '(' num2str(row) ',:) = {'];
                numEmpties = varIndices(j+1) - varIndices(j);
                for i=1:numEmpties
                    str = [str '{},']; %#ok<AGROW>
                end
                str = [str(1:length(str)-1) '};'];
            elseif row>size(a,1) || ~isequal(subsrefRecurser(a,subsrefStruct),rhsValue)
                str = [dotSubsStr, '{' num2str(row) ',' num2str(relcol) '} = ' rhs ';'];
            end
            ...
            subsrefStruct = substruct('.',colname,'()',{row,relcol});
            if emptyRHS && ~singleCell
                str = [dotSubsStr, '(' num2str(row) ',:) = ' rhs ';'];
            elseif row>size(a,1) || ~isequal(subsrefRecurser(a,subsrefStruct),rhsValue)
                str = [dotSubsStr, '(' num2str(row) ',' num2str(relcol) ') = ' rhs ';'];
            end
        end
        ...
        if iscell(vardata) % e.g. x.Weight{10} = rhs;
            % Generate code only if the new value is different
            subsrefStruct = substruct('.',colname,'{}',{row});
            if row>size(a,1) || ~isequal(subsrefRecurser(a,subsrefStruct),rhsValue)
                if size(a,1)==1 && row>1 % force the var to grow vertically
                    str = [dotSubsStr, '{' num2str(row) ',1} = ' rhs ';'];
                else
                    str = [dotSubsStr, '{' num2str(row) '} = ' rhs ';'];
                end
            end
        elseif colClasses{j} == "char"
            ...
            if isempty(rhsValue)
                ...
                [~, columns] = size(a.dotReference(colname));
                if columns == 1
                    subsrefStruct = substruct('.',colname,'()',{row, 1});
                else
                    subsrefStruct = substruct('.',colname,'()',{row, [1, columns]});
                end
                if row>size(a,1) || ~isequal(subsrefRecurser(a,subsrefStruct),rhsValue)
                    str =  [dotSubsStr, '(' num2str(row) ',1:' num2str(length(rhsValue)) ') = ' rhs ';'];
                end
            end
        elseif colClasses{j} == "datetime"
            rhs = strtrim(rhs);
            if ismember(lower(rhs),{'''now''' '''yesterday''' '''today''' '''tomorrow'''}) || ...
                ...
                str = [dotSubsStr, '(' num2str(row) ') = datetime(' rhs ');'];
            elseif isempty(rhsValue)
                ...
                % format by default.
                dt = subsrefRecurser(a,subsrefStruct);
                try
