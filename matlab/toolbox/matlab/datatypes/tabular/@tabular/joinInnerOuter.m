...
import matlab.internal.tabular.broadcastRows
...
try
    ...
    % When both inputs are timetables and either input has implicit row 
    % times, we force the output to have explicit row times. The output 
    % dimensions may be incorrect here, but they will be fixed in the 
    % subsequent call to createLike.
    if ~a.rowDim.requireUniqueLabels && ~b.rowDim.requireUniqueLabels
        c.rowDim = matlab.internal.tabular.private.explicitRowTimesDim(a.rowDim.length,a.rowDim.labels);
    end
    c.rowDim = c.rowDim.createLike(length(il));

    % Merge rowDim properties in order.
    % Prevent events on eventtables.
    if isa(c,"eventtable") && istimetable(b)
        b.rowDim = b.rowDim.setTimeEvents([]);
    end
    c.rowDim = c.rowDim.mergeProps(a.rowDim); c.rowDim = c.rowDim.mergeProps(b.rowDim);
    ...
    if mergeKeyProps
        ...
        c.varDim = c.varDim.fillEmptyProps(b.varDim,rightKeys(i),leftKeys(i));
    end
    ...
    for j = 1:numLeftVars
        aVarRows = selectRows(a_data{leftVars(j)},ilSrc);
        c_data{j} = broadcastRows(aVarRows,c_nrows,ilDest);
    end
    b_data = b.data;
    for j = 1:numRightVars
        bVarRows = selectRows(b_data{rightVars(j)},irSrc);
        c_data{j+numLeftVars} = broadcastRows(bVarRows,c_nrows,irDest);
    end
    c.data = c_data;
