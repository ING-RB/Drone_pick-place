function t = dotAssign(t,idxOp,b)
%

% DOTASSIGN Subscripted assignment into a table using dot.
...
% may follow that, and row labels are inherited from the table. For dot
% assignment, both assignment and deletion need to be handled by dotAssign.
...
try
    ...
    if ~isa(idxOp,'matlab.indexing.IndexingOperation')
        % Internal tabular methods can call dotAssign directly by specifying the
    ...
    % built-in types, idxOp(2).Type==Parens guarantees that length(idxOp)==2, but for a 
    % var that is itself a table, parens need not be the end, so need to check that.
    deleting = isnumeric(b) && isequal(b,[]) && builtin('_isEmptySqrBrktLiteral',b) ...
        && (isscalar(idxOp) || ((length(idxOp) == 2) && (idxOp(2).Type == matlab.indexing.IndexingOperationType.ParenDelete)));
    ...
    if isnumeric(varName)
        ...
        if isNewVar
            ...
            elseif varIndex > t_nvars+1
                ...
                [~,~,~,~,~,updatedVarDim] = t.subs2inds(varIndex,'varDim',matlab.internal.tabular.private.tabularDimension.subsType_assignment);
    ...
    elseif (isstring(varName) && isscalar(varName)) ...
        ...
        if isempty(varIndex)
                ...
                if varName == "Properties"
                    try
                        if isscalar(idxOp)
                            ...
                            t = t.setProperty(idxOp(2:end),b);
                        end
                    catch ME
                        if ~isscalar(idxOp) && (ME.identifier == "MATLAB:table:UnknownProperty")
            ...
            elseif matches(varName,t.metaDim.labels{1})
                ...
                elseif deleting % && ~isscalar(idxOp)
            ...
            elseif deleting
                ...
                % Variable names are stored as cellstrs, so if the new variable
                % name is a string, convert it to a char row vector before
                % moving forward.
                varName = char(varName);

                % If this is a new variable, it will go at the end. Its name is guaranteed
                % to not conflict; if it did, we'd be assigning to an existing var.
                varIndex = t_nvars + 1;
    ...
    if deleting
        ...
        % first index.
            ...
            if isscalar(idxOp(2).Indices) ...
                    || ~isColon(idxOp(2).Indices{1}) ...
                    || all(cellfun(@isColon,idxOp(2).Indices))
            ...
            % Since we are inside a try-catch, do the assignment in a local
            % function to avoid data copies.
            var_j = localTranslatedAssign(move(var_j),idxOp,[],b);
        ...
        % subscripting. Cannot create a new var implicitly by deeper indexing.
            ...
            if isNewVar && (length(idxOp) > 2) && idxOp(2).Type ~= matlab.indexing.IndexingOperationType.Dot
            ...
            indiceslvl2 = [];
            if idxOp(2).Type == matlab.indexing.IndexingOperationType.Dot % dot indexing into variable
                % If the assignment is not to an existing var, check for some common
                % mistakes to suggest what may have been intended
                if isNewVar
                    if strcmpi(varName,'Properties') && (isCharString(idxOp(2).Name) || (isstring(idxOp(2).Name) && isscalar(idxOp(2).Name)))
                ...
                if isNewVar
                    ...
                    if t_nrows == 0 && isColon(indiceslvl2{1})
                        if t_nvars == 0
                    ...
                    for k = 2:length(indiceslvl2)
                        if isColon(indiceslvl2{k})
                            indiceslvl2{k} = 1:size(b,k);
                        end
                    end
                else
                    if varIndex > 0
                ...
                if haveLabelSubscripts
                    ...
                    [indiceslvl2{1},~,~,~,~,updatedRowDim] = t.subs2inds(subs1,'rowDim',matlab.internal.tabular.private.tabularDimension.subsType_assignment);
                    % There are some linear indexing cases that should have row semantics, or that
                    % are not even legal. In those cases idxOp(2).Indices{1} can't be interpreted as row
                ...
                if isscalar(indiceslvl2) % linear indexing into the LHS
                    ...
                    elseif all(size(var_j)==0) || isNewVar
                        ...
                        if isNewVar || isNonEmptySubscript(indiceslvl2{1})
                            indiceslvl2 = [indiceslvl2 {1}];
                        end
                    elseif ismatrix(var_j) && isempty(var_j) && ~isrow(var_j) % Nx0 or 0xM, excluding 1x0
                        ...
                        if isNonEmptySubscript(indiceslvl2{1})
            ...
            % a literal [] should be treated as a genuine assignment, this case will also be
            % handled correctly by the forwarding syntax. A RHS that is a '' (0x0 char) should 
            % be treated as assignment, however builtin types would treat that as a deletion.
            % Assignment of any other empty char will have the desired effect, so turn b 
            % into a 0x1 char to prevent it from deleting.
            if ischar(b) && isequal(b,'') && ~isobject(var_j) && (idxOp(end).Type == matlab.indexing.IndexingOperationType.Paren)
                b = b(:);
            end
            ...
            % Since we are inside a try-catch, do the assignment in a local
            % function to avoid data copies.
            var_j = localTranslatedAssign(move(var_j),idxOp,indiceslvl2,b);            
        end
        ...
        if varLen < t_nrows % t's original number of rows
            ...
            var_j = matlab.internal.datatypes.lengthenVar(var_j,t_nrows);
        end
        if varIndex > 0
            t.data{varIndex} = var_j;
        elseif varIndex == 0
        ...
        if varLen > t_nrows % t's original number of rows
            ...
            for j = 1:t.varDim.length
                if size(t.data{j},1) < varLen
                    t.data{j} = matlab.internal.datatypes.lengthenVar(t.data{j}, varLen);
                end
            end
        end

        % After the variable assignment is complete, check for any cross
        % variable incompatability that could have been introduced. Currently,
        % this is only required for eventtables and is a no-op for other tabular
        % types.
        t.validateAcrossVars();
    end
catch ME
    throw(ME);
end

function tf = isNonEmptySubscript(idx)
    tf = (isnumeric(idx) && ~isempty(idx)) || ...
         (islogical(idx) && any(idx));

function var = localTranslatedAssign(var, idxOp, indiceslvl2, rhs)
% Helper to do the assignment and/or forwarding. This is mainly needed to avoid
% unnecesary data copies when doing the assignment inside a try-catch.
    idxOpTypelvl2 = idxOp(2).Type;
    if isempty(indiceslvl2)
        % Either the second level is dot subscripting or it does not require any
        % modification to the indices.
        var.(idxOp(2:end)) = rhs;
    elseif length(idxOp) == 2
        if idxOpTypelvl2 == matlab.indexing.IndexingOperationType.Brace
            var{indiceslvl2{:}} = rhs;
        else % Paren or ParenDelete
            var(indiceslvl2{:}) = rhs;
        end
    else % length(idxOp) > 2
        if idxOpTypelvl2 == matlab.indexing.IndexingOperationType.Brace
            var{indiceslvl2{:}}.(idxOp(3:end)) = rhs;
        else % Paren or ParenDelete
            var(indiceslvl2{:}).(idxOp(3:end)) = rhs;
        end
    end
