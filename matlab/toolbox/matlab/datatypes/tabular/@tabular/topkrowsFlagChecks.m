...
if nargin < 2 % default is sort by all data vars, do not treat [] as "default behavior"
    ...
    try
        ...
        if any(vars == 0)
            if isequal(vars,0) 
                labels = {a.metaDim.labels{1}}; %#ok<CCAT1>
            else
        ...
        throwAsCaller(throwInstead(ME,{'MATLAB:badsubscript'},'MATLAB:table:topkrows:BadNumericVarIndices'));
    end
end
...
if nargin < 3
    ...
    if rem(numel(varargin),2) == 0
        ...
        if isempty(sortMode) && ...
           (ischar(sortMode) || iscellstr(sortMode) || isa(sortMode,'double')) %#ok<ISCLSTR>
            % Empty direction allowed because of legacy sortrows behavior
        else
            if ischar(sortMode)
                sortMode = cellstr(sortMode);
            elseif ~iscellstr(sortMode) %#ok<ISCLSTR>
