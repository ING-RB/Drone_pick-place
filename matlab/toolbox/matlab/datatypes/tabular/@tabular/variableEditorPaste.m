...
import matlab.internal.tabular.private.subsasgnRecurser
...
if isscalar(columns) && columns(1) == 0 && ...
...
if ~isempty(I)
    ...
    for k=1:length(I)
        if istabular(data)
            if data.varDim.length == 1
                % Allow pasting of a single cell selection into multiple cells
                variableData = data(:, 1);
            else
                variableData = data(:, k);
            end
            ...
            s = substruct('.',varNames{I(k)},'()',{rows,1:size(variableData,2)});       
            if classes{I(k)} == "cell"
                this = subsasgnRecurser(this,s,variableData);
            elseif isCatClass(I(k)) % categorical or its subclasses
                ...
                try
                    this = subsasgnRecurser(this,s,cell2mat(variableData));
                catch %#ok<CTCH>
                    this = subsasgnRecurser(this,s,variableData);
                end
            end
...
if appendedColumnCount>0    
    ...
    if istabular(data)
        ...
        data = data(:, end-appendedIndices+1:end);
    else
        ...
        if istabular(data)
            ...
            try
                ...
                if all(cellfun(@(x) isnumeric(x),variableData))
                    this(rows, lastVariableIndex+index) = table(cell2mat(variableData));
                else
                    this(rows, lastVariableIndex+index) = table(variableData);
                end
            catch %#ok<CTCH>
                this(rows, lastVariableIndex+index) = table(variableData);
            end
...
ind = 1;
varName = newVarName;
while matches(varName,varNames)
