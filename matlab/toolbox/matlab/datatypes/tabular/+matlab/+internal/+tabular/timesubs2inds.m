function locs = timesubs2inds(subsMillis,labelsMillis,tolMillis,doubledoubleMinus)
% TIMESUBS2INDS Convert datetime or duration subscripts to indices.

%   Copyright 2022 The MathWorks, Inc.

if isscalar(tolMillis), tolMillis = repmat(tolMillis,size(subsMillis)); end

numSubs = numel(subsMillis);
numLabels = numel(labelsMillis);
if numLabels == 0
...
elseif numSubs <= 50 % better performance for small number of subscripts
    subsMillis = subsMillis(:);
    labelsMillis = labelsMillis(:);
    ...
    for i = 1:numSubs
        if doubledoubleMinus % use double-double substraction returning a double
            d = matlab.internal.datetime.datetimeSubtract(labelsMillis,subsMillis(i)); % don't need full precision
        else % use core minus
    ...
    locs = zeros(numSubs,2); % unsorted locations in A and B for matches, preliminary guess at length

    % Presort subscripts and row labels.
    [subsMillis,ordSubs] = sort(subsMillis); % A
    [labelsMillis,ordLabels] = sort(labelsMillis); % B
    ...
    for i = 1:numSubs
        ...
        while true % catch B up to A
            if doubledoubleMinus % use double-double substraction returning a double
                d_ij = matlab.internal.datetime.datetimeSubtract(labelsMillis(j),subsMillis_i); % don't need full precision
            else % use core minus
        ...
        if abs(d_ij) <= tolMillis_i ... % A - tol <= B <= A + tol, found finite A in B
            ...
            k = k + 1; % advance C
            locs(k,1) = ordSubs(i); locs(k,2) = ordLabels(j); % save A's and B's unsorted positions
            ...
            if j < numLabels
                ...
                while true % step through B
                    if doubledoubleMinus
                        d_ij = matlab.internal.datetime.datetimeSubtract(labelsMillis(jj),subsMillis_i); % don't need full precision
                    else
                        d_ij = labelsMillis(jj) - subsMillis_i;
                    end
                    if ~((d_ij <= tolMillis_i) || (labelsMillis(jj) == subsMillis_i && ~isnan(tolMillis_i))), break, end
                    k = k + 1; % advance C
                    locs(k,1) = ordSubs(i); locs(k,2) = ordLabels(jj); % save A's and B's unsorted positions
        ...
        elseif d_ij > tolMillis_i ... % B > A + tol, A needs to catch up
            ...
            k = k + 1; % advance C
            locs(k,1) = ordSubs(i); % save A's unsorted position and a zero
            ...
            locs(k+(1:(numSubs-i+1)),1) = ordSubs(i:numSubs); % save A's unsorted positions and zeros
            break
        end
    end
