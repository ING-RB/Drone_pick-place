function [c,ia,ib] = intersect(ain,bin,varargin) %#codegen
...
if isa(ain,'categorical') && isa(bin,'categorical')
    coder.internal.errorIf(ain.isOrdinal ~= bin.isOrdinal, ...
                           'MATLAB:categorical:setmembership:OrdinalMismatch','SETDIFF');
    a = ain;
    b = bin;
elseif isa(ain,'categorical')
    coder.internal.assert(matlab.internal.coder.datatypes.isScalarText(bin) || ...
                          ...
                          'MATLAB:categorical:setmembership:TypeMismatch','SETDIFF');
    a = ain;
...
[ccodes,ia,ib] = categorical.setmembershipHelper(@intersect,acodes,bcodes,varargin{:});
