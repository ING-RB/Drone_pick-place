function [c,ia,ib] = union(ain,bin,varargin) %#codegen
...
if isa(ain,'categorical') && isa(bin,'categorical')
    coder.internal.errorIf(ain.isOrdinal ~= bin.isOrdinal, ...
                           'MATLAB:categorical:setmembership:OrdinalMismatch','UNION');
    a = ain;
    b = bin;
elseif isa(ain,'categorical')
    coder.internal.assert(matlab.internal.coder.datatypes.isScalarText(bin) || ...
                          ...
                          'MATLAB:categorical:setmembership:TypeMismatch','UNION');
    a = ain;
...
[ccodes,ia,ib] = categorical.setmembershipHelper(@union,acodes,bcodes,varargin{:});
