...
coder.internal.assert(isa(t,'matlab.internal.coder.tabular'), 'MATLAB:table2array:NonTable');
...
if nvars > 0
    ...
    for i = 1:nvars
        allcells = isequal(class(t_vars{i}),'cell');
        if ~allcells, break, end
    end
    coder.internal.errorIf(allcells,'MATLAB:table2array:AllCellVariables');
end
a = t{:,:};
