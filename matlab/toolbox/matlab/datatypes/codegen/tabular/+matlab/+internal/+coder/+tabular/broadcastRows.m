function arrayOut = broadcastRows(arrayIn, nRowsOut, idxRowDestIn) %#codegen
...
%  necessary using matlab.internal.coder.datatypes.defaultarrayLike rules.
%
...
%       21    24    27
%
% See also matlab.internal.coder.datatypes.defaultarrayLike.

% Copyright 2020-2021 The MathWorks, Inc.

if isa(arrayIn,'tabular')
    assert(false);
% TODO: implement broadcastRows for nested tables in codegen.
%    
%     % Convert row-index to a logical index of the correct size.
%     idxRowDestCopy = idxRowDestIn;
...
%     arrayOut(~idxRowDest,:) = matlab.internal.coder.datatypes.defaultarrayLike(sizeOfDefaultData,'Like',arrayIn);
else
    szOut = size(arrayIn); szOut(1) = nRowsOut;
    arrayOut = matlab.internal.coder.datatypes.defaultarrayLike(szOut,'Like',arrayIn);
    if islogical(idxRowDestIn)
        idxRowDest = find(idxRowDestIn);
    else
        idxRowDest = idxRowDestIn;
    end
    if iscell(arrayIn)
        for i = 1:numel(idxRowDest)
            szIn = size(arrayIn);
            for j = 1:prod(szIn(2:end))
                arrayOut{idxRowDest(i),j} = arrayIn{i,j};
            end
        end
    else
        if ~(isempty(idxRowDestIn) || isempty(arrayIn))
            arrayOut(idxRowDestIn,:) = reshape(arrayIn,size(arrayIn,1),[]);
        end
    end
end
