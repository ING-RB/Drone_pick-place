...
% obtained from another calendarDuration array.

% Copyright 2014-2016 The MathWorks, Inc.

if isa(data, 'table')
    % try converting the table to an array.  If it is an array of
    % calendarDurations, the paste will succeed.  Otherwise, if it can't be
    % converted to an array or it isn't an array of calendarDurations, it
    % will fail below and the user will receive an appropriate error
    % message.
    try
...
% Paste data onto existing calendarDuration variables
s = substruct('()', {rows,columns});
