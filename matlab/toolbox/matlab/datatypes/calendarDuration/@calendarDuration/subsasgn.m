...
try
    ...
    switch s(1).type
    case '()'
        % Normally, only creation multi-level paren assignments like
        % d(i).Property = val come here, and simple paren assignments like
        % d(i) = val go directly to parenAssign. However, someone (including
        % tabular, for assignments like t.d(i) = val when d is a
        % calendarDuration) can call subsasgn explicitly.
