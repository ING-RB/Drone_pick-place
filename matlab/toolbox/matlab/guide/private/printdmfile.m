...
%                           Default is 0.
%     excludedPropertyList  - (cell array of structures), type/properties fields
%                           enumerate the properties that will not be 
%                           generated in the output m file. 
%                           Example:
%                             o.excludedPropertyList{1}.type = 'uicontrol'
%                             o.excludedPropertyList{1}.properties = 
%                                     {'Enable', 'ButtonDownFcn'};
%                             o.excludedPropertyList{2}.type = 'figure'
%                             o.excludedPropertyList{2}.properties = 
%                                     {'Position'};
...
% In this revision we'll just write out regular objects without accounting
% for any annotation layer stuff
...
if ~isempty(mat)
   ...
   if isfield(options, 'showMessageForMATFile') & options.showMessageForMATFile ==1 
       warning(message('MATLAB:guide:printdmfile:KeepFilesTogether', Mat_filename, M_filename)); 
   end 
end
...
if ~ishandle(handle)
    error(message('MATLAB:guide:printdmfile:InvalidHandle'));
end


% Check if 'handle' refers to a figure
if ~strcmp(get(handle,'type'),'figure')
    error(message('MATLAB:guide:printdmfile:InvalidFigureHandle'));
end
...
% Get the path of the filename specified
...
% g1975759: cannot export to M-file if there is an axes
if strcmpi('scribe.scribeaxes', Type)
    SUP = 0;
end

if strncmpi('graph',Type,5)
    if isempty(gcbo)
        disp(getString(message('MATLAB:guide:printdmfile:ObjectsAreNotSupported',Type)))
        disp(getString(message('MATLAB:guide:printdmfile:Skipping')))
    else
        warndlg(getString(message('MATLAB:guide:printdmfile:DialogObjectsAreNotSupported',Type)));
    end
    SUP = 0;
end
...
if ~isempty(gS.contextmenu)
    ...
    for i=1:length(gS.contextmenu)
        ...
        if ~isempty(menudata.special)
            indices = find(ismember(gS.oldhandlelist,menudata.special));
            if ~isempty(indices)
               str = ' ';
               for k=1:length(indices)
                   str = [str, 'h', num2str(indices(k)),' '];
               end
...
% If field 'SerializedAnnotationV7' exists in the appdata, ignore it.
% Fix for to g1055682 suppress the annotation field generated in HG1 
% hgsave, which was fixed in g881142.
if isfield(thisappdata, 'SerializedAnnotationV7')
    thisappdata = rmfield(thisappdata, 'SerializedAnnotationV7');
end
...
%     Stop generating code for the unsupported properties in HG2 in GUIDE 
%     for g991044. The properties include:
%     uicontrol: 'Clipping'
%     uibuttongroup: 'OldSelectedObject'
%     (resolved in HG2 in g984479)
%     text: 'DVIMode' and 'IncludeRenderer'
%     (resolved in HG2 in g991166)
    if (strcmp(Type, 'uicontrol') && strcmp(PropName, 'Clipping')) ||...
       (strcmp(Type, 'uibuttongroup') && strcmp(PropName, 'OldSelectedObject')) ||...
       (strcmp(Type, 'text') && (strcmp(PropName, 'DVIMode') || strcmp(PropName, 'IncludeRenderer')))||...
       (strcmp(PropName, 'EraseMode'))||...
       (strcmp(PropName, 'Internal'))
        continue;
    end
    
    value = Props.(PropName);
    realValue = [];
    if ObjectExist && isempty(find(ismember(Callbacks,PropName),1))
        try 
    ...
    % Account for OnOffSwitchState
    if ~isempty(value) && isa(value, 'matlab.lang.OnOffSwitchState')
        value = char(value);
    end
    
    if ~isempty(realValue) && isa(realValue, 'matlab.lang.OnOffSwitchState')
        realValue = char(realValue);
    end
    ...
    defaultValue = [];
    try 
        defaultValue = get(0,defaultName);
    catch ex
    end
...
elseif isstruct(value) 
    ...
    found = false;
    % handle anonymous function handle case
    if isa(value, 'function_handle')
        afn = func2str(value);        
        if isequal(strfind(afn, '@'),1)
            str = afn;
            found =true;
        end
    end
    ...
    if ~found
        error(message('MATLAB:guide:printdmfile:UnsupportedDataType')); 
    end
end
...
dim =size(value); 
if (length(dim)>2) 
    error(message('MATLAB:guide:printdmfile:UnableToGenerateCodeForChar')); 
end 
...
if (length(dim)>2) 
    error(message('MATLAB:guide:printdmfile:UnableToGenerateCodeForCell')); 
end 
...
if fid < 1
    error(message('MATLAB:guide:printdmfile:ErrorWritingFile'));
end
...
creator =[...
    ...
    '   if isa(createfcn,''function_handle'')', NL,...
    '       createfcn(hObject, eventdata);', NL,...
    '   else',NL,...
    '       eval(createfcn);',NL,...
    '   end',NL,...
    'end', NL];
...
%    oldLen = length(file);
    ...
    if ~isempty(callbacks)
        ...
        for i=1:length(names)
            ...
            if (ismember(names(i), callbacks) && ~isempty(value))
                newVal = guidemfile('renameThisCallback', hS.handle,char(names(i)), file, mfile, false);
                if ~isempty(newVal)
...
% remove all instance properties, if any
...
% remove properties that are in the excludedPropertyList, if any
...
if strcmpi(hS.type,'figure')
    ...
    % If figureVisibility option was specified as not-default, use that
    % option for the figure's Visible property.  Else, use the default 'off'.
    if isfield(options,'figureVisibility') && ~strcmp(options.figureVisibility,'default')
        props.Visible = options.figureVisibility;
    end
end
