...
try
    ...
    switch action
        ...
        case 'exportAppDesigner'
            varargout = openAppMaintenancePage(varargin{:},'migrate');

        case 'getProperties'
            varargout = getProperties(varargin{:});

        case 'helpGettingStarted'
             helpview('matlab', 'guide_getting_started');
        ...
        case 'openAppMaintenancePageToMigrate'
            varargout = openAppMaintenancePage(varargin{:},'migrate');
            
        case 'openAppMaintenancePageToExport'
            varargout = openAppMaintenancePage(varargin{:},'export');
...
% ****************************************************************************
% Open the GUIDE App Maintenance Options Page.
% This function is executed after the GUIDE Banner Button is clicked.  In
% this case, there is an open app in GUIDE.  The input to this function, 
% varargin, is the legacy HG1 handle to the open file.  If an unsaved app
% is opened in GUIDE, varargin is an empty double.
% ****************************************************************************
function out = openAppMaintenancePage(varargin)
out = {};

% Get the file name of the app opened in GUIDE.
fig = varargin{1};
filename = get(fig, 'filename');

% Launch the GUIDE App Maintenance Options app and pass in the filename of the opened
% app.
guide.internal.launchGUIDEAppMaintenanceOptions(filename, varargin{2});
...
% Inputs:
% - Input 1: handle to the opened Figure that will be exported
% - Input 2: optional. If specified as non-empty, this input is the
% full file name (including path), that the exported file will be saved as.
% - Input 3: optional.  If specified as non-empty, this input can
% supress the warning that appears after the export process.  A value of
% false indicates that the warning will not be shown.  A value of true
% indicates that the warning will be shown.
% - Input 4: optional.  Has no effect if it is specified as char 'default'.  If
% specified as not-default, this input specifies the Visible property of
% the Figure that is being exported.  If 'on', the figure will be exported
% as 'Visible' 'on'.  If 'off', the figure will be exported as 'Visible
% 'off'.  This allows accurate app export while giving users a good
% experience.
...
if success && ~istemp
    ...
    % Obtain the destination file name.  If the optional second argument is 
    % non-empty, the destination file name was specified as a second input argument.
    if nargin < 2 || isempty(varargin{2})
        [exportmfile, filterindex] = getOutputFilename(fig, filter, title, [infilename '_export.m']);
    else 
        exportmfile = varargin{2};
        filterindex = 1;
    end
    ...
    if ~isempty(exportmfile)
        ...
        % An optional third argument provides the capability to supress the
        % warning message that appears after the export process.
        if nargin >= 3 && ~isempty(varargin{3})
            choices.showMessageForMATFile = varargin{3};
        else
            choices.showMessageForMATFile = 1;
        end
        
        % An optional fourth argument provides the capability to specify
        % the Visible property of the exported GUIDE App.
        if nargin == 4 && ~isempty(varargin{4})
            choices.figureVisibility = varargin{4};
        else
            choices.figureVisibility = 'default';
        end
...
if ~isempty(prefstring)
    ...
    if pref
        if isappdata(fig,'GUIDELayoutEditor')
            ...
            frame = matlab.ui.internal.JavaMigrationTools.suppressedJavaFrame(fig);
...
%remove annotationpane. see g2448943.
if ~isempty(all)
    all(ishghandle(all, 'annotationpane')) = []; 
end
units= get(all,'units');
...
%remove annotationpane. see g2448943.
if ~isempty(all)
    copies(ishghandle(copies, 'annotationpane')) = []; 
end
for i=1:length(all)
...
switch info.Type
    case ACTIVEX
        ...
        instance = matlab.ui.internal.JavaMigrationTools.suppressedActXControl(info.ProgID, pos, double(fig));
end
...
if isappdata(0, 'MathWorks_GUIDE_testmode')
    ...
    index = find(ismember(list{1}, 'MSVidCtl.MSVidCtl.1'));
    info = {char(list{2}(index)), char(list{1}(index))};
else
    [h, info] = matlab.ui.internal.JavaMigrationTools.suppressedActXControlselect('User',getDialogTitle());
end
...
controls = matlab.ui.internal.JavaMigrationTools.suppressedActXControllist;
out{1} = controls(:,2);
out{2} = controls(:,1);
...
for i=numChildren:-1:1
    ...
    % warning can be thrown
     if ( strcmp(childObjectType,'legend') ||...
         ...
         (isa(childObjectHandle,'matlab.graphics.axis.AbstractAxes') && ~isa(childObjectHandle, 'matlab.graphics.axis.Axes')) %g1750686
