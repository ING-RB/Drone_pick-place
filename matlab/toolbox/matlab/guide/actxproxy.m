    ...
    if ishandle(hObject) && isappdata(hObject, 'Control')
        ...
        try
            ...
            if isappdata(0, genvarname(['OpenGuiWhenRunning_', fname])) && ~isfield(data,'keepactivexposition')
                ...
                [activex, container] = matlab.ui.internal.JavaMigrationTools.suppressedActXControl(control.ProgID, getpixelposition(hObject), fig);
                set(container,'units',get(hObject,'units'));
            else
                % This is coming from loading a GUI in GUIDE
                activex = matlab.ui.internal.JavaMigrationTools.suppressedActXControl(control.ProgID, getpixelposition(hObject), fig);
            end
        catch me
            delete(hObject);
