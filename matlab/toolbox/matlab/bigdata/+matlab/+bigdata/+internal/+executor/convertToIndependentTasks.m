...
%
%  [..] = convertToIndependentTasks(..,'GetCacheStoreFunction',@myGetCacheStoreFunction,..)
%
%  [..] = convertToIndependentTasks(..,'ResolvePartitionStrategyFunction',@myResolvePartitionStrategyFunction,..)
%
...
%
% ResolvePartitionStrategyFunction:
%
% The function handle ResolvePartitionStrategyFunction resolves a
% partition strategy in the same way as the environment uses for execution.
% The signature of this function handle must be:
%   partitionStrategy = resolvePartitionStrategy(partitionStrategy)
%
...
%  3. Wrap each still-connected sub-graph into a single task that has no
...
p.addParameter('ResolvePartitionStrategyFunction', @(~) 1);
p.parse(varargin{:});
factories = p.Results;
...
%  - inputExecutions: An array of pass-forward executions generated by the
...
switch communicationType
    ...
    case OutputCommunicationType.Broadcast
        ...
        [actionableExecution, passforwardExecution] = iCreateBroadcastExecution(execution, task);
        stageTask = stageTask.addExecution(actionableExecution, task.IsExecutionLarge);
...
if isOutput
    ...
    elseif task.OutputPartitionStrategy.IsBroadcast || isempty(factories.CreateStreamFactoryFunction)
        ...
        [outputExecution, ~] = iCreateBroadcastExecution(execution, task);
        stageTask = stageTask.addExecution(outputExecution, task.IsExecutionLarge);
        stageTask = stageTask.addBroadcastOutput(task);
    else
...
if task.OutputCommunicationType == OutputCommunicationType.AnyToAny
    outputPartitionStrategy = feval(factories.ResolvePartitionStrategyFunction, task.OutputPartitionStrategy);
    execution = CompositeDataProcessorBuilder(inputExecutions, task.DataProcessorFactory, outputPartitionStrategy);
else
...
numOutputs = task.NumOutputs;
requiresPartitionIndices = (task.OutputCommunicationType == OutputCommunicationType.AnyToAny);
writerExecution = CompositeDataProcessorBuilder(execution, ...
    @(partition) LocalWriteProcessor(writerFactory(partition), numOutputs, requiresPartitionIndices));
readerExecution = CompositeDataProcessorBuilder(CompositeDataProcessorBuilder.empty(), @(partition) LocalReadProcessor(readerFactory(partition), numOutputs));

function [setterExecution, getterExecution] = iCreateBroadcastExecution(execution, task)
% Create a pair of CompositeDataProcessorBuilder objects that represents
% setting and getting a broadcasted value.
import matlab.bigdata.internal.executor.BroadcastProcessorFactory;
import matlab.bigdata.internal.executor.BroadcastedConstantProcessorFactory;
import matlab.bigdata.internal.executor.CompositeDataProcessorBuilder;
setterExecution = CompositeDataProcessorBuilder(execution, BroadcastProcessorFactory(task.Id, task.NumOutputs));
getterExecution = CompositeDataProcessorBuilder(CompositeDataProcessorBuilder.empty(), BroadcastedConstantProcessorFactory(task.Id));
...
streamWriterExecution = CompositeDataProcessorBuilder(execution, ...
    @(partition) LocalWriteProcessor(streamWriterFactory(partition), numOutputs));
...
import matlab.bigdata.internal.executor.CompositeDataProcessorBuilder;
import matlab.bigdata.internal.executor.GatedPassthroughProcessorFactory;
import matlab.bigdata.internal.io.CacheProcessorFactory;

% First add a GatedPassthroughProcessor factory that tells the framework
% that input is only needed if there does not exist a cache entry.
cacheEntryKey = task.CacheEntryKey;
cacheStoreFcn = factories.GetCacheStoreFunction;
numVariables = task.NumOutputs;
execution = CompositeDataProcessorBuilder(execution, ...
    GatedPassthroughProcessorFactory(...
    @(partition) ~hasEntry(feval(cacheStoreFcn), cacheEntryKey, partition.PartitionIndex), numVariables)...
    );

% Now add the CacheProcessor factory itself.
execution = CompositeDataProcessorBuilder(execution, ...
    CacheProcessorFactory(task.CacheEntryKey, factories.GetCacheStoreFunction, numVariables));
