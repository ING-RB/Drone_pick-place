...
classdef (Sealed) BroadcastMap < handle
    ...
    methods
        ...
        function append(obj, other)
            % Add all Broadcast key/value pairs from one Broadcast map to
            % another.
            otherMap = other.Map;
            keys = otherMap.keys;
            
            for ii = 1:numel(keys)
                [partitionIndices, values] = getPartitions(other, keys{ii});
                setPartitions(obj, keys{ii}, partitionIndices, values);
            end
        end
        
        function other = subset(obj, keys)
            % Filter to a subset of the broadcast keys.
            other = matlab.bigdata.internal.executor.BroadcastMap;
            
            for ii = 1:numel(keys)
                [partitionIndices, values] = getPartitions(obj, keys{ii});
                setPartitions(other, keys{ii}, partitionIndices, values);
            end
        end
    end
end
