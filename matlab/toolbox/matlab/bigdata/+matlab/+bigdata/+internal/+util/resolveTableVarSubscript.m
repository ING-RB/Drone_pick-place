...
elseif iscellstr(subscript) %#ok<ISCLSTR> strings already converted to cellstr
    varNames = subscript;
elseif matlab.bigdata.internal.util.isColonSubscript(subscript)
    varNames = tableVarNames;
elseif isa(subscript, "pattern")
    varNames = tableVarNames(matches(tableVarNames, subscript));
