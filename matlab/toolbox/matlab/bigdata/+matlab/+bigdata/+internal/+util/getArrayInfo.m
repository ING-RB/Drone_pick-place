...
% Check upfront if a row of the tall array is so big that head might run
% out of memory and fail.
isRowReadCheap = true;
if ~isempty(adaptor.SmallSizes)
    numElementsInRow = prod(adaptor.SmallSizes);
    % If each row of the tall array contains more than 10 million elements,
    % flag it so that later on we avoid reading the tall array to get its
    % preview. This threshold is considered assuming that we have approx.
    % 1GB of available memory to gather a preview of 9 rows of complex
    % double in the tall array.
    isRowReadCheap = isfinite(numElementsInRow) && numElementsInRow < 10e6;
end

% By default, set up the available information in the adaptor. This will be
% later replaced with the preview if possible.
...
elseif isRowReadCheap && matlab.bigdata.internal.util.isPreviewCheap(partitionedArray)
