...
classdef TallSize < handle
    ...
    methods (Static)
        ...
        function obj = buildKnownSize(val)
            assert(isnumeric(val) && isscalar(val) && isfinite(val), ...
                'Assertion failed: Tall size must be a finite scalar numeric.');
    ...
    methods
        ...
        % Update the size - must be scalar double
            ...
            if ~(isnan(obj.SizeImpl) || obj.SizeImpl == newVal)
                matlab.bigdata.internal.throw( ...
                    message('MATLAB:bigdata:array:BadTallSizeKnown', newVal, obj.SizeImpl));
            end
            
            if obj.IsDefinitelyNonUnity && newVal == 1
                matlab.bigdata.internal.throw( ...
                    message('MATLAB:bigdata:array:BadTallSize', newVal));
            end
            if obj.IsDefinitelyNonZero && newVal == 0
                matlab.bigdata.internal.throw( ...
                    message('MATLAB:bigdata:array:BadTallSize', newVal));
            end
            obj.SizeImpl             = newVal;
