...
if ~istall(subs{1})
    ...
    if isnumeric(subs{1})
        verifyNumericSubscript(subs{1}, 1, inf);
    end
...
elseif tall.getClass(subs{1}) == "logical"
    ...
    % Treat row indexing with a tall logical vector as a special operation
    % for potential optimization afterwards if it's done on tabular-based
    % data. logicalRowSubsref will behave as filterslices.
    outPa = logicalRowSubsref(hGetValueImpl(subs{1}), outPa);
...
if isscalar(subs) && ~istall(subs{1})
...
if isColonDescriptor(sub)
    resolved = sub;
    % Make sure we don't try to check an empty expression
    if numel(sub) > 0
        ...
        isAllRealIntegerGE1 = true;
    end
    
    outTallSize = numel(resolved);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [subInChunk, overallMinAndMax, reverse, isVector] = iResolveColonDescriptorSub1ThisChunk(...
    inSub, chunkOffset, chunkTallSize)

if inSub.Stride == 0
    % Subscript of form 1:0:7 - MATLAB treats this as empty.
    subInChunk = zeros(1,0);
    overallMinAndMax = [NaN,NaN];
    reverse = false;
    isVector = true;
    return;
end
...
newStart = A + n1 * D;
newStop  = A + n2 * D;
subInChunk = (newStart:D:newStop) - chunkOffset;

% Calculate overall start and stop. This has to account for when stride
% ignores elements near the end.
overallMinAndMax = [minValue(inSub), maxValue(inSub)];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [subInChunk, overallMinAndMax, reverse, isVector] = iResolveEndMarkerSub1ThisChunk(...
...
overallMinAndMax = [min(numericSub), max(numericSub)];

subInChunk = numericSub - chunkOffset;
valid    = subInChunk > 0 & subInChunk <= chunkTallSize;
subInChunk = subInChunk(valid);
end
...
function [subInChunk, overallMinAndMax, reverse, isVector] = iResolveColonFormSub1ThisChunk(...
    inSub, chunkOffset, chunkTallSize, totalTallSize)

if isColonDescriptor(inSub)
    [subInChunk, overallMinAndMax, reverse, isVector] = iResolveColonDescriptorSub1ThisChunk(...
        inSub, chunkOffset, chunkTallSize);
else
    [subInChunk, overallMinAndMax, reverse, isVector] = iResolveEndMarkerSub1ThisChunk(...
        inSub, chunkOffset, chunkTallSize, totalTallSize);
end
if ~isempty(overallMinAndMax) && overallMinAndMax(2) > totalTallSize
    error(message('MATLAB:badsubscript', ...
...
[sub1InChunk, sub1OverallMinMax, reverse, isSub1Vector] = ...
...
out             = x(sub1InChunk, subs{2:end});

% When there is only one subscript, we have to prevent the row nature of
% subscript converting the input column vector into a row vector.
if isscalar(subs) && iscolumn(x) && ~iscolumn(out)
    out = out.';
end
...
if info.RelativeIndexInPartition ~= 1
    ...
    % Return reverse of the same height as out for compatibility with
    % grouping functions, all of the outputs must be of the same height.
    reverse = repmat(reverse, size(out, 1), 1);
end
...
if isempty(subs{1})
    hasFinished = true;
elseif isempty(sub1OverallMinMax)
    hasFinished = info.IsLastChunk;
else
    allRequestedSlicesAreAfterThisPartition = sub1OverallMinMax(1) >= partitionOffsets(thisPartition+1);
    thisChunkFinalGlobalIndex               = thisChunkOffset + size(x, 1);
    allRequestedSlicesHaveBeenSupplied      = sub1OverallMinMax(2) <= thisChunkFinalGlobalIndex;
    hasFinished = info.IsLastChunk || ...
