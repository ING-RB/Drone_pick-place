%DatetimeFamilyAdaptor Adaptor class for datetime, duration and calendarDuration data
    ...
    methods (Access=protected)
        ...
        function sample = buildSampleImpl(obj, ~, sz, ~)
            ...
            if clz == "datetime"
                ...
                sample.TimeZone = obj.Proto.TimeZone;
            elseif clz == "duration"
                sample = repmat(milliseconds(1), sz);
            else
                sample = repmat(caldays(1), sz);
            end
            % Ensure metadata is copied from the held prototype to the
            % sample.
            sample = [obj.Proto([]); sample];
        end
    end
