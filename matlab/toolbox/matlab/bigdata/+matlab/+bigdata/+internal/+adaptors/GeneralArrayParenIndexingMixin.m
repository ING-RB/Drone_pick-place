...
classdef GeneralArrayParenIndexingMixin
    methods
        function out = subsrefParens(obj, pa, szPa, S)
            ...
            % Use a sample to do some up-front error checking of small dims
            if numel(S.subs)>=2 && ~isempty(obj.SmallSizes) && ~any(isnan(obj.SmallSizes)) %#ok<MCNPN>
                sample = obj.buildSample("double"); %#ok<MCNPN>
                myS = S;
                myS.subs{1} = ':';
                % Ignore end markers - replace with :
                isEnd = cellfun('isclass', S.subs, 'matlab.bigdata.internal.util.EndMarker');
                myS.subs(isEnd) = {':'};
                % Call in-memory subsref, which will throw for bad indices or
            ...
            isGuaranteedColumnVector = obj.NDims == 2 && obj.getSizeInDim(2) == 1; %#ok<MCNPN>
            inTallSize = obj.getSizeInDim(1); %#ok<MCNPN>
            isGuaranteedNotScalar = isGuaranteedColumnVector && ~isnan(inTallSize) && inTallSize ~= 1;
            if isscalar(S(1).subs) && isGuaranteedColumnVector && isGuaranteedNotScalar
        ...
        function out = subsasgnParens(obj, pa, ~, S, b)
            ...
            if istall(b)
                ...
                if isscalar(S.subs) && obj.isKnownNotColumn() %#ok<MCNPN>
                ...
                if istall(subs{1})
                    ...
                    smallSubs = subs(2:end);
                    pa = generalpartitionfun(...
                        @(info, ca, cb, filter) iApplyTallWithTallIdx(info, ca, cb, filter, smallSubs{:}),...
            ...
            elseif isscalar(b)
                if istall(subs{1})
                    smallSubs = subs(2:end); % Make sure subs doesn't get included in the anonymous function below.
                    pa = slicefun(@(pa, filter) iApplyScalar(pa, b, filter, smallSubs{:}), pa, hGetValueImpl(subs{1}));
                else
        ...
        function out = subsasgnParensDeleting(obj, pa, szPa, S)
            ...
            elseif isColonSubscript(1)
                ...
                if ~isnan(obj.NDims) && obj.NDims ~= numel(S.subs) %#ok<MCNPN>
...
if isscalar(varargin)
    if isequal(size(varargin{1}), size(A)) || ...
        ...
        isNDVector = (nnz(size(A)~=1) == 1); % Only one non-unity dimension
...
oldSzA = size(A);
% Empty inputs A of the form (N, 0) or (N, 0, M) are not supported to
% prevent empty chunks of the form [] to grow. [] is treated as a special
% case as if there was no data initialized.
if oldSzA(2) == 0
    error(message('MATLAB:bigdata:array:SubsasgnScalarNonEmptyInputs'));
end
varargin(2:end) = iResolveEndMarkers(oldSzA, 2, varargin(2:end));
A(varargin{:}) = B;
% Check if there is any growth in the first dimension, this will throw a
% more comprehensive error when an invalid chunk is generated.
if size(A, 1) > oldSzA(1)
    error(message('MATLAB:bigdata:array:SubsasgnScalarInvalidGrowth'));
end
end
...
if isscalar(varargin) && ~iscolumn(A)
    error(message('MATLAB:bigdata:array:SubsasgnSingleSubDims'));
end
...
numSubs = numel(smallSubscripts);
foldTrailingDimensions = (numSubs < numel(obj.SmallSizes));
if foldTrailingDimensions
    % Too few subscripts - fold the trailing dimensions
    indexingSizeIn = [obj.SmallSizes(1:numSubs-1), prod(obj.SmallSizes(numSubs:end))];
elseif numSubs > numel(obj.SmallSizes)
    % Too many subscripts - pad size with ones
    indexingSizeIn = [obj.SmallSizes, ones(1, numSubs - numel(obj.SmallSizes))];
else
    % Right number of subscripts
    indexingSizeIn = obj.SmallSizes;
end

% Find the maximum referenced index in each dimension
indexingSizeOut = indexingSizeIn;
for idx = 1:numel(smallSubscripts)
    ...
    elseif isnumeric(sub)
        if ~isempty(sub)
            indexingSizeOut(idx) = max(max(sub(:)), indexingSizeIn(idx));
        end
    elseif islogical(sub)
        lastTrue = find(sub(:), 1, 'last');
        if ~isempty(lastTrue)
            indexingSizeOut(idx) = max(lastTrue, indexingSizeIn(idx));
        end
    elseif isColonDescriptor(sub)
        indexingSizeOut(idx) = max(sub.Stop, indexingSizeIn(idx));
    elseif isEndMarker(sub)
        if ~isnan(indexingSizeIn(idx))
            indexingSizeOut(idx) = max(max(resolve(sub, [indexingSizeIn(idx), 1], 1)), indexingSizeIn(idx));
        end
    elseif isobject(sub)
        sub = subsindex(sub) + 1;
        indexingSizeOut(idx) = max(sub(:));
    else
        indexingSizeOut(idx) = nan;
    end
end

% Last dimension cannot be grown if number of subscripts is less than
% number of dimensions of LHS.
if foldTrailingDimensions && indexingSizeOut(end) > indexingSizeIn(end)
    % MATLAB uses the ID indexed_matrix_cannot_be_resized but overrides the message.
    msg = message("MATLAB:matrix:ambiguousArrayGrowth");
    err = MException("MATLAB:indexed_matrix_cannot_be_resized", getString(msg));
    throwAsCaller(err);
end

% Work out the output size
if foldTrailingDimensions
    % Only the leading dimensions can grow. Trailing dimensions stay
    % unchanged.
    outSize = [indexingSizeOut(1:numSubs-1), obj.SmallSizes(numSubs:end)];
else
    outSize = indexingSizeOut;
end

obj = resetSmallSizes(obj, outSize);
end
