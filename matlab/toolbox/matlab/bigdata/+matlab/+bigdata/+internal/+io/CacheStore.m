...
classdef (Abstract) CacheStore < handle & matlab.mixin.Heterogeneous
    ...
    methods (Sealed)
        % Check if a given cache entry exists in this cache store.
        function tf = hasEntry(objs, cacheEntryKey, partitionIndex)
            tf = false;
            for obj = objs(:)'
                tf = tf || obj.doHasEntry(cacheEntryKey.Id, partitionIndex);
            end
        end
    ...
    methods (Access = private)
        % Check for the existence of a cache entry
        function tf = doHasEntry(obj, cacheEntryId, partitionIndex)
            tf = any(obj.CacheEntries.Id == cacheEntryId ...
                & obj.CacheEntries.PartitionIndex == partitionIndex);
        end
