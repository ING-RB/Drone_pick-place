...
classdef PagedRandomAccessMap < handle
    ...
    methods
        function out = get(obj, indices)
            ...
            pageIndices = matlab.bigdata.internal.util.discretizerows(indices, obj.PageBoundaries);
            ...
            for ii = 1 : numel(uniquePageIndices)
                if isPageInMemory(ii)
                    selectedIndices = unique(indexSlices(indices, ii == uniquePageIndicesMap), 'rows');
                    isSelected = quickIsmemberRows(obj.MemoryPages(idx(ii)).Indices, selectedIndices);
                    outIndices{ii} = indexSlices(obj.MemoryPages(idx(ii)).Indices, isSelected);
            ...
            for ii = 1 : numel(uniquePageIndices)
                if ~isPageInMemory(ii)
                    ...
                    selectedIndices = unique(indices(ii == uniquePageIndicesMap), 'rows');
                    isSelected = quickIsmemberRows(obj.MemoryPages(1).Indices, selectedIndices);
            ...
            [~, orderIdx] = quickIsmemberRows(indices, outIndices);
