...
classdef (Abstract, InferiorClasses = { ...
    ...
    methods (Abstract)
        ...
        %
        % Syntax:
        %  tA = logicalElementfun(functionHandle,tX,tY)
        %  tA = logicalElementfun(options,functionHandle,tX,tY)
        %
        %  This is a special primitive for row-indexing optimization on
        %  tabular data and its use is restricted to the logical operators:
        %  and, or, not. These are the only logical operators supported by
        %  matlab.io.RowFilter.
        %
        % See also: elementfun
        %
        varargout = logicalElementfun(options, functionHandle, varargin);

        %SMALLTALLCOMPARISON Apply an element-wise comparator with an in-memory scalar operand.
        %
        % Syntax:
        %  tA = smallTallComparison(functionHandle,smallOperand,isTallFirstArg,tX,tY)
        %  tA = smallTallComparison(options,functionHandle,smallOperand,isTallFirstArg,tX,tY)
        %
        %  This is a special primitive for row-indexing optimization on
        %  tabular data and its use is restricted by the current
        %  limitations of matlab.io.RowFilter (see
        %  tall/invokeBinaryComparison for further details).
        %
        % Inputs:
        %  - functionHandle must be an element-wise function handle of type
        %  function_handle or matlab.bigdata.internal.FunctionHandle. It
        ...
        %  function handle rules.
        %
        %  - smallOperand is an in-memory scalar operand to perform the
        %  comparison of the tall array against it.
        %
        %  - isTallFirstArg is an in-memory logical scalar that flags
        %  whether the tall array is the first argument of the comparison
        %  or not.
        %
        %  - Inputs [tX,tY,..] follow the general input rules with
        %  singleton expansion in all dimensions. In addition, all of
        ...
        % See also: elementfun
        %
        varargout = smallTallComparison(options, functionHandle, isTallFirstArg, varargin);
        
        %ASSERTADAPTORCORRECT Acts like elementfun and checks each element with the metadata in the adaptor.
        %
        % Syntax:
        %  tA = assertAdaptorCorrect(functionHandle,tX)
        %  tA = assertAdaptorCorrect(options,functionHandle,tX)
        %
        % Inputs:
        %  - functionHandle must be an element-wise function handle of type
        %  function_handle or matlab.bigdata.internal.FunctionHandle. It
        %  will be applied to the inputs [tX,tY,..] as per the chunk-wise
        %  function handle rules.
        %  - Input tX follows the general input rules with singleton
        %  expansion in all dimensions.
        %  - options (optional) must be a PartitionedArrayOptions object.
        %
        % Outputs:
        %  - Output tA will be a PartitionedArray instance of the same size
        %  and with the same data as tX if the data in tX matches the
        %  metadata as defined in functionHandle.
        %
        % Error Conditions:
        %  - If the data in the input doesn't match the metadata, it will
        %  assert with an assertion of the type: 'An internal consistency
        %  error occurred. Details:...'.
        %
        varargout = assertAdaptorCorrect(options, functionHandle, varargin);
        ...
        %LOGICALROWSUBSREF Subscripted reference of rows of a Partitioned Array with a Logical Partitioned Array as indices.
        %
        % Syntax:
        %  [tX2,tY2,..] = logicalRowSubsref(tSubs,tX,tY,..)
        %
        %  This is a special primitive for row-indexing optimization on
        %  tabular data that behaves as filterslices.
        %
        % See also: filterslices
        %
        varargout = logicalRowSubsref(subs, varargin);
        
        %SUBSREFTABULARVAR Subscripted reference of table variables of a Partitioned Array.
        %
        % Syntax:
        % [tX2,tY2,...] = subsrefTabularVar(subsrefType,subs,isGrouped,tX,tY,...);
        %
        % Inputs:
        % - subsrefType is a character vector or string with the subsref
        % type to perform: '()', '{}', or '.'
        %
        % - subs is a cell array with the subscripts values.
        %
        % - isGrouped is a logical, numeric flag specifying if the input is
        % a grouped variable. PartitionedArray is not a grouped variable,
        % it is set to false by default.
        %
        % - Each of [tX,tY,..] is a PartitionedArray
        %
        % Outputs:
        % - Each of [tX2,tY2,..] is a PartitionedArray with the same tall
        % size as [tX,tY] and small size as defined by the subscripts in
        % subs.
        %
        % Example:
        %  import matlab.bigdata.internal.lazyeval.LazyPartitionedArray;
        %
        %  % tX is a PartitionedArray of with a table of size 100x2
        %  tX = LazyPartitionedArray.createFromConstant(table(rand(100,1), rand(100,1)));
        %
        %  % Extract first variable of tX
        %  tY = subsrefTabularVar('()',{'Var1},false,tX);
        varargout = subsrefTabularVar(subsrefType, subs, false, varargin);
