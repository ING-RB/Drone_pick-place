...
classdef TracePlotter < handle & matlab.mixin.SetGet
    ...
    methods (Access = private)
        ...
        function idx = findEdges(obj, sourceIds, destId)
            ...
            assert(isscalar(destId), 'Assertion failed: Expected destId to be a scalar ID.');
...
if isfield(s, 'FunctionHandle')
    info.Function = char(matlab.bigdata.internal.util.unwrapFunctionHandle(s.FunctionHandle));
else
    info.Function = '';
end
