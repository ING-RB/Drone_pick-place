...
classdef CommunicationLogger < handle
    ...
    methods (Access = private)
        function handleExecuteBegin(obj, executionId, taskGraph)
            ...
            % Capture the size of operations to be sent to workers. Any
            % operation that is not evaluated as a broadcast task will be
            % broadcasted to workers.
            isNonBroadcastTask = ~arrayfun(@(x) x.ExecutionPartitionStrategy.IsBroadcast, taskGraph.Tasks);
            nonBroadcastProcessors = {taskGraph.Tasks(isNonBroadcastTask).DataProcessorFactory};
            numBytes = iGetSizeInBytes(nonBroadcastProcessors);
            entry = obj.createLogEntry("OpTree", "Client", "Broadcast", numBytes);
            obj.log(executionId, entry);
        end
        ...
        function handleProcessReturn(obj, processor, invokeData)
            ...
            % Instances of broadcast -> broadcast can only happen if the
            % client is broadcasting an array (e.g. scalar constants).
            source(destination == "Broadcast" & source == "Broadcast") = "Client";
        ...
        function openLog(obj, executionId)
            ...
            if numReferences == 0
                ...
                if isHdfs
                    location = strcat(obj.OutputFolder, '/', executionId, '/', workerFolder);
                else
                    location = fullfile(obj.OutputFolder, executionId, workerFolder);
                    if numLogFiles == 0
                        mkdir(location);
                    end
                end
                ...
                fileType = "auto";
                filePattern = "";
                writeFunction = createWriteFunction(fileType, location, filePattern, isIri, isHdfs);
...
function sz = iGetSizeInBytes(data)
% Get the size of a set of chunks in bytes.
if license('test', 'Distrib_Computing_Toolbox') ...
        && exist('distcompserialize', 'file') == 3
    % We use distcompserialize if available because whos does not give an
    % accurate size in bytes of handle based objects.
    sz = numel(distcompserialize(data));
else
    whosData = whos('data');
    sz = whosData.bytes;
end
