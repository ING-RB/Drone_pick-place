        ...
        function add(obj, isLastOfInputs, varargin)
            ...
            for ii = 1:numel(obj.Buffer)
                if obj.IsBufferInitialized(ii)
                    ...
                    obj.Buffer{ii} = cat(1, obj.Buffer{ii}, varargin{ii}{:});
                else
                    if ~isempty(varargin{ii})
                        obj.Buffer{ii} = matlab.bigdata.internal.util.vertcatCellContents(varargin{ii});
                        % Only Initialize the buffer if the result of the
                        % vertical concatenation is not an UnknownEmptyArray.
                        obj.IsBufferInitialized(ii) = ~matlab.bigdata.internal.UnknownEmptyArray.isUnknown(obj.Buffer{ii});
                    end
                    if isLastOfInputs(ii) && ~obj.IsBufferInitialized(ii)
                        % If we reach to the end of the partition and the
                        % buffer hasn't been initialized, we received
                        % UnknownEmptyArray or "zero" blocks, represent
                        % this with UnknownEmptyArray and initialize the
                        % buffer.
                        obj.Buffer{ii} = matlab.bigdata.internal.UnknownEmptyArray.build();
