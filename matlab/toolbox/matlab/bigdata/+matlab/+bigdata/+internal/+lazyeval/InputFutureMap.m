...
classdef (Sealed) InputFutureMap < handle
    ...
    methods (Static)
        ...
        function [obj, additionalConstants] = createFromClosures(inputFutures, dependencyClosures)
            ...
            if any(~isInputDone)
                [~, indices] = ismember([inputFutures(~isInputDone).Closure], dependencyClosures);
                inputIndices(~isInputDone) = indices;
                multiplexIndices(~isInputDone) = [inputFutures(~isInputDone).ArgoutIndex];
            end
    ...
    methods
        ...
        %   preProcessorFactory = PassthroughProcessorFactory();
        %   preProcessorFactory = InputMapProcessorFactory(preProcessorFactory, preMap);
        ...
        %   processorFactory = MyProcessorFactory();
        %   processorFactory = InputMapProcessorFactory(processorFactory, map);
