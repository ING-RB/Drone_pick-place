...
classdef (Sealed) FusedAggregateOperation < matlab.bigdata.internal.lazyeval.AggregateFusibleOperation
    ...
    methods
        ...
        function tasks = createExecutionTasks(obj, taskDependencies, inputFutureMap)
            ...
            isSinglePartition = all(arrayfun(@(x) x.OutputPartitionStrategy.isKnownSinglePartition, taskDependencies));
            ...
            if isSinglePartition
                ...
                tasks = iCreateFusedFragment(obj.Operations, ...
                    ...
                    taskDependencies, inputFutureMap, isInputBroadcast);
            else
                ...
                isInputBroadcast = true(1, sum(numIntermediatesPerOperation));
...
for ii = 1 : numOperations
    ...
    factory = createExtractFactoryFcn(operations{ii}, ...
        opInputFutureMap, isInputBroadcast(inputIndices));
    extractTasks{ii} = ExecutionTask.createSimpleTask(opDependencies, factory, numOutputsPerOperation(ii));
    
    factory = createReduceFactoryFcn(operations{ii});
    reduceTasks{ii} = ExecutionTask.createSimpleTask(extractTasks{ii}, factory, numOutputsPerOperation(ii));
...
passthroughProcessor = PassthroughProcessorFactory(numOperations, sum(numOutputsPerOperation));
outTask = createOutTaskFcn(reduceTasks, passthroughProcessor, sum(numOutputsPerOperation), 'IsPassBoundary', true);
...
factory = PassthroughProcessorFactory(inputFutureMap.NumOperationInputs);
