...
classdef (Sealed) ChunkResizeOperation < matlab.bigdata.internal.lazyeval.Operation
    ...
    methods
        function task = createExecutionTasks(obj, taskDependencies, inputFutureMap)
            ...
            processorFactory = OutputBufferProcessFactory(processorFactory, obj.MinBytesPerChunk, obj.MaxTimePerChunk);
    ...
    methods (Static)
        ...
        function out = minBytesPerChunkForSparkVisualization(in)
            % Get the default desired min bytes per chunk for visualization
            % when running on Spark.
            persistent value
            if isempty(value)
