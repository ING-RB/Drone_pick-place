    ...
    methods
        function tasks = createExecutionTasks(obj, taskDependencies, inputFutureMap)
            ...
            for partId = uniquePartIds(:)'
                ...
                [inputFutureMap, preMap] = inputFutureMap.factorOutDependencies(find(isInPartitioning));
                
                factory = PassthroughProcessorFactory(preMap.NumOperationInputs);
                factory = InputMapProcessorFactory(factory, preMap);
                factory = OutputBufferProcessFactory(factory);
                newTask = ExecutionTask.createAllToOneTask(groupTaskDependencies, factory, sum([groupTaskDependencies.NumOutputs]));
            ...
            processorFactory = PassthroughProcessorFactory(obj.NumInputs);
            processorFactory = InputMapProcessorFactory(processorFactory, inputFutureMap);
            
            tasks(end + 1) = ExecutionTask.createBroadcastTask(taskDependencies, processorFactory, obj.NumOutputs, 'IsPassBoundary', true);
        end
    end
...
for ii = 1:numel(partitionStrategies)
    ...
    for jj = 1:numel(uniquePartitionStrategies)
        if isCompatible(partitionStrategies{ii}, uniquePartitionStrategies{jj})
            partitionIds(ii) = jj;
            break;
        end
    end
