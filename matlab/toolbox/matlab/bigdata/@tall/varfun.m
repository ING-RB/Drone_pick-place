...
%   3) When A is a tall timetable, and 'OutputFormat' is 'timetable' or
%   'auto', FUN must return an array with the same size in the first
%   dimension as the input. Specify 'OutputFormat' as 'table' when FUN is a
%   reduction function such as MEAN.
%   4) 'GroupingVariables' and 'ErrorHandler' name-value arguments are not
%   supported.
%
...
aVarNames = getVariableNames(a.Adaptor);
...
outputFormat = iResolveOutputFormat(allowedOutputFormats, supplied.OutputFormat, outputFormat, defaultFmt);
...
for jvar = 1:ndataVars
    ...
    try
        ...
        throw(MException(message('MATLAB:table:varfun:FunFailed', funName, varname_j)).addCause(E));
    end
...
switch outputFormat
  case 'uniform'
    if ndataVars > 0
        for idx = 1:ndataVars
            b_data{idx} = tall.validateScalar(b_data{idx}, ...
  ...
  case {'table', 'timetable'}
    ...
    b_varnames = matlab.internal.tabular.makeValidVariableNames(...
        strcat(funName,{'_'},dataVars), 'warnLength');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function outputFormat = iResolveOutputFormat(allowedOutputFormats, outputFormatSupplied, ...
                                             outputFormat, defaultOutputFormat)
if outputFormatSupplied
    outputFormat = matlab.internal.datatypes.getChoice(...
        outputFormat, allowedOutputFormats, "MATLAB:table:varfun:InvalidOutputFormat");
