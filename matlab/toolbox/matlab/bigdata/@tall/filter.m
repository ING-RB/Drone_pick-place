...
% Normalize filter coefficients, if needed.
if ~isempty(a) && a(1) ~= 1
    b = b ./ a(1);
    a = a ./ a(1);
end
...
% Conditionally emit the final delay in the first partition that contains
% data. This partition will be the partition with the smallest ID that is
% greater than 0. (The row with ID == 0 is the row that contains the
% initial delay of the filter).
if info.IsLastChunk && info.PartitionId == min(delayTable.PartitionId(delayTable.PartitionId > 0))
...
% construction already correct.  The delay satisfies an equivalent 
% linear separability condition as the filter solution Y:
%
%     Z = Z_z0 (zero-state) + Z_x0 (zero-signal)
%
% This first term was computed in the first pass and stored in delayTable.

import matlab.bigdata.internal.util.makeIIRFilterUpdateFunction
...
inputIsColVec = iscolumn(delayTable.Delay{1});
zeroSignalDelayFcn = makeIIRFilterUpdateFunction(inputIsColVec, b, a);
...
for ii=2:height(delayTable)
    ...
    % Correct the delay for the current row, assuming a zero-signal input.
    row = delayTable(ii, :);
    numDataSlices = row.NumDataSlices;
    zZeroSignal = zeroSignalDelayFcn(numDataSlices, prevDelay);
