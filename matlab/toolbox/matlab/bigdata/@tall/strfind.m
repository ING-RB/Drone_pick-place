...
%   PATTERN must be a string scalar or pattern object scalar.
...
if ~isSingleStringOrScalarPattern(pattern)
    error(message('MATLAB:string:MustBeTextOrPatternScalar', 'PATTERN'));
end
...
function tf = isSingleStringOrScalarPattern(in)
% check if the input is a single string (char vector, scalar string or
% scalar cellstr) or a scalar pattern object
tf = isNonTallScalarString(in) ...
    || (iscellstr(in) && isscalar(in)) ...
    || (isa(in, "pattern") && isscalar(in)); %#ok<ISCLSTR>
end % isSingleString
