...
while argId <= length(optionalInputs)
    ...
    elseif startsWith('SamplePoints', optionalInputs{argId}, 'IgnoreCase', true)
        ...
        if any(startsWith({'includenan', 'omitnan', 'includemissing', 'omitmissing'}, optionalInputs{argId}, 'IgnoreCase', true))
...
try
    ...
    if isscalar(window)
        ...
        % window must already be in [NB NF] vector form
        if iscolumn(window)
            window = window.';
        end
        
        % floor is used to ensure that non-integer values are converted
        % into valid padding slices for stencilfun
        window = floor(window);
    end
catch
...
if ~(matlab.internal.datatypes.isScalarText(nanflag) ...
        && any(strncmpi(nanflag, {'includenan', 'omitnan', 'includemissing', 'omitmissing'}, strlength(nanflag))))
    error(message('MATLAB:movfun:wrongString'));
end
end
