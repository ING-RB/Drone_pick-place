...
if istabular(in)
    error(message("MATLAB:bigdata:array:TabularMathUnsupported", upper(fcnStruct.Name)))
end
...
if strncmpi(dirFlag, 'reverse', strlength(dirFlag))
...
outStruct = struct('Name', fcnStruct.Name, 'OutputType', fcnStruct.TypeRule, ...
    'AllowTabularMaths', false);
out = invokeOutputInfo(outStruct, out, {in});
...
nanFlags   = {'omitnan', 'includenan', 'omitmissing', 'includemissing'};

hasDirFlag = false;
hasNanFlag = false;
% Set defaults
dirFlag = dirFlags{1};
nanFlag = defaultNaNFlag;
for k = 1:numel(flags)
    thisFlag = flags{k};
    if ~matlab.internal.datatypes.isScalarText(thisFlag)
        throwCumfunError(fcnName, 'wrongFlag', 'MATLAB:cumfun:wrongFlag');
    end

    if any(startsWith(dirFlags, thisFlag, 'IgnoreCase', true))
        if hasDirFlag
            throwCumfunError(fcnName, 'duplicateDirection', 'MATLAB:cumfun:duplicateDirection');
        end
        hasDirFlag = true;
        dirFlag = thisFlag;
    elseif any(startsWith(nanFlags, thisFlag, 'IgnoreCase', true))
        if hasNanFlag
            throwCumfunError(fcnName, 'duplicateNaNFlag', 'MATLAB:cumfun:duplicateNaNFlag');
        end
        hasNanFlag = true;
        nanFlag = thisFlag;
    else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function throwCumfunError(fcnName, errorIdKey, errorIdForMessage)
error(['MATLAB:', fcnName, ':', errorIdKey], '%s', getString(message(errorIdForMessage)));
end
