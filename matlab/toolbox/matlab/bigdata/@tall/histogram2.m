...
tall.checkIsTall(upper(mfilename), 2, y);
...
cax = newplot(cax);
% When the user hasn't set up EdgeColor, do not include it in the parameter
% list for histogram2. It doesn't allow [] as a placeholder for EdgeColor.
...
opts = struct('NumBins',[],'XBinEdges',[],'YBinEdges',[],'XBinLimits',[],...
    ...
    'DisplayStyle','bar3','EdgeAlpha',1,'EdgeColor',[],...
...
if ~isempty(input)
    ...
    if isnumeric(in) || islogical(in)
        if isscalar(input) || ~(isnumeric(input{2}) || islogical(input{2}))
            % Numbins
            if isscalar(in)
                in = [in in];
            end
