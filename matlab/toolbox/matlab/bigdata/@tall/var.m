function [varX, meanX] = var(varargin)
%VAR Variance
%   Y = VAR(X)
...
% Use the in-memory version to check the remaining arguments
tall.validateSyntax(@var, varargin, 'DefaultType', 'double');
...
useDefaultDim = isequal(dim, []);

if useDefaultDim || isReducingTallDimension(dim)
    % Reduction
    if useDefaultDim
        ...
        % DIM includes 1
        aggregateFcn = @(chunk) chunkVar(chunk, normFlag, dim, missing);
        reduceFcn = @(chunk) updateVar(chunk, dim, missing);
        tmpCell = aggregatefun(aggregateFcn, reduceFcn, x);
    end

    % output is in the first element of the cell array.
    [varX, meanX] = clientfun(@normalizeOutput, tmpCell, normFlag, size(x), useDefaultDim);
    ...
    [varX, meanX] = slicefun(@(x) var(x, normFlag, dim, missing), x);
end
end
...
if isReducingTallDimension(dim)
    ...
    if any(strcmpi(missing, ["includenan", "includemissing"]))
        countPerElement = prod(mySize(data, dim));
        locCount = repmat(countPerElement, size(locVar));
    else
...
if size(inCell,1) == 1
    ...
    assert(isReducingTallDimension(dim));
    vVar = cat(1, inCell{:,1});
    vMean = cat(1, inCell{:,2});
    vCount = cat(1, inCell{:,3});
    vIsEmpty = cat(1, inCell{:,4});
    if all(mySize(vVar, dim) <= 1) % return itself
        locVar = vVar;
        locMean = vMean;
        locCount = vCount;
        locIsEmpty = vIsEmpty;
        ...
        if any(strcmpi(missing, ["omitnan", "omitmissing"]))
            vMean(vCount == 0) = 0;
            vVar(vCount == 0) = 0;
        end
        locMean = sum(vCount .* vMean, 1) ./ locCount;
        locVar = sum(vCount .* vVar, 1) ./ locCount;
        ...
        countProd = prod(vCount ./ locCount, 1) .* ones(size(locVar));
        locVarAdjust = abs(diff(vMean, 1, 1)) .^ 2 .* countProd;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [varX, meanX] = normalizeOutput(inCell, normFlag, sizeX, dimUnspecified)
varX = inCell{1};
meanX = inCell{2};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Helper to get reduced size over multiple dimensions
function sz = mySize(data, dim)
if isnumeric(dim)
    sz = arrayfun( @(d) size(data, d), dim );
else
    % all
    sz = numel(data);
end
