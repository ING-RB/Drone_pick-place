...
if ~isempty(input)
    in = input{1};
    inputoffset = 0;
    % The second input can be numeric or logical for numeric inputs. For
    % categorical inputs, it can be a categorical array, a cellstr, or a
    % string array with at least two elements. Otherwise, it will be
    % considered as the name of a name-value pair.
    if ~matlab.internal.datatypes.isScalarText(in) ...
            && (isnumeric(in) || islogical(in) || iscategorical(in) || iscellstr(in) || isstring(in))
        if isCat
            % For categorical/histogram, the second input for the
            % categories can be categorical, cellstr or string.
            validateattributes(in,{'cell','categorical','string'},{}, ...
    ...
    for i = 1:2:inputlen
        ...
        switch name
            case 'Categories'
                validateattributes(value,{'cell','categorical','string'},{}, ...
            ...
            case 'Normalization'
                opts.Normalization = validatestring(value, {'count', 'countdensity', 'cumcount',...
                    'percentage', 'probability', 'pdf', 'cdf'}, funcname, 'Normalization', i+2+inputoffset);
            case 'DisplayStyle'
