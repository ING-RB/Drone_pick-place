classdef CodePublishingService < handle
    ...
    methods
        ...
        % varargin: can optionally include 1) error callback function
        % 2) Code to be executed post publishing
        function publishCode(this, channel, code, varargin)
            ...
            this.publishCodeOnChannel(channel, code, postExecutionCode, errorFcn);
        end
        ...
        function discardCode(this, channel)
            ...
            if isempty(this.channelMap)
                this.channelMap = string.empty;
                this.codeArray = string.empty;
            end
        end
    end
    ...
    methods(Access = protected)
        ...
        function publishCodeOnChannel(this, channel, code, postExecutionCode, errorFcn)
            msg.code = cellstr(code);
            if ~isempty(errorFcn)
                msg.errorFcn = char(errorFcn);
            end
            if ~isempty(postExecutionCode)
                msg.postExecutionCode = postExecutionCode;
            end
           
            % Pass state boolean indicating if we are in MATLAB Online so
            % that the Copy button of code generation can be removed
            % (g3572492_
            msg.isMOL = matlab.graphics.internal.toolstrip.FigureToolstripManager.isMATLABOnline();
            
            jsonTxt = jsonencode(msg);
