classdef DialogHandlerService < handle
   ...
   % whether we are in remote client or not
   
   % TODO: This class needs to be updated to just use webwindow in both
   % environments and not distinguish between local and remote clients.
   ...
   % Copyright 2017-2023 The MathWorks, Inc.
    ...
    properties
        dialogList
        dialogHandlerMap % Hash the dialogHandler itself so that it can be updated to closeAllCefWindows        
    end
    ...
    methods (Access='protected')
        ...
        function this = DialogHandlerService(cefDialogHandler, IframeDialogHandler, varargin)
            ...
            this.dialogHandlerMap = containers.Map;
        ...
        function show(this, cefDialogHandler, IframeDialogHandler, varargin)
            ...
            elseif (nargin > 3)
                ...
                try                    
                    if internal.matlab.datatoolsservices.DialogHandlerService.isRemoteClient && ~isempty(IframeDialogHandler)                                
        ...
        function createCefDialog (this, dialogHandler)
            if ((dialogHandler.createNewWindow || ~isprop(dialogHandler,'createNewWindow')) && isprop(dialogHandler, 'url'))                
                remotePort = matlab.internal.getDebugPort();
                ...
                if isprop(dialogHandler, 'titleString') && ~isempty(dialogHandler.titleString)
                    browser.Title = dialogHandler.titleString;
                end
                ...
                this.dialogHandlerMap(dialogHandler.ID) = dialogHandler;
            else
        ...
        function closeCefWindow(this, src, dialogHandler)
            if (nargin > 2)
                ...
                if (isKey(this.dialogHandlerMap, dialogHandler.ID))
                    remove(this.dialogHandlerMap, dialogHandler.ID);
                end
    ...
    methods(Static)
        % Returns true if we are in a remote client environment or
        % otherwise.
        function isRemoteClient = isRemoteClient()
        ...
        % This function is used to close the dialogs that matches the provided ID. 
        % Fetch all the dialogs that start with the provided nameSpace and call 
        % closeCefWindow on the dialog. 
        function closeAllDialogs(idMatchString)            
            this = internal.matlab.datatoolsservices.DialogHandlerService.getInstance;
            % Check if the instance exists, if dialogs were never launched,
            % 'this' will be [];
            if ~isempty(this)
                currentDialogIDs = string(keys(this.dialogHandlerMap));
                idsToBeClosed = currentDialogIDs(startsWith(currentDialogIDs, idMatchString));
                for i=1:length(idsToBeClosed)
                    dialogID = char(idsToBeClosed(i));
                    if isKey(this.dialogList, dialogID)
                        dialogHandler = this.dialogHandlerMap(dialogID);
                        this.closeCefWindow(this.dialogList(dialogID), dialogHandler);                        
                    end
                end
            end
        end
