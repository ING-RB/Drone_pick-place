...
classdef Action < handle & dynamicprops & matlab.mixin.Heterogeneous
    %ACTION class 
    ...
    methods
        ...
        function this = Action(props)
            if (nargin>0) && isstruct(props)                
                ...
                if ~(isfield(props,'ID'))
                    props.ID = internal.matlab.datatoolsservices.actiondataservice.Action.generateDefaultActionId();
                end
        ...
        function set.Enabled(this, newValue)
            % If newValue is same as existing value, do not update.
            if newValue == this.Enabled_I
                return;
            end
    ...
    methods (Access='protected')
        ...
        function internalSetProperty(this, name, newValue, doNotify)
            ...
            if (doNotify)
                aed = internal.matlab.datatoolsservices.actiondataservice.ActionEventData;
                aed.Action = this;
                aed.Property = name;
                aed.OldValue = oldValue;
