classdef ColormapEditor < internal.matlab.editorconverters.EditorConverter
    ...
    methods
        ...
        function value = getClientValue(this)
            if isequal(this.colormap, this.PARULA_LOCAL)
                value = 'parula';
            elseif isequal(this.colormap, parula)
                value = 'parula';
            else
                this.initColormapsForFigure();
        ...
        function value = getServerValue(this)
            ...
            elseif ischar(this.colormap) || isstring(this.colormap)
                if isKey(this.builtInsMap, this.colormap)
                    value = this.builtInsMap(this.colormap);
                else
                    % try to convert to numeric
                    value = str2num(this.colormap); %#ok<ST2NM>
                end
            else
                value = this.colormap;
            end
    ...
    methods(Access = private)
        function initColormapsForFigure(this)
            % The size of the colormap for a given pre-defined value will match
            % the current figure's colormap size, if there is a current figure,
            % otherwise it will be 256.  The predefined colormaps need to be
            % reevaluated in the current figure's context, if the sizes differ,
            % in order for the sizes to match when compared.
            if isempty(this.builtInsMap) || ...
                    ~isequal(size(this.builtInsMap('parula')), size(parula))
                builtInsValues = arrayfun(@(n) feval(n), this.PREDEFINED_COLORMAPS, 'UniformOutput', false);
                this.builtInsMap = containers.Map(this.PREDEFINED_COLORMAPS, builtInsValues);
            end
        end
