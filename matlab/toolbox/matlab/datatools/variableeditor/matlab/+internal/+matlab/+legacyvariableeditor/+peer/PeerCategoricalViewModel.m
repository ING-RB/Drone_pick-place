classdef PeerCategoricalViewModel < ...
        ...
        internal.matlab.legacyvariableeditor.CategoricalViewModel
    ...
    methods
        function this = PeerCategoricalViewModel(parentNode, variable)
            ...
            widgets = widgetRegistryInstance(1).getWidgets('', 'categorical');
            m.renderer = widgets.CellRenderer;
            m.editor = widgets.Editor;
            m.inplaceeditor = widgets.InPlaceEditor;
            m.class = 'categorical';
            % Categoricals do not have header labels
            m.ShowColumnHeaderLabels = false;
            tableProperties = internal.matlab.legacyvariableeditor.peer.PeerUtils.toJSON(true, m);
            
            this = this@internal.matlab.legacyvariableeditor.peer.PeerArrayViewModel(...
                parentNode, variable, 'TableModelProperties', tableProperties);
            
            this@internal.matlab.legacyvariableeditor.CategoricalViewModel(variable.DataModel);
            ...
            % g1687308: Adding the ColumnMinWidth property only for categorical columns so as to prevent the users from
            % reducing the width such that the dropdown icon disppears.
            ColumnModelChangeListenerState = this.ColumnModelChangeListener.Enabled;
            this.ColumnModelChangeListener.Enabled = false;
            for col = this.StartColumn: this.EndColumn
                if col == this.EndColumn
                    this.ColumnModelChangeListener.Enabled = ColumnModelChangeListenerState;
                end
                this.setColumnModelProperty(col, 'ColumnMinWidth', internal.matlab.legacyvariableeditor.VEColumnConstants.categoricalMinWidth);
            end
    ...
    methods(Access='public')
        ...
        function [renderedData, renderedDims] = getRenderedData(this, ...
            ...
            data = this.getRenderedData@internal.matlab.legacyvariableeditor.CategoricalViewModel(...
                startRow, endRow, startColumn, endColumn);
            ...
            f=get(0,'format');
            format('long');
            longData = this.getRenderedData@internal.matlab.legacyvariableeditor.CategoricalViewModel(startRow,endRow,startColumn,endColumn);
            format(f);
