classdef PeerTableViewModel < internal.matlab.legacyvariableeditor.peer.PeerArrayViewModel & ...
        internal.matlab.legacyvariableeditor.TableViewModel & ...
    ...
    methods
        function this = PeerTableViewModel(parentNode, variable, viewID, usercontext)
            if nargin < 3 
                usercontext = '';
                viewID = '';
            elseif nargin < 4
			...
			this@internal.matlab.legacyvariableeditor.TableViewModel(variable.DataModel, viewID);
            this.usercontext = usercontext;
            % Instantiates the Action State Handler
            this.ActionStateHandler = internal.matlab.legacyvariableeditor.peer.PeerActionStateHandler(this, variable);
            ...
            if size(this.DataModel.Data, 2) > 0
                ...
                if ~internal.matlab.legacyvariableeditor.peer.PeerUtils.isLiveEditor(this.usercontext)
				  ...
				  w = internal.matlab.datatoolsservices.WidgetRegistry.getInstance();
                  w.registerWidgets('internal.matlab.legacyvariableeditor.peer.PeerTableViewModel','', 'variableeditor_peer/PeerTableViewModel','','')
                end
                ...
                if isprop(currentData.Properties, 'RowTimes')
        ...
        % This method calculates and sets the default 'columnWidth'
        % property based on headerWidth computations. NOTE: All references
        % of LE will be removed when we get better default column widths on
        % the client side
        function setDefaultColumnWidth(this, data)
            ...
            startColumnIndexes = internal.matlab.legacyvariableeditor.TableViewModel.getColumnStartIndicies(data, startCol, endCol);
            for col=startCol:endCol
                % Column widths defined in the VEColumnConstants file. 
                % TODO: Remove defaultWidth computation on server once we
                % receive client side width calculations on table.
                defaultWidth = internal.matlab.legacyvariableeditor.VEColumnConstants.defaultColumnWidth;
                isLiveEditor = internal.matlab.legacyvariableeditor.peer.PeerUtils.isLiveEditor(this.usercontext);
                if isLiveEditor
                    defaultWidth = internal.matlab.legacyvariableeditor.VEColumnConstants.defaultLiveEditorColumnWidth;
                end
                width = defaultWidth;
                ...
                if ~(islogical(isColumnManuallyResized{1}) && isColumnManuallyResized{1}==true)
                    % Do not special case datetimes and complex for LE for
                    % consistency
                    if ~isLiveEditor
                        if strcmp(classType, 'datetime')
                            % Set datetime column width wider than default
                           width = internal.matlab.legacyvariableeditor.VEColumnConstants.datetimeColumnWidth;
                        elseif internal.matlab.legacyvariableeditor.peer.PeerUtils.isNumericType(classType)
                            colData = table2array(data(:, col));
                            if ~isreal(colData)
                                % Set complex number column width wider than
                                % default
                                width = internal.matlab.legacyvariableeditor.VEColumnConstants.complexNumDefaultWidth;
                            end
                        end
                    end
                    ...
                    if this.UseTableColumnNamesForView
                        % Take into account, Live Editor fonts and header
                        % spacing
                        % TODO: Deprecate all our legacyVEFiles
                    ...
                    if (width > defaultWidth || groupColumnSize > 1 || ...
        ...
        function [renderedData, renderedDims] = getRenderedData(this, startRow, endRow, ...
            ...
            [data, ~, startRow, endRow, startColumn, endColumn] = this.getRenderedData@internal.matlab.legacyvariableeditor.TableViewModel(...
            ...
            % If there is no rawData, exit early
            if isempty(rawData)
            ...
            startColumnIndexes = internal.matlab.legacyvariableeditor.TableViewModel.getColumnStartIndicies(rawData, startColumn, endColumn);
            ...
            % that cannot be concatenated. Ignore computing longNumericData(g2032641).
            try
                ...
                for col=1:(size(startColumnIndexes,2)-1)
                    ...
                    if internal.matlab.legacyvariableeditor.peer.PeerUtils.isNumericType(columnClass)
            ...
            for col=1:(size(startColumnIndexes,2)-1)
                ...
                for row=1:size(data,1)
                    ...
                    for dataIndex=startColumnIndexes(col):startColumnIndexes(col+1)-1
                        ...
                        editorValue = '';
                        % Indexing will not work for rawData containing
                        % objects that cannot be concatenated, return
                        % entire rawData as currData.
                        try
                        ...
                        if isMetaData(row,dataIndex)
                            ...
                            elseif (dataSize(1) == 1)
                                editorValue = sprintf('%s.(''%s'')', this.DataModel.Name,varName);
        ...
        function jsonData = getJSONforCell(this, doEscapeValues, data, longData, isMeta, editorValue, row, col)
            if ~isMeta %~this.MetaData(row,col)
                jsonData = ...
                    internal.matlab.legacyvariableeditor.peer.PeerUtils.toJSON(doEscapeValues,...
                    struct('value', data,...
                    'editValue', longData,...
                    'isMetaData', '0')...
                );
        ...
        function varargout = handlePeerEvents(this, es, ed)
            ...
            if isfield(ed.EventData,'type')
                switch ed.EventData.type
                    ...
                    otherwise
                        varargout{1} = this.handlePeerEvents@internal.matlab.legacyvariableeditor.peer.PeerArrayViewModel(es, ed);
                end
            end
        end
        ...
        function varargout = getData(this,varargin)
            ...
            % data.
            t = this.getData@internal.matlab.legacyvariableeditor.ArrayViewModel(varargin{:});
            v = table2cell(t);
            varargout{1} = v;
        end
            ...
            try
                if ~isempty(row)
                    ...
                    if isempty(data)
                        ...
                        data = internal.matlab.legacyvariableeditor.peer.PeerUtils.parseStringQuotes(data, classType);
                        ...
                        if (isStr)
                            ...
                            if ~isequal(origData, '''') && ~isequal(origData, '"')
                                ...
                                data = internal.matlab.legacyvariableeditor.peer.PeerUtils.escapeSpecialCharsForStrings(data);
                    ...
                    savedWarning = internal.matlab.legacyvariableeditor.peer.PeerUtils.disableWarning();
                    ...
                    % enable warning
                    internal.matlab.legacyvariableeditor.peer.PeerUtils.resumeWarning(savedWarning);
       ...
       function status = handlePropertySet(this, ~, ed)
            ...
            this.handlePropertySet@internal.matlab.legacyvariableeditor.peer.PeerArrayViewModel([], ed);
                ...
                if strcmp(property,'HeaderName')
                    ...
                    try
                        ...
                        if ~any(ismember(columnNames, value))
                            ...
                            cmd = [matlab.internal.tabular.generateVariableNameAssignmentString(this.DataModel.Data, column+1, value, this.DataModel.Name) ';'];
    ...
    methods(Access='protected')       
        function isValid = validateInput(this,value,row,column)
            ...
            classType = this.getClassType(row, column);
            if internal.matlab.legacyvariableeditor.peer.PeerUtils.isNumericType(classType)
        ...
        function result = evaluateClientSetData(this, data, row, column)
            ...
            result = [];
            classType = this.getClassType(row, column);
            if internal.matlab.legacyvariableeditor.peer.PeerUtils.isNumericType(classType) || isequal(classType, 'logical')
        ...
        function classType = getClassType(this, ~, column) %#ok<INUSL>
            classType = eval(sprintf('class(this.DataModel.Data.(%d))',column));
        end
        ...
        function updateColumMetaInfo(this)
            ...
            startColumnIndexes = internal.matlab.legacyvariableeditor.TableViewModel.getColumnStartIndicies(currentData, startCol, endCol);
            
            % g1772972: Use the Original Data for setting the IsSortable and
            % IsFilterable Flag so that they are not affected by data
            % manipulation (Filtering, etc)
            
            % NOTE: This is a stop-gap fix and will need to be
            % re-visited when we enable adding, deleting or re-ordering of columns
            % in the LE Tables
            originalData = this.ActionStateHandler.OrigData;
            if width(originalData) ~= width(currentData)
                TableForPropUpdate = currentData;
            else
                TableForPropUpdate = originalData;
            end
            for col=startCol:endCol
                % Set Header Name
                colValForValidation = TableForPropUpdate(:,col);
                if this.UseTableColumnNamesForView
                    name = currentData.Properties.VariableNames{col};
                    % if the datatype has changed, ensure that we get this
                    % data from the currentData instead of the
                    % TableForPropUpdate. Making this an LE specific check,
                    % in MOL usecases, currentData could be completely out
                    % of sync with originalData
                    % Leaving the g1772972 fix intact and just fixing g1884594 
                    if internal.matlab.legacyvariableeditor.peer.PeerUtils.isLiveEditor(this.usercontext)
                        if ~(strcmp(class(currentData.(name)),class(originalData.(name))))
                            colValForValidation = currentData(:, col);                        
                        end                    
                    end
                    this.setColumnModelProperty(col,'HeaderName',name, false);
                end
                
                % Set the sortable flag
                % g1772972: Use the Original Data for setting IsSortable
                isSortable = internal.matlab.legacyvariableeditor.peer.PeerUtils.checkIsSortable(colValForValidation, false);
                this.setColumnModelProperty(col,'IsSortable',isSortable, false);
                
                % Set the filterable flag
                % g1772972: Use the Original Data for setting IsFilterable
                isFilterable = internal.matlab.legacyvariableeditor.peer.PeerUtils.checkIsFilterable(colValForValidation, false);
                this.setColumnModelProperty(col,'IsFilterable',isFilterable, false);

                % Set the Group Column Sizes
                startIndex=col-startCol+1;
                this.setColumnModelProperty(col,'GroupColumnSize',num2str(startColumnIndexes(startIndex+1)-startColumnIndexes(startIndex)), false);
                ...
                switch classType
                    case {'categorical' 'nominal' 'ordinal'}
                        ...
                        this.setColumnModelProperties(col,...
                            'categories', cats,...
                            'isProtected', isprotected(currentData.(this.getVariableName(':',col))),...
                            'EditorConverter', 'addSingleQuotesConverter', ...
                            'ColumnMinWidth', internal.matlab.legacyvariableeditor.VEColumnConstants.categoricalMinWidth);
                    case {'datetime'}
        ...
        function updateRowMetaInfo(this)
            ...
            if this.UseTableRowNamesForView
                ...
                if isprop(currentData.Properties, 'RowNames')
                    rowNames = currentData.Properties.RowNames;
                elseif isprop(currentData.Properties, 'RowTimes')
