classdef PeerStructureViewModel < internal.matlab.legacyvariableeditor.peer.PeerArrayViewModel & internal.matlab.legacyvariableeditor.StructureViewModel
    % PEERSTRUCTUREVIEWMODEL Peer Model Structure View Model for scalar
    % structures
    ...
    methods
        function this = PeerStructureViewModel(parentNode, variable, viewID, fieldColumnNameTag, valueColumnName, setupModelProperties)
            if nargin<3
                viewID = '';
                fieldColumnNameTag = 'Field';  
            elseif nargin<4 || isempty(fieldColumnNameTag)
                fieldColumnNameTag = 'Field';  
            end
            fieldColumnName = getString(message(...
                sprintf('MATLAB:codetools:variableeditor:%s', fieldColumnNameTag)));
            this = this@internal.matlab.legacyvariableeditor.peer.PeerArrayViewModel(parentNode, variable, 'viewID', viewID, 'SortableColumn', fieldColumnName);
            this@internal.matlab.legacyvariableeditor.StructureViewModel(variable.DataModel);
            this.viewID = viewID;

            if nargin < 6 || setupModelProperties
                if nargin<5 || isempty(valueColumnName)
                ...
                textWidgets = widgetRegistry(1).getWidgets('', 'char');
        ...
        function handleTableModelUpdate(this, ~, ed)
            ...
            this.handleTableModelUpdate@internal.matlab.legacyvariableeditor.peer.PeerArrayViewModel([],[]);
    ...
    methods(Access='protected')
        function setupPagedDataHandler(this, variable)
            ...
            if ~internal.matlab.datatoolsservices.FormatDataUtils.isVarEmpty(variable.DataModel.Data)
        ...
        function [renderedData, renderedDims] = renderData(this, data, ...
                startRow, endRow, startColumn, endColumn, metaData)
            arguments
                this
                data
                startRow
            ...
            sEndRow = min(sStartRow + size(renderedData, 1) -1, endRow-1);
            % allocate as much as we are rendering data, allocating upto
            % endRows can be huge as we include WindowBlockRows
            rowStrs = strtrim(cellstr(num2str((sStartRow:sEndRow)'))');
            ...
            for row=1:size(renderedData, 1)
                ...
                % Resolve scalar logicals to be 1 or 0 G1154075
                ...
                renderedData{row,1} = ...
                    internal.matlab.legacyvariableeditor.peer.PeerUtils.toJSON(true, struct(...
                    'value', char(varName),...
                ...
                if showValueColumn
                    ...
                    valueForJSON = struct('value', varValue,...
                        ...
                        'editorValue', this.getSubVarName(this.DataModel.Name,varName),...
                        'isMetaData', int2str(metaData(row, 1)),...
                        'row', rowString, ...
                    ...
                    renderedData{row,currentColumn} = ...
                        internal.matlab.legacyvariableeditor.peer.PeerUtils.toJSON(escapeVal, valueForJSON);
                end
    ...
    methods(Access = public)
        function [renderedData, renderedDims] = getRenderedData(this, ...
            ...
            data = this.getRenderedData@internal.matlab.legacyvariableeditor.StructureViewModel(...
                startRow, endRow, startColumn, endColumn);
        ...
        function editable = isEditable(this, row, col)
            ...
            editable = this.isEditable@internal.matlab.legacyvariableeditor.StructureViewModel(...
                row,col);
        end
        ...
        function varargout = setSelection(this, varargin)
            ...
            varargout{1} = this.setSelection@internal.matlab.legacyvariableeditor.peer.PeerArrayViewModel(varargin{:});  
        ...
        function [renderedData, renderedDims] = refresh(this, ~, eventData)
           ...
           [renderedData, renderedDims] = this.refreshRenderedData(dataBounds);
            ...
            if size(eventData.Range, 2) == 1 ...
                ...
                dataChangeEvent = internal.matlab.legacyvariableeditor.DataChangeEventData;
                if this.SortAscending
                ...
                dataChangeEvent = internal.matlab.legacyvariableeditor.DataChangeEventData;
                [I,J] = meshgrid(dataBounds.startRow:dataBounds.endRow,dataBounds.startColumn:dataBounds.endColumn);
                I = I(:)';
                J = J(:)';
                dataChangeEvent.Range = [I;J];
            ...
            this.updateSelectedRowIntervals();
            this.setSelection(this.SelectedRowIntervals, this.SelectedColumnIntervals);
            % this is just to ensure that this.EndRow is reset
            % appropriately based on the current size from the DataModel. 
            this.getCurrentPage();
                ...
                for i=1:length(this.SelectedFields)
                    for j=1:size(newFields, 1)
                        if strcmp(this.SelectedFields{i},newFields{j})
                            tempArray{countSelectedFields} = this.SelectedFields{i}; 
