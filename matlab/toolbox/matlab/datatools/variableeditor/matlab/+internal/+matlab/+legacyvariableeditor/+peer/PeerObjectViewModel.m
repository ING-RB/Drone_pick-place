classdef PeerObjectViewModel < internal.matlab.legacyvariableeditor.peer.PeerStructureViewModel & ...
        internal.matlab.legacyvariableeditor.ObjectViewModel
    ...
    methods
        function this = PeerObjectViewModel(parentNode, variable, ...
                viewID, propertyColumnName, valueColumnName, setupModelProperties)
            % Creates a new PeerObjectViewModel
            if nargin<3 || isempty(propertyColumnName)
               viewID = '';
            end
            if nargin<4 || isempty(propertyColumnName)
               propertyColumnName = 'Property';
            end
            if nargin<5 || isempty(valueColumnName)
                valueColumnName = 'Value';
            end
            if nargin<6 || isempty(setupModelProperties)
                % setupModelProperties is used to specify whether to setup
                % table/row/column/cell ModelProperties.  These are used by the
                % Variable Editor, but not necessarily for other clients (like
                % the inspector).  If not specified, assume they should be set.
                setupModelProperties = true;
            end
            
            this@internal.matlab.legacyvariableeditor.ObjectViewModel(variable.DataModel);
            this = this@internal.matlab.legacyvariableeditor.peer.PeerStructureViewModel(...
                parentNode, variable, viewID, propertyColumnName, valueColumnName, setupModelProperties);
    ...
    methods (Access = public)
        function [renderedData, renderedDims] = getRenderedData(this, ...
            ...
            data = this.getRenderedData@internal.matlab.legacyvariableeditor.ObjectViewModel(...
                startRow, endRow, startColumn, endColumn);
        ...
        function editable = isEditable(this, row, col)
            ...
            editable = this.isEditable@internal.matlab.legacyvariableeditor.ObjectViewModel(...
                row,col);
        end
