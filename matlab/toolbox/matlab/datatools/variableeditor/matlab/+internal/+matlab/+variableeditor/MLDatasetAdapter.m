classdef MLDatasetAdapter < internal.matlab.variableeditor.MLAdapter
    %MLDatasetAdapter
    %   MATLAB Dataset Variable Editor Mixin
    ...
    methods
        function storedValue = get.DataModel(this)
            if isempty(this.DataModel_I)
                this.DataModel = internal.matlab.variableeditor.MLDatasetDataModel(this.Name, this.Workspace);
            end
    ...
    methods
        function storedValue = get.ViewModel(this)
            if isempty(this.ViewModel_I)
                this.ViewModel_I = internal.matlab.variableeditor.DatasetViewModel(this.DataModel);
            end
    ...
    % Constructor
    methods
        function hObj = MLDatasetAdapter(name, workspace, data)
            hObj.Name = name;
    ...
    methods(Static)
        function c = getClassType()
            c = internal.matlab.variableeditor.MLDatasetDataModel.ClassType;
        end
    end
end
