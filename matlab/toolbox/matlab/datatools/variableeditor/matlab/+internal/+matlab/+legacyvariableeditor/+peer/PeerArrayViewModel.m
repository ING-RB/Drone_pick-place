classdef PeerArrayViewModel < internal.matlab.legacyvariableeditor.ArrayViewModel & internal.matlab.legacyvariableeditor.peer.PeerVariableNode
    ...
    methods
        function this = PeerArrayViewModel(parentNode, variable, varargin)
            this@internal.matlab.legacyvariableeditor.ArrayViewModel(variable.DataModel);
            this = this@internal.matlab.legacyvariableeditor.peer.PeerVariableNode(...
                parentNode, ...
                internal.matlab.legacyvariableeditor.peer.PeerArrayViewModel.PeerNodeType, ...
        ...
        function varargout = setSelection(this,selectedRows,selectedColumns,varargin)
            ...
            varargout{1} = this.setSelection@internal.matlab.legacyvariableeditor.ArrayViewModel(...
        ...
        function varargout = handlePeerEvents(this, ~, ed)
            ...
            if isfield(ed.EventData,'type')
                switch ed.EventData.type
                    ...
                    case 'setCurrentPage'
                        varargout{1} = this.handleSetCurrentPage(ed.EventData);
                    otherwise
                        this.sendErrorMessage(getString(message(...
        ...
        % Handles setting current page for the viewPort. If doUpdate is
        % explicitly turned off, then do not update metaInformation.
        function data = handleSetCurrentPage(this, varargin)
            ...
            doUpdate = this.getStructValue(varargin{1}, 'doUpdate');
            if isempty(doUpdate)
                doUpdate = true;
            end
            data = [];
            this.setCurrentPage(startRow, endRow, startColumn, endColumn, doUpdate);
        end
        ...
        function data = handleClientGetStringData(this, varargin)
            ...
            stringData = internal.matlab.legacyvariableeditor.peer.PeerDataUtils.getStringData(this.getDataForStringDisplay(), data, endRow-startRow+1, size(this.DataModel.Data,2)-startColumn+1, scalingFactor);
        ...
        function logDebug(this, class, method, message, varargin)
            if internal.matlab.legacyvariableeditor.peer.PeerUtils.isDebug && ...
                ...
                gridNode = this.PagedDataHandler.getGridNode;
                internal.matlab.legacyvariableeditor.peer.PeerUtils.logDebug(gridNode, class, method, message, varargin{:});
            end
        end
        ...
        function varargout = handleClientSetData(this, varargin)
            ...
            try
                if ~isempty(row)
                    ...
                    if isempty(data)
                        ...
                        if isStr || (ischar(data) && startsWith(data, '"') && endsWith(data, '"'))
                            ...
                            if ~isequal(origData, '''') && ~isequal(origData, '"')
                                ...
                                data = internal.matlab.legacyvariableeditor.peer.PeerUtils.parseStringQuotes(data, 'string');
                    ...
                    % if current value is a value summary then do not send
                    % peer event with disp value
                    [dispValue, isMetaData] = this.getDispValue(eValue, data, row, column);
                    ...
                    this.sendPeerEvent('dataChangeStatus','status', 'success', 'dispValue', dispValue, 'isMetaData', isMetaData, 'row', row-1, 'column', column-1);
        ...
        function [dispValue, isMetaData] = getDispValue(this, editValue, data, row, column)
            import internal.matlab.datatoolsservices.FormatDataUtils;
            isMetaData = false;
            if FormatDataUtils.isValueSummaryClass(class(editValue)) || ...
                    (isnumeric(editValue) && internal.matlab.datatoolsservices.FormatDataUtils.isSummaryValue(editValue))
                dispValue = FormatDataUtils.getValueSummaryString(editValue, '');
                isMetaData = true;
            elseif isnumeric(editValue) && ~isscalar(editValue)
                dispValue = FormatDataUtils.getNumericNonScalarValueDisplay(editValue);
            elseif ~ischar(editValue)
        ...
        function setCurrentPage(this, startRow, endRow, startColumn, endColumn, doUpdate)
            ...
            if this.StartRow ~= max(1, startRow)
                this.StartRow = max(1, startRow);
                updateCurrentPage = true;                
            end
            if this.EndRow ~= min(s(1), endRow)  
                this.EndRow = min(s(1), endRow);
                updateCurrentPage = true;                
            end
            if this.StartColumn ~= max(1, startColumn)  
                this.StartColumn = max(1, startColumn);
                updateCurrentPage = true;                
            end
            if this.EndColumn ~= min(s(2), endColumn)  
                this.EndColumn = min(s(2), endColumn);
                updateCurrentPage = true;                
            end
            
            if doUpdate || updateCurrentPage
            ...
            if ~isempty(cellProps)
                ...
                for r=sRow:eRow
                    ...
                    for c=sCol:eCol
                        ...
                        map.RowNumber = r;
                        map.ColumnNumber = c;
                        cmpca{c-sCol+1} = internal.matlab.legacyvariableeditor.peer.PeerUtils.toJSON(true, map);
                    end
        ...
        function updateTableModelInformation(this)
            ...
            tableModelProps = internal.matlab.legacyvariableeditor.peer.PeerUtils.toJSON(true, this.TableModelProperties);
        ...
        function updateRowModelInformation(this, startRow, endRow)
            ...
            if ~isempty(rmpa)
                for i=sRow:eRow
                    ...
                    map.RowNumber = i;
                    rmpca{i-sRow+1} = internal.matlab.legacyvariableeditor.peer.PeerUtils.toJSON(true, map);
                end
            end
            rowModelProps = '[';
            if ~isempty(rmpca)
        ...
        function updateColumnModelInformation(this, startColumn, endColumn)
            ...
            if ~isempty(cmpa)
                for i=sCol:eCol
                    ...
                    map.ColumnNumber = i;
                    cmpca{i-sCol+1} = internal.matlab.legacyvariableeditor.peer.PeerUtils.toJSON(true, map);
                end
            end
            columnModelProps = '[';
            if ~isempty(cmpca)
        ...
        function [renderedData, renderedDims] = getRenderedData(this,startRow,endRow,startColumn,endColumn)
            data = this.getRenderedData@internal.matlab.legacyvariableeditor.ArrayViewModel(startRow,endRow,startColumn,endColumn);
            ...
            for row=1:min(size(renderedData,1),size(data,1))
                for col=1:min(size(renderedData,2),size(data,2))
                    jsonData = internal.matlab.legacyvariableeditor.peer.PeerUtils.toJSON(true, 'value',data{row,col},...
    ...
    methods(Access='protected')
        ...
        function [renderedData, renderedDims] = refresh(this, eventSource, eventData)
            ...
            % If the dimensions have changed send the client the new size
            if eventData.DimensionsChanged
                this.handleClientGetSize();
            end
            
            % Refreshes the data block passed in, and updates the client
            this.refresh@internal.matlab.legacyvariableeditor.ArrayViewModel(...
        ...
        function msgOnError = getMsgOnError(this, row, column, msg)
            ...
            idx = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.getInstance.documentIndex(this.DataModel.Name,...
                this.DataModel.Workspace);
            
            msgOnError = ['internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.getInstance.Documents(' ...
