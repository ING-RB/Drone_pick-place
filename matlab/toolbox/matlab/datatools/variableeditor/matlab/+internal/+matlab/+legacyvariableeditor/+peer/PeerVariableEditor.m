classdef PeerVariableEditor < handle
    ...
    properties(Constant)
        ...
        ActionManagerNamespace = '/LegacyVEActionManager';
        startPath = 'internal.matlab.legacyvariableeditor.Actions'; 
        ContextMenuManagerNamespace = '/LegacyVEContextMenuManager';
    ...
    methods
        function storedValue = get.PeerManager(this)
            if isempty(this.PeerManager_I) || ~isvalid(this.PeerManager_I)
                this.PeerManager_I = internal.matlab.legacyvariableeditor.peer.PeerManagerFactory.getInstance.createInstance(internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.PeerModelChannel, false);
            end
    ...
    methods(Access='protected')
        ...
        function initVEActions(this)
            actionNamespace = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.ActionManagerNamespace;            
            pathToScan = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.startPath;
        ...
        function initVEContextMenu(this)            
            contextNamespace = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.ContextMenuManagerNamespace;
            queryString = ['[data-ManagerChannel=''' this.PeerManager_I.Channel ''']'];
            pathToXMLFile = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.VEContextMenuActionsFile;
            this.ContextMenuProvider = this.PeerManager_I.initContextMenu(queryString, pathToXMLFile, contextNamespace);           
        end        
    end
    ...
    methods(Static, Access='public')
        ...
        function obj = getInstance(varargin)
            ...
            if isempty(managerInstance) || (nargin>0 && strcmpi(varargin{1},internal.matlab.legacyvariableeditor.peer.PeerManager.ForceNewInstance))
                managerInstance = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor();
            end
            obj = managerInstance;
        end
        ...
        function startup()
            ...
            [~] = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.getInstance;
        end
        ...
        function varDocument = openvar(name, workspace, data, userContext)
            ...
            % data
            if nargin<=2 || isa(data,'internal.matlab.legacyvariableeditor.NullValueObject')
                try
            ...
            varDocument = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.getInstance.PeerManager.openvar(name, workspace, data, userContext);
        end
        ...
        function closevar(name, workspace)
            ...
            internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.getInstance.PeerManager.closevar(name, workspace);
        end
        
        function closeAllVariables()
            internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.getInstance.PeerManager.closeAllVariables();
        end

        function hasDoc = containsDocument(doc)
            hasDoc = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.getInstance.PeerManager.containsDocument(doc);
        end
        ...
        function index = documentIndex(name, workspace)
            ...
            index = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.getInstance.PeerManager.documentIndex(name, workspace);
        end

        function index = docIdIndex(docID)
            index = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.getInstance.PeerManager.docIdIndex(docID);
        end
    end
end
