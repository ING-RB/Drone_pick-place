classdef (CaseInsensitiveProperties=true, TruncatedProperties=true, ConstructOnLoad=true) BlockSelectionModel <  internal.matlab.legacyvariableeditor.SelectionModel 
        ...
        function selectionString = getSelectionString(~, selectionString, dataSize, selectedEntries)
            if ~isempty(selectedEntries)
                ...
                for i=1:size(selectedEntries,1)
                    ...
                    selectionString = [selectionString internal.matlab.legacyvariableeditor.BlockSelectionModel.localCreateSubindex(selectedEntries(i,:),dataSize)]; %#ok<AGROW>
                end
                if size(selectedEntries,1) > 1
                    selectionString = [selectionString ']'];
                end
