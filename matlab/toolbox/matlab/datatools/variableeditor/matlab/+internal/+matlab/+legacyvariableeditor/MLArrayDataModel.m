classdef MLArrayDataModel < internal.matlab.legacyvariableeditor.ArrayDataModel & internal.matlab.legacyvariableeditor.NamedVariable & internal.matlab.legacyvariableeditor.MLNamedVariableObserver
    %MLARRAYDATAMODEL
    ...
    methods(Access='public')
        ...
        function varargout = setData(this,varargin)
			...
			% with 'internal.matlab.legacyvariableeditor'. We strip this off because the ArrayDataModel
			% setData function does not need it and cannot handle the extra input argument
            if strfind(varargin{end}, 'internal.matlab.legacyvariableeditor') == 1
                errorMsg = c{end};
                c(end) = [];
            end
            setStrings = this.setData@internal.matlab.legacyvariableeditor.ArrayDataModel(c{:});
        ...
        function data = variableChanged(this, varargin)
            ...
            % reason we derive the class from the adapter
             if nargin>=4 && ~isa(this, 'internal.matlab.workspace.MLWorkspaceDataModel') 
                 manager = internal.matlab.legacyvariableeditor.peer.PeerVariableEditor.getInstance.PeerManager;
            ...
            if nargin>=4 &&...
                    ...
                    ~(internal.matlab.datatoolsservices.FormatDataUtils.checkIsString(this.Data) && internal.matlab.datatoolsservices.FormatDataUtils.checkIsString(varargin{1})) &&...
                    (...
                        ...
                        (... % We've gone from a scalar to non-scalar or vice-versa
                            (this.adapterChangeForSameClass(manager, varargin{:}))...
                        )...
                    )
                data = [];
                return
        ...
        function comp = adapterChangeForSameClass(this, manager, varargin)
            if ~isempty(manager)
                ...
                newAdapterName = manager.getAdapterClassNameForData(varargin{3}, varargin{2}, varargin{1});
                currAdapterName = manager.getAdapterClassNameForData(class(this.Data), size(this.Data), this.Data);
                ...
                % Fallback condition to allow adapters to be switched in case the manager is empty
                comp = (~isscalar(this.Data) && isscalar(varargin{1})) || (isscalar(this.Data) && ~isscalar(varargin{1}));
            end
        end
        ...
        function eq = equalityCheck(~, oldData, newData)
            eq = internal.matlab.legacyvariableeditor.areVariablesEqual(oldData, newData);
        end
    end %methods
