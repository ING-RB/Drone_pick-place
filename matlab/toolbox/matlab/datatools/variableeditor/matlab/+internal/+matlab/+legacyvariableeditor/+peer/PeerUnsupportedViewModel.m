classdef PeerUnsupportedViewModel < internal.matlab.legacyvariableeditor.MLUnsupportedViewModel & internal.matlab.legacyvariableeditor.peer.PeerVariableNode
    ...
    methods
        function this = PeerUnsupportedViewModel(parentNode, variable)
            this@internal.matlab.legacyvariableeditor.MLUnsupportedViewModel(variable.DataModel);
            this = this@internal.matlab.legacyvariableeditor.peer.PeerVariableNode(parentNode,internal.matlab.legacyvariableeditor.peer.PeerUnsupportedViewModel.PeerNodeType,'name',variable.Name);
        end
        ...
        function renderedData = getRenderedData(this,varargin)
            renderedData = getRenderedData@internal.matlab.legacyvariableeditor.MLUnsupportedViewModel(this);
                        
            % Truncate the length of the text to avoid excessive bandwidth
            % consumption.
            if length(renderedData)>internal.matlab.legacyvariableeditor.peer.PeerUnsupportedViewModel.TextLengthLimit
                 renderedData = sprintf('%s\n...',renderedData(1:internal.matlab.legacyvariableeditor.peer.PeerUnsupportedViewModel.TextLengthLimit));
            end
