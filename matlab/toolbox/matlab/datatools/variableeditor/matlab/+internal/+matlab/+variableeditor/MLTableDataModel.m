classdef MLTableDataModel < internal.matlab.variableeditor.MLArrayDataModel & internal.matlab.variableeditor.TableDataModel & ...
    ...
    methods(Access='protected')
        ...
        function [I,J] = doCompare(this, newData)
            ...
            c = this.funcCall(@(var)(iscell(var)) || (isdatetime(var) && isempty(var.TimeZone)), this.Data, false);
            cNew = this.funcCall(@(var)(iscell(var)) || (isdatetime(var) && isempty(var.TimeZone)), newData, false);
            ...
            colNameChanged = ~isempty(setdiff(this.getVarNameHelper(this.Data), this.getVarNameHelper(newData)));
            ...
            if any([c{:}]) || any([cNew{:}]) || colNameChanged
                ...
                if isscalar(I) 
                    ...
                    [~,J] = find(cellfun(@(a,b) ~isequal(a,b), this.convertToCell(this.Data(I,:)), this.convertToCell(newData(I,:))));
                    if length(J) > 1
                        [I,J] = meshgrid(I,J); 
                    end
                else
