classdef PeerManager < internal.matlab.legacyvariableeditor.MLManager
    ...
    methods
        ...
        function set.HasFocus(this, newValue)
            if strcmp(this.Channel,internal.matlab.legacyvariableeditor.peer.PeerManagerFactory.PeerModelChannel)
               return;
            end

            if this.HasFocus_I && ~newValue
                this.HasFocus_I = newValue;
                if ~internal.matlab.legacyvariableeditor.peer.PeerManagerFactory.inFocusUpdate
                    internal.matlab.legacyvariableeditor.peer.PeerManagerFactory.setFocusedManager([]);
                end

                % Fire event when manager loses focus
                eventdata = internal.matlab.legacyvariableeditor.ManagerEventData;
            ...
            elseif newValue
                this.HasFocus_I = newValue;
                if ~internal.matlab.legacyvariableeditor.peer.PeerManagerFactory.inFocusUpdate
                    internal.matlab.legacyvariableeditor.peer.PeerManagerFactory.setFocusedManager(this);
                end
        ...
        function [ActionManager] = initActions(this, actionNamespace, startPath, classType)
            if (isempty(this.ActionManager)) || ~isvalid(this.ActionManager)
                peerProvider = internal.matlab.datatoolsservices.actiondataservice.remote.PeerActionDataServiceProvider(actionNamespace);
                ActionManager = internal.matlab.datatoolsservices.actiondataservice.ActionManager(this, peerProvider);
                if (nargin < 4)
                    classType = 'internal.matlab.datatoolsservices.actiondataservice.Action';
                end
                ActionManager.initActions(startPath, classType);            
                this.setProperty('ActionDataServiceNameSpace',actionNamespace);
        ...
        function [provider] = initContextMenu(this, queryString, xmlPath, contextNamespace)           
            this.ContextMenuManager = internal.matlab.datatoolsservices.contextmenuservice.ContextMenuManager.getInstance();
            provider = this.ContextMenuManager.createMenuProvider(xmlPath, contextNamespace, struct('channel', contextNamespace, 'queryString', queryString));
    ...
    methods(Access='public')
        function this = PeerManager(Channel, RootType, IgnoreUpdates)
            this@internal.matlab.legacyvariableeditor.MLManager(IgnoreUpdates);
    ...
    methods
                    ...
                    switch ed.EventData.type
                        case 'OpenVariable'
                            if isfield(ed.EventData,'workspace') && strcmp(ed.EventData.workspace,'test')
                                ...
                                value = internal.matlab.legacyvariableeditor.NullValueObject(ed.EventData.variable);
        ...
        function delete(this)
            ...
            if ~isempty(this.ContextMenuManager) && isvalid(this.ContextMenuManager)
                this.ContextMenuManager.delete;
            end
        end
    ...
    methods(Access='protected')
        ...
        function varDocument = createDocument(this, veVar, userContext, docID)
            root = this.getRoot();
            varDocument = internal.matlab.legacyvariableeditor.peer.PeerDocument(root, this, veVar, userContext, docID);
        ...
        function delayedOpenVar(this, variable, ws, userContext, value)
            if ~isempty(userContext) && isa(userContext,'java.util.HashMap')
                ...
                userContext = internal.matlab.legacyvariableeditor.peer.PeerUtils.toJSON(true, s);
            end
            ...
            if isa(value,'internal.matlab.legacyvariableeditor.NullValueObject')
                % No value was passed in so we need to evaluate the value
                % at a later time in the calling workspace
                imv = 'internal.matlab.legacyvariableeditor';
                ...
                internal.matlab.datatoolsservices.executeCmd(openCmd);
            else
    ...
    methods(Access='public')
        ...
        function classname = getAdapterClassNameForData(this, varClass, varSize, data)
            classname = this.getAdapterClassNameForData@internal.matlab.legacyvariableeditor.MLManager(varClass, varSize, data);
            classname = strrep(classname, 'internal.matlab.legacyvariableeditor.ML', 'internal.matlab.legacyvariableeditor.peer.Peer');
        end
        ...
        function doc = doDelayedDocumentCreation(this)
            ...
            while ~isempty(this.DelayedDocumentList)
                ...
                if ~isa(s.veVar, 'internal.matlab.legacyvariableeditor.VariableEditorMixin')
        ...
        function varDocument = openvar(this, name, ws, data, userContext, delayDocCreation)
            if nargin <= 3 || (~istall(data) && isempty(data)) || isa(data,'internal.matlab.legacyvariableeditor.NullValueObject')
            ...
            if internal.matlab.legacyvariableeditor.peer.PeerUtils.isLiveEditor(userContext)
				...
				w.registerWidgets('internal.matlab.legacyvariableeditor.peer.PeerLogicalArrayViewModel','', 'variableeditor_peer/PeerArrayViewModel','','');
            else
                w = internal.matlab.datatoolsservices.WidgetRegistry.getInstance();
				...
				w.registerWidgets('internal.matlab.legacyvariableeditor.peer.PeerLogicalArrayViewModel','', 'variableeditor/views/NumericArrayView','','');
            end
            ...
            if ~delayDocCreation
                if ~this.isVariableOpen(name, ws)
                    varDocument = this.openvar@internal.matlab.legacyvariableeditor.MLManager(name, ws, data, userContext); %#ok<NASGU>
