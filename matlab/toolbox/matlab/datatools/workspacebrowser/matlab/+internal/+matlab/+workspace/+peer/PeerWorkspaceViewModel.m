classdef PeerWorkspaceViewModel < internal.matlab.legacyvariableeditor.peer.PeerStructureViewModel
    ...
    methods
        function this = PeerWorkspaceViewModel(parentNode, variable, viewID)
            this = this@internal.matlab.legacyvariableeditor.peer.PeerStructureViewModel(parentNode, variable, ...
                            viewID, 'Name', 'Value');            
        ...
        function sendVariableEvent(this, type, Variables)
            internal.matlab.legacyvariableeditor.peer.PeerUtils.sendPeerEvent(this.PeerNode, type, 'Variables', Variables)
        end
        ...
        function varargout = handlePeerEvents(this, ~, ed)
            ...
            if isfield(ed.EventData,'type')
                switch ed.EventData.type
                    ...
                    otherwise
                        this.handlePeerEvents@internal.matlab.legacyvariableeditor.peer.PeerStructureViewModel([], ed);
                end
            end
        end
        ...
        function varargout = handleClientSetData(this, varargin)
            ...
            if (strcmp(fieldName{1}, 'Name'))
                ...
                varargout{1} = ...
                    this.handleClientSetData@internal.matlab.legacyvariableeditor.peer.PeerStructureViewModel(hm);
            end
        end
        ...
        function [renderedData, renderedDims] = getRenderedData(this, ...
            ...
            [data, ~, metadata] = this.getRenderedData@internal.matlab.legacyvariableeditor.StructureViewModel(...
                startRow, endRow, startColumn, endColumn);
            ...
            valueSummary = structfun(@(x) isa(x, 'internal.matlab.workspace.ObjectValueSummary'), dmData);
            valueSummary = valueSummary(startRow:min(endRow, length(valueSummary)));
            if any(valueSummary)
                data = this.getUpdatedDataForObjects(data, dmData, valueSummary, startRow);
            end

            [renderedData, renderedDims] = this.renderData(data, startRow, ...
                endRow, startColumn, endColumn, metadata);
        end
        
        function data = getUpdatedDataForObjects(this, data, dmData, valueSummary, startRow)
            fn = fieldnames(dmData);
            valueSummaryIdx = find(valueSummary)' + (startRow - 1);
            if ~this.SortAscending
                % Flip the data to work with it in order
                data = flipud(data);
            end
            ...
            for idx = valueSummaryIdx
                ...
                data(dataIdx, 2) = dmData.(fn{idx}).DisplayValue;
                data{dataIdx, 3} = dmData.(fn{idx}).DisplaySize;
                data{dataIdx, 4} = dmData.(fn{idx}).DisplayClass;
            end
            
            if ~this.SortAscending
                % Flip the data back to how it was
                data = flipud(data);
            end
        end
    end
    ...
    methods(Access = protected)
        function [renderedData, renderedDims] = refresh(this, ~ ,ed)
            ...
            [renderedData, renderedDims] = this.refresh@internal.matlab.legacyvariableeditor.peer.PeerStructureViewModel([], ed);            
        end        
