classdef VideoWriter < matlab.mixin.SetGet & dynamicprops
    ...
    %   FILENAME is a character vector or string scalar enclosed in single
    %   quotation marks that specifies the name of the file to create. If
    %   filename does not include the extension '.avi', the VideoWriter
    %   constructor appends the extension.
    %
    ...
    %   a VideoWriter object. PROFILE is a character vector or string
    %   scalar enclosed in single quotation marks that describes the type
    %   of file to create. Specifying a profile sets default values for
    %   video properties such as VideoCompressionMethod. Possible values:
    ...
    %                            file in seconds.
    %   FileFormat             - character vector or string scalar, specifying the type of file to write.
    %   Filename               - character vector or string scalar, specifying the name of the file.
    ...
    %                            applies to objects associated with Motion
    %                            JPEG 2000 files. 
    %   Path                   - character vector or string scalar, specifying the fully qualified file
    %                            path.
    ...
    %                            video frame.
    %   VideoCompressionMethod - character vector or string scalar, indicating the type of video 
    %                            compression.
    %   VideoFormat            - character vector or string scalar, indicating the MATLAB representation 
    %                            of the video format.    
    ...
    properties (Dependent, Transient, SetAccess=private)
        Duration; %The total duration of the file, in seconds.
    end
    ...
    properties(SetAccess=private, Hidden, Transient)
        IsOpen = false; %Indicates p4 get //mwpdb/...if the file is open for writing.
    ...
    properties(SetAccess=private, GetAccess=public, Dependent, Hidden)
        % As writing of frames is now asynchronous, this property tracks
        % the number of frames that have been successfully written to the
        % file. Unlike the FrameCount property, this does not block until
        % all frames requested by the user are written to the disk. This is
        % required to support clients such as videoinput.
        NumFramesWrittenToDisk
    end
    ...
    methods
        function obj = VideoWriter(filename, profile)
            ...
            elseif nargin == 2
                filename = convertStringsToChars(filename);
                profile = convertStringsToChars(profile);
            ...
            % File-name and Profile have already been validated at this
            % point.
        ...
        function value = get.NumFramesWrittenToDisk(obj)
            value = obj.Profile.VideoProperties.FrameCount;
        end
    ...
    methods (Hidden)
        ...
        function getdisp(obj)
            if (length(obj) > 1)
                % Defer to built in
                getdisp@matlab.mixin.SetGet(obj);
                return;
            end
    ...
    methods(Static, Access=private)
        ...
        function fullFileName = generateFullOutputFileName(fileName, profile)
            ...
            validateattributes(fileName, {'char', 'string'}, {'scalartext', 'nonempty'}, 'VideoWriter');
            ...
            % If remote location, throw error
            if matlab.io.internal.vfs.validators.hasIriPrefix(fileName)
                error(message('MATLAB:audiovideo:VideoWriter:RemoteNotSupported'));
            end
