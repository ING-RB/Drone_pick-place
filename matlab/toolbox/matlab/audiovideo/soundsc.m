...
if any(isinf(x))
    warning(message('MATLAB:audiovideo:soundsc:infValuesFound'));
end

if any(isnan(x))
    warning(message('MATLAB:audiovideo:soundsc:NaNValuesFound'));
end

user_scale = false;

if (nargin>1) && isequal(size(varargin{end}),[1 2])
    user_scale = true;
    v = varargin{end};
    if ~(isnumeric(v) && ~any(isinf(v)) && v(1)<=v(2))
        error(message('MATLAB:audiovideo:soundsc:invalidSLIM'));
    end
end
