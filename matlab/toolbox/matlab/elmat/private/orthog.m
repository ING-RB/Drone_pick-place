function Q = orthog(n, k, classname)
...
%           Matrix of eigenvectors of the second difference matrix (gallery('tridiag', n)).
...
%   K = 5:  Q(i,j) = (SIN( 2*PI*(i-1)*(j-1)/n ) + COS( 2*PI*(i-1)*(j-1)/n))/SQRT(n).
...
if k == 1
   ...
   m = cast((1:n)'*(1:n) * (1/(n+1)), classname);
   Q = sinpi(m) * sqrt(2/(n+1));

elseif k == 2

   m = cast((1:n)'*(1:n) * (2/(2*n+1)), classname);
   Q = sinpi(m) * (2/sqrt(2*n+1));

elseif k == 3                          % Vandermonde based on roots of unity

   m = cast(0:n-1, classname);
   m = m'*m*(2/n);
   Q = (cospi(m) + 1i*sinpi(m)) / sqrt(n);
...
elseif k == 5                          % Hartley matrix
   ...
   m = cast((0:n-1)'*(0:n-1) * (2/n), classname);
   Q = (cospi(m) + sinpi(m))/sqrt(n);

elseif k == 6                          % DCT4

   m = cast((1:n)' -1/2, classname);
   m = (m*m')/n;
   Q = sqrt(2/n)*cospi(m);
...
elseif k == -1
                                       % Extrema of T(n-1)
   m = cast((0:n-1)'*(0:n-1) * (1/max(1,n-1)), classname);
   Q = cospi(m);

elseif k == -2
                                       % Zeros of T(n)
   m = cast((0:n-1)'*(.5:n-.5) * (1/n), classname);
   Q = cospi(m);
