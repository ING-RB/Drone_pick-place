function [helpNode, helpstr, fcnName, found] = help2xml(dom, topic, pagetitle, helpCommandOption)
    ...
    if (nargin>0 && (iscell(topic) || size(topic,1) > 1 || any(find(topic==newline))))
        ...
        pgtitle = matlab.internal.help.fixsymbols(pgtitle);
    ...
    if hasLocalFunction
        fcnName =  matlab.lang.internal.introspective.extractCaseCorrectedName(localTopic, topic);
        qualifiedTopic = localTopic;
    else
        nameResolver = matlab.lang.internal.introspective.resolveName(topic, JustChecking=false, FindBuiltins=true);
        ...
        if ~isempty(classInfo)
            ...
            if isempty(viewFormat)
                ...
                [fcnName, qualifyingPath] = matlab.lang.internal.introspective.fixFileNameCase([fcnName, extension], qualifyingPath);
...
function topic = handleClassInfo(classInfo,topic,dom,helpNode)
    ...
    if ~isempty(metacls)
        if classInfo.isConstructor
            ...
            matlab.internal.help.class2xml.buildConstructorXml(constructorMeta, dom, helpNode);

        elseif classInfo.isMethod
            methodName = regexp(topic,'\w+$','once','match');

            methodMeta = findClassMemberMeta(metacls.Methods, methodName);
            matlab.internal.help.class2xml.buildMethodXml(metacls, methodMeta, dom, helpNode);
        ...
        elseif classInfo.isSimpleElement
            ...
            case 'properties'
                matlab.internal.help.class2xml.buildPropertyXml(metacls, classElement, dom, helpNode);
            case 'events'
                matlab.internal.help.class2xml.buildEventXml(metacls, classElement, dom, helpNode);
            case 'enumeration'
                matlab.internal.help.class2xml.buildEnumerationXml(metacls, classElement, dom, helpNode);
            end
...
function class2xmlObj = getClass2XmlObj(classFilePath, metaInfo)
    ...
    class2xmlObj = matlab.internal.help.class2xml(helpContainerObj);
...
function nameForTitle = buildMethodTitleName(classInfo)
    fullClassName = matlab.lang.internal.introspective.makePackagedName(classInfo.packageName, classInfo.className);
