...
if (nargin < 1)
   error(message('MATLAB:playshow:ShowUnspecified'));
end
...
case 'initialize'
   ...
   switch layout
      ...
      otherwise
         error(message('MATLAB:playshow:unsupportedOption'));
   end
...
fig=figure( ...
   'Name',getString(message('MATLAB:playshow:FigureSlideshowPlayer')), ...
   'NumberTitle','off', ...
   'IntegerHandle','off', ...
   'Visible','off', ...
   'MenuBar','none', ...
...
% The Next button
slideData.nextHandle = buildButton(getString(message('MATLAB:playshow:ButtonStart')),[mfilename ' #next'],'next');
% The Reset button
slideData.resetHandle = buildButton(getString(message('MATLAB:playshow:ButtonReset')),[mfilename ' #reset'],'reset');
% The View code button
if slideData.isSlideScript
   action = ['edit ' filename];
   slideData.viewHandle = buildButton(getString(message('MATLAB:playshow:ButtonViewCode')),action,'info');
else
   action = ['helpwin ' filename];
   slideData.viewHandle = buildButton(getString(message('MATLAB:playshow:ButtonMoreInfo')),action,'info');
end
...
try
   % Slurp in the file.
   f = fopen(locateMFile(filename));
   err = getString(message('MATLAB:playshow:NotAValidPLAYSHOWFile',filename));
catch
   err = getString(message('MATLAB:playshow:NotOnThePath',filename));
end

try
   F = fread(f, 'uint8=>uint8');
   fclose(f);
   txt = char(F');
...
%===============================================================================
function closePlayshowFigure(fig,unused) %#ok<INUSD> Callbacks require functions with two arguments.
...
set(slideData.labelHandle, ...
   'String',getString(message('MATLAB:playshow:SlideNumberOfN',slideNumber,slideshowLength)))
drawnow;
...
if (i==1)
   nextString = getString(message('MATLAB:playshow:ButtonStart'));
   resetEnable = 'off';
else
   nextString = getString(message('MATLAB:playshow:ButtonNext'));
   resetEnable = 'on';
end
...
% Evaluate the code.
slideData.warningState = warning('off','backtrace');
...
try
   ...
   beep;
   fprintf('%s',getString(message('MATLAB:playshow:ErrorInSlideshow',e.message)))
...
f = fopen(locateMFile(filename));
if (f == -1)
    error(message('MATLAB:playshow:NotOnThePath',filename));
end
F = fread(f, 'uint8=>uint8');
fclose(f);
txt = char(F');
...
disp(' ');
disp(getString(message('MATLAB:playshow:EndOfDemo')))
disp(' ');

%==========================================================================
function nextPage
%nextPage Ready the screen for the next step.
%   When running -nodesktop, HOME doesn't scroll the screen, so we have to
%   use CLC.  We want to use HOME when we can because it preserves the
%   previous commands.
...
if ~any(strcmp(type,{'beginning','trailing','both'}))
    error(message('MATLAB:removeExtraNewlines:BadType'))
end

if ~isequal(type,'trailing')
    % Remove extra newlines from the beginning.
    text = text(find((text ~= newline),1):end);
end
