...
%   Copyright 2017-2022 The MathWorks, Inc.
    ...
    properties(Access=private)
        ...
        Issues            % backend codeIssues object
    end
    ...
    methods(Hidden, Static)
        function obj = create(issues)
            arguments
                issues codeIssues
            end

            % Get all the checks run in the analysis.
            % The checks are returned in a table.
            checks = matlab.internal.codecompatibilityreport.codeAnalyzerChecks;
            rerunConfiguration.includeSubfolders = ~issues.modelMessages.topFolderOnly;
            rerunConfiguration.resolvedNameList = string([issues.modelMessages.folders.toArray(); issues.modelMessages.files.toArray()]);

            % Using modelMessages to get all issues, including suppressed
            % issues.
            obj = CodeCompatibilityAnalysis(issues.Files, checks, createLocations(issues.modelMessages), rerunConfiguration);
            obj.Issues = issues;
        end
    end
    methods(Hidden)
    ...
    methods(Hidden)
        ...
        % backend Server
        function server = matlab.codeanalyzerreport.internal.Server(obj)
            server = matlab.codeanalyzerreport.internal.Server(obj.Issues, IsCompatibilityReport=true);
        end
    end
end

function locations = createLocations(modelMessages)
    msgArray = modelMessages.getAllMessages();
    if isempty(msgArray)
        File = string.empty(0, 1);
        LineNumber = double.empty(0, 1);
        ColumnRange = double.empty(0, 2);
        Severity = categorical.empty(0, 1);
        Suppression = categorical.empty(0, 1);
        Identifier = categorical.empty(0, 1);
        Description = string.empty(0, 1);
    else
        File = string({msgArray.filename})';
        LineNumber = double([msgArray.startLine]');
        ColumnRange = double([msgArray.startColumn; msgArray.endColumn]');
        Severity = categorical({msgArray.severityId})';
        Suppression = categorical({msgArray.suppression})';
        Identifier = categorical({msgArray.tag})';
        Description = string({msgArray.message})';
    end

    locations = table(Identifier, Description, Severity, Suppression, File, LineNumber, ColumnRange);
end
