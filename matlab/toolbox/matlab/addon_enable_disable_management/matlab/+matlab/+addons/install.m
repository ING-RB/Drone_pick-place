...
%   NEWADDON = MATLAB.ADDONS.INSTALL(FILENAME,INSTALLOPTION) installs the
%   add-on specified by FILENAME using the specified INSTALLOPTION and
%   returns information about the installed add-on. FILENAME must be a
%   valid toolbox installation file (*.mltbx). You can specify FILENAME
...
if hasLicense(toolboxPackage) && ~agreeToLicense
    ...
    agreeButtonLabel = message('matlab_addons:install:licenseAgreementDialogAgreeButtonLabel').getString;
    cancelButtonLabel = message('matlab_addons:install:licenseAgreementDialogCancelButtonLabel').getString;
    text = toolboxPackage.getLicense.text;
    agreedToLicense = matlab.internal.addons.installer.createLicenseAgreementDialog(title, text, agreeButtonLabel, cancelButtonLabel);
    if agreedToLicense
        launchInstallation;
    end
else
    ...
    function launchInstallation
        if (strcmpi(installOption, installOptionOverwrite) == 1)
            ...
            if hasAddOnWithIdentifierAndVersion(string(identifier), string(version))
        ...
        if hasAnyVersionInstalled(identifier) && installedAddonsCache.hasEnabledVersion(identifier)
