...
function create(file, varargin)
   % narginchk(1,2);
    ...
    p.addOptional('doPackage',true);

    p.parse(varargin{:});
    
    dependencyOption = p.Results.dependencyOption; 
    doPackage = p.Results.doPackage;
    ...
    if isdir(file)
        ...
        configKey = service.createNewProject(java.lang.String(projectFileString));
        
        % Add the root folder and update the fileset
        service.setToolboxRoot(configKey, rootPath);
        
        % Examples, apps, and doc
        toolboxDependencies = matlab.depfun.internal.format.ToolboxPackaging.findDependencies(rootPath, []);
