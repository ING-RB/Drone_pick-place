classdef appbuilder < handle
    ...
    methods
        ...
        function obj = appbuilder(appname, entrypointfile, fileslist, ...
            ...
            try
                ...
                entryPoint = appcreate.internal.appbuilder.qualifiedName(entrypointfile);
                ...
                if (isempty(varargin) || numel(varargin) == 1 && isempty(varargin{1}))
                    ...
                    obj.AppScaledIcons = {};
                    obj.GUID = char(matlab.lang.internal.uuid);
    ...
    methods(Static)
        ...
        function [f,msg] = foldersInMatlabRoot(varargin)
            f=[]; msg='';
            containsList = contains(varargin, [matlabroot, filesep]);
            isMLRoot = strcmp(varargin, matlabroot);
            errorList = containsList | isMLRoot;
            if any(errorList)
                msg = getString(message('MATLAB:apps:errorcheck:CannotPackageMathWorksFiles'));
                f = varargin(errorList);
            end
        end
        ...
        function [f,msg] = filesNotToPackage(varargin)
            f=[]; msg='';
            try
                isFolder = cellfun(@(x) exist(x, 'dir'), varargin) == 7;
                cellOfFolders = varargin(isFolder);
                cellOfFiles = varargin(~isFolder);

                if ~isempty(cellOfFolders)
                    [f,msg] = appcreate.internal.appbuilder.foldersInMatlabRoot(cellOfFolders{:});
                end

                if ~isempty(cellOfFiles)
                    isDeployableFunc=@(x) matlab.depfun.internal.isdeployable(x, 'MATLAB', false);
                    [allSuccess,allMessages]=cellfun(isDeployableFunc,cellOfFiles,'UniformOutput',0);
