classdef lsm6dsm < lsm6dsl
    %LSM6DSM connects to the LSM6DSM sensor connected to a hardware object
    %
    %   IMU = lsm6dsm(a) returns a System object, IMU that reads sensor
    %   data from the LSM6DSM sensor connected to the I2C bus of an
    %   hardware board. 'a' is a hardware object.
    %
    %   IMU = lsm6dsm(a, 'Name', Value, ...) returns a LSM6DSM System object
    ...
    %
    %   lsm6dsm Properties
    %   I2CAddress      : Specify the I2C Address of the LSM6DSM.
    ...
    %
    %  Note: For targets other than Arduino, lsm6dsm object is supported 
    ...
    %
    %   Example 1: Read one sample of acceleration value from LSM6DSM sensor
    %
    %   a = arduino('COM3','Uno','Libraries','I2C'); % Create arduino object with I2C library included
    %   sensorObj = lsm6dsm(a);
    %   accelData  =  sensorObj.readAcceleration;
    %
    %   Example 2: Read and plot acceleration values from an LSM6DSM sensor
    %
    %   a = arduino('COM3','Uno','Libraries','I2C'); % create arduino object with I2C library included
    %   sensorObj = lsm6dsm(a,'SampleRate',100,'SamplesPerRead',50);
    %   sensorObj.OutputFormat = 'matrix';
    %   figure;
    %   xlabel('Samples read');
    %   ylabel('Acceleration (m/s^2)');
    %   title('Acceleration values from LSM6DSM sensor');
    ...
    %   clear
    %
    %   See also lsm6ds3h, lsm6ds3, mpu6050, lsm6dsl, lsm6dso, lsm6dsr, read,
    %   readAcceleration, readAngularVelocity, readTemperature
    
    %   Copyright 2020 The MathWorks, Inc.
    %#codegen
    % W.r.t to current features, lsm6dsm is equivalent to lsm6dsl
    methods
        function obj = lsm6dsm(varargin)
            obj@lsm6dsl(varargin{:})
        end
    end
end
