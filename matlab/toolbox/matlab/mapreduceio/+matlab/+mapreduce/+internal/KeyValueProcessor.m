...
if iIsCellScalarNumericLogical(input, keysOrValuesStr)
    ...
    if ~isempty(input)
        cellClass = class(input{1});
        inputIsCellClass = cellfun('isclass', input, cellClass);
        if ~all(inputIsCellClass(:))
...
elseif ~matlab.io.internal.validators.isCellOfCharVectors(input)
...
import matlab.mapreduce.internal.validateLogicalAndNumeric;
if matlab.io.internal.validators.isCharVector(input)
    input = {input};
