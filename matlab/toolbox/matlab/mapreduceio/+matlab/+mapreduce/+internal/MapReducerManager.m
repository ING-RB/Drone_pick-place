...
classdef (Sealed) MapReducerManager < handle
    ...
    methods
        ...
        function mr = getDefault(obj)
            ...
            % Check if the auto-create pool option has been enabled since
            % the last call without a parallel MapReducer instance. If so,
            % clear the cached default MapReducer, it might be a new
            % MapReducer instance.
            if iIsParallelAvailable() ...
                    && parallel.internal.bigdata.ParallelPoolExecutor.isAutoCreateEnabled() ...
                    && ~isa(obj.CachedDefault, 'matlab.mapreduce.ParallelMapReducer')
                invalidateDefaultCache(obj);
            end
    ...
    methods (Hidden)
        ...
        function obj = MapReducerManager(isParallelAvailable)
            ...
            obj.StartOfStackNodes = MapReducerStackNode();
            if obj.IsParallelAvailable
                obj.PoolAddedListener = parallel.internal.parpool.subscribePoolCreation(...
                    parallel.internal.general.weakCallbackMethod(obj, @(m, ~) invalidateDefaultCache(m)));
            end
        end
    end
    ...
    methods (Access = private)
        ...
        function findNextCurrent(obj)
            ...
            while isvalid(node)
                nextMapReducer = node.MapReducer;
...
if matlab.internal.parallel.isPCTInstalled()
    isParallelAvailable = ~parallel.internal.pool.isPoolThreadWorker() ...
