...
%   Copyright 2015-2024 The MathWorks, Inc.
    ...
    for k=1:numel(fn)
        ...
        if isa(fn{k},'sym') && ...
            ~all(feval2logicalCast(symengine,'symobj::map',formula(fn{k}),'testtype','Type::Arithmetical', false))
    ...
    if doMatchBackgroundColor
        ax = ancestor(cax,'matlab.graphics.axis.Axes'); % cax could be HGTransform, etc.
        matlab.graphics.internal.matchBackgroundColor(ax,hObj,'FaceColor');
    end
    switch nextPlot
...
function hObj=singleFsurf(cax,fn,extraOpts,args)
    ...
    if ishghandle(ax,'axes')
        ...
        [autostyle,~,automarker] = matlab.graphics.chart.internal.nextstyle(ax,false,useNextLineStyle || useNextMarker,false);
        if useNextLineStyle
