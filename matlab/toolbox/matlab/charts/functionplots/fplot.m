...
if nargin > 0
    ...
    calledForPolar = false;
    if nargin>0 && isequal(varargin{end},'CalledForPolar')
        calledForPolar = true;
        varargin(end) = [];
    end
    ...
    ispolar = ~isempty(cax) && isa(cax, 'matlab.graphics.axis.PolarAxes');
    ...
    linespecFound = false;
    if calledForPolar
        extraOpts = [extraOpts {'XRange_I', [0, 2*pi]}];
    end
    ...
    if numel(fn)==1
        if ischar(fn{1})
            ...
            warning(message('MATLAB:fplot:StringFunctionsDeprecated',fstr));
        end
    end
    ...
    for k=1:numel(fn)
        ...
        if isa(fn{k},'sym') && ...
            ~all(feval2sym_NaNsingularity(symengine,'symobj::map',formula(fn{k}),'testtype','Type::Arithmetical'))
            error(message('MATLAB:fplot:InvalidExpression'));
        end
    end

    fnDisplay = ''; % default sentinel
    ...
    if numel(fn)==1
        ...
        if ~isempty(cax) && isa(cax,'matlab.graphics.axis.PolarAxes')
            validParameters{end+1} = 'ThetaRange';
        end
    else
    ...
    nextPlot = 'add';
    if doPlot && (isempty(cax) || isa(cax,'matlab.graphics.axis.AbstractAxes'))
        cax = newplot(cax);
    ...
    for i = 1:numel(hObj)
        ...
        if  ~isscalar(hObj) && ~isempty(fnDisplay)
            hObj(i).updateDisplayName(fnDisplay,i);
        end
    end
...
function hObj=singleFplot(cax,fn,limits,extraOpts,args)
    ...
    if ishghandle(ax,'axes')
        useNextColor = ~any(cellfun(@(x) isequal(x,'Color'),args)) && ~any(cellfun(@(x) isequal(x,'Color'),extraOpts));
        useNextLineStyle = ~any(cellfun(@(x) isequal(x,'LineStyle'),args)) && ~any(cellfun(@(x) isequal(x,'LineStyle'),extraOpts));
        useNextMarker = ~any(cellfun(@(x) isequal(x,'Marker'),args)) && ~any(cellfun(@(x) isequal(x,'Marker'),extraOpts));

        [autostyle,autocolor,automarker] = matlab.graphics.chart.internal.nextstyle(ax,useNextColor,useNextLineStyle || useNextMarker, true);
        if useNextColor
            extraOpts = [extraOpts, {'Color_I'}, {autocolor}];
        end
        if useNextLineStyle
            extraOpts = [extraOpts, {'LineStyle_I'}, {autostyle}];
        end
        if useNextMarker
            extraOpts = [extraOpts, {'Marker_I'}, {automarker}];
        end
    end
