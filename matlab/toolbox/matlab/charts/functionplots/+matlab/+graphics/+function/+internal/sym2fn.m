function fn = sym2fn(sf,varargin)
  ...
  if nargin > 1
    ...
    while any(strcmp(vars,'~'))
      vars{find(strcmp(vars,'~'),1)} = evalin2char(symengine,'genident()');
    end
    var_f = sym(vars);
  else
    var_f = symvar(sf,2);
  ...
  units = findUnits(sf);
  if ~isempty(units)
    [sf,units] = separateUnits(sf); % errors for inconsistent units
    units = formula(units);
    for unit=units
      warning(message('symbolic:generate:UnitsNotSupported',char(unit),1));
    end
    sf = subs(sf,units,1);
  end
  ...
  try
    ...
    fn = @(varargin) double(feval2sym_NaNsingularity(symengine,'symobj::evalForPlot',f2,varargin{:}));
  end
...
function y = tryBoth(f1, f2, varargin)
  prevWarning = warning('off','MATLAB:integral:MaxIntervalCountReached');
  t = onCleanup(@() warning(prevWarning));
  try
    y = double(feval(f1,varargin{:}));
  catch
    y = double(feval2sym_NaNsingularity(symengine,'symobj::evalForPlot',f2,varargin{:}));
  end
end
