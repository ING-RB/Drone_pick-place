function poleCandidates = findPoles(y, pixelCoordinatesX, pixelCoordinatesY, lim)
  %
...
%   Copyright 2015-2023 The MathWorks, Inc.
  ...
  if numel(pixelCoordinatesX) > 2
    curve = diff(slopes);
    curveLeft = [curve curve(end) curve(end)].*y > 1e-3;
    curveRight = [curve(1) curve(1) curve].*y > 1e-3;
    poleCandidates = poleCandidates & ((curveLeft & [curveLeft(2:end) false]) | (curveRight & [false curveRight(1:end-1)]));
  end
  % (d) the same for still higher derivatives
  if numel(pixelCoordinatesX) > 3
    curve3 = diff(curve);
    curveLeft = [curve3 curve3(end) curve3(end) curve3(end)].*y > 1e-3;
    curveRight = [curve3(1) curve3(1) curve3(1) curve3].*y > 1e-3;
    poleCandidates = poleCandidates & ((curveLeft & [curveLeft(2:end) false]) | (curveRight & [false curveRight(1:end-1)]));
  end
  ...
  % (g) neither are points admissible as pole candidates that are within any explicitly set axes limits
  if ~isempty(lim)
    poleCandidates = poleCandidates & (y < lim(1) | y > lim(2));
  end
end
