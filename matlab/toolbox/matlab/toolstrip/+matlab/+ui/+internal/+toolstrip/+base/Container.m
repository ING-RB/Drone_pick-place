classdef (Abstract) Container < matlab.ui.internal.toolstrip.base.Component
    ...
    methods
        ...
        function objs = contains(this, str)
            % Method "contains":
            %
            %   "contains(container, str)": return all the objects in the
            %   container whose tags contain the specified string or [] if
            %   not found. The container can be toolstrip, tabgroup, tab,
            %   section, panel, column and popuplist.
            %
            %   Example:
            %       ts1 = matlab.ui.internal.toolstrip.Toolstrip('ts1')
            %       tab1 = ts1.addTab('tab1','title1')
            %       tab1.addSection('sec1','title2')
            %       obj = ts1.contains('sec') % returns "sec1" handle 

            % Process input
            str = matlab.ui.internal.toolstrip.base.Utility.hString2Char(str);
            if ischar(str)
                objs = tree_deep_contains_all(this, str);
            else
                error(message('MATLAB:toolstrip:container:invalidContains'));
            end
        end
    ...
    methods (Hidden)
        ...
        function attach(this, channel, peerId)
            % Method "attach"
            % 
            % find the peer node for the container and attach this MCOS object

            if ~hasPeerNode(this)
                this.PeerModelChannel = channel;
                this.attachToPeer(peerId);
            end
        end
