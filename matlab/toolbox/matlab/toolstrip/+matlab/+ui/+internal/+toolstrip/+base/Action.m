classdef Action < handle & dynamicprops & matlab.ui.internal.toolstrip.base.ActionInterface
    % Action Class
    %
    ...
    methods
        ...
        function addProperty(this, name)
            ...
            switch name
                ...
                case 'DecimalFormat'
                    prop = this.addprop('DecimalFormat');
                    prop.Dependent = true;
                    prop.GetMethod = @(this) getDecimalFormatProperty(this);
                    prop.SetMethod = @(this, data) setDecimalFormatProperty(this, data);
                    this.MCOSProperties = [this.MCOSProperties; {'DecimalFormatPrivate'}];
                    this.PeerProperties = [this.PeerProperties; {'format'}];
                case 'Steps'
    ...
    methods (Access = public)
        ...
        % DecimalFormat: must be 'nf' or 'ne'.  Append '%0.' in front and
        % use lower case for e and f
        function value = getDecimalFormatProperty(this)
            value = this.DecimalFormatPrivate(4:end);
        end        
        function setDecimalFormatProperty(this, value)
            [OK, value] = matlab.ui.internal.toolstrip.base.Utility.validate(value, 'string');
            if OK
                if ~(strcmpi(value(end),'e') || strcmpi(value(end),'f'))
                    error(message('MATLAB:toolstrip:control:invalidDecimalFormat'))
                end
                n = str2double(value(1:end-1));
                if isempty(n) || isnan(n) || ~isfinite(n) || mod(n,1)~=0
                    error(message('MATLAB:toolstrip:control:invalidDecimalFormat'))
                end
                this.DecimalFormatPrivate = ['%0.' lower(value)];
                this.setPeerProperty('format', this.DecimalFormatPrivate);
            else
                error(message('MATLAB:toolstrip:control:invalidDecimalFormat'))
            end
        end
        ...
        function PropertySetCallback(this,src,data) %#ok<*INUSL>
            ...
            if viewmodel.internal.factory.ManagerFactoryProducer.isViewModel(this.Peer)
                if (strcmp(data.SrcLang, 'JS'))
                    % get event data
                    eventdata = matlab.ui.internal.toolstrip.base.Utility.processViewModelPropertySetData(data);
            ...
            if executeCallbacks
                ...
                switch eventdata.EventData.Property
                    ...
                    case 'Text'
                        ...
                        eventdata.EventData.Property = 'Value';
                        if ~isempty(findprop(this,'ItemSelectedFcn'))
                            % In case of an editable Dropdown, ignore this
                            % property set because SelectedItem should take
                            % care of ValueChanged.
                            eventdata.EventData.IgnorePropertySet = true;
                        end
                        % run callback fcn
        ...
        function PeerEventCallback(this,src,data)
            % get event data
            if viewmodel.internal.factory.ManagerFactoryProducer.isViewModel(this.Peer)       
                eventdata = matlab.ui.internal.toolstrip.base.Utility.processViewModelEventDataForActions(data);
            else
                eventdata = matlab.ui.internal.toolstrip.base.Utility.processPeerEventDataForActions(data);
            end
