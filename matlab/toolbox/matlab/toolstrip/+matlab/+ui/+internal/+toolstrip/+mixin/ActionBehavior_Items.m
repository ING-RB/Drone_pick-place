    ...
    methods
        ...
        function this = replaceAllItems(this, items)
            ...
            if matlab.ui.internal.toolstrip.base.Utility.validate(items, 'stringNx1')
                % g2631359, g2580001 update selectedIndex value if the new items do not contain the previously selected item

                if ~isempty(this.Value) && ~isempty(setdiff(this.Value, cellstr(items)))
                    this.SelectedIndex = -1;
                end
                action.Items = [items items];
            elseif matlab.ui.internal.toolstrip.base.Utility.validate(items, 'stringNx2')
                % g2631359, g2580001 update selectedIndex value if the new items do not contain the previously selected item
                if ~isempty(this.Value) && ~isempty(setdiff(this.Value, cellstr(items(:,1))))
                    this.SelectedIndex = -1;
                end
                action.Items = items;
            else
