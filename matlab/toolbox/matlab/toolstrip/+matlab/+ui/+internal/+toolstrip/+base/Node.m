classdef Node < matlab.mixin.Heterogeneous & handle
    ...
    methods (Hidden, Access = protected)
        ...
        function objs = tree_flat_contains_all(parent, str)
            %% Return all the components with the partially matched tag if they are in the hierarchy.  Return [] if no such component is found.
            children = parent.Children;
            objs = [];
            for k = 1:length(children)
                if contains(children(k).Tag, str)
                    objs = [objs; children(k)]; %#ok<*AGROW>
                end
            end
        end
        
        function objs = tree_deep_contains_all(parent, str)
            %% Return all the components with the partially matched tag if they are in the hierarchy.  Return [] if no such component is found.
            objs = tree_flat_contains_all(parent, str);
            children = parent.Children;
            for k = 1:length(children)
                objs = [objs; tree_deep_contains_all(children(k), str)];
            end
        end
