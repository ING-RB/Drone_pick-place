classdef (Abstract) ActionBehavior_Popup < handle
    ...
    methods
        ...
        function set.Popup(this, value)
            ...
            if isempty(value)
                if hasPeerNode(this)
                    if viewmodel.internal.factory.ManagerFactoryProducer.isViewModel(action.getPeer())
                        action.getPeer().setProperty('popupId','');
                    else
                ...
                if hasPeerNode(this)
                    ...
                    if viewmodel.internal.factory.ManagerFactoryProducer.isViewModel(action.getPeer())
                        action.getPeer().setProperty('popupId',value.getId());
                    else
                        source = java.util.HashMap();
                        source.put('source','MCOS');
