classdef Utility < handle
    ...
    methods (Static)
        ...
        function eventdata = processViewModelPropertySetData(peerdata)
            % get event data
            structure = peerdata.Data;
            % get property name
            structure.Property = getPropertyfromKey(structure.key);
            structure = rmfield(structure,'key');
            structure.NewValue = structure.newValue;
        ...
        function eventdata = processPeerEventDataForActions(peerdata)
            % get event data
            hashmap = peerdata.getData();
            if hashmap.isEmpty
        ...
        function eventdata = processViewModelEventDataForActions(peerdata)
            structure = peerdata.Data;
            ...
            % get event type and remove the "eventType" field
            if isfield(structure,'eventType')
                structure.EventType = getEventfromKey(structure.eventType);
                structure = rmfield(structure,'eventType');
            else
                structure.EventType = '';
        ...
        function eventdata = processPeerEventData(peerdata)
            if ~isa(peerdata, 'com.mathworks.peermodel.impl.EventImpl')
                % get event data
                peerData = peerdata.data;
                if isempty(peerData) || isempty(fieldnames(peerData))
                    eventdata = matlab.ui.internal.toolstrip.base.ToolstripEventData(struct('EventType','ForcedSync'));
                else
                    structure = peerData;
                    % remove "type" field if present
                    ...
                    % get event type and remove the "eventType" field
                    structure.EventType = 'placeHolder';
                    if isfield(structure,'eventType')
                        structure.EventType = getEventfromKey(structure.eventType);
                        structure = rmfield(structure,'eventType');
                    end
                ...
                eventdata = matlab.ui.internal.toolstrip.base.Utility.processPeerEventDataForActions(peerdata);
            end
        end
