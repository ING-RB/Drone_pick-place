        ...
        function PeerEventCallback(this,~,data)
            ...
            if strcmp(eventdata.EventData.EventType,'DropDownPerformed')
                if ~isempty(this.DynamicPopupFcn)
                    ...
                    if ~isempty(action)
                        % g2492061: flush event queue, so popup model on JS is
                        % up-to-date. This is essential in scenarios where popup is
                        % created dynamically, and Dojo needs the model to determine
                        % the position (to the right of dropdown, or to the left) of
                        % popup.
                        message.type = 'flush_properties';
                        message.eventType = 'peerEvent';
                        action.dispatchEvent(message);
                    end
                end
