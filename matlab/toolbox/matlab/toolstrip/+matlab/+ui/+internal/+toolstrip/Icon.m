classdef Icon < handle
    ...
    %               |   Image File   |   Image Array   |   ImageIcon   |   Built-In   |   CSS Class   |
    % ImageSource     PNG path (S+J)     3D uint8       SwingIcon (S+J)   PNG path (S)       ''
    % ImageClass           ''               ''                 ''         CSS class (J)   CSS class (J)
    %
    % PeerNode         icon=base64      icon=base64       icon=base64     icon=class      icon=class
    % PeerNode        iconpath=path     iconpath=''       iconpath=''     iconpath=path   iconpath=''
    ...
    methods
        ...
        function this = Icon(source, imagefile)
            ...
            %
            %   (3) Construct a custom icon from a javax.swing.ImageIcon
            %   object (Java)
            %
            %   Example:
            ...
            if nargin == 1
                ...
                elseif string(class(source)) == "uint8"
                    this.ImageSource = javax.swing.ImageIcon(matlab.graphics.internal.im2java(source));
                    this.ImageClass = '';
                    this.Description_ = '';
                ...
                this.ImageSource = fullfile(toolboxdir('shared'),'controllib','general','resources','toolstrip_icons',[imagefile '.png']);
        ...
        function url = getBase64URL(this)
            ...
            if isa(this.ImageSource,'javax.swing.ImageIcon') && ...
                ...
                if ~contains(imagefile,'jar!')
                    ...
                    bytes = fread(fid);
                    fclose(fid);
                    url = localFromByteToURL(bytes,type);
                else
                    ...
                    url = localFromImgToURL(img,type);
                end
            end
        end
    ...
    methods (Static)
        ...
        function showStandardIcons()
            ...
            %
            %   Display all the available standard icons with their ids.
            web('https://iconidcatalog.mathworks.com/results?category=Toolstrip%20icons&query=', '-browser');
        end

        % Used for Java-based Rendering
...
function url = localFromImgToURL(img,type)
...
function url = localFromByteToURL(bytes,type)
    % ImageIcons - int8
    % Standard Icons/ImagePath specified/CSS class name specified - double
    if isa(bytes, 'int8')
        str = matlab.net.base64encode(bytes);
    else
        str = matlab.net.base64encode(uint8(bytes));
    end
