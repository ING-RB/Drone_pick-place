function createSlider(this, widget_node)
    ...
    usesmallfont = widget_node.getProperty('useSmallFont');
    localSetLabel(labels, locations, jh, usesmallfont);
...
function localSetLabel(labels, locations, jh, usesmallfont)
    if any(labels.size==0)
        ...
        for ct=1:len
            jlabel = javaObjectEDT('com.mathworks.mwswing.MJLabel',labels(ct));
            font = jlabel.getFont();
            if usesmallfont
                font = font.deriveFont(com.mathworks.util.ResolutionUtils.scaleSize(10));
            else
                font = font.deriveFont(com.mathworks.util.ResolutionUtils.scaleSize(11));
            end
            setFont(jlabel,font);
            if isjava(locations)
                labeltable.put(java.lang.Integer(locations(ct).intValue),jlabel);
            else
                labeltable.put(java.lang.Integer(locations(ct)),jlabel);
            end
        end
        % display
