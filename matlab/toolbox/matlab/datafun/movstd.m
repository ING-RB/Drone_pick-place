function M = movstd(A,k,varargin)
% Syntax:
%     M = movstd(A,k)
%     M = movstd(A,[kb kf])
%     M = movstd(___,w)
...
if ~isduration(k) && ~istabular(A)
    try
        M = matlab.internal.math.movstd(A,k,varargin{:});
    catch ME
    ...
    M = matlab.internal.math.applyMovFun("movstd", @matlab.internal.math.movstdInternal, hasBiasOption, omitNaNByDefault, A, k, varargin{:});
end
end