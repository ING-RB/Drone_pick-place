function M = movmedian(A,k,varargin)
% Syntax:
%     M = movmedian(A,k)
%     M = movmedian(A,[kb kf])
...
if ~isduration(k) && ~istabular(A)
    try
        M = matlab.internal.math.movmedian(A,k,varargin{:});
    catch ME
    ...
    M = matlab.internal.math.applyMovFun("movmedian", @matlab.internal.math.movmedianInternal, hasBiasOption, omitNaNByDefault, A, k, varargin{:});
end