function M = movmad(A,k,varargin)
% Syntax:
%     M = movmad(A,k)
%     M = movmad(A,[kb kf])
...
if ~isduration(k) && ~istabular(A)
    try
        M = matlab.internal.math.movmad(A,k,varargin{:});
    catch ME
    ...
    M = matlab.internal.math.applyMovFun("movmad", @matlab.internal.math.movmadInternal, hasBiasOption, omitNaNByDefault, A, k, varargin{:});
end