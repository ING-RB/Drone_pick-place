...
% Parse node input data.
    ...
    elseif iscell(pts)
        ...
        for i = 1:length(pts)
            ...
            tf = matlab.internal.math.checkUniformNufftGrid(pts{i});
            if ~tf && matlab.internal.math.checkUniformNufftGrid(flip(pts{i}))
                tf = true;
                pts{i} = flip(pts{i});
...
% N-D NUFFT via interpolation.
    ...
    if qnodes.type == "scattered"
        ...
        if any(nonSingletonDims)
            ...
            y = matlab.internal.math.nufftninterp(reshape(x, [N, 1]), ...
                [], qnodes.values);
        else
        ...
        if needsShift
            ...
            % Implicit query points are by default of the form
            % (0:(N-1))/N, but the builtin expects them to be in the range
            % [0 N), so the sample points need the scaling factor.
            N = qnodes.count;
            snodes.values = mod(snodes.values ./ N(:)', 1); 
        end
        % Squeeze out singleton dimensions.
        nonSingletonDims = N > 1;
        if any(nonSingletonDims)
            ...
            % Compute the transform.
            y = matlab.internal.math.nufftninterp(x, ...
                snodes.values, N);
        else
            y = sum(x(:));
        end
