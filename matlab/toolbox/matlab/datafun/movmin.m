function M = movmin(A,k,varargin)
% Syntax:
%     M = movmin(A,k)
%     M = movmin(A,[kb kf])
...
if ~isduration(k) && ~istabular(A)
    try
        M = matlab.internal.math.movmin(A,k,varargin{:});
    catch ME
    ...
    M = matlab.internal.math.applyMovFun("movmin", @matlab.internal.math.movminInternal, hasBiasOption, omitNaNByDefault, A, k, varargin{:});
end