function IA = ismissingKernel(A,indicators,stdize,dataVars,tableOutput)
...
if nargin <= 1 || (nargin == 5 && isempty(indicators))    
    if ~istabular(A)
        % ismissing(A)
        IA = arraySwitch(A,false);
    elseif ~tableOutput
        % ismissing(A) % AisTable
        IA = false(size(A));
        for jj = 1:width(A)
            IA(:,jj) = arraySwitch(A.(jj),true);
        end
    else
        % ismissing(A,'OutputFormat','table')
        IA = A;
        % Remove properties that no longer apply to data
        IA.Properties.VariableUnits = {};
        IA.Properties.VariableContinuity = {};
        for jj = 1:width(A)
            IA.(jj) = arraySwitch(A.(jj),true);
        end
    end
    ...
    if ~istabular(A)
        ...
        if ~stdize && ~tableOutput
            % ismissing(A,indicators)
            IA = false(size(A));
            for jj = dataVars
                IA(:,jj) = arraySwitchIndicators(A.(jj),true,indstruct,stdize,indicators);
            end
        else
            % standardizeMissing(A,indicators)
            % standardizeMissing(A,indicators,'DataVariables',dataVars)
            % ismissing(A,indicators,'OutputFormat','table')
            IA = A;
            for jj = dataVars
                IA.(jj) = arraySwitchIndicators(A.(jj),true,indstruct,stdize,indicators);
            end
...
elseif isequal(class(A), 'string')
    IA = ismissing(A);
elseif AinTable && ~istabular(A)
    % Do not recurse in nested tables or timetables.
    IA = ismissing(A);
else
    % Return false for types that do not define missing
    IA = false(size(A));
end
if AinTable
...
%--------------------------------------------------------------------------
function IA = arraySwitchIndicators(A,AinTable,ind,stdize,indicators)
...
elseif ischar(A)
    if ~AinTable
        if ~stdize
            ...
            if ~isfield(ind,'Char') || ind.IndIsCell
                error(message('MATLAB:ismissing:IndicatorsChar'));
            end
            IA = A; % Flow-through. Cannot standardize char arrays.
        end
    else
        % Convert to string via helper for correct N-D char array behavior.
...
elseif (isinteger(A) || islogical(A))
    ...
    if ~stdize
        ...
        IA = A; % Flow-through. Cannot standardize integer/logical arrays.
    end
    ...
    if AinTable
        ...
        if ~stdize
            if ~(istabular(A) || isstruct(A) || ...
                    iscell(A) || iscalendarduration(A))
                IA = ismissingUnknownWithIndicatorsInTable(A,indicators);
            else
                IA = false(size(A));
            end
        else
        ...
        if iscalendarduration(A)
            ...
            if ~stdize
                error(message('MATLAB:ismissing:FirstInputWithIndicatorsInvalid',class(A)));
            else
...
if isfield(ind,typeid) || ind.hasMissingObj
    ...
    if ~AinTable && ind.IndIsCell && ...
            (~ind.IndIsCellStr || (~iscategorical(A) && ~iscellstr(A))) %#ok<ISCLSTR>
...
elseif stdize || ~AisTable || istabular(ind) || ...
        isstruct(ind) || iscell(ind) || iscategorical(ind)
    % categorical indicators are not supported
    % cellstr indicators within another cell are also not supported
    % Do not error for object indicators for ismissing in a table.
...
%--------------------------------------------------------------------------
function IA = ismissingUnknownWithIndicatorsInTable(A,indicators)
% A is an unknown class, indicators are the raw indicators input
IA = false(size(A));
if iscell(indicators)
    for k = 1:numel(indicators)
        IA = IA | applyIsmissingUnknown(A,indicators{k});
    end
else
    IA = IA | applyIsmissingUnknown(A,indicators);
end
end
%--------------------------------------------------------------------------
function IA = applyIsmissingUnknown(A,ind)
    try
        IA = ismissing(A,ind);
    catch
        IA = false(size(A));
