function M = movprod(A,k,varargin)
% Syntax:
%     M = movprod(A,k)
%     M = movprod(A,[kb kf])
...
if ~isduration(k) && ~istabular(A)
    try
        M = matlab.internal.math.movprod(A,k,varargin{:});
    catch ME
    ...
    M = matlab.internal.math.applyMovFun("movprod", @matlab.internal.math.movprodInternal, hasBiasOption, omitNaNByDefault, A, k, varargin{:});
end