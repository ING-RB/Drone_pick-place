...
if (nargin == 1) && canCallBuiltinHelper(A)
    % One numeric input (no need to parse optional inputs):
    if nargout > 1
        [B,I] = matlab.internal.math.sortrowsHelper(A);
    else
        B = matlab.internal.math.sortrowsHelper(A);
    end
else
    [col, nanflag, compareflag] = matlab.internal.math.sortrowsParseInputs(ismatrix(A),size(A,2),A,varargin{:});
    if ~iscell(A)
        if canCallBuiltinHelper(A)
            if nargout > 1
                [B,I] = matlab.internal.math.sortrowsHelper(A, col, nanflag, compareflag);
            else
                B = matlab.internal.math.sortrowsHelper(A, col, nanflag, compareflag);
            end
        else
...
%--------------------------------------------------------------------------
function tf = canCallBuiltinHelper(A)
...
tf = (isnumeric(A) || ischar(A) || islogical(A)) && ...
    ~isobject(A) && ~issparse(A) && ismatrix(A);
%--------------------------------------------------------------------------
...
if ~isempty(A)
    ...
    for k = n:-1:1
        ck = col(k);
        ack = abs(ck);
        if isnumeric(A{1,ack}) || islogical(A{1,ack})
            if ~all(cellfun(@isscalar,A(:,ack)))
                error(message('MATLAB:sortrows:nonScalarCell'));
            end
            cellclass = class(A{1,ack});
            if ~all(cellfun('isclass',A(:,ack),cellclass))
                error(message('MATLAB:sortrows:MixedDataTypes'));
            end
