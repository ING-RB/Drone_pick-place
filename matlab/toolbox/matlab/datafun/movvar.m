function M = movvar(A,k,varargin)
% Syntax:
%     M = movvar(A,k)
%     M = movvar(A,[kb kf])
%     M = movvar(___,w)
...
if ~isduration(k) && ~istabular(A)
    try
        M = matlab.internal.math.movvar(A,k,varargin{:});
    catch ME
    ...
    M = matlab.internal.math.applyMovFun("movvar", @matlab.internal.math.movvarInternal, hasBiasOption, omitNaNByDefault, A, k, varargin{:});
end