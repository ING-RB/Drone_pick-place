...
%   are as follows:
%
%       'WriteMode'    One of these options: 'overwrite' (the default)
%                      or 'append'. 
%
%       'Compression'  One of these options: 'none' (the default), 'gzip', 
...
p.addParameter('writemode','overwrite', ...
    @(x) validateattributes(x,{'char','string'},{'scalartext','nonempty'},'','WRITEMODE'));

p.addParameter('compression','none', ...
     @(x) validateattributes(x,{'char','string'},{'scalartext','nonempty'},'','COMPRESSION'));
...
if strcmpi(mode,'append')
    ...
    try
        ...
        % In the context of fits filenames, '(', '[', '*', '>' and '<' are
        % special reserved characters and are not supported for regular
        % file creation.
        if contains(filename, {'(','[','*','>','<'})
            error(message('MATLAB:imagesci:fits:fileCreateError'));
        else
            throw(ME);
        end
