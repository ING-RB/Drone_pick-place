...
%   HDFDFR8 is a gateway to the HDF 24-bit raster image interface (DF24). 
%
%   The general syntax for HDFDF24 is
%   HDFDF24(funcstr,param1,param2,...). There is a one-to-one
%   correspondence between DF24 functions in the HDF library and valid
%   values for funcstr.  For example,
...
%   elements along the first dimension vary fastest.  HDFDF24 does not
%   automatically convert from C-style ordering to MATLAB-style ordering,
%   which means that MATLAB image arrays need to be permuted when using
%   HDFDF24 to read or write from HDF files.  The exact permutation depends
%   on the interlace format specified by, for example,
%   HDFDF24('setil',...).  The following calls to PERMUTE will convert HDF
%   arrays to MATLAB arrays, according to the specified interlace format:
%
...
%       Writes a 24-bit raster image to file by overwriting all existing
%       data.
%
%     status = hdfdf24('setcompress',compress_type,...)
%       Sets the compress method for the next raster image written to the
%       file. compress_type can be 'none', 'rle', 'jpeg', or 'imcomp'. If
%       compress_type is 'jpeg', then two additional parameters must be
%       passed in: quality (a scalar between 0 and 100) and force_baseline
%       (either 0 or 1).  Other compression types do not have additional
%       parameters.
%
...
%       Sets the interlace format of the next raster image written to the
%       file; interlace can be 'pixel', 'line', or 'component'.
%
%     ref = hdfdf24('lastref')
%       Reports the last reference number assigned to a 24-bit raster
%       image.
%
%   Read functions
%   ----------------
%   Read functions determine the dimensions and interlace format of an
...
%
%   See also HDF, MATLAB.IO.HDF4.SD, HDFAN, HDFDFR8, HDFH, HDFHD, 
