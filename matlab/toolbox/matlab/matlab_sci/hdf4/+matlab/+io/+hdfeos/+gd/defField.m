...
% Can be a char vector or a string scalar
name = convertStringsToChars(name);

% Must be a cellstr or a string array
if isstring(idimlist) || iscellstr(idimlist)
    idimlist = cellstr(idimlist);
elseif iscell(idimlist)
    matlab.io.internal.imagesci.validateTextInCell(idimlist, 'hdfeos');
end

% Can be a char vector or a string scalar
dtype = convertStringsToChars(dtype);

% Can be char vector or string scalar
mergeCode = convertStringsToChars(mergeCode);
