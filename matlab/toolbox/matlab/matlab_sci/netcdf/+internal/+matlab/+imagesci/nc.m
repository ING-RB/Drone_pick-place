    ...
    methods (Access=protected)
        ...
        function openToAppend(this)
            ...
            if(fid~=-1)
                ...
                error(message('MATLAB:imagesci:netcdf:fileDoesNotExist', this.Filename));
            end
    ...
    methods (Access=protected)
        ...
        function writeVariableSchema(this, schemaStructs, fullGroupName)
            ...
            for infoInd = 1:length(schemaStructs)
                ...
                % Handle string support for the variable schema (schema is a scalar struct).
        ...
        function writeGroupSchema(this, schemaStructs, parentGroup)
            ...
            % Handle string support for the groups schemas.
            % Since the incoming group schemas can contain attribute values
            % which we don't want to convert to character vectors, we selectively
            % convert the group name(s) and format to character vectors. Note format
            % is optional.
            [schemaStructs.Name] = convertStringsToChars(schemaStructs.Name); 
            if isfield(schemaStructs,'Format')
                [schemaStructs.Format] = convertStringsToChars(schemaStructs.Format); 
            end
