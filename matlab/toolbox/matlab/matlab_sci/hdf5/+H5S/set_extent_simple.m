...
if nargin > 3
    if isstring(h5_maxdims) || iscellstr(h5_maxdims)
        h5_maxdims = cellstr(h5_maxdims);
    elseif iscell(h5_maxdims)
        matlab.io.internal.imagesci.validateTextInCell(h5_maxdims, 'hdf5lib');
        
        h5_maxdims = cellfun(@(x) convertStringsToChars(x), h5_maxdims, 'UniformOutput', false);
    end
end
...
matlab.internal.sci.hdf5lib2('H5Sset_extent_simple',...
    space_id, rank, h5_dims, h5_maxdims);            
