...
%
%   name = H5R.get_name(loc_id, ref_type, ref, 'TextEncoding', 'UTF-8')
%   retrieves the name for the object identified by ref while ensuring
%   that the name is handled as UTF-8 encoded text.  This usage is
%   unnecessary if the HDF5 file accurately specifies the use of UTF-8
%   encoding for the resulting link name.
%
%   Example:
%       plist = 'H5P_DEFAULT';
%       space = 'H5S_ALL';
...
if nargin > 1
    ref_type = convertStringsToChars(ref_type);
end

useUtf8 = matlab.io.internal.imagesci.h5ParseEncoding(varargin);
name = matlab.internal.sci.hdf5lib2('H5Rget_name',...
