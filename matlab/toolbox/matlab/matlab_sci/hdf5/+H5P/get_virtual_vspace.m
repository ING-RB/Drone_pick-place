function virtual_dspace_id = get_virtual_vspace(dcpl_id,mapping_index)
%H5P.get_virtual_vspace  Get dataspace identifier virtual dataset selection. 
%   virtual_dspace_id = get_virtual_vspace(dcpl_id, mapping_index) takes
%   the dataset creation property list for the virtual dataset, dcpl_id,
%   and the mapping index, index, and returns a dataspace identifier for
%   the selection within the virtual dataset used in the mapping. The
%   dataspace identifier must be closed with H5S.close().
%
%   dcpl_id = Virtual dataset creation property list identifier
%
%   index = Mapping index The value of index is 0 (zero) or greater and
%   less than count   (0 ≤ index < count), where count is the number of
%   mappings returned by H5P.get_virtual_count.
%
%   virtual_dspace_id = Dataspace identifier of the virtual selection
%
%   Example:
...
%         % Get virtual dataspace identifer
%         vds_spaceID = H5P.get_virtual_vspace(vdcplID,0);
%         % Read the data
%         H5D.read(vdsDatasetID,'H5ML_DEFAULT','H5S_ALL','H5S_ALL','H5P_DEFAULT')
%         H5D.close(vdsDatasetID);
%         H5F.close(vdsFileID)
%             
%   See also H5P.get_virtual_count, H5P.get_virtual_srcspace, H5P.set_virtual
...
virtual_dspace_id = matlab.internal.sci.hdf5lib2('H5Pget_virtual_vspace',...
    dcpl_id,mapping_index);
virtual_dspace_id = H5ML.id(virtual_dspace_id,'H5Sclose');
