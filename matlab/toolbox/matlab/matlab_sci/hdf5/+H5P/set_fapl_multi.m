...
if nargin == 2
   matlab.internal.sci.hdf5lib2('H5Pset_fapl_multi', id, varargin{end});
   return
end
...
validateattributes(memb_fapl,{'double', 'int64', 'H5ML.id'},{},'','MEMB_FAPL');

% If not the default case, have to unwrap each of the IDs.
memb_fap = zeros(1,numel(memb_fapl), 'int64');
for i = 1 : length(memb_fapl)
   memb_fap(i) = H5ML.unwrap_ids(memb_fapl(i));
end
matlab.internal.sci.hdf5lib2('H5Pset_fapl_multi',...
    id, memb_map, memb_fap, memb_name, memb_addr, relax);            
