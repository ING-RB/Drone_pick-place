...
%   H5D_CONTIGUOUS, H5D_CHUNKED or H5D_VIRTUAL.
%
%   Example:  
...
validateattributes(dcpl,{'H5ML.id'},{'nonempty','scalar'});
if nargin > 1
    layout = convertStringsToChars(layout);
end
validateattributes(layout,{'numeric','char'},{'nonempty'});
if isnumeric(layout)
    validateattributes(layout,{'numeric'},{'scalar','finite','nonnan'});
elseif ischar(layout)
    validateattributes(layout,{'char'},{'scalartext'});
end

matlab.internal.sci.hdf5lib2('H5Pset_layout', dcpl, layout);            
