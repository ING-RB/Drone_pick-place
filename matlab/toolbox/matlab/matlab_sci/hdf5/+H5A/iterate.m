...
switch ( nargin )
	case 3
        ...
        status = matlab.internal.sci.hdf5lib2('H5Aiterate', varargin{:});
        if nargout > 0
            varargout{1} = status;
        end
	case 6
        maxInputs = 6;
         % Do not convert the user supplied input "cdata_in" to char as the
         % user may use char and strings differently 
        [varargin{1:maxInputs-1}] = convertStringsToChars(varargin{1:maxInputs-1});
		...
		[status,n,cdata_out] = matlab.internal.sci.hdf5lib2('H5Aiterate', varargin{:});
