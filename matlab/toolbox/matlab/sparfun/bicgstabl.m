...
if ~isfloat(b)
    error(message('MATLAB:bicgstabl:RHSInvalidClass'))
end
...
% Helper arrays should be dense, real, and same underlying type as b
...
if (n2b == 0)                      % if    rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg('bicgstabl',0,maxit,0,flag,iter,NaN);
    end
    return
end
...
ukm1 = zeros(n,1,"like",proto);
rk = b - iterapp('mtimes',afun,atype,x0,varargin{:});
xk = zeros(n,1,"like",proto);
r0 = rk;
rho0 = 1;
omega = 1;
ut = zeros(n,ell+1,"like",proto);
rt = zeros(n,ell+1,"like",proto);
alpha = 0;
Tau = zeros(ell+1,ell+1,"like",proto);
sigma = zeros(ell+1,1,"like",proto);
gamma = zeros(ell+1,1,"like",proto);
gammap = zeros(ell+1,1,"like",proto);
gammapp = zeros(ell+1,1,"like",proto);
stag = 0;                          % stagnation of the method

moresteps = 0;
maxmsteps = min([floor(n/50),4*ell,n-maxit]);
maxstagsteps = 3;
resvec = zeros(2*ell*maxit+1,1,"like",proto);
resvec(1) = norm(rk);
normrmin = resvec(1);
...
for kk = 1 : maxit
    ...
    for jj = 1 : ell
        ...
        if existM1
            put = iterapp('mldivide',m1fun,m1type,ut(:,jj),varargin{:});
            if ~allfinite(put)
                flag = 2;
                resvec = resvec(1:2*ell*(kk-1)+jj);
                break
        ...
        if existM2
            put = iterapp('mldivide',m2fun,m2type,put,varargin{:});
            if ~allfinite(put)
                flag = 2;
                resvec = resvec(1:2*ell*(kk-1)+jj);
                break
        ...
        ut(:,jj+1) = iterapp('mtimes',afun,atype,put,varargin{:});
        ...
        if existM1
            prt = iterapp('mldivide',m1fun,m1type,rt(:,jj),varargin{:});
            if ~allfinite(prt)
                flag = 2;
                resvec = resvec(1:2*ell*(kk-1)+jj);
                break
        ...
        if existM2
            prt = iterapp('mldivide',m2fun,m2type,prt,varargin{:});
            if ~allfinite(prt)
                flag = 2;
                resvec = resvec(1:2*ell*(kk-1)+jj);
                break
        ...
        rt(:,jj+1) = iterapp('mtimes',afun,atype,prt,varargin{:});
        
        normr = norm(rt(:,1));
        normr_act = normr;
        resvec((kk-1)*ell*2+jj+1) = normr_act;
        
        if abs(alpha)*norm(ut(:,1)) < epsT*norm(xt)
            stag = stag + 1;
        else
            stag = 0;
        end
        ...
        if (normr <= tolb || stag >= maxstagsteps || moresteps)
            ...
            if existM1
                pxt = iterapp('mldivide',m1fun,m1type,xt,varargin{:});
                if ~allfinite(pxt)
                    flag = 2;
            ...
            if existM2
                pxt = iterapp('mldivide',m2fun,m2type,pxt,varargin{:});
                if ~allfinite(pxt)
                    flag = 2;
                    resvec = resvec(1:2*ell*(kk-1)+jj+1);
                    break
            ...
            rt(:,1) = b - iterapp('mtimes',afun,atype,x0+pxt,varargin{:});
            normr_act = norm(rt(:,1));
    ...
    if abs(gamma(2))*norm(rt(:,1)) < epsT*norm(xt)
        stag = stag + 1;
    else
        stag = 0;
    end
    ...
    for jj = 2 : ell
        ...
        if abs(gammapp(jj))*norm(rt(:,jj)) < epsT*norm(xt)
            stag = stag + 1;
        else
            stag = 0;
        end
...
if isempty(kk)
    kk = 0;
    xt = xmin; % don't have xt from last iteration, set equal xmin
end
...
if flag == 0
    ...
    if existM1
        pxt = iterapp('mldivide',m1fun,m1type,xmin,varargin{:});
        pxt2 = iterapp('mldivide',m1fun,m1type,xt,varargin{:});
        if ~allfinite(pxt) || ~allfinite(pxt2)
            flag = 2;
            x = xmin;
            iter = imin;
            relres = normr_act/n2b;
    ...
    if existM2
        pxt = iterapp('mldivide',m2fun,m2type,pxt,varargin{:});
        pxt2 = iterapp('mldivide',m2fun,m2type,pxt2,varargin{:});
        if ~allfinite(pxt) || ~allfinite(pxt2)
            flag = 2;
            x = xmin;
            iter = imin;
    ...
    r_comp = b - iterapp('mtimes',afun,atype,x0+pxt,varargin{:});
    r_comp2 = b - iterapp('mtimes',afun,atype,x0+pxt2,varargin{:});
