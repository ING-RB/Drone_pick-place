...
[A, n, B, k, Amatrix, eigsSigma, shiftAndInvert, cholB, permB, scaleB, spdB,...
    innerOpts, useEig, originalB, proto] = checkInputs(varargin{:});
...
if useEig
    if nargout <= 1
        V = fullEig(A, B, n, k, cholB, permB, scaleB, eigsSigma, proto);
    else
        [V, D] = fullEig(A, B, n, k, cholB, permB, scaleB, eigsSigma, proto);
...
if spdB && ~spdBout && ~any(isNotConverged)
    ...
    [V, D] = fullEig(Asmall, Bsmall, size(VV, 2), k, false, [], 1, eigsSigma, proto);
    V = VV*V;
    d = diag(D);
end
...
if isfloat(varargin{1})
    ...
    if ~isempty(notFunc)
        if support
            error(message('MATLAB:eigs:NonFloatOrFunction'));
        else
            error(message('MATLAB:eigs:NonDoubleOrFunction'));
        end
    end
...
if nargin < 3-Amatrix
    ...
    Bpresent = isBpresent(varargin(3-Amatrix:end), n, support);
    if Bpresent
...
tol = []; % placeholder to ensure tol is in the workspace for later use
maxit = 300;
p =  min(max(2*k,20),n);
...
if nargin >= optsStart
    ...
    if isfield(opts,'v0')
        [v0,useSingle] = checkV0(opts.v0, n, NameValueFlag,useSingle,support);
...
% A is specified by a function. Check output of Afun and verify return type
% matches other user supplied inputs
if ~Amatrix
    if isempty(v0)
        vec = A(eye(n,1));
    else
        vec = A(eye(n,1,"like",real(full(zeros("like",v0)))));
    end
    if ~iscolumn(vec) || ~isfloat(vec) || length(vec) ~= n || ...
            (~support && ~isUnderlyingType(vec,'double'))
        % Will only error if the user provides a function handle that does
        % not output a vector of the expected size and type
        if ~support
            error(message('MATLAB:eigs:InvalidFhandleOutput',n))
        else
            error(message('MATLAB:eigs:InvalidFhandleOutputFloat',n))
        end
    end
    AfunReturnsSingle = isUnderlyingType(vec,'single');
    if isempty(B) && isempty(v0)
        % No other type dependent inputs provided, use function handle's
        % return type as operational type
        useSingle = useSingle || AfunReturnsSingle;
    elseif useSingle ~= AfunReturnsSingle
        % Superiorfloat of type dependent inputs doesn't match return type
        % of function handle - error
        if useSingle
            error(message('MATLAB:eigs:FhandleShouldReturnSingleToMatchBAndV0'))
        else
            error(message('MATLAB:eigs:FhandleShouldReturnDoubleToMatchBAndV0'))
        end
    end
end
...
% Cast all data matrices to single if useSingle
if useSingle
    if Amatrix
        A = single(A);
    end
    ...
    v0 = single(v0);
    originalB = single(originalB);
    scaleB = single(scaleB);
end

% Helper arrays should be dense, real, and same underlying type as v0
proto = real(full(zeros("like",v0)));
...
function Bpresent = isBpresent(args, n, support)
B = args{1};
...
if ~isscalar(B)
    % B is not a scalar.
    checkClassOfB(B, support);
    if ~isequal(size(B), [n,n])
...
function checkClassOfB(B, support)
if ~isUnderlyingType(B,'double') && ~(support && isUnderlyingType(B,'single'))
    if ~support 
        error(message('MATLAB:eigs:BnonDouble'));
    else
        error(message('MATLAB:eigs:BnonFloat'));
    end
end
end
...
function [v0, useSingle] = checkV0(v0, n, NameValueFlag, useSingle, support)
if ~iscolumn(v0) || length(v0) ~= n || ~isfloat(v0) || ...
        (~support && ~isUnderlyingType(v0,'double'))
    if ~support
        if NameValueFlag
        ...
        if NameValueFlag
            error(message('MATLAB:eigs:WrongSizeStartVectorFloat'));
        else
            error(message('MATLAB:eigs:WrongSizeOptsV0Float'));
        end
    end
end
useSingle = useSingle || isUnderlyingType(v0,'single');
v0 = full(v0);
end
...
function [V, D] = fullEig (A, B, n, k, cholB, permB, scaleB, eigsSigma, proto)
% Use EIG(FULL(A),FULL(B)) instead of Krylov Schur algorithm
if k == 0
    if nargout <= 1
        V = zeros(0,1,"like",proto);
        return;
    else
        V = zeros(n,0,"like",proto);
        D = zeros(0,0,"like",proto);
        return;
    end
end
...
if isnumeric(A)
    ...
    if ischar(eigsSigma) && ~strcmp(eigsSigma,'smallestabs')
        ...
        if (isfloat(eigsSigma) && eigsSigma == 0) || strcmp(eigsSigma,'smallestabs')
            ...
            invA = eye(n,"like",proto);
            for i = 1:n
                invA(:,i) = A(invA(:,i));
            end
            A = eye(n,"like",proto) / invA;
        else
            ...
            % => A = sigma*B*(U + eye(n)) / U
            if isempty(B)
                sB = eigsSigma*eye(n,"like",proto);
            else
                sB = eigsSigma*B;
            end
            U = zeros(n,n,"like",proto);
            for i = 1:n
                U(:,i) = A(sB(:,i));
            end
            A = sB*(U+eye(n,"like",proto)) / U;
        end
    end
end
...
function [applyOP, applyM] = getOps(A, B, n, spdB, shiftAndInvert, R, cholB, ...
    permB, Amatrix, innerOpts, eigsSigma, proto)
...
if isempty(B) % Standard problem
    if ~shiftAndInvert % OP = A
        ...
        applyOP = AminusSigmaISolve(A, innerOpts.sigma, Amatrix, n, applyA, eigsSigma, proto);
    end
    ...
    if ~shiftAndInvert
        ...
        applyOP = AminusSigmaBSolve(A, B, innerOpts.sigma, Amatrix, n, ...
            applyA, applyB, cholB, eigsSigma, proto);
...
function applyOP = AminusSigmaISolve(A, sigma, Amatrix, n, applyA, eigsSigma, proto)
if Amatrix
    % Build A - sigma I
    if sigma == 0
        AminusSigmaI = A;
    else
        AminusSigmaI = A - sigma * eye(n,"like",sparse(proto));
    end
...
function applyOP = AminusSigmaBSolve(A, B, sigma, Amatrix, n, applyA, ...
    applyB, cholB, eigsSigma, proto)
if Amatrix
    if sigma == 0
        AminusSigmaB = A;
    elseif cholB
        AminusSigmaB = A - sigma * applyB(eye(n,"like",sparse(proto)));
    else
...
if ~isempty(v)
    ...
    % if first one fails)
    [v, stopAlgorithm] = randomRestart(applyM, randStr, n, proto);
    if stopAlgorithm
...
if innerOpts.ishermprob
    [V, d, isNotConverged, stopAlgorithm] = KSherm(applyOP, applyM, n, k, ...
        innerOpts, randStr, shiftAndInvert, proto);
...
V = zeros(n,p,"like",proto);
v = innerOpts.v0;
epsT = eps("like",proto); % Chose tolerance depending on the working precision
stopAlgorithm = false;
k0 = k; % k0 is original k. (variable k will be adaptively increased)
emptyLikeProto = zeros(0,0,"like",proto);
Alpha = emptyLikeProto;
Beta = emptyLikeProto;
d = emptyLikeProto;
c = emptyLikeProto;
normRes = emptyLikeProto;
justRestarted = false;
sizeV = 1;
...
for mm=1:maxit
    ...
    for jj = sizeV : p
        ...
        [r, normRes, stopAlgorithm] = robustReorthogonalize(V, r, applyM, jj, randStr, proto);
    ...
    isNotConverged = ~(res(1:k0)' < tol*max(epsT^(2/3), abs(d(1:k0))));
    nconv = nnz(~isNotConverged);
    
    if innerOpts.disp
        minrelres = min(res(isNotConverged)' ./ max(epsT^(2/3), ...
...
for mm = 1 : maxit
    ...
    for jj = sizeV: p
        ...
        % Reorthogonalize
        [r, normRes, stopAlgorithm, w] = robustReorthogonalize(V, r, applyM, jj, randStr, proto, w);
        ...
        % Save data
        v = r;
        H = [H w; zeros(1,jj-1,"like",proto) normRes]; %#ok<AGROW>
    end
...
function [r, normRes, stopAlgorithm, w] = robustReorthogonalize(V, r, ...
    applyM, index, randStr, proto, wIn)

Mr = applyM(r);
normResOld = sqrt(abs(r'*Mr));

if nargin < 7
    wIn = zeros(index,1,"like",r);
end
w = wIn;
stopAlgorithm = false;
...
w = w + dw;
Mr = applyM(r);
normRes = normHelper(r, Mr, proto, applyM);
...
if normRes <= (1/sqrt(2))*normResOld
    ...
    normRes = 0;
    w = wIn;
    [r, stopAlgorithm] = randomRestart(applyM, randStr, V, proto, index);
else
    r = r / normRes;
end
end

% Helper to calculate a scaled norm without using scaleR in
% robustReorthogonalize
function normRes = normHelper(r, Mr, proto, applyM)
normRes = sqrt(abs(r'*Mr));
if normRes < sqrt(realmin("like",proto)) * length(r)
    scaleR = 2^(-nextpow2(normRes));
    r = r * scaleR;
    Mr = applyM(r);
    normRes = sqrt(abs(r'*Mr)) / scaleR;
end
end

function [r, stopAlgorithm] = randomRestart(applyM, randStr, V, proto, index)
% Two syntaxes: randomRestart(applyM, randStr, n, proto) and randomRestart(applyM, randStr, V, proto, index)
stopAlgorithm = true;
tol = 1e-10;
if isUnderlyingType(proto,"single")
    tol = 1e-5;
end
% orthogonalize against V(:, 1:index) only if they are provided
needOrthogonalize = nargin == 5;
...
for restart=1:3
    ...
    r = randn(randStr,n,1,"like",proto);
