...
[A,m,n,k,sigma,u0,v0,InnerOpts,Options,proto,useSingle] = checkInputs(A,VarIn,randStr);
...
if k == 0
    if nargout <= 1
        U = zeros(0,1,"like",proto);
    else
        U = eye(m,0,"like",proto);
        S = zeros(0,0,"like",proto);
        V = eye(n,0,"like",proto);
        if InnerOpts.fail == "keep"
            flag = false(k,1);
        else
            flag = 0;
        end
...
elseif sigma == "smallest" || sigma == "smallestnz"
    ...
    if ComputeNonZeroSV
        ...
        if InnerOpts.p < sizeR
            ...
            resTol = 1e8;
            if useSingle
                resTol = 1e4; % approximately 1/eps('single')
            end
            checkResiduals = S(1, 1) / S(end, end) > resTol;
        ...
        if checkResiduals
            ...
            res1 = A*V - U*S;
            res2 = A'*U - V*S;
            maxres = max(vecnorm(res1, 2, 1), vecnorm(res2, 2, 1))';
            ...
            if ~all(maxres < fac*InnerOpts.tol*diag(S))
        ...
        U = zeros(mIn,0,"like",proto);
        V = zeros(nIn,0,"like",proto);
        S = zeros(0,0,"like",proto);
    ...
    if nargout <= 1
        ...
        if zeroSing > 0
            ...
            if issparse(A)
                ...
                Uzero = full(spdiags(ones(zeroSing, 1, "like",proto), -sizeR, mIn, zeroSing));
                Uzero = Q1func(Uzero, 'notransp');
                
                % Find vectors in the nullspace of A'. These are right singular
                % vectors corresponding to the zero singular values
                Vzero = full(spdiags(ones(zeroSing, 1, "like",proto), -sizeR, nIn, zeroSing));
...
function [A,m,n,k,sigma,u0,v0,InnerOpts,Options,proto,useSingle] = checkInputs(A,VarIn,randStr)
...
% Get A and MN
[A,m,n,VarIn,useSingle] = checkA(A,VarIn,support);
...
% Get sigma
sigma = getSigma(A,VarIn,support);

% Set defaults for options or get options
[InnerOpts,Options,u0,v0,useSingle] = getOptions(VarIn,m,n,k,sigma,useSingle,support);

if isnumeric(sigma) && sigma == 0
    warning(message('MATLAB:svds:SigmaZero'))
end

% Check function handle, unless k is zero
if ~isfloat(A) && k~=0
    useSingle = checkAfunReturnType(A, m, n, u0, v0, useSingle, support);
end

if useSingle
    if isUnderlyingType(A,'double')
        A = single(A);
    end
    u0 = single(u0);
    v0 = single(v0);
    if isfield(Options, 'v0')
        Options.v0 = single(Options.v0);
    end
    if isfield(Options, 'u0')
        Options.u0 = single(Options.u0);
    end
end
...
if isempty(u0) && isempty(v0)
    if n > m
        u0 = randn(randStr,m,1,"like",proto);
    else
        v0 = randn(randStr,n,1,"like",proto);
    end
end

end

function [A,m,n,VarIn,useSingle] = checkA(A,VarIn,support)
...
if isa(A, 'function_handle')
    ...
    % Error Check m and n
    if ~isPosInt(MN) || ~isrow(MN) || length(MN) ~= 2 || ~allfinite(MN)
        error(message('MATLAB:svds:InvalidMN'));
    else
        m = double(full(MN(1)));
        n = double(full(MN(2)));
...
elseif ismatrix(A) && isfloat(A)
    ...
    if support
        error(message('MATLAB:svds:InvalidAFloat'))
    else
        error(message('MATLAB:svds:InvalidA'))
    end
end
...
function sigma = getSigma(A,VarIn,support)
...
if length(VarIn) < 2
    ...
    if (ischar(sigma) && isrow(sigma)) || (isstring(sigma) && isscalar(sigma))
        ...
        if isempty(j) || (strlength(sigma) == 0)
            errorForInvalidSigma(support);
        else
            % Reset sigma to the correct valid sigma for cheaper checking
            sigma = ValidSigmas{j};
        end
    elseif isfloat(sigma) && (support || isUnderlyingType(sigma,'double'))
...
function errorForInvalidSigma(support)
if support
    error(message('MATLAB:svds:InvalidSigmaFloat'))
else
    error(message('MATLAB:svds:InvalidSigma'))
end
end

function [InnerOpts,Options,u0,v0,useSingle] = getOptions(VarIn,m,n,k,sigma,useSingle,support)
...
if nVarIn >= 3
    ...
    if isfield(Options, 'v0')
        ...
        if ~iscolumn(v0) || length(v0) ~= n || ~isfloat(v0) || ...
            ...
            if ~support
                ...
                if NameValueFlag
                    error(message('MATLAB:svds:InvalidV0NameValueFloat'))
                else
                    error(message('MATLAB:svds:NonFloatV0'))
                end
            end
        end
    ...
    if isfield(Options, 'u0')
        ...
        elseif ~iscolumn(u0) || length(u0) ~= m || ~isfloat(u0) || ...
                (~support && ~isUnderlyingType(u0,"double"))
            if ~support
                if NameValueFlag
                ...
                if NameValueFlag
                    error(message('MATLAB:svds:InvalidU0NameValueFloat'))
                else
                    error(message('MATLAB:svds:NonFloatU0'))
                end
            end
        end
        useSingle = useSingle || isUnderlyingType(u0, 'single');
    end
...
function Amat = getMatrix(Afun, m, n, proto)
...
I = eye(nIn, "like", proto);

% Initialize Amat in the correct class
Amat = zeros(mIn, nIn, "like", proto);
...
if ~issparse(Amat) || m == n
    % Direct call to svd
    [U, S, V] = svd(full(Amat), 'econ', 'vector');
else
    % Make Amat tall
    if m < n
        Amat = Amat';
...
function useSingle = checkAfunReturnType(Afun, m, n, u0, v0, useSingle, support)
% For wide matrices, we pass A' to Lanczos and call Afun with 'transp'
% first
if m >= n
    mIn = m;
    nIn = n;
...
if isempty(u0) && isempty(v0)
    vec = Afun(eye(nIn,1), transflag);
elseif isempty(u0)
    vec = Afun(v0, transflag);
else
    vec = Afun(u0, transflag);
end

if ~iscolumn(vec) || ~isfloat(vec) || length(vec) ~= mIn || ...
        (~support && ~isUnderlyingType(vec,'double'))
    % Will only error if the user provides a function handle that does not
    % output a vector of the expected size or datatype
    if ~support
        error(message('MATLAB:svds:InvalidFhandleOutput', transflag, mIn));
    else
        error(message('MATLAB:svds:InvalidFhandleOutputFloat', transflag, mIn));
    end
end
...
elseif useSingle ~= AfunReturnsSingle
    ...
    if useSingle
        startingVectorType = 'single';
    else
        startingVectorType = 'double';
    end
    error(message('MATLAB:svds:FhandleShouldReturnSameTypeAsU0OrV0', startingVectorType))
end
end
