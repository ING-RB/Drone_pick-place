...
if ~iscolumn(u) || ~isfloat(u) || length(u) ~= m || ...
        (~support && ~isUnderlyingType(u,'double'))
    if ~support
        error(message('MATLAB:svds:InvalidFhandleOutput', f1, m));
    else
        error(message('MATLAB:svds:InvalidFhandleOutputFloat', f1, m));
    end
end
...
[U,S,V,nconv,nrIter,isNotConverged] = LanczosBDInner(Afun, U, B, V, k, ...
    m, n, f1, f2, InnerOpts, nrIter, randStr, support);
...
for nrRestartAfterConverge=1:InnerOpts.maxit
    ...
    v = randn(randStr, size(V, 1), 1, "like", v);
    v = orthAgainstRobust(V, v, m, n, Anorm, randStr, k);
    
    % Find next u
    Av = full(Afun(v,f1));
    Uk = matlab.internal.math.viewColumns(U, k);
    u = Av - Uk * (Uk' * Av);
    Uk = []; %#ok<NASGU> % release shared copy of U
...
while nrIter < InnerOpts.maxit
    ...
    for j = size(B, 1):InnerOpts.p
        ...
        if nrIter == 1 && j == 1
            ...
            vtemp = Afun(u,f2);
            if ~iscolumn(vtemp) || ~isfloat(vtemp) || length(vtemp) ~= n || ...
                    (~support && ~isUnderlyingType(vtemp,'double'))
                if ~support
                    error(message('MATLAB:svds:InvalidFhandleOutput', f2, n));
                else
                    error(message('MATLAB:svds:InvalidFhandleOutputFloat', f2, n));
                end
            end
    ...
    if ~allfinite(B) || ~allfinite(U) || ~allfinite(V)
        error(message('MATLAB:svds:BadCondition'))
    end
    
    % Find svd of B 
    [Uin,Sin,Vin] = svd(B,'vector');
    
    % Calculate k Ritz values/vectors
    U(:, 1:kplus) = U*matlab.internal.math.viewColumns(Uin, kplus);
    V(:, 1:kplus) = V*matlab.internal.math.viewColumns(Vin, kplus);
    S = diag(Sin(1:kplus));
    ...
    % Find next u
    Av = Afun(v,f1);
    Uk = matlab.internal.math.viewColumns(U, kplus);
    u = Av - Uk * (Uk' * Av);
    Uk = []; %#ok<NASGU> % release shared copy of U
    ...
    % Initialize B,U, and V for Lanczos restart
    B = [S, bounds; zeros(1,kplus,"like",S), normu];
    U(:, kplus + 1) = u;
    V(:, kplus + 1) = v;
...
% Reorthogonalize
Xi = matlab.internal.math.viewColumns(X, i);
y = y - Xi * (Xi' * y);

% Find norm(y)
normy = norm(y);

if normy <= max(m,n)*Bnorm*eps("like",normy)
    % Attempt to find another Krylov subspace
    normy = zeros(1,"like",normy);
    y = randn(randStr,size(y,1),1,"like",y);
