...
while ido ~= 99
    ...
    cols = checkIpntr(ido,ipntr,ndouble);
    ...
    if innerOpts.eigsDisplay
        eigsIter = displayRitzValues(ipntr,ido,eigsIter,p, ...
            isrealprob,ishermprob,workl,whch,k);
    end
...
flag = processEUPDinfo(nOutputs<3,info,eupdfun,iparam,k);

end

% Local functions in CallARPACK
function cols = checkIpntr(ido,ipntr,ndouble)
...
function eigsIter = displayRitzValues(ipntr,ido,eigsIter,p, ...
    isrealprob,ishermprob,workl,whch,k)
...
function flag = processEUPDinfo(warnNonConvergence,info,eupdfun,iparam,k)
