...
if ~isfloat(b)
    error(message('MATLAB:bicgstab:RHSInvalidClass'))
end
...
if (n2b == 0)                      % if    rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg('bicgstab',0,maxit,0,flag,iter,NaN);
    end
    return
end
...
resvec = zeros(2*maxit+1,1,"like",proto); % Preallocate vector for norm of residuals
...
for ii = 1 : maxit
    ...
    if existM1
        ph = iterapp('mldivide',m1fun,m1type,p,varargin{:});
        if ~allfinite(ph)
            flag = 2;
            resvec = resvec(1:2*ii-1);
            break
    ...
    if existM2
        ph = iterapp('mldivide',m2fun,m2type,ph,varargin{:});
        if ~allfinite(ph)
            flag = 2;
            resvec = resvec(1:2*ii-1);
            break
    ...
    v = iterapp('mtimes',afun,atype,ph,varargin{:});
    rtv = rt' * v;
    if (rtv == 0) || isinf(rtv)
        flag = 4;
        resvec = resvec(1:2*ii-1);
    ...
    if (normr <= tolb || stag >= maxstagsteps || moresteps)
        s = b - iterapp('mtimes',afun,atype,xhalf,varargin{:});
        normr_act = norm(s);
    ...
    if existM1
        sh = iterapp('mldivide',m1fun,m1type,s,varargin{:});
        if ~allfinite(sh)
            flag = 2;
            resvec = resvec(1:2*ii);
            break
    ...
    if existM2
        sh = iterapp('mldivide',m2fun,m2type,sh,varargin{:});
        if ~allfinite(sh)
            flag = 2;
            resvec = resvec(1:2*ii);
            break
    ...
    t = iterapp('mtimes',afun,atype,sh,varargin{:});
    tt = t' * t;
    if (tt == 0) || isinf(tt)
        flag = 4;
        resvec = resvec(1:2*ii);
