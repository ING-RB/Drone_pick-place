...
if ~isfloat(b)
    error(message('MATLAB:symmlq:RHSInvalidClass'))
end
...
if (n2b == 0)                      % if rhs vector is all zeros
    ...
    if nargout >= 6
        resveccg = zeros("like",proto); % resveccg(1) = norm(b-A*xcg) = norm(0)
    end
    if (nargout < 2)
        itermsg('symmlq',0,maxit,0,flag,iter,NaN);
    end
    return
end
...
resvec = zeros(maxit+1,1,"like",proto); % Preallocate vector for SYMMLQ residuals
...
for ii = 1 : nits
    ...
    if abs(zeta)*norm(w) < epsT*norm(x)
    ...
    if (normrcg <= tolb || stag >= maxstagsteps || moresteps)
        ...
        r = b - iterapp('mtimes',afun,atype,xcg,varargin{:});
        normrcg_act = norm(r);
        if (normrcg_act <= tolb)
            x = xcg;
            flag = 0;
