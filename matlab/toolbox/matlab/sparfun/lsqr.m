...
if ~isfloat(b)
    error(message('MATLAB:lsqr:RHSInvalidClass'))
end
...
u = b;
if xInit
    u = u - iterapp('mtimes',afun,atype,x,varargin{:},'notransp');
end
beta = norm(u);
...
s = 0;
phibar = beta;
v = iterapp('mtimes',afun,atype,u,varargin{:},'transp');
if ~nKnown
...
if existM2
    v = iterapp('mldivide',m2fun,m2type,v,varargin{:},'transp');
    if ~allfinite(v)
        flag = 2;
        relres = normr/n2b;
        iter = 0;
        resvec = normr;
        lsvec = zeros(0,1,"like",proto);
        if nargout < 2
...
if existM1
    v = iterapp('mldivide',m1fun,m1type,v,varargin{:},'transp');
    if ~allfinite(v)
        flag = 2;
        relres = normr/n2b;
        iter = 0;
...
if (normar == 0)               % if alpha_1 == 0 | beta_1 == 0
    ...
    lsvec = zeros(0,1,"like",proto);        % no estimate for norm(A*inv(M),'fro') yet
    if (nargout < 2)
        itermsg('lsqr',tol,maxit,0,flag,iter,relres);
    end
    return
...
sumnormd2 = 0;
resvec = zeros(maxit+1,1,"like",proto); % Preallocate vector for norm of residuals
resvec(1) = normr;                      % resvec(1,1) = norm(b-A*x0)
lsvec = zeros(maxit,1,"like",proto);    % Preallocate vector for least squares estimates
...
for ii = 1 : maxit
    if existM1
        z = iterapp('mldivide',m1fun,m1type,v,varargin{:},'notransp');
        if ~allfinite(z)
            flag = 2;
            iter = ii-1;
    ...
    if existM2
        z = iterapp('mldivide',m2fun,m2type,z,varargin{:},'notransp');
        if ~allfinite(z)
            flag = 2;
            iter = ii-1;
            resvec = resvec(1:iter+1);
    ...
    u = iterapp('mtimes',afun,atype,z,varargin{:},'notransp') - alpha * u;
    beta = norm(u);
    u = u / beta;
    ...
    if ~isfinite(phi) || rho == 0 || ~isfinite(rho)
        % phi directly flows into x below, and computation of d involves
        % division by rho: These are scalars we can check to verify if x
    ...
    normr = abs(s) * normr;
    resvec(ii+1) = normr;
    vt = iterapp('mtimes',afun,atype,u,varargin{:},'transp');
    if existM2
        vt = iterapp('mldivide',m2fun,m2type,vt,varargin{:},'transp');
        if ~allfinite(vt)
            flag = 2;
            iter = ii;
            resvec = resvec(1:iter+1);
    ...
    if existM1
        vt = iterapp('mldivide',m1fun,m1type,vt,varargin{:},'transp');
        if ~allfinite(vt)
            flag = 2;
            iter = ii;
            resvec = resvec(1:iter+1);
    ...
    v = vt - beta * v;
    alpha = norm(v);
    if (alpha == 0 || ~isfinite(alpha)) && ii < maxit
        flag = 4;
        iter = ii;
        resvec = resvec(1:iter+1);
        lsvec = lsvec(1:iter);
