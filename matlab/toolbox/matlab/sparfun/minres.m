...
if ~isfloat(b)
    error(message('MATLAB:minres:RHSInvalidClass'))
end
...
n2b = norm(b);                     % Norm of rhs vector, b if
if (n2b == 0)                      % rhs vector is all zeros
    ...
    if nargout >= 6	               % resveccg(1) = norm(b-A*xcg) = norm(0)
        resveccg = zeros("like",proto);
    end
    if (nargout < 2)
        itermsg('minres',0,maxit,0,flag,iter,NaN);
    end
    return
end
...
resvec = zeros(maxit+1,1,"like",proto);     % Preallocate vector for MINRES residuals
resvec(1) = normr;                          % resvec(1) = norm(b-A*x0)
if nargout >= 6
    resveccg = zeros(maxit+2,1,"like",proto); % Preallocate vector for CG residuals
...
vold = r;
if existM1
    u = iterapp('mldivide',m1fun,m1type,vold,varargin{:});
    if ~allfinite(u)
        flag = 2;
        relres = normr / n2b;
        resvec = resvec(1);
...
if existM2
    v = iterapp('mldivide',m2fun,m2type,u,varargin{:});
    if ~allfinite(v)
        flag = 2;
        relres = normr / n2b;
        resvec = resvec(1);
...
snprod = beta1;
vv = v / beta1;
v = iterapp('mtimes',afun,atype,vv,varargin{:});
Amvv = v;
alpha = vv' * v;
