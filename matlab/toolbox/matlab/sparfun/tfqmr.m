...
%   is not applied. M may be a function handle MFUN such that MFUN(X)
%   returns M\X.
%
...
if ~isfloat(b)
    error(message('MATLAB:tfqmr:RHSInvalidClass'))
end
...
if (n2b == 0)                      % if rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg('tfqmr',0,maxit,0,flag,iter,NaN);
    end
    return
end
...
%v = A*M1(u_m);
if existM1
    pu_m = iterapp('mldivide',m1fun,m1type,u_m,varargin{:});
    if ~allfinite(pu_m)
        flag = 2;
        relres = normr/n2b;
        iter = 0;
...
if existM2
    pu_m = iterapp('mldivide',m2fun,m2type,pu_m,varargin{:});
    if ~allfinite(pu_m)
        flag = 2;
        relres = normr/n2b;
        iter = 0;
...
v = iterapp('mtimes',afun,atype,pu_m,varargin{:});
...
resvec = zeros(maxit+1,1,"like",proto);
resvec(1) = norm(r);
normrmin = resvec(1);
...
for mm = 1 : maxit*2
    ...
    if existM1
        pu_m = iterapp('mldivide',m1fun,m1type,u_mp1,varargin{:});
        if ~allfinite(pu_m)
            flag = 2;
            resvec = resvec(1:mm+1);
            break
    ...
    Au_new = iterapp('mtimes',afun,atype,pu_m,varargin{:});
...
if flag == 0
    ...
    if flag == 1 || flag == 3
        r_comp2 = b - iterapp('mtimes',afun,atype,x,varargin{:});
