...
if ~isfloat(b)
    error(message('MATLAB:qmr:RHSInvalidClass'))
end
...
if (n2b == 0)                      % if rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg('qmr',0,maxit,0,flag,iter,NaN);
    end
    return
end
...
if existM1
    y = iterapp('mldivide',m1fun,m1type,vt,varargin{:},'notransp');
    if ~allfinite(y)
        flag = 2;
        relres = normr/n2b;
        iter = 0;
...
wt = r;
if existM2
    z = iterapp('mldivide',m2fun,m2type,wt,varargin{:},'transp');
    if ~allfinite(z)
        flag = 2;
        relres = normr/n2b;
        iter = 0;
...
for ii = 1 : maxit
    ...
    if existM2
        yt = iterapp('mldivide',m2fun,m2type,y,varargin{:},'notransp');
        if ~allfinite(yt)
            flag = 2;
            break
        end
    ...
    if existM1
        zt = iterapp('mldivide',m1fun,m1type,z,varargin{:},'transp');
        if ~allfinite(zt)
            flag = 2;
            break
        end
    ...
    pt = iterapp('mtimes',afun,atype,p,varargin{:},'notransp');
    epsilon = q' * pt;
    ...
    if strcmp(atype,'matrix')
        ...
        wt = iterapp('mtimes',afun,atype,q,varargin{:},'transp');
    end
    wt = wt - conj(beta) * w;
    if existM2
        z = iterapp('mldivide',m2fun,m2type,wt,varargin{:},'transp');
