...
if ~isfloat(b)
    error(message('MATLAB:bicg:RHSInvalidClass'))
end
...
epsT = eps("like", proto);
if tol < epsT
    warning(message('MATLAB:bicg:tooSmallTolerance'));
    warned = true;
    tol = epsT;
elseif tol >= 1
...
if (n2b == 0)                      % if    rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg('bicg',0,maxit,0,flag,iter,NaN);
    end
    return
end
...
r = b - iterapp('mtimes',afun,atype,x,varargin{:},'notransp');
...
for ii = 1 : maxit
    if existM1
        y = iterapp('mldivide',m1fun,m1type,r,varargin{:},'notransp');
        if ~allfinite(y)
            flag = 2;
            break
        end
    ...
    if existM2
        z = iterapp('mldivide',m2fun,m2type,y,varargin{:},'notransp');
        yt = iterapp('mldivide',m2fun,m2type,rt,varargin{:},'transp');
    ...
    if existM1
        zt = iterapp('mldivide',m1fun,m1type,yt,varargin{:},'transp');
        if ~allfinite(zt)
            flag = 2;
            break
        end
    ...
    q = iterapp('mtimes',afun,atype,p,varargin{:},'notransp');
    if strcmp(atype,'matrix')
        qt = A' * pt;
    else
        qt = iterapp('mtimes',afun,atype,pt,varargin{:},'transp');
    end
    ptq = pt' * q;
    if ptq == 0
        flag = 4;
...
if flag == 0
    ...
    r = b - iterapp('mtimes',afun,atype,xmin,varargin{:},'notransp');
    if flag == 1
        r_comp = b - iterapp('mtimes',afun,atype,x,varargin{:},'notransp');
