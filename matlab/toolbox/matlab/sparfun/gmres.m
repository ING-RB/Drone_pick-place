...
if ~isfloat(b)
    error(message('MATLAB:gmres:RHSInvalidClass'))
end
...
proto = real(full(zeros("like", b)));useSingle = support && useSingle;
...
if (n2b == 0)                     % if    rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg('gmres',0,maxit,0,flag,iter,NaN);
    end
    return
end
...
if existM1
    r = iterapp('mldivide',m1fun,m1type,r,varargin{:});
    if ~x0iszero
        minv_b = iterapp('mldivide',m1fun,m1type,b,varargin{:});
    else
        minv_b = r;
    end
    if ~allfinite(r) || ~allfinite(minv_b)
        flag = 2;
        x = xmin;
        relres = normr / n2b;
        iter = [0 0];
...
if existM2
    r = iterapp('mldivide',m2fun,m2type,r,varargin{:});
    if ~x0iszero
        minv_b = iterapp('mldivide',m2fun,m2type,minv_b,varargin{:});
    else
        minv_b = r;
    end
    if ~allfinite(r) || ~allfinite(minv_b)
...
resvec = zeros(inner*outer+1,1,"like",proto); % Preallocate vector for norm of residuals
resvec(1) = normr;                % resvec(1) = norm(b-A*x0)
normrmin = normr;                 % Norm of residual from xmin

%  Preallocate J to hold the Given's rotation constants.
J = zeros(2,inner,"like",proto);

U = zeros(n,inner,"like",proto);
R = zeros(inner,inner,"like",proto);
w = zeros(inner+1,1,"like",proto);
...
for outiter = 1 : outer
    ...
    for initer = 1 : inner
        ...
        %  Apply A to v.
        v = iterapp('mtimes',afun,atype,v,varargin{:});
        %  Apply Preconditioner.
        if existM1
            v = iterapp('mldivide',m1fun,m1type,v,varargin{:});
            if ~allfinite(v)
                flag = 2;
                break
            end
        ...
        if existM2
            v = iterapp('mldivide',m2fun,m2type,v,varargin{:});
            if ~allfinite(v)
                flag = 2;
                break
            end
        ...
        if (normr <= tolb || stag >= maxstagsteps || moresteps)
            ...
            r = b - iterapp('mtimes',afun,atype,xm,varargin{:});
            normr_act = norm(r);
            if normr_act <= tol*n2b
                x = xm;
                flag = 0;
                iter = [outiter, initer];
                break
            ...
            minv_r = r;
            if existM1
                minv_r = iterapp('mldivide',m1fun,m1type,r,varargin{:});
                if ~allfinite(minv_r)
                    flag = 2;
                    break
                end
            ...
            if existM2
                minv_r = iterapp('mldivide',m2fun,m2type,minv_r,varargin{:});
                if ~allfinite(minv_r)
                    flag = 2;
                    break
                end
