function [out, graphVizOutput] = callgraphviz (varargin)
    ...
    if ~matches(algorithm,{'dot','neato','twopi'})
        error(message('MATLAB:callgraphviz:UnrecognizedAlgorithmName', algorithm));
    end
    
    graphviz_cmd = fullfile(matlabroot, 'bin', computer('arch'), 'mwdot' );
    ...
    [out,graphVizOutput]=system(['"' graphviz_cmd '"' ' -K' algorithm varargin{2:end}]);
end
