...
%   Copyright 2012-2024 The MathWorks, Inc.
    ...
    methods(Static) 
        ...
        function initClasses
            ...
            for n=1:numel(unkClasses)
                ...
                sym = matlab.depfun.internal.MatlabSymbol(unkClasses{n}, ...
                                   MatlabType.BuiltinClass, whichResult, ...
                                   unkClasses{n}, whichResult);
        ...
        function s = makeCodeOrDataSymbol(f, type)
            ...
            if ismember(ext, matlab.depfun.internal.requirementsConstants.dataFileExt)
    ...
    methods
        ...
        % pre-computed class name and path to the class constructor file.
            ...
            s.Symbol = symStr;
            s.DotQualified = contains(symStr,'.');
        ...
        % the path and the base name of the built-in symbol.
            ...
            if isProxy(sym)
               ...
               [requirementsConstants.BuiltInStrAndATrailingSpace '('], ...
                    ...
                    % Regular file
                    if ~contains(sym.WhichResult, requirementsConstants.BuiltInStr)
                        pth = sym.WhichResult;
        ...
        % Return an empty MatlabSymbol array if the symbol is not a proxy.
            ...
            % Directed dispatch (using dot-notation)
            [pfiles, ftype] = sym.classList.classFiles(sym);
            ...
            if ~isempty(pfiles)
                ...
                % constructors. Signal, I'm looking at you again.
        ...
        % Serves like a copy constructor of MatlabSymbol class
            obj = matlab.depfun.internal.MatlabSymbol(sym.Symbol, ...
        ...
        function updateSymbol(symObj, newSym)
            ...
            if symObj.Type ~= matlab.depfun.internal.MatlabType.Data
                symObj.DotQualified = contains(newSym,'.');
            end
        end
        ...
        %--------------------------------------------------------------
        function determineMatlabTypeBasedOnFileExtension(symObj)
            import matlab.depfun.internal.MatlabType
            import matlab.depfun.internal.requirementsConstants

            if isfullpath(symObj.WhichResult) ...
                    && symObj.Type == MatlabType.NotYetKnown
                switch lower(symObj.Ext)
                    case requirementsConstants.dataFileExt
                        symObj.Type = MatlabType.Data;
                        % Data files never count as 'dot-qualified'.
                        symObj.DotQualified = false;
                    case requirementsConstants.simulinkModelExt
                        symObj.Type = MatlabType.SimulinkModel;
                    case requirementsConstants.stateflowFileExt
                        symObj.Type = MatlabType.MCOSClass;
                    case requirementsConstants.simulinkDataDictionaryExt
                        symObj.Type = MatlabType.SimulinkDataDictionary;
                    otherwise
        ...
        function determineMatlabType(symObj)
            ...
            removedCounter = 0;
            while symObj.Type == MatlabType.NotYetKnown && removedCounter<=1
        ...
        %--------------------------------------------------------------
        function heuristicsForExternalAPI(symObj)
            import matlab.depfun.internal.MatlabType

            % External API are considered as built-ins if they are
            % installed.
            if contains(symObj.WhichResult,  ' is a Java ')
                symObj.Type = MatlabType.JavaAPI;
            else
                symObj.Type = RefineTypeForDynamicExternalInterface(symObj.Symbol, symObj.Type);
            end
        end
            ...
            if ~isempty(name)
                ...
                if ~strcmp(name,symObj.Symbol) && ...
                    ...
                    if (isfullpath(clsFile) && matlabFileExists(clsFile)) || ...
                        ...
                        if type ~= MatlabType.NotYetKnown
                            ...
                            if isStaticMethod(symObj) || symObj.FullPathProvidedByUser
                                ...
                                if contains(symObj.WhichResult, requirementsConstants.BuiltInStr)
                                    % TO DO: Stopping hard-coding 'empty'
                                    rightmostDotIdx = regexp(symObj.Symbol,'\.empty$','ONCE');
                                    if ~isempty(rightmostDotIdx)
                                        symObj.Type = type;
                                    end
                                    symObj.updateSymbol(name);
                                end
                            end
                        end
                    end
                    ...
                    if symObj.Type ~= MatlabType.NotYetKnown
                        ...
                        if isClass(symObj)
                            % Symbol could be unqualified if the class file
                            % is passed in the root set. Here is the
                            % perfect place to update it with fully
        ...
        % OVERLOAD Add paths to overloaded methods this symbol represents
            ...
            function pth = symbolPath(symbolName, whichResult)
                ...
                if isempty(clsDir) && ...
                  contains(whichResult, requirementsConstants.BuiltInStr) && ...
        ...
        %
        %  To evaluate the answer returned by WHICH, check the following:
                    ...
                    %Name isn't dot-qualified.  
                        if numAts == 0
                            if strncmp(symObj.Symbol,fileName, length(symObj.Symbol))
                                ...
                                if numPlus > 0
                                % MCOS package function
                                    symObj.Symbol = qualifiedName(symObj.WhichResult);
                                end
                            else
                                symObj.Type = MatlabType.Ignorable;
                            end
                           ...
                           symObj.Type = MatlabType.NotYetKnown;
                            ...
                            elseif numAts == 1 && numPlus == 0  %Case 3 or 4
                                ...
                                symPathName = ...
                                 ...
                                 d = dir(symPathName);
                                 cel = struct2cell(d);
                                 if any(ismember({'schema.m' 'schema.p'},cel(1,:)))
        ...
        function found = findCorrespondentMCode(symObj, mExt)
            ...
            if cacheExist(mWhichResult,'file')
                found = true;
                r = parseFile(matlab.depfun.internal.MatlabInspector.MFileParser('get'), mWhichResult);
                if ~r.isempty
        ...
        function tf = isStaticMethod(sym)
            ...
            if isMethod(sym) && isDotQualified(sym) && isPrincipal(sym)
                sym_proxy = proxy(sym);
        ...
        % Path must contain one at-sign and end in schema.m.
            ...
            if ~isempty(schemaIdx)
                ...
                tf = ~isempty(atIdx) && isscalar(atIdx) && ...
        ...
        % isExtrinsic Is the symbol an extrinsic file? (Ask the symbol type.)
        ...
        % isBuiltin Is the symbol a builtin function? (Ask the symbol type.)
            tf = isBuiltin(o.Type);
        end
        ...
        %--------------------------------------------------------------
        function tf = isCppAPI(o)
            tf = isCppAPI(o.Type);
        end

        %--------------------------------------------------------------
        function tf = isSimulinkModel(o)
            tf = isSimulinkModel(o.Type);
        end

        %--------------------------------------------------------------
        function tf = isSimulinkDataDictionary(o)
            tf = isSimulinkDataDictionary(o.Type);
        end
    end %methods block
end  %class
...
% LocalWords:  dirs substr builtin's OO Ps CLASSFILE Recuse extensionless QSymbol BLKCOM
% LocalWords:  MCode Sliceable sliceable fileseps
