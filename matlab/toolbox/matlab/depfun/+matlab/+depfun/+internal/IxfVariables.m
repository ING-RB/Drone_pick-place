...
% accommodate full paths (which make matching more accurate) and the need for
...
%   Copyright 2013-2020 The MathWorks, Inc.
    ...
    methods
        ...
        % (See initialize_path_variables for the values.)
            ...
            vs = values(obj.Variables);
            for k=1:numel(ks)
                item = strrep(item, ks{k}, vs{k});
            end
        end
        ...
        function free = unbind(obj, bound)
            ...
            % multiple matches; we take a cue from regular expression matches
    ...
    methods(Access = private)
        ...
        %
        % For ease of maintenance, please keep the list in alphabetical
        % order. (By variable name.)
            obj.Variables = containers.Map;
            obj.Variables('$ARCH') = matlab.depfun.internal.requirementsConstants.arch;
