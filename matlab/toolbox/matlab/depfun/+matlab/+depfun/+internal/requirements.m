...
%
% RESOURCES: A cell array of the resources (resources contain parts)
%            required to execute the functions in the input ITEMS.
%
% UNPACKAGED_PARTS: A struct that contains three fields.
%   EXCLUDED: A cell array of parts that ITEMS requires but the TARGET 
%             forbids.
%   EXPECTED: A cell array of parts that ITEMS requires and will be present
%             in the TARGET.
% UNRESOLVED: A map of analyzed files and unresolved symbols in them.
...
if (~isempty(reqDB) && reqDB == '0') || tgt ~= Target.MCR
    useDatabase = false;
end
...
%       items, target, '-Z', supportpackages
...
% Validate input count -- at least one, no more than ten.
maxArg = 10;
narginchk(1,maxArg);
...
% need to process -Z before calling SearchPath since it's not relevant and
% SearchPath doesn't like it
spOption = {};
mccsetpathInputs = varargin;
for ii = 1:numel(mccsetpathInputs)
    if strcmp(mccsetpathInputs{ii},'-Z')
        % maybe error if target isn't MCR?
        spOption = mccsetpathInputs{ii+1};
        mccsetpathInputs(ii:ii+1) = [];
        break;
    end
end
...
% arguments).
if tgt == Target.MCR || tgt == Target.Deploytool
   if ~matlab.depfun.internal.mccsetpath('HasPathBeenScoped')
       restorePath = onCleanup(@()matlab.depfun.internal.mccsetpath(true));
       matlab.depfun.internal.mccsetpath(false, mccsetpathInputs{2:end});
   end
end
...
clearCompletion = onCleanup(@()clear('c'));
...
if tgt == Target.MCR || tgt == Target.Deploytool
    [spkgData,spkgMode] = matlab.depfun.internal.SPMode.validateSupportPackageInputs(spOption);
    c.setSupportPackageMode(spkgMode)
    c.setSupportPackageList(spkgData)
end

if nargout == 1
    parts = c.parts();
elseif nargout >= 2
    [parts, resources] = c.requirements();
end
if nargout > 2
    unpackaged_parts.excluded = c.excludedFiles();
    unpackaged_parts.expected = c.LicensedToolboxFilesInRuntimeCalledByUser;
    unpackaged_parts.unresolved = c.unresolvedSymbols();
end

% LocalWords:  unpackaged setbyme
