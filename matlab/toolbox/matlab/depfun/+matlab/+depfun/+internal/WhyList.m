...
%   Copyright 2012-2020 The MathWorks, Inc.
    ...
    methods (Access = private)
        ...
        function [listed, match] = listedBy(obj, target, fcn)
            ...
            % G2061831
            % Paths, which don't start with matlabroot and are not a built-in,
            % can skip the following comparison with elist. 
            filter = ~cellfun('isempty', regexp(fcn, ['(' ...
                                  requirementsConstants.canonicalMatlabRootPattern, ...
                             '|', requirementsConstants.BuiltInStr ')'], 'once'));
            if ~any(filter)
                return;
            end                
            ...
            if ~isempty(obj.List) && isKey(obj.List, target)
                ...
                while all(listed) == false && k <= length(elist)
                    ...
                    % G2061831 only do regexp for paths starts with matlabroot
                    matched = false(size(fcn));
                    matched(filter) = ~cellfun('isempty', regexp(fcn(filter), elist{k}, 'once'));
