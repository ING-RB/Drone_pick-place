...
%   Copyright 2012-2024 The MathWorks, Inc.
    ...
    properties (Access = public)
        ...
        CalledDuringDBBuild = false % Used to identify if the schema is called during database build or database use.
    end
    ...
    methods(Access = public)
        ...
        function addRules(obj, file)
            ...
            % rules file.
            restorePath = onCleanup(@()rmpath(ruleFcnDir));
            addpath(ruleFcnDir);
    ...
    methods(Access = private)
        ...
        function initVariables(obj)
            ...
            obj.commonBlock(...
                ...
                matlab.depfun.internal.Schema.esc(matlab.depfun.internal.requirementsConstants.arch);
        ...
        function [tline,statement] = statement2fcn(~, tline)  
    ...
    methods (Access = public)
        ...
        function exclude(obj, varargin)
            ...
            % Convert full paths to patterns
        ...
        function expect(obj, varargin)
            ...
            if varargin{1} == matlab.depfun.internal.Target.MCR
                ...
                if( obj.CalledDuringDBBuild )
                    list =  nv.MatlabModulesInBaseMatlabRuntime();
                    % g3260835 - 24b workaround for fixedpoint_numerictype
                    rmIdx = strcmp(list, fullfile(matlabroot, 'toolbox/fixedpoint/fixedpoint'));
                    list(rmIdx) = [];
                else
...
% destination list. (Combination of REPLACE and MOVE.)
    ...
    % destinationMap must be a handle object, because this is a
    % side-effect.
    [~, replacedIdx] = setdiff(fileList, keptFiles, 'legacy');
    
    % Replaced files. For example, p-file entry points.
    replacedFiles = fileList(replacedIdx);
    % Replacement files. For example, correspondent m-files for p-file
    % entry points.
    replacementFiles = keptFiles(replacedIdx);    
    ...
    % Make a note of why the file was replaced in the set, and add the
    % file to the list of replaced files.
    if ~isempty(replacedIdx)
        rememberReplacement(destinationMap, replacementFiles, ...
                            replacedFiles, reason, rule);
    end
end

% LocalWords:  canonicalization canonicalizes Za Itty Aiieeeee MCC's pth RDL
