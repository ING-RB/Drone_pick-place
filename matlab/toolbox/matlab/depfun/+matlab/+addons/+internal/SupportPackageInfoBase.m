classdef (Abstract) SupportPackageInfoBase < matlab.mixin.Heterogeneous & handle
%
...
%   Copyright 2015-2020 The MathWorks, Inc.
    ...
    properties (Access = protected) 
        ...
        % Cell array of the files and folders that should always be
        % included but not analyzed. 
        % All files and folders should be specified using a full path
        % Listing folders is preferred to individual files
        % Take care listing platform specific files and folders. Either use wildcards
        % or commands like computer('arch') or mexext to ensure the paths are correct. 
        % Any files under matlabroot will be ignored
        includeAsIsList = {};
    ...
    methods
        ...
        % function to get the list of files and folders that should be
        % included for deployment without being analyzed. 
        function filesAndFolders = getIncludeAsIsList(obj)
            filesAndFolders = obj.includeAsIsList;
        end
        ...
        function set.includeAsIsList(obj, newlist)
            obj.includeAsIsList = cellstr(newlist);
        end

        function set.mandatoryIncludeList(obj, newlist)
            obj.mandatoryIncludeList = cellstr(newlist);
        end
        
    end
    methods(Sealed = true)
        function filesAndFolders = getIncludeListCellStr(obj, fileList)
            filesAndFolders = cellstr(getIncludeList(obj, fileList));
        end

        function filesAndFolders = getIncludeAsIsListCellStr(obj)
            filesAndFolders = cellstr(getIncludeAsIsList(obj));
        end
    end
