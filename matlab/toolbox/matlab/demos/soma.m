...
uicontrol(figh,...                  % increment solution selection
   'Style'   , 'pushbutton',...
   'Units'   , 'normalized',...
...
uicontrol(figh   ,...                 % the [quit] button
   'Style'          , 'pushbutton',...
   'Units'          , 'normalized',...
...
uicontrol(figh   ,...                 % the info button
   'Style'          , 'pushbutton',...
   'Units'          , 'normalized',...
...
uicontrol(figh,...                  % rotate solution xy
   'Style',    'pushbutton',...
   'Units',    'normalized',...
...
uicontrol(figh,...                  % rotate solution yz
   'Style',    'pushbutton',...
   'Units',    'normalized',...
...
for c = 1:3                          % paint a sample V
   xx = xv+v(c,1)+dx;
   yy = yv+v(c,2)+dy;
   patch(xx*sqsize, yy*sqsize, purple);
end
...
for c = 1:4                          % paint a sample  L
   xx = xv+l(c,1)+dx;
   yy = yv+l(c,2)+dy;
   patch(xx*sqsize, yy*sqsize, blue);
end
...
for c = 1:4                          % paint a sample T
   xx = xv+t(c,1)+dx;
   yy = yv+t(c,2)+dy;
   patch(xx*sqsize, yy*sqsize, green);
end
...
for c = 1:4                          % paint a sample Z
   xx = xv+z(c,1)+dx;
   yy = yv+z(c,2)+dy;
   patch(xx*sqsize, yy*sqsize, yellow);
end
...
for c = 1:4                          % paint a sample S
   xx = xv+s(c,1)+dx;
   yy = yv+s(c,2)+dy;
   patch(xx*sqsize, yy*sqsize, orange);
end
...
for c = 1:4                          % paint a sample Y
   xx = xv+y(c,1)+dx;
   yy = yv+y(c,2)+dy;
   patch(xx*sqsize, yy*sqsize, violet);
end
...
layery = .32;
lw = sqsize*4;
text(layerx+.005, layery, getString(message('MATLAB:demos:soma:LabelBottom')), 'color', 'black');
text(layerx+lw+.01, layery, getString(message('MATLAB:demos:soma:LabelMiddle')), 'color', 'black');
text(layerx+2*lw+.025, layery, getString(message('MATLAB:demos:soma:LabelTop')), 'color', 'black');
