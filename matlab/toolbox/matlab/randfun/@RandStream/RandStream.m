...
%   Copyright 2008-2024 The MathWorks, Inc.
    ...
    methods % subsref is overloaded, but need these so that struct will work
        function b = get.State(a),           b = matlab.internal.math.RandStream_getset_mex('state',a);         end
        function b = get.Substream(a),       b = matlab.internal.math.RandStream_getset_mex('substream',a);     end
        function b = get.NormalTransform(a), b = matlab.internal.math.RandStream_getset_mex('randnalg',a);      end
        function b = get.Antithetic(a),      b = matlab.internal.math.RandStream_getset_mex('antithetic',a);    end
        function b = get.FullPrecision(a),   b = matlab.internal.math.RandStream_getset_mex('fullprecision',a); end
    end
    ...
    properties(Constant=true, GetAccess='protected')
        ...
        BuiltinTypes = ...
             {'dsfmt19937'  'mcg16807' 'mlfg6331_64'   'mrg32k3a' 'mt19937ar' 'philox4x32_10' 'shr3cong' 'swb2712' 'threefry4x64_20'};
        CompatNames = ...
             {'simdTwister' 'v4'       'multFibonacci' 'combRecursive' 'twister'   'philox' 'v5normal' 'v5uniform' 'threefry'};
        BuiltinDescrs = ...
             {'MATLAB:RandStream:list:dsfmt19937Description' ...
              'MATLAB:RandStream:list:mcg16807Description' ...
              'MATLAB:RandStream:list:mlfg6331_64Description' ...
              'MATLAB:RandStream:list:mrg32k3aDescription' ...
              'MATLAB:RandStream:list:mt19937arDescription' ... ...
              'MATLAB:RandStream:list:philox4x32_10Description' ...
              'MATLAB:RandStream:list:shr3congDescription' ...
              'MATLAB:RandStream:list:swb2712Description' ...
              'MATLAB:RandStream:list:threefry4x64_20Description'
              };
    ...
    methods(Static=true, Access='public')
        ...
        %            RANDSTREAM.SETGLOBALSTREAM
            ...
            for i = 1:numel(streamIdx)
                ...
                s = RandStream.createUninitializedObject();
                s.Type = type;
                ...
                s.StreamID = matlab.internal.math.RandStream_create_mex(...
                    s.Type,s.NumStreams,s.StreamIndex,s.Seed,s.Params);
                if ~isempty(randnalg)
                    set(s, 'NormalTransform', randnalg);
                end
                streams{i} = s;
            end
    ...
    methods(Access='public')
        ...
        %            RANDSTREAM/RAND, RANDSTREAM/RANDI, RANDSTREAM/RANDN.
            ...
            % Special case to create uninitialized object (internal use only)
            if isa(type, "matlab.internal.math.UninitializedTag")
                return
            end
        ...
        function disp(s)
            if isvalid(s) && s.StreamID>0
                ...
                if isequal(lower(s.Type),'legacy')
                    state = matlab.internal.math.RandStream_getset_mex('state',s);
                    switch state{1}(1)
        ...
        function [varargout] = subsref(a,s)
            ...
            case '.'
                ...
                otherwise
                    ...
                    switch s(1).subs
                    case 'State'
                        varargout{1} = matlab.internal.math.RandStream_getset_mex('state',a);
                    case 'Substream'
                        varargout{1} = matlab.internal.math.RandStream_getset_mex('substream',a);
                    case 'NormalTransform'
                        varargout{1} = matlab.internal.math.RandStream_getset_mex('randnalg',a);
                    case 'Antithetic'
                        varargout{1} = matlab.internal.math.RandStream_getset_mex('antithetic',a);
                    case 'FullPrecision'
                        varargout{1} = matlab.internal.math.RandStream_getset_mex('fullprecision',a);
        ...
        function c = subsasgn(a,s,b)
            ...
            case '.'
                ...
                switch s(1).subs
                case 'State'
                    matlab.internal.math.RandStream_getset_mex('state',a,b);
                case 'Substream'
                    matlab.internal.math.RandStream_getset_mex('substream',a,b);
                case 'NormalTransform'
                    matlab.internal.math.RandStream_getset_mex('randnalg',a,b);
                case 'Antithetic'
                    matlab.internal.math.RandStream_getset_mex('antithetic',a,b);
                case 'FullPrecision'
                    matlab.internal.math.RandStream_getset_mex('fullprecision',a,b);
                case 'Seed'
    ...
    methods(Access='protected')
        function advance(s,nsteps)
            matlab.internal.math.RandStream_advance(s,nsteps);
        end

        function tf = isGlobalStream(s)
            tf = isequal(s, localGetSetGlobalStream("nocreate"));
        end
    end
    ...
    methods(Hidden=true, Static=true, Access='public')
        function a = loadobj(b)
            if isequal(b.Type,'')
                ...
                a = RandStream.createUninitializedObject();
                return
            end
            ...
            try
                ...
                a.StreamID = matlab.internal.math.RandStream_create_mex(...
                    a.Type,a.NumStreams,a.StreamIndex,a.Seed,a.Params);
        ...
        function seed = shuffleSeed
            ...
            seed0 = mod(floor(now*8640000),2^31-1); %#ok<TNOW1> % traditionally this was sum(100*clock)
            for i = 1:100
                seed = mod(floor(now*8640000),2^31-1); %#ok<TNOW1> 
                if seed ~= seed0, break; end
                % use a while loop instead of pause() to prevent side effects
                % like update traversal and execution of callbacks from occurring
                t=tic; while toc(t)<0.01; end % 0.01 is the smallest recommended interval
            end
        end
        ...
        function restoreDefaultGlobalStream()
            % Set the global random number stream back to its default state.
            %   RandStream.restoreDefaultGlobalStream() sets the global random number 
            %   stream back to its default state. Note that this does not invalidate
            %   the existing global stream so any references to it will remain usable.
            %
            %   See also RANDSTREAM, RANDSTREAM.GETGLOBALSTREAM, 
            %            RANDSTREAM.SETGLOBALSTREAM.
            localGetSetGlobalStream([]);
        end
        function name = matlabCodegenRedirect(~)
            name = 'coder.internal.RandStream';
        end
    end
        
    methods(Static=true, Access='protected')
        function globe = createGlobalStream()
            % Create a RandStream object for the global stream.
            % We only get here if creating afresh from the default. We read
            % the default from C++ in-case it has already been initialized
            % there.
            globe = RandStream.createUninitializedObject();
            globe.Params = [];
            globe.SpawnIncr = uint64(0);
            [globe.StreamID,globe.Type,globe.Seed,globe.NumStreams,globe.StreamIndex] = ...
                matlab.internal.math.getsetGlobalRandStream();
            % antithetic, normalTransform, substream, and state come from existing globalstream C++ object
        end
        function legacy = createLegacyStream()
            legacy = RandStream.createUninitializedObject();
            legacy.Type = 'legacy';
            ...
            legacy.StreamID = matlab.internal.math.getLegacyRandStream();
            % antithetic, normalTransform, substream, and state come from existing legacystream C++ object
        end
    end
    
    methods(Static=true, Access='private')
        function obj = createUninitializedObject()
        % Create uninitialized RandStream object.
        % This object cannot be used until properties are populated.
            persistent uninitializedTag;
            if isempty(uninitializedTag)
                uninitializedTag = matlab.internal.math.UninitializedTag();
            end

            % Just use the special constructor syntax.
            obj = RandStream(uninitializedTag);
        end
    end
    ...
    methods(Hidden=true, Access='public')
        ...
        function  delete(s)
            id = s.StreamID;
            s.StreamID = 0; % Ensure is reset in case we get interrupted
            matlab.internal.math.clearGlobalRandStreamIfMatches(id);
            matlab.internal.math.RandStream_delete(id);
        end
...
% Special syntax to return the current global state immediately, even if empty.
if nargin==1 && isequal(stream,"nocreate")
    stream = globe;
    return
end
...
if matlab.internal.math.RandStream_inLegacyMode()
    if isempty(legacy) || ~isvalid(legacy)
        legacy = RandStream.createLegacyStream();
    end
    globe = legacy;
elseif isempty(globe) || ~isvalid(globe)
    % Only create the default if we're not about to overwrite it.
...
elseif isa(stream,'RandStream') && isvalid(stream)
    matlab.internal.math.getsetGlobalRandStream(stream.StreamID);
    globe = stream;
elseif isempty(stream)
    % Clear the existing global stream. A new stream with default values
    % will be created on next use.
    globe = [];
    matlab.internal.math.clearGlobalRandStream();
else
