...
if varInSelTable
    ...
    for k=1:length(aeind)
        ...
        if ~feature('webui')
            com.mathworks.page.datamgr.brushing.ArrayEditorManager.setSelection(varStr);
        end
        % Updates Selection on BrushAction if present.
        if ~isempty(h.VEBrushAction)
            h.VEBrushAction.setSelection(varStr, h.SelectionTable(ind).I);
        end
        if ~feature('webui')
            com.mathworks.mlwidgets.array.brushing.BrushingActionFactory.setVarBrushedState(varName,any(h.SelectionTable(ind).I(:)));
        end
    end
end
