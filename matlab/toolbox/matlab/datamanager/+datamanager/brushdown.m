...
dlg = getappdata(fig,'brushing_disambiguateDlg');
...
if ~feature('webui') && strcmp(fig.Renderer,'painters') && ~isempty(findobj(gca,'type','surface'))
    % JSD_OGL_REMOVAL
...
if ~isempty(evd.HitPrimitive)
    % Test for axes ancestors. Use the 'node' argument to be sure that they 
    % are true ancestors and not just reported as a parent (1763353)
    obj = ancestor(evd.HitObject, 'axes', 'node');
else
