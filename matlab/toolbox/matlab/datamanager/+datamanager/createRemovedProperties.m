...
zdataPresent = ~isempty(h.findprop('ZData')) && ~isempty(h.ZData);
if zdataPresent
    zdata = get(h,'ZData');
else
    zdata = [];
...
pvPairs = {'BrushData',brushdata};
if manMode && ~isequaln(xdata,currentxdata)
    pvPairs(end+1:end+2) = {'XData',xdata};
end
if ~isequaln(ydata,currentydata)
    pvPairs(end+1:end+2) = {'YData',ydata};
end
if zdataPresent && ~isequaln(zdata,currentzdata)
    pvPairs(end+1:end+2) = {'ZData',zdata};
end
