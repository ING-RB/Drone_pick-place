...
if isempty(axeshandles)
    matlab.graphics.internal.BasicFitUtils.createaxislegend(axesH,[],legH, @bfitlisten, @bfitlistenon, @bfitlistenoff, @createdatalegend);
end

for i=1:length(axeshandles)
    if ~isgraphics(axeshandles(i))
        continue;
    end

    ah = handle(axeshandles(i));
    if ~isequal(ah, handle(axesH))
        continue;
    end
    ...
    elseif (isempty(legend('-find',ah)) && (isempty(legH) || (~isempty(legH) && axesH~=axeshandles(i))))
        continue;
    end
    ...
    matlab.graphics.internal.BasicFitUtils.createaxislegend(ah,axesdata,legH, @bfitlisten, @bfitlistenon, @bfitlistenoff, @createdatalegend);
end
