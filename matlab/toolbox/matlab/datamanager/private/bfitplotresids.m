...
if ~isempty(residinfo.axes) && ishghandle(residinfo.axes)
    ...
    residaxespos = get(residinfo.axes, 'position'); 
...
if ~subploton % residuals in separate figure
    ...
    updateResidualAxesXLim(residinfo.axes, axesH);
else % subplot
    ...
    if isempty(residaxespos)
        ...
        pos(4) = height;
        pos(2) = y + height ;
        set(axesH,'outerposition', pos);
        pos(2) = y;
        residinfo.axes = axes('outerposition', pos, 'parent', figH);
    else
...
function updateResidualAxesXLim(residualAxes, axesH)
    % Removing all the fits when residuals are plotted can corrupt the limits
    % on the axes containing the data. g2146360
    if strcmp(residualAxes.XLimMode,'auto')
        residualAxes.XLim = axesH.XLim;
    end
