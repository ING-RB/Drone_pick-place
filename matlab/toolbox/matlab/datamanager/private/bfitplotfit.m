...
curve = matlab.graphics.function.FunctionLine(...
    @(x) fun(x,pp,meanx,stdx),'Parent',hParent,'Tag',...
    name,'Color',color_order(colorindex,:),'XLimInclude','off');
% g2450533 - axis equal can cause function line to zoom the limits to
% infinity since ylim will update xlim and vice-versa. To resolve this
% issue, YLimInclude is set to 'off' in such cases
if isequal(axesh.DataAspectRatio,[1 1 1])
    set(curve,'YLimInclude','off');
end
