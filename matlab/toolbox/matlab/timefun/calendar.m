...
elseif nargin == 1
    ...
    if (isscalar(c) && (isnumeric(c) || isdatetime(c) || iscellstr(c)) ) || matlab.internal.datatypes.isScalarText(c, false)  % don't allow empty/missing text, allow scalar cellstr
        c = datevec(c);
    else
    ...
    if ~isnumeric(c) || ~isnumeric(m) || ~isscalar(c) || ~isscalar(m)
