...
%   ADDTODATE is not recommended. Use datetime values to represent points 
%   in time, duration or calendarDuration values to represent elapsed times, 
%   and the plus operator (+) instead.
%
...
%   T:  character vector or string scalar defining which date field to add to (see NOTE 1).
%
%   RETURN PARAMETERS:
...
%
%   See also DATETIME, DURATION, CALENDARDURATION, DATETIME/PLUS, DATETIME/MINUS.

%   Copyright 2002-2024 The MathWorks, Inc.
...
if ~isnumeric(dtNumber) || ~isscalar(dtNumber) || ~isreal(dtNumber)
...
if ~isnumeric(additionalQuantity) || ~isscalar(additionalQuantity) || abs(additionalQuantity) > 1e16 || ~isreal(additionalQuantity) || isnan(additionalQuantity)
    error(message('MATLAB:addtodate:InputQuantity'));
end
dateField = convertStringsToChars(dateField);
if ~ischar(dateField)
