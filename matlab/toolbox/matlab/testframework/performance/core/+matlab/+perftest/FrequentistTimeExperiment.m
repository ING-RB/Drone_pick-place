classdef FrequentistTimeExperiment < matlab.perftest.TimeExperiment
    ...
    %       NumWarmups            - Number of warmup measurements (default 5)
    %       MinSamples            - Minimum number of samples to collect after warmup (default 4)
    %       MaxSamples            - Maximum number of samples to collect after warmup (default 256)
    ...
    methods (Access = ?matlab.perftest.TimeExperiment)
        function experiment = FrequentistTimeExperiment(varargin)
            ...
            p.addParameter('NumWarmups',               5, @validateNumWarmups);
            p.addParameter('MinSamples',               4, @validateSampleSize);
            p.addParameter('MaxSamples',             256, @validateSampleSize);
    ...
    methods(Hidden, Access=protected)
        function doRun(experiment, operator, runner, suite, runIdentifier)
            ...
            % run repeatedly
            maxMeasurements = experiment.MeasurementPlugin.MaxEstimationPhaseCount + experiment.NumWarmups + experiment.MaxSamples;
        ...
        function stop = earlyTerminationFcn(experiment, result, currentIndex)
            ...
            if ~experiment.MeasurementPlugin.UsingLoopedMeasurement
                ...
                if experiment.MeasurementPlugin.Estimating
                    stop = false;
                    return
                end

                numSamples = experiment.MeasurementPlugin.RepeatIndex - experiment.NumWarmups - experiment.MeasurementPlugin.EstimationPhaseCount;
            end
        ...
        function handleMeasurementEvent(experiment,evt)
            ...
            if ~experiment.MeasurementPlugin.UsingLoopedMeasurement
                ...
                if experiment.MeasurementPlugin.Estimating || idx <= (experiment.NumWarmups + experiment.MeasurementPlugin.EstimationPhaseCount)
                    % no accumulation occurs yet
                    return;
                end
            end
