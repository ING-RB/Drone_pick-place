classdef TimeExperiment < matlab.unittest.measurement.internal.Experiment
    ...
    %   a matlab.perftest.TimeResult object for each 
    %   element of the suite. TimeExperiment can be configured to collect
    ...
    methods (Static)
        ...
        function experiment = limitingSamplingError(varargin)
            ...
            %   of the following:
            %
            %       * NumWarmups            - Number of warmup runs (default 5)
            %       * MinSamples            - Minimum number of samples to collect (default 4)
            %       * MaxSamples            - Maximum number of samples to collect (default 256)
    ...
    methods(Sealed)
        function result = run(experiment, suite, varargin)
            ...
            uuid = categorical(string(runIdentifier));
            
            operator = experiment.Operator;
            operator.Result = TimeResult({suite.Name}, 'MeasuredTime', uuid);
    ...
    methods (Hidden, Access = protected)
        function experiment = TimeExperiment(varargin)
            ...
            experiment@matlab.unittest.measurement.internal.Experiment(runner,operator,plugin);
        end
    end
end
...
for idx = 1:numel(result)
    ...
    numEstimationExecuted = nnz(testActivity{:,"Objective"} == "estimation");
    numWarmupsExecuted = min(nnz(testActivity.Iteration <= numWarmups), height(testActivity));
    warmupStartIdx = numEstimationExecuted + 1;
    warmupEndIdx = numEstimationExecuted + numWarmupsExecuted;
    testActivity.Objective(warmupStartIdx:warmupEndIdx) = categorical({'warmup'});
...
% LocalWords:  runperf Frequentist NUMSAMPLES Warmups NUMWARMUPS mynamespace LIMITINGSAMPLINGERROR
% LocalWords:  warmup WITHFIXEDSAMPLESIZE
