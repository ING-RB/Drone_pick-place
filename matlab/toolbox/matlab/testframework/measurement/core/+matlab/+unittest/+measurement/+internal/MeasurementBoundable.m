classdef(Hidden) MeasurementBoundable < matlab.unittest.internal.Measurable
    ...
    methods(Sealed)
        function startMeasuring(measurable, label)
            ...
            %   and labels the measurement with LABEL. LABEL must be a valid MATLAB variable name.
            %   The startMeasuring method requires a subsequent call to stopMeasuring
            ...
            measurable.notify('MeasurementStarted', LabelEventData(label));
        end
        ...
        function stopMeasuring(measurable, label)
            ...
            measurable.notify('MeasurementStopped', LabelEventData(label));
        end
    end
end

% LocalWords:  tfprintf perftest fid abcdef filecleanup
