classdef ExperimentOperator < handle
    ...
    properties
        Result = matlab.unittest.measurement.DefaultMeasurementResult.empty;
    end
    ...
    properties(SetAccess = immutable, ...
            GetAccess = {?matlab.unittest.measurement.MeasurementPlugin})
        KeepMeasuringOverheadEstimator;
    end

    properties(SetAccess = immutable, ...
            GetAccess = {?matlab.unittest.measurement.MeasurementPlugin})
        LoopedMeasurementOverheadEstimator;
    end
    
    
    methods
        function operator = ExperimentOperator(meter,calibrator,keepMeasuringEstimator, loopedMeasurementEstimator)
            import matlab.unittest.measurement.internal.DefaultCalibrator;
            import matlab.unittest.measurement.internal.looping.DefaultKeepMeasuringOverheadEstimator;
            import matlab.unittest.measurement.internal.looping.LoopedMeasurementOverheadEstimator;
            ...
            if nargin < 3
                keepMeasuringEstimator = DefaultKeepMeasuringOverheadEstimator;
            end

            if nargin < 4
                loopedMeasurementEstimator = LoopedMeasurementOverheadEstimator;
            end
            
            operator.Calibrator = calibrator;
            operator.KeepMeasuringOverheadEstimator = keepMeasuringEstimator;
            operator.LoopedMeasurementOverheadEstimator = loopedMeasurementEstimator;
        end
        
        function completeTestRun(operator,idx, estimating)
            ...
            if (estimating)
                measResult.InternalTestActivity.Objective(end) = "estimation";
            end
        ...
        function estimateKeepMeasuringOverhead(operator)
            meterCopy = copy(operator.Meter);
            meterCopy.clear;
            operator.KeepMeasuringOverheadEstimator.estimate(meterCopy);
        end

        function estimateLoopedMeasurementOverhead(operator)
            meterCopy = copy(operator.Meter);
            meterCopy.clear;
            operator.LoopedMeasurementOverheadEstimator.estimate(meterCopy);
        end
    end
