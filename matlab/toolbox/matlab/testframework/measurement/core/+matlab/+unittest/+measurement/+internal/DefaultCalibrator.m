classdef DefaultCalibrator < matlab.unittest.measurement.internal.Calibrator
    ...
    methods
        ...
        function calibrate(calibrator, meter, suite)
            ...
            operator.Result = DefaultMeasurementResult({calibrationSuite.Name, selfMeasuredCalibrationSuite.Name}, 'CalibrationOffset');
            
            % Run the suite 11 times (5 warmup, 6 samples)
            runner = TestRunner.withNoPlugins;
            runner.addPlugin(MeasurementPlugin(operator));
            runner.runRepeatedly([calibrationSuite, selfMeasuredCalibrationSuite], 11);
            
            result = applyWarmups(operator.Result, 5);
            ...
            calibrationThreshold = cell(numel(result),1);
            for k = 1:numel(result)
                calibrationThreshold{k} = getMinimumValue(result(k).InternalTestActivity.Measurement(6:11));
            end
            ...
            % Keep the self-measured calibration if we need to swap with it later
            if numel(selfMeasuredCalibrationSuite) > 0
                calibrator.SelfMeasurableCalibrationResult = result(end);
                calibrator.SelfMeasurableCalibrationValue = calibrationThreshold{end};
            end
        end
...
suite = TestSuite.fromNamespace('matlab.unittest.measurement.internal.calibration');
end
...
import matlab.unittest.TestSuite;
suite = TestSuite.fromNamespace('matlab.unittest.measurement.internal.calibration.selfmeasured');
end

% LocalWords:  warmup Warmups selfmeasured
