classdef AccumulatedMeasurement < matlab.unittest.measurement.internal.MeasurementInterface
    ...
    properties(SetAccess = {?matlab.unittest.measurement.MeasurementPlugin})
    ...
    methods
        ...
        function measurement = updateLastMeasurementsLoopingData(measurement, niter, overhead)
            if ~isempty(measurement.Measurements)
                measurement.Measurements(end).NumIterations = niter;
                measurement.Measurements(end).Overhead = overhead;
            end
        end
    end
