classdef (Sealed) MethodCallBehavior < matlab.mock.InteractionBehavior
    ...
    properties (Access=private)
        ...
        %   not produce outputs.
        %
        HasOutputs (1,1) logical = false;
        
        ExactInputs (1,1) logical = false;
    end
    ...
    properties (Hidden, Constant, Access=protected)
        ActionClassName = "matlab.mock.actions.MethodCallAction";
    end
    ...
    methods
        ...
        function when(behavior, action)
            ...
            behavior.validateElementUnambiguousSpecification;
    ...
    methods (Hidden, Access=protected)
        function summary = getElementDisplaySummary(behaviorElement)
            if behaviorElement.isAmbiguousSpecification
                displayedInputs = getString(message("MATLAB:mock:display:UnspecifiedSummary"));
            else
                displayedInputs = strjoin([string.empty, behaviorElement.InputRequirements.OneLineSummary], ", ");
            end
            summary = getOutputArgumentDisplay(behaviorElement.Nargout) + behaviorElement.getStaticMethodCallDisplay + ...
                behaviorElement.Name + "(" + displayedInputs + ")";
        end
        
        function validateElementUnambiguousSpecification(behaviorElement)
            if behaviorElement.isAmbiguousSpecification
                error(message('MATLAB:mock:MethodCallBehavior:AmbiguousMethodCallSpecification', ...
                    behaviorElement.Name, 'withExactInputs', 'withAnyInputs'));
            end
        end
    end
    ...
    methods (Access=private)
        ...
        function bool = hasMinimalInputs(behavior)
            ...
            label = builtin("_getMockLabel", behavior.Inputs{1});
