classdef InteractionCatalog < handle
    ...
    methods
        ...
        function applyCheckToAllOrderedRecords(catalog, checker)
            allRecords = [catalog.MethodCallRecord.Entries, catalog.PropertyGetRecord.Entries, catalog.PropertySetRecord.Entries];
            [~, idx] = sort([allRecords.ID]);
            orderedRecords = allRecords(idx);
            
            idx = 0;
            while idx < numel(orderedRecords) && ~checker.isDone()
                idx = idx + 1;
                checker.check(orderedRecords(idx).Value);
            end
        end
        
        function clearAllRecords(catalog)
            catalog.MethodCallRecord.clear;
            catalog.PropertyGetRecord.clear;
            catalog.PropertySetRecord.clear;
        end

        function disableRecordTracking(catalog)
            catalog.MethodCallRecord.disableAddingNewEntries;
            catalog.PropertyGetRecord.disableAddingNewEntries;
            catalog.PropertySetRecord.disableAddingNewEntries;
        end
    end
