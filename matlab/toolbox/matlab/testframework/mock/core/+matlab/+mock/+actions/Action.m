classdef (Hidden, Abstract) Action < matlab.mixin.Scalar
    ...
    methods (Sealed)
        ...
        function repeated = repeat(action, numTimes)
            % repeat - Perform the same action multiple times
            %
            ...
            %           AssignOutputs(true).repeat(5).then(AssignOutputs(false)).then(ThrowException));
            %
            %       % Use the mock
            %       for i = 1:7
            %       	isAccountOpen = mock.isOpen
            ...
            if isnumeric(action)
                repeated = numTimes.repeat(action);
                return;
            end
