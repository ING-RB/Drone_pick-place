classdef FilterSummaryPart < matlab.unittest.internal.dom.ReportDocumentPart
    ...
    methods(Hidden) % Fill template holes ---------------------------------
        ...
        function fillFilterSummaryText(docPart)
            ...
            if numFiltered == 1
                docPart.appendText(docPart.Catalog.getString('SingleFilterSummaryText'));
            else
                docPart.appendText(docPart.Catalog.getString('FilterSummaryText',numFiltered));
            end
        end
        
        function fillTestNameColumnLabel(docPart)
            docPart.appendText(docPart.Catalog.getString('FilteredTestNameColumnLabel'));
        end
        
        function fillFilterTableRowParts(docPart)
            docPart.appendDocParts(docPart.FilterTableRowParts);
        end
    end
end
