classdef LineMetricHandler < matlab.unittest.internal.coverage.metrics.MetricHandler
    %
    ...
    methods
        function metric = getMetricInstance(~,covData)
            metric  = matlab.unittest.internal.coverage.metrics.LineMetric(covData);
        end

        function formatter = getFormatter(~)
            formatter = matlab.unittest.internal.coverage.NullCoverageReportFormatter;
        end
