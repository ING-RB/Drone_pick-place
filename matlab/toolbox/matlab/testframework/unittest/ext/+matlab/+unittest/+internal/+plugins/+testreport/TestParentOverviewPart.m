classdef TestParentOverviewPart < matlab.unittest.internal.dom.ReportDocumentPart
    ...
    methods(Hidden) % Fill template holes ---------------------------------
        function fillTestParentName(docPart)
            docPart.appendLinkTargets(docPart.OverviewLinkTarget);
            docPart.appendInternalLinks(docPart.LinkToTestParentDetails);
        end
        
        function fillTotalDurationText(docPart)
            docPart.appendText(docPart.Catalog.getString('TimeInSeconds',...
        ...
        function fillResultsIcons(docPart)
            ...
            for k=1:numImages
                docPart.appendInternalLinks(docPart.ImageObjects(k));
                if k~=numImages
