classdef FigureDiagnostic < matlab.unittest.internal.mixin.PrefixMixin & ...
    ...
    methods(Hidden)
        function diagnoseWith(diag,diagData)
            ...
            fileNameWithoutExt = fullfile(diagData.ArtifactsStorageFolder, ...
                diag.Prefix + uuid);

            artifactsCell = arrayfun(@(format) figureToArtifact(...
                diag.Figure, fileNameWithoutExt, format, diagData.ArtifactsDisplayFolder),...
                diag.Formats, 'UniformOutput', false);
            artifacts = [artifactsCell{:}];

            fileLinksCell = arrayfun(@(artifact) createFileLink(artifact,diagData), ...
...
function artifact = figureToArtifact(fig,fileNameWithoutExt,format, artifactsDisplayFolder)
import matlab.unittest.diagnostics.FileArtifact;
file = fileNameWithoutExt + "." + format;

if format ~= "fig" && matlab.ui.internal.isUIFigure(fig)
    capture = @exportapp;
else
    capture = @saveas;
end
...
artifact = FileArtifact(file, FinalLocation=artifactsDisplayFolder);
end


function cmdText = createCommandThatOpensFile(artifact, finalArtifactFileName)
charSafeFile = strrep(char(finalArtifactFileName),'''','''''');
...
validateGeneratedPathname(prefix + uuid + "." + format,'Prefix');
end

function fileLink = createFileLink(artifact,diagData)
import matlab.unittest.internal.diagnostics.CommandHyperlinkableString;

displayText = diagData.createDisplayFileName(artifact.FullPath);
fileLink = indentWithArrow(CommandHyperlinkableString(...
    displayText, createCommandThatOpensFile(artifact,displayText)));
end
