classdef EventListPart < matlab.unittest.internal.dom.ReportDocumentPart
    ...
    methods(Hidden) % Fill template holes ---------------------------------
        function fillEventsLabel(docPart)
            ...
            docPart.appendText(labelText);
        end
        
        function fillEventParts(docPart)
            docPart.appendDocParts(docPart.EventParts);
        end
    end
...
if isa(eventRecord,'matlab.unittest.internal.eventrecords.QualificationEventRecord')
    docPart = QualificationEventPart(eventRecord);
elseif isa(eventRecord,'matlab.unittest.internal.eventrecords.ExceptionEventRecord')
    docPart = ExceptionEventPart(eventRecord);
elseif isa(eventRecord,'matlab.unittest.internal.eventrecords.LoggedDiagnosticEventRecord')
