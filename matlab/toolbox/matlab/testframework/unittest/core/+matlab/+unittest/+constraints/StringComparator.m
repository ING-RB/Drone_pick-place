    ...
    methods(Hidden, Access=protected)
        ...
        function bool = containerSatisfiedBy(comparator, actVal, expVal)
            ...
            elseif builtin('isstring',expVal)
                ...
                bool = builtin('iscell',actVal) && comparator.haveSameSize(actVal,expVal);
    ...
    methods (Access=private)
        function bool = charsAreEquivalent(comparator,actVal,expVal)
            bool = isequal(actVal,expVal) || ...
                ...
                comparator.satisfyEmptyRequirement(actVal,expVal);
        end
        ...
        function bool = stringsHaveSameStructure(comparator,actVal,expVal)
            if ~comparator.haveSameSize(actVal,expVal)
                bool = false;
        ...
        function elemComparisons = getStringElementComparisons(comparator,comparison)
            ...
            expVal = comparison.ExpectedValue;
             ...
             if numel(expVal) < 2 || isequaln(actVal, expVal)
                elemComparisons = comparator.EmptyComparisonArray;
                return;
            end
        ...
        function elemComparisons = getCellElementComparisons(comparator,comparison)
            ...
            if ~builtin('iscellstr',actVal)
                firstNonCharInd = find(cellfun(@(x) ~builtin('ischar',x),actVal),1);
                actVal = actVal(1:firstNonCharInd-1);
                expVal = expVal(1:firstNonCharInd-1);
            elseif isequaln(actVal, expVal) % Short cut. Note: isequaln required since strcmp({''},{char.empty(1,0)}) returns true.
                elemComparisons = comparator.EmptyComparisonArray;
                return;
            end
            ...
            if textArraysAreEqual && ~comparator.IgnoreWhitespace
                mask = cellfun(@(actElem,expElem) comparator.haveSameSize(actElem,expElem),actVal,expVal);
                textArraysAreEqual = all(mask(:));
            end
            
            if ~textArraysAreEqual
                elemComparisons = comparator.createCellElementComparison(find(~mask,1),comparison);
            elseif ~isempty(firstNonCharInd)
                elemComparisons = comparator.createCellElementComparison(firstNonCharInd,comparison);
            else
                elemComparisons = comparator.EmptyComparisonArray;
            end
        end
        ...
        function conds = getCharConditionsFor(comparator,actVal,expVal)
            ...
            if comparator.satisfyEmptyRequirement(actVal,expVal)
                conds = MessageDiagnostic('MATLAB:unittest:StringComparator:CharsEqualIgnoringEmptyShape');
            elseif ~comparator.textValuesAreEqual(actVal,expVal)
        ...
        function conds = getStringConditionsFor(comparator,actVal,expVal)
            ...
            if ~comparator.haveSameSize(actVal,expVal)
                conds = ConstraintDiagnosticFactory.generateSizeMismatchDiagnostic(actVal,expVal);
            elseif comparator.satisfyEmptyRequirement(actVal,expVal)
                conds = MessageDiagnostic('MATLAB:unittest:StringComparator:StringsEqualIgnoringEmptyShape');
        ...
        function conds = getCellConditionsFor(comparator,actVal,expVal)
            ...
            if ~comparator.haveSameSize(actVal,expVal)
                conds = ConstraintDiagnosticFactory.generateSizeMismatchDiagnostic(actVal,expVal);
            elseif comparator.IgnoreEmptyShape
                conds = MessageDiagnostic('MATLAB:unittest:StringComparator:CellstrsEqualIgnoringEmptyShape');
            else
        ...
        function bool = haveSameSize(comparator,actVal,expVal)
            bool = comparator.satisfyEmptyRequirement(actVal,expVal) || ...
                isequal(size(actVal), size(expVal));
        end

        function bool = satisfyEmptyRequirement(comparator,actVal,expVal)
