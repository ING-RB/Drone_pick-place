classdef (SupportExtensionMethods) TestResult  < matlab.unittest.internal.BaseTestResult
    ...
    properties (Hidden,Transient, SetAccess = {?matlab.unittest.internal.TestRunData,?matlab.unittest.internal.TestRunStrategy,...
            ?matlab.unittest.internal.TestElementModifier})
        % ParallelTestRunStrategy needs to set these.
    ...
    methods (Hidden, Access={?matlab.unittest.internal.BaseTestResult, ...
        ...
        function obj = appendDetailsProperty(obj, propertyName, value, ~)
            ...
            for idx = 1:numel(obj)
                assert(~obj(idx).Finalized,message('MATLAB:unittest:TestResult:CannotModifyResultAfterFinalized'));
        ...
        function obj = replaceDetailsProperty(obj, propertyName, value, ~)
            for idx = 1:numel(obj)
                assert(~obj(idx).Finalized,message('MATLAB:unittest:TestResult:CannotModifyResultAfterFinalized'));
                obj(idx).Details.(propertyName) = value;
            end
    ...
    methods(Hidden, Access=protected)
        ...
        function text = getRerunHyperlink(result,outputVarName)
            ...
            key = base64Encode(BasicDigester("SHA224").computeDigest(allFailureIdentifiers));

            text = [' (' sprintf('<a href ="matlab:matlab.unittest.internal.rerunFailedTests(''%s'',''%s'')">%s</a>', outputVarName,key,...
                getString(message('MATLAB:unittest:TestResult:Rerun'))) ,')'];
        end
    end
