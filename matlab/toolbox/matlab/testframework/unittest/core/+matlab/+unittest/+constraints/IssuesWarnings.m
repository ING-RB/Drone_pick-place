classdef IssuesWarnings < matlab.unittest.internal.constraints.WarningQualificationConstraint & ...
    ...
    methods(Hidden,Sealed,Access=protected)
        function diag = getCasualConstraintDiagnosticFor(constraint,fullCasualMethodName,actual,~)
            import matlab.unittest.diagnostics.ConstraintDiagnostic;
            import matlab.unittest.internal.diagnostics.ConstraintDiagnosticFactory;
            import matlab.unittest.internal.diagnostics.DiagnosticSense;
            ...
            if ~constraint.isFunction(actual)
                diag = constraint.buildIsFunctionDiagnosticFor(actual);
                diag.applyAlias(fullCasualMethodName);
                return
            end
        ...
        function args = getInputArguments(constraint)
            args = {constraint.ExpectedWarnings};
            if constraint.Exact
                args = [args,{'Exactly',true}];
            end
            if constraint.RespectCount
            ...
            if constraint.RespectSet
                args = [args,{'RespectingSet',true}];
            end
            if constraint.Nargout > 0
                args = [args,{'WhenNargoutIs',constraint.Nargout}];
            end
        end
    end
    ...
    methods(Access=private)
        ...
        function diag = createWrongWarningOrderDiagnostic(constraint, actualThatMatchedExpectedSpecifications)
            ...
            if isempty(actualThatMatchedExpectedSpecifications)
                return;
            end
...
function warnings = validateExpectedWarnings(warnings)
    ...
    if isa(warnings,'message')
        validateattributes(warnings,{'message'}, {'nonempty'},'','ExpectedWarnings');
        % Ensure the messages are valid by calling getString
        arrayfun(@getString,warnings,'UniformOutput',false);
    else
        mustBeTextArray(warnings,'ExpectedWarnings');
        mustBeNonempty(warnings);
...
% LocalWords:  abc Evalc's sublist strs Unhit unittest fevalc strjoin
