classdef Throws < matlab.unittest.internal.constraints.FunctionHandleConstraint & ...
    ...
    methods(Hidden,Access=protected)
        ...
        function invoke(constraint,fcn)
            ...
            try
                ...
                % Validate message catalog for actual exception thrown
                try
                    if isempty(ex.cause)
                        ex.message;
                    else
    ...
    methods(Hidden,Sealed,Access=protected)
        function args = getInputArguments(constraint)
            args = {constraint.ExpectedException};
            if ~isempty(constraint.RequiredCauses)
                args = [args,{'CausedBy',constraint.RequiredCauses}];
            end
...
if isa(exception,'meta.class')
    ...
    mustBeTextScalar(exception,'ExpectedException');
...
% LocalWords:  maxlhs Diags Descs Unhit sz Attrs inp Func Formattable unittest
% LocalWords:  ismissing
