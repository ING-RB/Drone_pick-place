classdef FailOnWarningsPlugin < matlab.unittest.plugins.QualifyingPlugin & ...
    ...
    methods (Hidden, Access=protected)
        ...
        function setupSharedTestFixture(plugin, pluginData)
            ...
            c.addTeardown(@()plugin.stopLoggingWarningsInSharedTestFixtureSetup);
        ...
        function setupTestClass(plugin, pluginData)
            plugin.startLoggingWarnings;
            c = matlab.unittest.internal.Teardownable;
            c.addTeardown(@()plugin.stopLoggingWarningsInTestClassSetup);
    ...
    methods (Access=private)
        ...
        function initializeSharedFixtureWarningStackForNewSharedTestFixture(plugin)
            import matlab.internal.diagnostic.Warning;
            plugin.SharedTestFixtureWarningsStack = [{Warning.empty(1,0)}, plugin.SharedTestFixtureWarningsStack];
        end
...
import matlab.unittest.internal.isValidMessageID;
...
% LocalWords:  mynamespace Parallelizable Teardownable
