classdef SuiteCreationLiaison < handle
    ...
    properties (SetAccess=immutable)
        % ParentName - Name of the test content, including any namespaces.
        ParentName;
    end
    ...
    properties (Dependent, SetAccess=private)
        ...
        % SimpleParentName - Name of the test content without any namespace prefixes.
        SimpleParentName;
    ...
    methods (Access=private)
        ...
        function filename = resolveFilename(liaison)
            ...
            if any(exist(getFilenameFromParentName(liaison.ParentName), "file") == [2,6])
                impliedFilename = whichFile(liaison.ParentName);
                [~, impliedSimpleParentName] = fileparts(impliedFilename);
                if strcmp(impliedSimpleParentName, liaison.SimpleParentName)
                    filename = impliedFilename;
                end
            end
        end
    end
...
namespace = 'matlab.unittest.internal.services.namingconvention.located';
...
% LocalWords:  namingconvention cls Locator MCheck unittest locator
