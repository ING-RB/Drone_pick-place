classdef TAPTestFilePlugin < matlab.unittest.plugins.TAPPlugin
    ...
    methods(Access={?matlab.unittest.plugins.TAPPlugin})
        function plugin = TAPTestFilePlugin(stallFile, clock, varargin)
    ...
    methods (Access=protected)
        ...
        function runTestSuite(plugin, pluginData)
            ...
            yaml.datetime = plugin.timestamp;
    ...
    methods(Access=private)
        ...
        function captureTimestampFor(plugin, idx)
            [plugin.ClassData(idx).Timestamp] = deal(plugin.timestamp);
        end
        ...
        function ts = timestamp(plugin)
            ts = char(plugin.Clock("now", TimeZone="local", Format="yyyy-MM-dd HH:mm:ssZ"));
        end
    end
...
% LocalWords:  mynamespace yaml datetime sok Fieldname yyyy THH HH DFFF FFFE FFFF
