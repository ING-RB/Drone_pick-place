classdef LoggedDiagnosticEventRecord < matlab.unittest.internal.eventrecords.EventRecord
    ...
    methods
        function txt = getFormattedReport(eventRecord, formatter)
            txt = formatter.getLoggedDiagnosticEventReport(eventRecord);
        end
    end
    ...
    methods(Access=protected)
        function diagRecord = convertToDiagnosticRecord(varargin)
            ...
            diagRecord = LoggedDiagnosticRecord(varargin{:});
        end
    end

    methods(Hidden)
        function exception = getFormattedException(~, ~)
            % Does nothing as LoggeDiagnosticEvent is intended to record
            % diagnostics events and not exception or qualification events
    ...
    methods(Static)
        function eventRecord = fromEventData(eventData,eventScope,eventLocation,varargin)
            ...
            formattableDiagnosticResults = eventData.DiagnosticResultsStore.getFormattableResults(varargin{:});
