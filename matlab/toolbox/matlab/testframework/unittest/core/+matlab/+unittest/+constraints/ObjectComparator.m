classdef ObjectComparator < matlab.unittest.constraints.Comparator & ...
    ...
    methods(Hidden, Access=protected)
        ...
        function bool = containerSatisfiedBy(comparator, actVal, expVal)
            ...
            if isidentical(actVal, expVal) || areEqualUsingIsequalOrIsequaln(actVal, expVal)
                bool = true;
                return;
            end
        ...
        function conds = getContainerConditionsFor(comparator, actVal, expVal)
            ...
            if isidentical(actVal, expVal)
                conds = MessageDiagnostic("MATLAB:unittest:ObjectComparator:Identical");
                
            elseif areEqualUsingIsequalOrIsequaln(actVal, expVal)
...
if ~isempty(mc)
    ...
    % Fall back to 'methods' to support java, udd, and oops objects
    className = builtin('class',obj);
    bool = ~isMethod(className,"isequal") || isMethod(className,"isequaln");
end
end

function bool = isMethod(className, methodName)
bool = any(string(methods(className)) == methodName);
end
...
% LocalWords:  Elementwise Equaln TOLOBJ conds unittest isequaln isidentical
