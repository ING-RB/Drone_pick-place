classdef EventRecord < handle & matlab.mixin.Heterogeneous
    ...
    methods(Abstract,Hidden)
        exception = getFormattedException(eventRecord, currentIndex);
    end
    ...
    methods(Sealed)
        function diagRecords = toDiagnosticRecord(eventRecords,varargin)
            import matlab.unittest.plugins.diagnosticrecord.DiagnosticRecord;
            diagRecordsCell = arrayfun(@(record) convertToDiagnosticRecord(record,varargin{:}),...
