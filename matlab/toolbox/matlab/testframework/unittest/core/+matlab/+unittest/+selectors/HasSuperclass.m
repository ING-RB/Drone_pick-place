classdef (Sealed) HasSuperclass < matlab.unittest.internal.selectors.SingleAttributeSelector
    ...
    properties (Constant, Hidden, Access=protected)
        AttributeClassName = "matlab.unittest.internal.selectors.SuperclassAttribute";
        AttributeAcceptMethodName = "acceptsSuperclass";
    end
    
    methods
        function selector = HasSuperclass(className)
            % HasSuperClass - Class constructor
            %
            ...
            mustBeTextScalar(className,'class');
            mustContainCharacters(className,'class');
            selector.Constraint = IsSupersetOf(char(className));
        end
    end
end
