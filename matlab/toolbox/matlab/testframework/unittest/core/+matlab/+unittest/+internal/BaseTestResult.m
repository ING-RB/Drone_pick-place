classdef (Hidden) BaseTestResult < matlab.mixin.CustomDisplay
    ...
    methods
        function t = table(result)
         ...
         function resultArray = assertSuccess(resultArray)
             % assertSuccess - Assert test session runs without failure
             %
             % TESTRESULT = assertSuccess(TESTRESULT) asserts that none of the tests
             % corresponding to the TESTRESULT array fail and returns TESTRESULT if the
             % assertion passes. TESTRESULT is an array of matlab.unittest.TestResult
             % objects.
             %
             %
             %   Examples:
             %
             %       % Run a suite of tests and assert that none of the tests fail                    
             %       result = assertSuccess(runtests('someTestFolder'));
             %
    ...
    methods (Abstract, Hidden, Access={?matlab.unittest.internal.BaseTestResult, ...
        ...
        result = appendDetailsProperty(result, propertyName, value, childIndex);
        result = replaceDetailsProperty(result, propertyName, value, childIndex)
    end
