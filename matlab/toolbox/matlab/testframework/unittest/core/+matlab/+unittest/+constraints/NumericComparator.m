classdef NumericComparator < matlab.unittest.constraints.Comparator & ...
    ...
    methods(Hidden, Access=protected)
        ...
        function bool = containerSatisfiedBy(comparator, actVal, expVal)
            bool = haveSameClass(actVal,expVal) && ...
                comparator.sizeCheckPasses(actVal,expVal) && ...
                haveSameSparsity(actVal,expVal) && ...
                comparator.equalityCheckPasses(actVal,expVal);
        end
        ...
        function conds = getContainerConditionsFor(comparator, actVal, expVal)
            ...
            elseif ~comparator.sizeCheckPasses(actVal,expVal)
            ...
            elseif haveSameComplexityAndValue(comparator,actVal,expVal)
                if ~haveSameSize(actVal,expVal)                    
                    conds = MessageDiagnostic('MATLAB:unittest:NumericComparator:EqualIgnoringEmptyShapes');
                elseif ~haveSameComplexity(actVal,expVal) 
                    conds = MessageDiagnostic('MATLAB:unittest:NumericComparator:EqualIgnoringComplexity');
                else
                    conds = MessageDiagnostic('MATLAB:unittest:NumericComparator:Equaln');
                end

            elseif ~comparator.toleranceIsSupportedBy(expVal)
                if  ~complexityCheckPasses(comparator,actVal,expVal)
    ...
    methods(Access=private)
        ...
        function bool = equalityCheckPasses(comparator,actVal,expVal)
            bool = comparator.haveSameComplexityAndValue(actVal,expVal) || ...
                comparator.toleranceIsSatisfiedBy(actVal,expVal);
        end
        
        function bool = haveSameComplexityAndValue(comparator,actVal,expVal)
            bool = comparator.complexityCheckPasses(actVal,expVal) && comparator.valueCheckPasses(actVal,expVal);
        end
        
        function bool = complexityCheckPasses(comparator,actVal,expVal)
            bool = true;
            if ~comparator.IgnoreComplexity
                bool = haveSameComplexity(actVal,expVal);
            end
        end
        
        function bool = valueCheckPasses(comparator,actVal,expVal)
            bool = ignoreEmpties(comparator,actVal,expVal) || isequaln(actVal, expVal);
        end

        function bool = sizeCheckPasses(comparator,actVal,expVal)
            bool = ignoreEmpties(comparator,actVal,expVal) || haveSameSize(actVal,expVal);
        end

        function bool = ignoreEmpties(comparator,actVal,expVal)
