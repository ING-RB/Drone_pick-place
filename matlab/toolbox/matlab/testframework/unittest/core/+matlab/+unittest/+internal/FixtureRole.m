classdef FixtureRole < matlab.mixin.Heterogeneous
    ...
    methods (Abstract)
        role = constructFixture(role, callback, locationProvider);
    ...
    methods (Sealed)
        ...
        function [doesFixtureNeedToBeReset,fixtureIdx] = getFixtureResetStatus(activeFixtures)
            doesFixtureNeedToBeReset = false;
            count = numel(activeFixtures);
            for fixtureIdx = 1:count
                if(activeFixtures(fixtureIdx).IsSetUpByRunner && ...
                        activeFixtures(fixtureIdx).IsUserVisible)
                    doesFixtureNeedToBeReset = needsReset_(activeFixtures(fixtureIdx).Instance);
                    if(doesFixtureNeedToBeReset)
                        return;
                    end
                end
            end
    ...
    methods (Sealed, Access=private)
        ...
        function roles = orderAccordingToFolderScope(roles)
            ...
            mask = ~isOnPath(fixtures);
            scopes(mask) = FolderScope.Within;
