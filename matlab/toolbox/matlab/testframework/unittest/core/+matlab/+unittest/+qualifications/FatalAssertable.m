classdef (SupportExtensionMethods) FatalAssertable < matlab.mixin.Copyable & ...
        matlab.unittest.internal.DiagnosticDataMixin & ...
        matlab.unittest.internal.qualifications.PostFailureCallbackProvider
    ...
    %       fatalAssertWarning - Fatally assert a function issues specific warning(s)
    ...
    methods(Sealed)
        function fatalAssertFail(fatalAssertable, varargin)
            ...
            qualifyFail(fatalAssertable.FatalAssertionDelegate, ...
                fatalAssertable, ...
                fatalAssertable.NotificationData, ...
                varargin{:});
        end
        ...
        function fatalAssertEqual(fatalAssertable, actual, expected, varargin)
            ...
            %       testCase.fatalAssertEqual(1.5, 2, ...
            %           'Difference between actual and expected must fall within relative tolerance', ...
            %           'RelTol', 0.1)
            %
            %
            %   See also
        ...
        function fatalAssertNotSameHandle(fatalAssertable, actual, notExpectedHandle, varargin)
            ...
            %   fatally asserts that ACTUAL is a different size and/or does not contain
            %   the same instances as the NOTEXPECTEDHANDLE handle array. This method
        ...
        function varargout = fatalAssertWarning(fatalAssertable, actual, warningID, varargin)
            ...
            %   that ACTUAL is a function handle that issues one or more warnings with
            %   the warning identifiers specified by WARNINGID. Warning identifiers can
            %   be defined as a character vector, a cell array of character vectors, or
            %   a string array. The fatalAssertWarning function call will ignore any other 
            %   warnings that may also be issued by the function handle, and only confirms 
            %   that the warning(s) specified, are issued at least once. 
            %
            %   Please note this method is a convenience method. There exists more
            ...
            %        end
            %
            %        testCase.fatalAssertWarning(@() warning('dummy:id','message'), {'dummy:id'})
            %
            %        testCase.fatalAssertWarning(@() warning("dummy:id","message"), "dummy:id")
            %
            %       % Failing scenarios
        ...
        function fatalAssertGreaterThan(fatalAssertable, actual, floor, varargin)
            ...
            %   The sizes of ACTUAL and FLOOR must be the same or be compatible. 
            %   This method is functionally equivalent to:
            %
            %       import matlab.unittest.constraints.IsGreaterThan;
        ...
        function fatalAssertGreaterThanOrEqual(fatalAssertable, actual, floor, varargin)
            ...
            %   the elements of FLOOR. The sizes of ACTUAL and FLOOR must be the same 
            %   or be compatible. This method is functionally equivalent to:
            %
        ...
        function fatalAssertLessThan(fatalAssertable, actual, ceiling, varargin)
            ...
            %   The sizes of ACTUAL and CEILING must be the same or be compatible. 
            %   This method is functionally equivalent to:
            %
        ...
        function fatalAssertLessThanOrEqual(fatalAssertable, actual, ceiling, varargin)
            ...
            %   elements of CEILING. The sizes of ACTUAL and CEILING must be the same 
            %   or be compatible. This method is functionally equivalent to:
            %
...
function notificationData = getNotificationData
notificationData = struct( ...
    "HasPassedListener",@(fatalAssertable)event.hasListener(fatalAssertable, "FatalAssertionPassed"), ...
    "NotifyPassed",@(fatalAssertable,evd)fatalAssertable.notify("FatalAssertionPassed", evd), ...
    "NotifyFailed",@(fatalAssertable,evd)fatalAssertable.notify("FatalAssertionFailed", evd), ...
    "OnFailureDiagnostics",@(fatalAssertable)fatalAssertable.FatalAssertionOnFailureTasks.getDefaultQualificationDiagnostics,...
    "DiagnosticData",@(fatalAssertable)fatalAssertable.DiagnosticData);
end
