classdef (SupportExtensionMethods) Assertable < matlab.mixin.Copyable & ...
    ...
    %       assertWarning - Assert a function issues specific warning(s)
    ...
    methods(Sealed)
        ...
        function assertNotSameHandle(assertable, actual, notExpectedHandle, varargin)
            ...
            %   ACTUAL is a different size and/or does not contain the same instances
            %   as the NOTEXPECTEDHANDLE handle array. This method is functionally
            %   equivalent to:
            %
            %       import matlab.unittest.constraints.IsSameHandleAs;
        ...
        function varargout = assertWarning(assertable, actual, warningID, varargin)
            ...
            %   function handle that issues one or more warnings with the waring
            %   identifiers specified by WARNINGID. Warning identifiers can be defined
            %   as a character vector, a cell array of character vectors, or a string
            %   array. The assertWarning function call will ignore any other warnings 
            %   that may also be issued by the function handle, and only confirms that the
            %   warning(s) specified, are issued at least once.
            %
            %   Please note this method is a convenience method. There exists more
            ...
            %        end
            %
            %        testCase.assertWarning(@() warning('dummy:id','message'), {'dummy:id'})
            %
            %        testCase.assertWarning(@() warning("dummy:id","message"), "dummy:id")
            %
            %
        ...
        function assertGreaterThan(assertable, actual, floor, varargin)
            ...
            %   of ACTUAL are greater than all the elements of FLOOR. The sizes of ACTUAL and 
            %   FLOOR must be the same or be compatible. 
            %   This method is functionally equivalent to:
            %
            %       import matlab.unittest.constraints.IsGreaterThan;
        ...
        function assertGreaterThanOrEqual(assertable, actual, floor, varargin)
            ...
            %   FLOOR. The sizes of ACTUAL and FLOOR must be the same or be compatible. 
            %   This method is functionally equivalent to:
            %
        ...
        function assertLessThan(assertable, actual, ceiling, varargin)
            ...
            %   of ACTUAL are less than all the elements of CEILING. The sizes of ACTUAL and 
            %   CEILING must be the same or be compatible. 
            %   This method is functionally equivalent to:
            %
        ...
        function assertLessThanOrEqual(assertable, actual, ceiling, varargin)
            ...
            %   CEILING. The sizes of ACTUAL and CEILING must be the same or be compatible.
            %   This method is functionally equivalent to:
            %
...
notificationData = struct( ...
    "HasPassedListener",@(assertable)event.hasListener(assertable, "AssertionPassed"), ...
    "NotifyPassed",@(assertable,evd)assertable.notify("AssertionPassed", evd), ...
    "NotifyFailed",@(assertable,evd)assertable.notify("AssertionFailed", evd),...
    "OnFailureDiagnostics",@(assertable)assertable.AssertionOnFailureTasks.getDefaultQualificationDiagnostics,...
    "DiagnosticData",@(assertable)assertable.DiagnosticData);
end
