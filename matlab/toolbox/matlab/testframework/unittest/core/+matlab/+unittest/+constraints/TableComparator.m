    ...
    methods(Hidden, Access=protected)
        ...
        function subComparisons = getElementComparisons(comparator,comparison)
            ...
            if comparison.IsUsingValueReference
                varNameToRef = @(varName) generateVarNameToRef(comparison.ValueReference,varName);
...
function varNameToRef = generateVarNameToRef(valueReference, variableName)
format = "%s.%s";

if  ~(isvarname(variableName) || iskeyword(variableName))
    variableName = strrep(variableName,'''','''''');
    format = "%s.('%s')";
end
varNameToRef = sprintf(format,valueReference,variableName);
end
% LocalWords:  unittest COMPOBJ conds Elems
