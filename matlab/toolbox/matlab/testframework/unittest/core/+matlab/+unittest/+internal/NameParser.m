classdef NameParser < handle
    ...
    properties (Constant, Hidden)
        ...
        % Regular expression to parse the Name. Parameter name allows the
        % following characters - 
        %   "@" for displaying function handles
        %   "." for displaying classnames or doubles
        %   "+", "-" for displaying higher or lower order numbers
        %   alphanumeric characters
        AllowedCharactersGroup = '[#=,\w|w*\@?\.?\-?\+?\w*]+\';

        RegularExpression = [ ...
            '^(?<ParentName>[^' ...
            matlab.unittest.internal.NameParser.ReservedCharactersRegularExpressionGroup ']+)' ...
            '(?<ClassParameters>\[' matlab.unittest.internal.NameParser.AllowedCharactersGroup '])?/' ...
            '(?<MethodParameters>\[' matlab.unittest.internal.NameParser.AllowedCharactersGroup '])?' ...
            '(?<TestName>[^' matlab.unittest.internal.NameParser.ReservedCharactersRegularExpressionGroup ']+)' ...
            '(?<TestParameters>\(' matlab.unittest.internal.NameParser.AllowedCharactersGroup '))?$'];
    end
    ...
    properties (SetAccess=private)
        ...
        ClassSetupParameters = struct('Property',{}, 'Name',{}, 'External',{});
        
        % MethodSetupParameters - Structure of MethodSetupParameter information
        MethodSetupParameters = struct('Property',{}, 'Name',{}, 'External',{});
        
        % TestMethodParameters - Structure of TestParameter information
        TestMethodParameters = struct('Property',{}, 'Name',{}, 'External',{});
    end
    ...
    methods
        function parser = NameParser(name)
            ...
            mustBeTextScalar(name,'name');
            mustContainCharacters(name,'name');
    ...
    methods (Access=private)
        function parameterStruct = parseParameters(parser, paramStr)
            ...
            parameterCell = regexp(parameters, '^(?<Property>\w+)=(?<Name>[\w+|w*\@?\.?\-?\+?\w*]+)(?<External>#ext)?$', 'names');
            parser.Valid = parser.Valid && (isempty(paramStr) || ~any(cellfun(@isempty, parameterCell)));
            
            parameterStruct = [parameterCell{:}];
            
            ext = num2cell(~cellfun(@isempty, {parameterStruct.External}));
            [parameterStruct.External] = ext{:};
        end
    end
end
