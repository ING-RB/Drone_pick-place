classdef ConstraintDiagnosticFactory
    ...
    methods (Static, Access = private)
        ...
        function diag = constructUnsupportedFailureTableDiagnostic(tolTypes, tolVals)
            import matlab.unittest.diagnostics.ConstraintDiagnostic;
            import matlab.unittest.internal.diagnostics.ConstraintDiagnosticFactory;

            diag = ConstraintDiagnostic;
            catalog = ConstraintDiagnosticFactory.MessageCatalog;
            diag.Description = catalog.getString("ToleranceInformation");
            diag.DisplayDescription = ~isempty(tolTypes);
            diag.DisplayConditions = true;
            for idx = 1:numel(tolTypes)
                subDiag = ConstraintDiagnostic;
                subDiag.DisplayActVal = true;
                subDiag.ActValHeader = tolTypes{idx};
                subDiag.ActVal = tolVals{idx};
    ...
    methods (Static, Hidden)
        ...
        function diag = generateFailureTableDiagnostic(failedIndices, actVal, expVal, varargin)
            ...
            if ~TableDiagnostic.supports(expVal)
                diag = ConstraintDiagnosticFactory.constructUnsupportedFailureTableDiagnostic(tolTypes, tolVals);
                return;
            end
...
requirementStr = createClassNameForCommandWindow(class(requirement));
description = createPassFailStatementString(requirementStr,isSatisfied,consType);
end
