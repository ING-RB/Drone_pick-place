classdef (SupportExtensionMethods) Assumable < matlab.mixin.Copyable & ...
    ...
    %       assumeWarning - Assume a function issues specific warning(s)
    ...
    methods(Sealed)
        ...
        function varargout = assumeWarning(assumable, actual, warningID, varargin)
            ...
            %   function handle that issues one or more warnings with the warning
            ...
            %   array. The assumeWarning function call will ignore any other warnings 
            ...
            %        end
            %
            %        testCase.assumeWarning(@() warning('dummy:id','message'), {'dummy:id'})
            %
            %        testCase.assumeWarning(@() warning("dummy:id","message"), "dummy:id")
            %
            %       % Failing scenarios
        ...
        function assumeLessThan(assumable, actual, ceiling, varargin)
            ...
            %   ACTUAL are less than all the elements of CEILING. The sizes of ACTUAL and 
...
notificationData = struct( ...
    "HasPassedListener",@(assumable)event.hasListener(assumable, "AssumptionPassed"), ...
    "NotifyPassed",@(assumable,evd)assumable.notify("AssumptionPassed", evd), ...
    "NotifyFailed",@(assumable,evd)assumable.notify("AssumptionFailed", evd),...
    "OnFailureDiagnostics",@(assumable)assumable.AssumptionOnFailureTasks.getAssumptionDiagnostics,...
    "DiagnosticData",@(assumable)assumable.DiagnosticData);
end
