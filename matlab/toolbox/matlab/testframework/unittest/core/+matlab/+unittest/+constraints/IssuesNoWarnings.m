    ...
    methods(Hidden,Sealed,Access=protected)
        function args = getInputArguments(constraint)
            if constraint.Nargout > 0
                args = {'WhenNargoutIs',constraint.Nargout};
            else
                args = {};
            end
        end
...
strs = arrayfun(@(alert)indentWithArrow(getIDAndMessageStrFrom(alert)), actualAlert);
str = char(join(strs, newline));
end

function str = getIDAndMessageStrFrom(warningAlert)
import matlab.unittest.internal.constraints.createAlertDisplayString;
str = createAlertDisplayString(warningAlert.identifier, warningAlert.message);
end
