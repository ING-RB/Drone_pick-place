classdef Eventually < matlab.unittest.internal.constraints.FunctionHandleConstraint
    ...
    methods
        ...
        function constraint = Eventually(anotherConstraint, varargin)
            ...
            p.addParameter('WithPollingIntervalOf',1, ...
                @(t) validateattributes(t, {'numeric'}, {'scalar', ...
                'nonnegative','<=',300}, '', 'pollingInterval'));
            p.parse(varargin{:});
            
            constraint.Timeout = p.Results.WithTimeoutOf;
            constraint.PollingInterval = p.Results.WithPollingIntervalOf;
        end
    ...
    methods(Access=private)
        function tf = eventuallySatisfiesInnerConstraint(constraint, fcn)
            ...
            while ~constraint.Passed && (constraint.ElapsedTime <= constraint.Timeout)
                ...
                if numAttempts >= 10
                    % Poll at a specified time interval (in seconds) specified
                    % via WithPollingIntervalOf between attempts to satisfy
                    % the constraint.
                    pause(constraint.PollingInterval);
                end
                constraint.updateWithAttempt(fcn, timerVal);
...
flushingNamespace = meta.package.fromName('matlab.unittest.internal.services.flushing');
flushingServiceClass = ?matlab.unittest.internal.services.flushing.FlushingService;
locator = ServiceLocator.forNamespace(flushingNamespace);
