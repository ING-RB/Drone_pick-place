classdef (SupportExtensionMethods) Verifiable < matlab.mixin.Copyable & ...
    ...
    %       verifyWarning - Verify a function issues specific warning(s)
    ...
    methods(Sealed)
        ...
        function verifyNotSameHandle(verifiable, actual, notExpectedHandle, varargin)
            ...
            %   that ACTUAL is a different size and/or does not contain the same
        ...
        function varargout = verifyWarning(verifiable, actual, warningID, varargin)
            ...
            %   array. The verifyWarning function call will ignore any other warnings 
            %   that may also be issued by the function handle, and only confirms that
            %   the warning(s) specified, are issued at least once.
            %
            %   Please note this method is a convenience method. There exists more
            ...
            %        end
            %
            %        testCase.verifyWarning(@() warning('dummy:id','message'), {'dummy:id'})
            %
            %        testCase.verifyWarning(@() warning("dummy:id","message"), "dummy:id")
            %
            %
        ...
        function verifyGreaterThan(verifiable, actual, floor, varargin)
            ...
            %   ACTUAL are greater than all the elements of FLOOR. The sizes of ACTUAL and 
...
notificationData = struct( ...
    "HasPassedListener",@(verifiable)event.hasListener(verifiable, "VerificationPassed"), ...
    "NotifyPassed",@(verifiable,evd)verifiable.notify("VerificationPassed", evd), ...
    "NotifyFailed",@(verifiable,evd)verifiable.notify("VerificationFailed", evd), ...
    "OnFailureDiagnostics",@(verifiable)verifiable.VerificationOnFailureTasks.getVerificationDiagnostics,...
    "DiagnosticData",@(verifiable)verifiable.DiagnosticData);
end
