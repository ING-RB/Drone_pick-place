    ...
    methods (Access=private)
        function printDiagnosticResults(plugin, eventData)
            ...
            for idx = 1:numel(formattableStrings)
                header = wrapHeader(plugin.MessageCatalog.getString('DiagnosticsValidationHeader'));
                result = concatenateIfNonempty(header, prependNewlineIfNonempty(formattableStrings(idx)));
                plugin.Printer.printFormatted(appendNewlineIfNonempty(prependNewlineIfNonempty(result)));
            end
        end
...
% LocalWords:  unittest mynamespace evd Formattable plugins formattable Parallelizable
