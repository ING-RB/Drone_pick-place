classdef UISingleSelectionStrategy < matlab.uiautomation.internal.UISelectionStrategy
    ...
    methods
        ...
        function indices = option2indices(strategy, indices)
            if ~strategy.isIndex(indices)
                text = convertCharsToStrings(indices);
                indices = [];
                if strategy.Validator.isValidTextInput(strategy, text)
                    indices = strategy.Validator.validateText(text, strategy.Options);
                end
            end
        end
    end
