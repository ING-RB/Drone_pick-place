...
%      float: double, single
%
%   See also MLDIVIDE, LSQMINNORM, LSQNONNEG, QR.
...
if isAlgSet
    ...
    if matlab.internal.math.partialMatch(alg,'chol')
        useChol = true;
    elseif matlab.internal.math.partialMatch(alg,'orth')
        useChol = false;
        if issparse(A) || issparse(b) || issparse(V)
...
elseif isequal(size(V),[nobs nobs])
    ...
    [T,Vperm,useChol] = factorcov(V, isAlgSet, useChol);
else
...
if useChol
    ...
    if issparse(A)
        [z,R,perm] = qr(A,b,'vector','econ');
    else
        [Q,R,perm] = qr(A,'vector','econ');
        z = Q'*b;
    end

    [absdiagR, normDiagR] = extractDiag(R);
    if issparse(R)
        % Use R to remove dependent columns in A (c.f. mldivide).
        keepCols = (absdiagR > 20.*normDiagR.*eps(class(R)).*(nobs+nvar));
    else
        % Use the rank-revealing QR to remove dependent columns of A.
        keepCols = (absdiagR > normDiagR.*max(nobs,nvar).*eps(class(R)));
    end
    rankA = nnz(keepCols);
    if rankA < nvar
    ...
    if nargout > 1
        ...
        if dfe > 0
            if issparse(A)
                nb = vecnorm(b, 2, 1);
                nz = vecnorm(z, 2, 1);
                mse = full((nb - nz) .* (nb + nz)) ./ dfe;
            else
                if rankA < nvar
                    Q = Q(:, keepCols);
                end
                res = b - Q*z;
                mse = vecnorm(res, 2, 1).^2 ./ dfe;
            end
        ...
        if nargout > 3
            ...
            stdx(perm,1:nrhs) = vecnorm(Rinv, 2, 2) * sqrt(mse); % an outer product if nrhs>1
        end
    end

else % ~useChol ('orth' case)
    % Use Paige's algorithm to avoid inverting V's Cholesky factor.

    [Q,R,perm] = qr(A, 'vector'); % returns the _full_ Q and R
    R((nvar+1):nobs,:) = []; % remove lower half (zeros)

    [absdiagR, normDiagR] = extractDiag(R);

    % Use the rank-revealing QR to remove dependent columns of A.
    rankA = nnz(absdiagR  > normDiagR.*max(nobs,nvar).*eps(class(R)));
    if rankA < nvar
        warning(message('MATLAB:lscov:RankDefDesignMat'));
        R = R(1:rankA,1:rankA);
        perm = perm(1:rankA);
    ...
    if zeroE
        ...
        feasible = all(err < vecnorm(e, 2, 1).*rankV.*eps(class(err)));
    end
    ...
    if nargout > 1
        % Compute the covariance matrix of the LS estimates, or just their
        % SDs. Fill in zeros corresponding to exact zero coefficients.
        C = R \ (Q' * T * (eye(rankV,outClass) - P*P'));
        if nargout > 3
            S = zeros(nvar,nvar,outClass);
            ...
            stdx = zeros(nvar,nrhs,outClass);
            stdx(perm,1:nrhs) = vecnorm(C, 2, 2) * sqrt(mse); % an outer product if nrhs>1
        end
    end
end
...
function [T,perm,useChol] = factorcov(V, isAlgSet,useChol)
...
%   If V is positive semidefinite, T is rectangular. Otherwise, an error is
%   thrown. If V is sparse, it must be positive definite.
%
%   [T,p,perm] = FACTORCOV(V), for V sparse, returns the permutation vector
%   that has been applied to V in the Cholesky factorization.

perm = [];

if useChol
    % Use Cholesky to factor V as T*T', with T lower triangular, if V is
    % positive definite.
    if issparse(V)
        [T,p,perm] = chol(V, 'vector', 'lower');
    else
        [T,p] = chol(V, 'lower');
    end
...
% considered positive.
tol = max(abs(D)) .* numel(D) .* eps(class(D));
...
poseigs = (D > tol);
T = U(:,poseigs) .* sqrt(D(poseigs)).';

if isAlgSet && useChol
    % It "orth" was chosen automatically, warn that the choice is being
    % ignored.
...
[Q,R,perm] = qr(T0,0);
[absdiagR, normDiagR] = extractDiag(R);
tol = normDiagR.*(max(m,n).^2).*eps(class(R));
rankT0 = nnz(absdiagR > tol);
...
if m > rankT0
    ...
    r = T0*e - b0;
    mse = vecnorm(r, 2, 1).^2 ./ (m-rankT0);
else
    mse = zeros(1,k,class(e));
end

function [d, normd] = extractDiag(X)
% [d, normd] = extractDiag(X) returns v, the diagonal in modulus of X and,
% and normd, the norm of the diagonal d.
...
normd = norm(d, 'inf');  % Return scalar for empties
