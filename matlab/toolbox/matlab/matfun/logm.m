...
% before calling Schur decomposition.
T = full(A);

% Check if T has the standard Schur form and 
schurInput = matlab.internal.math.isschur(T);
if ~schurInput
    % T is guaranteed to have only finite elements.
    [Q, T] = matlab.internal.math.nofinitecheck.schur(T);
end
stayReal = isreal(T);
