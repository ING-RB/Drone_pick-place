classdef (Sealed) decomposition < matlab.mixin.CustomDisplay & matlab.mixin.internal.Scalar
    ...
    methods
        function f = decomposition(A, varargin)
            ...
            [type, uplo, ...
                ...
                luSymPivot, ldlPivot, ranktol, iterRef, noSparseLDL] = parseInputs(varargin);

            switch type
                case 'auto'
                    [f.Underlying, type, f.ScaleFactor] = chooseDecomposition(A, ...
                        banddensity, mindegree, luPivot, luSymPivot, ldlPivot, ranktol, iterRef, noSparseLDL);
                case 'qr'
                    if issparse(A)
                ...
                case 'lu'
                    ...
                    if issparse(A)
                        f.Underlying = matlab.internal.decomposition.SparseLU(A, luPivot, luSymPivot, mindegree, iterRef);
                    else
                ...
                case 'ldl'
                    ...
                    if issparse(A)
                        ...
                        f.Underlying = matlab.internal.decomposition.SparseLDL(A, ldlPivot, mindegree, iterRef);
                    else
                ...
                case 'banded'
                    ...
                    if size(A, 1) ~= size(A, 2)  || (~support && ~isa(A,'double')) ...
                        || (support && ~isa(A,'double') && ~isa(A,'single'))
                        if support
                            error(message('MATLAB:decomposition:InvalidAForBandedFloat'));
                        else
                    ...
                    % Tridiagonal A
                    if kl==1 && ku==1 && issparse(A)
                        f.Underlying = matlab.internal.decomposition.Tridiagonal(A);
                    else
        ...
        function x = mldivide(f,b)
            ...
            if isa(b, 'decomposition')
                ...
                if f.IsSingle ~= isa(b, 'single')
                    support = matlab.internal.feature("SingleSparse");
                    if (issparse(b) || f.IsSparse) && ~support
    ...
    methods (Hidden)
        ...
        function t = keyMatch(~,~) %#ok<STOUT> 
            ...
            % Not supported for decomposition
            error(message("MATLAB:decomposition:InvalidTypeKeyMatch"));
        end

        function h = keyHash(~) %#ok<STOUT> 
            %KEYHASH Generates a hash code
            % Not supported for decomposition
            error(message("MATLAB:decomposition:InvalidTypeKeyHash"));
        end
    end
end

function [type, uplo, checkcond, banddensity, mindegree, ...
    luPivot, luSymPivot, ldlPivot, ranktol, iterRef, noSparseLDL] = parseInputs(args)
...
NameList = {'CheckCondition', 'BandDensity', 'LUPivotTolerance', 'LDLPivotTolerance', 'RankTolerance', 'AllowIterativeRefinement', 'NoSparseLDL'};
uploList = {'upper', 'lower'};
nrUndocNames = 2; % Last element in NameList is undocumented, do not show in error messages
argOffset = 0;
...
if nnz(indType) + nnz(indName) ~= 1
    ...
    validatestring(args{1}, [typeList NameList(1:end-nrUndocNames)]);
end
...
if nnz(indType) == 1
    ...
    if ismember(type, {'chol', 'ldl', 'triangular'})
        ...
        if nnz(indName) + nnz(indUpLo) ~= 1
            % Let validatestring give the error message
            validatestring(args{2}, [uploList NameList(1:end-nrUndocNames)]);
        end
...
if argOffset == numel(args)
    return;
end

for ii=1:2:(numel(args)-argOffset)
    ...
    if nnz(indName) ~= 1
        % Let validatestring give the error message
        validatestring(args{argOffset+ii}, NameList(1:end-nrUndocNames));
    else
        indName = find(indName);
    end
    
    if argOffset+ii+1 > numel(args)
    ...
    switch indName
        ...
        case 4 %'LDLPivotTolerance'
            validateattributes(value, {'numeric'}, {'scalar', 'real', '>=', 0, '<=', 0.5});
            ldlPivot = double(value);
            
        case 5 %'RankTolerance'
            validateattributes(value, {'numeric'}, {'scalar', 'real', 'nonnegative', 'nonnan'});
            ranktol = double(value);

        case 6 %'AllowIterativeRefinement'
            validateattributes(value, {'logical'}, {'scalar'});
            iterRef = value;
...
function [decA, type, scal] = chooseDecomposition(A, banddensity, mindegree, ...
    luPivot, luSymPivot, ldlPivot, ranktol, iterRef, noSparseLDL)
scal = 1;
if issparse(A)
    ...
    % Tridiagonal A (fast version of banded LU)
    if kl==1 && ku==1
    ...
    if kl == ku && ishermitian(A)
        ...
        if isreal(A) && matlab.internal.decomposition.builtin.sparseLDLsupported() && ~noSparseLDL
            try
                decA = matlab.internal.decomposition.SparseLDL(A, ldlPivot, mindegree, iterRef);
                type = 'ldl';
                return
            catch
    ...
    decA = matlab.internal.decomposition.SparseLU(A, luPivot, luSymPivot, mindegree, iterRef);
    type = 'lu';
    ...
    if (isreal(A) && size(A, 1) <= 16) || (~isreal(A) && size(A, 1) <= 8)
        decA = matlab.internal.decomposition.DenseLU(A);
        type = 'lu';
        return
    end
    
    % Hessenberg and tridiagonal cases
    if matlab.internal.decomposition.builtin.isUpperHessenberg(A)
        if matlab.internal.decomposition.builtin.isLowerHessenberg(A)
            decA = matlab.internal.decomposition.Banded(A, 1, 1);
            type = 'banded';
        else
    ...
    % Symmetric (positive/negative) definite A
    if ishermitian(A)
        % Symmetric positive definite A -> Cholesky decomposition
        d = diag(A);
        if all(d > 0)
