...
if nargin > 2 && ~isempty(options)
    ...
    if isfield(options,'TolBlk') && ~isempty(options.TolBlk)
        ...
        if delta <= 0
            error(message('MATLAB:funm:NegTolBlk'));
        end
    end
    ...
    if isfield(options,'TolTay') && ~isempty(options.TolTay)
        tol = options.TolTay;
        if tol <= 0
            error(message('MATLAB:funm:NegTolTay'));
        end
    end
...
if isdiag(T) % Handle special case of diagonal T.
...
for k = 1:maxterms
    ...
    if isinf(f)
        error(message('MATLAB:funm:InfDeriv'))
    end
    F_old = F;
    F = F + P*f;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function S = logm_pf(A,m,I)
%LOGM_PF   Pade approximation to matrix log by partial fraction expansion.
%   LOGM_PF(A,m,n) is an [m/m] Pade approximant to LOG(I+A),
