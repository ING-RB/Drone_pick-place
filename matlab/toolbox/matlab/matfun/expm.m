...
if s ~= 0
    T = T./(2.^s);
    % Tpowers represents powers of T, and expm_params only
    % populates powers 2, 4, and 6.  Therefore, we only need
    % to consider a re-scaling of these powers.
    % Moreover, Tpowers{2} is guaranteed to be finite, or s would be 0.
    Tpowers{2} = Tpowers{2} ./ (2.^(s.*2));
    for ii = 4:2:6
        if allfinite(Tpowers{ii})
            Tpowers{ii} = Tpowers{ii} ./ (2.^(s.*ii));
        else
            % Overflow in computing the original Tpower; need to
            % recalculate using rescaled values.
            Tpowers{ii} = Tpowers{ii-2} * Tpowers{2};
        end
    end
end
...
for j = 1:n-1
    switch block_struct(j)
        ...
        case 1
            ...
            if max(ave,df) < log(realmax(class(T)))
...
[s, m, Tpowers] = expm_params_basic(T);
if isa(T, 'single')
    % expm_params_basic can sometimes over-estimate scaling factor s
    % for single precision. Call expm_params_single to see if a small s
    % and m can be used.
    [s1, m1] = expm_params_single(T);
    if s1 <= s+2
        s = s1;
        m = m1;
    end
...
function [s, m, Tpowers] = expm_params_basic(T)
...
if isinf(s)
    ...
    s = s - (t == 0.5); % adjust s if normT/theta(end) is a power of 2.
end
m = 13;
end

function [s, m] = expm_params_single(T)
% for single precision T, we try to use fewer terms for performance
m_vals = [3 5 7];
% theta_m for m=1:7.
