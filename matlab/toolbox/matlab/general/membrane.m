...
arguments
    k  (1,1) {mustBeInteger, mustBePositive, validatekthEigenfunction} = 1
    m  (1,1) {mustBeInteger, mustBePositive} = 15
    n  (1,1) {mustBeInteger, mustBePositive} = min(m,9)
    np (1,1) {mustBeInteger, mustBePositive} = min(n,2)
end
...
function validatekthEigenfunction(x)
% only first 12 eigenfunction of the L-shaped membrane is supported.
if ~(x >= 1 && x <= 12)
