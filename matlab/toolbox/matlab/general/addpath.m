...
% Number of  input arguments
    ...
    if (n == 1 && isscalar(varargin{1}) && any(varargin{1} == pathsep))
        % handle deprecated input form
        p = catdirs(mfilename,varargin{1:n});
    else
        % Check for and resolve any package specifiers in input
        resolveddirs = resolveAnyPackages('installed',varargin{1:n});
        if isempty(resolveddirs)
            return
        end
        p = catdirs(mfilename,resolveddirs{:});
    end
