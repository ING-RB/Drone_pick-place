...
if fid ~= -1
    ...
    template = ['function p = pathdef', 10, ...
                ...
                '    [bp, ep] = matlab.internal.packages.getPathForPackagesInstalledAfter(filetimestamp);', 10, ...
                '    p = [bp pathsep userpath pathsep getenv(''MATLABPATH'') pathsep ...', 10, ...
                '        p pathsep ep];', 10, ...
                'else', 10, ...
...
% start of the entry. If we're on PC, we want to do our comparisons in a
% case-insensitive fashion.
if ispc
    mlr_dirs = cellfun(@(x) ismember(1,x),strfind(lower(dirnamesIn),lower(mlroot)));
else
    mlr_dirs = cellfun(@(x) ismember(1,x),strfind(dirnamesIn,mlroot));
end

% We'll need to wrap all the entries in strings, so do some quote escaping
dirnames = strrep(dirnamesIn, '''', '''''');
