...
args.className = 'matlab.ui.container.GridLayout';

args.functionName = 'uigridlayout';
...
args.userInputs = updatedInput;
try
    gridComponent = matlab.ui.control.internal.model.ComponentCreation.createComponent(args);
catch ex
    error('MATLAB:ui:GridLayout:unknownInput', ...
        ex.message);
end
    ...
    function tf = isPossibleParent(input)
        ...
        tf = isobject(input) && any(ishghandle(input));
    end
    ...
    function tf = isPossibleSize(x)
        ...
        % Specifically check for non-empty so that when [] is passed in as
        % first argument, it is treated as a parent, not a size. g2503851
        tf = (~isempty(x) && isnumeric(x)) || isNumericCell;
    end
