classdef (Hidden) PositionPropertiesComponentController < appdesservices.internal.interfaces.controller.AbstractControllerMixin
    ...
    methods (Static = true, Access='private')
        function viewPvPairs = getSizeLocationPropertiesForView(objModel, propertyNames)
            ...
            viewPvPairs = {};
            checkForProperties = ["InnerPosition", "Position_I", "OuterPosition"];
            propertiesPresent = ismemberForStringArrays(checkForProperties, propertyNames);

            if propertiesPresent(1) || propertiesPresent(2)
                ...
                size = objModel.Position(3:4);
                loc = objModel.Position(1:2);
            ...
            if propertiesPresent(3)
                size = objModel.OuterPosition(3:4);
                loc = objModel.OuterPosition(1:2);
                viewPvPairs = [viewPvPairs, ...
                    ... Send the size/location properties because the view only
                    ... knows how to work with those
                    {'OuterLocation', round(loc)},...
                    {'OuterSize', round(size)},...
                    ];
            end
    ...
    methods (Static = true)
        ...
        function viewPvPairs = getPositionUnitsPropertiesForView(objModel, propertyNames)
            ...
            if(ismemberForStringArrays("Units", propertyNames))
        ...
        function viewPvPairs = getPositionRestrictionPropertiesForView(objModel, propertyNames)
            import appdesservices.internal.util.ismemberForStringArrays;
            viewPvPairs = {};
            checkFor = ["AspectRatioLimits", "IsSizeFixed"];
