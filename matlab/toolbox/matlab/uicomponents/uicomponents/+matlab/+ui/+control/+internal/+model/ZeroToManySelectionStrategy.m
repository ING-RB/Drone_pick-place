classdef (Hidden) ZeroToManySelectionStrategy < matlab.ui.control.internal.model.StateComponentSelectionStrategy
    ...
    methods(Access = {  ?matlab.ui.control.internal.model.StateComponentSelectionStrategy, ...
        ...
        function calibrateSelectedIndexInTextSpace(obj, currentSelectedText)
            ...
            areElementPresent = matlab.ui.control.internal.model.PropertyHandling.areElementsPresent(currentSelectedText, obj.Component.PrivateItems);
        ...
        function newValue = validateValuePresentInItems(obj, newValue)
            ...
            if isempty(obj.Component.PrivateItems)
                ...
                areElementPresent = matlab.ui.control.internal.model.PropertyHandling.areElementsPresent(newValue, obj.Component.PrivateItems );
                ...
                % are char arrays), or newValue is the empty cell {}.
                % Verify that each element in newValue appears at max as
                % many times that the string appears in Items
                [isValid, ~] = matlab.ui.control.internal.model.PropertyHandling.validateSubset(newValue, obj.Component.PrivateItems);
                if(~isValid)
        ...
        function newValueData = validateValuePresentInItemsData(obj, newValueData)
            ...
            if(isempty(obj.Component.PrivateItems) && ~isValueEmpty)
                ...
                % ItemsData
                areElementsPresent = matlab.ui.control.internal.model.PropertyHandling.areElementsPresent(newValueData, itemsData);
                ...
                [isValid, ~] = matlab.ui.control.internal.model.PropertyHandling.validateSubset(newValueData, itemsData);
                if(~isValid)
                    messageObj = message('MATLAB:ui:components:notASubset', ...
                        'Value', 'ItemsData',  'ItemsData', 'Items');
        ...
        function value = validateValueIndex(obj, index)
            % Validate ValueIndex coming from public setter
            value = index;

            % Return on an empty ValueIndex []
            if isequal(value, [])
                return;
            end

            % If Items is empty, the Value should be {}
            itemsCount = numel(obj.Component.PrivateItems);
            
            % Throw an error message for an invalid ValueIndex
            if(~isnumeric(index) || isempty(index) || ~isvector(index) || any(rem(index, 1) ~=0) || any(index < 1) || any(index > itemsCount))
                messageObj = message('MATLAB:ui:components:invalidValueIndexZeroToMany', ...
                    'ValueIndex', 'Items');
                    
                % MnemonicField is last section of error id
                mnemonicField = 'invalidValueIndex';
            ...
            % Return ValueIndex as a row vector 
        ...
        function index = getIndexGivenSelectedText(obj, value)
            ...
            if ~iscell(value)
                % Assume that value represents one and only one element
            ...
            for k = 1:length(value)
                ...
                ind = matlab.ui.control.internal.model.PropertyHandling.findElementInVector(thisValue, remainingText);
        ...
        function index = getIndexGivenSelectedData(obj, valueData)
            ...
            if iscell(remainingTextData) && ~iscell(valueData)
                % Assume that valueData represents one and only one element
                % if the data and items data are not both cells
                valueData = {valueData};
            end
            for k = 1:length(valueData)
                ...
                % Find the corresponding text                
                ind = matlab.ui.control.internal.model.PropertyHandling.findElementInVector(thisValueData, remainingTextData);
        ...
        function index = getValueIndexGivenSelectedIndex(obj, index)
            % Return customer facing ValueIndex
            if isequal(index, -1)
                index = [];
            end
        end
