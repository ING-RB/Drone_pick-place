classdef (Sealed, ConstructOnLoad=true) DatePicker < ...
        ...
        matlab.ui.control.internal.model.mixin.PlaceholderComponent & ...s
    ...
    properties(Dependent, AbortSet)
        Value = NaT('Format', 'default');
        DisabledDates = datetime.empty();
        DisabledDaysOfWeek = [];
        Limits = [datetime(0000, 1, 1), datetime(9999, 12, 31)];
        Editable matlab.internal.datatype.matlab.graphics.datatype.on_off = 'on';
    end
    ...
    properties(Hidden)
        DisplayFormatMode matlab.internal.datatype.matlab.graphics.datatype.AutoManual = 'auto';
    end
    properties(Access = {?appdesservices.internal.interfaces.model.AbstractModel})
        ...
        PrivateValue = NaT('Format', 'default');
        PrivateDisplayFormat = 'dd-MMM-uuuu';
        PrivateDisabledDates = datetime.empty();
        PrivateDisabledDaysOfWeek = [];
        PrivateLimits = [datetime(0000, 1, 1, 'Format', 'default'), datetime(9999, 12, 31, 'Format', 'default')];
        PrivateEditable matlab.internal.datatype.matlab.graphics.datatype.on_off = 'on';
        PrivateDisplayFormatMode matlab.internal.datatype.matlab.graphics.datatype.AutoManual = 'auto';
    end
    ...
    methods
        function obj = DatePicker(varargin)
            ...
            % Do not remove above white space
            % Defaults
            defaultPosition = [100, 100, 150, 22];
        ...
        function set.DisplayFormat(obj, newValue)
            ...
            try
                value = matlab.ui.control.internal.model.PropertyHandling.validateText(newValue);

                % Capture information about whether value contains minutes
                originalValueContainedMinutes = obj.formatContainsMinutes(value);

                if originalValueContainedMinutes
                    value = obj.replaceMinutesIndicatorWithMonths(value);
                end

                % Remove time portion to avoid mm/MM warnings and time
                % related validation errors:
                value = matlab.internal.datetime.filterTimeIdentifiers(value);

                if isempty(value) && ~isempty(newValue)


                    % Value contained only time elements that were filtered
                    messageObj = message('MATLAB:ui:components:displayFormatWithOnlyTime', ...
                        'DisplayFormat');
                    ...
                    % Add Doc link
                    docLinkId = 'MATLAB:ui:components:datepickerDisplayFormatDocLink';
                    messageText = matlab.ui.control.internal.model.PropertyHandling.createMessageWithDocLink(messageText, docLinkId, 'DatePicker.DisplayFormat');

                    % Effectively this is an error state, the error will be
                    % thrown in the same path as other error states

                else
                    % Make sure that Format is valid.  Use new datetime object
                    % so that the component data is not corrupted
                    [~] = datetime('today', 'Format', value);
                end
            catch me
            ...
            % Property Setting
            obj.PrivateDisplayFormat = value;
                        ...
                        switch (me.identifier)
                            case 'MATLAB:datetime:dateshift:InvalidDOW'
                                ...
                                messageObj = message('MATLAB:ui:components:disabledDaysOfWeekDayNameInvalid', ...
                                    reshape(newValue{index}, 1, numel(newValue{index})));
                            case {'MATLAB:datetime:AmbiguousInput', 'MATLAB:datetime:dateshift:AmbiguousDOW'}
                                %'Ambiguous input: 'S'.'
