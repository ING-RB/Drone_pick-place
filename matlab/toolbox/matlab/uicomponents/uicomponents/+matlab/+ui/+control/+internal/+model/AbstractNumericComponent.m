classdef (Hidden) AbstractNumericComponent < ...
    ...
    properties(Dependent, AbortSet)
        ...
        LowerLimitInclusive matlab.internal.datatype.matlab.graphics.datatype.on_off = 'on';
        
        UpperLimitInclusive matlab.internal.datatype.matlab.graphics.datatype.on_off = 'on';
        
        RoundFractionalValues matlab.internal.datatype.matlab.graphics.datatype.on_off = 'off';
        
        ValueDisplayFormat = '%11.4g';

        AllowEmpty matlab.internal.datatype.matlab.graphics.datatype.on_off = 'off';
    ...
    properties(Access = {...
        ...
        PrivateLowerLimitInclusive matlab.internal.datatype.matlab.graphics.datatype.on_off = 'on';
        
        PrivateUpperLimitInclusive matlab.internal.datatype.matlab.graphics.datatype.on_off = 'on';
        
        PrivateRoundFractionalValues matlab.internal.datatype.matlab.graphics.datatype.on_off = 'off';
        
        PrivateValueDisplayFormat = '%11.4g';

        PrivateAllowEmpty matlab.internal.datatype.matlab.graphics.datatype.on_off = 'off';
    end
    ...
    methods
        function obj = AbstractNumericComponent(varargin)
            ...
            obj.doSetPrivateHorizontalAlignment('right');
        ...
        function set.Value(obj, newValue)
            ...
            if isequal(newValue, [])
                if obj.AllowEmpty
                    ...
                    messageObj = message('MATLAB:ui:components:invalidEmptyDoubleInput', 'Value', 'AllowEmpty', 'off');
                end
            else
            ...
            %
            % See g1173623
            if(isempty(messageObj) && strcmp(obj.PrivateRoundFractionalValues, 'on'))
            ...
            if (isempty(messageObj) && ~obj.isValueWithinLimits(newValue))
                messageObj = message('MATLAB:ui:components:valueNotInRange', ...
                    'Value', 'Limits');
        ...
        function set.AllowEmpty(obj, newAllowEmpty)
            
            % Error Checking done through the datatype specification

            % Property Setting
            obj.PrivateAllowEmpty = newAllowEmpty;
            
            if ~newAllowEmpty && isempty(obj.Value)

                obj.calibrate();

                % Update View
                markPropertiesDirty(obj, {'AllowEmpty', 'Value'});
            else
            
                % Update View
                markPropertiesDirty(obj, {'AllowEmpty'});
            end
        end
        
        function value = get.AllowEmpty(obj)
            value = obj.PrivateAllowEmpty;
        end
        ...
        function set.LowerLimitInclusive(obj, newIncludeLowerLimit)
            ...
            % Error Checking done through the datatype specification
            if ~newIncludeLowerLimit && isequal(obj.PrivateLimits(1), obj.PrivateLimits(2))
                messageObj = message('MATLAB:ui:components:invalidLimitInclusivity', 'LowerLimitInclusive', 'on', 'Limits');
                
                % Use string from object
                messageText = getString(messageObj);
                
                mnemonicField = 'invalidLowerLimitInclusive';
        ...
        function set.Limits(obj, newValue)
            % Error Checking
            rowVectorLimits = matlab.ui.control.internal.model.PropertyHandling.validateNonDecreasingLimitsInput(obj, newValue);
            
            % Limits must be inclusive when single value is accepted
            if isequal(rowVectorLimits(1), rowVectorLimits(2))
                obj.UpperLimitInclusive = true;
                obj.LowerLimitInclusive = true;
            end
        ...
        function set.UpperLimitInclusive(obj, newIncludeUpperLimit)
            ...
            % Validate special case of Limits being equal
            if ~newIncludeUpperLimit && isequal(obj.PrivateLimits(1), obj.PrivateLimits(2))
                messageObj = message('MATLAB:ui:components:invalidLimitInclusivity', 'UpperLimitInclusive', 'on', 'Limits');
                
                % Use string from object
                messageText = getString(messageObj);
                
                mnemonicField = 'invalidUpperLimitInclusive';
    ...
    methods(Access = 'private')
        ...
        function calibrateValue(obj)
            % This method is called after the limits, inclusions and
            % AllowEmpty are changed to update the current value such that
            % it is within the new limits
            %
            % Empty value is not valid when AllowEmpty is 'off'.
            % 
            ...
            if isempty(oldValue)
                if obj.AllowEmpty
                    ...
                    % When empty is invalid, prefer 0
                    oldValue = 0;
                    obj.PrivateValue = 0;
                end
            end
