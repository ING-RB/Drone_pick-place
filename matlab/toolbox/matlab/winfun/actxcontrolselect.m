...
%  ACTXCONTROLSELECT will be removed in a future release.
...
if feature("webui")
    error(message('MATLAB:actxcontrol:FunctionHasBeenRemoved','ACTXCONTROLSELECT'));
end
...
warning(message('MATLAB:ui:actxcontrolselect:FunctionToBeRemoved'));
...
set(handles.OKButton,'String',  getString(message('MATLAB:actxcontrol:selectCreateButton')));

if isempty(activexList)
    activexList = matlab.ui.internal.JavaMigrationTools.suppressedActXControllist;
end
...
if isequal(handles.User,'GUIDE')
    ...
    handles.helpmapfile = fullfile(docroot, '/matlab/helptargets.map');
...
prompt1 = getString(message('MATLAB:actxcontrol:selectControlPrompt1'));
prompt2 = getString(message('MATLAB:actxcontrol:selectControlPrompt2'));
...
if ~isempty(output)
    ...
    h=[];
    if ~isequal(handles.User, 'GUIDE')
        for i=1:size(handles.EventCallback,1)
        ...
        if ishandle(handles.Parent)
            h = matlab.ui.internal.JavaMigrationTools.suppressedActXControl(progid, handles.Position, handles.Parent, callback);    
        else
            h = matlab.ui.internal.JavaMigrationTools.suppressedActXControl(progid, handles.Position, gcf, callback);    
        end
    end
    output ={};
    output{1} = h;
...
% restore to single selection as long as the user selected an item
set(hObject,'max', 1);
...
progid = char(handles.controls{index, 2});
location = char(handles.controls{index, 3});
pos = getpixelposition(handles.PromptEdit);
...
control =[];
try
    control = matlab.ui.internal.JavaMigrationTools.suppressedActXControl(progid, pos, handles.TestFigure);
    delete(control);
    control = matlab.ui.internal.JavaMigrationTools.suppressedActXControl(progid, pos, handles.figure1);
    ...
    set(handles.PromptEdit, 'String', me.message);
end
