...
% Last Modified by GUIDE v2.5 28-Jan-2010 15:18:00
if feature("webui")
    error(message('MATLAB:actxcontrol:FunctionHasBeenRemoved','ACTXCONTROLPROPERTY'));
end
...
if nargin>3 && ishandle(varargin{1}) && strcmpi(get(varargin{1},'Type'), 'Figure') ...
    ...
    handles.EventTable = uitable('v0', handles.figure1);
    warning(orig_state);
    
    %%%% This functionality is currently not migratable; the container
    %%%% scroll pane is not available.
    ...
    %%%% Java-like syntax unavailable, use:
    %%%%   set(handles.EventTable, 'Position', pos);
...
figs = findobj(get(0,'Children'),'Type','Figure', 'HandleVisibility','on');

cf = getString(message('MATLAB:actxcontrol:createpropertyCurrentFiguregcf'));
cfholder = -1;
if (~isempty(figs))
    tags = Local_FigureTitle(figs);
    fighandles = [cfholder; figs(:)];
    fignames = {cf, tags{:}};    
else
    fighandles = cfholder;
    fignames = {cf};    
...
title = {getString(message('MATLAB:actxcontrol:createpropertyEvent')), getString(message('MATLAB:actxcontrol:createpropertyEventHandler'))};
data = handles.EventCallback;
if isempty(handles.EventCallback)
    data = {};    
end

%%%% Java-like syntax unavailable, instead use the following lines:
%%%%   set(handles.EventTable, 'Data', data);
%%%%   set(handles.EventTable, 'ColumnNames', title);
%%%%   set(handles.EventTable, 'ColumnEditable', [true false]);
handles.EventTable.setData(data);
handles.EventTable.setColumnNames(title);
handles.EventTable.setEditable(1, 0);

%%%% Java-like syntax unavailable, instead use:
%%%%   set(handles.EventTable, 'ColumnEditable', [false false]);
%%%%   or
%%%%   set(handles.EventTable, 'ColumnEditable', [false true]);
if isempty(handles.EventCallback)
    handles.EventTable.setEditable(2, 0);
else
    handles.EventTable.setEditable(2, 1);
end

%%%% Java-like syntax unavailable, use:
%%%%   pos = get(handles.EventTable, 'Position');
pos = handles.EventTable.getPosition;

%%%% ColumnWidth currently not available, not migratable.
...
handles = guidata(handles.figure1);
[filename, pathname] = uigetfile('*.m', getString(message('MATLAB:actxcontrol:createpropertyPickFile')));
if ~isequal(filename,0) &&  ~isequal(pathname,0)
    [p,n,e]=fileparts(filename);
    
    %%%% SelectedRows currently unavailable and not migratable.
