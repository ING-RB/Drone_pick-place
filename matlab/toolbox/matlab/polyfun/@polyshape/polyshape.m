...
% Copyright 2016-2024 The MathWorks, Inc.
    ...
    methods (Static, Hidden = true)
        ...
        % Redirect to codegen class
        function name = matlabCodegenRedirect(~)
            name = 'coder.internal.polyshape';
        end
    ...
    methods (Access = private)
        ...
        function [PG, shapeId, vertexId] = booleanFun(subject, clip, collinear, boolfun, simplify)
            ...
            if numel(clip) == 0 || numel(subject) == 0
                ...
                for i=1:numel(P2)
                    ...
                    [PG(i).Underlying, mapIdx] = ...
                        boolfun(P2(i).Underlying, Q2(i).Underlying, keepCollinear,simplify);
                    PG(i).SimplifyState = simplify;
        ...
        function [PG, shapeId, vertexId] = booleanVec(pshapes, collinear, callUnion,simplify)
            if ~isvector(pshapes)
            ...
            PG = polyshape();
            if callUnion
                [PG.Underlying, mapIdx] = matlab.internal.polygon.builtin.cpolygon.unionvec(u2n, keepCollinear,simplify);
            else
                [PG.Underlying, mapIdx] = matlab.internal.polygon.builtin.cpolygon.intersectvec(u2n, keepCollinear,simplify);
            end
            if PG.isEmptyShape()
                [PG,shapeId,vertexId]=addAttributesEmptyPolyShape(PG,keepCollinear);
            else
        ...
        function [has_clip, collinear, simplify] = parseIntersectUnionArgs(clipCanBeLine, varargin)
            ...
            if ninputs > 0
                ...
                for ia = next_inp:2:ninputs
                    ...
                    next_arg = varargin{ia+1};
                    if ~(ischar(this_arg) || isstring(this_arg))
                        error(message('MATLAB:polyshape:collinearSimplifyParameter'));
                    else
                        this_arg = char(this_arg);
                        if (strncmpi(this_arg, 'Simplify', max(2, length(this_arg))))
                            if isscalar(next_arg) && (islogical(next_arg) || isnumeric(next_arg))
