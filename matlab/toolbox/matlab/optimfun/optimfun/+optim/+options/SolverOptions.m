classdef (Abstract) SolverOptions < matlab.mixin.CustomDisplay
    ...
    methods (Hidden)
        ...
        function obj = SolverOptions(varargin)
            ...
            if nargin > 0
                ...
                % See comment about warning in catch clause
                warnstate = warning();
                ...
                for i = 1:2:length(pvPairs)
                    try
                        ...
                        % Disable warnings introduced for 24b for intlinprog
                        % deprecation. If we don't disable here, the
                        % warnings will display a second time in the
                        % scenario that ME is thrown and we reach the
                        % next line. This can be removed after 24b when the
                        % warning is gone.
                        warning('off','optim:options:Intlinprog:OptionDeprecation')
                        warning('off','optim:options:Intlinprog:AlgorithmDeprecation')
    ...
    methods (Hidden, Access = protected)
        ...
        function header = addExtraHeader(~, header) 
            %ADDEXTRAHEADER Add extra information to the display header
            %
            %   HEADER = ADDEXTRAHEADER(OBJ, HEADER) allows extra
            %   information to be added to the header when the options are
            %   displayed.
            %
            %   Can be customized by sub-classes.
        end
        ...
        function footer = addExtraFooter(~, footer)
            %ADDEXTRAFOOTER Add extra information to the display footer
            %
            %   FOOTER = ADDEXTRAFOOTER(OBJ, FOOTER) allows extra
            %   information to be added to the footer when the options are
            %   displayed.
            %
            %   Can be customized by sub-classes.
        end
    ...
    methods (Hidden)
        ...
        function obj = convertForSolver(obj, Solver)
            ...
            if ~isa(obj, objStr)
                ...
                convertOptsText = matlab.internal.optimfun.utils.addLink( ...
