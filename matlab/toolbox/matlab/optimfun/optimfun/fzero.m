...
if allOptionsDefault
    ...
    if nargin == 1
        ...
        if isa(FunFcnIn,'struct')
            [FunFcnIn,x,options] = matlab.internal.optimfun.utils.separateOptimStruct(FunFcnIn);
...
if ~allOptionsDefault
    ...
    flag = 'fast';
    % Check that options is a struct - the check earlier guarantees it's non-empty
    if ~isa(options,'struct')
        error('MATLAB:fzero:ArgNotStruct',...
            getString(message('MATLAB:optimfun:commonMessages:ArgNotStruct', 3)));
    end

    tol = optimget(options,'TolX',defaultopt,flag);
    funValCheck = strcmp(optimget(options,'FunValCheck',defaultopt,flag),'on');
    printtype = optimget(options,'Display',defaultopt,flag);
    switch printtype
    ...
    outputfcn = optimget(options,'OutputFcn',defaultopt,flag);
    ...
    plotfcns = optimget(options,'PlotFcns',defaultopt,flag);
...
if numel(x) == 2 % Interval input
    ...
    if ~(isscalar(fa) && isscalar(fb))
        error('MATLAB:fzero:InitialValueNotScalar',...
            getString(message('MATLAB:optimfun:fzero:InitialValueNotScalar')));
    end
    ...
    if ~isUnderlyingType(fa,'float') || ~isUnderlyingType(fb,'float')
