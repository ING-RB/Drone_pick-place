...
if nargin == 1
    if isstruct(funfcn)
        [funfcn,x,options] = matlab.internal.optimfun.utils.separateOptimStruct(funfcn);
...
f = funfcn(x,varargin{:});
if isempty(f)
    error(message('MATLAB:optimfun:commonMessages:UsrObjReturnsEmpty'));
end
if ~isUnderlyingType(f,'float')
