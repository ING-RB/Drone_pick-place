...
outputsRequested = nargout;

% If just 'defaults' passed in, return the default options in X
if nargin == 1 && outputsRequested <= 1 && strcmpi(C,'defaults')
    x = struct('Display','notify','TolX','10*eps*norm(C,1)*length(C)');
    return
end
...
if nargin == 1
    ...
    if isa(C,'struct')
        [C,d,options] = matlab.internal.optimfun.utils.separateOptimStruct(C);
...
% Get tolerance and verbosity level
[tol,verbosity] = processOptions(options,C);
...
lambda = w;
resnorm = resid'*resid;
if outputsRequested > 4 || verbosity > 1
    output.iterations = outeriter;
    output.algorithm = 'active-set';
...
%--------------------------------------------------------------------------
function [tol,verbosity] = processOptions(options,C)
% Check options and return tol and verbosity
...
if ~isempty(options)
    ...
    defaultopt.TolX = '10*eps*norm(c,1)*length(c)';
    ...
    if ischar(tol) || isStringScalar(tol)
