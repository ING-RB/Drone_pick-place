...
switch field
    ...
    case {'HessUpdate'}
        if ~iscell(value)
            % If character string, has to be bfgs, lbfgs, dfp or steepdesc
            [validvalue, errmsg, errid] = stringsType(field,value,{'bfgs';'lbfgs';'dfp';'steepdesc'});
        else
    ...
    case {'UseParallel'}
        ...
        [value,validvalue] = matlab.internal.optimfun.utils.validateUseParallel(value,false,true);
        if ~validvalue
    ...
    case {'Algorithm'}
        ...
        % 'dual-simplex-highs', trust-region, 'quasi-newton'
        if ~iscell(value)
...
valid = numel(value) == 1 && any(strcmpi(value{1},strings)) || numel(value) == 2 && ...
    any(strcmpi(value{1},strings)) && isreal(value{2}) && isscalar(value{2}) && value{2} > 0 && value{2} == floor(value{2});

if ~valid
    errid = 'MATLAB:optimoptioncheckfield:notAStringPosIntegerCellType';
    allstrings = formatCellArrayOfStrings(strings);
    errmsg = getString(message('MATLAB:optimfun:optimoptioncheckfield:notAStringPosIntegerCellType',field,allstrings));
else
    errid = '';
    errmsg = '';
end
...
% in all the correct places while building up the string of possible string values.
    ...
    for index = 2:(length(strings)-1)
        ...
        allstrings = [allstrings, ', ''', strings{index},''''];%#ok
    end
    if length(strings) > 2
