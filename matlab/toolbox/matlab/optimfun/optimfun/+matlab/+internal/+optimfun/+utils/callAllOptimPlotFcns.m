...
% Remove buttons - either the solve is over or the plot window is being
% closed. Therefore, there is no way the buttons can be useful from here on
% out. Mostly, this removes the buttons from lingering in the plot after
% the solve is done where someone may interpret them as if the solver is
% still running.
btns = fig.Children.findobj('Type','UIControl');
for k = 1:numel(btns)
    btns(k).Visible = matlab.lang.OnOffSwitchState.off;
end
