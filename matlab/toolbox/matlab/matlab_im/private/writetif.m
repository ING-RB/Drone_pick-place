...
matlab.internal.imagesci.wtifc(data, map, filename, writemode, colorspace, required_tags);
...
p = inputParser;
p.addParameter('compression',default_compression, ...
    @(x) validateattributes(x,{'char', 'string'},{'nonempty', 'scalartext'},'','COMPRESSION'));
p.addParameter('description','', ...
    @(x) validateattributes(x,{'char', 'string'},{'nonempty', 'scalartext'},'','DESCRIPTION'));
p.addParameter('resolution',72, ...
    @(x) validateattributes(x,{'double'},{'nonempty','vector'},'','RESOLUTION'));
p.addParameter('rowsperstrip',-1, ...
    @(x) validateattributes(x,{'numeric'},{'scalar','>=',1},'','ROWSPERSTRIP'));
p.addParameter('writemode','overwrite', ...
    @(x) validateattributes(x,{'char', 'string'},{'nonempty', 'scalartext'},'','MODE'));
p.addParameter('colorspace',default_colorspace, ...
    @(x) validateattributes(x,{'char', 'string'},{'nonempty', 'scalartext'},'','COLORSPACE'));
p.parse(varargin{:});
