function [X, map] = readpnm(filename, imageIndex, options)
...
%
%   [X, MAP] = READPNM(FILENAME,'AutoOrient',tf) same as above, AutoOrient 
%   value is ignored.
%
...
arguments
    ...
    % AutoOrient value is ignored for PNM images
    options.AutoOrient (1,1) {mustBeA(options.AutoOrient,'logical')} = false %#ok<INUSA> 
end
   
   fullImageInfo = impnminfo(filename);
   

   validateattributes(imageIndex, {'double'}, ...
       {'scalar', '>=', 1, '<=', numel(fullImageInfo)}, 'readpnm', 'idx');
   
   
   info = fullImageInfo(imageIndex);
