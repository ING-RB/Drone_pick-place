function [X, map, mask] = readico(filename, index, options)
...
%
%   [...] = READICO(FILENAME,...'AutoOrient',tf) same as above, AutoOrient 
%   value is ignored.
%
...
arguments
    ...
    % AutoOrient value is ignored for ICO images
...
if strcmp(info(index).CompressionType, 'png')
    ...
    infoPNG = matlab.internal.imagesci.pnginfoc(filename, false, double(info(index).ImageDataOffset));
    
    background = infoPNG.BackgroundColor;
    if isempty(infoPNG.BackgroundColor)
        background = 'none';
    end
    [X, map, mask] = readpngutil(filename, background, double(info(index).ImageDataOffset));
    return;
else
