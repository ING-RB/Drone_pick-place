...
validateattributes(X,{'logical','uint8','single','double'},{'real'},'','X');
...
% Generate grayscale colormap if map is empty and number of components is 1
...
fid=matlab.internal.fopen(filename,'W','l');
...
if biBitCount==1
  ...
  fclose(fid);
  fid=matlab.internal.fopen(filename,'A+','b');
  assert(fid~=-1, message('MATLAB:imagesci:imwrite:fileOpen',filename));
  fseek(fid,0,'eof');
  fwrite(fid,X,'ubit1');
else
