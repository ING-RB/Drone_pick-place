...
p.addRequired('filename',@(x)validateattributes(x,{'char', 'string'},{'nonempty', 'scalartext'}));

p.addParameter('writemode','overwrite', ...
    @(x) validateattributes(x,{'char', 'string'},{'nonempty', 'scalartext'},'','WRITEMODE'));
p.addParameter('comment','', ...
    @(x) validateattributes(x,{'char','cell', 'string'},{'nonempty'},'','COMMENT'));
p.addParameter('disposalmethod', 'donotspecify',...
    @(x) validateattributes(x,{'char', 'string'},{'nonempty', 'scalartext'},'','DISPOSALMETHOD'));
p.addParameter('delaytime',[], ...
    @(x) validateattributes(x,{'double'},{'scalar','>=',0,'<=',655}));
p.addParameter('transparentcolor',[], ...
    @(x) validateattributes(x,{'double'},{'integer','scalar'},'','TRANSPARENTCOLOR') );
p.addParameter('loopcount',[], ...
    @(x) isnumeric(x) && isscalar(x) && (isinf(x) || (x<=65535 && x>=0)));
p.addParameter('location',[0 0], ...
...
%check if the third dimension of input 4-D data is equal to 1
if (nd==4) && (size(mat,3) ~= 1)
    error(message('MATLAB:imagesci:writegif:indexedOnly'));
end 
...
if(isempty(map) && not(writemode==1))
    ...
    % Create the gray colormap as a three-column array
    t = (0:n-1)'/max(n-1,1);
    map = [t t t];
end
