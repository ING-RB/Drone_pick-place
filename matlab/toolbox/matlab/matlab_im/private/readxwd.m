function [M,CM] = readxwd(fname,options)
%READXWD Read image data from an XWD file.
%   [X,MAP] = READXWD(FILENAME) reads image data from a XWD file.
...
%    1-bit XYPixmaps
%
%   [X,MAP] = READXWD(FILENAME,'AutoOrient',tf) same as above, AutoOrient 
%   value is ignored.
%
...
arguments
    ...
    % AutoOrient value is ignored for XWD images
...
if (info.ByteOrder == 0)
    fid = matlab.internal.fopen(fname, 'r', 'ieee-le');
elseif (info.ByteOrder == 1)
    fid = matlab.internal.fopen(fname, 'r', 'ieee-be');
else
