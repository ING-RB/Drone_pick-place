...
fid = matlab.internal.fopen(filename, 'r', 'ieee-le');  % ICO files are little-endian
...
for p = 1:imcount
    ...
    if info(p).Width == 0 && info(p).Height == 0
        infoPNG = matlab.internal.imagesci.pnginfoc(filename, false, double(info(p).ResourceDataOffset));
