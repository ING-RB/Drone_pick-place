function [X,map] = readpcx(filename, options)
...
%
%   [X,MAP] = READPCX(FILENAME,'AutoOrient',tf) same as above, AutoOrient 
%   value is ignored.
%
...
arguments
    ...
    % AutoOrient value is ignored for PCX images
...
assert(status ~= -1, 'PCX header too small.');
buffer = fread(fid,'*uint8');
fclose(fid);

scanLineLength = info.NumColorPlanes * info.BytesPerLine;

[X, endIndex] = matlab.internal.imagesci.pcxdrle(buffer, info.Height, scanLineLength);
X = X';
...
case 1
    ...
    case 1
        ...
        for k = 1:8
            X(:,k:8:end) = matlab.internal.imagesci.bitslice(XX, 8-k+1, 8-k+1);
        end
    ...
    otherwise
        error(message('MATLAB:imagesci:readpcx:badBitDepthNumPlanesCombination', ...
...
case 4
    ...
    for k = 1:8
        X(:,k:8:end) = matlab.internal.imagesci.bitslice(tmp, 8-k+1, 8-k+1);
    end
