...
function [info, colorTableType] = imgifinfo(filename)
...
while (separator ~= 59)
   ...
   elseif (separator == 44)
      ...
      if LCTbool
         colorTableType(k) = matlab.io.internal.imagesci.ColorTableType.Local;
      ...
      elseif GCTbool
         colorTableType(k) = matlab.io.internal.imagesci.ColorTableType.Global;
         % Use the Global Color Table
         info(k).BitDepth = bitdepth;
         info(k).ColorTable = GlobalColorTable;   
      
      else
         colorTableType(k) = matlab.io.internal.imagesci.ColorTableType.Default;
