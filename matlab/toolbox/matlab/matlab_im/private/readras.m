function [X, map, alpha] = readras(filename,options)
...
%
%   [X,MAP,ALPHA] = READRAS(FILENAME,'AutoOrient',tf) same as above,
...
arguments
    ...
    % AutoOrient value is ignored for RAS images
   ...
   %
   fid = matlab.internal.fopen(filename, 'rb', 'ieee-be');
   if fid < 0
   ...
   switch info.Type
      ...
      case RT_FORMAT_TIFF
         error(message('MATLAB:imagesci:readras:unsupportedRasType', 'RT_FORMAT_TIFF'));
      case RT_FORMAT_IFF
         error(message('MATLAB:imagesci:readras:unsupportedRasType', 'RT_FORMAT_IFF'));
      case RT_EXPERIMENTAL
         error(message('MATLAB:imagesci:readras:unsupportedRasType', 'RT_EXPERIMENTAL'));
      otherwise
