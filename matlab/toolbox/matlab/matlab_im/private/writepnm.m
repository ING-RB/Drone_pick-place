...
%  Copyright 2001-2020 The MathWorks, Inc.
   ...
   p = inputParser;
   p.addParameter('maxvalue',maxval, ...
       @(x) validateattributes(x,{'numeric'},{'scalar','integer','>=',1},'','MAXVALUE'));
   p.addParameter('encoding','raw', ...
       @(x) validateattributes(x,{'char', 'string'},{'nonempty', 'scalartext'},'','ENCODING'));
   p.parse(varargin{:});
...
function write_raw_data(filename, format, data, height, width, maxval)
   ...
   fid = matlab.internal.fopen(filename, 'W', 'ieee-be');
...
function write_plain_data(filename, format, data, height, width, maxval)
   ...
   fid = matlab.internal.fopen(filename, 'W');
   if (fid < 0)
