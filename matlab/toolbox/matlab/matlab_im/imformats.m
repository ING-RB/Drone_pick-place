...
fmts(end).write = '';
fmts(end).alpha = 0;
fmts(end).description = 'Aperio ScanScope Virtual Slide';
...
if isMATLABDesktop
    fmts(end + 1).ext = {'heif','heic','heifs','heics'};
    fmts(end).isa = @isheif;
    fmts(end).info = @imheifinfo;
    fmts(end).read = @readheif;
    fmts(end).write = '';
    fmts(end).alpha = 1;
    fmts(end).description = 'High Efficiency Image File/Container (HEIF/HEIC)';
end
...
format_values = matlab.io.internal.imagesci.convertStringsInStruct(format_values);
...
new_values = matlab.io.internal.imagesci.convertStringsInStruct(new_values);
