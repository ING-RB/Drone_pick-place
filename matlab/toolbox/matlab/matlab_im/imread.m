...
%       --------------   -----
%       'AutoOrient'     A logical value. If true, the returned image is 
%                        transformed (rotated and/or mirrored) to be 
...
try
    ...
    errorID = MEDownload.identifier;    
    errorID = replace(errorID, 'getFileFromURL', 'imread');
    throw(MException(errorID, '%s', MEDownload.message));
end
...
if isempty(fmt_s)
    ...
    try
        ...
        % If the 'filename' input is not a file
    ...
    if (~isempty(cached_fmt)) && ismember(ext(2:end), cached_fmt.ext)
        ...
        try
            ...
            if strcmpi(ME.identifier, 'MATLAB:imagesci:imread:fileFormat') && ...
                    matlab.io.internal.vfs.validators.hasIriPrefix(source) && ...
                ...
                % login page.
                error(message('MATLAB:imagesci:imread:readHTML', source));
            else
                rethrow(ME);
            end
        end
...
try
    ...
    if matlab.io.internal.vfs.validators.hasIriPrefix(source) && ...
            any(matlab.io.internal.vfs.validators.GetScheme(source) == ["http", "https"]) && ...
            contains(matlab.io.internal.filesystem.getContentType(source), "text/html", IgnoreCase=true)
        % if we are reading a remote file from an HTTP link and the
        % content type is HTML, this error is because we are trying to read
        % HTML file as any of supported format. This can happen when authentication is
    ...
    function varargout = call_format_specific_reader()
        try
            ...
            if was_cached_fmt_used && ~was_format_specified
                % Try again using format determined from filename
                fmt_s = get_format_info(fullname);
                needToThrow = false; % read might still succeed
                try
                    [varargout{1:nargout}] = feval(fmt_s.read, fullname, extraArgs{:});
                catch MERead2
                    MERead = MERead2;
                    needToThrow = true; % read still failed
                end
            end

            if needToThrow
                % preprocess the exception
                if isUrl % make sure the error has correct file name
                    msgtext = replace(MERead.message, fullname, source);
                    MERead = MException(MERead.identifier, msgtext);
                end
                % make sure the error does not mention misleading argument
                % positions
                if strcmp(MERead.identifier, 'MATLAB:TooManyInputs') && ...
                        ~any(contains(MERead.message, 'AutoOrient'))
                    MERead = MException(MERead.identifier, ...
                        string(message('MATLAB:TooManyInputs')));
                end
                % throw the exception
                throwAsCaller(MERead);
            end    
        end
    end
end
...
otherwise
	...
	source = varargin{1};
    % Validate that the second argument is of valid type and non-empty
    validateattributes(varargin{2}, {'char', 'string', 'numeric'}, {'nonempty'});
    ...
    if ischar(varargin{2})
        if (~isempty(cached_fmt)) && ismember(varargin{2}, cached_fmt.ext)
            ...
            if all(isspace(varargin{2}))
                error(message('MATLAB:imagesci:imread:emptyCharArg'));
            end
