...
%
%    STARTAT sets the Running property of the timer object, OBJ, to 'On',
...
%
%    STARTAT(OBJ,S) starts the timer at the time specified by the text
%    representation of the date S. S is a character vector or string scalar
%    containing the text representation of the date. It must use date
%    format 0,1,2,6,13,14,15,16, or 23, as defined by the DATESTR function.
%    Dates with two-character years are interpreted to be within the 100
...
% check for not enough parameters (common error)
    ...
    isTextDate = ischar(varargin{1})||iscellstr(varargin{1});

    % Convert zoned datetime inputs to the local time zone.
    varargin{1} = convertDatetimeTimeZone(varargin{1});
    ...
    for lcv = 1:len
        compareAgainstCurrFiringTime = now;
        startDelay = 86400*(delay(lcv) - compareAgainstCurrFiringTime); % 86400 is number of seconds per day
        if strcmp(obj(lcv).Running,'off')
            

            hasDateFormat = isTextDate && (nargin == 3) && ischar(varargin{2});
            % Validate the correct delay based on start time when using
            % text version of the date.
            if isTextDate && startDelay < 0
                % if a date format was passed in, and it contained d or m
                %, then we don't need to anything, since user explicitly asked for it
                % otherwise, use today's date.
                [delayyear, delaymonth, delayday, delayhour, delayminute, delaysec] = datevec(delay(lcv));
                
                % you will need to keep the ~ , otherwise the output type for
                % cuyear changes to a vector!, Super useless setup
                [curyear, ~, ~, ~, ~, ~] = datevec(compareAgainstCurrFiringTime);
                
                if ~(hasDateFormat)
                    % If the 13/14/15/16 form of datestr is passed in, datenum
                    ...
                    if ( (delayyear == curyear) && (delaymonth == 1) && (delayday == 1) )
                        dateStringsPossibly = varargin{1};
                        if iscellstr(dateStringsPossibly)  %#ok<ISCLSTR>, this will only be a cellstr at this point of code execution
                            theDateStr = dateStringsPossibly{lcv};
                        elseif ischar(dateStringsPossibly)
                            theDateStr = dateStringsPossibly(lcv, :);
                        else
                            continue;
                        end
                        startDelay = matlab.internal.timer.fixStartDelayIfDateAssumptionWasPossiblyMade(theDateStr, ...
                            startDelay, ...
                            compareAgainstCurrFiringTime);
                    end
                else
                    fmt = varargin{2};
                    startDelay = matlab.internal.timer.fixStartDelayIfDatenumWithFormatWasPassed(fmt, ...
                        delayyear, ...
                        delaymonth, ...
                        delayday, ...
                        delayhour, ...
...
function t = convertDatetimeTimeZone(t)
%convertDatetimeTimeZone   Convert zoned datetimes to local time to ensure that
%   time zone offsets are correctly accounted for when starting timers.

    needsLocalTimeZone = isdatetime(t) && ~isempty(t.TimeZone);
    if needsLocalTimeZone
        t.TimeZone = 'local';
    end
end
