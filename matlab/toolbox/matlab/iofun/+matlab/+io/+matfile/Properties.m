...
% Copyright 2011-2024 The MathWorks, Inc.
    ...
    end % end properties (Hidden = true)
    ...
    methods
        ...
        function isInProtectedLoading = get.ProtectedLoading(obj)
            isInProtectedLoading = obj.ProtectedLoading;
        end
        
        function set.ProtectedLoading(obj, isInProtectedLoading)
            if ~isscalar(isInProtectedLoading) || ...
                    (~islogical(isInProtectedLoading) && ...
                    isInProtectedLoading ~= 1 && isInProtectedLoading ~= 0)
                error(message('MATLAB:load:protectedLoadingIllegalValue'));
            end
            obj.ProtectedLoading = isInProtectedLoading;
        end
    ...
    methods(Access = public, Hidden = true)
        ...
        function obj = Properties(Source)
            ...
            if exist(obj.Source, 'file')
                ...
                obj.SupportsPartialAccess = matlab.io.internal.isPartialMATArrayAccessEfficient(obj.Source);
            else
                % obj.SupportsPartialAccess is true by default for new files
                obj.Writable = true;
                obj.ProtectedLoading = true;
            end
        end
