classdef ToUniqueFile < matlab.automation.streams.OutputStream
    %ToUniqueFile  Write text output to a unique file
    %   ToUniqueFile is an output streams that sends text to a unique file
    %   for each instance of the class. When text is printed to this stream,
    %   it opens the file, appends the text to the end of the file,
    %   and closes the file.
    %
    ...
    methods
        function stream = ToUniqueFile(folder, varargin)
           %ToUniqueFile - Create an output stream to a unique file
           %   STREAM = ToUniqueFile(FOLDER) creates an output stream that
           %   writes text output to a file in FOLDER. Each instance of
           %   ToUniqueFile creates a file with a unique filename.
           %
           ...
           %   scalar or a character vector. EXTENSION must begin with a period (.).
            ...
            import matlab.automation.internal.validatePathname;
            defaultPrefix = '';
        ...
        function print(stream, formatStr, varargin)
            ...
            assert(fid > 0, 'MATLAB:automation:ToUniqueFile:OpenFailed', msg);
        ...
        function filename = get.Filename(stream)
            ...
            if isempty(stream.FilenameModifier)
                stream.FilenameModifier = uuid;
            end
            
            filename = fullfile(stream.BaseFolder, stream.Prefix + stream.FilenameModifier + stream.Extension);
        end
    end
...
mustBeTextScalar(input,'Prefix');
mustContainCharacters(input,'Prefix');
ensureNoFileseps(input);
end

function validateExtension(input)
import matlab.automation.internal.mustBeTextScalar;
mustBeTextScalar(input,'Extension');
if strlength(input)==0
    return;
end
if ~startsWith(input,'.')
    error(message('MATLAB:automation:ToUniqueFile:ExtensionMustStartWithADot'));
end
ensureNoFileseps(input);
end
...
if contains(input,'/')
    error(message('MATLAB:automation:ToUniqueFile:CharacterProhibited','/'));
elseif contains(input,'\')
    error(message('MATLAB:automation:ToUniqueFile:CharacterProhibited','\'));
end
end

% LocalWords:  mypackage fid lang Fileseps strlength ADot
