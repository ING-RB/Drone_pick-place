classdef DiagnosticResult
    % DiagnosticResult - Result of diagnostic evaluation of a Diagnostic object
    %
    %   The DiagnosticResult class is a data structure which captures the result of a
    %   diagnosed Diagnostic. It allows for a safe alternative to working with
    %   the diagnosed Diagnostic instances directly.
    %
    %   A DiagnosticResult instance should not be created directly. Instead
    %   it is provided as property values on event data such as the
    %   QualificationEventData, LoggedDiagnosticEventData, QualificationDiagnosticRecord,
    ...
    methods(Hidden)
        function result = DiagnosticResult(artifacts,diagnosticText)
            if ~isequal(diagnosticText,'')
                validateattributes(diagnosticText,{'char','string'},{'scalartext'},'','DiagnosticText');
            end
