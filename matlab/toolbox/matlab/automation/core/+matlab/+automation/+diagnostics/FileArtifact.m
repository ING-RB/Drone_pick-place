classdef FileArtifact < matlab.automation.diagnostics.Artifact
    ...
    methods (Hidden)
        function artifact = FileArtifact(file, varargin)
            % FileArtifact = Class constructor
            %
            ...
            file = matlab.automation.internal.fileResolver(file);
            [location,name,ext] = fileparts(file);
            artifact = artifact@matlab.automation.diagnostics.Artifact(location,...
                [name,ext], varargin{:});
        end
    end
    methods
