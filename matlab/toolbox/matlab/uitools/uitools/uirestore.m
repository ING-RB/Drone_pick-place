...
%   Copyright 1984-2014 The MathWorks, Inc.
    ...
    % notify the toolstrip we are restoring previous activity
    if ~isempty(fig) && isvalid(fig) && strcmpi(fig.DefaultTools, 'toolstrip')
		% if my previous state was such that the toolstrip was enabled, re-enable it here
		if isfield(uistate, 'ToolstripDisabled') && ~uistate.ToolstripDisabled 
			ptn = matlab.plottools.notifier.PlotToolsNotifier();
			ptn.notifyWorkflowEnd(fig);
		end
    end
