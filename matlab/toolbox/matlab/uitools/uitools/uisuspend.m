...
%   Copyright 1984-2014 The MathWorks, Inc.
    ...
    chi = findobj(fig,'-property','ButtonDownFcn','-property','Interruptible',...
        '-property','BusyAction','-property','ContextMenu');
    ...
    uistate = struct(...
            ...
            'ContextMenu',           Lwrap(get(chi, {'ContextMenu'})));	
    ...
    if setdefaults
        ...
        % do nothing for ContextMenu
    end
    
    % notify the toolstrip we are suspending current activity
    if ~isempty(fig) && isvalid(fig) && strcmpi(fig.DefaultTools, 'toolstrip')
	
		uistate.ToolstripDisabled = matlab.graphics.internal.toolstrip.FigureToolstripManager.isToolstripDisabled(fig);
	
        ptn = matlab.plottools.notifier.PlotToolsNotifier();
        ptn.notifyWorkflowStart(fig);
    end
