...
elseif strcmp(action,'execUndo')
    ...
    if ~isempty(hCommand) && ...
        ...
        % Remove command from stack and return if one of the variables is
        % no longer valid.
        for idx = 1:length(hCommand.InverseVarargin)
            cmdVarargin = hCommand.InverseVarargin{idx};
            if isa(cmdVarargin,'matlab.graphics.axis.Axes') && ~isvalid(cmdVarargin)
                cmd_manager.popundo();
                return;
            end
        end
...
elseif strcmp(action,'execRedo')
    ...
    if ~isempty(hCommand) && ...
        ...
        % Remove command from stack and return if one of the variables is
        % no longer valid.
        for idx = 1:length(hCommand.Varargin)
            cmdVarargin = hCommand.Varargin{idx};
            if isa(cmdVarargin,'matlab.graphics.axis.Axes') && ~isvalid(cmdVarargin)
                cmd_manager.popredo();
                return;
            end
