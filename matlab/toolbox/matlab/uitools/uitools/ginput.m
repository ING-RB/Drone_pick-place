...
if isWebFigureType(fig, 'UIFigure')
    for k = 1:4
        crossHair(k) = uilabel(fig, 'Visible', 'off', 'HandleVisibility', 'off'); %#ok<AGROW>         
    end
else
    for k = 1:4
        crossHair(k) = uicontrol(fig, 'Style', 'text', 'Visible', 'off', 'Units', 'pixels', 'HandleVisibility', 'off', 'HitTest', 'off'); %#ok<AGROW>
    end
end
matlab.graphics.internal.themes.specifyThemePropertyMappings(crossHair,'BackgroundColor','--mw-graphics-colorNeutral-line-tertiary');
end
