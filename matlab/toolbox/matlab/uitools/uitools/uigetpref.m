...
%         ''
%	      'You can save your figure manually by typing ''savefig(gcf)'''},...
%       {'always','never';'Yes','No'},...
%       'ExtraOptions','Cancel',...
...
if nargin > 0
    prefGroup = convertStringsToChars(prefGroup);
end

if nargin > 1
    prefPref = convertStringsToChars(prefPref);
end

if nargin > 2
    dlgTitle = convertStringsToChars(dlgTitle);
end

if nargin > 3
    dlgQuestion = convertStringsToChars(dlgQuestion);
end

if nargin > 4
    if isstring(prefOptions)
        prefOptions = cellstr(prefOptions);
    end
end
...
if ~ischar(val) || ~strcmp(val, askVal)
    % return the preference choice value in lower case
    val=lower(val);
    return;
end

% If dialog will be shown, check if supported in execution context
...
checkboxString=locParseInputs(varargin,'CheckboxString',getString(message('MATLAB:uistring:preferencedialogs:DoNotShowThisDialogAgain')));
...
% Add in any custom dialog properties
extraDialogProps = locParseInputs(varargin,'DialogProps',struct);
extraDialogPropNames = fieldnames(extraDialogProps);
for k=1:length(extraDialogPropNames)
    hFig.(extraDialogPropNames{k}) = extraDialogProps.(extraDialogPropNames{k});
end
...
len = length(pNames) + length(extraButtons);
% len = length(pNames);
optButton = ones(1,len) * -1;
count = 0;
for i=1:length(pNames)
    count = count +1;
    optButton(count)=uicontrol( ...
        'Style'       ,'pushbutton'                           , ...
        'KeyPressFcn' ,{@doControlKeyPress, defaultButton}    , ...
        'String'      ,pNames{i}                              , ...
...
if isDecaf
    textExtent = adjustMinimumValues(textExtent, 200, 30);
end
checkExtent=get(hCheckbox,'Extent') + 20;   % add 20 pixels for checkbox art
if isDecaf
    checkExtent = adjustMinimumValues(checkExtent, 140, 37);
end
...
if ~isempty(hHelp)
    ...
    if isDecaf
        helpExtent = adjustMinimumValues(helpExtent, 24, 17);
    end
else
    helpExtent=[0 0 0 0];
end
...
for i=length(optButton):-1:1
    ...
    if isDecaf
        ex = adjustMinimumValues(ex, 37, 19);
    end
...
horizSize=max([cumHorizExtent, checkExtent(3), textExtent(3), helpExtent(3), minHorizSize]);
...
for i=length(optButton):-1:1
    ...
    if strcmp(get(optButton(i),'String'), defaultButton) && ~isDecaf
        setdefaultbutton(hFig, optButton(i));
    end
...
if newCheckValue % ~= checkboxInitState
    %the checkbox state is true.  We will need to set preferences
...
function doFigureKeyPress(obj, evd, default) %#ok<INUSD,INUSL>
switch(evd.Key)
    case {'return','space','escape'}
        delete(gcbf);
end

function doControlKeyPress(obj, evd, default) %#ok<INUSD,INUSL>
switch(evd.Key)
    case {'return','escape'}
        delete(gcbf);
end

function doCallback(obj, evd, button) %#ok<INUSL>
set(gcbf,'UserData',button);

function doHelp(obj, evd, helpFcn) %#ok<INUSL>
...
function extentVal = adjustMinimumValues(extentVal, minWidth, minHeight)
% Only adjust if we are in Decaf mode. 
extentVal(3) = max(extentVal(3),minWidth);
extentVal(4) = max(extentVal(4),minHeight);
