...
if isempty(findprop(handle(fig),'Basic_Fit_Resid_Figure'))
    ...
    if ~isempty(axesList) 
        plotChildren = plotchild(axesList, 2, true);
        plotChildren = findobj(plotChildren,'flat','-function',@(x) matlab.graphics.illustration.internal.islegendable(x));
...
if isempty(scribeunder)
    ...
    if ~isempty(scribeunder)
        underkids = matlab.graphics.annotation.internal.getScribeGrid(scribeunder,'-peek');
    else
        underkids = [];
    end
