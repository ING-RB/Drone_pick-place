...
%   Copyright 2002-2023 The MathWorks, Inc.
...
if nargin > 1
    ...
    for lp=1:length(defaultVariableNames)
        if isstring(defaultVariableNames{lp})
            defaultVariableNames{lp} = char(defaultVariableNames{lp});
        end
        if ~ischar(defaultVariableNames{lp})
            error(message('MATLAB:export2wsdlg:NonCellInput'));
        end
    end
end
...
if (nargin > 5)   % help button wanted
    helpButton = uicontrol(hDialog,...
        'String', getString(message('MATLAB:uistring:export2wsdlg:Help')), ...
        'Units', 'pixels', ...
...
cancelButton = uicontrol(hDialog,...
    'String', getString(message('MATLAB:uistring:export2wsdlg:Cancel')), ...
    'Units', 'pixels', ...
    'Callback', {@CancelCallback, hDialog, retButtonRequest},...
    'Tag','CancelButton');
okButton = uicontrol(hDialog,...
    'String', getString(message('MATLAB:uistring:export2wsdlg:OK')), ...
    'Units', 'pixels',...
    'Tag','OkButton');
...
if ishghandle(hDialog)
    ...
    addlistener(hDialog, 'ObjectBeingDestroyed', @(~,~)delete(c));
end
...
if ~isempty(badnames)
    if (length(badnames) == 1)
        badnamemsg = getString(message('MATLAB:uistring:export2wsdlg:NotValidMATLABVariableNamesOneVariables', ...
            badnames{1}));
    elseif (length(badnames) == 2)
        badnamemsg = getString(message('MATLAB:uistring:export2wsdlg:NotValidMATLABVariableNamesTwoVariables', ...
            badnames{1}, badnames{2}));
    else
        badnamespart1 = sprintf('"%s", ', badnames{1:end-2});
        badnamemsg = getString(message('MATLAB:uistring:export2wsdlg:NotValidMATLABVariableNamesThreeVariables',...
...
if ~isempty(dupnames)
    ...
    elseif (length(dupnames) == 2)
        queststr = getString(message('MATLAB:uistring:export2wsdlg:OverWriteQuestionTwoVariables',...
            dupnames{1}, dupnames{2}));
    else
        queststrpart1 = sprintf('"%s", ', dupnames{1:end-2});
        queststr = getString(message('MATLAB:uistring:export2wsdlg:OverWriteQuestionThreeVariables',...
            queststrpart1, dupnames{end-1}, dupnames{end}));
    end
    buttonName = questdlg(queststr, dialogname, ...
        getString(message('MATLAB:uistring:export2wsdlg:Yes')), ...
        getString(message('MATLAB:uistring:export2wsdlg:No')), ...
        getString(message('MATLAB:uistring:export2wsdlg:Yes')));
