...
switch cmd
    ...
    case 'EditCopy'
        ...
        elseif isactiveuimode(hfig,'Exploration.Brushing')
            if datamanager.isFigureLinked(hfig)
                internal.matlab.datatoolsservices.executeCmd("datamanager.copySelection(gco)");
            else
    ...
    case 'EditFigureProperties'
        ...
        % Show the inspector
        matlab.graphics.internal.propertyinspector.propertyinspector('show',hfig);
    case 'EditAxesProperties'
        % domymenu menubar axesprop
        ax = get(hfig,'CurrentAxes');
        if ~isempty(ax)
           % Show inspector with axes properties
    ...
    case 'EditObjectProperties'
        ...
        % Show inspector with current object properties
...
jpeer = matlab.ui.internal.JavaMigrationTools.suppressedJavaFrame(hfig);
...
% self-contained Charts or handlevisibility off figure don't support colormap editor
% g2298438
cmapObj = get(hfig,'CurrentAxes');
if isa(cmapObj,'matlab.graphics.chart.Chart') || ...
        strcmpi(get(hfig, 'HandleVisibility'),'off')
