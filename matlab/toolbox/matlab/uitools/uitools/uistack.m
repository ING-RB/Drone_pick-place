...
% If the list is not empty, check for invalid handles
if ~isempty(Handles) && ~all(ishghandle(Handles),'all')
...
StackOpt = getStackOptForSpecialType(SameType, StackOpt);
...
switch StackOpt
    ...
    otherwise
        error(message('MATLAB:uistack:InvalidStackOption','up','down','top','bottom'));
...
function opt = getStackOptForSpecialType(item, StackOpt)
opt = StackOpt;
persistent optStruct;
if isempty(optStruct)
    optStruct = struct('up', 'down',...
        'down', 'up',...
        'top', 'bottom',...
        'bottom', 'top');
end
...
if ishghandle(item)
    % For uimenu, uitoolbar, uipushtool, and uitoggletool, using the passed-in
    % argument StackOpt would cause the following counterintuitive behavior:
    % uimenu / uipushtool / uitoggletool with "up" passed in would be moved to the right
    % uitoolbar with "up" passed in would be moved down
    % 
    % This is unintuitive as for all other components, passing in "up" moves
    % them upwards or towards the left. Therefore, to make uistack work for
    % these componenets the same way it works for others, we reverse the
    % direction of stacking.
    item = handle(item);
    if (isa(item, 'matlab.ui.container.Menu') || ...
            isa(item, 'matlab.ui.container.toolbar.PushTool') ||...
            isa(item, 'matlab.ui.container.toolbar.ToggleTool') ||...
            isa(item, 'matlab.ui.container.Toolbar'))
        opt = optStruct.(StackOpt);
    end
end
