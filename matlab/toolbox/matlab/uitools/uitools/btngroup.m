...
for k = (1+propertyOffset):2:nargin
  ...
  property = varargin{k+1};  %#ok<NASGU> % used later in EVAL
  
  idx = strmatch(propertyString, propertyNames);
  if (isempty(idx))
    error(message('MATLAB:btngroup:UnknownProperty', propertyString));
  elseif (length(idx) > 1)
...
if (isempty(groupId))
  error(message('MATLAB:btngroup:NoGroupIDMAtrix'));
end
if (isempty(buttonId))
  error(message('MATLAB:btngroup:NoBtnIDMatrix'));
end
...
set(axHandle, 'SortMethod', 'ChildOrder');
...
%xOffset = zeros(groupSize); this definition for xoffset does not appear to be used
%yOffset = zeros(groupSize); this def for yoffset does not seem to be used
%m = 1; this seems to never be used 
...
%x2 = buttonSep2+2*bevelWidth; these variables and the values assigned to them don't seem to be used
%x3 = 1-x2;
x4 = 1-x1;
y1 = x1;
%y2 = x2;
...
%handles = []; this is appears to be not needed
...
% Remove Floating toolbar from buttons
toolbarHandle = get(axHandle,'Toolbar');
set(toolbarHandle,'Visible', 'off'); 
