classdef FigureToolManager < matlab.mixin.SetGet & ...
        matlab.plottools.notifier.PlotToolsNotifier
%
...
function locCommandStackChanged(obj,hThis)
    ...
    % Here we update the undo/redo buttons
    % We only want to update when the interaction is done on the selected figure (zoom case)
    % The server doesn't know what the selected figure is so we use gcf
    if isequal(fig, curFig) 
        hThis.notifyToolstripQAB(fig);
    end
...
function locUpdateUndoUI(hThis, fig,cmd_manager)
    ...
    % If invalid handle, load by searching tag, but only if the menubar is
    % on, for performance reasons
    if (isempty(hMenu) || ~ishghandle(hMenu)) && ~strcmp(fig.MenuBar,'none')
