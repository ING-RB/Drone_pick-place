...
%If we clicked on a UIComponent or button on hover-based AxesToolbar, return unless we are
%suspending the callback.
if ~isempty(h) && (isa(h, 'matlab.ui.control.ClientComponent') || ...
        isa(h, 'matlab.ui.control.internal.model.ComponentModel') || ...
        isa(h, 'matlab.graphics.controls.internal.Control'))
...
if ~isempty(h) && isprop(h,'UIContextMenu') && (isempty(h.UIContextMenu) || ~isempty(h.UIContextMenu.Parent))
    % Colorbar contexmenu in uifigure has an empty parent which cannot be
    % restored  
