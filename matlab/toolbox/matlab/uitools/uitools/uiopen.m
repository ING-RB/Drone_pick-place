...
if direct
    ...
    % Throws error if in -nojvm mode (Required if running with direct=true)
    matlab.ui.internal.utils.checkJVMError;
else
    % Check if supported in execution context (Required if running with direct=false)
    % Note, file dialogs are supported in -noFigureWindows
    matlab.ui.internal.utils.validateModalDialogsCapability(AllowInNoFigureWindows=true);
