...
% Copyright 2006-2023 The MathWorks, Inc.
    ...
    if isWebUI
        % API for folder chooser dialog
        dirdlg = matlab.ui.internal.dialog.WebFileChooser();
        
        % setup
        dirdlg.setDialogTitle(dialog_title);
        dirdlg.setDirectory(dialog_pathname);
        
        fileTypeFilters = '';
        dlgType = 2;
        % dlgtype is 2 for open dir dialog
        dirdlg.showDialog(fileTypeFilters, dlgType);

        % g2966617 uigetdir does not include a trailing file separator, so
        % declaring optional trailingSep argument as false
        dirname = dirdlg.getFilePath(false);
    else
        dirdlg = matlab.ui.internal.dialog.FolderChooser();
        % Call FolderChooserMO in non-desktop environments
        if ~Capability.isSupported(Capability.LocalClient)
            dirdlg = matlab.ui.internal.dialog.FolderChooserMO();
        end
        
        dirdlg.InitialPathName = dialog_pathname;
        dirdlg.Title = dialog_title;
        
        % Decaf check not needed when diabling all windows for native dialogs
