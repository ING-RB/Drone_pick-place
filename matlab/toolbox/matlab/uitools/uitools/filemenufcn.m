...
try
  ...
  if strcmp(types{typevalue,4},'fig') && ~fromExport
   saveas(hfig,filename);
    ...
    if feature('webui')
        % use saveas which will determine what args to pass to print 
        saveas(hfig, filename);
    else
        c1 = matlab.graphics.internal.export.disableWarningInScope('MATLAB:print:ExcludesUIInFutureRelease');
        c2 = matlab.graphics.internal.export.disableWarningInScope('MATLAB:print:RasterNotSupportedInFutureRelease');
        c3 = matlab.graphics.internal.export.disableWarningInScope('MATLAB:print:PostscriptNotSupportedInFutureRelease');
        c4 = matlab.graphics.internal.export.disableWarningInScope('MATLAB:hgexport:NotSupportedInFutureRelease');
...
if feature('webui')
   % remove other deprecated formats
   list( ismember(list(:,1),{'*.ps', '*.bmp'}) , :) = []; 
end
