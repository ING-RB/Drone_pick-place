...
% If charSet is empty, use UTF-8 for text
if isempty(char(charSet))
   charSet = 'UTF-8';
end
...
function reader = xmlreader()
% Choose an appropriate XML reader for contentType xmldom based on
% presence of Java
javaPresent = isempty(javachk('jvm'));
if(javaPresent)
    reader = @xmlread;
else
    reader = @(filename)parseFile(matlab.io.xml.dom.Parser, filename);
end