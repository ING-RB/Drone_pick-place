...
% Undocumented options FOR INTERNAL USE ONLY. These options may change or
% be eliminated in future releases.
%
%     Debug       - true to display request and response messages
%     NativeDebug - true to display deails of every network operation
...
classdef weboptions <  matlab.mixin.CustomDisplay
    properties (AbortSet)
        ...
        %
        %   This timeout does not limit how long it may take to send or receive very
        %   large files or messages.
        Timeout = 5
        ...
        %   value. If this property is nonempty, then MATLAB uses Basic HTTP
        %   authentication, which sends the username and password in the clear to the
        %   server. 
        %
        %   You do not need to set this property on Windows for authentication with
        %   NTLM or Kerberos, if you are properly logged into a Windows domain,
        %   because this information is taken from the system. To use any other
        %   scheme that requires a username and password, such as Digest or NTLM on
        %   Linux, without exposing your credentials to the server, you should use
        %   the HTTP interface and specify these credentials in a Credentials object.
        %
        %   The default value is the empty string.
        %
        % See also matlab.net.http.Credentials
        Username = ''
    ...
    properties (Dependent)
        ...
        %
        %   You may specify additional semicolon-separated name=value parameters within
        %   the MediaType string if the server requires these as part of the
        %   Content-Type header field in the request. For example:
    ...
    properties
        ...
        %   fails.
        %
        %   By default, the property value is set to the character vector
        %   'default' which tells MATLAB to use system certificates. If you
        %   need additional certificates, you can add your certificates to
        %   system certificates. MATLAB provides no tools for managing
        ...
        %   used
        %
        %   If set to empty, the only verification done for the server
        %   certificate is that the domain matches the host name of the
        %   server and that it has not expired: the signature is not
        %   validated.
        %
        %   Set this to empty only if you are having trouble establishing a
        %   connection due to a missing or expired certificate in the
        %   default PEM file and you do not have any alternative
        %   certificates.
    ...
    properties (Hidden)
        RealMediaType = 'auto'
        Debug(1,1) logical = false   % display request and response messages
        NativeDebug(1,1) int32 = 0;  % detailed output; same as HTTPOptions.Debug
    end
    ...
    methods
        ...
        % Validate and set CharacterEncoding.
            ...
            if ~strcmp(value, defaultValue)
                options.CharacterEncoding = validateCharacterEncoding(options, value);
            else
        ...
        % Validate and set Username.
            options.Username = mustBeStringOrSecretIDScalar(value, 'Username');
        end
        ...
        % Validate and set Password.
            options.Password = mustBeStringOrSecretIDScalar(value, 'Password');
        end
        ...
        % Validate and set MediaType
            ...
            if ~isa(value, mediaType)
                try
                    % parse string using MediaType
                    options.RealMediaType = matlab.net.http.MediaType(value);
                catch e
                    ...
                    if strcmpi(value,'auto')
                        ...
                        options.RealMediaType = char(lower(value));
                        return
                    else
                        % Correct type, so syntax must be bad
                        if options.Debug || options.NativeDebug
                            rethrow(e)
                        else
                            throwAsCaller(e)
                        end
                ...
                options.RealMediaType = value;
            end
        end
        
        %------------------------------------------------------------------
        
        function type = get.MediaType(options)
            if isempty(options.RealMediaType)
                type = '';
            else
                type = char(options.RealMediaType);
            end
        end
        ...
        function options = set.NativeDebug(options, value)
            options.NativeDebug = value;
        end
    end
        ...
        % Password to replace each character with '*'.
            ...
            if isscalar(options)
                ...
                if ~isempty(username) && ~(class(username) == "secretID")
                    group.PropertyList.Username = repmat('*', size(username));
                end
            end
        end
    end
...
p.addParameter('NativeDebug',options.NativeDebug);
...
function value = validateCharacterEncoding(options, value)
...
try
    validateattributes(value, ["string", "char", "secretID"], string.empty, mfilename, propname);
    if class(value) == "secretID"
        validateattributes(value, "secretID", "scalar", mfilename, propname);
        result = value;
    else
        validateattributes(value, ["char" "string"], "scalartext", mfilename, propname);
        result = char(value(:)');
    end
catch ex
    throwAsCaller(ex);
