...
% Copyright 2015-2018 The Mathworks, Inc.
    ...
    methods
        ...
        %      a string, any required percent-encoding will be performed only on
        %      characters that should be encoded, and this will be done whether or not
        %      those characters were originally encoded in QUERYSTR, so the result from
        %      the string method may not exactly match QUERYSTR (although the meaning
        %      will be the same when used in a URI).
        %
            ...
            if nargin > 0
                if nargin == 1 && (isempty(varargin{1}) || iscell(varargin{1}))
                    ...
                    if (isstring(firstArg) || ischar(firstArg)) && ...
                       (isscalar(varargin) && (isempty(literalArg) || ...
                        ...
                        if isscalar(varargin) && isstruct(firstArg)
        ...
        %   character and including other punctuation.  In addition, any special
        %   characters in Name or Value not permitted in a query are percent-encoded,
        %   except that a space is encoded as '+' (see the QUERYSTR argument to the
        %   QueryParameter constructor for more information on encoding).
        %
...
% returns more or fewer strings than the length of values.  
    ...
    if isa(values, 'datetime') 
        ...
        try
            ...
            % For numbers or logicals, it's not compatible with webread/webwrite unless
            % we use num2str on it first.
