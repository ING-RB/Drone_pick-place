classdef (Sealed) CookieField < matlab.net.http.HeaderField
    ...
    %
    %     resp = matlab.net.http.RequestMessage();
    %     resp.send('http://www.google.com');
    ...
    %        resp = resp.addFields(matlab.net.http.field.CookieField([cookieInfos.Cookie]));
    %     end   
    %     resp = resp.send('http://www.google.com');
    %
    ...
    %
    %   In addition, if the initial exchange above involved multiple message messages for
