    ...
    methods
        ...
        %   errors if invalid.
            ...
            if nargin > 1 && (length(varargin) > 1 || ...
                 (isscalar(varargin) && (ischar(varargin{1}) || isscalar(varargin{1}))))
                ...
                if nargin == 1
                    ...
                    if ~isstring(params) || ~ismatrix(params) || ...
                        ...
                        % specified illegal value for first param name
...
% Verify syntax of token and return string. Throw on error.
    ...
    ic = regexp(charName, '[^' + matlab.net.http.HeaderField.TokenChars + ']', 'once', 'match');
    if isempty(ic)
        token = string(str);
    else
...
function [type, subtype] = getTypeAndSubtype(str, silent)
    ...
    if numel(res) == 2
        ...
        try
            t = getToken(res(1));
            subt = getToken(res(2));
            % Assign later to type and subtype if there is no error to avoid overwriting.
            type = t;
            subtype = subt;
        catch ME
            if(~silent)
