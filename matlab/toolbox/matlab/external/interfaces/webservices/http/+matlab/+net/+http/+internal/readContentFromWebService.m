...
%   [data, payloadLength, payload] = readContentFromWebService(connector, ~, convert, raw) 
%   reads the content from the Web service indicated by
...
% Copyright 2015-2018 The MathWorks, Inc.
    ...
    if(useConnector && strcmp(connector.RequestMethod, "HEAD")) % For head request only headers are available
        data = [];
        payload = [];
    ...
    if downloadToFile
        ...
        if (~isempty(e) || nargout > 1) && exist(filename,'file')
        ...
        if useConnector
            try
                ...
                % if an exception occurred during data transfer (including ctrl/c), 
                % insert any partial payload into the exception
                payload = connector.getResponsePayload();
                if isempty(payload)
                    rethrow(me);
                else
                    throw(matlab.net.http.internal.ExceptionWithPayload(payload, me));
