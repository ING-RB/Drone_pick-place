        ...
        % Use RFC 6265 parsing rules for each of the attributes
                    ...
                    switch name
                        case 'Expires'
                            ...
                            if isempty(tv)
                                ...
                                % do that requires "manual" parsing as datetime does not 
                                % automatically recognize those formats. 
                                % Hopefully these cover reasonable
                                % options; mathworks.com implements the 1st one.
                                % (g1352119)
                                formats = {'eee, dd-MMM-yyyy HH:mm:ss z' 'e, d-MMM-y H:m:s z' ...
                                           'e d-MMM-y H:m:s z' 'd-MMM-y H:m:s z'...
                                           'd MMM y H:m:s z' ...
                                           };
            ...
            if isempty(info.Path)
                ...
                if ~isempty(uri)
                    ...
                    if isempty(info.Path) || strlength(info.Path) == 0 || ~startsWith(info.Path,'/')
                        info.Path = '/';
                    end
                end
            else
