classdef (Sealed) AuthInfo < matlab.mixin.CustomDisplay
    ...
    %   authentication or specify authorization information, or you need to
    %   implement an authentication protocol (such as OAuth) that is not handled
    %   automatically.
    %
    %   AuthInfo properties:
    ...
    %                     which MATLAB uses to match up with the Realm in Credentials.
    %                     Note that AuthInfos returned by AuthenticateField.convert
    %                     always have a realm parameter.
    %
    ...
    %         from client) from RFC 2617, <a href="http://tools.ietf.org/html/rfc2617#section-3.2.2">section 3.2.2</a>:
    %            realm       string
    %            username    string
    %            nonce       string
    ...
    %        MATLAB does not provide any special processing for Schemes whose
    %        numeric value is less than 0. All parameter names and values are
    %        acceptable, as long as they can be converted to strings. A future
    %        release may implement special handing for properties of additional
    %        Schemes, consistent with Internet standards.
    %
    %   AuthInfo methods:
    ...
    %   functions. If the parameter is one of the known parameters listed above, its
    ...
    methods
        ...
        %   the value of the NAME parameter). However it will set VALUE to an empty
        ...
        %   exactly as it would appear in an Authorization or Authenticate header field.
                        ...
                        switch charName
                            ...
                            case 'nc'
                                ...
                                if ischar(value) || isstring(value)
                                    try
                                        nc = hex2dec(getString(value, mfilename, charName));
                                    catch
                                        if validate
    ...
    methods (Access={?matlab.net.http.RequestMessage,?tHTTPAuthInfoUnit})
        function obj = getStrongest(obj, authInfo)
        % Return the strongest authInfo in the obj array weaker than the
        % provided authInfo.  Returns the strongest one if authInfo empty.
            if isscalar(obj)
                if ~isempty(authInfo)
                    obj = [];
                end
            else
                % delete all the AuthInfos equal to or stronger than authInfo
                % and return the top one remaining
                if ~isempty(authInfo)
                    obj([obj.Scheme] >= authInfo.Scheme) = [];
                end
                [~, i] = max([obj.Scheme]);
                if isempty(i)
                    obj = [];
        ...
        % worksFor Check whether this authInfo applies to the challenge in authInfo
            ...
            if isempty(authInfo)
                ...
                if obj.Scheme == authInfo.Scheme && ...
                   ...
                   if ~isempty(thisDomains) && ~isempty(otherDomains)
