classdef (Sealed) MessageBody 
    ...
    %   described for the Data property.
    %
    ...
    properties (Dependent)
        ...
        %   In a RequestMessage, this is the MATLAB data that you inserted, prior to
        %   any conversion to a uint8 payload. In a ResponseMessage, this is
        %   MATLAB data after conversion from a uint8 payload received from the
        %   server. Data may be of multiple types.
        %
        %   In a RequestMessage, conversion of Data to a payload happens when
        ...
        %
        %   To insert data in a RequestMessage without conversion, set the Payload
        %   property instead of the Data property.
        %
        ...
        %   options for how to convert the data. See the documentation on the various
        %   ContentConsumers.
        %
        %   application/json
        ...
        %   to the raw bytes received if you specify a history return argument or set
    ...
    properties (Transient, SetAccess={?matlab.net.http.Message, ...
        ...
        %   add a Content-Type field to the header, this property will be set in the
        %   copy to the value of the ContentTypeField, if there is one. Otherwise it
        %   is left empty. The send and complete methods of RequestMessage then set
        %   this property based on the type of data you stored in Data and the value
        %   of the ContentTypeField in the RequestMessage, as described for the Data
        %   property.
        %
    ...
    methods
        ...
        % show Display or return a human-readable version of the data
        ...
        %   returns the Data only if is a scalar string or character vector. If it is
        ...
        %   RequestMessage.
        %
        %   If BODY is an array, this method assumes the data was in a multipart
        %   message. In this case it displays (or returns) information on each
        %   member, combined into one string, separated by part numbers.
        %
        %   The output or return value of this method is designed to be
        %   human-readable and can change in future releases. It is not intended to
        %   be parsed.
        %
    ...
    methods (Access=private)
        ...
        % must be a char vector or string
            ...
            if ~isempty(obj.ContentType) 
                mediaType = obj.ContentType.Type + '/' + obj.ContentType.Subtype;
                charset = upper(matlab.net.internal.getCharsetForMediaType(obj.ContentType));
                if ~isempty(charset)
