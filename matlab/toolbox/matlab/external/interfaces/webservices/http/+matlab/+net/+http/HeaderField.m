...
%   Copyright 2015-2023 The MathWorks, Inc. 
    ...
    methods (Static, Access=protected, Hidden)
        ...
        %   the header need to override this.
            tf = true;
        end
        ...
        % or struct input; false to silently allow any value.
            ...
            if ~isempty(fields)
                ...
                if isstruct(varargin{1})
                    ...
                    structs = varargin{1};
                    len = length(structs);
                    assert(isscalar(varargin) && (isempty(structs) || isvector(structs)));
                    if check
                        for idx = 1 : len
                            fields(idx) = HeaderField.createHeaderField(structs(idx).Name, ...
                                          structs(idx).Value);
                        end
                    else
                        for idx = 1 : len
                            fields(idx) = createFieldSafe(structs(idx).Name, ...
                                          structs(idx).Value);
                        end
                    end
                else
