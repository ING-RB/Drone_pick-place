...
%    Debug               - 1: low level debugging output during transfer
%                          2: output data as well as headers

% Copyright 2015-2019 The MathWorks, Inc.
    
    properties
        % MaxRedirects - number of redirects that should be followed automatically
        %   for a given request. If nonzero, cookies received from the server in each
        %   redirect response will be copied into in the redirected message. Set to
        %   zero to disable redirections or Inf to allow unlimited redirections. Once
        %   this count has been reached, the next redirect message will be returned
        %   as the response message. Default is 20.
        %
        % See also ResponseMessage
        MaxRedirects (1,1) {mustBeCount} = 20 % stored as double
        
        % ConnectTimeout - the connection timeout in seconds 
        %   This value determines how long we wait to complete an initial connection
        %   attempt before throwing an error. If a proxy is involved, this applies to
        %   the connection to the proxy; otherwise it applies to the connection to the
        %   server. This does not limit how long it may take to receive a complete
        %   response. Set the value to Inf to wait indefinitely. The default value is 10
        %   seconds.
        %
        %   To specify a timeout for connection to a server when a proxy is involved, 
        %   use ResponseTimeout.
        %
        % See also ResponseTimeout, DataTimeout
        ConnectTimeout (1,1) {mustBeNonnegative} = 10 % stored as double
        ...
        %  returned as the response message. 
        %
        % See also Credentials.
        Authenticate (1,1) logical = true
        
        % Credentials - a vector of Credentials to be used for authentication
        %   This is used only if Authenticate is set. Default is a default Credentials
        %   object, which allows authentication for schemes such as Kerberos and NTLM on
        %   Windows that do not require specifying a username or password. If you expect
        %   to be accessing the same server multiple times during a session, then for
        %   maximum performance you should specify the same Credentials vector (or the
        %   same HTTPOptions object) each time, because the Credentials vector contains
        %   cached information that speeds up subsequent authentications.
        %
        %   If you set this to empty, then no authentication will take place.
        %
        ...
        %
        % See also Authenticate, ProxyURI, Credentials
        Credentials matlab.net.http.Credentials = matlab.net.http.Credentials;
        ...
        UseProgressMonitor (1,1) logical = false
        ...
        % HTTPException
        SavePayload (1,1) logical = false
        
        % ConvertResponse - true to convert response data
        %   This option specifies how the raw payload received from the server in a
        %   ResponseMessage (a uint8 vector in MessageBody.Payload) is processed.
        %
        %   If true (the default):
        %
        %      If no ContentConsumer is specified, the payload is converted and
        %      stored in MessageBody.Data based on the Content-Type in the message.
        %      For example, "application/json" content is converted to a struct.
        %      See MessageBody.Data for conversion rules.
        %
        %      If a ContentConsumer is specified, the uint8 payload is passed to the
        %      ContentConsumer for further processing.
		%
		%   If false:
        %
        %      The ContentConsumer, if specified, is ignored. Behavior depends on
        %      whether the Content-Type specifies character data. If the Content-Type
        %      has an explicit or default charset, the payload will be converted to a
        %      string and stored in MessageBody.Data, and no futher processing of the
        %      string will be done. If Content-Type is not character data, or MATLAB
        %      cannot determine the charset from the Content-Type, then
        %      MessageBody.Data just contains original payload as a uint8 vector.
        %
        %      If you want to see the original uint8 payload without any character
        %      conversion, specify true for this property and use a BinaryConsumer,
        %      or set SavePayload to true and examine MessageBody.Payload.
        %
        %   In all cases, MessageBody.Payload is deleted unless you have specified
        %   SavePayload.
        %
        %   If no ContentConsumer was specified, this property is ignored if the
        %   message was encoded (i.e., compressed) and could not be decoded or
        %   decoding was suppressed by DecodeResponse.
        %
        % See also MessageBody, SavePayload, Debug, DecodeResponse, RequestMessage,
        % matlab.net.http.field.ContentTypeField, matlab.net.http.io.ContentConsumer,
        % matlab.net.http.io.BinaryConsumer
        ConvertResponse (1,1) logical = true
        ...
        %   Content-Type. A response is encoded when there is a Content-Encoding field
        %   that specifies a compression algorithm such as 'gzip'.
        %
        %   If this is false and the payload is encoded, the raw unencoded payload is
        %   saved in MessageBody.Payload and, if no ContentConsumer is specified,
        %   MessageBody.Data remains empty and no conversion is done, regardless of the
        %   setting of ConvertResponse. Even if this is true, decoding will not occur if
        %   the Content-Encoding type is not supported by MATLAB. The only content
        ...
        DecodeResponse (1,1) logical = true
        
        % ProgressMonitorFcn - progress monitor handler
        %   This is a handle to a function returning a matlab.net.http.ProgressMonitor
        ...
        %
        %   CertificateFilename is a character vector or string denoting
        %   the location of a file containing certificates in PEM format.
        %   The location must be in the current directory, in a directory
        %   on the MATLAB path, or a full or relative path to a file. If
        %   this property is set and you request an HTTPS connection, the
        %   certificate from the server is validated against the
        %   certification authority certificates in the specified PEM file.
        %   This is used by standard https mechanisms to validate the
        %   signature on the server's certificate and the entire
        %   certificate chain. A connection is not allowed if verification
        %   fails.
        %
        %   By default, the property value is set to the string "default"
        %   which tells MATLAB to use system certificates. If you need
        %   additional certificates, you can add them to the system
        %   certificates. MATLAB provides no tools for managing
        %   certificates or certificate files, but there are various third
        %   party tools that can do so using PEM files. PEM files are ASCII
        %   files and can be simply concatenated. Since security of HTTPS
        %   connections depend on integrity of this file, you should
        %   protect it appropriately.
        %
        %   If you specify the value 'default' system certificates will be
        %   used.
        %
        %   If you set this to empty, the signature on the server
        %   certificate is always trusted and not validated against any CA
        %   certificates. Set this to empty only if you are having trouble
        %   establishing a connection due to a missing or expired
        %   certificate in the default PEM file and you do not have any
        %   alternative certificates.
        %
        %   Whether or not this property is empty, MATLAB will verify that
        %   the name in the server certificate matches the host name of the
        %   server and that it has not expired. This check can be disabled
        %   using VerifyServerName.
        %
        ...
        VerifyServerName (1,1) logical = true
    
        % DataTimeout - the timeout in seconds between packets on the network
        %   This timeout is enforced during a connection once an initial connection has
        %   been established. If this timeout is exceeded while waiting to send or
        %   receive the next expected packet, MATLAB closes the connection and throws an
        %   error. In this case you can obtain any partial data received from
        %   HTTPException.History.
        %
        %   Default is Inf (no timeout).
        %
        % See also HTTPException, ResponseTimeout
        DataTimeout (1,1) {mustBeNonnegative} = Inf % stored as double
        
        % ResponseTimeout - timeout to receive initial response
        %   This is the amount of time in seconds to wait for the header of the response
        %   from the server after sending the last packet of a request. If this timeout
        %   is exceeded the connection is closed and an error is thrown. Default is Inf
        %   (no timeout).
        %
        %   This timeout is useful when to limit the wait time when sending a request to
        %   a server through a proxy, since ConnectTimeout only applies to the proxy
        %   connection time.
        %
        % See also ConnectTimeout, DataTimeout
        ResponseTimeout (1,1) {mustBeNonnegative} = Inf % stored as double
        
        % KeepAliveTimeout - connection keep-alive timeout
        %   This is how long the connection to the server stays open after an initial
        %   connect, enabling multiple successive messages to be sent over the same
        %   connection. A value of Inf enables persistent connections, keeping the
        %   connection open as long as the server is willing.  A value of 0 closes the
        %   connection after each message. Other values are not supported.
        %
        %   This value has no effect on success of an operation, as MATLAB always keeps
        %   the connection open long enough to get the expected response from the server
        %   (provided no other timeouts have been exceeded). A value of 0 can seriously
        %   affect performance of many short messages to the same server.
        KeepAliveTimeout (1,1) {mustBeNonnegative} = Inf % stored as double
    end
        
    properties (Constant, Access=private)
        DefaultCertificateFilename = "default";
    end
    
    properties (Hidden, Transient=true)
        % Debug - print debugging information during message transfer
        %   Setting this to nonzero displays every request and response in the command
        %   window. Setting this option also causes MessageBody.Payload to be saved for
        %   the payload of the transmitted or received message. Set to 2 to output data
        %   as well as headers.
        Debug (1,1) int32 = 0;
    end
    ...
    methods
        ...
        %    values.
            ...
            obj = matlab.net.internal.copyParamsToProps(obj, varargin, 'Debug');
...
function mustBeCount(arg)
% Validation function requiring value to be integer 0-Inf
    mustBeNonnegative(arg);
    if ~isinf(arg)
        mustBeInteger(arg);
    end
end
