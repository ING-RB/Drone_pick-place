classdef (Sealed) CredentialInfo < matlab.mixin.CustomDisplay & handle
    % CredentialInfo Information about a successful or attempted Basic or Digest
    %   authentication. A vector of successful CredentialInfos is stored in each
    %   Credentials object that has been used to used to authenticate a request
    %   using Basic or Digest.
    %
    % This class is for internal use only. It may change in a future release.
    
    % Copywrite 2015-2024 The MathWorks, Inc.
    ...
    properties (Access={?matlab.net.http.Credentials,...
        ...
        % came from the challenge in an AuthenticateField.  This could be empty if
        % this CredentialInfo was constructed from preferences where we 
        % don't know the Scheme required by the host (yet).
        AuthInfo
    ...
    properties (SetAccess=private)
        ...
        % URI of the proxy, with Host and Port only.  A given Credentials object 
        % may contain CredentialInfos for both proxies and servers.
        ForProxy = false
    end
    
    properties (Access={?matlab.net.http.Credentials,?matlab.net.http.internal.HTTPConnector,?tHTTPCredentialsUnit})
    ...
    methods
       function set.Password(obj, str)
        ...
        function set.AuthInfo(obj, value)
            assert(~isempty(value) || obj.ForProxy); %#ok<MCSUP>
            obj.AuthInfo = value;
        end
    ...
    methods (Access={?matlab.net.http.Credentials,?matlab.net.http.RequestMessage})
        ...
        %  Credentials object that allows Basic or Digest, for a request that does not
        %  already contain a matching CredentialInfo, to create actual credentials that
        %  can be used to authenticate the next request. If authentication with this
        %  CredentialInfo is successful, the infrastructure adds this object to its
        %  vector of CredentialInfos in the appropriate Credentials object.
        %
        ...
        %             be "" to indicate an empty username, but not [].
            ...
            obj.Username = obj.getSecretFromValue(username);
            obj.Password = obj.getSecretFromValue(password);
            if nargin > 4
        ...
        % assumption that all paths under www.host.com/foo use the same credentials.
            ...
            if ~(any(obj.URIs <= uri))
                ...
                for i = 1 : length(obj.URIs)
                    ...
                    if len > 0 && testURI.Path(1) == ""
                        % matchPath doesn't count the root, so if the root
                        % is at the head of the path, add 1 to matching length
                        % so we include it
                        len = len + 1;
                    end
    ...
    methods (Access = protected, Hidden)
        ...
        function secret = getSecretFromValue(~, input)
            if class(input) == "secretID"
                input = getSecret(input);
            end
            secret = input;
        end
    end
