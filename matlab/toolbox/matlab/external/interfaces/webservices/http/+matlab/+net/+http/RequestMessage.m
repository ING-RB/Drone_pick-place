...
properties (Dependent)
    ...
    methods
        ...
        function obj = set.Consumer(obj, value)
            if isa(value, 'matlab.net.http.io.ContentConsumer')
                obj.Consumer = matlab.lang.WeakReference(value);
            else
                obj.Consumer = value;
            end
        end

        function value = get.Consumer(obj)
            if isa(obj.Consumer, 'matlab.lang.WeakReference') 
                value = obj.Consumer.Handle;
            else
                value = obj.Consumer;
            end
        end
        ...
        %               must name a Host. If a string, and it does not mention a Scheme,
        %               'http' is assumed; for example 'www.google.com' and
        ...
        %               are involved.  These intermediate messages will be in HISTORY.
        %
        %     COMPLETEDREQUEST
        ...
        %               request. In the normal case where there is just a single request
        %               and response, this contains one record. There may be multiple
        %               LogRecords in this history in the case of an authentication
        %               containing multiple messages and for each redirection. One use
        %               of this history, in addition to debugging, is to obtain all the
        %               Set-Cookie headers from response messages, which you may need to
        %               send back in subsequent requests. When this function returns
        %               normally, the last record in this history always contains the
        %               same value as COMPLETEDREQUEST and RESPONSE (headers only). In
        %               general a LogRecord contains only headers. To log message
        %               bodies, specify OPTIONS.SavePayload.
        %
        %               For some authentication schemes, such as Negotiate or NTLM, the
        %               history may not show all intermediate requests and responses.
        %
        ...
        %          header, set REQUEST.Completed to true prior to sending. If you used
        %          the complete method to complete the request, you should specify the
        %          same value of URI and OPTIONS that you provided to complete, or there
        %          may be unpredictable results. Even if Completed is set, unspecified
        %          fields in the RequestLine will be filled in with default values.
        ...
        %          completing the message. However this will not override a header field
        %          that a ContentProvider might add. Even if you override a field, for
        %          some header field types, SEND may still reject the message if the
        %          value is not valid. To prevent any checking of the value of a given
        %          field, or to override a field that a ContentProvider would add, add a
        %          field of type matlab.http.field.GenericField to the header with the
        %          desired name and value. Neither SEND nor a ContentProvider will add
        %          any header fields with names equal to any GenericField headers and
        %          will not check their correctness.
        %       4. To send raw binary data without conversion, you can insert a uint8
        %          vector into either Body.Data or Body.Payload. The only difference
        %          between Payload and Data is that data in Body.Data is subject to
        %          conversion based on the Content-Type field in the message, while
        %          Body.Payload is not. Note that SEND will always try to convert
        %          nonempty Body.Data if Body.Payload is empty, even if Completed is
        %          already set. See MessageBody.Data for conversion rules.
        %
        %     SEND throws an MException if the message could not be completed because
        %     its headers are not well-formed or conversion of Body.Data fails. If the
        %     message was completed but the Web service cannot be reached, or the Web
        %     service does not respond within the timeout period specified in OPTIONS,
        %     or a conversion error occurs trying to convert the response payload to
        %     MATLAB data, SEND throws an HTTPException. If the Web service responds and
        %     returns an HTTP error status, SEND returns normally, setting the Status
        %     property of RESPONSE to indicate the error returned from the server. You
        %     should always check RESPONSE.StatusCode to determine whether the request
        %     was accepted.
        %
        %     After SEND returns, you can examine the COMPLETEDREQUEST to see what was
        %     sent, including the converted Data in Body.Payload if OPTIONS.SavePayload
        %     is set. If multiple messages were involved in accessing the server (for
        %     example, there were redirections or an authentication exchange occurred),
        %     it will contain the last such request. To see the first, or intermediate
        %     messages, look at HISTORY.
        %
        ...
        %
        %     For RESPONSE, COMPLETEDREQUEST, and all messages in HISTORY, the Completed
        %     property will be set only if the message contains no body or Body.Payload
        %     contains the raw data that was sent or received. Normally the Payload is
        %     not preserved unless HTTPOptions.SavePayload is set.
        %
        %     If you need to send the same message multiple times, and it contains data
        %     that is time-consuming to convert (e.g., a very large JSON structure) you
        %     may wish to use complete to convert the data once (which converts
        %     Body.Data into Body.Payload) and send the completed request.
        %
        ...
        %   matlab.net.http.io.FileConsumer
            ...
            if (isempty(options.ProxyURI) && isempty(options.Credentials)) || ~options.Authenticate
                % Don't authenticate if not requested or there are no Credentials,
                % and we're not talking to a proxy. In the proxy case we might end
                % up using options.ProxyCredentials that was set from preferences.
                [response, request, history] = ...
                    obj.sendOneRequest(connector, options, CredentialInfo.empty, CredentialInfo.empty, ...
                                       history);
            else
                ...
                %  a huge use case, where both the proxy and the server require password-based
                %  authentication.
                %
                %  authInfo = []
                %  proxyAuthInfo = []
                ...
                if ~isempty(options.ProxyURI)
                    ...
                    [response, request, history] = ...
                        obj.sendAndAuthenticate([], completedURI, options.ProxyURI, connector, ...
                    ...
                    if response.StatusCode ~= StatusCode.ProxyAuthenticationRequired
                        if response.StatusCode == StatusCode.Unauthorized
                            ...
                            [response, request, history] = ...
                                 obj.sendAndAuthenticate(response, completedURI, options.ProxyURI, connector, ...
        ...
        %   URI of the proxy (which will contain only a Host and, if explicitly provided,
        %   the Port).
        %
    ...
    methods (Access=private)
        ...
        % or the proxy
            ...
            if options.UseProxy
                if isempty(options.ProxyURI)
                    ...
                    target = proxyURI;
                    % libcurl adds this when it knows message is to proxy
                    obj = obj.addIfEmpty(HeaderField('Proxy-Connection','Keep-Alive'));
                end
            else
                proxyURI = [];
                target = uri;
            ...
            % Add to test keep-alive connections
            %obj = obj.addIfEmpty(ConnectionField('keep-alive'),4);

            % Add Accept-Encoding unless there is one
            obj = obj.addIfEmpty(HeaderField('Accept-Encoding', 'gzip, deflate'), 6);
            ...
            if isempty(payloadLength) || payloadLength > 0
                ...
                if ~wasCompleted && any(obj.Method == bodyExpectedMethods)
                    % These methods expect a payload
                    if isempty(obj.getFields('Content-Length'))
                        % warn unless user explicitly set Content-Length
                        % field (whose value we expect to be zero)
            ...
            % fields has prevented them from being added.  However, leave intact
            % empty-valued fields that curl might add, so we'll pass these down
            % to tell curl not to add them.
            curlFields = ["Expect" "Authorization" "Proxy-Authorization" "Cookie"];
            obj.Header(arrayfun(@(x) isempty(x.Value) && ~any(strcmpi(x.Name,curlFields)), obj.Header)) = [];

            % Now we only have the real fields left in the message plus fields we
            % want curl to remove
        ...
        function [response, request, history] = ...
                   sendAndAuthenticate(obj, response, hostURI, proxyURI, connector, options, ...
                                       forProxy, history)
        % Send a message and authenticate if necessary, using options.Credentials

            import matlab.net.http.*
            import matlab.net.http.internal.CredentialInfo
            ...
            while (~done)
                ...
                [hostCred, hostCredInfo] = creds.getBestCredInfo(hostURI, authInfos, false);
                if forProxy
                    credURI = options.ProxyURI;
                    [proxyCred, proxyCredInfo] = creds.getBestCredInfo(options.ProxyURI, authInfos, forProxy); % step 1
                    credInfo = proxyCredInfo;
                    cred = proxyCred;
                else
                    proxyCredInfo = CredentialInfo.empty;
                    credURI = hostURI;
                    credInfo = hostCredInfo;
                    cred = hostCred;
                    if isempty(credInfo)
                        connector.HostSchemes = [];
                    else
                        connector.HostSchemes = credInfo.AuthInfo.Scheme;
                    end
                end
                if ~isempty(proxyCredInfo) && ~isempty(proxyCredInfo.AuthInfo)
                    % Found a CredentialInfo, so try it proactively. This can
                    % work for Basic or Digest.  The only Scheme we want to try
                    % is the one last tried in the credInfo.
                    connector.ProxySchemes = proxyCredInfo.AuthInfo.Scheme;
                else
                    connector.ProxySchemes = AuthenticationScheme.getSupportedSchemes;
                end
                if ~isempty(hostCredInfo) && ~isempty(hostCredInfo.AuthInfo)
                    connector.HostSchemes = hostCredInfo.AuthInfo.Scheme;
                else
                    connector.HostSchemes = AuthenticationScheme.getSupportedSchemes;
                end

                if ~isempty(credInfo) % step 2
                   [response, request, done, history] = ...
                        obj.sendWithCredInfo(credURI, connector, options, cred, hostCredInfo, ...
                    ...
                    while (~done)
                        ...
                        [newCreds, cred, schemes] = creds.getCredentials(credURI, authInfos); % 3.1
                        if forProxy
                            % if authenticating to a proxy, also get the host schemes
                            % and cred so we can provide that, based only on the URI
                            [~, hostCred, hostSchemes] = creds.getCredentials(hostURI, []); % 3.1
                        else
                            hostCred = [];
                        end
                        if isempty(authInfos) && (~forProxy || ~strcmpi(connector.URI.Scheme, 'https'))
                            % First time through, don't allow Basic unless the cred
                            % we selected is Basic-only or this is https to a proxy.  The latter
                            % is needed because https causes curl to send a CONNECT request, and
                            % when CONNECT comes back with a 407 response, curl reports an error, which
                            % throws an exception, rather than giving us a challenge to
                            % which we could reply with authorization.
                            if isempty(cred) || ~all(cred.Scheme == AuthenticationScheme.Basic)
                                schemes(schemes == AuthenticationScheme.Basic) = [];
                            end
                        end
                        % Set all possible schemes allowed
                        if forProxy
                            proxyCredInfo = credInfo;
                            connector.ProxySchemes = schemes;
                            connector.HostSchemes = hostSchemes;
                        else
                            proxyCredInfo = CredentialInfo.empty;
                            connector.HostSchemes = schemes;
                        end
                        if ~isempty(cred) % 3.2
                            if forProxy && strcmpi(connector.URI.Scheme, "https")
                                % if we authenticating to a proxy and we have proxy credentials, construct
                                % a CredentialInfo to pass in this information. This is needed because, in the case
                                % of HTTPS, curl wants to authenticate to the proxy all by itself, without
                                % giving us the chance to respond to a Proxy-Authenticate response.
                                connector.ProxyCredentialInfo = CredentialInfo(AuthInfo.empty, ...
                                    connector.ProxyURI, cred.Username, cred.Password, forProxy);
                            end
                            if cred.Scheme == AuthenticationScheme.Basic % 3.2.1
                                ...
                                [response, request, done, history] = ...
                                    obj.sendWithBasic(response, connector, ...
                                      options, authInfos, ...
                                      hostURI, proxyURI, creds, cred, hostCred, forProxy, history);
                                ...
                                % credentials if response is empty. (But in the case of proxy HTTPS,
                                % we do send credentials from the start.)
                                [response, request, done, history] = ...
                                   obj.sendAfterChallenge(response, connector, ...
                                   options, authInfos, hostURI, proxyURI, cred, hostCred, forProxy, history);
                            ...
                            if forProxy
                                ...
                                hostCredInfo = CredentialInfo.empty;
                            end
                            if isempty(response)
                                [response, request, history] = obj.sendOneRequest(...
                                         connector, options, hostCredInfo, proxyCredInfo, history);
                            end
                            done = true;
                        end
                        if ~isempty(authInfos)
                            % first time, keep using original creds; subsequently
                            % use modified creds based on authInfos
                            creds =  newCreds;
                        end
                    end
                end
        ...
        function [response, request, history] = sendOneRequest(obj, connector, options, ...
                                                            hostCredInfo, proxyCredInfo, history)
        ...
        % forProxy set, credInfo applies to the proxy, not the server.
            ...
            import matlab.net.http.internal.*
            e = [];
            if(~isempty(proxyCredInfo))
                connector.ProxyCredentialInfo = proxyCredInfo;
            end
            ...
            % flag to check if we saved the REQUEST object from HISTORY
            % returned by CONNECTOR.SENDREQUEST so that it can be reused
            % later.
            requestFromHistoryExists = false;
            try
                ...
                [~, history] = connector.sendRequest(hostCredInfo, history);
                ...
                if ~isempty(history(end).Request)
                    ...
                    % save the last request to return as completed request,
                    % for returning the REQUEST object to the user.
                    request = history(end).Request;
                    requestFromHistoryExists = true;
                end
            catch e
            ...
            if nargout > 1 || log || ~isempty(e)
                ...
                try
                    if ~requestFromHistoryExists
                        % the request object does not exist, so get a new one from
                        % HTTPConnector.
                        try
                        ...
                        if ~isempty(obj.Body) && isempty(provider)
                            ...
                            % have the original Data, so put that into the message if there
                            % is any.
                ...
                if ~isempty(e)
                    ...
                    if (connector.HeadersSent)
                        ...
                        history(end).Response = connector.getResponseNoBody();
            ...
            try
                ...
                savePayload = nargin > 2 && (options.Debug ~= 0 || options.SavePayload);
        ...
        function [response, request, done, history] = ...
             sendWithCredInfo(obj, uri, connector, options, cred, hostCredInfo, ...
                              credInfo, forProxy, history)
        ...
        %       should try again to find another Credential or CredentialInfo.
        %
        ...
        % unsuccessful case, the last challenge.
            ...
            [response, request, history] = obj.sendOneRequest(connector, options, ...
                                                hostCredInfo, proxyCredInfo, history);  % 2.1
                ...
                if isBasicChallenge(authInfos, cred) % 2.3.1
                    ...
                    if pathCompare(uri,credInfo.URIs) % 2.3.1.1
                        ...
                        if isempty(credInfo.AuthInfo)
                            ...
                            [response, request, history, done] = ...
                                obj.sendWithNewCredInfo(cred, uri, response, ...
                                   ...
                                   history, true, credInfo, hostCredInfo);
                        else
                        ...
                        if ~isempty(cred.GetCredentialsFcn) % 2.3.1.2.1
                            ...
                            % CredentialInfo from this same cred
                            [response, request, history, done] = ...
                                obj.sendWithNewCredInfo(cred, uri, response, ...
                                   authInfos, forProxy, connector, options, ...
                                   history, true, [], hostCredInfo);
                        else
                            % No GetCredentialsFcn; perhaps there's another cred
                            % to try, based on the authInfos
                            done = false;
                        end
                    end
                else
        ...
        function [response, request, done, history] = ...
                 sendAfterChallenge(obj, response, connector, options, authInfos, ...
                                    hostURI, proxyURI, cred, hostCred, forProxy, history)
        ...
        % find a better Credentials object matching the information in authInfos.
        %
        ...
        % or give up if there aren't any.
            ...
            if isempty(response)
                ...
                % remove Digest and Basic as options since we're not providing username/password
                % However in the case of proxy https we are providing username/password
                % proactively, so don't do this.
                if ~forProxy || connector.URI.Scheme ~= "https"
                    isBorD = @(x) x == AuthenticationScheme.Basic | x == AuthenticationScheme.Digest;
                    if forProxy
                        connector.ProxySchemes(isBorD(connector.ProxySchemes)) = [];
                    else
                        connector.HostSchemes(isBorD(connector.HostSchemes)) = [];
                    end
                end
                [response, request, history] = obj.sendOneRequest(connector, options, ...
                    CredentialInfo.empty, CredentialInfo.empty, history); % 3.2.2.1.1
        ...
        function [response, request, done, history] = ...
                 sendWithBasic(obj, response, connector, options, authInfos, hostURI, proxyURI, ...
                              creds, cred, hostCred, forProxy, history)
        ...
        % finish. If done is false, we updated creds and caller should try again.
           ...
           if ~isempty(hostCred)
               hostCredInfos = hostCred.getCommonPrefixCredInfos(hostURI);
           else
               hostCredInfos = [];
           end
           for i = 1 : length(credInfos)
               credInfo = credInfos(i);
               if isempty(hostCredInfos)
                   % If no host credentials, this is for proxy authenticaiton only.
                   % Try each credInfo in cred that matches until success
                   [response, request, history] = obj.sendOneRequest(connector, ...
                                options, credInfo, CredentialInfo.empty, history); % 3.2.1.2.1
                   if authenticationSuccess(response, forProxy)
                       % It works; trim the existing credInfo to contain just the common
                       % prefix
                       credInfo.chopCommonPrefix(uri); % 3.2.1.2.2.1
                       credInfo.LastUsed = datetime('now');
                       return;
                   end
               else
                   % If there are host credentials, do exactly as above for each host
                   % credential for each proxy credential
                   for j = 1 : length(hostCredInfos)
                       [response, request, history] = obj.sendOneRequest(connector, ...
                                options, credInfo, hostCredInfos(j), history); % 3.2.1.2.1
                       if authenticationSuccess(response, forProxy)
                           % It works; trim the existing credInfo to contain just the common
                           % prefix
                           credInfo.chopCommonPrefix(uri); % 3.2.1.2.2.1
                           credInfo.LastUsed = datetime('now');
                           hostCredInfos(j).chopCommonPrefix(hostURI);
                           hostCredInfos(j).LastUsed = datetime('now');
                           return;
                       end
                   end
               end
           ...
           if ~isempty(response)
               ...
               if isempty(authInfos)
                   ...
                   if all(ais == lower(string(AuthenticationScheme.Basic)))
                       ...
                       if isscalar(cred.Scheme) && cred.Scheme ~= AuthenticationScheme.Basic
        ...
        function [response, request, history, done] = sendWithNewCredInfo(obj, ...
                               ...
                               connector, options, history, forBasic, origCredInfo, hostCredInfo)
        ...
        % challenges. If successful, add the new CredentialInfo unconditionally.
           ...
           while true
               ...
               elseif isnumeric(credInfo) && credInfo == 0
                   ...
                   % if we have a credInfo, remove any schemes that aren't among the authInfos.
                   % This is necessary because allowing other schemes can prevent the desired
                   % scheme from being used.
                   if forProxy
                       connector.ProxySchemes = intersect(connector.ProxySchemes, [authInfos.Scheme]);
                   else
                       connector.HostSchemes = intersect(connector.HostSchemes, [authInfos.Scheme]);
                   end
               end
               if (forProxy)
                   proxyCredInfo = credInfo;
               else
                   hostCredInfo = credInfo;
                   proxyCredInfo = CredentialInfo.empty;
               end
               [response, request, history] = ...
                   obj.sendOneRequest(connector, options, ...
                                      hostCredInfo, proxyCredInfo, history); % 2.3.1.2.1.2, 3.2.2.2.1, 3.2.1.3.2
    ...
    methods (Static, Access=protected)
        ...
        %    TBD doesn't check Body.Data, so not a perfect test. If uint8 or char, should check that it's a
...
%
%   Purges unknown schemes from the list (ones where AuthInfo.Scheme is not an
...
%   Returns [] if there are no AuthenticateFields or no challenges match scheme.
    ...
    if isempty(fields)
        ...
        if nargin > 1
            ...
            % remove those whose schemes we don't know about
            authInfo(arrayfun(@(a) ~isa(a.Scheme, 'matlab.net.http.AuthenticationScheme'), authInfo)) = [];
        end
    end
end
...
% Test whether the authInfos and cred intersect only in Basic
    ...
    % [authInfo.Scheme] operation could error out if they are mixed types, so convert
    % all to strings for comparison
    basicScheme = matlab.net.http.AuthenticationScheme.Basic;
...
% Returns [] if there is no proxy setting
    ...
    if(~isempty(proxyInfo.Host))
        proxy = matlab.net.URI();
        proxy.Host = char(proxyInfo.Host);
        proxy.Port = proxyInfo.Port;
        username   = char(proxyInfo.Username);
        password   = char(proxyInfo.Password);
    end
end
...
% find the special ProxyCredentials that matches all proxyURIs.
    ...
    if ~isempty(credInfo)
        ...
        if ~isempty(creds)
            ...
            cred = matlab.net.http.Credentials.empty;
        end
        if (isscalar(cred.Scope) && cred.Scope == proxyURI) || cred == options.ProxyCredentials
            % We got a match, and it has just one Scope that matches this proxyURI
            % exactly, or the match was with the ProxyCredentials.
            % Since we know it is intended for the proxy, force its
            % username and password to be the desired one.
            cred.Scope = proxyURI;
            cred.Username = username;
            cred.Password = password;
        else
            % Its scope is empty or has more than one URI, and it's not the
            % ProxyCredentials, and the username and password don't both match. In
            % this case create a new CredentialInfo under this Credential that
