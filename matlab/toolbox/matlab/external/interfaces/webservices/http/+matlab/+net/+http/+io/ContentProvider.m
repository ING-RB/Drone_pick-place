...
%     CurrentDelegate to the delegate and the delegate's MyDelegator to the current
%     provider (i.e., the delegator), calls the delegate's complete and start
%     methods, and returns a GetDataFcn function handle. Then the delegator's getData
%     method calls the delegate's GetDataFcn to obtain the data, possibly altering it
%     before returning it to MATLAB. Providers generally do not have to check whether
...
% Copyright 2016-2024 The MathWorks, Inc.
    ...
    properties (WeakHandle, Access=protected)
    ...
    methods (Access=protected)
        ...
        % See also complete, start, expectedContentLength, getData
            ...
            weakDelegate = matlab.lang.WeakReference(delegate);
            getDataFcn = @(len)weakDelegate.Handle.getDataInternal(len);
        end
    end
    ...
    methods (Access={?matlab.net.http.RequestMessage, ?matlab.net.http.internal.HTTPConnector, ?matlab.net.http.io.MultipartProvider})
        ...
        function completeInternal(obj, uri)
            ...
            if ~isscalar(obj.Request)
                ...
                validateattributes(obj.Request, {'matlab.net.http.RequestMessage'}, {'scalar'}, mfilename, msg.getString);
            end
