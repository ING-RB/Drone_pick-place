classdef (Sealed) URI
    ...
    properties (Dependent)
        ...
        %   the Path is not [] and there are nonempty components in the URI prior to
    ...
    properties (Dependent, Access=private)
        ...
        %   includes trailing ones. Used to simplify URI comparisons.
        Segments string
    ...
    methods
        ...
        % string before getting into these methods, and this coercion converts [] to
        % string.empty.
        ...
        %     followed by two hex digits representing the value of the octet. For
        ...
        %     directly, you will see the decoded version. This option does not permit
        %     you to construct an illegal URI: it only prevents re-encoding of '%'
        %     characters. Characters that must always be encoded, such as '\' and ' '
        %     in the Host or Path, will still be percent-encoded.
        %
    ...
    methods (Access= {?matlab.net.http.Credentials, ...
        ...
        function [len, isSubordinate] = matchLength(obj, uri)
        ...
        %
        % If the Scheme, UserInfo, Host, Port and Path are of this URI are identical to
        % uri, then returns double(intmax).
        %
        % It uses matchPath() to count matching path segments.
        %
        % If obj is a vector, returns vector of lengths.
        %
        % isSubordinate is true if uri is equal to or beneath this URI
        %
        ...
        % of RFC 2617.
            ...
            len(length(obj)) = 0;
            if nargout > 1
                isSubordinate(length(obj)) = false;
            end
            ...
            for i = 1 : length(obj)
                if isequal(this.Scheme, uri.Scheme) && ...
                   ...
                   isequal(this.Port, uri.Port)
                    if nargout == 1
                        nsegs = obj(i).matchPath(uri);
                    else
                        [nsegs, isSubordinate(i)] = obj(i).matchPath(uri);
                    end
                    len(i) = 1 + nsegs;
                else
                    len(i) = 0;
        ...
        function [len, isSubordinate] = matchPath(obj, uri)
        % matchPath compares segments of this URI.Path with uri.Path, left to right.
        %   len           number of matching segments, not counting root at head of path
        %   isSubordinate true if uri.Path is equal to or beneath obj.Path
        ...
        %
        % If the paths are the same (after the above canonicalization) returns intmax.
            [len, isSubordinate] = matchCount(obj.canonicalizePath(), uri.canonicalizePath());
        end
...
%         %  uriLen  number of segments in uri.Host
%
...
%                 oh = string.empty(1,0);
%             else
%                 oh = string(strsplit(obj.Host, '.'));
%             end
%             if isempty(uri.Host)
%                 uh = string.empty(1,0);
%             else
%                 uh = string(strsplit(uri.Host), '.');
%             end
%             if reverse
...
%         end
        ...
        function uris = resolve(obj, uris)
        % resolve returns the uri (which can be a vector) resolved against this URI.
        ...
        %   values from this URI.  In addition, if uri.Path is nonempty and relative,
        %   and it the replacement results in a nonempty Host, then make the Path
        %   absolute.
            for i = 1 : length(uris)
                uri = uris(i);
                if isempty(uri.Scheme)
                    uris(i).Scheme = obj.Scheme;
                    if isempty(uri.UserInfo), uris(i).UserInfo = obj.UserInfo; end
                    if isempty(uri.Host),     uris(i).Host = obj.Host;         end
                    if isempty(uri.Port),     uris(i).Port = obj.Port;         end
                    if uris(i).needsMakeAbsolutePath()
                        uris(i).Path = ["" uri.Path];
                    else
                    end
                else
                end
    ...
    methods (Static, Access={?matlab.net.http.field.HostField, ...
        ...
        % If str is empty the result is an empty URI.
            ...
            % don't want to misinterpret 'foo' as a scheme.
            res = regexp(str, '^([^:/?#]*:)?//','once');
            if isempty(res)
...
function [len, isSubordinate] = matchCount(a,b)
% Given 2 string vectors of possibly different length, return the number of contiguous
% matching elements starting from the first using ~= comparison. Returns 0 if
% one is empty or no matches. Returns intmax if all segments match or both are empty.
% If both paths start with the root (first element is empty string) don't count
% it.
% isSubordinate returns true if b is beneath a, i.e., all segments of a were
% matched to leading segments of b, or a is empty

    % search only up to the shortest one
    last = min([length(a) length(b)]);
    if last == 0
        % one is zero length
        len = 0;
        if length(a) == length(b)
            len = intmax; % both empty means identical
        end
        % all paths b are subordinate to any path a at the top level (length 0)
        isSubordinate = isempty(a);
    else
        % find first nonmatching element
        matchLen = find(a(1:last) ~= b(1:last), 1);
        if isempty(matchLen)
            % all elements match
            if last == length(a) && last == length(b)
                % a and b match completely
                len = intmax;
                isSubordinate = true;
            else
                % shorter one completely contained in longer one
                len = last;
                isSubordinate = len == length(a); % shorter one is a
            end
        else
            % match only in some elements
            len = matchLen - 1;
            isSubordinate = false;
        end
        % if we have a match > 0, and the first segment is empty (root)
        % subtract one because we don't count the root
        if len > 0 && a(1) == ""
            len = len - 1;
        end
    end
