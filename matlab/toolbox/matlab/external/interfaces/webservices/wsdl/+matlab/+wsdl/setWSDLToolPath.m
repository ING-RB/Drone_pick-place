...
%   only need to specify these once.  The arguments may be strings or character
%   vectors.
%
%   Values of TOOL are:
%
...
% warned        don't issue warning if set
    ...
    if isempty(version) 
        ...
        version = version{1}{1};
        if ~contains(version, versionNeeded)
