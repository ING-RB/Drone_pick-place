function classFcn = createWSDLClient(wsdlURL, varargin)
    ...
    %   web service based on a WSDL specification.  WSDLURL is a string or character
    %   vector naming a URL or a file path to a WSDL document.  This function creates
    %   a MATLAB class file for each SOAP binding in the WSDL and possibly a support
    %   package.  The generated MATLAB files and folders are placed in the current
    %   folder.  Any previous files with the same names as the generated ones will
    %   be overwritten. CLASSFCN is a handle to the class that was created, or a
    %   cell array of handles if multiple classes were created.  Use 'help' on the
    %   class name to get more information on usage.
    %
    ...
    if nargin > 0
        wsdlURL = convertStringsToChars(wsdlURL);
    end
    ...
    try 
        [abspath, localWSDL, istemp] = wsdlread(wsdlURL,webOptions);
    catch e
    ...
    if ~exist(javaTempDir,'dir') || status ~= 0 || ...
            ...
            (~isempty(msg) && contains(msg,'Error'))
    ...
    % collection
    [R, schemaCol] = parseWsdl(abspath, wsdlURL, fullPkgName, javaTempDir, pkgName);
    clear cleanWSDL
    ...
    if ~isempty(serviceClass)
        ...
        try 
            ...
            serviceJar = strrep(string(serviceJar.getCanonicalFile),'%20',' ');
            if ~serviceJar.equals(absJarPath)
        ...
        if(isa(e, 'matlab.exception.JavaException'))
            ...
            if ~isempty(exception)
                if regexp(exception, ...
                    ...
                    error(message('MATLAB:webservices:ConnectionRefused',wsdlURL, e.ExceptionObject.getLocalizedMessage()));
                end
                if regexp(exception, ...
...
function [getters, classesUsed] = makeClasses(dirName, pkgName, wsdl, R, schemaCol, ...
    ...
    for i = 1 : length(javaClasses)
        ...
        for l = 1 : length(template)
            line = template{l};
            isNeedsLine = contains(line,'${NEEDS');
            if contains(line,'$}$')
                copyLines(end) = [];
            elseif isNeedsLine
    ...
    for i = 1 : length(xmlSchemas)
        ...
        for j = 0 : soc.getCount-1
                    ...
                    if isEnumeration(restriction)
                        ...
                        for ifacet = 1 : facets.getCount
                            ...
                            if isSchemaType(facet, 'EnumerationFacet')
                                ...
                                fields(end+1).name = mangleName(value,0); %#ok<AGROW>
            ...
            elseif isSchemaType(item, 'ComplexType')
                ...
                if ~isempty(javaClass) && (isSchemaType(part, 'Sequence') || isSchemaType(part, 'All'))
                    ...
                    for k = 0 : xmlFields.getCount-1
                        ...
                        if isSchemaType(element, 'Element')
                            fields(end+1).name = mangleName(char(element.getName),0); %#ok<AGROW>
    ...
    if res
        ...
        if isSimple 
            if isSchemaType(restriction, 'SimpleTypeRestriction')
                ...
                if ~contains(char(restrictionQName.toString),'XML')
