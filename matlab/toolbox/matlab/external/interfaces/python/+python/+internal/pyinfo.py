...
r"""Query Python for the information necessary to initialize an embedded interpreter.

Example:
  C:\Python311\python.exe env_data.py
"""
import sys
import platform
import os
...
class LibraryFinder(object):
    ...
    def _check_lib_folder(self):
        ...
        for home in set(possible_homes):
            # Check home folder
            abpath = self._check_folder(home)
            if abpath is not None:
                return abpath
            # Check home/lib folder
            lib_folder = os.path.join(home, 'lib')
            abpath = self._check_folder(lib_folder)
            if abpath is not None:
                return abpath
            # Check home/bin folder
            lib_folder = os.path.join(home, 'bin')
            abpath = self._check_folder(lib_folder)
            if abpath is not None:
                return abpath
...
def main():
    ...
    try:
        ...
        #Not much to do here, probably Python 2.7. Version support is handled in C++ layer
        pass
    sys.stdout.write('\n'.join(data))
