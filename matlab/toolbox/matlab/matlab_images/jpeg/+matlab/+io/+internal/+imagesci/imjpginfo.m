...
[info,exif_offset, idx] = matlab.io.internal.imagesci.imjpgbaselineinfo(fid, "readXmpData", options.readXmpData);
...
raw_tags = matlab.io.internal.tifftagsread(filename, offset, 0, 1);

exif = matlab.io.internal.tifftagsprocess ( raw_tags );
if isempty(exif)
    return;
end
...
if numel(idx) > 1
    raw_tags = matlab.io.internal.tifftagsread(filename, offset, 1, 1);
    thumbnail = matlab.io.internal.tifftagsprocess(raw_tags);
else
    thumbnail = [];
end
