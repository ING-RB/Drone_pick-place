function [info, exif_offset, ifd_idx] = imjpgbaselineinfo(fid, options)
...
%
%   INFO = IMJPGBASELINEINFO(FILENAME,"readXmpData",true) returns a structure with 
...
% Import the matlab.io.xml.dom package
...
while 1
    ...
    switch (marker)
        ...
        case 65505
            % FFE1:  APP1 Application segment (maybe Exif or XMP)
            ...
            % Check if reading XMP data is needed
            if options.readXmpData
                % Check for XMP metadata
                if strcmp(deblank(identifier),'http:/')
                    string_id = char(fread(fid,22,'char'));
                    % Verify the whole identifier string for XMP
                    if(strcmp(deblank(string_id'),'/ns.adobe.com/xap/1.0/'))

                        XMPposition = ftell(fid);

                        % Invoke fopen on the file id to resolve the full path name
                        filename = fopen(fid);
                        % fopen the file with UTF-8 encoding
                        fid2 = matlab.internal.fopen(filename,'r','ieee-be', 'UTF-8');

                        % Ensure we clean up fid2 when finished
                        cfid = onCleanup(@()fclose(fid2));

                        % Seek to the end of the XMP identifier
                        fseek(fid2, XMPposition, 'bof');
                        raw_data =  transpose(fread(fid2,marker_length,'*char'));
                        % Find the start of the XMP packet
                        start_index = strfind(raw_data,'<?xpacket begin');
                        % Find the end of the XMP packet
                        % Both '<?xpacket end="r">' and '<?xpacket end="w"'>
                        % have same length and begin with same phrase '<?xpacket end'
                        end_index = strfind(raw_data,'<?xpacket end')+strlength('<?xpacket end="r"?>');

                        xmp_packet = raw_data(start_index:end_index-1);
                        % xmp_packet will be empty if the XMP identifier exists
                        % and either the start and/or the end is not found
                        if (isempty(xmp_packet))
                            warning(message('MATLAB:imagesci:imfinfo:unknownXMPpacket'));
                            continue;
                        end

                        try
                            % pass XMP packet to XML parser
                            xml_parsed = parseString(Parser,xmp_packet);

                            % pass XML object to XMP metadata extractor
                            info.XMPData = matlab.io.internal.imagesci.getXMPMetadata(xml_parsed);
                        catch
                            warning(message('MATLAB:imagesci:imfinfo:badXMPpacket'))
                            continue;
                        end

                        if(isempty(fieldnames(info.XMPData)))
                            % if there is no XMP Data in image, remove field
                            info = rmfield(info,'XMPData');
        ...
        case 65534
            ...
            ucomment = native2unicode(comment', 'UTF-8');
            if any(ucomment == 65533)
                % It's not UTF-8
                ucomment = native2unicode(comment', matlab.internal.i18n.locale.legacy.Encoding);
            end
            info.Comment{com_marker_count,1} = ucomment;
