classdef ComponentAdapter < appdesigner.internal.componentadapterapi.ComponentRegistration
    ...
    properties (Access=public)
        % by default codegen properties are cached for better lookup
        % performance, some components (user authored) should not be cached
        % and should get the latest every time
        shouldCacheCodegenProperties = true;
        
        % by default component callback properties names are cached. User
        % authored components should not be cached and get the latest every
        % time (see g3386570)
        shouldCacheCallbackPropertyNames = true;
    end
