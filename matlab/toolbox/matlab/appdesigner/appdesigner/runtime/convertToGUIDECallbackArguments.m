...
function handles = localCreateHandles(appFigure)
    ...
    % Find all components with Tag not empty and with a Type property.
    % It is important to filter out components without a Type property (g2493633)
    migratedComponents = findall(appFigure, '-not', 'Tag', '', '-property', 'Type');
    handles = struct();
