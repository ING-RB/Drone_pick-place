classdef (Sealed) ReleaseUtil
    ...
    methods (Static, Access=public)
        ...
        function isEarlier = isEarlierThanCurrentRelease(release)
            ...
            isEarlier = ReleaseUtil.isEarlierThan(release, ReleaseUtil.getCurrentRelease());
        end
        ...
        function isEarlier = isEarlierThan(release1, release2)
            % Returns true if release1 is earlier than release2.
            import appdesigner.internal.serialization.util.ReleaseUtil;

            year1 = ReleaseUtil.getReleaseYear(release1);
            year2 = ReleaseUtil.getReleaseYear(release2);

            if year1 == year2
                isEarlier = ReleaseUtil.getReleaseLetter(release1) < ReleaseUtil.getReleaseLetter(release2);
            else
                isEarlier = year1 < year2;
            end
        end
        ...
        function valid = isValidReleaseString(release)
            % Returns true for strings in the format 'RXXXX(a|b)' as long
            % as the year is >= 2016.
            import appdesigner.internal.serialization.util.ReleaseUtil;
            year = ReleaseUtil.getReleaseYear(release);
