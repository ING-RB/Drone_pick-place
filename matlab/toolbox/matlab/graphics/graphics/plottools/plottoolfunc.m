...
try
    ...
    case 'getPlotManager'
        pm = matlab.graphics.annotation.internal.getplotmanager;
...
% 'hold on' is called before the plot is made.
...
axList = findall(fig,'-regexp','Type','.*axes','-and', '-isa','matlab.graphics.axis.Axes',...
    '-or','-isa','matlab.graphics.axis.PolarAxes');
ax = [];
isHandleInvisible = false;
...
if ~isempty(ax) && strcmp(ax.HandleVisibility,'off')
    isHandleInvisible = true;
end
set (fig, 'units', oldUnits);
if isempty (ax) 
    ax = get(fig, 'CurrentAxes');
    if ~isempty(ax) && ismember(ax,axList)
        % Make sure gca is  part of axList so we do not try to drop 
        % onto an axes in an Orange Chart (1881217)
...
out = { ax, is3d, ispolar, isHandleInvisible};
...
for i = 1:numel(existingAxes)
    if isa(existingAxes(i).Parent,'matlab.graphics.layout.Layout')
        showErrorDialog(getString(message('MATLAB:plottools:DlgLayoutNotSupported')));
        return
    end
end
...
for i = 1:numPlots
    if i <= length (siblingAxes)
        ...
        isPositionableChart = isa(ax,'matlab.graphics.chart.internal.SubplotPositionableChart') || ...
                              isa(ax,'matlab.graphics.chart.internal.PositionableChartWithAxes');
        if isa(ax,'matlab.graphics.chart.Chart') && ~isPositionableChart
