...
% Find scribe layer
[scribeax, child_scribeaxes] = matlab.graphics.annotation.internal.findAllScribeLayers(fig);
...
for k=1:length(scribeax)
    if ((isobject(scribeax(k)) && isvalid(scribeax(k))) || ....       
        ...
        % Only allow matlab.graphics.shape.internal.ScribeObject children
        % to be selected to exlude the Axes Toolbar (which should not be
        % selected by scribeSelectAll)
        scribeChildren = findobj(scribeax(k),'-depth',1,'-isa','matlab.graphics.shape.internal.ScribeObject');
        if isempty(shapes)
