...
serialized = matlab.graphics.annotation.internal.getCopyStructureFromObject(hAxes);
...
for i =1:length(hAxesVector)  
    if isequal(proxyVal, plotedit({'getProxyValueFromHandle',hAxesVector(i)}))        
        serialized = matlab.graphics.annotation.internal.getCopyStructureFromObject(hAxesVector(i));
        vectorSerialized = getappdata(0,'ScribeAxesCopyBuffer');
...
hAx = matlab.graphics.annotation.internal.getObjectFromCopyStructure(vectorSerialized(id));
