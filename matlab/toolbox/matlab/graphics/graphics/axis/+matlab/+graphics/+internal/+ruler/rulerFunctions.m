...
if nargs > 0 && isscalar(args{1})  ...
        && (isgraphics(args{1},'matlab.graphics.axis.AbstractAxes') || ...
...
if isa(ax,'matlab.graphics.chart.Chart')
    ...
    fh = str2func(func);
    ruler = ax;
elseif ~isequal(axle,'C') && (isa(ax,'matlab.graphics.axis.GeographicAxes') || isa(ax,'map.graphics.axis.MapAxes'))
    throwAsCaller(MException(message('MATLAB:Chart:UnsupportedConvenienceFunction', func, ax(1).Type)));
...
if ~isempty(mode) && matlab.graphics.internal.isCharOrString(val)
    switch lower(val)
        ...
        case {'tickaligned','tight','padded'}
            if strcmp(family,'lim') && all(isprop(ax,[axle 'LimitMethod']),'all')
                set(ax,[axle 'LimitMethod'],val);
                set(ruler,mode,'auto');
                return
            end
        case 'method'
            if strcmp(family,'lim') && all(isprop(ax,[axle 'LimitMethod']),'all')
                out{1} = get(ax,([axle 'LimitMethod']));
                return
            end
    end
end
...
try
    switch family
        case {'lim','ticks','tickangle','scale'}
            set(ruler,prop,val);
        case 'ticklabels'
            matlab.graphics.internal.ruler.setTickLabel(ruler, val);
        case 'tickformat'
            thetaflag = axle == "Theta" && ax.ThetaAxisUnits == "degrees";
            matlab.graphics.internal.ruler.setTickFormat(ruler, val, thetaflag);
    end
catch err
    % Check for recognized errors and replace them with custom error
    % messages specific to the convenience function.
    err = swapKnownErrorIDs(err, axle, val, ax);

    % Exclude LimitMethod options in error message from polar axes and
    % colorbar rulers. Note that the R/Theta options can be removed when
    % LimitMethods are documented for polaraxes.
    if strcmp(err.identifier,'MATLAB:rulerFunctions:InvalidLimitsMode') && ...
            (strcmp(axle,'R') || strcmp(axle,'Theta') || ~isprop(ruler,[axle 'LimitMethod']))
        msg=message('MATLAB:rulerFunctions:InvalidLimitsModeOnly').getString;
        err=MException('MATLAB:rulerFunctions:InvalidLimitsMode', ...
            msg);
    end
...
switch axle
    case 'x'
        ...
        if strcmp(family,'tickangle') || strcmp(family,'scale')
            % thetatickangle, thetascale, and rscale do not exist
    ...
    case 'y'
        % This function does not work on a polar axes.
        if strcmp(family,'scale')
            % rscale & thetascale do not exist
            err = MException(message('MATLAB:rulerFunctions:PolarAxes',func));
        else
...
function err = swapKnownErrorIDs(err, axle, val, ax)
...
switch err.identifier
    ...
    case 'MATLAB:graphics:DurationRuler:Format'
        ...
        % These error messages occur when you try to set the Scale property to
        % an invalid value.
    case {'MATLAB:datatypes:InvalidEnumValue', 'MATLAB:datatypes:InvalidEnumValueFor'}
        err = MException(message('MATLAB:rulerFunctions:InvalidValueScaling'));
end
