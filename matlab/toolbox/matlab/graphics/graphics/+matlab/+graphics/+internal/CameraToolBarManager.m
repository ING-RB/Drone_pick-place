classdef CameraToolBarManager < handle
    ...
    properties(Transient, Hidden)
        ...
        PrincipalAxisHandlesTags string
        ModeHandles matlab.ui.container.toolbar.ToggleTool
        ModeHandlesTags string
    ...
    properties(Transient, Hidden)
        ...
        codeGenProperties cell = {"CameraPosition","CameraTarget","CameraUpVector","CameraViewAngle","Projection"}
    end
    ...
    methods
        function h=createToolbar(hObj, hfig)      
            ...
            isMOLDockedFigure = hObj.isRemoteClient();
            if isMOLDockedFigure
                % Do not create toolbar buttons for morpable figures in the 
                % Live Editor because camera toolbar is not displayed in Live
                % Editor figure outputs. Do not include them in MOL because
                % it uses a figure toolstrip
                hObj.ModeHandlesTags = ["orbit", "orbitscenelight", "pan", "dollyhv",...
                    "dollyfb", "zoom", "roll"];
                hObj.PrincipalAxisHandlesTags = ["x","y","z","none"];
                h = [];
            else
                if ~strcmpi(hfig.DefaultTools, 'toolstrip')

                    % Creating toolbar with empty parent so that MGG can access Tag when added to figure
                    h = uitoolbar('Parent', [], 'HandleVisibility', 'off', 'Tag', 'CameraToolBar');
                    h.Parent = hfig;
                    props.Parent = h;
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'orbitCamera.svg');
                    props.Tag = 'orbit';
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'sceneLight.svg');
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'tiltCamera.svg');
                    props.Tag = 'pan';
                    u(end+1) = uitoggletool(props);
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'panCamera.svg');
                    props.Tag = 'dollyhv';
                    u(end+1) = uitoggletool(props);
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'xAxisView.svg');
                    props.Tag = 'x';
                    u(end+1) = uitoggletool(props,...
                        'Separator', 'on');
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'yAxisView.svg');
                    props.Tag = 'y';
                    u(end+1) = uitoggletool(props);
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'zAxisView.svg');
                    props.Tag = 'z';
                    u(end+1) = uitoggletool(props);
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', '3dAxis.svg');
                    props.Tag = 'none';
                    u(end+1) = uitoggletool(props);

                    hObj.PrincipalAxisHandles = u;
                    hObj.PrincipalAxisHandlesTags = string(get(hObj.PrincipalAxisHandles,'Tag'));
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'lightBulbUI.svg');
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'orthographicProjection.svg');
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'perspectiveProjection.svg');
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'restore.svg');
                    props.Tag = 'reset';
                    u(end+1) = uipushtool(props,...
                    ...
                    props.Icon = fullfile(matlabroot, 'ui', 'icons', '16x16', 'stop.svg');
                    props.Tag = 'stopmoving';
                    u(end+1) = uipushtool(props);

                    hObj.stopMovingHandle = u;

                    set(hObj.mainToolbarHandle, 'Visible', 'off','serializable','off');
                end
            end
        end
        ...
        function updateToolbar(hObj, hfig)
            ...
            % Update Figure toolstrip with new Camera settings
            if strcmpi(hfig.DefaultTools, 'toolstrip')
            ...
            if ~isempty(hObj.mode)
                matlab.graphics.interaction.internal.disableAllWebAxesModes(hfig);
                initWindowCallbacks(hObj, hfig);
            end
        end
                
        function result = isRemoteClient(~)
    ...
    methods
        function down_callback(hObj, hfig, evd, set_motion_fcn)
            ...
            % If we clicked on a UIComponent object or a datatip, return
            if isa(h, 'matlab.ui.control.internal.model.ComponentModel') ||...
                    isa(h,'matlab.graphics.shape.internal.ScribePeer')
            ...
            if isprop(h,'ButtonDownFcn')
                hObj.ButtonDownFcn = get(h,'ButtonDownFcn');
                set(h,'ButtonDownFcn',[]);
            end
        ...
        function up_callback(hObj, hfig,haxes)
            ...
            if isprop(hObj.CurrentObj,'ButtonDownFcn')
                set(hObj.CurrentObj,'ButtonDownFcn',hObj.ButtonDownFcn);
            end
            ...
            % Check to make sure the figure has not been destroyed. This
            % happens if the callback is delayed due to an animation.
            if (~isvalid(hfig))
                return
            end
            % generate camera code for any mouse/keyboard based interaction
            matlab.graphics.internal.propertyinspector.generatePropertyEditingCode(haxes, hObj.codeGenProperties)
        end
    ...
    methods
        function setmode(hObj, hfig, newmode)
            ...
            if strcmp(newmode, 'nomode')
                ...
                scribeclearmode(hfig, @(f)callnomode(hObj, f), hfig);
                matlab.graphics.interaction.internal.disableAllWebAxesModes(hfig);
                hObj.mode = newmode;
                hObj.wcb = getWindowCallBacks(hObj, hfig);
                hObj.cursor = getWindowCursor(hObj, hfig);
            ...
            % If the last toolbar stored in the closure has been deleted,
            % check and don't call nomode here
            function callnomode(hObj, fig)
                if isvalid(hObj)
                    hObj.nomode(fig);
                end
            end
        ...
        function setmodegui(hObj, hfig, newmode)
            %no-op if attempting to be called while toolbar is not active.
            if ~hObj.isOpen
                return;
            end
        ...
        function setcoordsys(hObj, hfig, newcoordsys)
            ...
            % generate code when axis is changed from toolstrip
            matlab.graphics.internal.propertyinspector.generatePropertyEditingCode(haxes, hObj.codeGenProperties)
            updateToolbar(hObj,hfig)
        end
        ...
        function nomode(hObj,hfig)
            ...
            if hObj.doContext && ~isempty(hObj.CurrentObj)
                set(hObj.CurrentObj,'UIContextMenu',hObj.UIContextMenu);
                set(hObj.CurrentObj,'ButtonDownFcn',hObj.ButtonDownFcn);
            end
        end
        ...
        function setprojection(hObj,hfig, proj)
            haxes = hfig.CurrentAxes;
            if matlab.graphics.internal.CameraToolBarManager.isValid3DAxes(haxes)
                camproj(haxes,lower(proj));
                % generate code for changing projection
                matlab.graphics.internal.propertyinspector.generatePropertyEditingCode(haxes, hObj.codeGenProperties)
            end
        end
        ...
        function resettarget(hObj,hfig)
            ...
            if matlab.graphics.internal.CameraToolBarManager.isValid3DAxes(haxes)
                ...
                validateScenelights(hObj,haxes)
                updateScenelightPosition(hObj,haxes);
                % Update generated code when user resets the camera
                matlab.graphics.internal.propertyinspector.generatePropertyEditingCode(haxes, hObj.codeGenProperties)
            end
        end
        ...
        function resetCameraProps(hObj,haxes)
            ...
            % update generated code when any camera property is reset (from context menu)
            matlab.graphics.internal.propertyinspector.generatePropertyEditingCode(haxes, hObj.codeGenProperties)
        end
        ...
        function dlgShown = showInfoDlg
            ...
            if ~CameratoolbarInfoDialogShown
                ...
                [~,dlgShown]=uigetpref('cameratoolbar','donotshowinfodlg',...
                    ...
                    'HelpFcn','helpview(''matlab'', ''axes_aspect_ratio'')',...
