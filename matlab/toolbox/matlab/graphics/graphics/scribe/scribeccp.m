...
if nargin == 1
    selobjs = unique(hMode.ModeStateData.SelectedObjects);
end
...
for i=1:length(selobjs)
    if isprop(selobjs(i), 'Axes') && ...
            ~isa(selobjs(i),'matlab.graphics.chart.Chart') && ...
            ~isa(selobjs(i),'matlab.ui.control.UIAxes')
    ...
    if isa(selobjs(i), 'matlab.graphics.illustration.Legend')
        matlab.graphics.illustration.legend.CreatePlotChildrenProxyValues(selobjs(i));
    end
    
    if isa(selobjs(i),'matlab.graphics.mixin.Legendable')
        matlab.graphics.illustration.legend.StorePlotChildrenProxyValues(selobjs(i));
    end
end

parentProxy = strings(size(hParents));
for i = 1:length(hParents)
    ...
    if ~isempty(parentP)
        ...
        parentProxy(i) = plotedit({'getProxyValueFromHandle',hParents{i},hMode});
    end
end
...
proxyList = strings(size(handles));
...
newObjs = ccpPaste(hMode,serialized,false,hParents,false);
% For undo/redo of annotations, localUndoCut performs undo operation which
% needs to register the interaction again
if any(isa(newObjs, 'matlab.graphics.shape.internal.OneDimensional')) || ...
   any(isa(newObjs, 'matlab.graphics.shape.internal.TwoDimensional'))
    
    if any(isa(newObjs, 'matlab.graphics.shape.internal.OneDimensional'))
        newObjs(~isa(newObjs, 'matlab.graphics.shape.internal.OneDimensional')) = [];
    elseif any(isa(newObjs, 'matlab.graphics.shape.internal.TwoDimensional'))
        newObjs(~isa(newObjs, 'matlab.graphics.shape.internal.TwoDimensional')) = [];
    end

    matlab.internal.editor.figure.generateToolstripLiveCode(ancestor(newObjs(1), 'figure'), matlab.internal.editor.figure.ActionID.ANNOTATION_ADDED);
    matlab.internal.editor.figure.generateToolstripLiveCode(newObjs, matlab.internal.editor.figure.ActionID.ANNOTATION_ADDED);
end
...
for i=1:length(selobjs)
    ...
    serialized{end+1} = matlab.graphics.annotation.internal.getCopyStructureFromObject(obj); %#ok<AGROW>
    ...
    elseif localIsAxesChild(obj) || localIsSubplotDecoration(obj)
        ...
        if ~isempty(pos)
            ...
            serialized{end}.ParentScribeProxyValue = matlab.graphics.internal.PlotEditModeUtils.getScribeProxyValue(obj.Parent);
        end
    end
...
while i<=len
    ...
    if isa(obj,'matlab.graphics.internal.Legacy')
        ...
        if isprop(obj, 'Axes') && (ishghandle(obj, 'colorbar') || ...
                ishghandle(obj, 'legend') || ishghandle(obj, 'bubblelegend'))&& any(selobjs == obj.Axes)
...
if ~isempty(hMode.ModeStateData.SelectedObjects)
    selAxes = findobj(unique(hMode.ModeStateData.SelectedObjects),'-function',@(obj)(isa(obj,'matlab.graphics.axis.AbstractAxes')),'-depth',0);
    selParent = findobj(unique(hMode.ModeStateData.SelectedObjects),'-function',@(obj)(isa(obj,'matlab.ui.container.Panel')),'-depth',0);
end
...
for i=1:length(serialized)
    ...
    % uicontainer.
    if ~isempty(hParents)
        if isgraphics(hParents(i),'matlab.graphics.axis.AbstractAxes')
            selAxes = hParents(i);
        end
    ...
    if localIsAxesChild(obj)
        ...
        if isempty(selAxes)
            for j=numel(selParent):-1:1
                if isfield(serialized{i},'specialParent')
                    currParent(j) = feval(serialized{i}.specialParent,'Parent',selParent(j));
                else
    ...
    if localIsAxesChild(obj(1))
        proxyVal = matlab.graphics.internal.PlotEditModeUtils.getScribeProxyValue(obj(1));
    ...
    for j=1:numel(obj)
        ...
        elseif ishghandle(obj(j), 'bubblelegend')
            % Need to undo deletion of BubbleLegend, account for just the
            % PlotChildren. 
            ...
            if isa(obj(j),'matlab.graphics.mixin.Legendable')
                ...
                if ~isempty(ax) && isvalid(ax)
                    ...
                    if ~isempty(leg) && isvalid(leg)
                        % update pcs / pce
                        pv = matlab.graphics.internal.PlotEditModeUtils.getScribeProxyValue(obj);
                        pcs_proxyvals = string(getappdata(leg,'PlotChildrenSpecifiedProxyValues'));
                        found = ismember(pv,pcs_proxyvals);
                        if found
                            % insert obj into its pre-deleted relative location in
                            % PlotChildren
                            tmpPCS = [leg.PlotChildrenSpecified; obj];
                            tmpPCS_proxyvals = arrayfun(@(x) matlab.graphics.internal.PlotEditModeUtils.getScribeProxyValue(x),tmpPCS);
                            ...
                            % If there are any PlotChildrenSpecified that
                            % are not part of PlotChildren, we must update
                            % PlotChildren. This happens in the case that
                            % AutoUpdate is set to 'off' and does not
                            % automatically add a deleted object's entry
                            % back into the legend.
                            if any(~ismember(leg.PlotChildrenSpecified, leg.PlotChildren))
                                %PlotChildrenSpecified must come first
                                %followed by other objects.
                                notSpecifiedChildren = ~ismember(leg.PlotChildren,leg.PlotChildrenSpecified);
                                leg.PlotChildren = [leg.PlotChildrenSpecified; leg.PlotChildren(notSpecifiedChildren)];
                            end
                        else
                            pce_proxyvals = string(getappdata(leg,'PlotChildrenExcludedProxyValues'));
                        ...
                        plotchildrenDeleted = string(getappdata(leg, 'PlotChildrenProxyValuesWhenAutoUpdateOff'));
                        if ~isempty(plotchildrenDeleted) && ismember(pv, plotchildrenDeleted)
                            tmpPC = [leg.PlotChildren; obj];
                            tmpPC_proxyvals = arrayfun(@(x) matlab.graphics.internal.PlotEditModeUtils.getScribeProxyValue(x),tmpPC);
                            pcs_proxyvals_reduced = plotchildrenDeleted(ismember(plotchildrenDeleted,tmpPC_proxyvals));
                            [~,d] = ismember(pcs_proxyvals_reduced,tmpPC_proxyvals);
                            leg.PlotChildren = tmpPC(d);



%                             leg.PlotChildren = [leg.PlotChildren; obj];
%                             leg.PlotChildrenMode = 'auto';
                        end
                    end
                end
            end
...
for currentAxes=1:length(selAxes)
    ...
    if ~isempty(bars)
        ...
        for kk=1:length(barPeerIDs)
            ...
            matlab.graphics.chart.internal.insertBars(selAxes(currentAxes), allsibBars, pastedBars, pastedBarChildPos);
...
if ~isempty(newObjs)
    if updateProxyList
        ...
        for j = 1:length(newHandles)
            matlab.graphics.internal.PlotEditModeUtils.setScribeProxyValue(newHandles(j),string.empty);
        end
        hMode.ModeStateData.ChangedObjectHandles = [hMode.ModeStateData.ChangedObjectHandles newHandles];
        proxyVals = matlab.lang.internal.uuid(1,length(newHandles));
        hMode.ModeStateData.ChangedObjectProxy = [hMode.ModeStateData.ChangedObjectProxy proxyVals];
        for j = 1:length(newHandles)
            matlab.graphics.internal.PlotEditModeUtils.setScribeProxyValue(newHandles(j),proxyVals(j));
        end
    else
        % Update the proxies appropriately:
        newHandles = unique(findall(newObjs),'legacy').';
        for j = 1:length(newHandles)
            if ~isempty(matlab.graphics.internal.PlotEditModeUtils.getScribeProxyValue(newHandles(j)))
                proxyVal = matlab.graphics.internal.PlotEditModeUtils.getScribeProxyValue(newHandles(j));
...
proxyVal = matlab.graphics.internal.PlotEditModeUtils.getScribeProxyValue(obj);
if isempty(proxyVal)
    return;
end
res = proxyVal == val;
...
%-------------------------------------------------------------------------%
function res = localIsSubplotDecoration(obj)

res = isa(obj,'matlab.graphics.illustration.subplot.Text');
...
for c = 1:numel(pc)
    ...
    proxyVal = matlab.graphics.internal.PlotEditModeUtils.getScribeProxyValue(oldChild);
...
% moved out of scribeccp into the Bar class
% in a future release.   
obj = matlab.graphics.annotation.internal.getObjectFromCopyStructure(serializedObj);
