...
%   location
%
%   COLORBAR(AX) creates a colorbar associated with axes AX
...
if ~isempty(args) && ~isempty(args{1}) && ...
        all(isgraphics(args{1},'matlab.graphics.axis.AbstractAxes'),'all')
    args = ['peer' args];
else
...
while ~done
    ...
    if ischar(arg)
        if strcmpi(arg,'peer')
            ...
            peeraxes = args{2};
            if ~(isscalar(peeraxes) && ...
                 isgraphics(peeraxes,'matlab.graphics.axis.AbstractAxes'))
                error(message('MATLAB:colorbar:InvalidPeer'));
            end
            peeraxes = handle(peeraxes);
            args(1:2) = [];
...
if isa(peeraxes,'matlab.graphics.chart.Chart')
    try
        if ~isempty(location) && ~strcmp(location, 'eastoutside')
            error(message('MATLAB:Chart:UnsupportedArgument','colorbar',peeraxes.Type));
        end
...
if mod(n,2)==0 % must be even
    for i=1:2:n
        ...
        elseif strcmpi(args{i},'Parent')
            ...
            elseif ~isa(handle(parent),'matlab.ui.internal.mixin.CanvasHostMixin')
                msg = message('MATLAB:colorbar:InvalidParent',get(parent,'Type'));
            end
            removeThese(i:i+1) = true;
        end
    end
else
