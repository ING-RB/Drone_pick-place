...
switch nargin
    ...
    case 1
        ...
        elseif ischar(varargin{1}) || isstring(varargin{1})
            ...
            % plotedit(fig)
            if numel(varargin{1})==1 && isgraphics(varargin{1})
                fig = ancestor(varargin{1},'figure');
            elseif numel(varargin{1})>1 && any(ishghandle(varargin{1}, 'figure'))
                arg = varargin{1};
                fig = arg(find(ishghandle(arg, 'figure'),1,'first'));
            else
                fig = gcf;
            end
            action = 'toggle';
...
switch lower(action)
    case {'on','toggle'}
        ...
        if isempty(firstTimeStart)
            ...
            %   At first time creation temporarily set the wait cursor
        ...
        if strcmpi(hMode.Enable,'off')
            matlab.graphics.internal.PlotEditInteractionUtils.cachePreviousModes(hMode, fig);
            % Activate the mode
        ...
        if ~isempty(oldptr)
            % If the mode was turned on the call activateuimode(fig,'Standard.EditPlot');
            % will have set the FigureState.Pointer to 'watch'. Restore the
            % oldptr setting
            setFigureState(hMode, 'Pointer', oldptr)
            
            oldptr =[]; %#ok<NASGU>
        end
        if ~isempty(oldPointerShapeCData)
            setFigureState(hMode, 'PointerShapeCData', oldPointerShapeCData)
            oldPointerShapeCData = []; %#ok<NASGU>
        end

        localSetPlotEditToolstripState(fig, plotEditState);
    case 'off'
        hMode = localGetMode(fig);
        ...
        activateuimode(fig,'');
        matlab.graphics.internal.PlotEditInteractionUtils.reenableAxesModes(hMode.ModeStateData.PlotSelectMode.ModeStateData);
        localSetPlotEditToolstripState(fig, false);
    case 'cut'
        hMode = localGetMode(fig);
    ...
    case 'showinspector'
        ...
        hMode = localGetMode(fig);
        matlab.graphics.internal.propertyinspector.propertyinspector('show',hMode.ModeStateData.PlotSelectMode.ModeStateData.SelectedObjects);
    case 'hideinspector'
        % Hides the inspector window
        matlab.graphics.internal.propertyinspector.propertyinspector('hide');
    case 'openandrestoreinspector'
        % Opens the property inspector and restores auto behavior
        hMode = localGetMode(fig);
        matlab.graphics.internal.propertyinspector.propertyinspector('openandrestoreinspector', hMode.ModeStateData.PlotSelectMode.ModeStateData.SelectedObjects);
    case 'getenabletools'
...
scribetogg = findall(fig,'Tag','Standard.EditPlot');
...
% the initial mouse down (see 1011667).
addContainerListeners(hMode.FigureHandle,hMode);

% Cache preselected graphics objects in the figure and unselect them
...
% Reregister any temporarily removed interactions
matlab.graphics.internal.PlotEditInteractionUtils.reregisterInteractions(fig, ...
    hMode.ModeStateData.PlotSelectMode.ModeStateData.Interactions);

%-----------------------------------------------%
function update_scribecontextmenu_cb(varargin)
% This method exists for reverse compatibility purposes. It is a no-op.

%-----------------------------------------------%
function scribe_cut_cb(varargin)
% This method exists for reverse compatibility purposes. It is a no-op.

%-----------------------------------------------%
function scribe_copy_cb(varargin)
% This method exists for reverse compatibility purposes. It is a no-op.
...
fig = hMode.FigureHandle;
matlab.graphics.annotation.internal.scribeSelectAll(fig);
...
function setFigureState(hMode, stateName, value)

hMode.FigureState.(stateName) = value; % Cached Pointer for exiting the mode should be

for k=1:length(hMode.RegisteredModes)
    if ~isempty(hMode.RegisteredModes(k))
        hMode.RegisteredModes(k).FigureState.(stateName) = value;
    end
end

function localSetPlotEditToolstripState(fig, value)

if feature('webui') 
    willBeDocked = matlab.graphics.internal.PlotEditModeUtils.willFigureBeDockedIntoContainer(fig);
    isDocked = strcmp(fig.WindowStyle,'docked');
    if isDocked || willBeDocked
        matlab.graphics.internal.toolstrip.FigureToolstripManager.getPlotEditState(fig, value);
        matlab.graphics.internal.toolstrip.toggleHoverHighlight(fig, value);

        matlab.graphics.internal.drawnow.callback(@(~,~) localSetPlotEditModeStateNoRecursion(fig, value));
    end
end


function localSetPlotEditModeStateNoRecursion(fig, value)

% Synchronize the plotedit mode state of all docked figures
f = setdiff(findobj(groot,'-depth',1,'type','figure','WindowStyle','docked'),fig);
if value
    newmodestate = 'Standard.EditPlot';
else
    newmodestate = '';
end

% Temporarily turn off FigureToolstripStateListeners so that the PlotSelectionChange
% listeners do not fire for each figure and trigger
% FigureToolstripManager.updategcf and a drawnow update for each figure
for k=1:length(f)
    curFig = f(k);
    if isvalid(curFig) && strcmpi(curFig.BeingDeleted, 'off')  ...
            && isprop(curFig, 'FigureToolstripStateListeners')
        enable(f(k).FigureToolstripStateListeners,false);
    end
end
...
for k=1:length(f)
    if value
        curFig = f(k);
        if strcmpi(curFig.BeingDeleted, 'off') && isvalid(curFig)
            % Make sure Standard.EditPlot is registered
            localGetMode(curFig);
        end
    end
    if value
        % Cache axes interaction modes for every figure
        matlab.graphics.internal.PlotEditInteractionUtils.cachePreviousModes(localGetMode(curFig), curFig);
    end
    activateuimode(f(k),newmodestate);
    if ~value
        % Reenable axes interaction modes for every figure
        hMode = localGetMode(curFig);
        matlab.graphics.internal.PlotEditInteractionUtils.reenableAxesModes(hMode.ModeStateData.PlotSelectMode.ModeStateData);
    end
end
...
for k=1:length(f)
    try
        ...
        if isvalid(curFig) && strcmpi(curFig.BeingDeleted, 'off') &&  ...
                isprop(curFig, 'FigureToolstripStateListeners')       
            enable(f(k).FigureToolstripStateListeners,true);
        end
    catch e
        % Make sure we re-activate all listeners in case of an error
    end
end
