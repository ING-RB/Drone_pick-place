...
%        right-click action.
%
%   Example: Pan the view of axes
%       plot(1:10);
%       pan on
%
%   Example: Pan the view of axes in the horizontal direction only
%       f = figure;
%       ax = axes(f);
%       plot(ax,1:10);
%       pan(ax,'xon');
%
%  See also ZOOM, ROTATE3D, LINKAXES, InteractionOptions
...
webFigureInputArgument =  (nargin > 0) && (isscalar(arg1) && isgraphics(arg1) && ...
    isa(arg1,'matlab.ui.Figure')) && matlab.ui.internal.isUIFigure(arg1) && ... 
    ~isWebFigureType(arg1,'EmbeddedMorphableFigure') && ...
    ~matlab.internal.editor.figure.FigureUtils.isEditorSnapshotGraphicsView(arg1);
...
if ~UseLegacyModes && (webAxesInputArgument || webFigureInputArgument || webFigureIsCurrent)
    ...
    switch(nargin)
        case 0
            matlab.graphics.interaction.webmodes.panWeb();
        case 1
            matlab.graphics.interaction.webmodes.panWeb(arg1);
        case 2
            matlab.graphics.interaction.webmodes.panWeb(arg1, arg2);
    end
    return
end
...
if ~isempty(ax) && ishghandle(ax(1))
    if isscalar(ax)
        ...
        % Gather DDUX data
        if buttonDownData.is2D
            if buttonDownData.isRulerHit
                matlab.graphics.interaction.internal.setInteractiveDDUXData(ax, 'rulerpan', 'mode');
            else
                matlab.graphics.interaction.internal.setInteractiveDDUXData(ax, 'dragpan', 'mode');
            end
        else
            if buttonDownData.isRulerHit
                matlab.graphics.interaction.internal.setInteractiveDDUXData(ax, 'rulerpan', 'mode');
            elseif buttonDownData.isLimitPanAndZoom
                matlab.graphics.interaction.internal.setInteractiveDDUXData(ax, 'dragpanlimits', 'mode');
            else
                matlab.graphics.interaction.internal.setInteractiveDDUXData(ax, 'dragpancamera', 'mode');
            end
        end
...
if pressData.is2D
    ...
    matlab.graphics.interaction.generateLiveCode(ax, matlab.internal.editor.figure.ActionID.PANZOOM);
    drawnow update;
