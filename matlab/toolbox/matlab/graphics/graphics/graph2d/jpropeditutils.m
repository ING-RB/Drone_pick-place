...
for i = 1:length(h) 
    ...
    figfontbold = settingsObj.matlab.ui.copyoptions.TextBold.ActiveValue; 
    if (figfontbold) 
        for j = 1:length(textList) 
    ...
    figfonttextBW = settingsObj.matlab.ui.copyoptions.TextBW.ActiveValue;
    ...
    figfontchange = settingsObj.matlab.ui.copyoptions.TextSizeChange.ActiveValue;
    if (figfontchange)
        figfontsize = settingsObj.matlab.ui.copyoptions.TextSizeChangePref.ActiveValue;
        if (figfontsize == 0)
            strfontsize = settingsObj.matlab.ui.copyoptions.TextSizeChangeTo.ActiveValue;
        ...
        elseif (figfontsize == 1)
            strfontincrease = settingsObj.matlab.ui.copyoptions.TextSizeIncrease.ActiveValue;
    ...
    figlineswidth = settingsObj.matlab.ui.copyoptions.LinesWidthCustom.ActiveValue;

    if (figlineswidth)
        strlineswidth = settingsObj.matlab.ui.copyoptions.LinesWidth.ActiveValue;
    ...
    figlinesstylechange = settingsObj.matlab.ui.copyoptions.LinesStyleChange.ActiveValue;
    if (figlinesstylechange) 
        ...
        for j = 1:length(lineList)
            ...
            linesstylepref = settingsObj.matlab.ui.copyoptions.LinesStylePref.ActiveValue;
            if (linesstylepref == 1) 
                styleindex = mod(j, 4); 
                if (styleindex == 0) 
                    styleindex = 4; 
    ...
    figlocklabels = settingsObj.matlab.ui.copyoptions.LockAxesAndTickLabels.ActiveValue;
    if (figlocklabels) 
        for j = 1:length(axesList) 
    ...
    figshowuicontrols = settingsObj.matlab.ui.copyoptions.ShowUiControls.ActiveValue;
