...
%     return the first matlab.graphics.mixin.Selectable with HitTest on,
    ...
    ChartAncestor = ancestor(evd.HitPrimitive,'matlab.graphics.chart.internal.PositionableChartWithAxes');
    if ~isempty(ChartAncestor)
        % Don't return axes for a ChartContainer chart
        retObj = ChartAncestor;
    elseif ~isempty(selectableAncestor)
...
elseif isa(hitObj,'matlab.graphics.shape.internal.ScribePeer')
        % if a ScribePeer is hit, return the corresponding PointDataTip                
        hTip = findall(ancestor(hitObj,'figure'), '-isa','matlab.graphics.shape.internal.PointDataTip');
        ind = arrayfun(@(h) h.TipHandle.ScribeHost.PeerHandle == hitObj,hTip);                   
        retObj = hTip(ind);
...
elseif isprop(evd,'HitPrimitive') && ~isempty(ancestor(evd.HitPrimitive,'matlab.graphics.chart.internal.PositionableChartWithAxes'))
    %Chart supports innerposition (needed for plot-edit)
    % Controls Plots allow for selection of plot objects
    if ~isempty(ancestor(evd.HitPrimitive, 'controllib.chart.internal.foundation.AbstractPlot'))
        retObj = ancestor(evd.HitPrimitive,'matlab.graphics.mixin.Selectable');    
    else
        retObj = ancestor(evd.HitPrimitive,'matlab.graphics.chart.internal.PositionableChartWithAxes');
    end       
