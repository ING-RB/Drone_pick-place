...
if ~strcmpi(fig.DefaultTools, 'toolstrip')
    ...
    for k=1:length(t)
        matlab.graphics.internal.PlotEditModeUtils.toggleOffNoCallbacks(t(k));
    end
end
%-------------------------------------------------------------------------%
...
hScribeAxes = matlab.graphics.annotation.internal.findAllScribeLayers(container);
...
fig = ancestor(hAnnotation, 'figure');

if strcmpi(fig.DefaultTools, 'toolstrip')
    if ~isprop(hAnnotation, 'SelectedListener')
        p = addprop(hAnnotation, 'SelectedListener');
        p.Transient = true;
        p.Hidden = true;
    end

    hAnnotation.SelectedListener = event.proplistener(hAnnotation, findprop(hAnnotation, 'Selected'),...
        'PostSet', @(e, d)localSelectChangeListener(e, d));
end
...
if isprop(hAnnotation,'Editing')
    ...
    plotedit({'registerTextEditUndoRedo',hAnnotation});
...
localSendCodeGen(hAnnotation);

%-------------------------------------------------------------------------%
function localSendCodeGen(annotation)

if ~isprop(annotation, 'Editing')
    % Needed for code generation in Live Editor for Toolstrip annotation interaction
    matlab.internal.editor.figure.generateToolstripLiveCode(ancestor(annotation, 'figure'),...
        matlab.internal.editor.figure.ActionID.ANNOTATION_ADDED);
    matlab.internal.editor.figure.generateToolstripLiveCode(annotation,...
        matlab.internal.editor.figure.ActionID.ANNOTATION_ADDED);
else
    % Add an editing listener to those annotation that have an Editing
    % Property
    p = addprop(annotation, 'CodeGenCallback');
    p.Transient = true;
    p.Hidden = true;

    % Add a listener to the Text object of the TextBox to know when it has
    % finished editing
    annotation.CodeGenCallback = event.proplistener(annotation.Text, findprop(annotation.Text,'Editing'),...
        'PostSet', @(e,d)localEditingCallback(e,d));
end

function localEditingCallback(~, eventData)

obj = eventData.AffectedObject;

if isprop(obj, 'CodeGenCallback')
    delete(obj.CodeGenCallback)
        
    % Remove the property entirely from the annotation
    delete(findprop(obj, 'CodeGenCallback'));
end

% Send Code Gen Event
matlab.internal.editor.figure.generateToolstripLiveCode(ancestor(obj, 'figure'),...
    matlab.internal.editor.figure.ActionID.ANNOTATION_ADDED);
matlab.internal.editor.figure.generateToolstripLiveCode(obj.Parent,...
    matlab.internal.editor.figure.ActionID.ANNOTATION_ADDED);
	
function localSelectChangeListener(~, eventData)
% If my selection changes while in spring loaded mode disable plot edit and
% re-enable toolstrip
fig = ancestor(eventData.AffectedObject, 'figure');
    
if isprop(eventData.AffectedObject, 'SelectedListener')
    delete(eventData.AffectedObject.SelectedListener);
    eventData.AffectedObject.SelectedListener = [];
end

% Re-enable the mode listener
if isprop(fig, 'FigureToolstripStateListeners')
    fig.FigureToolstripStateListeners.enable(true);
end

% When the annotation is finished editing make sure to generate
% code
