...
if ~isempty(parent) && isprop(parent,'AutoResizeChildren') && strcmp(parent.AutoResizeChildren,'on')
    error(message('MATLAB:plotyy:AutoResizeChildren'))
end
...
set(ax(1),'Box','on')
isInLayout=isa(get(ax(1),'Parent'),'matlab.graphics.layout.Layout');
if ~isInLayout
...
% Set the axis color if there is a single line
isThemed = localIsThemedColorOrder(fig);
if isscalar(h1)
    if isThemed
        matlab.graphics.internal.themes.specifyThemePropertyMappings(...
            ax(1),'YColor','--mw-graphics-colorOrder-1-primary');
    elseif isprop(h1,'Color')
        set(ax(1),'YColor',h1.Color)
    end
end
...
if isInLayout
    set(ax(2),'Layout',get(ax(1),'Layout'));
else
    set(ax(2),'InnerPosition',get(ax(1),'InnerPosition'));
end
...
nsi = ax(1).NextSeriesIndex;
numH2 = numel(h2);
hasSeriesIndexProp = isprop(h2(1),'SeriesIndex');
% Check to see if the second plot has a single line
if numH2 == 1
    if hasSeriesIndexProp
        h2.SeriesIndex=nsi;
    end
    if isThemed
        numColors = height(ax(2).ColorOrder);
        nsi = numColors + nsi - numColors*ceil(nsi/numColors);
        matlab.graphics.internal.themes.specifyThemePropertyMappings(...
            ax(2),'YColor',"--mw-graphics-colorOrder-"+nsi+"-primary");
    elseif isprop(h2,'Color')
        set(ax(2),'YColor',h2.Color)
    end
elseif hasSeriesIndexProp
    set(h2,{'SeriesIndex'},num2cell(nsi:(nsi+numH2-1))')
end
...
%---------------------------------------------
function isThemed = localIsThemedColorOrder(fig)
co = get(fig,'DefaultAxesColorOrder');
isThemed = false;
if ~isempty(fig.Theme)
    themeCo = matlab.graphics.internal.themes.getAttributeValue(fig.Theme,'DiscreteColorList');
    isThemed = isequal(themeCo,co);
end