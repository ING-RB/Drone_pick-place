...
if strcmp(method,'menubar')
    if nargin == 3 && ~isempty(object) && ...
       ishandle(object) && strcmp(get(object,'type'),'figure')
      fig=object;
    else
      fig=gcbf;
    end    
...
try
    switch method
        ...
        case 'update'
            ...
            copyBufferFig = getcopybuffer('noforce');
            if ~isempty(copyBufferFig) ...
                    && ~isempty(LGetSelection(copyBufferFig))
                set(findall(UIC,'Tag','figMenuEditPaste'),...
                    'Enable','on');
            else
                set(findall(UIC,'Tag','figMenuEditPaste'),...
                    'Enable','off');
            ...
            if ~isempty(callingObject)
                try
                    ...
                    warning(message('MATLAB:domymenu:ObjectCorrupted'));
                end
            end
        case 'updatemenu'
            myFig = get(A,'Figure');
    ...
    warning(message('MATLAB:domymenu:ExecutionError', me.message));
end
...
if ~isempty(A)
    ud.ObjectStore = A;
    if any(ishandle(HG)) % we may have deleted the object
...
switch method
    ...
    case 'toggletoolbar'
        ...
        if isempty(hTB)
            ...
            offon = {'off' 'on'};
            set(findall(hTB,'Tag','Standard.EditPlot'),...
    ...
    case 'lockaxes'
        ...
        for a = selection
            if isa(a,'axisobj')
                axesObjects = [axesObjects a]; %#ok<AGROW>
                axLock(end+1) = get(a,'Draggable'); %#ok<AGROW>
            end
        end
        if ~isempty(axLock)
    ...
    case 'axesprop'
        ...
        hAxesIndices = find(isgraphics(h,'axes') | isgraphics(h,'polaraxes'));
        ...
        if ploteditOn
            ...
            for a = selection
                if isa(a,'axisobj')
                    ...
                    axObjects = [axObjects a]; %#ok<AGROW>
                end
            end
        end
    ...
    case 'addlight'
        ...
        if ~isempty(lightH)
            matlab.graphics.internal.propertyinspector.propertyinspector('show',lightH);
        end
...
matlab.graphics.annotation.internal.registerUndoInsertText(axisH,textType);
...
%if ~isempty(textH) && ishandle(textH) && ~strcmp(get(textH,'BeingDeleted'),'on')
if any(ishandle(textH)) && ~strcmp(get(textH,'BeingDeleted'),'on')
    %Note that the user can turn editing off by closing the
    %figure or deleting the object
    %propedit(textH,'-noopen');
    % Set the <textType> on the axes so that it may give space (in case of
    % multiline), if necessary
    textString = get(textH,'String');
    % Text does not cause the axes to resize unless it has been modified.
    % In this case, convert the input to a cell array
...
parents = [];
for a = objects
    parents(end+1) = get(a,'Parent'); %#ok<AGROW>
end
parents = findobj(unique(parents),'flat','-regexp','Type','.*axes');
%filter out r11-style annotation layers
parents(strcmp(get(parents,'Tag'),'ScribeOverlayAxesActive')) = [];
...
elseif isnumeric(aObj) || isobject(aObj)
    editH = aObj(ishandle(aObj));
