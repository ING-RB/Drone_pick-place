...
%
%   Example: Zoom in on a plot
%       plot(1:10);
%       zoom on
%
%   Example: Zoom in on a plot in the horizontal direction only
%       f = figure;
%       ax = axes(f);
%       plot(ax,1:10);
%       zoom(ax,'xon');
%
%   See also PAN, ROTATE3D, LINKAXES, InteractionOptions
...
if ~UseLegacyModes && (webAxesInputArgument || webFigureInputArgument || webFigureIsCurrent)
    ...
    switch(nargin)
        case 0
            matlab.graphics.interaction.webmodes.zoomWeb();
        case 1
            matlab.graphics.interaction.webmodes.zoomWeb(varargin{1});
        case 2
            matlab.graphics.interaction.webmodes.zoomWeb(varargin{1}, varargin{2});
    end
    return
end
...
% it doesn't need to. For example, openfig calls zoom(fig,'off')
...
% reset 2-D axes
if matlab.graphics.interaction.internal.is2DLimits(hAxes(1))
...
matlab.graphics.interaction.internal.initializeView(hAxes);
...
switch (lower(fig_sel_type))
    ...
    otherwise % left click, center click, double click
        ...
        if ~isempty(fig_mod) && isscalar(fig_mod) && ...
            ...
            hMode.fireActionPreCallback(localConstructEvd(hAxes));
            if ~matlab.graphics.interaction.internal.is2DLimits(hAxes(1))...
                    && strcmp(matlab.graphics.interaction.internal.getAxes3DPanAndZoomStyle(hFigure,hAxes),'limits')
                local3DLimitZoom(hFigure, hAxes, evd, 1/zoom_factor);
                matlab.graphics.interaction.internal.setInteractiveDDUXData(ax, 'dragzoomlimits', 'mode');
            else
...
if ~isempty(hAxesVector)
   ...
   switch sel_type
       ...
       case 'normal' % left click        
         if strcmpi(hMode.ModeStateData.Direction,'out') && matlab.graphics.interaction.internal.is2DLimits(hAxesVector(1))
       ...
       case 'open' % double click
          ...
          matlab.graphics.interaction.internal.setInteractiveDDUXData(hAxesVector(1), 'dblclickrestoreview', 'mode');
       case 'alt' % right click
          % zoom out
          if dorightclick
              if strcmpi(hMode.ModeStateData.Direction,'in') && matlab.graphics.interaction.internal.is2DLimits(hAxesVector(1))
...
if matlab.graphics.interaction.internal.is2DLimits(hAxesVector(1))
...
% The following warning suppression is needed to address g2041090
cleanupObj = matlab.ui.internal.JavaMigrationTools.suppressJavaFrameWarning();
hMode.ModeStateData.MotionEvd = evd;

function cursor = constraintToCursorName(ax,cons,dir)
if matlab.graphics.interaction.internal.is2DLimits(ax)
...
if matlab.graphics.interaction.internal.is2DLimits(hAxes) 
    localApplyZoomFactor(hMode,hAxes,zoomFactor,true);
    matlab.graphics.interaction.internal.setInteractiveDDUXData(hAxes, 'scrollzoom', 'mode');
elseif isLimitPanAndZoom
    localApplyZoomFactor(hMode, hAxes, zoomFactor, true);
    matlab.graphics.interaction.internal.setInteractiveDDUXData(hAxes, 'scrollzoomlimits', 'mode');
else
    localApplyZoomFactor(hMode,hAxes,zoomFactor,false);
    matlab.graphics.interaction.internal.setInteractiveDDUXData(hAxes, 'scrollzoomcamera', 'mode');
end
...
if ~consumekey
    matlab.graphics.interaction.internal.FigureKeyPressManager.forwardToCommandWindow(hFigure, evd);
end
...
if strcmp(get(hAxes,'dataaspectratiomode'),'manual')
    ...
    ndx = newdx / olddx;
    ndy = newdy / olddy;
    pixPosRect = hAxes.GetLayoutInformation.PlotBox;
    pdx = pixPosRect(3)*ndx;
    pdy = pixPosRect(4)*ndy;
...
if (all(currXLim == newXLim) && all(currYLim == newYLim)) || ...
...
bounds = nan(6,1);
if withinBounds(currLimits, boundlimits)
    bounds = boundlimits;
end

function boundLimits = withinBounds(limits, bounds)
boundLimits = matlab.graphics.interaction.internal.isWithinLimits(limits(1:2), bounds(1:2)) && ...
...
% theme to blue only if a web figure in legacy modes, keep grey in java figures
if matlab.graphics.interaction.internal.isWebAxes(hAxes)
    matlab.graphics.internal.themes.specifyThemePropertyMappings(hLines, ...
                    'Color', '--mw-borderColor-focus');
end
...
% Ensure that the zoom lines do not trigger any dataCursor affordances %
% (e.g. datacursormode button in the AxesToolbar)
hLines.setInteractionHint('DataCursor','off');
...
[x,y] = matlab.graphics.interaction.internal.zoom.calculateZoomLineData(x_origpt, x_currpt, y_origpt, y_currpt, x_end, y_end, constraint);
hLines.XData = x;
hLines.YData = y;
...
if pointMode
    ...
    matlab.graphics.interaction.internal.setInteractiveDDUXData(currentAxes, 'clickzoom', 'mode');
    return;
end
...
matlab.graphics.interaction.internal.setInteractiveDDUXData(currentAxes, 'regionzoom', 'mode');
...
elseif buttonDownData.isLimitPanAndZoom
    ...
    matlab.graphics.interaction.internal.setInteractiveDDUXData(hAxes, 'dragzoomlimits', 'mode');
else
    % Add zoom operation to undo/redo stack
    origVa = buttonDownData.CameraViewAngle;
    newVa = get(hAxes,'CameraViewAngle');
    localCreate3DCameraUndo(hAxes,origVa,newVa);
    matlab.graphics.interaction.internal.setInteractiveDDUXData(hAxes, 'dragzoomcamera', 'mode');
end
...
if buttonDownData.isLimitPanAndZoom  % limit zoom
    ...
    matlab.graphics.interaction.internal.setInteractiveDDUXData(hAxes, 'clickzoomlimits', 'mode');
else % camera zoom
    local3DCameraZoomOnCurrentPoint(hAxes,fac,buttonDownData.MaxViewAngle);
    matlab.graphics.interaction.internal.setInteractiveDDUXData(hAxes, 'clickzoomcamera', 'mode');
end
...
matlab.graphics.interaction.generateLiveCode(hAxes, matlab.internal.editor.figure.ActionID.PANZOOM);
...
% limits
bound_lim = matlab.graphics.interaction.internal.isWithinLimits(orig_limits(1:2), bounds(1:2)) && ...
            matlab.graphics.interaction.internal.isWithinLimits(orig_limits(3:4), bounds(3:4)) && ...
            matlab.graphics.interaction.internal.isWithinLimits(orig_limits(5:6), bounds(5:6));
...
orig_bounds = getappdata(hAxes,'zoom_zoomOrigAxesLimits');
extents = getGraphicsExtents(hAxes);
untrans_bounds = matlab.graphics.interaction.internal.getWiderLimits(orig_bounds, extents);
