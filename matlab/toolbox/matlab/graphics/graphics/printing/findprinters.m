...
%FINDPRINTERS Returns default printer and list of available printers
%   If the jvm is available it is queried for the information 
% 
%   Otherwise, searches standard unix places for files in the following order:
...
def = '';
if isunix
    cmd = ''; % Forces reading from PRINTER before LPDEST
...
function [def, printerNames] = LocalMakeSureDefaultIsFirst(def, printerNames)
    ...
    found = strcmpi(printerNames, def); 
    if any(found) 
        % we'll replace the caller-specified default with the value from
        % the list, to make sure the case matches what the print system
        % has
        idx = find(found);
        idx = idx(1); % just in case there's more than 1
        def = printerNames{idx};
        printerNames(idx) = [];
