...
if isempty(style)
  cleanup = matlab.graphics.internal.export.disableWarningInScope('MATLAB:hgexport:NotSupportedInFutureRelease');
  try      
    style = hgexport('readstyle','Default'); %#ok
  catch ex %#ok<*NASGU>
    style = hgexport('factorystyle'); %#ok
  end
end
ui.style = style;
ui.styles = getStyles;
...
function doLoadStyle(hSrc, eventData, fig) %#ok<INUSL>
  ...
  if ui.active
    ...
    if strcmpi(stylename,'default')
      ui.style = hgexport('factorystyle'); %#ok
    else
      try
        ui.style = hgexport('readstyle',stylename); %#ok
      catch ex
...
function doApply(hSrc, eventData,fig)
  ...
  if ~isempty(ui.figure) && ishghandle(ui.figure)
    ...
    try
      cleanup = matlab.graphics.internal.export.disableWarningInScope('MATLAB:hgexport:NotSupportedInFutureRelease');
      ui.oldstate = hgexport(ui.figure,tempname,ui.style,'applystyle', true); %#ok
    catch ex
...
function doSave(hSrc, eventData,fig) %#ok<INUSL>
  ...
  if ui.active
    ...
    if ~isempty(name)
      cleanup = matlab.graphics.internal.export.disableWarningInScope('MATLAB:hgexport:NotSupportedInFutureRelease');
      hgexport('writestyle',ui.style,name); %#ok
      ui.styles = getStyles;
      ui.active = false;
      clearDirty(ui);
      updateStyleLists(ui);
...
function initStandardStyles(path)
  ...
  if ~exist(wordfile,'file')
    word = hgexport('factorystyle'); %#ok
    hgexport('writestyle',word,'MSWord'); %#ok
  end
  
  % Initialize PowerPoint style.
  pptfile = fullfile(path,'PowerPoint.txt');
  if ~exist(pptfile,'file')
    ppt = hgexport('factorystyle'); %#ok
    ppt.FontWeight = 'bold';
    ppt.FontMode = 'scaled';
    ppt.ScaledFontSize = '140';
    ppt.LineMode = 'fixed';
    ppt.FixedLineWidth = 2;
    hgexport('writestyle',ppt,'PowerPoint'); %#ok
  end
