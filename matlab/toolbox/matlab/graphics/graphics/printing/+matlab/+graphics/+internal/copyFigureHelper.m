function varargout = copyFigureHelper(hFigure)
...
% copy java figure to the clipboard
% JSD_OGL_REMOVAL The figure Renderer/RendererMode usages in this function
% are deprecated in webui. This code should only run for Java figures.
function doJavaFigureCopy(hFigure)
    ...
    if ispc 
        ...
        switch (formatPref)
            ...
            case 2
                ...
                %  use bitmap 
                %    if user set renderer explicitly to image or 
                %    we don't think we can do vector 
...
% copy a web figure to the clipboard 
% returns parameters used to copy the figure (not including figure handle)
function args = doWebFigureCopy(hFigure)
    s = settings;
    copyopts = s.matlab.ui.copyoptions;
    contentType = {'ContentType', copyopts.ContentType.ActiveValue};

    backgroundColor = {'BackgroundColor', copyopts.FigureBackgroundColor.ActiveValue};
    switch copyopts.Grayscale.ActiveValue
        case true
    ...
    if ~copyopts.MatchFigureScreen.ActiveValue
       % if not matching current size, get specified size
       width = str2num(copyopts.Width.ActiveValue);
       height = str2num(copyopts.Height.ActiveValue);
       units = copyopts.Units.ActiveValue;
       sizeArgs = {'Width', width, 'Height', height, 'Units', units};
    end

    preserve =  copyopts.MaintainAspectRatio.ActiveValue;
    if preserve
        preserve = 'on';
    else
        preserve = 'off';
    end
    preserveAspectRatio = {'PreserveAspectRatio', preserve};

    % Get current Screen DPI if the setting is 'Screen'
    currResValue = copyopts.Resolution.ActiveValue;
    if(strcmpi(currResValue,'Screen'))
        currResValue = get(groot,'ScreenPixelsPerInch');
    end
    resolution = {'Resolution', currResValue};
    
    args = [contentType(:)' backgroundColor(:)' sizeArgs(:)' ...
        colorspace(:)' preserveAspectRatio(:)' resolution(:)'];
    copygraphics(hFigure, args{:})
end 
