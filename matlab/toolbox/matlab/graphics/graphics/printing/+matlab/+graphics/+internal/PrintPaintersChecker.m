classdef (Sealed) PrintPaintersChecker < handle
    ...
    properties 
        ...
        % Don't auto-switch to painter for interpolated (texturemap/colormap) geometry
        % beyond 6400 vertexData limit.
        InterpolatedGeometryLimit = 6400;
    end
    ...
    methods (Access = public)
        ...
        function exceeds = exceedsIntepolatedLimits(obj, fig)
            % Check if objects with Facecolor 'Texturemap/Interp'
            % exceeds VertexData limit of 6400. (g1769901)
            
            exceeds = false;
            
            % Get all TriangleStrip and Quadrilateral whose FaceColor is
            % set to 'interp' or 'texturemap'
            allGeom = findobjinternal(fig, 'visible', 'on', ...
                {'-isa', 'matlab.graphics.primitive.world.TriangleStrip', ...
                '-or', '-isa', 'matlab.graphics.primitive.world.Quadrilateral'},...
                '-and', 'ColorBinding', 'interpolated');
            
            count = 0;
            for idx = 1:length(allGeom)
                count = count + size(allGeom(idx).VertexData, 2);
            end
            
            if count >= obj.InterpolatedGeometryLimit
                exceeds = true;
            end
        end
    ...
    methods (Access = private) 
        ...
        function count = countIfVisible(~, h)
            ...
            while vis == true 
                ...
                if ~isa(p, 'matlab.graphics.axis.Axes')
