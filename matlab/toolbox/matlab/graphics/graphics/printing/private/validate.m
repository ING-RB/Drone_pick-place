...
if strcmp(pj.DriverClass, 'MW' ) 
    ...
    % to create a file. This is because 'print foo' is easier
    ...
    if (~isempty(pj.FileName) && ~pj.Verbose ) ...
        ...
        if feature('webui') ||  ...
            (ishghandle(pj.Handles{1}, 'figure') && ...
             matlab.ui.internal.isUIFigure(pj.Handles{1}))
            defDev = 'pdfwrite';
            defExt = 'pdf';
            defCls = 'BI';
            if pj.DriverColor
            ...
            % only webui uses this 
            pj.Colorspace = colorspace;
        else
            % java figs - maintains existing behavior
            defDevC = 'psc';
            defDevG = 'ps';
            defExt = 'ps';
            defCls = 'PS';
...
% Validate use of fillpage/bestfit/noresize options
%    can't do -fillpage, -bestfit, and -noresize at same time
...
if pj.FillPage || pj.BestFit || pj.NoResize
  if pj.FillPage
      option = 'fillpage';
  elseif pj.BestFit
      option = 'bestfit';
  ...
  if nnz([pj.FillPage pj.BestFit pj.NoResize]) > 1
  ...
  if isfig && isPage && isAuto
    % bestfit is default as of 25a with new figure size that is larger than 
    % Java figure size if PaperPosition in 'auto' mode
    % bestfit not used by default if user manually sets PaperPosition
    pj.BestFit = true;
  end
end
