...
% Use print dialog when webfigures are enabled
if matlab.ui.internal.dialog.DialogUtils.checkDecaf
    warning(message("MATLAB:printpreview:FunctionRemoved"));
    uiprintdlg(fig);
    return;
end
...
matlab.ui.internal.JavaMigrationTools.suppressedJavaComponent(panel, java.awt.BorderLayout.WEST, figPreview);
...
[jpanelBtns, pnlButtons] = matlab.ui.internal.JavaMigrationTools.suppressedJavaComponent(jpanelBtns, [], handle(figPreview));
...
helpview('matlab', 'printpreviewhelp');
...
if isappdata(h,'PrintDefaultPaperInformation')
    ...
    if isempty(h.Theme)
        % Ignore the InvertHarcopy flag when Themes is used. 
