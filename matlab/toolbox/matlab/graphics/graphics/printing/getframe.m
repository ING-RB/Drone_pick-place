...
%   Copyright 1984-2025 The MathWorks, Inc.
    ...
    offsetRectSpecified = false;
    if nargin < 2
        offsetRect = [];
    else 
        offsetRectSpecified = true;
    end

    % input handle must be scalar
    if length(h) > 1
        error(message('MATLAB:getframe:InputHandleMustBeScalar'));
    end

    % Standalone Visualizations are not supported
    if isa(h, 'matlab.graphics.chart.Chart')
        error(message('MATLAB:getframe:StandaloneVisualizationNotSupported'));
    end

    % Only support using a figure or axes as the component handle
    if ~(isgraphics(h, 'figure') || isgraphics(h, 'matlab.graphics.axis.AbstractAxes'))
        error(message('MATLAB:capturescreen:BadObject'));
    end

    parentFig = ancestor(h, 'figure');
    % Before drawnow make sure that the figure is ready to be synced.
    matlab.graphics.internal.waitForFigureReady(parentFig);
    ...
    matlab.graphics.internal.prepareFigureForPrint(parentFig, false);
...
function [offsetRect, scaledOffsetRect, includeDecorations] = calculateOffsetRect(h, parentFig, offsetRect, offsetRectSpecified)
    % Rounding operation to be applied after conversion to device pixels.
    roundOp = @localBasicRounding;

    if isgraphics(h, 'matlab.graphics.axis.AbstractAxes')
    ...
    if isempty(offsetRect)
        ...
        [offsetRect, ~, withinClientRect] = localCheckInsideFigure(offsetRect, parentFig);

        includeDecorations = ~withinClientRect;
     
        % warn if the specified rectangle is outside the web figure Position
        if ~withinClientRect
        ...
        if offsetRectSpecified
            % want to ensure that caller-specified width & height are honored 
            whRect = matlab.ui.internal.PositionUtils.getPixelRectangleInDevicePixels([0 0 offsetRect(3:4)], parentFig);
            xyRect = matlab.ui.internal.PositionUtils.getPixelRectangleInDevicePixels(offsetRect, parentFig);
            offsetRect = [xyRect(1:2) whRect(3:4)];
        else
