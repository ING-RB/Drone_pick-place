...
if ~UseLegacyModes && (webAxesInputArgument || webFigureInputArgument || webFigureIsCurrent)
    switch(nargin)
        case 1
            matlab.graphics.interaction.webmodes.modeFunctionHelper('datacursor', varargin{1});
        case 2
            matlab.graphics.interaction.webmodes.modeFunctionHelper('datacursor',  varargin{1},  varargin{2});
    end
    if nargout>0
        % Get the data cursor tool object
        hMode = localGetMode(fig,action);
        retval = localGetObj(hMode,action);
    end
    return
end
...
elseif nargin==2
     fig = varargin{1};
     action = varargin{2}; 
    if ~strcmpi(action,'-nocontextmenu') && nargout > 0
...
% Get the data cursor tool object
hMode = localGetMode(fig,action);
hTool = localGetObj(hMode,action);
...
%-----------------------------------------------%
function hTool = localGetObj(hMode,action)

hTool = hMode.ModeStateData.DataCursorTool;

% Create tool object
if isempty(hTool) || ~isvalid(hTool)
    hTool = matlab.graphics.shape.internal.DataCursorManager(hMode,action);
    hMode.UIContextmenu = hTool.UIContextMenu;
end

%-----------------------------------------------%
function hMode = localGetMode(hFig,action)
...
dataCursorToolbarButton = matlab.ui.internal.findToolbarModeButtonsById(findall(tb),'Exploration.DataCursor');
set(dataCursorToolbarButton,'State','on');
...
tb = findobj(allchild(fig),'flat','Type','uitoolbar');
dataCursorToolbarButton = matlab.ui.internal.findToolbarModeButtonsById(findall(tb),'Exploration.DataCursor');
set(dataCursorToolbarButton,'State','off');
...
if isempty(hTip) || docreate
    ...
    % Update interpolation value, position, and content
    hTool.updateInterpolateValue(hTip.Cursor);
    hTip.Cursor.moveTo(figPoint);
    hTool.updateDataTipSettings(hTip, hTarget);
end
