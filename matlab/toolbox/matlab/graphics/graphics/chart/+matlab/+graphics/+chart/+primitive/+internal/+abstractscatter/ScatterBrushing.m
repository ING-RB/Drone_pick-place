    ...
    methods(Access='public')
        ...
        function doUpdate(hObj, updateState)
               ...
               % Delete any existing brushing handles
                ...
                % R2008a behavior.
                % Get the first non zero entry in brushdata 
                [r,~] = find(brushData,1);
                brushColor = matlab.graphics.chart.primitive.brushingUtils.getBrushingColor(brushData(r,:),brushStyleMap);
                ...
                for layer=1:size(brushData,1)
                   ...
                   hM.Style=parent.MarkerHandle.Style;
                   if isscalar(s)
                       hM.Size = parent.mapSize(s,updateState);
                       hM.SizeBinding = 'object';
                   else
                       hM.Size = parent.mapSize(s(I),updateState);
                   ...
                   switch lower(hM.Style)
                       case {'plus','point','asterisk','x','vbar','hbar'}
                   ...
                   hM.AnchorPointClipping = parent.MarkerHandle.AnchorPointClipping;
        ...
        function remove(~,gobj,keepflag)
            ...
            newCData = datamanager.createRemovedCData(gobj,keepflag);
            pvPairs = [pvPairs,{'CData',newCData}];
    ...
    methods (Static = true)
        ...
        function gStruct = serializeBrushDataStruct(varargin)
            ...
            if nargin==0 || isempty(varargin{1})
                ...
                gStruct.Cdata = get(varargin{1},'CData_I');
            end
        end
        ...
        function deserializeBrushDataStruct(brushDataStruct,gObj)
            ...
            if any(size(brushDataStruct.Cdata)>1)
