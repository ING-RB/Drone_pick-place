function [cout, hand, cf] = contourf(varargin)
    ...
    %
    %   [C,H] = CONTOURF(...) returns contour matrix C and a handle, H, to
    %   a contour object.
    %
    %   Examples:
    %       z = peaks;
    ...
    narginchk(1, Inf);
    [pvpairs, ~, ~, errmsg, warnmsg] = matlab.graphics.chart.internal.contourobjHelper('parseargs', true, args{:});
    if ~isempty(errmsg)
        error(errmsg);
    end
    if ~isempty(warnmsg)
    ...
    pvpairs = [{ ...
        'EdgeColor_I', [0, 0, 0], ...
        'FaceColor_I', 'flat', ...
        'Fill_I', true, ... % Setting FaceColor_I bypasses setting Fill_I.
        'ShowText_I', 'off', ...
        'Is3D_I', 'off'} ...
        pvpairs];
