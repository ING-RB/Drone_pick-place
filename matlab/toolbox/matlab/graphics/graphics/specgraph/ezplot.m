function hh = ezplot(varargin)
    ...
    if (twofuns)
        [f, fx0, varx] = matlab.graphics.chart.internal.ezfcnchk(f, 0, 't');
    else
        [f, fx0, varx] = matlab.graphics.chart.internal.ezfcnchk(f);
    end
    ...
    if (twofuns)
        ...
        [fy, fy0, vary] = matlab.graphics.chart.internal.ezfcnchk(args{1}, 0, 't');
...
function [hp, newcax] = ezplot1(cax, f, vars, labels, xrange, fig)
    ...
    fig = ancestor(cax, 'figure');
    pixpos = hgconvertunits(fig, get(cax, 'InnerPosition'), get(cax, 'Units'), ...
        'pixels', ancestor(cax,  'matlab.ui.internal.mixin.CanvasHostMixin','node'));
