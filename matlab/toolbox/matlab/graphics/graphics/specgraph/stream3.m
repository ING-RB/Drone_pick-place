...
if step<=0
  error(message('MATLAB:stream3:NonPositiveStep'));
end

if maxvert<=0
  error(message('MATLAB:stream3:NonPositiveVertexCount'));
end

[msg, x, y, z] = xyzuvwcheck(x,y,z,u,v,w);  error(msg)

if ~isequal(size(sx), size(sy), size(sz)) 
  error(message('MATLAB:stream3:StartSizeMismatch'));
end
...
elseif nin==9 || nin==10    % stream3(x,y,z,u,v,w,sx,sy,sz)
  ...
  error(message('MATLAB:stream3:WrongNumberOfInputs')); 
end
