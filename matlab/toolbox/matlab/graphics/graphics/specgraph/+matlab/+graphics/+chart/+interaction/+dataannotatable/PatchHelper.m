classdef(Sealed = true) PatchHelper < handle
    ...
    methods(Access = public, Static = true)
        ...
        function [index, interpolationFactor] = getInterpolatedPoint(hObj, position)
            ...
            % figure.
            [index, interpolationFactor] = localGetNearestVertex(hObj, position, true);
        end
        
        function [index, interpolationFactor] = getInterpolatedPointInDataUnits(hObj, position)
            % Returns the index and interpolation factor representing the
            % point on the object nearest to a 1x2 pixel position in the
            % figure.
            [index, interpolationFactor] = localGetNearestVertex(hObj, position, false);
        end
...
function [index, interp] = localGetNearestVertex(hPatch, position, isPointPixel)
...
if ((strcmp(hPatch.FaceColor,'none') )...
    ...
    size(hPatch.Faces,2)<=1
    
    % There are no faces drawn, find the closest visible vertex
    index = localPointCloudNearest(pickUtils, hPatch, position, isPointPixel);

else
    NeedInterp = (nargout>1);
    
    % The faces are visible, even if they are only really a mesh
    [vertIndex, faceIndex, interp] = localPickNearest(pickUtils, hPatch, position, faces, verts, NeedInterp, isPointPixel);
    ...
    if NeedInterp && isempty(interp) ...
        ...
        if ~isempty(thisFace)
            ...
            if size(tri.Points, 1)==numVerts
                ...
                [~, ~, interp] = localPickNearest(pickUtils, hPatch, position, subFaces, faceVerts, NeedInterp, isPointPixel);
                if ~isempty(interp)
...
function index = localPointCloudNearest(pickUtils, hPatch, position, isPointPixel)
...
vertIndex = pickUtils.nearestPoint(hPatch, position, isPointPixel, verts);
...
function [vertIndex, faceIndex, interp] = localPickNearest(pickUtils, hPatch, position, faces, verts, NeedInterp, isPointPixel)
% Execute a hit test and convert the result into an interpolation structure
[vertIndex, faceIndex, intFactors] = pickUtils.nearestFacePoint(hPatch, position, isPointPixel, faces, verts);
