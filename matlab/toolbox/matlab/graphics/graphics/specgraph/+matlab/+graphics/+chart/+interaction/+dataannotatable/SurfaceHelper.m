classdef(Sealed = true) SurfaceHelper < handle
    ...
    methods(Access = public, Static = true)
        ...
        function [index, interpolationFactor] = getInterpolatedPointInDataUnits(hObj, position)
            % Returns the index and interpolation factor representing the
            % point on the object nearest to position in the data units
...
function [index, interp] = localGetNearestVertex(hSurface, position, isPixelPoint)
...
if localCheckDataConsistency(xdata, ydata, zdata)
    if all(size(zdata)>1)
        ...
        if strcmp(hSurface.FaceColor,'none') && strcmp(hSurface.LineStyle,'none')
            ...
            index = pickUtils.nearestPoint(hSurface, position, isPixelPoint, verts);
        ...
        if strcmpi(hSurface.LineStyle,'none')
            % Only markers are visible - treat the data as a point cloud
            index = pickUtils.nearestPoint(hSurface, position, isPixelPoint, xdata, ydata, zdata);
        else
            % This is a surface with no faces.  Treat the data as a single line
            [index1, index2, t] = pickUtils.nearestSegment(hSurface, position, isPixelPoint, xdata, ydata, zdata);
            if t<=0.5
                index = index1;
            else
                index = index2;
