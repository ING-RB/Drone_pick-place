...
function target = localConvertToAxesPoint(hObj, pixelPosition)
  offset = [0 0];
  parentCanvasContainer = ancestor(hObj,'matlab.ui.internal.mixin.CanvasHostMixin');
  if ~isa(parentCanvasContainer,'matlab.ui.Figure')
     canvasContainerPosition = getpixelposition(parentCanvasContainer,true);
     offset = canvasContainerPosition(1:2);
  end
  target = matlab.graphics.chart.internal.convertViewerCoordsToDataSpaceCoords(hObj, pixelPosition - offset,true).';
end
