classdef ( ConstructOnLoad=true,Sealed) BarBrushing < matlab.graphics.primitive.world.Group
    ...
    methods(Access='public')
        ...
        function doUpdate(hObj, updateState)
            ...
            brushData = zeros(size(parent.BrushData,1),size(parent.BarOrder,2));% BarOrder will be less then the length of BrushData if there are NANs in YData
            for i = 1: size(parent.BrushData,1)
                tempbrushData = parent.BrushData(i,:);
                brushData(i,:) = tempbrushData(parent.BarOrder);
            end            
        ...
        function remove(~,gobj,keepflag)
            ...
            % Remove CData based on the brushed points
    ...
    methods (Static = true)
        ...
        function gStruct = serializeBrushDataStruct(varargin)
            ...
            if nargin==0 || isempty(varargin{1})
                fnames = [fieldnames(gStruct); {'Cdata'}];
