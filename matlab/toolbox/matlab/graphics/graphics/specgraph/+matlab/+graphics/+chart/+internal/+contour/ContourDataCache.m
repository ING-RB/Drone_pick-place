classdef (Sealed) ContourDataCache < handle
    ...
    methods
        function validateCache(cache, x, y, z)
            ...
            newZData = ~isequaln(z, cache.ZData);
            if newZData || ~isequaln(x, cache.XData) || ...
                ...
                % Only ZData is needed to calculate the ZMin and ZMax, so
                % only clear these values if ZData has changed.
                if newZData
                    cache.ZMin = [];
                    cache.ZMax = [];
                end
        ...
        function [zmin, zmax] = getZDataRange(cache, z)
            ...
            % Call validateCache so that if ZData has changed the cache is
            % cleared. Provide the cached XData and YData, so they do not
            % trigger an unnecessary clearing of the cache.
            cache.validateCache(cache.XData, cache.YData, z);
            
            % Check if the cache is populated.
            if isempty(cache.ZMin) || isempty(cache.ZMax)
                % Cache is empty. Calculate and store the range.
                zmin = 0;
                zmax = 0;
                
                % Calculate the range of the finite data.
                if ~isempty(z)
