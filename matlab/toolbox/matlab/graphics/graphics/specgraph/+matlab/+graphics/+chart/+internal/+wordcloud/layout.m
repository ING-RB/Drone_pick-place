...
%     [x,y] = SAMPLE(width, height, wordWidth, wordHeight, customStream);
...
%     fontsize: a numeric vector of pixel-units font sizes from scaling args.fontsize
...
% Using local RandomNumer Stream, twister is the default MATLAB randStream
customStream = RandStream('twister','Seed',args.LayoutNum);
...
shapeScaleFactor = integerPixels(height*args.fontsize(1))/maxPix;
...
attemptCount = 0;
while k <= num_words
    [k, attemptCount, layoutData] = tryOneWord(k, attemptCount,shape, sample, layoutData, customStream);
end
% prepare output
scaling = layoutData.origHeight/layoutData.height;
data.fontsize = integerPixels(args.fontsize*scaling*layoutData.origHeight);
data.pos = layoutData.pos;
data.mask = layoutData.mask;
data.layoutSize =[layoutData.width layoutData.height];

function [k, attemptCount, layoutData] = tryOneWord(k,attemptCount,shape,sample,layoutData,customStream)
...
[x,y] = sample(width, height, symw, symh, customStream);
x = round(x);
y = round(y);
box = wordShape(1:4);
if strcmp(layoutData.style,'oval')
    ...
    [xy, gotPosition] = computeRectPosition(layoutData.mask,x,y,box,customStream);
    controls = [100 20];
end
if ~gotPosition
...
function [xy, gotPosition] = computeRectPosition(mask,x,y,box,customStream)
gotPosition = false;
xy = [nan nan];
...
if over
    ...
    for idx=gaps.'
        ...
        for k=1:4 % try 4 different random locations within this span
            y = round(sampleDim(gap_y + box(3), gap_y + span - box(4),customStream));
            min_y = y - box(3);
            max_y = y + box(4);
            over = overlaps(mask, i1, i2, min_y, max_y);
            if ~over
...
if strip(k-1) + strip(k) >= height
    ...
    strip(k+1:k+2) = [height dy];
end

function y = sampleDim(low,high,customStream)
% y is a random point between [low high] near the center

span = high - low;
y = low + rand(customStream)*span;
...
wordShape = wordShape*layoutData.scaleFactor;
wordShape = ceil(abs(wordShape)).*sign(wordShape); % round away from 0
...
function px = integerPixels(x)
eps = 1e-10;
px = max(1,floor(x+eps));
