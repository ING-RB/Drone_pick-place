function finalBarColors = setPrimitiveColors(updateState, hPrim, numBars, color, cdata, cdatashape, alpha)
%
...
finalCData = zeros(0,4,'uint8');
if ~isempty(hPrim.ColorData)
    finalCData = hPrim.ColorData;
end
if width(finalCData) == 1
    finalBarColors = repmat(finalCData, 1, numBars);
elseif width(finalCData) == 4*numBars
    finalBarColors = finalCData(:, 1:4:end);
else
    finalBarColors = finalCData;
end
