classdef (Sealed)PolygonAdaptor < matlab.graphics.chart.interaction.dataannotatable.AnnotationAdaptor
    ...
    methods
        ...
        function coordinateData = createCoordinateData(hObj, valueSource, dataIndex, interpolationFactor)
            ...
            % Get the data descriptors for a polygin given the index and
            % interpolation factor where interpolationFactor is an optional argument
            if nargin < 4
                interpolationFactor = 0;
            end
            coordinateData = CoordinateData.empty(0,1);
            % Get the data descriptors for an object given the dataIndex and
            % interpolation factor.
            poly = hObj.AnnotationTarget;
            % We will use the reported position.
            if ~isnan(poly.Shape.Vertices(dataIndex))
                ReportedPos = doGetReportedPosition(hObj,dataIndex,interpolationFactor);
                location = ReportedPos.getLocation(poly);
            else
                location = matlab.graphics.shape.internal.util.SimplePoint([NaN NaN NaN]);
            end
            
            % For 2d objects location does not contain the 3rd coordinate
            location3 = [];
            if numel(location) == 3
            ...
            [xLoc, yLoc, zLoc] = matlab.graphics.internal.makeNonNumeric(poly,location(1),location(2),location3);
            calculatedPosition = {xLoc, yLoc, zLoc};
            
            % Construct default data source from DimensionNames e.g. XData,
            % YData etc.           
            dimensionData = strcat(poly.DimensionNames,'Data');
        ...
        function valueSources = getAllValidValueSources(hObj)
            ...
            dimensionNames = poly.DimensionNames;
            for i=1:numel(dimensionNames)
                if strcmpi(dimensionNames{i},'Z') && ...
                        (~isprop(poly,'ZData') || (isprop(poly,'ZData') && ...
                        isempty(poly.ZData)))
                    continue;
                end
    ...
    methods(Access='protected', Hidden=true)
        ...
        function index = doGetNearestPoint(hObj, position)
            ...
            index = localGetNearestVertex(hObj.AnnotationTarget, position, true);
        end
        ...
        function [index, interpolationFactor] = doGetInterpolatedPoint(hObj, position)
            ...
            [index, ~, interpolationFactor] = localGetNearestEdgeVertex(hObj.AnnotationTarget, position, true);
        end
        
        function [index, interpolationFactor] = doGetInterpolatedPointInDataUnits(hObj, position)
            % Returns the index and interpolation factor representing the
            % point on the object nearest to a position in data units.
            
            [index, ~, interpolationFactor] = localGetNearestEdgeVertex(hObj.AnnotationTarget, position, false);
        end
...
%-------------------------------------------------------------------------%
function index = localGetNearestVertex(hObj, position, isPointPixel)
...
index = utils.nearestPoint(hObj, position, isPointPixel, xData, yData);
end

%-------------------------------------------------------------------------%
function [index, index2, interpolationFactor] = localGetNearestEdgeVertex(hObj, position, isPointPixel)
...
% closest segment
[index, index2, interpolationFactor] = pickUtils.nearestSegment(hObj, position, isPointPixel, xData, yData);
...
if 1 <= index && index <= numPoints
    if isnan(hObj.Shape.Vertices(index))
        interpolationFactor = 0;
        index = localGetNearestVertex(hObj, position, isPointPixel);
    end
end
