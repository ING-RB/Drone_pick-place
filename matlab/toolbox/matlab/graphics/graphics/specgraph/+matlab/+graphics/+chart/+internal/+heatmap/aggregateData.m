function [xData, yData, colorData, counts, inds] = aggregateData(...
...
% Row indices (used by data tips when ColorMethod == 'none')
inds = NaN(m,n);
...
switch colorMethod
    case 'none'
        ...
        colorData = accumarray([y,x],v,[m,n],@(w) double(sum(w)),NaN);

        % Needed to find the right row for data tip when colormethod is
        % none and user can select non-numeric table columns
        inds = accumarray([y,x],1:numel(v),[m,n],@sum,NaN);
    case 'count'
        colorData = counts;
    case 'mean'
        assert(~isempty(colorVariable), message('MATLAB:graphics:heatmap:NoColorVariable', 'mean'));
        colorData = accumarray([y,x],v,[m,n],@(w) double(mean(w)),NaN);
    case 'median'
        assert(~isempty(colorVariable), message('MATLAB:graphics:heatmap:NoColorVariable', 'median'));
        colorData = accumarray([y,x],v,[m,n],@(w) double(median(w)),NaN);
    case 'min'
        assert(~isempty(colorVariable), message('MATLAB:graphics:heatmap:NoColorVariable', 'min'));
        colorData = accumarray([y,x],v,[m,n],@(w) double(min(w)),NaN);
    case 'max'
        assert(~isempty(colorVariable), message('MATLAB:graphics:heatmap:NoColorVariable', 'max'));
        colorData = accumarray([y,x],v,[m,n],@(w) double(max(w)),NaN);
    case 'sum'
        assert(~isempty(colorVariable), message('MATLAB:graphics:heatmap:NoColorVariable', 'sum'));
        colorData = double(accumarray([y,x],v,[m,n],@sum));
end
