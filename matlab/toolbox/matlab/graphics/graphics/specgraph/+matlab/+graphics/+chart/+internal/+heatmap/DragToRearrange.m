classdef DragToRearrange < matlab.graphics.interaction.uiaxes.Drag
    ...
    methods (Access = protected)
        ...
        function customEventData = start(hDrag, ~, eventData)
            ...
            % Blank the labels of the heatmap. Freeze the layout so that
            % hiding labels doesn't cause the layout to change.
            hChart.freezeLayout();
    ...
    methods (Access = protected)
        ...
        function showDragHandles(hDrag, dim, index)
            ...
            % Refresh gap colors to assure theme match at time of 
            % interaction.
            hGap.FaceColor = getThemeContainerColor(hDrag.HeatmapChart);
            hGap.EdgeColor = hHeatmap.LineColor;
...
function color = getThemeContainerColor(hHeatmap)
% Use color from theme if available; assume light theme if no theme is present.
hTC = ancestor(hHeatmap, 'matlab.graphics.mixin.ThemeContainer');
theme = gobjects(0);
if ~isempty(hTC)
    theme = hTC.Theme;
end
