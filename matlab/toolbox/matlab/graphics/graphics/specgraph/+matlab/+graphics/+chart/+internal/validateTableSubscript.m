...
% Cast strings to character vectors so that the final properties store
% character vectors.
...
% table.
if iscellstr(subscript) || isnumeric(subscript) %#ok<ISCLSTR>
    % Numeric vectors or cell arrays of character vectors must be scalar.
    if numel(subscript)>1
        err = MException(message('MATLAB:Chart:NonScalarTableSubscript', propName));
    elseif iscellstr(subscript) && numel(subscript) == 1 && ~isrow(subscript{1}) %#ok<ISCLSTR>
...
elseif isempty(err) && ischar(subscript)
    % Character vectors must be empty or row vectors.
    if ~isempty(subscript) && ~isrow(subscript)
        err = MException(message('MATLAB:Chart:InvalidTableSubscript', propName));
    end
end
