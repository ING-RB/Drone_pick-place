...
if all(ismissing(xLimits)) || isempty(xData)
    ...
    numericXLimits = [0.5 1.5];
else
    xAxis.Limits = xLimits;
    numericXLimits = xAxis.makeNumeric(xLimits)+[-0.5 0.5];
end
...
if all(ismissing(yLimits)) || isempty(yData)
    ...
    numericYLimits = [0.5 1.5];
else
    yAxis.Limits = yLimits;
    numericYLimits = yAxis.makeNumeric(yLimits)+[-0.5 0.5];
end

% Push the limits directly to the data space. This is necessary because
% above the limits are being set directly on the rulers, and synchronizing
% the rulers with the data space is ordinarily handled during the
% prep-traversal. Without this step the data space could have stale limits.
% This can be disabled to allow the view to drift from the value of the
% limits properties, which is used by continuous drag pan.
if hObj.UpdateDataSpaceLimits
    hAx.DataSpace.XLim = numericXLimits;
    hAx.DataSpace.YLim = numericYLimits;
end
