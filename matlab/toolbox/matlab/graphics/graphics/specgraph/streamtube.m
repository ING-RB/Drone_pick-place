function hout=streamtube(varargin)
    ...
    if any(isnan(u(:))) ||  any(isnan(v(:))) ||  any(isnan(w(:)))
        error(message('MATLAB:streamtube:NaNData'))
    end
    ...
    if isempty(width)
        ...
        maxwidth = .05*max(vertmax-vertmin);
        if ~isempty(div)
            for k = 1:length(divi)
                if scale==0
                    width{k} = divi{k};
    ...
    for k = 1:length(verts)
        ...
        if ~isempty(vv) && size(vv,1)>1
            if iscell(width)
                [xv, yv, zv] = tubecoords(vv,width{k},n);
            else
                [xv, yv, zv] = tubecoords(vv,width,n);
            end
    ...
    if ~isempty(h)
        if ~isdeployed
            makemcode('RegisterHandle',h,'IgnoreHandle',h(1),'FunctionName','streamtube');
        end
    end 
end
...
function [x, y, z, u, v, w, sx, sy, sz, verts, div, width, scale, n] = parseargs(nin, vargin)
    ...
    elseif nin==1 || nin==2    % streamtube(verts)  or streamtube(verts,scale)
        ...
        error(message('MATLAB:streamtube:WrongNumberOfInputs'));
    end
    ...
    if ~isempty(scale)
        if length(scale) == 2
            ...
            error(message('MATLAB:streamtube:InvalidSizeInput'));
        end
    end
end
