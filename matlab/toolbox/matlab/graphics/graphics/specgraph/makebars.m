...
if ((nin>1) && isscalar(varargin{nin}) && ~isscalar(varargin{nin-1})) || ...
    ...
    if ~isnumeric(varargin{nin})
        msg = struct('message',getString(message('MATLAB:bar:WidthNonNumeric')),...
            'identifier','MATLAB:bar:WidthNonNumeric');
        return
    end
    barwidth = double(varargin{nin});
    nin=nin-1;
end
varargin = matlab.graphics.chart.internal.getRealData(varargin(1:nin), true);
[msg,x,y] = xychk(varargin{1:nin});

if ~isempty(msg) || size(y,2)==0
    return
end

if ~(isnumeric(y) || isduration(y))
    msg = struct('message',getString(message('MATLAB:bar:InvalidHeights')),...
        'identifier','MATLAB:bar:InvalidHeights');
    return
end
...
if threeD
    ...
    % Initialize yy and zz as missing values in the same format as the
    % current values of y and z.
    zz = z([]);
    zz(max(1,nn),mm) = missing;
    yy = y([]);
    yy(max(1,nn),mm) = missing;
    xx = zeros(nn,4);
    
    if nn==0
        % usecase: data=ones(0,3); bar3(data)
        zz = zz([],:);
        yy = yy([],:);
    end
    ...
    if numel(y)<2 || all(y(1)==y,'all')
        % Empty y, scalar y, or all y are equal.
        delta = ones(size(y));
        t = y - 0.5*delta;
    elseif isnumeric(y) && all(abs(diff(y,2,1)) < eps(max(y,[],'all')),'all') % Uniform spacing
    ...
    else % Spacing is unequal or non-numeric with >1 bar along y.
        if isnumeric(y) && ishist==1 % Width of bin is average of difference between points.
            dy = diff(y); dy = (dy([1 1:end],:)+dy([1:end end],:))/2;
            t = [(y(1:end-1,:)+y(2:end,:))/2;y(end,:)+(y(end,:)-y(end-1,:))/2]-dy/2;
        elseif isnumeric(y) && ishist==-1 % Width of bin is difference between edges
            dy = [diff(y,1);zeros(1,size(y,2))];
            t = y + dy*groupwidth/2;
        else % Width of bin is minimum of difference between points.
            dy = ones(n,1)*min(diff(y,1,1),[],1);
            t = y;
        end
        if plottype~=0  % Stacked or detached
            delta = dy * groupwidth;
    ...
    if ~isempty(yy)
        ...
        xx(ii1,[1 4]) = nan;
        xx(ii2,[2 3]) = nan;
        yy(ii1,[(1:4:mm) (4:4:mm)]) = missing;
        yy(ii2,[(2:4:mm) (3:4:mm)]) = missing;
        zz(ii1,[(1:4:mm) (4:4:mm)]) = missing;
        zz(ii2,[(2:4:mm) (3:4:mm)]) = missing;
    end
    arg8 = zz;
else
    ...
    if nn > 0
        ...
        equal = max(abs(diff(x,2))) <= max(abs(x),[],'all')*sqrt(eps(class(x)));
        if ishist==0
            if isempty(equal)
                equal = 0;
            end
