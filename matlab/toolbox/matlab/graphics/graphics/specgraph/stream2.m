...
if step<=0
  error(message('MATLAB:stream2:NonPositiveStep'));
end

if maxvert<=0
  error(message('MATLAB:stream2:NonPositiveVertexCount'));
end

[msg, x, y] = xyuvcheck(x,y,u,v);  error(msg) 

if ~isequal(size(sx), size(sy)) 
  error(message('MATLAB:stream2:StartSizeMismatch'));
end
...
elseif nin==6 || nin==7    % stream2(x,y,u,v,sx,sy)
  ...
  error(message('MATLAB:stream2:WrongNumberOfInputs')); 
end
