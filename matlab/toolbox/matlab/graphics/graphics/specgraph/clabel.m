...
function h = plusLabelsAuto(cax, cs, is3D, bTextList, textList, bContourZLevel, extContourZLevel, labelTextPairs)
    ...
    % Create custom Random Number Generator Stream and generate location
    % value
    customStream = RandStream('dsfmt19937','Seed',314159);
    r = rand(customStream,1);
...
function h = inlineLabelsManual(cax, hContour, is3D, bContourZLevel, extContourZLevel, labelTextPairs)
    ...
    % Create custom Random Number Generator Stream
    customStream = RandStream('dsfmt19937','Seed',314159);
    getStartParam = rand(customStream,1);
    ...
    [xDir, yDir, axScaleXPos, axScaleYPos, dummyExtent] = matlab.graphics.chart.internal.contourobjHelper('contourLabelScaleParams', cax, [xmin, xmax], [ymin, ymax]);
    ...
    while (1)
        ...
        % Label the point.
        if okay
            [bValid, zLevel, lab, lp, xc, yc, trot] = matlab.graphics.chart.internal.contourobjHelper('contourLabelRenderParams', cs, i, k, labels, dummyExtent, xDir, yDir, axScaleXPos, axScaleYPos, true, p, 0, [], getStartParam);
            if bValid
                for jj = 1 : lp
