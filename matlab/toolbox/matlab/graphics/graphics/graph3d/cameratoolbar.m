...
if nin==0
    ...
    show(hfig);
    % Delay the setmode to WhenViewIsReady for JSD since it needs to wait
    % for the tool to be ready.
    % We can call setmode directly for Java
    if feature('webui')
        matlab.ui.internal.dialog.DialogHelper.dispatchWhenViewIsReady(hfig,...
...
switch arg
    ...
    case 'motion'
        ax = vargin{2};
        motion_callback(manager, hfig, ax);
        vargin(2) = [];
    case 'stopmoving'
        stopmoving(manager);
    case 'updatetoolbar'
...
if numel(vargin) > 1
    if nargin>0 && isscalar(varargin{1}) && ishghandle(varargin{1},'figure')
        warning(message('MATLAB:cameratoolbar:OnlyOneInputFigure'));
    else
        warning(message('MATLAB:cameratoolbar:OnlyOneInput'));
    end
end
...
if strcmpi(hfig.DefaultTools, 'toolstrip')
    % Use the toolstrip for docked figures in MOL
    matlab.ui.internal.dialog.DialogHelper.dispatchWhenViewIsReady(hfig,...
        @(~,~)  matlab.graphics.internal.toolstrip.cameratoolbarHelper(hfig,'show'));
else
   manager.isOpen = true;
   set(manager.mainToolbarHandle, 'Visible', 'on');
end
publishToClient(hfig, true);

function hide(manager, hfig)

if strcmpi(hfig.DefaultTools, 'toolstrip')
    % Use the toolstrip for docked figures in MOL
    matlab.ui.internal.dialog.DialogHelper.dispatchWhenViewIsReady(hfig,...
        @(~,~)  matlab.graphics.internal.toolstrip.cameratoolbarHelper(hfig,'close'));
else
...
function toggle(manager, hfig)
if matlab.ui.internal.isUIFigure(hfig)
    manager.isOpen = ~manager.isOpen;
    publishToClient(hfig, manager.isOpen);
end
...
function isValid = isUIFigureValidArguments(arg)
invalidArguments = {'noreset','show','hide','toggle','close'};
isValid = true;
for a = invalidArguments
    if strcmpi(arg,a)
        isValid = false;
        return;
    end
...
function publishToClient(hfig, doShow)
% channelID = matlab.ui.internal.FigureServices.getUniqueChannelId(hfig);
% 
...
% end
% 
% message.publish("/embeddedfigure/ServerToClient"+...
%     channelID,...
%     struct('eventType','ContextualToolstrip','ToolstripTag',...
%     'motwToolstrip.cameraToolbarToolstripTabGroup2','ToolstripContextId','motwToolstrip.cameraToolbarContext',...
%     'Action',action));
% end
