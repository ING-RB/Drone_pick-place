...
for o = 1:numel(obj)
    switch lower(obj(o).Type)
        ...
        case 'scatter'
            success(o) = alpha_Scatter(obj(o), param);
        case {'area','bar','histogram','histogram2','categoricalhistogram',...
...
elseif isCharOrString(param)
    switch lower(param)
        ...
        case 'color'
            ...
            if strcmp(obj.FaceColor,'flat') || strcmp(obj.FaceColor,'interp')
                % FaceColor is 'flat', so use FaceVertexCData
                data = obj.FaceVertexCData;
            elseif strcmp(obj.FaceColor,'none')
                % FaceColor is 'none', alpha doesn't matter, don't change
                % anything
                data = obj.FaceVertexAlphaData;
            else
                % Otherwise, FaceColor is RGB, so handle that as if it was 
                % an RGB in FaceVertexCData
                data = obj.FaceColor;
            end
...
if ~strcmpi(proptype,'Ignore')
    if strcmpi(proptype,'FaceVertexAlphaData')
        if isempty(data) 
            % If 'FaceVertexAlphaData' is going to be set to empty, make 
            % sure FaceAlpha is not 'flat'.
            if ~isa(obj.FaceAlpha,'double')
                obj.FaceAlpha = 1;
            end
        else
            % If 'FaceVertexAlphaData' is going to be set to a non-empty 
            % value, then change 'FaceAlpha' to 'flat' so that 
            % 'FaceVertexAlphaData' is not ignored.
            obj.FaceAlpha = 'flat';
        end
    end
    set(obj,proptype,data);
...
elseif isCharOrString(param)
    switch lower(param)
        ...
        case 'clear'
            ...
            data = 0;
        case {'flat', 'interp'}
            % If ZData isn't empty, copy to AlphaData
            % change FaceColor to flat;
            if isprop(obj, 'ZData') && isprop(obj, 'AlphaData') && any(size(obj.AlphaData) ~= size(obj.ZData))
                set(obj, 'AlphaData', obj.ZData);
            end
            
            if isprop(obj, 'FaceAlpha') && isprop(obj, 'FaceColor')
                set(obj, 'FaceAlpha', param, 'FaceColor', param); 
            end
...
if success && isprop(obj,'FaceColor') && isprop(obj,'FaceAlpha') && strcmpi(proptype,'AlphaData') && isa(obj.FaceAlpha,'double')
    if strcmp(obj.FaceColor, 'interp')
        obj.FaceAlpha = 'interp';
    else
        obj.FaceAlpha = 'flat';
    end
end
...
function success = alpha_Scatter(obj, param)
% Scatter has 'MarkerFaceAlpha', 'MarkerEdgeAlpha', 'AlphaData', and 'AlphaDataMapping'
% 'MarkerFaceAlpha' and 'MarkerEdgeAlpha': scalar in range [0,1] | 'flat' 
% 'AlphaData': scalar | vector
% 'AlphaDataMapping': 'none' | 'scaled' | 'direct'
%
% using alpha() on Scatter objects will apply to both Face and Edge alphas
import matlab.graphics.internal.isCharOrString;
if isfloat(param) && isscalar(param)
    proptype={'MarkerFaceAlpha','MarkerEdgeAlpha'};
    data = {param param};
elseif isnumeric(param) 
    if numel(param) == numel(obj.XData)
        proptype={'AlphaData'};
        data={param};
    else
        proptype={'ignore'};
...
elseif isCharOrString(param)
    switch lower(param)
        ...
        case {'x','y','z'}
            proptype = {'AlphaData'};
            data = {obj.([upper(char(param)) 'Data'])};
        case 'color'
            sz = size(obj.CData);
            proptype={'AlphaData'};
            % CData can be: one RGB triplet, n RGB triplets, or a length(n) vector 
            if isequal(sz,[1 3])
                % one rgb triplet
                data = {ones(size(obj.XData))};
                proptype{2}='AlphaDataMapping';
                data{2}='none';
            elseif min(sz)==1
                % vector
                data = {obj.CData};
            elseif sz(2)==3
                cmap = colormap(ancestor(obj,{'Axes','Figure'}));
                data = {rgb2ind(reshape(obj.CData,[],1,3),cmap)};
            end
        case 'rand'
            proptype={'AlphaData'};
            data={rand(size(obj.XData))};
        case {'scaled','direct','none'}
            proptype = {'AlphaDataMapping'};
            data = {param};
        case {'texture' 'interp'}
...
for i = 1:numel(proptype)
    if ~strcmpi(proptype{i},'Ignore')
        if strcmpi(proptype{i},'AlphaData')
            if isempty(data{i}) 
                % If 'AlphaData' is going to be set to empty, make sure 
                % 'Marker*Alpha' properties are not 'flat'.
                if ~isa(obj.MarkerFaceAlpha,'double')
                    obj.MarkerFaceAlpha = 1;
                end
                if ~isa(obj.MarkerEdgeAlpha,'double')
                    obj.MarkerEdgeAlpha = 1;
                end
            else
                % If 'AlphaData' is going to be set to a non-empty, update
                % the 'Marker*Alpha' properties to 'flat' so that 'AlphaData' 
                % is not ignored.
                obj.MarkerFaceAlpha = 'flat';
                obj.MarkerEdgeAlpha = 'flat';
            end
        end
        set(obj,proptype{i},data{i});
        success = true;
    else
        success = false;
    end
