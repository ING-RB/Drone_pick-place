...
if nargs == 1
    ...
    hh = matlab.graphics.chart.primitive.Surface('ZData',z,'EdgeColor','flat', ...
...
elseif nargs == 2
    ...
    [mx, nx] = size(x);
    if mx == my && nx == ny
        hh = matlab.graphics.chart.primitive.Surface('ZData',x,'CData',y,'EdgeColor','flat', ...
            'FaceLighting','none','EdgeLighting','flat','Parent',parax);
    else
        if my*ny == 2 % must be [az el]
            hh = matlab.graphics.chart.primitive.Surface('ZData',x,'EdgeColor','flat', ...
                'FaceLighting','none','EdgeLighting','flat','Parent',parax);
            set(cax,'View',y);
            user_view = 1;
        else
...
elseif nargs == 3
    ...
    if min(size(y)) == 1 && min(size(z)) == 1 % old style
        ...
        hh = matlab.graphics.chart.primitive.Surface('XData',x,'YData',y,'ZData',z,...
            'EdgeColor','flat','FaceLighting','none','EdgeLighting','flat','Parent',parax);
    end
elseif nargs == 4
...
matlab.graphics.internal.matchBackgroundColor(cax,hh,'FaceColor');
