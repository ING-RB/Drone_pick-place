classdef PrimitiveBubbleChartPropertyView <  matlab.graphics.internal.propertyinspector.views.DataspaceMixin & matlab.graphics.internal.propertyinspector.views.CommonPropertyViews & matlab.graphics.internal.propertyinspector.views.IconDataMixin
    ...
    methods(Static)
        function iconProps = getIconProperties(hBubble)
            iconProps.shape = string(matlab.graphics.internal.propertyinspector.views.Shapes.circle);  
            iconProps.edgeColor = hBubble.MarkerEdgeColor;
            iconProps.faceColor = hBubble.MarkerFaceColor;
            % use CData if the color is flat 
            if strcmpi(iconProps.edgeColor,'flat')
                iconProps.edgeColor = hBubble.CData(1,:);
            end
            if strcmpi(iconProps.faceColor,'flat')
                iconProps.faceColor = hBubble.CData(1,:);
            end
            % setting the color to auto makes it white
    ...
    methods
        function this = PrimitiveBubbleChartPropertyView(obj)
            this@matlab.graphics.internal.propertyinspector.views.CommonPropertyViews(obj);
            
            g3 = this.createGroup(getString(message('MATLAB:propertyinspector:Markers')),'','');
            g3.addProperties('LineWidth','MarkerEdgeColor','MarkerFaceColor');
            g3.Expanded = true;
