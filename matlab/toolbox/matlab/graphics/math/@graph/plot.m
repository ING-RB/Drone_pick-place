...
%
%   PLOT(G,LINESPEC) plots the undirected graph G using the color,
%   marker, and line style specified by LINESPEC. For example, use
%   'ro--' to plot red circles as nodes and red dashed lines as edges.
%
%   PLOT(G,...,'Layout',L) plots the graph G using the layout method
%   specified by L, which must be one of the following:
...
G = args{1};
validateattributes(G,{'graph'},{},nameOffset);
...
if needAutoColor && ~isempty(cax) % if cax is empty, we'll error in the GraphPlot constructor...
    [~,nextColor,~] = matlab.graphics.chart.internal.nextstyle(cax,true,false,true);
...
if hasNodeNames
    args = [{'NodeNames', names}, args];
end
[w, hasEdgeWeights] = getEdgeWeights(G);
if hasEdgeWeights
    args = [{'EdgeWeights', w}, args];
end
...
if matches(cax.NextPlot, ["replaceall","replace"])
    cax.Box = 'on';
    if ~useData
        cax.XTick = [];
        cax.YTick = [];
...
%--------------------------------------------------------------------------
function [plotargin, is3D, useData, needAutoColor] = checkinputnames(plotargin,nameoffset)
...
names = [setdiff(properties('matlab.graphics.chart.primitive.GraphPlot'),...
        {'Children','ArrowSize','ArrowPosition','Annotation','BeingDeleted','Type'}); ...
...
for i = 1:2:numel(plotargin)
    ...
    elseif matches(plotargin{i}, ["XData", "YData", "ZData"])
        useData = true;
    elseif strcmp(plotargin{i}, 'NodeColor')
        setNodeColor = true;
    elseif strcmp(plotargin{i}, 'EdgeColor')
        setEdgeColor = true;
    end
...
needAutoColor = ~setNodeColor || ~setEdgeColor;
end
