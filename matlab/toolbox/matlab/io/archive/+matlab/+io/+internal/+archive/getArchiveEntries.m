...
if ~isempty(files)
    inputs = struct('path','','file','', 'OriginalEntry', true);
...
for i = 1:length(files)
    ...
    if matlab.io.internal.archive.isAbsolute(filename)
    ...
    % used to determine if a filename with a wildcard
    % was provided as input or generated by dir
    inputs(i).OriginalEntry = true;
end
...
while ~isempty(inputs)
    ...
    file    = inputs(1).file;
    originalEntry = inputs(1).OriginalEntry;
    inputs(1) = [];

    if contains(pathstr, '*')
        % wildcards are not supported in filepaths. Supported in filenames only
        error(message("MATLAB:io:archive:getArchiveEntries:FilepathHasWildcard"));
    end
    ...
    if hasStar && ~ispc && isfolder(fullFileName)
        error(message("MATLAB:io:archive:getArchiveEntries:WildcardInFolderName"));
    end
    ...
    if hasStar && originalEntry
        ...
        if contains(name, "**")
            ...
            error(message("MATLAB:io:archive:getArchiveEntries:FilenameHasRecursiveWildcard"));
        end
    ...
    elseif ~isFile(fullFileName, dirContents)
        ...
        error(eid,'%s',getString(message('MATLAB:io:archive:getArchiveEntries:fileDoesNotExist',fullFileName)));
        ...
        if  rootDirIsPWD && ~matlab.io.internal.archive.isAbsolute(fullFileName)
...
for i = 1:length(dirContents)
    ...
    inputs(end).OriginalEntry = false;
end
...
%--------------------------------------------------------------------------
function absoluteFileName = getAbsoluteFileName(filename, dirContents, hasTilde, fcnName)
% Return the absolute filename if it's not in the current directory,
% otherwise return filename.

if isscalar(dirContents) && ~hasTilde
    % File is found in pwd
    absoluteFileName = filename;
else
    % Looks up the input filename on the MATLAB Search Path - Explicitly
    % set ResolveSymbolicLinks and CaseCorrectAndResolveOnWindows to false
    % to prevent future compatibility issues if their default values change.
    result = matlab.io.internal.filesystem.resolvePath(filename, ResolveSymbolicLinks=false, ...
        CaseCorrectAndResolveOnWindows=false);
    if result.Type == "File" || result.Type == "SymbolicLink"
        absoluteFileName = char(result.ResolvedPath);
    else
        eid = sprintf('MATLAB:%s:fopenError',fcnName);
        error(eid,'%s',getString(message('MATLAB:io:archive:getArchiveEntries:fopenError',filename)));
    end
end
