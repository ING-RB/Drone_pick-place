...
elseif isnumeric(files) || ~iscell(files) || ...
   ...
   error(message('MATLAB:io:archive:checkFilesDirInputs:isNumeric'));
end
...
if numel(varargin) >= 1 && ~isempty(varargin{1})
   ...
   if ~ischar(rootDir) || ~exist(rootDir,'dir')
      error(message('MATLAB:io:archive:checkFilesDirInputs:nonExistDir',num2str(rootDir)))
   end
else
   rootDir = '';
end
...
if numel(varargin) == 2 && ~isempty(varargin{2})
   ...
   if ~ischar(outputDir)
      error(message('MATLAB:io:archive:checkFilesDirInputs:invalidDirname', outputDir))
   elseif ~isfolder(outputDir)
      if isfile(outputDir)
          error(message('MATLAB:io:archive:checkFilesDirInputs:invalidDir', outputDir));
      end
