...
[files, rootDir] = matlab.io.internal.archive.checkFilesDirInputs(archiveFcn, files, varargin{:});

% archiveFilename must be a character string
if ~ischar(archiveFilename)
   eid = sprintf('MATLAB:%s:invalidFileString', archiveFcn);
   error(eid, '%s', ...
      getString(message('MATLAB:io:archive:parseArchiveInputs:invalidFileString', upper(archiveFcn))));
end

% Get the compression function if requested
[~, ~, archiveExt]=fileparts(archiveFilename);
if nargout >= 4
   compressFcn = matlab.io.internal.archive.getArchiveCompressFcn(archiveExt, archiveFcn);
else
   compressFcn = [];
end
...
if isempty(compressFcn) && ...
      (isempty(archiveExt) || ~strcmpi(archiveExt,defArchiveExt))
...
if isempty(name) || exist(archiveFilename,'dir') 
   ...
   error(eid, '%s', ...
      getString(message('MATLAB:io:archive:parseArchiveInputs:isDirectory', upper(archiveFcn), archiveFilename)));
end
