classdef DetectImportOptionsWordDocument < matlab.io.internal.functions.ExecutableFunction & ...
    ...
    methods
        function opts = execute(func, supplied)
            ...
            if strcmp(func.DatetimeType,'exceldatenum')
                error(message('MATLAB:textio:detectImportOptions:ExcelDateWithWordDocument'));
            end
            ...
            func.WordDocument =  matlab.io.xmltree.internal.XMLTree.fromDOCX(func.Filename);
            func.WordDocument.registerNamespace("w","http://schemas.openxmlformats.org/wordprocessingml/2006/main");


            detectedOpts = matlab.io.word.internal.detection.detect(func.WordDocument,func.getOptionsForDetection(),supplied);
    ...
    methods (Access = private)
        function func = getOptionsForDetection(func)
            if ismissing(func.TableSelector) || isempty(func.TableSelector)
                func.TableSelector = "(//w:tbl)[" + func.TableIndex + "]";
            end
        end
    end
...
function opts = buildOptsFromDetection(detectedOpts, supplied, func)
    opts = matlab.io.word.WordDocumentImportOptions(...
    ...
    opts.VariableNames = makeVariableNames(func, ...
        detectedOpts.VariableNames, func.ReadVariableNames);
    opts.SelectedVariableNames = opts.VariableNames(detectedOpts.SelectedVariables);
end

function variableNames = makeVariableNames(func, variableNames, readVariableNames)
...
function checkWrongParamsWrongType(supplied)
    ...
    if isempty(params)
        ...
        % as "parameter" properties. These parameters are supported for Word files, so
    ...
    matlab.io.internal.utility.assertUnsupportedParamsForFileType(params,supplied,'word');
end
