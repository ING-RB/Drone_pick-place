classdef LogicalTypeConversionOptions < matlab.mixin.Scalar
    %LOGICALTYPECONVERSIONOPTIONS Controls conversion of logical types.
    ...
    methods
        function obj = LogicalTypeConversionOptions(opts)
            arguments
                opts.?matlab.io.internal.arrow.conversion.LogicalTypeConversionOptions
            end
    ...
    methods(Static, Hidden)
        function obj = loadobj(s)
            import matlab.io.internal.arrow.conversion.LogicalTypeConversionOptions

            % Error if we are sure that a version incompatibility is about to occur.
            if s.EarliestSupportedVersion > LogicalTypeConversionOptions.ClassVersion
                error(message("MATLAB:io:common:validation:UnsupportedClassVersion"));
            end

            obj = LogicalTypeConversionOptions();
