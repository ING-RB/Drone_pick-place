classdef HTMLImportOptions < matlab.io.ImportOptions & ...
    ...
    %matlab.io.html.HTMLImportOptions Options for importing data from an HTML file
    %
    ...
    %
    %   See Also: detectImportOptions, readtable, htmlImportOptions,
    ...
    methods (Access = {?matlab.io.internal.functions.ReadTableWithImportOptionsHTML})
        function params = getOptionsStruct(obj)
            params = struct();
            params.ParserType = 'html';
            ...
            params.MergedCellColumnRule = obj.MergedCellColumnRule;
            params.MergedCellRowRule = obj.MergedCellRowRule;
    ...
    methods (Access = protected)
        function str = saveLoadNames(obj)
            % setdiff(string(properties('matlab.io.html.HTMLImportOptions')), string(properties('matlab.io.ImportOptions')))
    ...
    methods(Static, Hidden)
        ...
        % always construct a default HTMLImportOptions
            obj = matlab.io.html.HTMLImportOptions();

            % Return a default HTMLImportOptions if the current object
            % version is less than the minimum compatible version of the
            % serialized object.
            if obj.isIncompatible(serialized, 'MATLAB:io:html:saveload:IncompatibleLoad')
                return;
            end
...
% Tries to set property to the saved value.
    try
        ...
        if ~strcmp(ME.identifier, "MATLAB:nonExistentField")
            ...
            warning(message('MATLAB:io:html:saveload:IncompatiblePropertyLoad',...
                            'matlab.io.html.HTMLImportOptions', prop));
        end
    end
end
