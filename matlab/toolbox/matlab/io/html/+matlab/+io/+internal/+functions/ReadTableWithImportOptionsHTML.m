classdef ReadTableWithImportOptionsHTML < matlab.io.internal.functions.ExecutableFunction &...
    ...
    methods
        function [T, func] = executeImpl(func,supplied)
            ...
            try
                HTMLDocument = func.HTMLDocument;
                assert(isa(HTMLDocument,' matlab.io.xmltree.internal.XMLTree'));
            catch
                htmlData = fileread(func.Filename);
                HTMLDocument =  matlab.io.xmltree.internal.XMLTree.fromHTML(htmlData);
            end
            ...
            selectedTable = HTMLDocument.xpath(selector);
            ...
            if isempty(selectedTable)
                ...
                [tableStringData,rowsWithHorzSpan,colsWithVertSpan] = ...
                    matlab.io.html.internal.htmlTable2str(selectedTable, opts_struct);
            end
            ...
            if opts_struct.EmptyRowRule == "skip"
                dropRow = dropRow | emptyRows;
            elseif opts_struct.EmptyRowRule == "error" && any(emptyRows)
                error(message('MATLAB:textio:io:EmptyRowRuleError', find(emptyRows,1)));
            else
                % For consistency with detection, always skip empty rows
                % before meta headers
                firstNonEmpty = find(~emptyRows,1);
                dropRow(1:(firstNonEmpty-1)) = true;
            end
            ...
            for i = 1:numel(out.Data)
                ...
                [omitrows, omitvars] = processRule(col_info.Placeholders(:, 1), missingRule,...
                    omitrows, omitvars, "MATLAB:io:html:readtable:MissingRuleError", i);
                [omitrows, omitvars] = processRule(col_info.Errors, errorRule,...
                    omitrows, omitvars, "MATLAB:io:html:readtable:ImportErrorRuleError", i);
            end
...
matlab.io.internal.utility.assertUnsupportedParamsForFileType(wrongparams,supplied,'html')
end
