    ...
    if contains(filename, "*")
        ...
        elseif isfolder(str)
            ...
            if ~isempty(listing)
                ...
                isFolderPresent = matlab.io.ftp.internal.matlab.isFolder(...
...
function location = recursiveUpload(obj, listing, remoteRelativeFolder)
    ...
    for ii = 1 : numel(listing)
        ...
        if listing(ii).isdir
            ...
            if ~isFolderPresent
                mkdir(obj, remoteRelativePath);
            end
            skipThis(ii) = 1;
        else
            if isempty(remoteRelativePath)
                fileNameIsEmpty = true;
            else
            ...
            if fileNameIsEmpty
                remoteRelativePath = fullfile(remoteRelativePath,listing(ii).name);
            end
