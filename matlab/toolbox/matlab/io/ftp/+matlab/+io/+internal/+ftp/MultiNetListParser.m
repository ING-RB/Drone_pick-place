classdef MultiNetListParser < matlab.io.internal.ftp.FTPListResultParser
    methods (Access = protected)
        function dirStruct = parseDirLines(~, lines, serverLocale, datetimeType)
            ...
            for i = 1 : numel(lines)
                ...
                name = name(1);
                filesAndFolders = split(name, ".");
                isdir = filesAndFolders(2) == "DIR";
                name(isdir) = extractBefore(name(isdir), ".DIR");
                bytes = NaN(size(name, 1));
                dates = datetime(thisLine(3) + " " + thisLine(4) + ":00", ...
