function filename = validateCertificate(filename)
...
% Copyright 2016-2019 The MathWorks, Inc.
    ...
    if ~isempty(filename) && ~(strcmpi(filename, 'default'))
        if exist(filename,'file')
            ...
            if fid > 0
                ...
                if isempty(regexp(c,'-----BEGIN[a-zA-Z_0-9 ]*CERTIFICATE-----\r*\n.*?-----END[a-zA-Z_0-9 ]*CERTIFICATE-----\r*\n','once'))
                    validatedFile = [];
                    error(message('MATLAB:io:ftp:ftp:BadCertificateFile'));
                end
                ...
                error(message('MATLAB:io:ftp:ftp:CertificateFileNotFound'));
            end
        else 
            error(message('MATLAB:io:ftp:ftp:CertificateFileNotFound'));
        end
