...
try
    ...
    isSupportedType = iscell(data) || isnumeric(data) || ischar(data) || islogical(data);
    isSparseType = issparse(data);
    % Check class of input data
    if ~isSupportedType || isSparseType
...
try
    ...
    try
        dlmwrite(file,data,','); %#ok<DLMWT> % write data.
    catch exception
...
try
    ...
    if isempty(strfind(range,':')) %#ok<*STREMP>
