...
classdef (Sealed, Hidden) SequenceFileSplitReader < matlab.io.datastore.splitreader.SplitReader
    ...
    % then loaded, reset must be called before further reading.
    ...
    properties (Hidden, Transient)
        ...
        PrevFilename; % The file that was read prior to the current file
    end
    ...
    methods
        ...
        function [data, info] = getNext(obj)
            ...
            while size < obj.KeyValueLimit && ~obj.ReadingDone
                try
                    if isempty(obj.KeyType)
                        Key(size+1) = obj.NextKey;
                        obj.KeyType = class(Key(size+1));
                    else
                        tempKey = obj.NextKey;
                        Key(size+1) = tempKey;
                        if obj.KeyValueLimit > 1 && ~strcmpi(class(tempKey),obj.KeyType)
                            id = 'MATLAB:datastoreio:keyvaluedatastore:invalidKeyConversion';
                            msg = '';
                            throw(MException(id,msg));
                        end
                    end
                catch e
                    invalidKeyError(obj, obj.KeyType, e);
                end
        ...
        function reset(obj)
            close(obj);
            % Reset the state of the Position properties too.
            obj.Position = 0;
            obj.PrevPosition = 0;
            initialize(obj);
        end
        ...
        % Percentage of read completion between 0.0 and 1.0 for the split.
            ...
            % Use the previous position of the read pointer here since the
            % next() method was preemptively called.
            readSize = obj.PrevPosition - obj.Split.Offset;
            
            % position is not updated on the final read method call; account
            % for this by checking for hasNext
            if ~hasNext(obj)
                readSize = obj.Split.Size;
            end 
    ...
    methods (Access = private)
        ...
        function next(obj)
            ...
            hadoopLoader = com.mathworks.storage.hdfsloader.GlobalHdfsLoader.get();
            if obj.ValuesOnly
                keyObject = hadoopLoader.newNullWritable();
            else
                keyObject = hadoopLoader.newMxArrayWritable2();
            end
            valueObject = hadoopLoader.newMxArrayWritable2();

            try
                obj.ReadingDone = ~obj.InternalSequenceReader.next(keyObject, valueObject);
            catch ME
                if strcmp(ME.identifier, 'MATLAB:Java:GenericException')
                    error(message('MATLAB:datastoreio:sequencefilesplitreader:unreadableFile',obj.Split.Filename));
                end
                rethrow(ME);
            end
            if ~obj.ReadingDone
                if obj.ValuesOnly
                    obj.NextValue = mxarraywritableproxydeserialize(valueObject);
                else
                    obj.NextKey = mxarraywritableproxydeserialize(keyObject);
                    obj.NextValue = {mxarraywritableproxydeserialize(valueObject)};
                end
        ...
        function createNewISequenceReader(obj)
            ...
            [keyClassName, valueClassName] = SequenceFileReader.getKeyValueClasses(obj.InternalSequenceReader);
            tf = SequenceFileReader.isValidSequenceClassNames(keyClassName, valueClassName, obj.ValuesOnly);
            assert(tf, 'Assertion failed: Sequence file contains unsupported class type.');
        end
