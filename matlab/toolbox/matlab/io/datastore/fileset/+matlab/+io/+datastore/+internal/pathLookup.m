        ...
        case 1
            ...
            for ii = 1:np
                ...
                try
                    ...
                    if e.identifier == "MATLAB:datastoreio:pathlookup:fileNotFound"
                        validateCloudEnvVariables(pth);
                    end
                ...
                try
                    if isIRI(pth)
                        try
                            ...
                            if err.identifier == "MATLAB:datastoreio:pathlookup:fileNotFound"
                                validateCloudEnvVariables(pth);
                            end
                            rethrow(err);
                        end
...
function iHandleEmptyFolderError(e, pth, noSuggestionInErr)
    ...
    isIri = matlab.io.datastore.internal.isIRI(pth);
    if ~isIri && matlab.io.internal.vfs.validators.hasIriPrefix(pth)
        error(message("MATLAB:virtualfilesystem:invalidIRI", pth));
    end
