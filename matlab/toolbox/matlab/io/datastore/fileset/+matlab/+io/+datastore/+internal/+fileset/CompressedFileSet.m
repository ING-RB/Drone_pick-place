...
%   Copyright 2017-2023 The MathWorks, Inc.
    ...
    methods (Hidden)
        ...
        % Forward the isequaln check to the isequal check.
        function tf = isequaln(varargin)
            %ISEQUALN Compare CompressedFileSet objects
        ...
        function newCopy = copyWithFileIndices(fs, indices, varargin)
            ...
            if ~isempty(newCopy.FileSizes)
                newCopy.EndOffset = newCopy.FileSizes(end);
            else
                newCopy.EndOffset = 0;
            end
        ...
        function setShuffledIndices(fs, idxes)
            ...
            %   the new indices.
            if numel(idxes) ~= fs.NumFiles || ~isempty(setdiff(1:fs.NumFiles, idxes))
        ...
        function setDuplicateIndices(fs, duplicateIndices, addedIndices)
            ...
            fs.NumFiles = numel(duplicateIndices);
        end
        ...
        function newCopy = copyAndOrShuffle(fs, indices)
            ...
            isUniqueIndices = numel(unique(indices)) == numel(indices);
            containsDuplicates = any(~a) || ~isUniqueIndices;
            if containsDuplicates
                ...
                newIndices = 1:numel(uniqueIndices);
                b(a) = newIndices;
                b = b(indices);
                if ~isequal(b,newIndices)
                    % Set any shuffling caused by the new list of indices,
                    % only when the indices are not ordered (shuffled).
                    setShuffledIndicesOnly(newCopy, b);
                end
                reset(newCopy);
            else
                % shuffle
    ...
    methods (Access = private)
        ...
        function tf = compareIfEqual(fs1, fs2)
            ...
            emptyS1ShuffIndices = isempty(s1.ShuffledIndices);
            if emptyS1ShuffIndices
                s1.ShuffledIndices = 1:fs1.NumFiles;
            end
            emptyS2ShuffIndices = isempty(s2.ShuffledIndices);
            if emptyS2ShuffIndices
                s2.ShuffledIndices = 1:fs2.NumFiles;
            end
            tf = isequal(s1,s2);
    ...
    methods (Access = protected)
        ...
        function setTransformedFiles(fs, folders)
            [~, outfiles] = getPaths(fs.InternalPaths, 1:fs.NumFiles);
            setInitialInternalPaths(fs, [folders, outfiles]);
            fs.FileNamesForTransform = [];
        end

        function folders = getFilesForTransform(fs)
            [folders, ~] = getPaths(fs.InternalPaths, 1:fs.NumFiles);
            if ~isempty(folders)
