...
%   Copyright 2018-2024 The MathWorks, Inc.
    ...
    methods (Static)
        ...
        function [fs, fileExts, folderNames, folders] = buildCompressed(location, nvStruct)
        ...
        function fs = buildFromFileInformation(nvStruct)
            ...
            % A datastore relying on ResolvedFileSetFactory will be backed by a FileSet by default (previously DsFileSet).
            fs = matlab.io.datastore.FileSet(internalFs);
        end
        ...
        function nvStruct = buildFileInformation(location, nvStruct)
            ...
            if isstruct(location)
                ...
                % Add the Location input on the nvStruct. This is used
                % later when populating the Folders property.
                nvStruct.Location = location;
...
function foldernames = iGetFolderNames(nvStruct)
    ...
    folderList = nvStruct.Files(:, 1);
    if any(startsWith(folderList, ["http://", "https://"]))
        error(message("MATLAB:io:datastore:common:validation:FoldernamesNotAllowedForHttp"));
    end

    P = matlab.io.internal.filesystem.Path(folderList);
    foldernames = cellfun(@(x,y)repmat({x},numel(y), 1), P.Name, ...
        nvStruct.Files(:,2), UniformOutput=false);
