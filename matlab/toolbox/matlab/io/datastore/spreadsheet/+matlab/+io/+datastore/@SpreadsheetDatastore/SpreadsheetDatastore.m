...
%                        SpreadsheetDatastore is always partitionable.
%   isShuffleable   -    Returns true if this datastore is shuffleable.
%                        SpreadsheetDatastore is not shuffleable.
%
...
%   Copyright 2015-2024 The MathWorks, Inc.
    ...
    properties (Access = private, Transient)
        %REMOTETOLOCAL Object created from the first file.
        RemoteToLocalObject;
    ...
    methods
        function ds = SpreadsheetDatastore(location, varargin)
            ...
            try
                ...
                initOutputTypeAndRowTimes(ds, resStruct.OutputType,...
                    resStruct.RowTimes, usingDefaults);

                % complete construction of the datastore
                completeConstruction(ds);

                % SchemaVersion indicates the release number of MATLAB.
                ds.SchemaVersion = matlab.io.datastore.internal.getVersionString();
    ...
    methods
        ...
        function set.Files(ds, files)
            try
                ...
                % Recalculate the Folders property on the next get.Folders.
        ...
        function set.Sheets(ds, sheets)
            try
                if isa(sheets,'numeric')
                    sheets = double(sheets);
                end
        ...
        function set.NumHeaderLines(ds, hdrLines)
            try
                if isa(hdrLines,'numeric')
                    hdrLines = double(hdrLines);
                end
    ...
    methods (Access = 'private')
        ...
        function file = getFirstFileName(ds)
            import matlab.io.datastore.mixin.RemoteToLocalFile;
            if ~matlab.io.datastore.internal.isIRI(ds.Files{1}) || ...
                    matlab.io.internal.common.validators.isGoogleSheet(ds.Files{1})
                file = ds.Files{1};
                return;
            end
            if isempty(ds.RemoteToLocalObject)
                ds.RemoteToLocalObject = RemoteToLocalFile(ds.Files{1});
            end
            file = ds.RemoteToLocalObject.LocalFileName;
        end
        ...
        %INITSHEETPROPERTIES Initialize the SpreadsheetDatastore properties
            ...
            ds.PrivateReadFailureRule = 'error';
            ds.PrivateMaxFailures = Inf;
        end
        ...
        %   after datastore construction.
            ...
            if ~isempty(prevFiles)
                ...
                % set to table for introspection
            ...
            if ~isempty(currFiles) && ~isempty(prevFiles)
                ...
                if strcmp(currFirstFile, prevFirstFile)
                    ...
                    % reset the first file name object
                    % when the files are different
                    ds.RemoteToLocalObject = [];
                end
                
                % try resetting the OutputType if both current Files and
                % the previous files are non-empty
        ...
        %VALIDATESHEETS Validates the sheets.
            ...
            sheetNamesOrNumbers = convertStringsToChars(sheetNamesOrNumbers);
            ...
            elseif isCharVector(sheetNamesOrNumbers)
                ...
                if ~isCellOfCharVectors(sheetNamesOrNumbers)
                    error(message('MATLAB:datastoreio:spreadsheetdatastore:invalidSheets', ds.Files{1}));
                end
        ...
        %VALIDATEVARIABLENAMES Validates the Variable Names
            ...
            if ds.ConstructionDone
                ...
                % make valid variable names (valid and unique) based on the
        ...
        %VALIDATEFORMATS Validates the VariableTypes
            ...
            prevVarTypes = ds.PrivateVariableTypes;
            ds.PrivateVariableTypes = varTypes;
            if ds.ConstructionDone
                try
                    ds.checkRowTimeType();
                catch
                    ds.PrivateVariableTypes = prevVarTypes;
        ...
        %VALIDATESELECTEDVARIABLENAMES Validates the SelectedVariableNames
            ...
            prevSelVarNamesIdx = ds.SelectedVariableNamesIdx;
            ds.SelectedVariableNamesIdx = locb;
            
            if ds.ConstructionDone
                try
                ds.checkRowTimeSelected();
                catch
                    ds.SelectedVariableNamesIdx = prevSelVarNamesIdx;
        ...
        %VALIDATESELECTEDVARIABLETYPES Validates the SelectedVariableTypes
            ...
            prevVarTypes = ds.PrivateVariableTypes(varsUsedIdx);
            ...
            if ds.ConstructionDone
                try
                    ...
                    ds.PrivateVariableTypes(varsUsedIdx) = prevVarTypes;
                     error(message("MATLAB:datastoreio:outputtype:rowTimeTypeChanged",...
                        ds.RowTimes));
                end
            end
        ...
        %postive interger or the strings 'file' or 'sheet'
            ...
            if isCharVector(readSize) && strcmpi(readSize, SpreadsheetDatastore.READSIZE_FILE)
            ...
            elseif isCharVector(readSize) && strcmpi(readSize, SpreadsheetDatastore.READSIZE_SHEET)
        ...
        %   stable state if anything goes wrong in the re-introspection.
            ...
            if flagToRead
                try
                    % For introspection, set OutputType to "table"
                    timeVarIdx = ds.TimeVariableIndex;
                    ds.OutputType = "table";
    ...
    methods (Hidden)
        ...
        %ISEQUALN   Return whether input SpreadsheetDatastore objects are
        %           equal, based on equality of the objects' properties.
        %           SpreadsheetDatastore inherits from
        ...
        %           comparison.
            isSpreadsheetDatastore = @(x) isa(x, "matlab.io.datastore.SpreadsheetDatastore");
            
            tf = true;

            % Verify that the object classes are correct and the properties are
            % equal
            if ~isSpreadsheetDatastore(ds) || ~isSpreadsheetDatastore(obj2) ...
            ...
            for objIdx = 1:length(varargin)
                obj = varargin{objIdx};
                if ~isSpreadsheetDatastore(ds) || ~isSpreadsheetDatastore(obj) ...
    ...
    methods (Static, Hidden)
        ...
        function varargout = supportsLocation(loc, nvStruct)
            ...
            import matlab.io.internal.spreadsheet.fileAttributesForGoogleSheet;
            import matlab.io.datastore.FileBasedDatastore;
            defaultExtensions = {'.xls', '.xlsx', '.xlsm', '.xltm', '.xltx'};
            tempLoc = loc; % keep backup to use for maintaining order of inputs

            % check if any input is a Google sheet and remove from list of
            % inputs provided to datastore path lookup infrastructure
            gsLocations = matlab.io.internal.common.validators.isGoogleSheet(loc);
            if any(gsLocations)
                if ischar(loc) && ~isscalar(loc)
                    % datastore only has single file and its a Google sheet.
                    files = [];
                    fileSizes = [];
                    tf = false;
                else
                    % datastore has both Google sheets and regular spreadsheets
                    loc(gsLocations) = [];
                    [tf, files, fileSizes] = FileBasedDatastore.supportsLocation(loc, ...
                        nvStruct, defaultExtensions);
                end
            else
                % return early since paths contain no Google Sheets
                [varargout{1:nargout}] = FileBasedDatastore.supportsLocation(loc, ...
                    nvStruct, defaultExtensions);
                return;
            end

            % get accurate size of Google sheet, place outputs back in the
            % proper order based on input ordering
            if ischar(tempLoc)
                % only single file in datastore
                cellOut = false;
                sizeOfOutput = 1;
            else
                % more than one file in datastore
                cellOut = true;
                sizeOfOutput = numel(tempLoc);
            end
            retFiles = cell(sizeOfOutput, 1);
            retFileSizes = zeros(sizeOfOutput, 1);
            jj = 1;
            for ii = 1 : sizeOfOutput
                if gsLocations(ii)
                    if cellOut
                        retFiles{ii} = tempLoc{ii};
                        googlesheetID = matlab.io.internal.common.validators.extractGoogleSheetIDFromURL(tempLoc{ii});
                        retFileSizes(ii) = fileAttributesForGoogleSheet(googlesheetID);
                    else
                        retFiles{ii} = tempLoc;
                        googlesheetID = matlab.io.internal.common.validators.extractGoogleSheetIDFromURL(tempLoc);
                        retFileSizes(ii) = fileAttributesForGoogleSheet(googlesheetID);
                    end
                else
                    retFiles{ii} = files{jj};
                    retFileSizes(ii) = fileSizes(jj);
                    jj = jj + 1;
                end
            end
            varargout{1} = tf;
            varargout{2} = retFiles;
            varargout{3} = retFileSizes;
        end
        ...
        function ds = loadobj(ds)
            ...
            switch class(ds)
            case 'struct'
                if isfield(ds, "PrivateOutputType")
    ...
    methods (Access = 'protected')
        ...
        function updateVariableNamesWithPreservationBehavior(ds)
            % Avoid doing the update if VariableNames is empty.
            if ds.ConstructionDone && ~isempty(ds.VariableNames)
...
%   parameters is done in this function.
    ...
    if isempty(pSheet)
        ...
        % Check for PreserveVariableNames using the InputParser.
        PreserveVariableNamesInput.configureInputParser(pSheet);
        
        addParameter(pSheet, 'OutputType', "auto");
        addParameter(pSheet, 'RowTimes', "");
    end

    % parse the input
    parse(pSheet, varargin{:});
    if ~isa(pSheet.Results.IncludeSubfolders,'logical') && ~isnumeric(pSheet.Results.IncludeSubfolders)
