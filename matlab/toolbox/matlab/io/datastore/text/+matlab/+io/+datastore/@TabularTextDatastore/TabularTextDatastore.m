classdef (Sealed) TabularTextDatastore < ...
                  ...
                  matlab.io.datastore.internal.ReturnsTabularData
...
%                        TabularTextDatastore is always partitionable.
%   isShuffleable   -    Returns true if this datastore is shuffleable.
%                        TabularTextDatastore is not shuffleable.
%
...
%   DatetimeLocale           - Specify the correct locale for importing
%                              dates.
%   OutputType               - Selects the data type of the output returned from the 
%                              preview, read, and readall functions.
%   RowTimes                 - Name of the time variable for OutputType "timetable".
%   SupportedOutputFormats   - List of formats supported for writing by
%                              this datastore.
...
%   Copyright 2014-2023 The MathWorks, Inc.
    ...
    properties (Dependent)
        ...
        %   as a character vector or string scalar. All the file encodings
        %   supported in the data import area can be provided as inputs.
        FileEncoding;
        ...
        %   vector, a cell array of character vectors, or a string array.
        %   Specify the variable names in the order in which they appear in
        %   the files. If you do not specify the variable names, they are
        %   detected from the first non-header line in the first file in
        %   the datastore. When modifying the VariableNames property, the
        %   number of new variable names must match the number of original
        %   variable names.
        VariableNames;
        ...
        %   single character vector, cell array of character vectors, or a
        %   string array. This option only applies to numeric fields.
        TreatAsMissing;
        ...
        %TEXTSCANFORMATS Format of the data fields.
        %   Format of the data fields, specified as a cell array of character
        %   vectors or a string array. Each character vector in the cell
        %   array or each element in the string array must contain one 
        ...
        %EXPONENTCHARACTERS Exponent characters.
        %   Exponent characters, specified as a character vector or string
        %   scalar. The default exponent characters are e, E, d, and D.
        ExponentCharacters;
        ...
        %WHITESPACE White-space characters.
        %   White-space characters, specified as a character vector or string
        %   scalar of one or more characters.
        Whitespace;
        ...
        %   conversion specifier. You can also set this property using a
        %   string array. The variables to read are indicated by the
        ...
        %DATETIMELOCALE Specify the correct locale for importing dates
        %   Import uses this locale to when reading the month names, day of
        %   week names, etc. When importing from text, if the names do not match
        %   the expected localized names, then the import conversion will
        %   fail. The default value is en_US - English, United States.
        %
    ...
    properties (Access = 'private')        
        ...
        PrivateCustomFormatsSelected = 'unset';
        ...
        % property to store DatetimeLocale
        PrivateDatetimeLocale;
    end

    % private properties to allow friends of TabularTextDatastore to modify
    % internal behavior.
    properties (Access = {?matlab.io.datastore.internal.shim}, Transient)
        % Flag that specifies if the datastore should keep track of all the
        % information necessary to return info structs from the read
        % method.
        ShouldCalcBytesForInfo (1,1) logical = true;
    end
    ...
    properties (Access = protected)        
        %BACKUPSPLITINFO required to check that datastore has moved to the
        %   next file
        BackupSplitInfo = [];
    end
    properties (Constant)
    ...
    methods
        function ds = TabularTextDatastore(location, varargin)
            ...
            import matlab.io.datastore.internal.ReturnsTabularData;
            ...
            try
                ...
                [~, files, fileSizes] = TabularTextDatastore.supportsLocation(location, resStruct);

                ds.populateFoldersFromResolvedPaths(location, {}, files);

                ds.PreserveVariableNames = resStruct.PreserveVariableNames;
                ds.ReadSize = resStruct.ReadSize;

                ds.TotalFiles = numel(files);
                ...
                % need to store the OutputType and RowTimes parameters
                defaults = {'OutputType', 'RowTimes'};
                usingDefaults = intersect(defaults, resStruct.UsingDefaults);
                outputType = resStruct.OutputType;
                rowTimes = resStruct.RowTimes;
                ...
                initOutputTypeAndRowTimes(ds, outputType, rowTimes, usingDefaults);
            catch ME
                throwAsCaller(ME);
            end
        end
    ...
    methods
        ...
        function set.Files(ds, files)
            try
                ...
                % Update the Folders property on the next get.Folders too.
                ds.updateFoldersProperty();
            catch ME
                throw(ME);
            end
        end
        ...
        function set.TreatAsMissing(ds, treatAsMissing)            
            try
                ...
                if ME.identifier == "MATLAB:textio:textio:InvalidStringOrCellStringProperty"
        ...
        % DatetimeLocale setter
        function set.DatetimeLocale(ds, dtLocale)
            dtLocale = convertStringsToChars(dtLocale);
            if isempty(dtLocale)
                error(message('MATLAB:datetime:InvalidLocale'));
            end
            validateDatetimeLocale(ds, dtLocale);
        end
        ...
        % DatetimeLocale getter
        function dtLocale = get.DatetimeLocale(ds)
            dtLocale = ds.PrivateDatetimeLocale;
        end
    ...
    methods (Access = 'protected')
        ...
        % Concrete updateVariableNamesWithPreservationBehavior method to implement
        % PreserveVariableNamesInput.
        function updateVariableNamesWithPreservationBehavior(ds)
            % We check if PrivateVariableNames is non-empty because this
            % method can be invoked on set of PreserveVariableNames during
            % deserialization (as this is a non-dependent property).
            % Depending on order that properties are populated,
            % VariableNames might not be initialized by this point.
            if ds.IntrospectionDone && ~isempty(ds.PrivateVariableNames)
                ds.VariableNames = ds.VariableNames;
            end    
        end
    ...
    methods (Hidden)
        ...
        function frac = progress(ds)
            ...
            % them
            readerHasData = ~isempty(ds.SplitReader) && hasNext(ds.SplitReader);
        ...
        %ISEQUALN   Return whether input TabularTextDatastore objects are
        %           equal, based on equality of the objects' properties.
        %           TabularTextDatastore inherits from
        ...
        %           comparison.
            isTabularTextDatastore = @(x) isa(x, "matlab.io.datastore.TabularTextDatastore");
            
            tf = true;

            % Verify that the object classes are correct and the properties are
            % equal
            if ~isTabularTextDatastore(ds) || ~isTabularTextDatastore(obj2) ...
            ...
            for objIdx = 1:length(varargin)
                obj = varargin{objIdx};
                if ~isTabularTextDatastore(ds) || ~isTabularTextDatastore(obj) ...
    ...
    methods (Static, Hidden)
        ...
        function ds = loadobj(ds)
            ...
            outputLoadArgs = {};
            if isstruct(ds) && isfield(ds, 'PrivateOutputType')
                outputLoadArgs = {ds.PrivateOutputType, ds.TimeVariableIndex};
                ds = rmfield(ds, {'PrivateOutputType',...
                    'TimeVariableIndex', 'OutputTypeInitialized'});
            end
            ...
            % SetOutputType and RowTimes
            ds.loadOutputTypeAndRowTimes(outputLoadArgs{:});
        end
    end
    
    methods (Static, Hidden)
        % supportsLocation for TabularTextDatastore  
        function varargout = supportsLocation(loc, nvStruct, shouldMatchEmptyFileExtensions)
            ...
            % Match empty file extensions by default.
            defaultExtensions = { '.txt', '.csv', '.dat', '.dlm', '.asc', '.text', ''};

            % Split the matching stages into nonempty extensions and empty extensions. 
            if nargin >= 3
                if shouldMatchEmptyFileExtensions
                    defaultExtensions = {''};
                else
                    defaultExtensions = defaultExtensions(1:end-1);
                end
            end
    ...
    methods (Access = 'private')
        ...
        %GETTEXTSCANARGS convert stored parameters to name-value pairs for textscan
            ...
            if ~isempty(ds.TreatAsMissing)
                args(end+1:end+2) = {'TreatAsEmpty', ds.TreatAsMissing};
            end
            args(end+1:end+8) = {'CommentStyle', ds.CommentStyle, ...
                'MultipleDelimsAsOne', ds.MultipleDelimitersAsOne,...
                'TextType', ds.PrivateTextType,...
                'DateLocale', ds.DatetimeLocale};
        end

        function args = getDetectFormatArgs(ds)
        % GETDETECTFORMATARGS convert stored parameters to name-value pairs
        % for matlab.io.text.internal.detectParametersFromString
            args = {'Delimiter', ds.Delimiter, ...
                    'ExpChars', ds.ExponentCharacters, ...
                    'EndOfLine', ds.RowDelimiter, ...
                    'Whitespace', ds.Whitespace, ...
             ...
             if ~isempty(ds.TreatAsMissing)
                ...
                args(end+1:end+2) = {'TreatAsEmpty', ...
                    matlab.io.internal.utility.unescape(ds.TreatAsMissing)};
            end
        end
        ...
        %VALIDATEREADSIZE Validates ReadSize.
            ...
            if isCharVector(readSize) && strcmpi(readSize, TabularTextDatastore.READSIZE_FILE)
                if ds.IntrospectionDone && ~isCharVector(prevReadSize)
                    resetFlag = true;
        ...
        %   row delimiter.
            ...
            if ds.IntrospectionDone
                try
                    % set OutputType to 'table' for introspection
                    timeVarIdx = ds.TimeVariableIndex;
                    ds.OutputType = "table";
                    
                    % introspect
                    introspectFile(ds);
                    
                    % reset OutputType if possible
                    if ~ds.resetOutputTypeAndRowTime(timeVarIdx)
                        warning(message('MATLAB:datastoreio:outputtype:outputTypeSetToTable'));
                    end
        ...
        %   after datastore construction.
            ...
            try
                ...
                if ~isempty(prevFiles) && ~isempty(timeVarIdx)
                    ds.RowTimes = timeVarIdx;
                end
            ...
            if ~isempty(currFiles) && ~isempty(prevFiles)
                ...
                % attempt to preserve the OutputType only if both prevFiles
                % and currFiles are non-empty
                if ~ds.resetOutputTypeAndRowTime(timeVarIdx)
                    warning(message('MATLAB:datastoreio:outputtype:outputTypeSetToTable'));
                end
            end
        ...
        %   be called after introspection.
            ...
            try
                % OutputType set to 'table' for introspection
                if ds.IntrospectionDone
                    timeVarIdx = ds.TimeVariableIndex;
                    ds.OutputType = "table";
                end
                ...
                if ds.IntrospectionDone && ~ds.resetOutputTypeAndRowTime(timeVarIdx)
                    warning(message('MATLAB:datastoreio:outputtype:outputTypeSetToTable'));
                end
                ...
                % Reset OutputType
                if ds.IntrospectionDone && ~isempty(timeVarIdx)
                    ds.RowTimes = timeVarIdx;
                end
                throw(ME);
            end
        ...
        %VALIDATEFORMATS Validates the TextscanFormats
            ...
            % Store values in case new TextscanFormats conflicts with the
            % OutputType
            if ds.IntrospectionDone
                prevFormats = ds.PrivateTextscanFormats;
                prevSelectedFormats = ds.PrivateSelectedFormats;
                prevSelectedVarNames = ds.SelectedVariableNames;
                timeVarIdx = ds.TimeVariableIndex;
                ds.OutputType = "table";
            end
            ...
            if ds.IntrospectionDone && ~isempty(timeVarIdx)
                try
                    ...
                    ds.PrivateTextscanFormats = prevFormats;
                    ds.PrivateSelectedFormats = prevSelectedFormats;
                    ds.SelectedVariableNames = prevSelectedVarNames;
                    ds.RowTimes = timeVarIdx;
                    if strcmpi(ME.identifier, 'MATLAB:datastoreio:outputtype:invalidRowTimeType')
                        error(message('MATLAB:datastoreio:outputtype:rowTimeTypeChanged',...
                            ds.RowTimes));
                    else
                       error(message('MATLAB:datastoreio:outputtype:rowTimeVariableSkipped',...
                        ds.RowTimes));
                    end
                end
            end
        ...
        %   and sets them accordingly.
            ...
            % Store values in case new Selected Formats conflicts with the
            % OutputType
            if ds.IntrospectionDone
                prevCustomFormatsSelected = ds.PrivateCustomFormatsSelected;
                prevSelectedFormats = ds.PrivateSelectedFormats;
                prevTextscanFormats = ds.PrivateTextscanFormats;
                timeVarIdx = ds.TimeVariableIndex;
                ds.OutputType = "table";
            end

            if ~strcmp(ds.PrivateCustomFormatsSelected,'varNames')
                % user set formats
                ds.PrivateCustomFormatsSelected = 'set';
            end
            ...
            % Check that the OutputType can be reset
            if ds.IntrospectionDone && ~isempty(timeVarIdx)
                try
                    ds.RowTimes = timeVarIdx;
                catch
                    ds.PrivateCustomFormatsSelected = prevCustomFormatsSelected;
                    ds.PrivateSelectedFormats = prevSelectedFormats;
                    ds.PrivateTextscanFormats = prevTextscanFormats;
                    ds.RowTimes = timeVarIdx;
                    error(message('MATLAB:datastoreio:outputtype:rowTimeTypeChanged',...
                        ds.RowTimes));
                end
        ...
        %VALIDATEACTIVEVARIABLENAMES Validates the SelectedVariableNames
            ...
            % Store values in case new Selected Variable Names conflicts
            % conflicts with OutputType
            if ds.IntrospectionDone
                % store all indices and formats
                prevSelectedVariableNamesIdx = ds.SelectedVariableNamesIdx;
                prevSelectedFormats = ds.SelectedFormats;
                timeVarIdx = ds.TimeVariableIndex;
                ds.OutputType = "table";
            end
            ...
            % the call to setter for SelectedFormats should be able to
            % change the formats without issuing a warning
            prevCustomFormatSelected = ds.PrivateCustomFormatsSelected;
            ds.PrivateCustomFormatsSelected = 'varNames';
            ds.SelectedFormats = ds.PrivateSelectedFormats(locb);
            ds.PrivateCustomFormatsSelected = prevCustomFormatSelected;

            if ds.IntrospectionDone && ~isempty(timeVarIdx)
                try
                    ds.RowTimes = timeVarIdx;
                catch ME
                    ds.SelectedVariableNamesIdx = prevSelectedVariableNamesIdx;
                    ds.SelectedFormats = prevSelectedFormats;
                    ds.RowTimes = timeVarIdx;
                    error(message('MATLAB:datastoreio:outputtype:rowTimeDeselected',...
                        ds.RowTimes));
                end
            end
        end
        ...
        %   and sets them accordingly.
            ...
            if ds.IntrospectionDone
                % make valid variable names (valid and unique) according to
                % the value of PreserveVariableNames.
                varNames = normalizeVariableNames(ds, varNames);
            end
        ...
        %   are valid and sets them accordingly.
            ...
            % Must convert strings to chars in order to support the sentinel
            % value "", which signifies there are no values to be treated
            % as missing. 
            treatAsMissing = convertStringsToChars(treatAsMissing);

            % using the builtin textscan interface to validate TreatAsMissing
            if ~isempty(treatAsMissing)
               try
                    % trim any leading or trailing insignificant
                    % whitespace. strtrim will error if treatAsMissing
                    % is neither a char array, cellstr, nor string array.
               ...
               if any(~isnan(str2double(treatAsMissing))) || any(strcmpi('nan', treatAsMissing))
                ...
                if ischar(treatAsMissing) || iscell(treatAsMissing)
                    % Reshape 1x0 and 0x1 empty arrays to be 0x0. 
                    treatAsMissing = reshape(treatAsMissing, 0, 0);
                else
                   error(message('MATLAB:datastoreio:tabulartextdatastore:invalidTreatAsMissing'));
                end
            end
        ...
        %VALIDATEREADVARIABLENAMES Validates ReadVariableNames to be logical.
            ...
            % reinitialize the datastore by introspection
            reInit(ds, 'PrivateReadVariableNames', logical(readVarNames), true);
        end
        ...
        function validateDatetimeLocale(ds, dtLocale)
            try
                % validate and capitalize the locale
                dtLocale = matlab.internal.datetime.verifyLocale(dtLocale);       
            catch ME
                if ischar(dtLocale)
                    error(message('MATLAB:textio:io:UnknownLocale',dtLocale));
                else
                    throw(ME);
                end
            end
            ds.PrivateDatetimeLocale = dtLocale;
            % reinitialize the datastore
            reInit(ds, 'PrivateDatetimeLocale', dtLocale, true);            
        end
        ...
        %   stable state if anything goes wrong in the re-read of the file.
            ...
            if flagToRead
                try
                    ...
                    timeVarIdx = ds.TimeVariableIndex;
                    ds.OutputType = "table"; % to instrospect, must be table
    ...
    methods (Static, Access = 'private')
        ...
        %LOADFROMSTRUCT initializes a datastore loaded as a struct correctly.
    ...
    methods (Access = 'protected')
        function varTypes = getVariableTypes(ds)
            varTypes = matlab.io.datastore.internal.util.convertFormatsToTypes(ds.PrivateTextscanFormats,...
                ds.PrivateTextType);
        end
    end
end
...
import matlab.io.datastore.mixin.PreserveVariableNamesInput;
import matlab.io.datastore.TabularTextDatastore;
import matlab.io.datastore.mixin.CrossPlatformFileRoots;
    ...
    if isempty(pTxt)
        ...
        addParameter(pTxt, 'DatetimeLocale', 'en_US');

        PreserveVariableNamesInput.configureInputParser(pTxt);
        
        addParameter(pTxt, 'OutputType', "auto");
        addParameter(pTxt, 'RowTimes', "");
    end
    
    % parse the input
    parse(pTxt, varargin{:});
    if ~isa(pTxt.Results.IncludeSubfolders,'logical') && ~isnumeric(pTxt.Results.IncludeSubfolders)
    ...
    resStruct = PreserveVariableNamesInput.crossUpdate(resStruct);
