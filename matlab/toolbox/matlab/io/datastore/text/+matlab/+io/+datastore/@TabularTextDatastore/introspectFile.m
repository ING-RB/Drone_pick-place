...
%   Copyright 2014-2020 The MathWorks, Inc.
    ...
    % maintain previous versions of SelectedVariableNames and
    % SelectedFormats in case these need to be overwritten
    prevVariableNames = ds.SelectedVariableNames;
    ...
    ds.SelectedVariableNamesIdx = 1:numel(ds.PrivateVariableNames);
    
    % final sets
    ds.VariableNames = validVarNames;
    ds.TextscanFormats = formatCell;

    % try to set back the SelectedFormats and SelectedVariableNames specified
    if strcmp(ds.PrivateCustomFormatsSelected,'set')
        % preserve previously set formats, warn the user that they might
        % have to change a variable for certain properties to take effect
        ds.SelectedVariableNames(prevSelectedVariableNamesIdx) = prevVariableNames;
        warnFormatChange = 0;
        for ii = 1 : numel(prevSelectedVariableNamesIdx)
            if ~strcmp(prevSelectedFormats{ii},formatCell{ii})
                warnFormatChange = 1;
            end
            ds.SelectedFormats(prevSelectedVariableNamesIdx(ii)) = prevSelectedFormats(ii);
        end
        if warnFormatChange
            warning(message('MATLAB:datastoreio:tabulartextdatastore:unchangedFormats'));
        end
    end
...
%   match the number of variable names detected or specified.
    ...
    import matlab.io.text.internal.determineVarNames;
    import matlab.io.text.internal.determineFormatString;    
    import matlab.io.text.internal.detectVariableNames;
    import matlab.io.text.internal.detectParametersFromString;
    ...
    types = {};
    % empty delimiters are not handled by detectParametersFromString
    if ~isempty(ds.Delimiter)
        ...
        [delim,header,multipleDelimsAsOne,types] = detectParametersFromString(...
            dataForDetection, delimSupplied, headerSupplied, ...
            ds.Delimiter, ds.NumHeaderLines, ds.DatetimeLocale,args);
    ...
    if readVarNames || ~ds.PrivateReadVariableNamesSupplied
        ...
        [raw,strIdx] = builtin('_textscan', varFormatData, ['%s%[' rowDelim ']'], 1, ...
    ...
    if isempty(inFormatCell)
        ...
        raw = builtin('_textscan', varFormatData, '%s', 1, 'Delimiter', '', ...
        ...
        if isempty(raw{1}) || isempty(raw{1}{1})
            ...
            formatStr = determineFormatString(rawContents, delim, whiteSpace, ...
                              rowDelim, treatAsMissing, types, ds.DatetimeType, ...
                              ds.DurationType, ds.DatetimeLocale, true, txtScanArgsforIntroSpection);
            % convert to a struct
    ...
    if ~ds.PrivateReadVariableNamesSupplied && ~matlab.io.text.internal.formatIsAllString(fmtstr)
...
%   '\r\n' it looks for either a \r or a \n or both.
    ...
    if strcmp(rowDelim, '\r\n')
        ...
        function tf = checkForRowDelimAfterData(rowDelim)
            tf = true;
            delimAfterData = builtin('_textscan', varFormatData, ['%*s%[' rowDelim ']'], 1, ...
                                     'Delimiter', '', 'Whitespace', whiteSpace, ...
                                     'Headerlines', hdrLines, 'EndOfLine', ...
