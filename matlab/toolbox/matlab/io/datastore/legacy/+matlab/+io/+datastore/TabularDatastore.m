...
%   Copyright 2016-2021 The MathWorks, Inc.
    ...
    methods (Hidden, Static)
        function t = emptyTabularWithVarTypes(ds,variableNames,textType,tabularType)
            ...
            if isa(ds,"matlab.io.datastore.TabularTextDatastore")
                formats = ds.SelectedFormats;
                if ~isempty(formats)
                    types = matlab.io.datastore.internal.util.convertFormatsToTypes(...
                        formats, textType);
                else
                    types = {};
                end
            elseif isa(ds, "matlab.io.datastore.SpreadsheetDatastore")
                types = ds.SelectedVariableTypes;
            end
            ...
            switch(nargin)
                case 3
                    ...
                    if ~isempty(variableNames)
                        t = table('Size', [0, numel(types)], ...
                            'VariableTypes', types, 'VariableNames', variableNames);
                    else
                        t = table('Size', [0, numel(types)], ...
                            'VariableTypes', types);
                    end
                case 4
                    ...
                    switch validStr
                        ...
                        case 'timetable'
                            ...
                            t = timetable('Size', [0, numel(types)], ...
                                'VariableTypes', types, 'VariableNames', variableNames);
                    end
            end
    ...
    methods (Sealed)
        ...
        function t = readall(ds, varargin)
            ...
            if matlab.io.datastore.read.validateReadallParameters(varargin{:})
                t = matlab.io.datastore.read.readallParallel(ds);
                return;
            end
            t = readAllData(ds);
            if ~isa(t, 'tabular')
