classdef DsFileSet < matlab.io.datastore.internal.HandleUnwantedHideable &...
                     matlab.mixin.Copyable & matlab.mixin.CustomDisplay
%DSFILESET A file set object for a collection of files.
%   FS = matlab.io.datastore.DsFileSet(LOCATION) creates a file set object that can be used
...
%
%   FS = matlab.io.datastore.DsFileSet(__,'FileSplitSize',SPLITSIZE) specifies the size in
...
%
%   FS = matlab.io.datastore.DsFileSet(__,'IncludeSubfolders',TF) specifies the logical
%   true or false to indicate whether the files in each folder and its
%   subfolders are included recursively or not.
%
%   FS = matlab.io.datastore.DsFileSet(__,'FileExtensions',EXTENSIONS) specifies the
...
%
%   FS = matlab.io.datastore.DsFileSet(__,'AlternateFileSystemRoots',ALTROOTS) specifies
...
%   Copyright 2017-2018 The MathWorks, Inc.
    ...
    methods
        ...
        function subfs = subset(fs, indices)
            ...
            %   SUBFS = SUBSET(FS, INDICES) creates a deep copy of the input file set, FS,
            %   resulting in the file set SUBFS that contains files corresponding to INDICES.
            %
            %   Example: Subset the first 4 files
            %   ----------------------------------
            %      folders = fullfile(matlabroot,'toolbox','matlab',{'demos','imagesci'});
            %      exts = {'.jpg','.png','.tif'};
            %      fs = matlab.io.datastore.DsFileSet(folders,'FileExtensions',exts)
            ...
            %
            %   See also matlab.io.datastore.ImageDatastore/subset, matlab.io.datastore.DsFileSet,
            %   matlab.io.datastore.Partitionable.
            import matlab.io.datastore.internal.validators.validateSubsetIndices;
            indices = validateSubsetIndices(indices, maxpartitions(fs), mfilename);
            subfs = copy(fs);
            ...
            if ~isempty(newCopy)
                ...
                newCopy.RecalculateFolders = true;
        ...
        function file = nextfile(fs)
            ...
            %            matlab.io.datastore.Partitionable.
            try
                [fName, fSize, offset, splitSize] = nextfile(fs.InternalFileSet);
                file = table(fName, fSize, offset, splitSize, 'VariableNames', ...
                    {'FileName', 'FileSize', 'Offset', 'SplitSize'});
            catch ME
                throw(ME);
            end
        end
    ...
    methods (Access = protected)
        ...
        function displayScalarObject(fs)
            %DISPLAYSCALAROBJECT controls the display of the DsFileSet.
            % display the properties
            disp(getHeader(fs));
            group = getPropertyGroups(fs);
            matlab.mixin.CustomDisplay.displayPropertyGroups(fs, group);
            % display message to get full list of files
            if matlab.internal.display.isHot()
                footerHole = "<a href=""matlab:helpview([docroot '/matlab/ref/matlab.io.datastore.dsfileset.resolve.html'])"">resolve</a>";
            else
                footerHole = "resolve";
            end
            disp(getString(message("MATLAB:datastoreio:dsfileset:footerLine",footerHole)));
        end
    end
...
function splitSize = iValidateSplitSize(splitSize)
    ...
    try
        ...
        if any(strcmp(ME.identifier, ["MATLAB:DsFileSet:unrecognizedStringChoice", ...
                "MATLAB:DsFileSet:invalidType"]))
