...
%   Copyright 2014-2018 The MathWorks, Inc.
    ...
    properties (Hidden)
        % split index to track splits that cannot be read
        ErrorSplitIdx = 0;
    end
            ...
            if currIdx < numSplits
                ...
                for sidx = currIdx + 1 : numSplits
                    ...
                    try
                        ...
                        if strcmpi(ds.ReadFailureRule,'skipfile')
                            % move to the next split that can be read
                            ds.PrivateReadFailuresList(sidx) = 1;
                            % display read failure warning only for the first read failure
                            dispFirstFailureWarning(ds,ds.Files{sidx});
                            ds.ErrorSplitIdx = sidx;
                            throw(ME);
                        else
    ...
    methods (Access = 'protected')
        ...
        function dispFirstFailureWarning(ds,currFileName)
            if ~ds.PrivateReadCounter
                prevState = warning('backtrace');
                warning('off','backtrace');
                c = onCleanup(@() warning(prevState));
                warning(message('MATLAB:datastoreio:filebaseddatastore:warnFirstFailure',currFileName));
                ds.PrivateReadCounter = true;
            end
        end

        function dispReadallWarning(ds)
            if sum(ds.PrivateReadFailuresList)
                prevState = warning('backtrace');
                warning('off','backtrace');
                c = onCleanup(@() warning(prevState));
    ...
    methods (Access = 'protected')
        function moveToSplit(ds, ii)
            ...
            ds.SplitIdx = ii;
            if isa(rdr,'matlab.io.datastore.splitreader.SequenceFileSplitReader') || ...
                    isa(rdr,'matlab.io.datastore.splitreader.MatKVFileSplitReader')
                keyType = ds.SplitReader.KeyType;
                ds.SplitReader = rdr;
                ds.SplitReader.KeyType = keyType;
            else
                ds.SplitReader = rdr;
            end
        end
