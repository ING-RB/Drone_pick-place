...
% remove the first and last variables' underscore (from the tabular display)
R1 = regexp(evalStr,' _');
R2 = regexp(evalStr,'_ ');
evalStr(R1(1):R2(1)) = ' ';
evalStr(R1(end):R2(end)) = ' ';

% Remove empty cell array text.
cellString = '{                        }';
cellStrReplace = '                          ';
evalStr = strrep(evalStr, cellString, cellStrReplace);

% Replace empty char arrays with ''.
emptyCharString = '{0×0 char}';
emptyCharStrReplace = '    ''''     ';
evalStr = strrep(evalStr, emptyCharString, emptyCharStrReplace);

% Replace empty double arrays from Var1 property with spaces.
emptyDoubleString = '{0×0 double}';
emptyDoubleStrReplace = '            ';
evalStr = strrep(evalStr, emptyDoubleString, emptyDoubleStrReplace);
