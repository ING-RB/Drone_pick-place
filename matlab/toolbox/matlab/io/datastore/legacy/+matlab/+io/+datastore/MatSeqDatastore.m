...
%   Copyright 2016-2020 The MathWorks, Inc.
    ...
    properties (Constant, Access = protected)
       ...
       READ_FAILURE_RULE = 'ReadFailureRule';
    ...
    methods (Access = protected)
        ...
        function initDatastore(ds, files, nvStruct)
            ...
            if ~isa(nvStruct.IncludeSubfolders,'logical') && ~isnumeric(nvStruct.IncludeSubfolders)
            ...
            fileInfo.FileSizesBytes = fileSizes;
            ds.Splitter = feval([splitterName '.create'], fileInfo);
            % Set the number of key-value pairs that need to be read
            ds.Splitter.KeyValueLimit = nvStruct.ReadSize;
        ...
        function [splitterName, fileInfo] = filterAndGetInfo(ds, fileType, files, fileSizes)
            ...
            switch fileType
                case 'mat'
                    ...
                    fileInfo = MatKVFileSplitter.filterMatFiles(files, ds.ValuesOnly, fileSizes);
                case 'seq'
        ...
        function [splitterName, fileInfo] = checkSupportAndGetInfo(ds, info, files, fileSizes)
            ...
            switch info.Support
                case 'MATSupport'
                    ...
                    [fileInfo, areMat, idx] = MatKVFileSplitter.filterMatFiles(files, ds.ValuesOnly, fileSizes);
                    if ~areMat
    ...
    methods (Static = true, Hidden = true)
        ...
        function [tf, info, files, fileSizes] = supportsLocation(files, nvStruct, checkMatOrSeq, userPassedFileType)
            ...
            try
                ...
                import matlab.io.datastore.splitter.MatKVFileSplitter;
                if checkForMat && MatKVFileSplitter.isMatSupported(info.Filename, nvStruct.ValuesOnly, fileSizes(1))
                    tf = true;
                    info.Support = 'MATSupport';
                    return;
                end
    ...
    methods (Hidden = true, Access = 'public')
        ...
        function frac = progress(ds)
            % Subtracting one from SplitIdx to get a zero-based index.
            if ~isempty(ds.SplitReader)
                frac = (ds.SplitIdx - 1) + progress(ds.SplitReader);
            else
                frac = 0;
            end
