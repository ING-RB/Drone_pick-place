...
try
    ...
    [uniq, count, groups, nans] = matlab.io.datastore.internal.util.groupAndCountLabels(copiedDs.Labels);
...
function fileCount = getFileCount(numargout, nums, count, numLabels)
    ...
    elseif all(~lessThanOne)
        ...
        numsum = sum(nums);
        if numsum > min(count)
            error(message('MATLAB:datastoreio:imagedatastore:sumPropMoreThanLabels', string(numsum), min(count)));
        end
        if numargout <= numel(nums)
            nums = nums(1:numargout);
        end
...
function ds = getNewDs(ds, idxes)
    ...
    if ~isempty(ds.Splitter.Files)
        ds.Splitter.Files.RecalculateFolders = true;
    end
end
