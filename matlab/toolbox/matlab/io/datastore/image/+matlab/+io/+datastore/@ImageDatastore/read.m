...
try
    ...
    if isequal(imds.CachedRead, 'off') || ~imds.IsReadFcnDefault
        ...
        if imds.ReadSize == 1
            ...
            ii = 1;
            [data{ii}, info] = imds.readData();
            while hasdata(imds) && ii < readSize
                ii = ii + 1;
                data{ii} = imds.readData();
            end
            if ii == 1
                data = data(1);
            end
    ...
    if strcmp(e.identifier,'MATLAB:datastoreio:splittabledatastore:noMoreData')
        throwAsCaller(e);
    end
    [data, info] = matlab.io.datastore.FileBasedDatastore.errorHandlerRoutine(imds,e,imds.SplitReader.Split.Filename);
