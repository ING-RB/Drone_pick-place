...
if nargin > 0
   % if arg is string, then convert to char
...
for idx = 1:length(fNameList)
    ...
    if ~isempty(details) && locIsEntitledForUse(currentFName, productName)
...
function isLicensedForUse = locIsEntitledForUse(toolboxLocation, productName)
% locIsEntitledForUse checks if a MW product is licensed

% This ensures that unlicensed MW products that are kept on the path due to
% help/example dependencies DO NOT show up in VER output.

% Skip if it is a user/add-on toolboxes (not within matlabroot)
if ~startsWith(toolboxLocation, [matlabroot, filesep]) % isNotInMATLABRoot
    isLicensedForUse = true;
    return
end

if isempty(productName)
    isLicensedForUse = false;
    return
end

try
    % Get the MW product base code.
    basecode = matlab.internal.product.getBaseCodeFromProductName(productName);
    % check if product is licensed.
    isLicensedForUse = matlab.internal.licensing.isProductLicensed(basecode);
catch
    % If MLRoot product is not found then just report it. 
    isLicensedForUse = true;
end
end
