...
for ii=1:nrLayers-1
    nrCross(ii) = matlab.internal.graph.helperNumberCrossings(M(order{ii}, order{ii+1}));
end
...
function xcoord = straightenEdges(xcoord, M, layersize)

% List of all nodes with indegree and outdegree equal to one, and which
% are not aligned with their successor nor their predecessor node.
[bend, pred, succ] = matlab.internal.graph.helperStraightenCandidates(M, xcoord);

% Compute nodes to the left and right of the bend nodes, in the same layer
layers = repelem(1:length(layersize), layersize);

% Note: bend nodes will always be >1 and < numnodes(h), because they must
% have a predecessor and a successor node. Therefore, this is safe.
left = bend-1;
left(layers(left) ~= layers(bend)) = NaN;
right = bend+1;
right(layers(right) ~= layers(bend)) = NaN;
...
for ll=1:20
    ...
    for ii=1:length(bend)
        ...
        if isnan(left(ii))
            ...
            leftBound = xcoord(left(ii)) + 1;
        end
        if isnan(right(ii))
            rightBound = NaN;
        else
            rightBound = xcoord(right(ii)) - 1;
        end

        if leftBound <= xp && xp <= rightBound
            % Align with predecessor node
            xcoord(bend(ii)) = xp;
            bend(ii) = NaN;
            anyChange = true;
        elseif leftBound <= xs && xs <= rightBound
            % Align with successor node
            xcoord(bend(ii)) = xs;
            bend(ii) = NaN;
            anyChange = true;
        elseif xb < xp && xb < xs && bend(ii) < rightBound
            % Try to alleviate the bend (b is left of s and p)
            xcoord(bend(ii)) = rightBound;
            anyChange = true;
        elseif xb > xp && xb > xs && bend(ii) > leftBound
            % Try to alleviate the bend (b is right of s and p)
            xcoord(bend(ii)) = leftBound;
            anyChange = true;
        end
    end
...
Mmark = matlab.internal.graph.helperMarkConflicts(-M, layersize, isInnerNode);

% Construct 4 versions of x coordinates, with varying bias
x = zeros(n, 4);
x(:, 1) = matlab.internal.graph.helperCompX(Mmark.', layersize,  true,  true);  % upper left
x(:, 2) = matlab.internal.graph.helperCompX(Mmark,   layersize,  true, false);  % lower left
x(:, 3) = matlab.internal.graph.helperCompX(Mmark.', layersize, false,  true);  % upper right
x(:, 4) = matlab.internal.graph.helperCompX(Mmark,   layersize, false, false);  % lower right
...
% Debugging helper: Plot the four variants of x-coordinates:
% invp(p) = 1:numel(p);
% for ii=1:length(order), y(order{ii}) = length(order)-ii+1; end
% dirLabel = ["upper left" "lower left" "upper right" "lower right"];
% figure(13);
% for ii=1:4
%     subplot(2, 2, ii);
%     plot(digraph(h), 'XData', x(invp, ii), 'YData', y);
%     title(dirLabel(ii));
% end
...
xcoord = straightenEdges(xcoord, M, layersize);

xcoord(p) = xcoord;
end % END assignXCoordinates
...
for ii=1:size(edges, 1)
    ...
    edgeC(1,:)   = nodeCoords(tail,:); % fix starting endpoint
    edgeC(end,:) = nodeCoords(head,:); % fix ending endpoint
    edgeCoords{ii} = edgeC;
end
