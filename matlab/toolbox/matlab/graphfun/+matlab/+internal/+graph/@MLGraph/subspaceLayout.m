...
elseif nnodes == 3
    ...
    % Standardize signs of coordinates
    for ii=1:outdim
        [mx, imax] = max(xyz(:, ii));
        [mn, imin] = min(xyz(:, ii));
        
        if abs(abs(mx) - abs(mn)) < 1e-3*max(abs(mx), abs(mn))
            % Visually, upper and lower bound are the same. Make the 
            % earlier entry be negative.
            if imax < imin
                xyz(:, ii) = -xyz(:, ii);
            end
        elseif abs(mn) > abs(mx)
            % Make the largest entry by magnitude be positive.
            xyz(:, ii) = -xyz(:, ii);
        end
    end
end
