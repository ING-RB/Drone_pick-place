...
%   edge of the graph G. COST is a vector of nonnegative edge weights,
...
%   For type 'betweenness', COST must have positive edge weights.
%
...
for ii=1:2:numel(varargin)
    ...
    switch opt
        case 'Cost'
            ...
            elseif any(strcmp(type, {'outcloseness', 'incloseness'}))
                validateattributes(wc, {'double'}, ...
...
switch type
    ...
    case {'hubs', 'authorities'}
        ...
        hubs = nan(n, 1, "like", wi);
        auth = nan(n, 1, "like", wi);
        for ii=1:max(bins)
...
hubs = ones(size(M, 1), 1, "like", full(M([])));
hubs = hubs / sum(hubs);
