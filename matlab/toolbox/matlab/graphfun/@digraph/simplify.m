...
[method, pickmethod, reducemethod, vars, omitSelfLoops, edgeprop] = parseInputs(g, varargin);
...
if ~hasEdgeProperties(g)
    ...
    gsimple = digraph(mlgSimple, [], g.NodeProperties);
    return;
end
...
% Note EdgeProperties is a table for all methods except 'first' and 'last'.
if strcmp(method, 'pick')
    switch pickmethod
        ...
        case {'min', 'max'}
            ...
            try
                ...
                varName = edgeprop.Properties.VariableNames{vars};
    ...
    for jj = 1:length(vars)
        ...
        varin = edgeprop{:, v};
        if istabular(varin)
            varName = edgeprop.Properties.VariableNames{v};
    ...
    if ~isempty(epSimpleCell)
        varnames = edgeprop.Properties.VariableNames(vars);
        edgeprop = table(epSimpleCell{:}, 'VariableNames', varnames);
...
gsimple = digraph(mlgSimple, edgeprop, g.NodeProperties);


function [method, pickmethod, reducemethod, vars, omitSelfLoops, edgeprop] = parseInputs(g, args)
        ...
        for ii=nameValueStart:2:length(args)
            ...
            switch opt
                case 'AggregationVariables'
                    ...
                    [valid, vars] = checkVariables(edgeprop,vars);
                ...
                case 'PickVariable'
                    if ~strcmp(method, 'pick') || ~matches(pickmethod, ["min", "max"])
...
if varsNeedCheck
    ...
    if hasEdgeWeights
        varnames = edgeprop.Properties.VariableNames;
        vars = find(varnames == "Weight");
    else
        if strcmp(method, 'pick')
...
if success
    ...
    if isnumeric(dataVars)
        ...
        elseif ischar(dataVars) || iscellstr(dataVars) || isstring(dataVars)
            varnames = A.Properties.VariableNames;
            dataVars = ismember(varnames,dataVars);
        end
