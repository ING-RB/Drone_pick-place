...
if formatTable
    if (ischar(s) || isstring(s)) && hasNodeNames
        nodeind = ~isnan(t(:, 2));
        
        if isstring(s)
            nodes = repmat(string(missing), size(t, 1), 1);
            nodes(nodeind) = names(t(nodeind, 2));
            
            edges = repmat(string(missing), size(t, 1), 2);
            edges(~nodeind, :) = string(names(t(~nodeind, [3 4])));
        else
            nodes = cell(size(t, 1), 1);
            nodes(nodeind) = names(t(nodeind, 2));
            
            edges = cell(size(t, 1), 2);
            edges(~nodeind, :) = names(t(~nodeind, [3 4]));
        end
    else
        nodes = t(:, 2);
        edges = t(:, [3 4]);
...
if digraph.isvalidoption(name) && digraph.partialMatch(name, "Restart")
    setEvents = false;
else
    setEvents = true;
    events = name;
...
if setEvents
    ...
    for ii=1:numel(events)
        eventid = digraph.partialMatch(events{ii}, searchLabelsExt);
