...
if indexOp(1).Type == matlab.indexing.IndexingOperationType.Dot
    ...
    if strcmp(varname, 'EndNodes')
        error(message('MATLAB:graphfun:graph:EditEdges'));
...
if isempty(indexOp)
    ...
    V = graph.validateNodeProperties(V);
    V = graph.minimizeNodeProperties(V);
    G.NodeProperties = V;
else
    ...
    if indexOp(1).Type == matlab.indexing.IndexingOperationType.Dot && ...
        ...
        if ~(indexOp(1).Type == matlab.indexing.IndexingOperationType.Dot && ...
            ...
            nodeprop = graph.validateNodeProperties(nodeprop);
        end
...
elseif length(nodeprop) ~= numNodes
    ...
    nodeprop = graph.validateName(nodeprop);
end
