...
%   be numeric node IDs or both be node names. If there are multiple edges
%   specified by s and t, they are all removed.
%
...
if ~ismultigraph(G)
    ...
    ed(ind, :) = [];
    H.Underlying = matlab.internal.graph.MLGraph(ed(:, 1), ed(:, 2), numnodes(H));
end

% Remove corresponding edge properties.
if hasEdgeProperties(H)
