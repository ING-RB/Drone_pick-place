...
if nargin <= 1
    ...
    elseif (isnumeric(w) || islogical(w)) && isvector(w)
        ...
        if length(w) ~= numedges(G) || ...
                ~(isa(w, 'double') || isa(w, 'logical') || ...
                (isa(w, 'single') && matlab.internal.feature('SingleSparse')))

            if matlab.internal.feature('SingleSparse')
                % New error ID which lists single as supported datatype
                error(message('MATLAB:graphfun:adjacency:InvalidWeightVectorFloat'));
            else
