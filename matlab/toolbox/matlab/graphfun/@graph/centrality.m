...
%   The centrality measure used is specified by TYPE.
%   TYPE can be:
%
...
%                       matrix.
%
%   CENTRALITY(..., 'Cost', COST) specifies the cost of traveling each edge
%   of the graph G. COST is a vector of nonnegative edge weights, and
%   COST(i) specifies the cost associated with edge findedge(G, i). Only
%   applies to types 'closeness' and 'betweenness'. For type 'betweenness',
%   COST must have positive edge weights.
%
...
%
%   CENTRALITY(..., 'FollowProbability', P) sets the probability that, in
...
for ii=1:2:numel(varargin)
    ...
    switch opt
        case 'Cost'
            wc = varargin{ii+1};
            if strcmp(type, 'betweenness')
                validateattributes(wc, {'double'}, ...
                    {'vector', 'real', 'positive', 'numel', numedges(g)}, '', 'Cost');
            elseif strcmp(type, 'closeness')
                validateattributes(wc, {'double'}, ...
                    {'vector', 'real', 'nonnegative', 'numel', numedges(g)}, '', 'Cost');
            else
        ...
        case 'Importance'
            ...
            if matlab.internal.feature('SingleSparse')
                weightDatatype = {'double', 'single'};
            else
                weightDatatype = {'double'};
            end
            validateattributes(wi, weightDatatype, ...
