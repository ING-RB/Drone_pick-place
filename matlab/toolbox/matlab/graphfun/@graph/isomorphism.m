...
%
%   See also GRAPH, ISISOMORPHIC, DIGRAPH/ISOMORPHISM

%   Copyright 2016-2020 The MathWorks, Inc.

if ~isa(G1, 'graph') || ~isa(G2, 'graph')
    error(message('MATLAB:graphfun:isomorphism:ClassMismatch'));
end

[nodeString, edgeString] = parseInputs(varargin{:});
...
% match.
% If nodeString/edgeString is empty, those properties are never accessed,
% so no need to turn them into a table.
if isempty(nodeString)
    nodeVar1 = [];
    nodeVar2 = [];
else
    [nodeVar1, nodeVar2] = extractVarProp(nodeString, getNodePropertiesTable(G1), getNodePropertiesTable(G2), 'Node');
end

if isempty(edgeString)
    edgeVar1 = [];
    edgeVar2 = [];
elseif isequal(edgeString, "Weight") && ...
        isnumeric(G1.EdgeProperties) && iscolumn(G1.EdgeProperties) && ...
        isnumeric(G2.EdgeProperties) && iscolumn(G2.EdgeProperties)
    edgeVar1 = G1.EdgeProperties;
    edgeVar2 = G2.EdgeProperties;
else
    [edgeVar1, edgeVar2] = extractVarProp(edgeString, getEdgePropertiesTable(G1), getEdgePropertiesTable(G2), 'Edge');
end
...
[nodeCat, nodeMatchPossible] = extractInd(nodeVar1, nodeVar2);
[edgeCat, edgeMatchPossible] = extractInd(edgeVar1, edgeVar2);
...
function [nodeString, edgeString] = parseInputs(varargin)
...
for ii=1:2:numel(varargin)
    ...
    if graph.partialMatch(name, "NodeVariables")
        edgeMatch = false;
        key = 'NodeVariables';
    elseif graph.partialMatch(name, "EdgeVariables")
        edgeMatch = true;
        key = 'EdgeVariables';
    else
        error(message('MATLAB:graphfun:isomorphism:ParseFlags'));
...
function [varProp1, varProp2] = extractVarProp(varnameString, t1, t2, nodeEdgeStr)

% Index into node variables - error if variable doesn't exist
varNames1 = t1.Properties.VariableNames;
varNames2 = t2.Properties.VariableNames;
[~, varInd1] = ismember(varnameString, varNames1);
[~, varInd2] = ismember(varnameString, varNames2);
if any(varInd1==0)
...
function [ind, matchPossible] = extractInd(varProp1, varProp2)
...
if isempty(varProp1)
    ...
    propTable = [varProp1; varProp2];
    [~, ~, ind] = unique(propTable);
    ind = uint64(reshape(ind, [size(varProp1, 1) 2]));
...
degrees = [degree(mlG1), degree(mlG2)];

% 2) size of connected component containing each node
bins1 = connectedComponents(mlG1);
bins2 = connectedComponents(mlG2);
compSizes = [compBinSize(bins1), compBinSize(bins2)];

% Compute updated matrix nodeCat
[~, ~, nodeCat] = unique([nodeCat(:) degrees(:) compSizes(:)], 'rows');
...
% Compute re-ordered graph
N = adjacency(mlg1, 1:numedges(mlg1));
modG1 = matlab.internal.graph.MLGraph(N(ind, ind));
...
N = N(ind, ind);
modG1 = matlab.internal.graph.MLGraph(N);
