...
s = size(X);
if isinteger(X) ||  isfloat(X)  || isa(X,'embedded.fi')
...
try
    ...
    error(message('MATLAB:timeseries:resample:interpnonfloat'))
end
...
% Remove all xi>max(x) xi<min(x) (extrapolation) and add them back as NaNs
% This avoids getting zeros in the values of k below for exprapolated
% values
y1 = yMat(1,:);
yend = yMat(end,:);
strictPreExtrap = repmat(cast(NaN(sum(xi<x(1)),1),class(y)),[prodDs,1]);
exactPreExtrap = repmat(y1,[sum(xi==x(1)),1]);
preExtrap = [strictPreExtrap;exactPreExtrap];
%preExtrap = [cast(NaN(sum(xi<x(1)),1),class(y)); repmat(y1,[sum(xi==x(1)),1])]; 
strictPostExtrap = repmat(cast(NaN(sum(xi>x(end)),1),class(y)),[prodDs,1]);
exactPostExtrap = repmat(yend,[sum(xi==x(end)),1]);
postExtrap = [exactPostExtrap; strictPostExtrap];
%postExtrap = [repmat(yend,[sum(xi==x(end)),1]);cast(NaN(sum(xi>x(end)),1),class(y))];

xi = xi(xi>x(1) & xi<x(end));

% Find the positions of the left side of the interval in x indices for xi 
h = diff(x);
[~,k] = histc(xi,x); %#ok<HISTC> % k is the index of the left side of the interval in x indices for each xi 

f1 = (x(k+1)-xi)./h(k); % Fraction through x interval of each xi 
f2 = 1.0 - f1;
p = 1:length(xi);
for j = 1:prodDs
...
% Add back any exprapolated values
yiMat = [preExtrap;yiMat;postExtrap];
