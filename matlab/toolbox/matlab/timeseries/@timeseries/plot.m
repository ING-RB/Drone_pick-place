...
if isempty(Time) || isempty(Data)
    if nargout>0
        varargout{1} = matlab.graphics.chart.primitive.Line.empty;
    end
    return
end
...
if isempty(ax)    
    ...
    if strcmp(get(f,'NextPlot'),'add')
        ...
        if isempty(ax)
            ax = axes('parent',f);
            % Axes reset will overwrite the ax DefaultTextInterpreter, must
            % set on the figure
            set(f,'DefaultTextInterpreter','none');
        elseif strcmp(ax.NextPlot,'replace')
            % We will recreate the title text object on newplot
            % Axes reset will overwrite the ax DefaultTextInterpreter, must
            % set on the figure
            set(f,'DefaultTextInterpreter','none');
        end
...
% Add xlabel, if creating a new plot (NextPlot is replace or replaceall) or
% if NextPlot is replacechildren and no xlabel is present
displayXLabel = strcmp(ax.NextPlot,'replace') || strcmp(ax.NextPlot,'replaceall') || ...
    (strcmp(ax.NextPlot,'replacechildren') && isempty(ax.XLabel.String));
if displayXLabel
    if ~absTimeFlag
        try 
...
% Add ylabel, if creating a new plot (NextPlot is replace or replaceall) or
% if NextPlot is replacechildren and no ylabel is present
displayYLabel = strcmp(ax.NextPlot,'replace') || strcmp(ax.NextPlot,'replaceall') || ...
    (strcmp(ax.NextPlot,'replacechildren') && isempty(ax.YLabel.String));
if displayYLabel
    ylabel(ax,ystr);
end

% Add title, if creating a new plot (NextPlot is replace or replaceall) or
% if NextPlot is replacechildren and no title is present
displayTitle = strcmp(ax.NextPlot,'replace') || strcmp(ax.NextPlot,'replaceall') || ...
    (strcmp(ax.NextPlot,'replacechildren') && isempty(ax.Title.String));
if displayTitle
...
%if pos(1) is non numeric convert it to datetime, if it is xVal will be the
%same as pos(1)
[xVal,~,~] = matlab.graphics.internal.makeNonNumeric(obj,pos(1),pos,0);
[~,ind] = min(abs(evtimes-xVal));
