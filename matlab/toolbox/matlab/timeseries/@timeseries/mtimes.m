    ...
    if ts1.IsTimeFirst == ts2.IsTimeFirst
        ...
        if ts1.DataInfo.isstorage
            try
                ...
                [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultmtimes(ts1.DataInfo,ts2.DataInfo,ts1.Data,ts2.Data,ts2.IsTimeFirst);
            end
        else
            [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultmtimes(ts1.DataInfo,ts2.DataInfo,ts1.Data,ts2.Data,ts2.IsTimeFirst);
        end
...
elseif isnumeric(ts2) || islogical(ts2)
    ...
    if isscalar(ts2)
        ...
        if ~isArrayFirst
            if ts1.IsTimeFirst
                % ts1 has scalar or row vector samples
                % ts2 is a column vector of scalar samples or a matrix
                ...
                if ts1.DataInfo.isstorage
                    ...
                    % ts1 isTimeFirst==false and so has >=3d samples
                    % ts2 is a column vector of scalar samples or a matrix
                    % where each column is a sample
                    samplesSize = getdatasamplesize(ts1);
                    samplesSize = [samplesSize(1) 1];
                    tsout.Data = zeros([samplesSize,tsout.Length]);
                    for k=1:ts1.Length
                        tsout.Data(:,:,k) = getdatasamples(ts1,k)*ts2;
                    end
                end
            end        
        else        
            if ts1.IsTimeFirst
                % ts1 has scalar or row vector samples
                % ts2 is a row vector of scalar samples or a matrix
                ...
                if ts1.DataInfo.isstorage
                    ...
                    % where each row is a sample        
                    for k=1:ts1.Length
                        tsout.Data(:,:,k) = ts2(:,k)*getdatasamples(ts1,k);
                    end
                end
            end    
        end   
