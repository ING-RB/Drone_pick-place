...
void rawI2CWrite(uint16_T* peripheralDataSizeResponse, uint8_T* payloadBufferRx, uint8_T* payloadBufferTx, uint8_T* status)
{
    ...
    *status = MW_I2C_NO_ACKNOWLEDGE;
    ...
    #if (MEMORY_ADJUSTMENT == 2)
    slaveAddress = packBytesInto16BitInteger(&payloadBufferRx[index]);
    #else
    memcpy(&slaveAddress, &payloadBufferRx[index], sizeof(uint16_T));
    #endif
    index += sizeof(uint16_T)*MEMORY_ADJUSTMENT;
    
    /* Reads 4 byte data length from the input buffer */
    #if (MEMORY_ADJUSTMENT == 2)
    numBytesToWrite = packBytesInto32BitInteger(&payloadBufferRx[index]);
    #else
    memcpy(&numBytesToWrite, &payloadBufferRx[index], sizeof(uint32_T));
    #endif
    index += sizeof(uint32_T)*MEMORY_ADJUSTMENT;
...
void rawI2CRead(uint16_T* peripheralDataSizeResponse, uint8_T* payloadBufferRx, uint8_T* payloadBufferTx, uint8_T* status)
{
    ...
    *status = MW_I2C_NO_ACKNOWLEDGE;
    ...
    /* Reads 4 byte data length from the input buffer */
    #if (MEMORY_ADJUSTMENT == 2)
    numBytesToRead = packBytesInto32BitInteger(&payloadBufferRx[index]);
    #else
    ...
    #if (MEMORY_ADJUSTMENT == 2)
    uint32_T localBuffer[MAX_PACKET_SIZE];
    #else
...
void setI2CFrequency(uint16_T* peripheralDataSizeResponse, uint8_T* payloadBufferRx, uint8_T* payloadBufferTx, uint8_T* status)
{
    ...
    *status = MW_I2C_NO_ACKNOWLEDGE;
    ...
    { 
        ...
        #if (MEMORY_ADJUSTMENT == 2)
        i2cBusFrequency = packBytesInto32BitInteger(&payloadBufferRx[index]);   
        #else
