classdef ByteOrder < handle
    %ByteOrder Class to convert the byter oder of input
    ...
    methods (Static)
        ...
        function changeData = changeByteOrder(data, datatype)
            ...
            if ~(isequal(datatype, 'int8') || isequal(datatype, 'uint8'))
                % Combine the byte into required data type
                changeData = matlabshared.svd.ByteOrder.concatenateBytes(data, datatype);
                % Convert the data into required format (little or Big
                % endian)
        ...
        function NumberOfBytes = getNumberOfBytes(DataType)
            matlabshared.svd.ByteOrder.allowedDataType(DataType);
            switch (DataType)
                case {'int8','uint8'}
                    NumberOfBytes = 1;
                case {'int16','uint16'}
        ...
        function outvalue = reverseBits(value, precision)
            ...
            NumberOfBytes = matlabshared.svd.ByteOrder.getNumberOfBytes(precision);

            outvalue = coder.nullcopy(cast(zeros(size(value)),precision));
            
            %for j = int32(1:numel(value))
            for j = 1:numel(value)
