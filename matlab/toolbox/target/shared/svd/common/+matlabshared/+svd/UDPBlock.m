classdef (StrictDefaults)UDPBlock <  matlabshared.svd.UDP
    % UDP base class
    ...
    methods
        function obj = UDPBlock(varargin)
            obj= obj@matlabshared.svd.UDP(varargin{:});
    ...
    methods (Access=protected)
        ...
        function validatePropertiesImpl(obj)
            if(isequal(obj.Direction,'Receive') && obj.LocalPort == -1)
                error(message('svd:svd:PortOutOfRange','LocalPort','>=','1'));
            end
        end
        ...
        function varargout= stepImpl(obj,varargin)
            if obj.DirectionEnum == SVDTypes.MW_Input
                ...
                [UDPOutput, UDPStatus] = receive(obj, obj.DataType, obj.DataLength ,obj.DataTypeLength);
                varargout{1} = UDPOutput;
                
                if nargout > 1
                    varargout{2} = UDPStatus;
                end
                ...
                if coder.target('Rtw')% done only for code gen
                    UDPStatus= send(obj, varargin{1}, class(varargin{1}));
                    if nargout <= 1
                        varargout{1} = UDPStatus;
                    end
    ...
    methods(Static, Access=protected)
        ...
        function [groups, PropertyList] = getPropertyGroupsImpl
            [~, PropertyListOut] = matlabshared.svd.UDP.getPropertyGroupsImpl;
