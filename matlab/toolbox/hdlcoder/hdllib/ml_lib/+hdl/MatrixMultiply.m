classdef MatrixMultiply < matlab.System
    % Computes the Matrix Multiplication of two streaming matrices.
    %
    % hdl.MatrixMultiply creates a streaming matrix multiply System Object
    % that multiplies two matrices by accepting streaming input.
    % 
    % Block parameters:
    % aRowSize           : Row size of matrix A.
    % aColumnSize        : Column size of matrix A.Must equal the row size
    %                      of matrix B.
    % bColumnSize        : Column size of matrix B.
    % dotProductSize     : Number of multipliers required to multiply two 
    %                      streaming matrices. Supported dotProduct sizes are
    %                      {1,2,4,8,16,32,64} and dot product size should
    %                      be less than or equal to column size matrix A.
    % LatencyStrategy    : Processing latency. Can be either 'Max', 'Min' or 'Zero'.
    % MajorOrder         : Stream input data in row-major or column-major order.
    % 
    % Block Interface:
    % Input ports:
    % aData              : matrix A input data to the module.
    % aValid             : Valid signal for matrix A input data.
    % bData              : matrix B input data to the module.
    % bValid             : Valid signal for matrix B input data.
    % cReady             : Input signal that indicates downstream module is
    %                      ready to take output data from processing
    %                      module.
    % Output ports:
    % cData              : matrix C output data from processing module.
    % cValid             : Valid signal for matrix C output data.
    % aReady             : Output signal that indicates processing module is 
    %                      ready to accept matrix A input data from upstream
    %                      module.
    % bReady             : Output signal that indicates processing module
    %                      is ready to accept the matrix B input data from
    %                      upstream module.
    %
    % Example:
    % M = hdl.MatrixMultiply('aRowSize',4, 'aColumnSize',4, 'bColumnSize', 4,...
    %                        'dotProductSize', '2', 'latencyStrategy', 'MAX', 'MajorOrder', 'Row');
    % creates a streaming matrix multiply system object with above
    % configured parameters.
    %
    % step method syntax:
    % [cData, cValid, aReady, bReady] = step(M, aData, aValid, bData, bValid, cReady)
    %
    %#codegen
...
methods(Access = public)
    ...
    methods(Access = protected)
        ...
        function validateInputsImpl(~, aDataIn,~,bDataIn,~,~)           
            % Streaming matrix multiply supports only single data types
            isInputSingle     = isa(aDataIn, 'single') && isa(bDataIn, 'single');
            coder.internal.errorIf(~isInputSingle, 'hdlmllib:hdlmllib:StreamMatMultSingleOnly'); 
