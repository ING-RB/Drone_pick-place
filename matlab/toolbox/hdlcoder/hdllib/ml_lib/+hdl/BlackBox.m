...
% BlackBox    
    properties (Nontunable)
        ...
        AddClockPort = 'on';
        AddResetPort = 'on';
        % AllowDistributedPipelining is replaced by AllowPipelineDistribution
        % since 23b, kept for forward compatibility
        AllowDistributedPipelining = 'off';
        AllowPipelineDistribution = 'off';
    ...
    properties (Constant, Hidden, Transient)
        ...
        AllowPipelineDistributionSet = matlab.system.StringSet({'on','off'});
        
        % hideProps contains properties that we want to hide from 
        % getBlackboxProperties and displayScalarObject
        % note that we don't set the props to hidden on the object directly 
        % because that would lose forward compatibility
        hideProps = {'AllowDistributedPipelining'};
   ...
   methods
       % Constructor
        ...
        function propertiesNameValuePairs = getBlackboxProperties(obj)
            ...
            propertiesNames = setdiff(propertiesNames, obj.hideProps);
        ...
        function set.AllowDistributedPipelining(obj,value)   
            validateattributes(value, {'char','string'}, {},'hdl.BlackBox','AllowPipelineDistribution');
            obj.AllowDistributedPipelining = convertStringsToChars(value);
            obj.AllowPipelineDistribution = convertStringsToChars(value);
        end

        function set.ClockEnableInputPort(obj,value)   
        ...
        function set.VHDLComponentLibrary(obj,value)
    ...
    methods (Access=protected)
        ...
        function displayScalarObject(obj)
           ...
           hdlblackboxPropsNames = setdiff(hdlblackboxPropsNames, obj.hideProps);
           allPropsNames = properties(obj);
           allPropsNames = setdiff(allPropsNames, obj.hideProps);
...
% LocalWords:  rtl Nontunable hdlblackbox Blackbox
