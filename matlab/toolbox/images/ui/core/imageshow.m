...
elseif isa(parent,'images.ui.graphics.Viewer')
    ...
    if isscalar(otherImages)
        if ~validOverlap
            warning(message('imagesui:image:repeatedImageDisplay'));
        end
    end
else
...
try
    obj = images.ui.graphics.Image(viewer, remainingInputs{:});
...
if ~isempty(varargin)
    ...
    elseif ischar(varargin{1}) || isstring(varargin{1})
        images.ui.graphics.internal.validate.productInstallation();
        ...
        if sz(3) == 4
            alphaData = im(:,:,4);
            im = im(:,:,1:3);
            varargin = [varargin,{'AlphaData',alphaData}];
        end
        if ~isempty(cmap)
            if size(cmap,1) > 256
                error(message('imagesui:image:invalidIndexedImage'));
            end
            varargin = [{'Interpolation'},{'nearest'},{'Colormap'},{cmap},{'DisplayRange'},{[0,size(cmap,1)-1]},varargin];
            isIndexed = true;
        end
    elseif isa(varargin{1},"categorical")
        error(message('imagesui:image:categoricalData'));
    else
        error(message('imagesui:image:requireImageData'));
    end
