...
%#codegen
    ...
    if hasMapInput
        % Extract signed distance field from map and adjust SafetyMargin accordingly
        ...
        cellHalfDiagLen = 1/(sqrt(2)*map.Resolution);
        ...
        % If the map was not passed as input then pass empty        
        sdfMap = [];
    end
    ...
    pathOpt = p;
    kinematicInfo = ki;
    solnInfo = struct('Cost',si.Cost,'NumIteration',si.NumIteration, 'Err', si.Err);
...
% 	3. It has minimum three rows
    ...
    % Check the orthogonality between distance vector & orientation vector 
    % of the input path. Display error when orthogonality condition is
    % met for all points on the path.
    if width(path)==3
        dotDkQk = dotOrientationWithTravelDir(path);
        dotDkQkThreshold = 1e-3;
        coder.internal.errorIf(all(abs(dotDkQk) < dotDkQkThreshold), ...
            'nav:navalgs:optimizepath:InputPathOrthogonality');
    end
end
...
function dotDkQk = dotOrientationWithTravelDir(path)
% dotOrientationWithTravelDir Compute the dot product between the distance vector and
% orientation vector of the input path
    x = path(:,1);
    y = path(:,2);
    yaw = path(1:end-1,3);
    % Compute different components of the distance and orientation vector
    cosBeta = cos(yaw);
    sinBeta = sin(yaw);
    delX = diff(x); 
    delY = diff(y);
    ...
    delX = delX./delS;
    delY = delY./delS;
    % Compute the dot product between distance vector and orientation
    % vector
    dotDkQk = delX .* cosBeta + delY .* sinBeta;
end
