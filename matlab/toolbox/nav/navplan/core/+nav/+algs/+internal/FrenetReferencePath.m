...
%#codegen
    ...
    methods
        ...
        % Validate number of input arguments
            ...
            if coder.target('MATLAB')
                ...
                VARSIZE_ENABLED = coder.const(coder.internal.eml_option_eq('VariableSizing','DisableInInference'));
        ...
        % Check for number of input arguments
            ...
            if isempty(obj.PathHandle) || ~any(obj.PathHandle == children)
                ...
                % Plot the waypoints
                obj.WaypointsHandle = plot(axisHandle, obj.Waypoints(:,1),obj.Waypoints(:,2),'o','Tag','Waypoints');

                % Plot the reference path
                obj.PathHandle = plot(axisHandle, obj.PathPoints(:,1),  obj.PathPoints(:,2),'-.','Tag','Reference Path','LineWidth',2);
                axisHandle.Children(1).DataTipTemplate.DataTipRows(3) = dataTipTextRow('Curvature',obj.PathPoints(:,4));
                obj.updateLineHandleColor();
                hold(axisHandle, 'off');
            else
                % Update Waypoints
    ...
    methods (Access = ?nav.algs.internal.FrenetReferencePath)
        function updateLineHandleColor(obj)
            % Retrieve line color
            pathColor = nav.internal.LineSpecColor.semantic("Path");

            % Update handles
            robotics.utils.internal.ThemeColorUtil.setThemeProperty(obj.WaypointsHandle,'MarkerFaceColor',pathColor);
            robotics.utils.internal.ThemeColorUtil.setThemeProperty(obj.WaypointsHandle,'MarkerEdgeColor',pathColor);
            robotics.utils.internal.ThemeColorUtil.setThemeProperty(obj.PathHandle,'Color',pathColor);
        end
