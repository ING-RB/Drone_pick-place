...
%#codegen
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, buildConfig) %#ok<INUSD>
            ...
            buildInfo.addSourcePaths({fullfile(matlabroot,'toolbox', ...
                                               'nav','navplan','builtins','libsrc','priorityqueuecodegen')});
    ...
    methods
        ...
        function obj = NodeMap(dim,maxSize)
            ...
            obj.NodeMapInternal = coder.opaquePtr('void', coder.internal.null);
            obj.NodeMapInternal = coder.ceval('priorityqueuecodegen_constructNodeMap', dim);
            obj.DataDim = dim;
            if (nargin==1)
                obj.maxAllowedElements = inf;
            elseif (nargin == 2)
                validateattributes(maxSize, {'double'},{'scalar', 'positive', 'finite', 'integer'}, 'NodeMap', 'Max Nodes');
                obj.maxAllowedElements = maxSize;
            end
        end
