...
sdfMap, options)
  ...
  tebGraph = nav.algs.internal.TimedElasticBandCarGraph(options, sdfMap);
  ...
  % 2.1. Apply small perturbation if all poses of input path are at the
  % same position (though they may have equal or different heading angles)
  [pthIn, deltaT] = perturbPath(pthIn, deltaT, options.MaxVelocity);
  ...
  for itr=1:outerIter
    ...
    if isAngleMoreThan90AllItr
        % Check whether any consecutive pose has more than 90 degree angle change.
        % IF yes, warn user to increase MaxPathStates.
        isAngleMoreThan90 = any(robotics.internal.wrapToPi(diff(pth(:,3))) > pi/2);
        isAngleMoreThan90AllItr = isAngleMoreThan90AllItr && isAngleMoreThan90;
    end
  ...
  coder.internal.warningIf(isAngleMoreThan90AllItr, 'nav:navalgs:optimizepath:AngleBetweenPosesLarge');
  ...
  if (nargout >= 3)
    ...
    solnInfo = struct('Cost',cost,'NumIteration',numIter, 'Err', err, ...
        'PoseObstacleDist', tebGraph.PoseObstacleDist);
  end
end
...
function [pth, deltaT] = perturbPath(pth, deltaT, velocity)
%perturbPath Apply small perturbation to the input path poses whose x, y
%location are near identical i.e., deltaT is close to zero

distThreshold = 1e-4;
deltaTThreshold = distThreshold/velocity;
if ~all(deltaT < deltaTThreshold)
    return
end

% We apply perturbation to x and y coordinates alternately. This makes
% sure the distance between poses is equal to distThreshold.
pth(1:2:end,1) = pth(1:2:end,1) + distThreshold;
pth(2:2:end,2) = pth(2:2:end,2) + distThreshold;

% Update deltaT
deltaT(:) = deltaTThreshold;
end
