...
%#codegen
    properties (Transient, Access = ?nav.algs.internal.InternalAccess)
        ...
        %ThemeListeners Listeners for updating colors when theme changes
        ThemeListeners
    end
    ...
    methods (Access = ?nav.algs.internal.DynamicCapsuleListBase)
        ...
        % graphic handles maintained by the list
            ...
            if numToDisplay > 0
                ...
                fColorBases = @()computeEgoColorBases(obj,axHandle);
                fColorEgoPatches = @(src,event)colorPatches(obj,obj.EgoPatchObj, numToDisplay, egoIndices, timeSteps, fColorBases, colorColliding{:});
                fColorEgoPatches([],[]);

                % React to theme change after plotting
                f = ancestor(axHandle,'figure');
                obj.ThemeListeners = [obj.ThemeListeners;addlistener(f,'Theme','PostSet',@(src,prop,event)fColorEgoPatches(src,prop))];
            end
        end
        ...
        % Clean up previously plotted obstacles
            ...
            if numToDisplay > 0
                % Color remaining patches
                fColorBases = @()computeObstacleColorBases(obj,axHandle);
                fColorObstaclePatches = @(src,event)colorPatches(obj,obj.ObstaclePatchObj, numToDisplay, obstacleIndices, timeSteps, fColorBases);
                fColorObstaclePatches([],[]);

                % React to theme change after plotting
                f = ancestor(axHandle,'figure');
                obj.ThemeListeners = [obj.ThemeListeners;addlistener(f,'Theme','PostSet',@(src,prop,event)fColorObstaclePatches(src,prop))];
            end
        end

        function colorBases = computeEgoColorBases(obj,ax)
            % Ego objects are varying shades of the first colorOrder
            colorBases = ax.ColorOrder(1,:).*linspace(1,.9,obj.NumEgos)';
        end

        function colorBases = computeObstacleColorBases(obj,ax)
            % Ignore first color, which is reserved for ego objects
            obsColors = ax.ColorOrder(2:end,:);
            ID = obj.ObstacleIDs;
            colorIdx = mod(ID-1,max(size(obsColors,1),1))+1;
            colorBases = obsColors(colorIdx,:);
        end
    ...
    methods (Access = protected)
        ...
        % Parse input struct, cache data and pre-allocate outputs
            ...
            if noOp == true
                ...
                for i = 1:obj.NumEgos
                    ...
                    if distanceOut
                        [collisionResult, dist] = robotics.core.internal.checkCollisionCapsuleWrapper(egoXYZ, egoUnitV, D1, R1, objXYZs, objUnitVs, D2, R2, false);
                    else
                        collisionResult = robotics.core.internal.checkCollisionCapsuleWrapper(egoXYZ, egoUnitV, D1, R1, objXYZs, objUnitVs, D2, R2, false);
                    end
        ...
        % the number of indices
            ...
            for i = 1:obj.NumEgos
                ...
                if invalidFound == 0
                    collisionFound(:,i) = double(any(robotics.core.internal.checkCollisionCapsuleWrapper(egoXYZ, egoUnitV, D1, R1, objXYZs, objUnitVs, D2, R2, false),2));
                else
                    collisionFound(validIndices,i) = double(any(robotics.core.internal.checkCollisionCapsuleWrapper(egoXYZ(:,validIndices), egoUnitV(:,validIndices), D1, R1, objXYZs, objUnitVs, D2, R2, false),2));
                end
            end
