...
%#codegen
    properties
        ...
        TieBreaker = 1
        % Flag to enable/disable diagonal movement
        DiagonalSearchFlag = 1;
    end
    ...
    properties (Access = private)
        ...
        PathCost
        % Number of points in found path
        NumPathPoints=0;

        % Indices of explored nodes during search.
        NodesExploredIndicesInternal
        % Grid points of explored nodes during search.
        NodesExploredInternal
    ...
    methods
        ...
        % function.
            ...
            obj.Col = col;
            obj.Zlim = zlim;
            obj.TotalElements = row*col;

            % Allocating maximum space to class properties
            obj.PathInternal = zeros(obj.TotalElements,2);
            obj.PathIndicesInternal = zeros(obj.TotalElements,1);
            obj.NodesExploredIndicesInternal = zeros(obj.TotalElements,1);
            obj.NodesExploredInternal =  zeros(obj.TotalElements,2);
        ...
        %plan Finds path between the start and goal using A* algorithm.
            ...
            % Asserting if rows/col are in accordance with map size.
            coder.internal.assert(row<=size(map,1),'nav:navalgs:plannerastargrid:AssertionFailedLessThan','Rows',size(map,1));
            coder.internal.assert(col<=size(map,2),'nav:navalgs:plannerastargrid:AssertionFailedLessThan','Cols',size(map,2));
            ...
            while (1) % Code will break when path found or when no path exist
                ...
                if OpenList.isEmpty()
                    ...
                    obj.ParentCol(startIn(1),startIn(2))=startIn(2);
                    obj.ParentRow(startIn(1),startIn(2))=startIn(1);
                    break;
                end
                ...
                if MinScoreNode(1)==obj.MapIndex(goalIn(1),goalIn(2))
                    ...
                    % and assigns to obj.Path and obj.PathIndices
                    reconstructPath(obj,CurrentRow,CurrentCol,startIn);
                    break;
                end
            ...
            obj.Start = startIn(1,1:2);
            obj.Goal = goalIn(1,1:2);

            % Using temporary variable to copy data to class property
            expNodes = find(ExploredNodes==1);
            obj.NumNodesExplored = size(expNodes,1);
            coder.internal.assert(obj.NumNodesExplored<=obj.TotalElements,...
                                  'nav:navalgs:plannerastargrid:AssertionFailedLessThan',...
                                  'NumNodesExplored',obj.TotalElements);
            % Copying values to class properties.
            obj.NodesExploredIndicesInternal(1:obj.NumNodesExplored,:) = expNodes;
            obj.NodesExploredInternal(1:obj.NumNodesExplored,:) = obj.AllNodes(obj.NodesExploredIndices,:);
        ...
        %path from current node to start.
            OptimalPath = coder.nullcopy(zeros(obj.TotalElements,2));
            pathIndices = coder.nullcopy(zeros(obj.TotalElements,1));
            k = 1;
            OptimalPath(1,:)=[CurrentRow CurrentCol];
            pathIndices(1)= obj.MapIndex(CurrentRow,CurrentCol);
            while (1)
                k=k+1;
                coder.internal.assert(k<=obj.TotalElements,...
                                      'nav:navalgs:plannerastargrid:AssertionFailedLessThan',...
                                      'PathPointCounter',obj.TotalElements);
            ...
            coder.internal.assert(k<=obj.TotalElements,...
                                  ...
                                  'PathPointCounter',obj.TotalElements);
            obj.NumPathPoints=k;
            obj.PathInternal(1:k,:) = flip(OptimalPath(1:k,:),1);
            obj.PathIndicesInternal(1:k,:) = flip(pathIndices(1:k,:),1);
        end

        % Getters
        function val = get.Path(obj)
            val = obj.PathInternal(1:obj.NumPathPoints,:);
        end
        function val = get.PathIndices(obj)
            val = obj.PathIndicesInternal(1:obj.NumPathPoints,:);
        end

        function val = get.NodesExplored(obj)
            val = obj.NodesExploredInternal(1:obj.NumNodesExplored,:);
        end
        function val = get.NodesExploredIndices(obj)
            val = obj.NodesExploredIndicesInternal(1:obj.NumNodesExplored,:);
        end

        function setHeuristicMethod(obj,distMethodVal)
        % setHeuristicMethod Sets the heuristic cost to builtin function.
            validateattributes(distMethodVal,{'double'},{'scalar'});
        ...
        % getHeuristicMethod Returns builtin cost function used.
            heuristicMethod = obj.HCostMethod;
        end

        function setGCostMethod(obj,distMethodVal)
        ...
        function setDiagonalSearchFlag(obj,val)
        % setDiagonalSearchFlag Sets the flag to enable/disable diagonal movement.
            obj.DiagonalSearchFlag = val;
        end
        function flag = getDiagonalSearchFlag(obj)
        % getDiagonalSearchFlag Returns the value of diagonal movement flag.
            flag =  obj.DiagonalSearchFlag;
        end
    ...
    methods(Access = private)
        ...
        function [Neighbors,NumNeighbors] = getNeighbors(obj)
        %Calling the neighbor function according to flag for diagonal
        %motion.
            switch(obj.DiagonalSearchFlag)
              case 0
                [Neighbors,NumNeighbors] = obj.getFourNeighbors();
              otherwise
                [Neighbors,NumNeighbors] = obj.getEightNeighbors();
            end
        end
    ...
    methods(Static)
        ...
        function [Neighbors,NumNeighbors] = getEightNeighbors()
        %getNeighbors Finds the neighboring cells which are from
        %ConnectingDistance away from the current cell including diagonal
        % movement.1 It assumes current grid cell to be [0,0].
        %
        % Following function will mark the neighbors in the grid in given
        % below patterns. Connection distance defines, how many grids it
        % can move from the current grid. '1' is for available positions
        % and '0' is for blocked positions.
        %
        ...
        %      0     1     0     1     0
        %
        % For current use case, ConnectingDistance is marked as 1, so only
        % 8 adjacent neighbors will be returned.
            ...
            % Marking grids blocked from line of sight as unavailable.
            ...
            % Marking current grid as unavailable, and extracting positions
            % of neighbors
        ...
        function [Neighbors,NumNeighbors] = getFourNeighbors()
        %getNeighbors Finds the neighboring cells which are from
        %ConnectingDistance away from the current cell in straight line.
        %It assumes current grid cell to be [0,0].
            ConnectingDistance = 1;
            NeighborCheck=zeros(2*ConnectingDistance+1);
            Mid=ConnectingDistance+1;

            % Marking straight line neighbors as available.
            NeighborCheck(Mid+ConnectingDistance,Mid)=1;
            NeighborCheck(Mid,Mid+ConnectingDistance)=1;
            NeighborCheck(Mid-ConnectingDistance,Mid)=1;
            NeighborCheck(Mid,Mid-ConnectingDistance)=1;
    ...
    methods (Static, Hidden)
        ...
        % the property should be made compile-time Constant.
            result = {'Row','Col','Zlim','TotalElements'};
        end
    end
end
% LocalWords:  UserDistanceFunctionReturnsNaNOrInf
