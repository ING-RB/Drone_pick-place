...
%   "Plus", specified either as a string scalar, string array, character 
...
function [numObst, orderedShapeIdx, mapSz, mapRes] = parseValidateArguments(...
    ...
    % 5. Warn users if specified number of shapes is more than specified
    % number of obstacles
    % tic
    mapShapeSz = numel(shapesBeforeValidation);
    if (mapShapeSz - numObst) == 1
        coder.internal.warning( ...
            'nav:navalgs:mapclutter:moreShapesThanObstacles', ...
            shapesBeforeValidation{mapShapeSz});
    elseif (mapShapeSz - numObst) > 1
        extraShapes = [shapesBeforeValidation{mapShapeSz-1}, ' and ', ...
                       shapesBeforeValidation{mapShapeSz}];
        coder.internal.warning( ...
            'nav:navalgs:mapclutter:moreShapesThanObstacles', ...
            extraShapes);
    ...
    % 6. If extraArgs is present then check if it's a valid shape. If it is
    ...
    % 7. Let the Name Value parser parse whatever in the args hasn't been
    %    parsed.
...
function uniqueShapeIds = parseValidateShapes(ipShapes, validShapes)
    ...
    % Check if shapes is a cell array of chars or strings or character array. 
    isShapesTypeInvalid = ~iscellstr(ipShapes) && ~isstring(ipShapes) && ~ischar(ipShapes);
    % If it is not then throw an error.
    coder.internal.errorIf(isShapesTypeInvalid,...
                           'nav:navalgs:mapclutter:invalidShapesType');
    % Convert string to char or char to char. Output will be cell array or
    % character array
    ipShapes = convertStringsToChars(ipShapes);
    if ischar(ipShapes)
        % If the input is a character array representing one shape, 
