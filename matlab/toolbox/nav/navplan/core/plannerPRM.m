...
%           % Samples states interpolated at distance 0.02 meters to draw curves.
%           states = interpolate(ss, ...
%                    nodes(edges(i,1),:), nodes(edges(i,2),:),0:0.02:1);
%           % Added [NaN, NaN] to the end to break line plot.
%           links = [links;states(:,1), states(:,2);NaN NaN];
%       end
%       plot(links(:,1),links(:,2),plannerLineSpec.tree{:});
%       plot(nodes(:,1), nodes(:,2), plannerLineSpec.state{:});
% 
...
%#codegen
    ...
    properties (Access = {?nav.algs.internal.InternalAccess})
        ...
        %AStar Handle to plannerAStar object
        AStar
    end
    ...
    methods
        ...
        %   as 1 if a path is found. Otherwise, it returns 0.
            ...
            if isempty(obj.Graph.Edges)|| height(graphNodes)<2
            ...
            [path] = obj.AStar.plan(start,goal);
        ...
        %   specified planner object.
            ...
            newObj      = plannerPRM(validator.StateSpace, validator, StateSampler=sampler);
        ...
        %   values to construct the new roadmap.
            ...
            while(1)
                ...
                if(obj.PregenerateSamples)
                    ...
                    states = obj.StateSampler.sample(maxNumNodes);
                else
                    states = nan(maxNumNodes,obj.StateSpace.NumStateVariables);
                    for nSample=1:maxNumNodes
                        states(nSample,:) = obj.StateSampler.sample();
                    end
                end
            ...
            % Create MATLAB digraph internal objects with column name, in
            % accordance to navGraph
            nodeTable = table(validStates, 'VariableNames', {'StateVector'});
            ...
            % Setting the graph object to create planner object.
            if isempty(obj.Graph.Edges) || height(obj.Graph.Nodes)<2 
            % When graph is empty or nodes are insufficient.

                % For backward compatibility, and supporting
                % code-generation, creating a dummy digraph object.
                state1 = obj.StateSpace.sampleUniform;
                state2 = obj.StateSpace.sampleUniform;
                nodeT = table([state1;state2],'VariableNames',{'StateVector'});
                edgeT = table([1 2], 'VariableNames', {'EndNodes'});
                graphObj = digraph(edgeT,nodeT);

            else
            % Using graph data with sampled nodes.
                graphObj = obj.Graph;
            end

            % Creates navGraph object
            ss = obj.StateSpace;
            navGraphObj = navGraph(graphObj,LinkWeightFcn=@(x,y)ss.distance(x,y));

            % Create plannerAStar object and generating lookup table.
            obj.AStar = plannerAStar(navGraphObj,HeuristicCostFcn=@(x,y)ss.distance(x,y));
        end
    ...
    methods (Access = private)
        ...
        %validateOptionalInput
            ...
            name = {'MaxNumNodes', 'MaxConnectionDistance', 'StateSampler'};

            % Create default value cell array
            default = {50, inf, stateSamplerUniform(obj.StateSpace)};
            ...
            stateSampler = parameterValue(parser, "StateSampler");
        ...
        % validateStateSampler Validates state sampler
            ...
            validateattributes(stateSampler, {'nav.StateSampler'}, {'scalar', 'nonempty'}, 'plannerPRM', 'StateSampler')

            % Validate StateSpace property of StateSampler
            if coder.target('MATLAB')
                if obj.StateSpace ~= stateSampler.StateSpace
                    coder.internal.error('nav:navalgs:plannerprm:StateSamplerRequireSameStateSpace');
                end
            end
        end
    end
    ...
    methods(Static, Hidden)
        function obj = loadobj(s)
            %loadobj Load a previously saved plannerPRM object              

            if isa(s, 'plannerPRM')
                obj = s;
                if isempty(s.StateSampler)
                    %StateSampler is introduced in R2023b
                    obj.StateSampler = stateSamplerUniform(ss);
                end
            else
                % Older MAT files may be resolved as struct
                obj = plannerPRM(s.StateValidator.StateSpace, s.StateValidator);                
                props = setdiff(fieldnames(s), {'StateSpace', 'StateValidator', 'NodesWeightsInternal'}); % NodesWeightsInternal is removed in R2023b
