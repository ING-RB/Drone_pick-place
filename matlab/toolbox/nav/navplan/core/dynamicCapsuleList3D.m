...
%   Copyright 2020-2023 The MathWorks, Inc.
    ...
    methods (Access = ?nav.algs.internal.DynamicCapsuleListBase)
        ...
        function colorPatches(obj, multiCapGroup, numToShow, objIndices, timeSteps, fColorBases, collisionList)
        %colorPatches Sets the color of each object set differently
            x = linspace(0,.9,obj.MaxNumSteps)';
            colorEnd  = ones(1,3)*.75;
            ...
            colorBases = fColorBases();
            collisionColor = nav.internal.LineSpecColor.rgb('Colliding');
                ...
                for i = 1:numToShow
                    ...
                    for j = numel(multiCapGroup.Children(transformIdx).Children):-1:1
                        if collisionList(j,transformIdx) == true
                            multiCapGroup.Children(transformIdx).Children(j).Children.Children.FaceColor = collisionColor;
                        else
