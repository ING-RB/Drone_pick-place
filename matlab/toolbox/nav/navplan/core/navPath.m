...
%
%   PATHOBJ = navPath(SPACE, STATES, MAXNUMSTATES) creates a path object
%   with the specified maximum number of states allowed in path
%   MAXNUMSTATES. The MAXNUMSTATES input also sets the value of the
%   MaxNumStates property.
%
%   navPath properties:
%      StateSpace        - State space for path
%      States            - Series of states for path
%      NumStates         - Number of state samples in path
%      MaxNumStates      - Maximum number of states allowed in path
%
%   navPath methods:
...
%#codegen
    ...
    properties (SetAccess = private)
        ...
        %MaxNumStates - Maximum number of states allowed in path
        %   Maximum number of states allowed in the path, specified as a
        %   positive scalar integer.
        %
        %   When specified as inf, the path is explicitly re sizeable. For
        %   code generation, DynamicMemoryAllocation must be set to 'On'.
        %
        %   When specified as a positive scalar integer, the maximum number
        %   of states in the object is limited to the specified value. Use
        %   this to create a re sizeable path when enabling DynamicMemoryAllocation
        %   is not allowed during code generation.
        %
        %   Default: inf
        MaxNumStates = inf
    end
    ...
    methods
        function obj = navPath(stateSpace, states, maxNumStates)
        %NAVPATH Create geometric path object
            ...
            switch nargin
              ...
              case 3
                ...
                if isempty(states)
                    validateattributes(states, {'numeric'}, {'size', ...
                                                             [0 obj.StateSpace.NumStateVariables]}, 'navPath', 'states');
                ...
                %NOTE: maxNumState input is required when dynamic memory is
                %disabled to specify and allocate upper bounds for
                %stateInternal variable.
                validateattributes(maxNumStates, {'numeric'},{'scalar', 'positive', 'nonnan'},'navPath','MaxNumStates');

                if size(states,1) > maxNumStates
                    coder.internal.error("nav:navalgs:navpath:NumStateValue", size(states,1),maxNumStates);
                end

                obj.MaxNumStates = maxNumStates;

                if ~coder.target("MATLAB") && ~coder.const(coder.internal.eml_option_eq('UseMalloc','VariableSizeArrays'))
                    %If dynamic memory allocation is disabled then preallocate
                    %of size NxM where N is maxNumStates and N is
                    %NumStateVariables.
                    obj.NumStates = size(statesInternal,1);
                    obj.StateInternal = zeros(maxNumStates, obj.StateSpace.NumStateVariables);
                    obj.StateInternal(1:obj.NumStates,:) = statesInternal;
                    return;
                end
            end
            ...
            obj.StateInternal = statesInternal;
            obj.NumStates = size(statesInternal,1);
        ...
        %      append(pathobj1, [1 1 pi/2; 2 2 pi])
            ...
            stateInternal = obj.StateSpace.enforceStateBounds(state);

            numStates = obj.NumStates + size(stateInternal,1);

            %Check memory overflow condition for DMA enabled
            if numStates > obj.MaxNumStates
                coder.internal.error("nav:navalgs:navpath:MaxNumStates", numStates, obj.MaxNumStates);
            end

            obj.StateInternal = [obj.StateInternal; stateInternal];
            obj.NumStates     = obj.NumStates + size(stateInternal,1);
        end
        ...
        %       plot(pathobj.States(:,1), pathobj.States(:,2), ".b")
            ...
            %Check memory overflow condition
            if numStates>obj.MaxNumStates
                coder.internal.error("nav:navalgs:navpath:MaxNumStates", numStates, obj.MaxNumStates);
            end
            ...
            elseif curNumStates == 2 && isequal(obj.States(1,:),obj.States(end,:))
                ...
                obj.StateInternal = obj.StateSpace.interpolate(obj.States(1,:), obj.States(end,:), interval);
                obj.NumStates = newNumStates;
            else
                ...
                obj.StateInternal = interpStates;
                obj.NumStates = newNumStates;
            end
        end
        ...
        %      pathobj2 = COPY(pathobj)
            ...
            if isempty(states)
                ...
                copyObj = navPath(copiedSpace, states, maxNumStates);
            end
        end
    end
    ...
    methods
        ...
        %get.States - Getter for States property
            states = obj.StateInternal(1:obj.NumStates,:);
        end
    end

    methods(Static)
        function obj = loadobj(s)
        %LOADOBJ Load saved navPath
            if(isstruct(s))
                if isfield(s,'MaxNumStates')
                    obj = navPath(s.StateSpace, s.States, s.MaxNumStates);
                elseif isfield(s,'States')
                    obj = navPath(s.StateSpace, s.States);
                else
                    obj = navPath(s.StateSpace);
                end
            elseif (isa(s,'navPath'))
                obj = s;
            end
        end
    end
