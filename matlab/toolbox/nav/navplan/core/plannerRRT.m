...
%
%   plannerRRT(STATESPACE, STATEVAL) creates a RRT planner
...
%
%   planner = plannerRRT(___,Name=Value) sets properties using one or
...
%      planner = plannerRRT(ss, sv, MaxIterations=2500);
...
%#codegen
    ...
    methods
        function obj = plannerRRT(stateSpace, stateValidator,varargin)
        ...
        %                 edge.
            ...
            for k = 1:obj.MaxIterations
                if obj.PregenerateSamples
                    ...
                    randState = obj.StateSampler.sample();
                end
    ...
    methods (Access = {?nav.algs.internal.InternalAccess})
        ...
        %extend The RRT "Extend" routine
            ...
            if obj.GoalReachedFcn(obj, newState, obj.CurrentGoalState)
        ...
        %   based on the properties of the specified OBJ.STATESPACE
            ...
            if isa(obj.StateSampler, 'stateSamplerMPNET')
                % We need to pre-generate samples for stateSamplerMPNET as
                % it needs to keep track of previous state while doing
                % sample generation
                obj.PregenerateSamples = true;
            end
        ...
        % validation for validatorOccupancyMap and CollisionBodyValidator
            ...
            if obj.PregenerateSamples
                ...
                obj.RandomSamplePool = obj.StateSampler.sample(obj.MaxIterations);
            else
    ...
    methods
        ...
        %set.StateSampler Setter for StateSampler
    ...
    methods(Access=private)
        function validateStateSampler(obj, stateSampler)
            % validateStateSampler Validates state sampler and also the
            % StateSpace property contained in it
