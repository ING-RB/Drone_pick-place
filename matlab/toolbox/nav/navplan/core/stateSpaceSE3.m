classdef stateSpaceSE3 < nav.StateSpace & ...
    ...
    methods
        ...
        %      states = interpolate(space, [2 10 3 0.2 0 0 0.8], [0 -2.5 4 0.7 0.3 0 0], [0:0.02:1])
            ...
            %Allocate memory
            interpState = zeros(numel(ratios),obj.NumStateVariables);

            % Calculate interpolated states
            colRatios = ratios(:);
            interpState(:,1:3) = repelem(state1(1:3), numel(colRatios),1) + colRatios*stateDiff;
            interpState(:,4:7) = compact(slerp(quaternion(state1(4:7)),quaternion(state2(4:7)),colRatios));
        end
