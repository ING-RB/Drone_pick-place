classdef WorkspaceImporterModel < handle & ...
    ...
    properties (Constant)
        %DefaultMaxLidarRange - Default maximum lidar range
        DefaultMaxLidarRange = 8

        %DefaultMinLidarRange - Default manimum lidar range
        DefaultMinLidarRange = 0.01
    end
    ...
    methods
        ...
        function [isSuccess, errorTag, errorArgs] = extractData(obj)
            ...
            upperBound = max(cellfun(@(x) max(nav.slamapp.internal.extractFiniteRanges(x)), obj.Scans));
            lowerBound = min(cellfun(@(x) min(nav.slamapp.internal.extractFiniteRanges(x)), obj.Scans));

            % use default bounds when the computed bounds are not valid
            if upperBound == lowerBound
                if upperBound > obj.DefaultMinLidarRange
                    % all the range values are finite, equal and greater
                    % than default minimum range then use default lower
                    % bound.
                    lowerBound = obj.DefaultMinLidarRange;
                else
                    % all the range values are either non-finite or less
                    % than default minimum lidar range then use default
                    % values for both upper and lower bounds.
                    upperBound = obj.DefaultMaxLidarRange;
                    lowerBound = obj.DefaultMinLidarRange;
                end
            end
