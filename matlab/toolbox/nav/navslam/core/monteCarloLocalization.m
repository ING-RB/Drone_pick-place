...
%   setParticles - Set particles and their weights
%
...
%#codegen
    ...
    methods
        ...
        %monteCarloLocalization Constructor
            ...
            obj.MCLObj = nav.algs.internal.MonteCarloLocalizationBuiltins;
        end
        ...
        %setParticles Set particles and weights.
        %   setParticles(OBJ, PARTICLES) allows you to set
        %   the particle values, where PARTICLES is an N-by-3 array
        %   of [x y yaw] poses. All particle weights of the particles will be
        %   initialized to 1/N.
        %
        %   setParticles(OBJ, PARTICLES, WEIGHTS) also allows you to
        %   set the particle weights. WEIGHTS is an N-element vector,
        %   where N is the number of particles.
        %
        %   See also getParticles.

            narginchk(2,3);

            if ~obj.isLocked
                if obj.UseLidarScan
                    obj.setup([ 0 0 0 ], lidarScan(0,0));
                else
                    obj.setup([ 0 0 0 ], 0, 0);
                end
                obj.reset;
            ...
            validateattributes(particles, {'double'}, ...
                               {'nonempty', 'ncols', 3}, 'setParticles', 'particles');
            numParticles = size(particles,1);
            numStoredParticles = nav.algs.internal.AccessMCL.getNumParticles(obj.MCLObj);

            coder.internal.errorIf(numParticles ~= numStoredParticles, ...
                                   'nav:navalgs:mcl:NumParticlesMismatch',numParticles, numStoredParticles);

            if nargin == 2
                % Initialize weights if not provided
                weights = ones(numParticles,1) * (1/numParticles);
            else
                validateattributes(weights, {'double'}, ...
                                   {'vector', 'numel', numParticles}, 'setParticles', 'weights');
            end

            obj.MCLObj.setParticles(particles, weights);
        end
    end
    ...
    methods (Access = private)
        ...
        %initialize Initialize by assigning all data to internal object
            obj.MCLObj.initialize(obj.Seed);
