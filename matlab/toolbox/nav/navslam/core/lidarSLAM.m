...
%#codegen
    ...
    methods
        ...
        %      lidarSlam.show
            ...
            if isa(currScan.Ranges, 'single')
                % Pre-allocated scans cell array assumes double scans
                currentScan = lidarScan(double(currScan.Ranges), double(currScan.Angles));
            else
            ...
            if (obj.NextScanId == 1)
                currGrid = obj.rasterizeScan(currentScan);
                obj.Scans{1} = currentScan; % and ignore the relative pose if it's provided
            ...
            if obj.UseCertifiedData
                ...
                if (obj.ScanRegistrationMethodInternal == nav.algs.internal.ScanRegistrationMethod.PhaseCorrelation)
                    ...
                    if obj.LastScanRegistrationMethod == nav.algs.internal.ScanRegistrationMethod.BranchAndBound
                        ...
                        relPoseImage = imregcorr(currGrid, Rgrid, lastGrid, Rgrid, 'Method', 'phasecorr');
                        ...
                        relPoseImage = imregcorr(currGrid, Rgrid, obj.LastGrid, Rgrid, 'Method', 'phasecorr');
                    end
                    ...
                    if obj.IdentityInformationMatrix && nargin < 3
                        ...
                        % necessary.
                        relPose = matchScansGrid(currentScan, obj.Scans{obj.NextScanId-1}, ...
                    ...
                    elseif obj.IdentityInformationMatrix && (nargin > 2)
                        ...
                        relPose = matchScansGrid(currentScan, obj.Scans{obj.NextScanId-1}, 'InitialPose', relPoseEstimate, ...
                            'MaxRange', obj.MaxLidarRange, 'Resolution', obj.MapResolution, ...
                    ...
                    elseif ~obj.IdentityInformationMatrix && nargin < 3
                        [relPose, stats] = matchScansGrid(currentScan, obj.Scans{obj.NextScanId-1}, ...
                            'MaxRange', obj.MaxLidarRange, 'Resolution', obj.MapResolution);
                    else
                        relPoseEstimate = robotics.internal.validation.validateMobilePose(relPoseEstimate(:), 'addScan', 'relPoseEstimate');
                        [relPose, stats] = matchScansGrid(currentScan, obj.Scans{obj.NextScanId-1}, 'InitialPose', relPoseEstimate, ...
                            'MaxRange', obj.MaxLidarRange, 'Resolution', obj.MapResolution, ...
                            'TranslationSearchRange', obj.SearchRangeWithGuess(1:2), ...
                            'RotationSearchRange', obj.SearchRangeWithGuess(3) );
                    end
                    
                    relPoseRefined = matchScans(currentScan,obj.Scans{obj.NextScanId-1}, 'initialPose', relPose);
            ...
            if obj.relativePoseAcceptable(relPose)
                isScanAccepted = true;
                loopClosureInfo = obj.acceptRelPoseAndSearchForLoopClosures(currentScan, relPose, infoMat);
            else
        ...
        %              Default: 'on'
            ...
            for i = 1:(obj.NextScanId-1)
                ...
                scPlot = plot(ax, scPoints(:,1), scPoints(:,2), '.', 'MarkerSize', 3);
                matlab.graphics.internal.themes.specifyThemePropertyMappings(scPlot, "Color", "--mw-graphics-colorOrder-7-secondary"); % pink
            end
            if displayPoses
                nds = obj.PoseGraph.nodeEstimates;
                pPlot = plot(ax, nds(:,1), nds(:,2), '.-', 'MarkerSize', 5);
                matlab.graphics.internal.themes.specifyThemePropertyMappings(pPlot, "Color", "--mw-graphics-colorSpace-rgb-blue"); % blue
            end
            hold(ax, 'off');
        end
    end
