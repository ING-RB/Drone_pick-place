function [gRot, scale, info] = estimateGravityRotationAndPoseScale(poses, gyroscopeReadings, accelerometerReadings, varargin)
%ESTIMATEGRAVITYROTATIONANDPOSESCALE Estimate gravity rotation and 
%   pose scale using accelerometer, gyroscope measurements and
%   factor graph optimization.
%
%   The estimateGravityRotationAndPoseScale function estimates the gravity
%   rotation and pose scale that helps in transforming input poses to the
%   local navigation reference frame of IMU using IMU measurements and
%   factor graph optimization. The gravity rotation transforms the gravity
%   vector from the local navigation reference frame of IMU to the pose
%   reference frame. The pose scale brings input poses to the metric scale,
...
%   The estimated rotation helps in transforming the input pose reference
%   frame to the local navigation reference frame of IMU.
%
%   Monocular camera sensor-based structure from motion (SfM) estimates
%   poses at an unknown scale different from metric measurements obtained
%   by an IMU. The accelerometer readings help estimate scale factor to
%   bring input poses to metric scale similar to IMU measurements.
%
%   Note that the estimated parameter accuracy depends on the quality of 
%   input pose estimates, specified IMU Noise parameters etc. Too small
%   value of estimated pose scale in the range of 1e-3 often represents
%   estimation failure. Observe the status flag in the info struct to
%   identify failure. The status flag stores the status of various
%   validations performed. Success status might not always guarantee
%   successful estimation. To be confident execute estimation multiple
%   times and observe the variance in the result along with validation
%   status. If the variance in consecutive estimations gives better
%   confidence in trusting the estimated parameters.
%
%   [GROT,SCALE,INFO] = estimateGravityRotationAndPoseScale(poses, ...
%                gyroscopeReadings, accelerometerReadings, Name=Value) 
%   estimates the rotation required to transform the gravity vector from
%   the local navigation reference frame of IMU (NED or ENU) to the input
%   pose reference frame. The function also estimates scale so that input
%   poses at an unknown scale can be converted to metric units similar to
%   those in the IMU measurements.
% 
...
%   Inputs:
%
%       poses - Poses estimated using monocular visual-odometry.
%
%          Poses are expected to be at an arbitrary scale. It is suggested 
%          to set the median depth of visual estimations to 1. This 
%          brings the poses close to metric scale of IMU measurements which
%          helps estimation.
%
...
% 
%      SCALE   - Scalar multiplier to scale the input poses, returned as
%                a numeric scalar.
%
%      Pose scale parameter is returned as a positive double scalar. While
%      using monocular camera sensor for pose estimation, the pose
%      estimates will be at a different scale than IMU measurements.
%      Multiply the translation component of input poses with the estimated
%      scale to bring them to the same metric scale as IMU measurements
%      (meters).
% 
...
%                 values provided.
%              FAILURE_BAD_SCALE_PREDICTION : Scale is below specified 
%                 threshold and prediction error is above 
%                 specified threshold.
%              FAILURE_BAD_SCALE_BIAS : Scale is below specified threshold 
%                 and bias variation from initial value is out of 
%                 expected bounds.
...
%                 threshold, prediction error is above specified threshold 
%                 and bias variation from initial value is out of 
%                 expected bounds.
...
%
%
%   estimateGravityRotationAndPoseScale Name-Value arguments:
%
...
%
%        ScaleThreshold - Scale threshold specified as a scalar double. 
%            Very low values convey that the estimation is unsuccessful 
%            and noise in IMU measurements may be dominating. 
%
%            Whenever the median depth of monocular visual-odometry 
%            estimated map points in input pose reference s 1 (meter) this 
%            parameter represents the lowest value median depth can take. 
%            For example scene median cannot be below 1 milli-meter (mm). 
%            Usually no landmark within 1mm of camera field-of-view can be 
%            observed within camera frame due to focal length issues. 
%
%            1e-3 (default) | double
%
...
%               0.1,2.3776,-0.7725,0.7071,0,0,0.7071];
%
...
%#codegen
    ...
    [gRot, scale, info] = nav.algs.internal.estimateGS(...
        true, 'estimateGravityRotationAndPoseScale',...
