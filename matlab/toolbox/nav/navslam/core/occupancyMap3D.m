classdef occupancyMap3D < handle
    ...
    properties (Constant, Access = {?occupancyMap3D, ?nav.algs.internal.occupancyMap3DBuiltins})
        %OccupiedThresholdDefault Default occupied threshold to consider location occupied
        %   This is needed to ensure the loading of the variable and
        %   validation of new incoming values are segregated.
        OccupiedThresholdDefault = 0.65

        %FreeThresholdDefault Default free threshold to consider location free
        %   This is needed to ensure the loading of the variable and
        %   validation of new incoming values are segregated.
        FreeThresholdDefault = 0.2
    end
    ...
    properties (Constant, Hidden)
        %DefaultViewAngle View angle when visualized
        DefaultViewAngle = [-50 50]
    end
    ...
    methods
        ...
        function map = occupancyMap3D(res, varargin)
            ...
            defaults = {occupancyMap3D.OccupiedThresholdDefault, ...
                occupancyMap3D.FreeThresholdDefault, [0.001 0.999]};
        ...
        function set.OccupiedThreshold(map, newOccupiedThreshold)
            ...
            map.Octree.setOccupiedThreshold(newOccupiedThreshold);
        end
        ...
        function set.FreeThreshold(map, newFreeThreshold)
            ...
            map.Octree.setFreeThreshold(newFreeThreshold);
        end

        function y = get.OccupiedThreshold(map)
            y = map.Octree.OccupiedThreshold();
        end

        function y = get.FreeThreshold(map)
            y = map.Octree.FreeThreshold();
        end
        ...
        function pOcc = getOccupancy(map, pos)
            ...
            xyz = map.validatePosition(pos, map, 'getOccupancy', 'xyz');
            pOcc = map.Octree.getOccupancy(xyz);
        end
        ...
        function tOcc = checkOccupancy(map, pos)
            ...
            xyz = map.validatePosition(pos, map, 'checkOccupancy', 'xyz');
            tOcc = map.Octree.checkOccupancy(xyz);
        end
        ...
        function newMap = copy(map)
            ...
            newMap.OccupiedThreshold = map.OccupiedThreshold;
            newMap.FreeThreshold = map.FreeThreshold;
        end
        ...
        function setOccupancy(map, pos, newOccupancy)
            ...
            % Validate values
            xyz = map.validatePosition(pos, map, 'setOccupancy', 'xyz');
            newOccupancy = map.validateOccupancyValues(newOccupancy, size(xyz, 1), 'setOccupancy', 'pOcc');

            for idx = 1:size(xyz, 1)
                map.Octree.setNodeValue(xyz(idx,:), newOccupancy(idx), false);
            end
        end
        ...
        function updateOccupancy(map, pos, newObservation)
            ...
            % Validate values
            xyz = map.validatePosition(pos, map, 'updateOccupancy', 'xyz');
            newObservation = map.validateOccupancyValues(newObservation, size(xyz, 1), 'updateOccupancy', 'obs');

            for idx = 1:size(xyz, 1)
                if islogical(newObservation(idx))
                    map.Octree.updateNodeBoolean(xyz(idx, :), newObservation(idx), false);
                else
                    map.Octree.updateNodeDouble(xyz(idx, :), newObservation(idx), false);
                end
            end
        end
        ...
        function insertPointCloud(map, pose, points, maxRange, invModel)
            %insertPointCloud Insert 3D points or pointCloud observation in map
            ...
            %
            %   insertPointCloud(___, INVMODEL) accepts an optional
            %   2-element vector, INVMODEL, dictating the occupancy update
            %   probabilities used for missed and hit cells.
            
            arguments
                map (1,1) {mustBeA(map,'occupancyMap3D')}
                pose (1,7) double {occupancyMap3D.validatePosition(pose,map,'insertPointCloud','sensorPose',7)}
                points {mustBeA(points,{'numeric','pointCloud'}),occupancyMap3D.validatePoints}
                maxRange (1,1) double {mustBePositive,mustBeFinite}
                invModel (1,2) double {mustBeInRange(invModel,0,1)} = [0.4 0.7];
            end
            ...
            if isa(points, 'pointCloud')
                ...
                pointsMat = reshape(points.Location, [], 3);
            else
                pointsMat = points;
            end
            map.Octree.insertPointCloud(pose, pointsMat, maxRange, invModel, false, false);
        end
        ...
        function [pts, isOccupied] = rayIntersection(map, sensorPose, directions, maxRange, ignoreUnknownCells)
            ...
            map.validatePosition(sensorPose, map, 'rayIntersection', 'sensorPose', 7);
            origin = double(sensorPose);
            validateattributes(directions, {'numeric'}, {'2d', 'nonempty'}, 'rayIntersection', 'directions');

            R = quat2rotm(double(origin(4:7)));
        ...
        function varargout = show(map, varargin)
            ...
            %                         created by show.
            %
            %       'SearchDepth'   - An integer-valued scalar in the range
            %                         [0, 16] which determines the resolution
            %                         at which the map is displayed. The
            %                         maximum depth is 16, corresponding to
            %                         voxels whose edge length is
            %                         1/map.Resolution. Each level above the
            %                         maximum depth doubles this minimum voxel
            %                         size.
            %
            %                         Default: 16
            ...
            % Defaults
            defaults.Parent = [];
            defaults.SearchDepth = 16;

            parser = inputParser();
            parser.addParameter('Parent', defaults.Parent, @robotics.internal.validation.validateAxesUIAxesHandle);
            parser.addParameter('SearchDepth', defaults.SearchDepth, @(x)validateattributes(x,{'numeric'},{'scalar','>=',0,'<=',16,'integer'},'show','SearchDepth'));
            parse(parser, varargin{:});

            % Ensure valid parent
            axHandle = parser.Results.Parent;
            isNewPlot = isempty(axHandle);
            if isNewPlot
                axHandle = newplot;
            end

            % Extract level of detail
            lod = parser.Results.SearchDepth;

            % Extract data
            visData = map.Octree.extractVisualizationData(lod);
            origin = visData(:,1:3);
            scale = visData(:,4);
            value = visData(:,5);

            % Extract populated world limits
            xyzLimits = map.computeVisualizationLimits();
            ...
            % Construct default faces/vertices
            [f,v] = map.voxelPatchData([0 0 0], 1);

            % Get threshold
            thresh = map.OccupiedThreshold;

            for i = 1:numLeaves
                if value(i) < -inf|| value(i) >= thresh
                    vertices(numVerticesPerNode*(j-1) + (1:numVerticesPerNode),:) = origin(i,:) + v*scale(i);
                    faces(numFacesPerNode*(j-1) + (1:numFacesPerNode),:) = f + numVerticesPerNode*(j-1);
            ...
            if ~isequal(axHandle.CameraViewAngleMode,'manual')
                % Set the 3-D view
                axHandle.View = map.DefaultViewAngle;
            end

            % Set limits
            if ~isequal(axHandle.XLimMode,'manual')
                axHandle.XLim = xyzLimits(1,:);
            end
            if ~isequal(axHandle.YLimMode,'manual')
                axHandle.YLim = xyzLimits(2,:);
            end
            if ~isequal(axHandle.ZLimMode,'manual')
                axHandle.ZLim = xyzLimits(3,:);
            end
    ...
    methods (Access = {?occupancyMap3D, ?nav.algs.internal.InternalAccess, ?nav.algs.internal.MapIO})
        ...
        function xyzLimits = computeVisualizationLimits(map)
            %computeVisualizationLimits Compute ax limits for show method
            ...
            xyzLimits = midWorldLimit + [-1 1]*deltaWorldLimit;
        end
        ...
        function occuMat = occupancyMatrix(map, varargin)
            %occupancyMatrix get the occupancyMatrix of a occupancy Map 3D as a logical 3D
            %matrix
            %Varargin allows to user to give custom MapLimits since 
            %occupancyMap3D does not have defined limits and can grow as 
            %points are added

            if nargin == 2
                worldLimits = varargin{1};
            else
                worldLimits = currentMapDimensions(map);
            end
            
            [gridSize, gridDim] = getGridInfo(map,worldLimits);

            [y,x,z] = ndgrid((flip(gridDim(1,1):1/map.Resolution:gridDim(1,2))),...
                (gridDim(2,1):1/map.Resolution:gridDim(2,2)),flip(gridDim(3,1):1/map.Resolution:gridDim(3,2)));
            occuMat = reshape(map.checkOccupancy([x(:), y(:), z(:)]),gridSize);
        end

        function gridId = world2grid(map,state,varargin)
            %world2grid convert world cordinates to grid cordinates
            %Varargin allows to user to give custom MapLimits since 
            %occupancyMap3D does not have defined limits and can grow as 
            %points are added
            if nargin == 3
            ...
            gridId = floor(([state(:,2) state(:,1) state(:,3)]- ...
                [worldLimits(2,1) worldLimits(1,1) worldLimits(3,1)])*map.Resolution)+1;
            
            gridSize = getGridInfo(map,worldLimits);
            gridId(:,[1,3]) = gridSize([1,3]) - gridId(:,[1,3]) + 1;
        end

        function worldXYZ = grid2world(map,gridId,varargin)
            %grid2world convert grid cordinates to world cordinates
            %Varargin allows to user to give custom MapLimits since 
            %occupancyMap3D does not have defined limits and can grow as 
            %points are added
            if nargin == 3
            ...
            gridId(:,[1,3]) = gridSize([1,3]) - gridId(:,[1,3]) + 1;
            worldXYZ = ([gridId(:,2) gridId(:,1) gridId(:,3)]-[1 1 1])/map.Resolution + worldLimits(:,1)'+...
                +(1/(2*map.Resolution));
        end

        function [gridSize, gridDim] = getGridInfo(map,worldLimits)
            %getGridInfo Return grid size and dimensions for the provided
            % map world limits
            
            gridDim = [worldLimits(2,:); worldLimits(1,:); worldLimits(3:end,:)]+1/(2*map.Resolution);
            gridSize = round((gridDim(:,2)-gridDim(:,1))*map.Resolution)'+1;
        end
    end
    ...
    methods (Static, Access = private)
        ...
        function xyz = validatePosition(pos, map, fcnName, argName, numelAllowed)
            arguments
                pos double {mustBeFinite,mustBeReal,mustBeNonempty}
                map
                fcnName
                argName = 'position';
                numelAllowed (1,1) {mustBeInteger, mustBePositive} = 3
            end
            %validatePosition Validate the position to be within world
            %limits

            % Validate the input format and type
            validateattributes(pos, {'numeric'}, {'2d', 'ncols', numelAllowed}, fcnName, argName);
            xyz = pos(:,1:3);
            xlimits = map.WorldLimits;
            ylimits = xlimits;
            zlimits = xlimits;
    ...
    methods (Static, Hidden)
        function map = loadobj(incomingMap)
            ...
            map.FreeThreshold = incomingMap.FreeThreshold;
            map.OccupiedThreshold = incomingMap.OccupiedThreshold;
        end
    end
