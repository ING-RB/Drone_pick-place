classdef occupancyMap3DBuildable < coder.ExternalDependency & handle
...
%% Methods inherited from coder.ExternalDependency
    methods (Static)
        ...
        %updateBuildInfo Add headers, libraries, and sources to the build info
            ...
            buildInfo.addSourcePaths({fullfile(matlabroot,'toolbox', ...
                                               'nav','navslam','builtins','libsrc','octomapcodegen','octomap')});
    ...
    methods
        function obj = occupancyMap3DBuildable(resolution)
            ...
            obj.Octomap = coder.opaquePtr('void', coder.internal.null);
        ...
        function delete(obj)
        %delete Ensure C++ octomap destructor is called on deletion
            coder.inline('always');
            coder.cinclude('octomapcodegen_api.hpp')

            coder.ceval('octomapCleanup_real64', obj.Octomap);
        end
        ...
        function thresh = getOccupiedThreshold(obj)
            coder.inline('always');
            coder.cinclude('octomapcodegen_api.hpp');

            % Call the C API
            thresh = 0;
            thresh = coder.ceval('octomapGetOccupancyProbThreshold', obj.Octomap);
        end

        function setOccupiedThreshold(obj, thresh)
            coder.inline('always');
            coder.cinclude('octomapcodegen_api.hpp');

            % Call the C API
            coder.ceval('octomapSetOccupancyProbThreshold', obj.Octomap, thresh);
        end

        function thresh = getFreeThreshold(obj)
            coder.inline('always');
            coder.cinclude('octomapcodegen_api.hpp');

            % Call the C API
            thresh = 0;
            thresh = coder.ceval('octomapGetFreeProbThreshold', obj.Octomap);
        end

        function thresh = setFreeThreshold(obj, thresh)
            coder.inline('always');
            coder.cinclude('octomapcodegen_api.hpp');

            % Call the C API
            coder.ceval('octomapSetFreeProbThreshold', obj.Octomap, thresh);
        end
        ...
        function occ = getOccupancy(obj, pos)
            ...
            coder.ceval('octomapGetOccupancy_real64', obj.Octomap, coder.ref(pos), uint32(size(pos,1)), coder.ref(occ));
        end
        ...
        function tOcc = checkOccupancy(obj, pos)
            ...
            % Call the C API
            tOcc = coder.nullcopy(zeros(size(pos,1),1));
            coder.ceval('octomapCheckOccupancy_real64', obj.Octomap, coder.ref(pos), uint32(size(pos,1)), coder.ref(tOcc));
        end
        ...
        function deserialization(obj, data)
            ...
            % Call the C API
            val = true;
            val = coder.ceval('octomapDeserialization_real64', obj.Octomap, coder.ref(data), uint32(numel(data)));
        ...
        function setNodeValue(obj, xyz, prob, lazyEval)
            ...
            % Call the C API
            coder.ceval('octomapSetNodeValue_real64', obj.Octomap, coder.ref(xyz), prob, lazyEval);
        end
        ...
        function updateNodeBoolean(obj, xyz, occupied, lazyEval)
            ...
            % Call the C API
            coder.ceval('octomapUpdateNodeBoolean_real64', obj.Octomap, coder.ref(xyz), occupied, lazyEval);
        end
        ...
        function updateNodeDouble(obj, xyz, probUpdate, lazyEval)
            ...
            % Call the C API
            coder.ceval('octomapUpdateNodeDouble_real64', obj.Octomap, coder.ref(xyz), probUpdate, lazyEval);
        end
        ...
        function insertPointCloud(obj, origin, points, maxRange, invModel, lazyEval, discretize)
            ...
            missProb = 0; hitProb = 0;
            missProb = coder.ceval('octomapGetProbMiss_real64',obj.Octomap);
            hitProb = coder.ceval('octomapGetProbHit_real64',obj.Octomap);
            coder.ceval('octomapSetProbMiss_real64',obj.Octomap,invModel(1));
            coder.ceval('octomapSetProbHit_real64',obj.Octomap,invModel(2));

            isInfInPointCloud = false;
            isInfInPointCloud = coder.ceval('octomapInsertPointCloud_real64', ...
                                            obj.Octomap, coder.ref(origin), coder.ref(points), uint32(nPoints), maxRange, lazyEval, discretize);
            coder.ceval('octomapSetProbMiss_real64',obj.Octomap,missProb);
            coder.ceval('octomapSetProbHit_real64',obj.Octomap,hitProb);
        ...
        function out =  getRayIntersection(obj, ptStart, ptDirections, occupiedThreshold, ignoreUnknownCells, maxRange)
            ...
            coder.ceval('octomapGetRayIntersection_real64', obj.Octomap, coder.ref(ptStart), coder.ref(ptDirections), uint32(nPtDirections), occupiedThreshold,...
