...
%#codegen
    ...
    methods
        ...
        function [nodePair, edgeId] = addPointLandmark(obj, varargin)
            ...
            if isNewLandmark
                ...
                obj.NodeDims(obj.NumNodes,1) = obj.PointLength;
                obj.NodeMap(obj.NumNodes,1) = obj.NodeMap(obj.NumNodes-1) + obj.NodeDims(obj.NumNodes-1);
                obj.IsLandmarkNode(obj.NumNodes,1) = true;
            end
        ...
        function [nodePair, edgeId] = addRelativePose(obj, varargin)
            ...
            if needNewPoseNode % a new node to be added for a new non-LC edge
                ...
                % meta data related to the new node
                obj.NodeDims(obj.NumNodes,1) = obj.PoseDeltaLength;
                obj.NodeMap(obj.NumNodes,1) = obj.NodeMap(obj.NumNodes-1) + obj.NodeDims(obj.NumNodes-1);
                obj.IsLandmarkNode(obj.NumNodes,1) = false;
    ...
    methods (Access = ?nav.algs.internal.InternalAccess)
        ...
        function plotGraph(obj, ax, idDisplayMode)
            ...
            for m = 1:size(landmarkEdgeNodePairs,1)
                ...
                hLandmarkEdges = obj.drawEdge(ax, hLandmarkEdges, n1, n2, '-');
                matlab.graphics.internal.themes.specifyThemePropertyMappings(hLandmarkEdges, "Color", "--mw-backgroundColor-secondary"); % very light gray
                hLandmarkNodes = obj.drawNode(ax, hLandmarkNodes, n2, 'd', 'MarkerSize', 3);
                matlab.graphics.internal.themes.specifyThemePropertyMappings(hLandmarkNodes, "Color", "--mw-graphics-colorSpace-rgb-green"); % dark green
            end            
            ...
            for k = 1:numel(trajIds)
                ...
                hTrajEdges = obj.drawEdge(ax, hTrajEdges, n1, n2, '-');
                matlab.graphics.internal.themes.specifyThemePropertyMappings(hTrajEdges, "Color", "--mw-graphics-colorSpace-rgb-blue"); % blue
                hTrajNodes = obj.drawEdge(ax, hTrajNodes, n1, n2, '.');
                matlab.graphics.internal.themes.specifyThemePropertyMappings(hTrajNodes, "Color", "--mw-graphics-colorSpace-rgb-blue"); % blue
            end
            ...
            if strcmp(idDisplayMode, 'all') || strcmp(idDisplayMode, 'nodes')
                nIds = (1:obj.NumNodes)';
                idPlot = obj.drawID(ax, nodeEstimates, num2str(nIds), 'FontWeight', 'bold');
                matlab.graphics.internal.themes.specifyThemePropertyMappings(idPlot, "Color", "--mw-color-primary"); % black
            end
            ...
            if strcmp(idDisplayMode, 'all')
                ...
                for i = 1:numel(trajIds)
                    ...
                    idPlot = obj.drawID(ax, 0.5*(n1+n2), num2str(trajIds(i)));
                    matlab.graphics.internal.themes.specifyThemePropertyMappings(idPlot, "Color", "--mw-graphics-colorSpace-rgb-blue"); % blue
                end
            end
            ...
            if ~isempty(lcIds)
                ...
                for i = 1:numel(lcIds)
                    ...
                    hLoops = obj.drawEdge(ax, hLoops, n1, n2);
                    matlab.graphics.internal.themes.specifyThemePropertyMappings(hLoops, "Color", "--mw-graphics-colorSpace-rgb-red"); % red

                    if ~strcmp(idDisplayMode, 'off')
                        idPlot = obj.drawID(ax, 0.5*(n1+n2), num2str(lcIds(i)));
                        matlab.graphics.internal.themes.specifyThemePropertyMappings(idPlot, "Color", "--mw-graphics-colorSpace-rgb-red"); % red
                    end
                    if strcmp(idDisplayMode, 'loopclosures')
                        idPlot1 = obj.drawID(ax, n1, num2str(lcEdges(i,1)), 'FontWeight', 'bold');
                        idPlot2 = obj.drawID(ax, n2, num2str(lcEdges(i,2)), 'FontWeight', 'bold');
                        matlab.graphics.internal.themes.specifyThemePropertyMappings(idPlot1, "Color", "--mw-color-primary"); % black
                        matlab.graphics.internal.themes.specifyThemePropertyMappings(idPlot2, "Color", "--mw-color-primary"); % black
                    end
                end
            end
            hold(ax, 'off');
    ...
    methods (Access = ?nav.algs.internal.InternalAccess)
        ...
        function batchFillPoseGraph(obj, nodeMatrix, edgeMeasurmentMarix, ...
                edgeInfoMatrix, edgePairMatrix, loopEdgeIds)
            %batchFillPoseGraph fills the pose graph with specified
            %   information. Note that this method will clear all the existing
            %   data from pose graph and adds newly specified data to it.
            %   This method expects all the nodes to be pose only.
            %
            %   nodeMatrix - node matrix containing vertically concatenated
            %                Tform node poses of size [NxTformSize(1),TformSize(2)
            %   edgeMeasurementMatrix - edge measurement matrix containing
            %                           vertically concatenated Tform relative
            %                           poses of size [NxTformSize(1),TformSize(2)
            %   edgeInfoMatrix - edge information matrix containing
            %                    vertically concatenated pose information
            %                    matrices of size [NxPoseInfoMatSize(1),PoseInfoMatSize(2)
            %   edgePairMatrix - edge pair matrix of size Nx2
            %   loopEdgeIds - position ids of loop edge pairs in edge pairs of size 1xM

            % nodeMatrix and edgeMeasurementMatrix store vertically
            % concatenated pose transformation matrices similar to BlockMatrix
            numNodes = size(nodeMatrix,1)/obj.TformSize(1);
            numEdges = size(edgeMeasurmentMarix,1)/obj.TformSize(1);

            % These parameters are only updated during simulation mode
            if coder.target('MATLAB')
                obj.MaxNumEdges = numEdges;
                obj.MaxNumNodes = numNodes;
            end

            obj.NumEdges = numEdges;
            obj.NumNodes = numNodes;
            obj.NumLoopClosureEdges = length(loopEdgeIds);

            % Fill node estimates block matrix
            obj.NodeEstimates = robotics.core.internal.BlockMatrix(obj.MaxNumNodes, 1, obj.TformSize);
            obj.NodeEstimates.Matrix(1:size(nodeMatrix,1),1:obj.TformSize(2)) = nodeMatrix(:,1:obj.TformSize(2));
            
            obj.NodeMap = zeros(obj.MaxNumNodes,1);
            obj.NodeDims = zeros(obj.MaxNumNodes,1);
            obj.NodeMap(1:numNodes) = 1:obj.PoseDeltaLength:((numNodes-1)*obj.PoseDeltaLength + 1);
            obj.NodeDims(1:numNodes) = obj.PoseDeltaLength;
            
            % All the nodes are expected to be pose nodes
            obj.IsLandmarkNode = false(obj.MaxNumNodes,1);
            
            loopEdgePairMatrix = edgePairMatrix(loopEdgeIds,:);
            % Store edge node pair info
            if coder.target('MATLAB')
                obj.EdgeNodePairs = edgePairMatrix(:,1:2);
                obj.LoopClosureEdgeNodePairs = loopEdgePairMatrix(:,1:2);
                obj.LoopClosureEdgeIDsInternal = loopEdgeIds(1,1:size(loopEdgePairMatrix,1));
            else
                obj.EdgeNodePairs = zeros(obj.MaxNumEdges, 2);
                obj.LoopClosureEdgeNodePairs = zeros(obj.MaxNumEdges, 2);
                obj.EdgeNodePairs(1:size(edgePairMatrix,1),1:2) = edgePairMatrix(:,1:2);
                obj.LoopClosureEdgeNodePairs(1:size(loopEdgePairMatrix,1),1:2) = loopEdgePairMatrix(:,1:2);
                obj.LoopClosureEdgeIDsInternal = zeros(1, obj.MaxNumEdges);
                obj.LoopClosureEdgeIDsInternal(1:size(loopEdgePairMatrix,1)) = loopEdgeIds(1:size(loopEdgePairMatrix,1));
            end

            % Store edge measurments in block matrix 
            obj.EdgeMeasurements = robotics.core.internal.BlockMatrix(obj.MaxNumEdges, 1, obj.TformSize);
            obj.EdgeMeasurements.Matrix(1:size(edgeMeasurmentMarix,1),1:obj.TformSize(2)) = edgeMeasurmentMarix(:,1:obj.TformSize(2));

            % Store edge information matrices in block matrix
            obj.EdgeInfoMatrices = robotics.core.internal.BlockMatrix(obj.MaxNumEdges, 1, obj.PoseInfoMatSize);
            obj.EdgeInfoMatrices.Matrix(1:size(edgeInfoMatrix,1),1:obj.PoseInfoMatSize(2)) = edgeInfoMatrix(:,1:obj.PoseInfoMatSize(2));
        end
    end
