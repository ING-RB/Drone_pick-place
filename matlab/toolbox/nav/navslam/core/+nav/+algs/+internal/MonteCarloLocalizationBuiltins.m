...
%#codegen
    ...
    methods
        function obj = MonteCarloLocalizationBuiltins
        %MonteCarloLocalizationBuiltins Constructor
            if coder.target('MATLAB')
                % Create MCOS class in MATLAB
                obj.MCOSObj = nav.algs.internal.MonteCarloLocalization;
            else
                obj.MCLData = coder.opaquePtr("void","NULL");
            end
        end

        function initialize(obj, randomSeed)
            if coder.target('MATLAB')
                % Initialize MCOS class in MATLAB
                obj.MCOSObj.initialize(randomSeed);
            else
    ...
    methods
        ...
        function numParticles = getNumParticles(obj)
            if coder.target('MATLAB')
                % Call MCOS method in MATLAB
                numParticles = obj.MCOSObj.getNumParticles();
            else
                % Generate code through external dependency
                numParticles = nav.algs.internal.coder.MonteCarloLocalizationBuildable.getNumParticles(obj.MCLData);
            end
        end
