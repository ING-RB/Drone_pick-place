...
namespace nav {
    ...
    octomapcodegen::octomapcodegen(real64_T resolution) : _map(new octomap::OcTree(resolution)), occ_free_thres{ 0.2 } {
        _map->setOccupancyThres(0.65);
    }
    ...
    real64_T octomapcodegen::octomapGetOccupiedThreshold() {
        return _map->getOccupancyThres();
    }

    void octomapcodegen::octomapSetOccupiedThreshold(real64_T probability) {
        _map->setOccupancyThres(probability);
    }

    real64_T octomapcodegen::octomapGetFreeThreshold()
    {
        return occ_free_thres;
    }

    void octomapcodegen::octomapSetFreeThreshold(real64_T probability)
    {
        occ_free_thres = probability;
    }
    ...
    void octomapcodegen::octomapCheckOccupancy(real64_T* xyz, uint32_T nRows, real64_T* ternaryValues) {
        // Retrieve occupancy/free threshold values
        real32_T occThresh  = _map->getOccupancyThresLog();
        real32_T freeThresh = octomap::logodds(occ_free_thres);
        real32_T logoddsVal;

        // For each row find the occupancy value and check whether the node is free, occupied, or unknown
        for (uint32_T i = 0; i < nRows; ++i) {
            // Since MATLAB stores matrices in column-major, data elements in a row are separated
            // by "rows" elements
            octomap::OcTreeNode* node{_map->search(xyz[i], xyz[nRows + i], xyz[2 * nRows + i])};
            if (node != nullptr) {
                logoddsVal = node->getLogOdds();
                
            } else {
                logoddsVal = 0.0;
            }
            ternaryValues[i] = (logoddsVal >= occThresh) ? 1.0 : (logoddsVal <= freeThresh) ? 0.0 : -1.0;
        }
    }
    ...
    void octomapcodegen::octomapSetProbHit(real64_T probHit) {
        _map->setProbHit(probHit);
    }
    
    void octomapcodegen::octomapSetProbMiss(real64_T probMiss) {
        _map->setProbMiss(probMiss);
    }
