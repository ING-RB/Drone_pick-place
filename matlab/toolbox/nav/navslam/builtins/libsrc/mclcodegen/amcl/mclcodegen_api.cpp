...
EXTERN_C MCL_CODEGEN_API void mclSetParticles_real64(void* mclObj,
    ...
    int count(set->sample_count);
    for (int i = 0; i < count; i++) {
        set->samples[i].pose.v[0] = *(particles + i);
        set->samples[i].pose.v[1] = *(particles + i + count*1);
        set->samples[i].pose.v[2] = *(particles + i + count*2);
