function [posECEF, gnssVelECEF, dopMatrix, clkBias, clkDrift] ...
    = computeLocation(p, pdot, satPos, satVel, initPosECEF, initVelECEF)
...
if (numel(p) < 4)
    posECEF = NaN(1, 3, 'like', p);
    gnssVelECEF = NaN(1, 3, 'like', p);
    dopMatrix = NaN(4, 4, 'like', p);
    clkBias = NaN(1, 1, 'like', p);
    clkDrift = NaN(1, 1, 'like', p);
    return;
end
...
posECEF = posEst(1:3).';
clkBias = posEst(end) / c;

gnssVelECEF = velEst(1:3).';
clkDrift = velEst(end) / c;
