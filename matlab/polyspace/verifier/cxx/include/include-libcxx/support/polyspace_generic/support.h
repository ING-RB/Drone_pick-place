...
#ifndef _LIBCPP_SUPPORT_POLYSPACE_GENERIC_SUPPORT_H
#define _LIBCPP_SUPPORT_POLYSPACE_GENERIC_SUPPORT_H
...
#if defined(_LIBCPP_COMPILER_GENERIC_POLYSPACE)
#define _LIBCPP_STATIC_INLINE static inline

_LIBCPP_STATIC_INLINE int __builtin_popcount(unsigned int x)
{
  // Binary: 0101...
...
_LIBCPP_STATIC_INLINE int __builtin_popcountl(unsigned long x)
{
  return __builtin_popcount(static_cast<int>(x));
}

_LIBCPP_STATIC_INLINE int __builtin_popcountll(unsigned long long x)
{
  // Binary: 0101...
...
_LIBCPP_STATIC_INLINE int __builtin_ctzll(unsigned long long mask)
{
  unsigned long where;
...
_LIBCPP_STATIC_INLINE int __builtin_ctzl(unsigned long mask)
{
  unsigned long where;
  // Search from LSB to MSB for first set bit.
  // Returns zero if no set bit is found.
...
_LIBCPP_STATIC_INLINE int __builtin_ctz(unsigned int mask)
{
...
// bit position. If x is 0, the result is undefined.
_LIBCPP_STATIC_INLINE int __builtin_clzll(unsigned long long mask)
{
  unsigned long where;
...
_LIBCPP_STATIC_INLINE int __builtin_clzl(unsigned long mask)
{
  unsigned long where;
  // Search from LSB to MSB for first set bit.
  // Returns zero if no set bit is found.
...
_LIBCPP_STATIC_INLINE int __builtin_clz(unsigned int x)
{
  return __builtin_clzl(x);
}
#endif // _LIBCPP_MSVC

#endif // _LIBCPP_SUPPORT_POLYSPACE_GENERIC_SUPPORT_H
