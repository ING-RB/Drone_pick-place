function checkInteraction(interactionFile, contentFolder, logFolder)
	...
	if isfield(interactionData.matlabInteraction,'initializationCode')
    ...
    for i = 1:numel(questions)
        correctnessAssessment = strjoin(questions{i}.correctnessAssessment,newline);
        task = struct;
	...
	try
		if iscell(initCode)        
			evalin('base',strjoin(initCode,newline))
		end
	catch MExc
		disp(MExc)
        doLog(['Initialization code failed: ' fullfile(d(i).folder,d(i).name)], logFolder);
...
function addFiles(filesToAdd, folder, contentFolder, lessonpath)
    for i = 1:numel(filesToAdd)
        ...
        if isfield(fileToAdd,'contents')
            %Name and contents (embedded file)
            filename = fullfile(folder, fileToAdd.name);
            contents = strjoin(fileToAdd.contents,newline);
            fid = fopen(filename,'wt');
            fprintf(fid,'%s',contents);
            fclose(fid);
        else
            %Non-embedded file
            if isfield(fileToAdd,'destPath')
                %srcFile and destPath (copy to folder\destPath\srcFile)
                srcFile = fullfile(contentFolder, strrep(fileToAdd.srcFile, '${lessonpath}', lessonpath));
                [~,srcFileName,ext] = fileparts(srcFile);
                destPath = fullfile(folder,fileToAdd.destPath);
                if ~exist(destPath,'dir')
                    mkdir(destPath);
                end
                filename = fullfile(folder, fileToAdd.destPath, fileToAdd.name);
            else
                %Copy to folder\srcFile
