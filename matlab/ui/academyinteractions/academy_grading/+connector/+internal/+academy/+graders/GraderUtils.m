classdef GraderUtils
    ...
    methods(Static)
        ...
        function createFoldersAndAddToPath
            ...
            for c = {GraderUtils.testFolder, GraderUtils.gradingFolder}
                ...
                try
                    ...
                    % Catch the error, but nothing to handle
                    % Maybe checking using exist(f, 'dir')
                    % == 7 is better, but I dont know if that causes other
                    % complications.
                end
                mkdir(f);
                addpath(f,'-end');
        ...
        function currentWorkspace = getCurrentWorkspace
            currentWorkspace = struct();
            baseWorkspaceVars = evalin('base', 'whos');
            % Store variables in current workspace in a struct
            for i = 1:numel(baseWorkspaceVars)
                name = baseWorkspaceVars(i).name;
                value = evalin('base', name);
                % Store variables if the name is not referenceVariables as
                % referenceVariables is used for grading
                if ~strcmp(name, 'referenceVariables')
                    currentWorkspace.(name) = value;
                end
            end
        end
        ...
        function bringReferenceVariablesIntoBaseWorkspace
            referenceVariables = struct();
            baseWorkspaceVars = evalin('base', 'whos');
            % Store variables in current workspace in a struct and delete them
            for i = 1:numel(baseWorkspaceVars)
                name = baseWorkspaceVars(i).name;
                value = evalin('base', name);
            ...
            assignin('base', 'referenceVariables', referenceVariables);
        end

        function restoreBaseWorkspace(currentWorkspace)
            fields = fieldnames(currentWorkspace);
        ...
        function runLiveScript(editorId, fileName)
            import matlab.internal.liveeditor.LiveEditorUtilities;
            LiveEditorUtilities.execute(editorId, fileName, true);          
        end

        function executeScriptInBackground(fileName, storeAsReferenceVariables)
            import matlab.internal.liveeditor.LiveEditorUtilities;
            import connector.internal.academy.graders.GraderUtils;
            if nargin < 2
                storeAsReferenceVariables = false;
            end
            if storeAsReferenceVariables
                % if we want to store as reference variables,
                % first store the current workspace and clear the workspace
                % before running the solution script
                currentWorkspace = GraderUtils.getCurrentWorkspace;
                evalin('base', 'clear');
            end
            isLiveScript = contains(fileName, '.mlx');
            % Run the solution file
            if isLiveScript
                LiveEditorUtilities.openAndExecute(fileName);
            else
                evalin('base', fileName);
            end
            % Push the referenceVariables and currentWorkspace back to base workspace
            % and restore the baseWorkspace
            if storeAsReferenceVariables
                GraderUtils.bringReferenceVariablesIntoBaseWorkspace();
                GraderUtils.restoreBaseWorkspace(currentWorkspace);
            end
        end
