...
if [ "$#" -ne "$numExpectedArgs" ]; then
    echo "matlab-glselector error: Expected $numExpectedArgs args, received $#"
    exit 5
fi
...
case "$ARCH" in
    ...
    *)
        ...
        # warning messages
        if [ "$nodisplay" = "0" -a "$noopengl" = "0" ] ; then
            ...
            if [ "$usemesa" = "0" -o "$nousemesa" = "1" ]; then
                ...
                test -e /usr/bin/ldd &&  ldd --version |  grep -q "(GNU libc) 2\.17"  \
                    && export LD_PRELOAD=:"${LD_PRELOAD:+${LD_PRELOAD}:}$MATLAB/bin/$ARCH/glibc-2.17_shim.so"


                $MATLAB/bin/$ARCH/need_softwareopengl $display > /dev/null 2>&1
                need_softwareopengl=$?;
